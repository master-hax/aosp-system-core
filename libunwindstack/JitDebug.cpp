 ...
 public:
  ...
  Symfile* GetCached(uint64_t pc, bool* race) {
    ...
    for (auto& it : entries_) {
      ...
      if (ContainsPc(symfile, pc)) {
        if (!this->check_cached_entries) {
          return symfile;
        }
  ...
  bool ContainsPc(Elf* symfile, uint64_t pc) {
    uint64_t addr, size;
    return symfile->GetTextRange(&addr, &size) && (addr <= pc) && (pc < addr + size);
  }
  ...
  bool ContainsPc(DexFile*, uint64_t) {
    // Delegate the range checking to the GetFunctionName method.
    // TODO: Implement this method to speed up lookups.
    return true;
  }
