...
Symfile* JitDebugImpl<Symfile, PointerT, Uint64_T>::Get(Maps* maps, uint64_t pc) {
  ...
  if (descriptor_addr_ == 0) {
    FindAndReadVariable(maps, Symfile::kJitDebugDescriptorName);
  }
  ...
  for (auto& entry : entries_) {
    // Find entry that contains symbols for the given PC.
    // WARNING: There can be multiple matching entries in ART due to GC.
    //          We should use the first (most recent) entry.
    if (entry.addr_ <= pc && (pc - entry.addr_) < entry.size_) {
...
template <>
bool JitCacheEntry<DexFile>::Init(Maps* maps, JitMemory* memory, uint64_t addr, uint64_t size) {
