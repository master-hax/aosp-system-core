...
struct JitCacheEntry {
  ...
  bool Init(JitDebugMemory* safe_memory, uint64_t addr, uint64_t size);
};
...
class JitDebugImpl : public JitDebug<Symfile> {
 public:
  ...
  bool Update(Maps* maps) override;
  Symfile* Get(uint64_t pc) override;

  void Init(Maps* maps);
  bool ReadNewEntries(JitDebugMemory* safe_memory);
...
template <typename Symfile, typename PointerT, typename SymSizeT>
bool JitDebugImpl<Symfile, PointerT, SymSizeT>::Update(Maps* maps) {
  ...
  for (int i = 0; i < kMaxRaceRetries; i++) {
    ...
    safe_memory.seqlock_addr_ = descriptor_addr_ + offsetof(JITDescriptor, action_seqlock);
    if (!this->memory_->Read32(safe_memory.seqlock_addr_, &safe_memory.expected_seqlock_)) {
      return false;
    }
    std::atomic_thread_fence(std::memory_order_acquire);

    // Add all new entries to our cache.
    if (!ReadNewEntries(&safe_memory)) {
...
template <typename Symfile, typename PointerT, typename Uint64_T>
Symfile* JitDebugImpl<Symfile, PointerT, Uint64_T>::Get(uint64_t pc) {
  std::lock_guard<std::mutex> guard(lock_);
...
template <typename Symfile>
bool JitCacheEntry<Symfile>::Init(JitDebugMemory* safe_memory, uint64_t addr, uint64_t size) {
  ...
  uint64_t pc_size = 0;
  symfile_->GetValidPcRange(&pc_start_, &pc_size);
  pc_end_ = pc_start_ + pc_size;
  return true;
}

template <typename Symfile, typename PointerT, typename Uint64_T>
bool JitDebugImpl<Symfile, PointerT, Uint64_T>::ReadNewEntries(JitDebugMemory* safe_memory) {
  std::unordered_set<uint64_t> seen_entry_addr;
  std::vector<std::unique_ptr<JitCacheEntry<Symfile>>> new_entries;
  ...
  for (uint64_t entry_addr = desc.first_entry; entry_addr != 0; entry_addr = entry.next) {
    ...
    new_entries.emplace_back(new JitCacheEntry<Symfile>());
    if (!new_entries.back()->Init(safe_memory, entry.symfile_addr, entry.symfile_size.value)) {
      return false;
    }
