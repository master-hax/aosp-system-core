...
class JitDebugImpl : public JitDebug {
  ...
  // Cached entries indexed by the *end* address of the valid PC range.
...
std::unique_ptr<JitDebug> JitDebug::Create(ArchEnum arch, std::shared_ptr<Memory>& memory,
  ...
  typedef uint64_t uint64_aligned4 __attribute__((aligned(4)));
  typedef uint64_t uint64_aligned8 __attribute__((aligned(8)));
  std::unique_ptr<JitDebug> jit;
  switch (arch) {
    case ARCH_X86:
      jit.reset(new JitDebugImpl<uint32_t, uint64_aligned4>());
      break;
    case ARCH_ARM:
    case ARCH_MIPS:
      jit.reset(new JitDebugImpl<uint32_t, uint64_aligned8>());
      break;
    case ARCH_ARM64:
    case ARCH_X86_64:
    case ARCH_MIPS64:
      jit.reset(new JitDebugImpl<uint64_t, uint64_t>());
      break;
    default:
      abort();
  }
...
bool JitDebugImpl<PointerT, SymSizeT>::ReadEntries(bool* race) {
  ...
  for (uint64_t entry_addr = desc.first_entry; entry_addr != 0; entry_addr = entry.next) {
    ...
    if (entry.register_timestamp <= max_timestamp) {
      break;
    }

    // Make a copy of the in-memory ELF file (while checking for data races).
    std::unique_ptr<MemoryBuffer> buffer(new MemoryBuffer());
    buffer->Resize(entry.symfile_size);
    if (!SafeRead(entry.symfile_addr, buffer->GetPtr(0), buffer->Size(), race, counter)) {
      return false;
    }

    new_entries.push_back(std::unique_ptr<CacheEntry>(
        new CacheEntry{.timestamp_ = entry.register_timestamp, .buffer_ = std::move(buffer)}));
  }

  // Load and validate the ELF files (outside the critical loop).
  for (auto& it : new_entries) {
    it->elf_.reset(new Elf(it->buffer_.get()));
    it->elf_->Init(true);
    if (!it->elf_->valid()) {
      return false;
    }
    if (!it->elf_->GetValidPcRange(&it->text_addr_, &it->text_size_)) {
      return false;
    }
  }

  // Save the results. We need to iterate in reverse (chronological) order.
  for (auto rit = new_entries.rbegin(); rit != new_entries.rend(); ++rit) {
    CacheEntry* it = rit->get();
    ...
    entries_.emplace(it->text_addr_ + it->text_size_, it);
