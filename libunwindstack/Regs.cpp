...
uint64_t RegsTmpl<AddressType>::GetRelPc(Elf* elf, const MapInfo* map_info) {
  ...
  size_t name_size = map_info->name.size();
  if (name_size >= 4) {
    if (map_info->name[name_size - 4] == '.' && map_info->name[name_size - 3] == 'o'
        && map_info->name[name_size - 2] == 'a' && map_info->name[name_size - 1] == 't') {
      return pc_ - map_info->start + load_bias + map_info->offset;
    }
  }

  return pc_ - map_info->start + load_bias;
}
