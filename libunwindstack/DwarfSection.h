...
class DwarfSectionTmpl : public DwarfSection {
 public:
  ...
  bool Eval(const DwarfCIE* cie, Memory* regular_memory, const dwarf_loc_regs_t& loc_regs, Regs* regs) override {
    ...
    case DWARF_LOCATION_REGISTER:
      cfa = cur_regs->value(loc->values[0]) + loc->values[1];
      break;
    ...
    for (const auto& entry : loc_regs) {
      ...
      switch (loc->type) {
      case DWARF_LOCATION_OFFSET:
        if (!regular_memory->Read(cfa + loc->values[0], cur_regs->addr(reg), sizeof(AddressType))) {
          return false;
        }
        break;
      case DWARF_LOCATION_VAL_OFFSET:
        cur_regs->set(reg, cfa + loc->values[0]);
        break;
      case DWARF_LOCATION_REGISTER:
        cur_regs->set(reg, cur_regs->value(loc->values[0]) + loc->values[1]);
        break;
      case DWARF_LOCATION_EXPRESSION:
      case DWARF_LOCATION_VAL_EXPRESSION:
      {
        ...
        if (loc->type == DWARF_LOCATION_EXPRESSION) {
          if (!regular_memory->Read(value, cur_regs->addr(reg), sizeof(AddressType))) {
            return false;
          }
        } else {
          cur_regs->set(reg, value);
        }
        break;
      }
      case DWARF_LOCATION_SAME:
