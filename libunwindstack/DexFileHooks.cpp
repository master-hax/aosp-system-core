...
void SetDexFileHooks(const DexFileHooks* dex_file_hooks) {
  ...
  // DexFileImpl objects.
  CHECK(g_dex_file_hooks.DexFileFromMemory == DexFileFromMemory ||
        g_dex_file_hooks.DexFileFromMemory == dex_file_hooks->DexFileFromMemory);
  CHECK(g_dex_file_hooks.DexFileFromFile == DexFileFromFile ||
        g_dex_file_hooks.DexFileFromFile == dex_file_hooks->DexFileFromFile);
  CHECK(g_dex_file_hooks.GetMethodInformation == GetMethodInformation ||
        g_dex_file_hooks.GetMethodInformation == dex_file_hooks->GetMethodInformation);
  CHECK(g_dex_file_hooks.FreeDexFile == FreeDexFile ||
        g_dex_file_hooks.FreeDexFile == dex_file_hooks->FreeDexFile);
