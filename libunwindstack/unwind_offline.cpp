...
int main() {
  ...
  switch (type) {
  case EM_386:
    regs = new Regs32(X86_REG_PC, X86_REG_SP, X86_REG_LAST);
    reg_size = X86_REG_LAST * sizeof(uint32_t);
    break;
  case EM_ARM:
    regs = new Regs32(ARM_REG_PC, ARM_REG_SP, ARM_REG_LAST);
    reg_size = ARM_REG_LAST * sizeof(uint32_t);
    break;
  case EM_X86_64:
    regs = new Regs32(X86_64_REG_PC, X86_64_REG_SP, X86_64_REG_LAST);
    reg_size = X86_64_REG_LAST * sizeof(uint64_t);
    break;
  case EM_AARCH64:
    regs = new Regs64(ARM64_REG_PC, ARM64_REG_SP, ARM64_REG_LAST);
  ...
  bytes = TEMP_FAILURE_RETRY(read(fd, regs->raw_data(), reg_size));
