...
// Without index, the symbol table is assumed to be sorted and accessed directly.
// If the symbol table is not sorted this method mind fail but should not crash.
// Otherwise, we remap the accesses through the index to guarantee sorted order.
template <bool UseIndex, typename SymType>
bool Symbols::BinarySearch(uint64_t addr, Memory* elf_memory, SymType* entry) {
  size_t first = 0;
  size_t last = UseIndex ? index_.size() : count_;
  while (first < last) {
    size_t current = first + (last - first) / 2;
    size_t remaped = UseIndex ? index_[current] : current;
...
bool Symbols::GetEntry(uint64_t addr, Memory* elf_memory, SymType* entry) {
  ...
  if (index_.empty() && BinarySearch<false>(addr, elf_memory, entry) && IsFunc(entry)) {
    return true;
  }
  ...
  if (index_.empty()) {
    ...
    for (uint32_t i = 0; i < count_; i++) {
      ...
      addr.push_back(entry->st_value);
      if (IsFunc(entry)) {
        index_.push_back(i);
      }
    }
    std::sort(index_.begin(), index_.end(), [&addr](auto a, auto b) { return addr[a] < addr[b]; });
  }
