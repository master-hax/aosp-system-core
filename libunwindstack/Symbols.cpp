...
bool Symbols::GetEntry(uint64_t addr, Memory* elf_memory, SymType* entry) {
  ...
  if (remap_.empty()) {
    ...
    // Stable-sort by address to make the remap list binary searchable.
    auto addr_lt = [&addrs](auto a, auto b) {
      return std::make_tuple(addrs[a], a) < std::make_tuple(addrs[b], b);
    };
    std::sort(remap_.begin(), remap_.end(), addr_lt);
...
bool Symbols::GetName(uint64_t addr, Memory* elf_memory, std::string* name, uint64_t* func_offset) {
  ...
  uint64_t end_addr = it->first;
  FuncInfo& info = it->second;
  *func_offset = addr - (end_addr - info.size);
