...
Memory* MapInfo::GetFileMemory() {
  ...
  // There are three possibilities when the offset is non-zero.
  ...
  if (!valid) {
    ...
    if (!name.empty()) {
      // Try to find a non-zero read-only map for the same file.
    }
    return nullptr;
  }
...
Memory* MapInfo::CreateMemory(const std::shared_ptr<Memory>& process_memory) {
  ...
  // only part of the elf file to be mapped into memory as executable.
  // In this case, there will be another read-only map that includes the
  // first part of the elf file. This is done by the linker rosalloc
  // option.
  ...
  // to the current offset but less than it.
  // For shared libraries, there should be a r-x map that has a non-zero
  // offset and then a r-- map that has a zero map.
  ...
  // than the offset from the r-x map. There can be multiple shared libs in
  // an apk, where the name is the same in all of the maps.
