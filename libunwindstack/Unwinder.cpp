...
void Unwinder::FillInFrame(MapInfo* map_info, Elf* elf, uint64_t adjusted_rel_pc, uint64_t func_pc) {
...
#ifndef DISABLE_DEX_READING
  // TODO: Nasty hack - the .dex section is within the ELF file, but its BSS and it is mmaped
  // some to the .vdex file. Scan the memory until we find the proper ELF file.
  uint64_t dex_rel_pc = 0;
  if (frame->dex_pc != 0) {
    MapInfo* dex_map_info;
    Elf* dex_elf = nullptr;
    for (uint64_t elf_pc = regs_->dex_pc(); (dex_map_info = maps_->Find(elf_pc)) != nullptr;
         elf_pc -= 0x1000) {
      dex_elf = dex_map_info->GetElf(process_memory_, true);
      if (dex_elf != nullptr && dex_elf->valid()) {
        uint64_t dex_rel_pc = elf->GetRelPc(regs_->dex_pc(), dex_map_info);
  ...
  if (dex_rel_pc != 0 && frame->function_name == "$dexfile") {
