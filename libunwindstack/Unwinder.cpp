...
void Unwinder::FillInFrame(MapInfo* map_info, Elf* elf, uint64_t adjusted_rel_pc, uint64_t func_pc) {
  ...
  if (frame->dex_pc != 0) {
    ...
    // so if want to find the ELF header, we need to try one of the previous maps.
    MapInfo* dex_map_info;
    Elf* dex_elf = nullptr;
    for (uint64_t addr = regs_->dex_pc(); (dex_map_info = maps_->Find(addr)) != nullptr;
         addr = dex_map_info->start - 1u) {
