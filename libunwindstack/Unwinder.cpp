...
FrameData Unwinder::BuildFrameFromPcOnly(uint64_t pc, Regs* regs, Maps* maps, JitDebug* jit_debug,
                                         std::shared_ptr<Memory> process_memory,
                                         bool resolve_names) {
  FrameData frame;

  MapInfo* map_info = maps->Find(pc);
  if (map_info == nullptr || regs == nullptr) {
    frame.pc = pc;
    frame.rel_pc = pc;
    return frame;
  }
...
FrameData Unwinder::BuildFrameFromPcOnly(uint64_t pc) {
  return BuildFrameFromPcOnly(pc, regs_, maps_, jit_debug_, process_memory_, resolve_names_);
}
