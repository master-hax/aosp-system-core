...
#ifndef DISABLE_DEX_READING
  if (regs_->dex_pc() != 0 && frames_.size() < max_frames_) {
    ...
    MapInfo* map_info;
    Elf* elf = nullptr;
    uint64_t rel_dex_pc = 0;
    for (uint64_t addr = regs_->dex_pc(); (map_info = maps_->Find(addr)) != nullptr;
         addr = map_info->start - 1u) {
      elf = map_info->GetElf(process_memory_, true);
      if (elf != nullptr && elf->valid()) {
        rel_dex_pc = elf->GetRelPc(regs_->dex_pc(), map_info);
        elf->GetFunctionName(rel_dex_pc, &frame->function_name, &frame->function_offset);
        break;
      }
    }
