...
inline bool ArmExidx::DecodePrefix2_0(uint8_t byte) {
  ...
  while (registers) {
    ...
    reg += bit;
    if (!process_memory_->Read32(cfa_, regs_->addr(reg + 3))) {
      status_ = ARM_STATUS_READ_FAILED;
      return false;
    }
    cfa_ += 4;
...
inline bool ArmExidx::DecodePrefix2_2(uint8_t byte) {
  ...
  for (size_t i = 4; i <= 4 + (byte & 0x7); i++) {
    if (!process_memory_->Read32(cfa_, regs_->addr(i))) {
      status_ = ARM_STATUS_READ_FAILED;
      return false;
    }
    cfa_ += 4;
  ...
  if (byte & 0x8) {
    if (!process_memory_->Read32(cfa_, regs_->addr(ARM_REG_R14))) {
      status_ = ARM_STATUS_READ_FAILED;
      return false;
    }
    cfa_ += 4;
...
inline bool ArmExidx::DecodePrefix2_3(uint8_t byte) {
  ...
  if (bits == 0) {
    ...
    if (!regs_->value(ARM_REG_PC)) {
      regs_->set(ARM_REG_PC, regs_->value(ARM_REG_LR));
    }
    status_ = ARM_STATUS_FINISH;
    return false;
  } else if (bits == 1) {
    ...
    while (byte) {
      ...
      reg += bit;
      if (!process_memory_->Read32(cfa_, regs_->addr(reg - 1))) {
        status_ = ARM_STATUS_READ_FAILED;
        return false;
      }
      cfa_ += 4;
