...
bool ArmExidx::ExtractEntry(uint32_t entry) {
  data_.clear();
  ...
  uint32_t data;
  if (!elf_memory_->Read32(entry + 4, &data)) {
  ...
  uint32_t addr = (entry + 4) + signed_data;
...
inline bool ArmExidx::DecodePrefix2_0(uint8_t byte) {
  ...
  // It is impossible for reg + 3 to be larger than the total number of
  // arm registers, so don't bother checking if reg + 3 is a valid register.
  uint8_t reg = 0;
  while (registers) {
    uint8_t bit = __builtin_ctz(registers) + 1;
    reg += bit;
    if (!process_memory_->Read32(cfa_, &(*regs_)[reg + 3])) {
      status_ = ARM_STATUS_READ_FAILED;
      return false;
    }
    cfa_ += 4;
...
inline bool ArmExidx::DecodePrefix2_3(uint8_t byte) {
  ...
  if (bits == 0) {
    ...
    // It is impossible for reg - 1 to be larger than the total number of
    // arm registers, so don't bother checking if reg - 1 is a valid register.
    uint8_t reg = 0;
    while (byte) {
      uint8_t bit = __builtin_ctz(byte) + 1;
      reg += bit;
      if (!process_memory_->Read32(cfa_, &(*regs_)[reg - 1])) {
        status_ = ARM_STATUS_READ_FAILED;
        return false;
      }
      cfa_ += 4;
