...
static size_t PartialReadHelper(const void* src_base, size_t src_len, uint64_t offset, void* dst,
                                size_t len) {
  if (offset >= src_len) {
    return 0;
  }
...
static size_t ProcessVmRead(pid_t pid, void* dst, const void* remote_src, size_t len) {
...
bool Memory::Read(uint64_t addr, void* dst, size_t size) {
  size_t rc = PartialRead(addr, dst, size);
  return rc == size;
}
...
size_t MemoryBuffer::PartialRead(uint64_t addr, void* dst, size_t size) {
...
size_t MemoryFileAtOffset::PartialRead(uint64_t addr, void* dst, size_t size) {
  return PartialReadHelper(data_, size_, addr, dst, size);
}

size_t MemoryRemote::PartialRead(uint64_t addr, void* dst, size_t size) {
  if (addr > UINTPTR_MAX) {
    return 0;
  }
...
size_t MemoryLocal::PartialRead(uint64_t addr, void* dst, size_t size) {
...
size_t MemoryRange::PartialRead(uint64_t addr, void* dst, size_t size) {
  if (addr > length_) {
    return 0;
  }
...
size_t MemoryOffline::PartialRead(uint64_t addr, void* dst, size_t size) {
  if (!memory_) {
    return 0;
  }
