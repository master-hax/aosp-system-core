  ...
  if (aug_string[0] == 'z') {
    for (size_t i = 1; i < aug_string.size(); i++) {
      if (aug_string[i] == 'P') {
        ...
        break;
      } else if (aug_string[i] == 'L' || aug_string[i] == 'R') {
        offset++;
      }
    }
  }
...
bool DwarfDebugFrame<AddressType>::AddFDEInfo(
  ...
  uint64_t value;
  if (!memory_.template ReadEncodedValue<AddressType>(encoding & 0xf, &value)) {
    return false;
  }
  fde_info.start = value;

  if (!memory_.template ReadEncodedValue<AddressType>(encoding & 0xf, &value)) {
    return false;
  }
  fde_info.end = fde_info.start + value;
  fdes_.push_back(fde_info);
...
template <typename AddressType>
bool DwarfDebugFrame<AddressType>::GetFDEOffset(uint64_t pc, uint64_t* fde_offset) {
  if (fde_count_ == 0) {
    return false;
  }
