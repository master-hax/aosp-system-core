...
void DwarfSectionImplNoHdr<AddressType>::GetFdes(std::vector<const DwarfFde*>* fdes) {
  for (auto& fde_entry : fde_entries_) {
    fdes->push_back(&fde_entry.second);
  }
  ...
  std::sort(fdes->begin(), fdes->end(), [](const DwarfFde* a, const DwarfFde* b) {
    if (a->pc_start == b->pc_start) {
      return a->pc_end < b->pc_end;
    }
...
const DwarfFde* DwarfSectionImplNoHdr<AddressType>::GetFdeFromPc(uint64_t pc) {
  ...
  // The debug_frame section might have overlapping pcs in fdes, so it
  // is necessary to do a linear search of the fdes by pc.
  while (next_entries_offset_ < entries_end_) {
    DwarfFde* fde;
    if (!GetNextCieOrFde(&fde)) {
      return nullptr;
