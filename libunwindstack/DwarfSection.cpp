...
bool DwarfSectionTmpl<AddressType>::EvalExpression(const DwarfLocation& loc, uint8_t version,
...
template <typename AddressType>
bool DwarfSectionTmpl<AddressType>::Eval(const DwarfCIE* cie, Memory* regular_memory,
    ...
    switch (loc->type) {
      ...
      case DWARF_LOCATION_REGISTER: {
        ...
        if (loc_regs.find(cur_reg) != loc_regs.end()) {
          ...
          // than a register.
          log(0,
              "Invalid indirection: register %d references register %d which is not "
              "stored somewhere other than the register.\n",
              reg, cur_reg);
          last_error_ = DWARF_ERROR_ILLEGAL_STATE;
          return false;
        }
...
template <typename AddressType>
bool DwarfSectionTmpl<AddressType>::FillInCIE(DwarfCIE* cie) {
  uint32_t length32;
...
template <typename AddressType>
const DwarfFDE* DwarfSectionTmpl<AddressType>::GetFDEFromOffset(uint64_t offset) {
...
template <typename AddressType>
bool DwarfSectionTmpl<AddressType>::FillInFDE(DwarfFDE* fde) {
  uint32_t length32;
  if (!memory_.ReadBytes(&length32, sizeof(length32))) {
    last_error_ = DWARF_ERROR_MEMORY_INVALID;
    return false;
...
template <typename AddressType>
bool DwarfSectionTmpl<AddressType>::GetCfaLocationInfo(uint64_t pc, const DwarfFDE* fde,
                                                       dwarf_loc_regs_t* loc_regs) {
...
template <typename AddressType>
bool DwarfSectionTmpl<AddressType>::Log(uint8_t indent, uint64_t pc, uint64_t load_bias,
                                        const DwarfFDE* fde) {
  DwarfCfa<AddressType> cfa(&memory_, fde);
