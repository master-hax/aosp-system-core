  ...
  switch (loc->type) {
    ...
    case DWARF_LOCATION_REGISTER: {
      ...
      if (entry != eval_info->loc_regs->end()) {
        if (!eval_info->regs_info.IsSaved(cur_reg)) {
          AddressType* cur_reg_ptr = eval_info->regs_info.Save(cur_reg);
    ...
    case DWARF_LOCATION_VAL_EXPRESSION: {
      AddressType value;
      if (!EvalExpression(*loc, regular_memory, &value, &eval_info->regs_info)) {
        return false;
      }
      if (loc->type == DWARF_LOCATION_EXPRESSION) {
        ...
        *reg_ptr = value;
        // See if the dex pc is being set.
        uint64_t end = loc->values[1];
        uint64_t start = end - loc->values[0];
        if (end - start >= sizeof(DEX_PC_MAGIC)) {
          uint8_t dex_magic[sizeof(DEX_PC_MAGIC)];
          memory_.set_cur_offset(start);
          if (memory_.ReadBytes(dex_magic, sizeof(dex_magic)) &&
              memcmp(DEX_PC_MAGIC, dex_magic, sizeof(dex_magic)) == 0) {
...
bool DwarfSectionImpl<AddressType>::Eval(const DwarfCie* cie, Memory* regular_memory,
  ...
  for (const auto& entry : loc_regs) {
    ...
    AddressType* reg_ptr;
    if (reg >= cur_regs->total_regs() || eval_info.regs_info.IsSaved(reg)) {
