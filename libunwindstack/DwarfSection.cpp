...
DwarfFDE* DwarfSectionTmpl<AddressType>::GetFDEFromOffset(uint64_t offset) {
  ...
  if (!memory_.ReadEncodedValue<AddressType>(cie->fde_address_encoding & 0xf, &fde->start_pc)) {
    return nullptr;
  }
  fde->start_pc = AdjustPcFromFDE(fde->start_pc);

  if (!memory_.ReadEncodedValue<AddressType>(cie->fde_address_encoding & 0xf, &fde->pc_length)) {
    return nullptr;
  }
  if (cie->augmentation_string.size() > 0 && cie->augmentation_string[0] == 'z') {
    // Augmentation Size
