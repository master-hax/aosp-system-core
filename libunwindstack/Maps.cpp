...
bool LocalUpdatableMaps::Reparse() {
  ...
  const size_t total_entries = maps_.size();
  auto new_map_iter = maps_.begin();
  for (auto old_map_iter = old_maps.begin(); old_map_iter != old_maps.end(); ++old_map_iter) {
    new_map_iter =
        std::lower_bound(new_map_iter, maps_.end(), *old_map_iter,
