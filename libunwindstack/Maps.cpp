...
static bool InternalParseLine(const char* line, MapInfo* map_info) {
  // Parse manually, sscanf can result in errors.
  // Read in start address.
  // Linux /proc/<pid>/maps lines:
  // 6f000000-6f01e000 rwxp 00000000 00:0c 16389419   /system/lib/libcomposer.so
  char* str;
  map_info->start = strtoul(line, &str, 16);

  if (*str++ != '-') {
    return false;
  }

  map_info->end = strtoul(str, &str, 16);
  if (!std::isspace(*str++)) {
    return false;
  }
  ...
  map_info->offset = strtoul(str, &str, 16);
  if (!std::isspace(*str)) {
    return false;
  }
  ...
  // Skip decimal digit.
  (void)strtoul(str, &str, 10);
  if (!std::isspace(*str) && *str != '\0') {
    return false;
  }
...
bool Maps::Parse() {
  ...
  // If the size of this buffer changes, modify the corresponding size
  // in the MapsTest.file_buffer_cross test.
  char buffer[2048];
  size_t leftover = 0;
  while (true) {
    ...
    char* line = buffer;
    while (bytes > 0) {
      char* newline = reinterpret_cast<char*>(memchr(line, '\n', bytes));
      if (newline == nullptr) {
