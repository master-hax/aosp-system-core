...
bool Maps::Parse() {
  return android::procinfo::ReadMapFile(
      ...
      [&](uint64_t start, uint64_t end, uint16_t flags, uint64_t pgoff, ino_t, const char* name) {
        ...
        maps_.emplace_back(
            new MapInfo(maps_.empty() ? nullptr : maps_.back().get(), start, end, pgoff, flags, name));
      });
}
...
void Maps::Add(uint64_t start, uint64_t end, uint64_t offset, uint64_t flags,
  ...
  auto map_info =
      std::make_unique<MapInfo>(maps_.empty() ? nullptr : maps_.back().get(), start, end, offset, flags, name);
...
void Maps::Sort() {
  std::sort(maps_.begin(), maps_.end(),
            [](const std::unique_ptr<MapInfo>& a, const std::unique_ptr<MapInfo>& b) { return a->start < b->start; });
