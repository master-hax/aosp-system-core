...
TEST_F(MapInfoTest, create_memory_file_backed_zero_offset) {
  MapInfo info{.start = 0x100, .end = 0x200, .offset = 0, .name = elf_.path};

  Memory* memory = info.CreateMemory(getpid());
...
TEST_F(MapInfoTest, create_memory_check_device_maps) {
  ...
  info.name = "/dev/does_not_exist";
  memory.reset(info.CreateMemory(getpid()));
  ASSERT_TRUE(memory.get() == nullptr);

  // These should be okay.
  info.name = "/dev/ashmem/does_not_exist";
  memory.reset(info.CreateMemory(getpid()));
  ASSERT_TRUE(memory.get() != nullptr);
  info.name = "/devsomething/does_not_exist";
  memory.reset(info.CreateMemory(getpid()));
  ASSERT_TRUE(memory.get() != nullptr);
}
