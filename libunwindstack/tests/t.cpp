...
TEST_F(UnwindOfflineTest, shared_lib_in_apk_arm64) {
  ...
  EXPECT_EQ(
      ...
      "  #03 pc 00000000000846f8  libc.so (abort+176)\n"
  ...
  EXPECT_EQ(0x7e7eecc6f8ULL, unwinder.frames()[3].pc);
...
// Verify that the frame after a signal is not adjusted.
TEST_F(UnwindOfflineTest, signal_offset_arm) {
  ASSERT_NO_FATAL_FAILURE(Init("signal_offset_arm/", ARCH_ARM));
  ...
  ASSERT_EQ(8U, unwinder.NumFrames()) << "Unwind:\n" << frame_info;
  EXPECT_EQ(
      "  #00 pc 0009a2dc  linker (__dl_syscall+28)\n"
      "  #01 pc 00032ef1  linker (__dl__ZL24debuggerd_signal_handleriP7siginfoPv+764)\n"
      "  #02 pc 00000947  libsigmux.so (sigmux_handle_signal+386)\n"
      "  #03 pc 0005a710  libc.so!libc.so (offset 0x29000)\n"
      "  #04 pc 000a8880  libc.so!libc.so (offset 0x60000) (__aeabi_unwind_cpp_pr0)\n"
      "  #05 pc 000ab7d0  libc.so!libc.so (offset 0x60000) (unwind_phase2+60)\n"
      "  #06 pc 000aba40  libc.so!libc.so (offset 0x60000) (__gnu_Unwind_Resume+88)\n"
