...
TEST_F(MapInfoGetBuildIDTest, no_elf_and_no_valid_elf_in_memory) {
  ...
  EXPECT_EQ("", info.GetBuildID(process_memory_));
...
TEST_F(MapInfoGetBuildIDTest, from_elf) {
  ...
  EXPECT_EQ("FAKE_BUILD_ID", map_info_->GetBuildID(process_memory_));
...
void MapInfoGetBuildIDTest::MultipleThreadTest(std::string expected_build_id) {
  ...
  for (size_t i = 0; i < kNumConcurrentThreads; i++) {
    std::thread* thread = new std::thread([i, this, &wait, &build_id_values]() {
      while (wait)
        ;
      build_id_values[i] = map_info_->GetBuildID(process_memory_);
    });
...
static void InitElfData(MemoryFake* memory, uint64_t offset) {
  ...
  memory->SetMemory(offset + ehdr.e_shoff + sizeof(shdr), &shdr, sizeof(shdr));
  ...
  shdr.sh_size = 0x1000;
  memory->SetMemory(offset + ehdr.e_shoff + 2 * sizeof(shdr), &shdr, sizeof(shdr));

  memory->SetMemory(offset + 0xf500, ".note.gnu.build-id", sizeof(".note.gnu.build-id"));
  memory->SetMemory(offset + 0xb000, note_section, sizeof(note_section));
}

TEST_F(MapInfoGetBuildIDTest, from_memory) {
  InitElfData(memory_, map_info_->start);

  EXPECT_EQ("ELF_BUILDID", map_info_->GetBuildID(process_memory_));
}
