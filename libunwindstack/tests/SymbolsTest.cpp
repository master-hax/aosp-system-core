...
TYPED_TEST_P(SymbolsTest, function_bounds_check) {
  ...
  ASSERT_TRUE(symbols.GetName(0x5000, 0, &this->memory_, &name, &func_offset));
  ASSERT_EQ("fake_function", name);
  ASSERT_EQ(0U, func_offset);

  name.clear();
  ASSERT_TRUE(symbols.GetName(0x500f, 0, &this->memory_, &name, &func_offset));
  ASSERT_EQ("fake_function", name);
  ASSERT_EQ(0xfU, func_offset);

  // Check one before and one after the function.
  ASSERT_FALSE(symbols.GetName(0x4fff, 0, &this->memory_, &name, &func_offset));
  ASSERT_FALSE(symbols.GetName(0x5010, 0, &this->memory_, &name, &func_offset));
}
...
TYPED_TEST_P(SymbolsTest, no_symbol) {
  ...
  sym.st_info = 0;
  this->memory_.SetMemory(offset, &sym, sizeof(sym));
  ASSERT_FALSE(symbols.GetName(0x5000, 0, &this->memory_, &name, &func_offset));
...
TYPED_TEST_P(SymbolsTest, multiple_entries) {
  ...
  std::string name;
  uint64_t func_offset;
  ASSERT_TRUE(symbols.GetName(0x3005, 0, &this->memory_, &name, &func_offset));
  ASSERT_EQ("function_two", name);
  ASSERT_EQ(1U, func_offset);

  name.clear();
  ASSERT_TRUE(symbols.GetName(0x5004, 0, &this->memory_, &name, &func_offset));
  ASSERT_EQ("function_one", name);
  ASSERT_EQ(4U, func_offset);

  name.clear();
  ASSERT_TRUE(symbols.GetName(0xa011, 0, &this->memory_, &name, &func_offset));
  ...
  name.clear();
  ASSERT_TRUE(symbols.GetName(0x5008, 0, &this->memory_, &name, &func_offset));
  ASSERT_EQ("function_one", name);
  ASSERT_EQ(8U, func_offset);

  name.clear();
  ASSERT_TRUE(symbols.GetName(0x3008, 0, &this->memory_, &name, &func_offset));
  ASSERT_EQ("function_two", name);
  ASSERT_EQ(4U, func_offset);

  name.clear();
  ASSERT_TRUE(symbols.GetName(0xa01a, 0, &this->memory_, &name, &func_offset));
  ASSERT_EQ("function_three", name);
...
TYPED_TEST_P(SymbolsTest, load_bias) {
  ...
  std::string name;
  uint64_t func_offset;
  ASSERT_TRUE(symbols.GetName(0x5004, 0x1000, &this->memory_, &name, &func_offset));
  ASSERT_EQ("fake_function", name);
  ASSERT_EQ(4U, func_offset);
  ...
  this->memory_.SetMemory(offset, &sym, sizeof(sym));
  ASSERT_FALSE(symbols.GetName(0x5004, 0x1000, &this->memory_, &name, &func_offset));
}
...
TYPED_TEST_P(SymbolsTest, symtab_value_out_of_bounds) {
  ...
  ASSERT_TRUE(symbols_end_at_200.GetName(0x5000, 0, &this->memory_, &name, &func_offset));
  ASSERT_EQ("fake_function", name);
  ASSERT_EQ(0U, func_offset);
  ASSERT_TRUE(symbols_end_at_200.GetName(0x3000, 0, &this->memory_, &name, &func_offset));
  ASSERT_EQ("function", name);
  ASSERT_EQ(0U, func_offset);

  // Now use the symbol table that ends at 0x100.
  ASSERT_FALSE(symbols_end_at_100.GetName(0x5000, 0, &this->memory_, &name, &func_offset));
  ASSERT_FALSE(symbols_end_at_100.GetName(0x3000, 0, &this->memory_, &name, &func_offset));
}
