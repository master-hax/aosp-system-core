...
class DwarfMemoryTest : public ::testing::Test {
 protected:
  ...
  template <typename AddressType> void GetEncodedSize_absptr();
  template <typename AddressType> void GetEncodedSize_data1();
  template <typename AddressType> void GetEncodedSize_data2();
  template <typename AddressType> void GetEncodedSize_data4();
  template <typename AddressType> void GetEncodedSize_data8();
  template <typename AddressType> void GetEncodedSize_unknown();
  template <typename AddressType> void ReadEncodedValue_omit();
  template <typename AddressType> void ReadEncodedValue_leb128();
  template <typename AddressType> void ReadEncodedValue_data1();
  template <typename AddressType> void ReadEncodedValue_data2();
  template <typename AddressType> void ReadEncodedValue_data4();
  template <typename AddressType> void ReadEncodedValue_data8();
  template <typename AddressType> void ReadEncodedValue_non_zero_adjust();
...
TEST_F(DwarfMemoryTest, ReadULEB128) {
  memory_.SetMemory(0, std::vector<uint8_t>{0x01,
                                            0x80, 0x24,
                                            0xff, 0xc3, 0xff, 0x7f});
...
TEST_F(DwarfMemoryTest, ReadSLEB128) {
  memory_.SetMemory(0, std::vector<uint8_t>{0x06,
                                            0x40,
                                            0x82, 0x34,
                                            0x89, 0x64,
                                            0xf9, 0xc3, 0x8f, 0x2f,
...
void DwarfMemoryTest::GetEncodedSize_absptr() {
  for (size_t i = 0; i < 16; i++) {
    uint8_t encoding = i << 4;
    ASSERT_EQ(sizeof(AddressType), dwarf_mem_->GetEncodedSize<AddressType>(encoding))
        << "Bad value 0x" << std::hex << static_cast<uint32_t>(encoding);
  }
}

TEST_F(DwarfMemoryTest, GetEncodedSize_absptr_uint32_t) {
  GetEncodedSize_absptr<uint32_t>();
}

TEST_F(DwarfMemoryTest, GetEncodedSize_absptr_uint64_t) {
  GetEncodedSize_absptr<uint64_t>();
}

template <typename AddressType>
void DwarfMemoryTest::GetEncodedSize_data1() {
  for (size_t i = 0; i < 16; i++) {
    uint8_t encoding = (i << 4) | 0x0d;
    ASSERT_EQ(1U, dwarf_mem_->GetEncodedSize<AddressType>(encoding))
        << "Bad value 0x" << std::hex << static_cast<uint32_t>(encoding);
    encoding = (i << 4) | 0x0e;
    ASSERT_EQ(1U, dwarf_mem_->GetEncodedSize<AddressType>(encoding))
        << "Bad value 0x" << std::hex << static_cast<uint32_t>(encoding);
...
TEST_F(DwarfMemoryTest, GetEncodedSize_data1_uint32_t) {
  GetEncodedSize_data1<uint32_t>();
}

TEST_F(DwarfMemoryTest, GetEncodedSize_data1_uint64_t) {
  GetEncodedSize_data1<uint64_t>();
}

template <typename AddressType>
void DwarfMemoryTest::GetEncodedSize_data2() {
  for (size_t i = 0; i < 16; i++) {
    uint8_t encoding = (i << 4) | 0x02;
    ASSERT_EQ(2U, dwarf_mem_->GetEncodedSize<AddressType>(encoding))
        << "Bad value 0x" << std::hex << static_cast<uint32_t>(encoding);
    encoding = (i << 4) | 0x0a;
    ASSERT_EQ(2U, dwarf_mem_->GetEncodedSize<AddressType>(encoding))
        << "Bad value 0x" << std::hex << static_cast<uint32_t>(encoding);
...
TEST_F(DwarfMemoryTest, GetEncodedSize_data2_uint32_t) {
  GetEncodedSize_data2<uint32_t>();
}

TEST_F(DwarfMemoryTest, GetEncodedSize_data2_uint64_t) {
  GetEncodedSize_data2<uint64_t>();
}

template <typename AddressType>
void DwarfMemoryTest::GetEncodedSize_data4() {
  for (size_t i = 0; i < 16; i++) {
    uint8_t encoding = (i << 4) | 0x03;
    ASSERT_EQ(4U, dwarf_mem_->GetEncodedSize<AddressType>(encoding))
        << "Bad value 0x" << std::hex << static_cast<uint32_t>(encoding);
    encoding = (i << 4) | 0x0b;
    ASSERT_EQ(4U, dwarf_mem_->GetEncodedSize<AddressType>(encoding))
        << "Bad value 0x" << std::hex << static_cast<uint32_t>(encoding);
...
TEST_F(DwarfMemoryTest, GetEncodedSize_data4_uint32_t) {
  GetEncodedSize_data4<uint32_t>();
}

TEST_F(DwarfMemoryTest, GetEncodedSize_data4_uint64_t) {
  GetEncodedSize_data4<uint64_t>();
}

template <typename AddressType>
void DwarfMemoryTest::GetEncodedSize_data8() {
  for (size_t i = 0; i < 16; i++) {
    uint8_t encoding = (i << 4) | 0x04;
    ASSERT_EQ(8U, dwarf_mem_->GetEncodedSize<AddressType>(encoding))
        << "Bad value 0x" << std::hex << static_cast<uint32_t>(encoding);
    encoding = (i << 4) | 0x0c;
    ASSERT_EQ(8U, dwarf_mem_->GetEncodedSize<AddressType>(encoding))
        << "Bad value 0x" << std::hex << static_cast<uint32_t>(encoding);
...
TEST_F(DwarfMemoryTest, GetEncodedSize_data8_uint32_t) {
  GetEncodedSize_data8<uint32_t>();
}

TEST_F(DwarfMemoryTest, GetEncodedSize_data8_uint64_t) {
  GetEncodedSize_data8<uint64_t>();
}

template <typename AddressType>
void DwarfMemoryTest::GetEncodedSize_unknown() {
  for (size_t i = 0; i < 16; i++) {
    uint8_t encoding = (i << 4) | 0x01;
    ASSERT_EQ(0U, dwarf_mem_->GetEncodedSize<AddressType>(encoding))
        << "Bad value 0x" << std::hex << static_cast<uint32_t>(encoding);
    encoding = (i << 4) | 0x09;
    ASSERT_EQ(0U, dwarf_mem_->GetEncodedSize<AddressType>(encoding))
        << "Bad value 0x" << std::hex << static_cast<uint32_t>(encoding);
...
TEST_F(DwarfMemoryTest, GetEncodedSize_unknown_uint32_t) {
  GetEncodedSize_unknown<uint32_t>();
}

TEST_F(DwarfMemoryTest, GetEncodedSize_unknown_uint64_t) {
  GetEncodedSize_unknown<uint64_t>();
}
