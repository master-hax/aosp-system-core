...
class JitDebugTest : public ::testing::Test {
 protected:
  ...
  void WriteEntry32(uint64_t addr, uint32_t prev, uint32_t next, uint32_t elf_addr,
                    uint64_t elf_size);
...
void JitDebugTest::WriteEntry32(uint64_t addr, uint32_t prev, uint32_t next, uint32_t elf_addr,
                                uint64_t elf_size) {
...
TEST_F(JitDebugTest, get_elf_32) {
  ...
  WriteDescriptor32(0xf800, 0x200000);
  WriteEntry32(0x200000, 0, 0, 0x4000, 0x1000);
...
TEST_F(JitDebugTest, get_elf_multiple_entries) {
  ...
  WriteDescriptor32(0xf800, 0x200000);
  WriteEntry32(0x200000, 0, 0x200100, 0x4000, 0x1000);
  WriteEntry32(0x200100, 0x200100, 0, 0x5000, 0x1000);
