...
class MemoryCacheTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    cm_.reset(new MemoryCache(memory_));
  ...
  MemoryFake* memory_;
  std::unique_ptr<MemoryCache> cm_;
...
TEST_F(MemoryCacheTest, cached_read) {
  for (size_t i = 1; i <= kMaxCachedSize; i++) {
    ...
    ASSERT_TRUE(cm_->ReadFully(0x8000 + i, buffer.data(), i)) << "Read failed at size " << i;
...
TEST_F(MemoryCacheTest, cached_read_across_caches) {
  ...
  ASSERT_TRUE(cm_->ReadFully(0x8ff0, buffer.data(), 32));
...
TEST_F(MemoryCacheTest, read_for_cache_fail) {
  ...
  ASSERT_TRUE(cm_->ReadFully(0xa010, buffer.data(), kMaxCachedSize));
...
TEST_F(MemoryCacheTest, read_for_cache_fail_cross) {
  ...
  std::vector<uint8_t> buffer(32);
  ASSERT_TRUE(cm_->ReadFully(0x9ff0, buffer.data(), 32));
  ASSERT_EQ(expect, buffer);
