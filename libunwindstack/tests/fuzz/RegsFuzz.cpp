...
OperationVec getOperations() {
  OperationVec operations = {
      ...
      [](FuzzedDataProvider* data_provider, Regs* regs, Memory*, Memory*) -> void {
      ...
      [](FuzzedDataProvider* data_provider, Regs*, Memory*, Memory*) -> void {
      ...
      [](FuzzedDataProvider*, Regs* regs, Memory* proc_mem, Memory*) -> void {
        regs->SetPcFromReturnAddress(proc_mem);
      },
      [](FuzzedDataProvider* data_provider, Regs* regs, Memory* proc_mem, Memory* elf_mem) -> void {
        std::unique_ptr<Elf> elf = std::make_unique<Elf>(elf_mem);
        regs->StepIfSignalHandler(data_provider->ConsumeIntegral<uint64_t>(), elf.get(), proc_mem);
        elf.release();
      }};
  return operations;
}
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
  ...
  std::shared_ptr<Memory> elf_mem =
      Memory::CreateOfflineMemory(elf_mem_blob.data(), 0, elf_mem_size);
  ...
  while (data_provider.remaining_bytes() > 0 && ops_run++ < kMaxOperations) {
    ...
    operations[op](&data_provider, reg.get(), elf_mem.get(), proc_mem.get());
  }
  return 0;
}
}  // namespace unwindstack
