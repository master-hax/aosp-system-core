...
TEST_F(MapInfoGetElfTest, prev_map_elf_not_set) {
  ...
  // Verify the read-only map now has the same elf object.
  ASSERT_NE(elf, info1.GetElf(process_memory_, ARCH_ARM));
}

// Verify that a read-only map followed by a read-write map will result
// in the same elf object in both maps.
TEST_F(MapInfoGetElfTest, read_only_followed_by_read_write_share_elf) {
