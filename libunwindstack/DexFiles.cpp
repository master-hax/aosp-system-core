...
bool DexFiles::ReadEntry32() {
  uint32_t ptr;
  if (!memory_->ReadFully(entry_addr_, &ptr, sizeof(ptr)) || ptr == 0) {
    entry_addr_ = 0;
    return false;
  }

  DEXFileEntry32 entry;
  if (!memory_->ReadFully(ptr, &entry, sizeof(entry)) || entry.dex_file == 0) {
    entry_addr_ = 0;
    return false;
  }
...
void DexFiles::Init(Maps* maps) {
  ...
  for (MapInfo* info : *maps) {
    ...
    uint64_t entry_addr;
    if (elf->GetGlobalVariable(dex_debug_name, &entry_addr) && entry_addr != 0) {
      entry_addr_ = info->start + entry_addr;
      break;
    }
  }
}
