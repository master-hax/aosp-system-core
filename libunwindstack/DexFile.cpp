...
bool DexFile::GetMethodInformation(uint64_t dex_offset, std::string* method_name,
  ...
  *method_name = std::move(method_info.name);
  *method_offset = dex_offset - method_info.offset;
  return true;
}

DexFileFromFile* DexFileFromFile::Create(uint64_t dex_file_offset_in_file,
  ...
  return new DexFileFromFile(std::move(*art_dex_file.release()));
}

DexFileFromMemory* DexFileFromMemory::Create(uint64_t dex_file_offset_in_memory, Memory* memory,
  ...
  for (size_t size = 0;;) {
    ...
    if (art_dex_file != nullptr) {
      return new DexFileFromMemory(std::move(*art_dex_file.release()), std::move(backing_memory));
    }
