...
Memory* ElfInterface::CreateGnuDebugdataMemory() {
  ...
  std::unique_ptr<MemoryVector> dst(new MemoryVector);
  int return_val;
  size_t src_offset = 0;
  size_t dst_offset = 0;
  ECoderStatus status;
  ...
  do {
    ...
    return_val = XzUnpacker_Code(&state, dst->GetPtr(dst_offset), &dst_remaining,
                                 &src[src_offset], &src_remaining, CODER_FINISH_ANY, &status);
