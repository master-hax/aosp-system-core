...
class DwarfCfa {
 public:
  DwarfCfa(DwarfMemory<AddressType>* memory, const DwarfFDE* fde)
    ...
    while ((cfa_offset = memory_->cur_offset()) < end_offset && cur_pc_ <= pc) {
      ...
      case 2:
      {
        ...
        (*loc_regs)[cfa_low] = { .type = DWARF_LOCATION_OFFSET,
                                 .values = { offset * fde_->cie->data_alignment_factor } };
        break;
      }
      case 3:
      {
 ...
 private:
  ...
  static bool cfa_offset_extended_sf(void* ptr, dwarf_loc_regs_t* loc_regs) {
    ...
    AddressType reg = cfa->operands_[0];
    if (sizeof(AddressType) == 4) {
      int32_t value = cfa->operands_[1] * cfa->fde_->cie->data_alignment_factor;
      (*loc_regs)[reg] = { .type = DWARF_LOCATION_OFFSET,
                           .values = { static_cast<uint64_t>(value) } };
    } else {
      int64_t value = cfa->operands_[1] * cfa->fde_->cie->data_alignment_factor;
      (*loc_regs)[reg] = { .type = DWARF_LOCATION_OFFSET,
                           .values = { static_cast<uint64_t>(value) } };
    }
    return true;
  ...
  static bool cfa_def_cfa_sf(void* ptr, dwarf_loc_regs_t* loc_regs) {
    ...
    int64_t offset64;
    if (sizeof(AddressType) == 4) {
      offset64 = static_cast<int32_t>(cfa->operands_[1] * cfa->fde_->cie->data_alignment_factor);
    } else {
      offset64 = cfa->operands_[1] * cfa->fde_->cie->data_alignment_factor;
    }
    (*loc_regs)[CFA_REG] = { .type = DWARF_LOCATION_REGISTER,
  ...
  static bool cfa_def_cfa_offset_sf(void* ptr, dwarf_loc_regs_t* loc_regs) {
    ...
    cfa_location->second.values[1] = cfa->operands_[0] * cfa->fde_->cie->data_alignment_factor;
    return true;
  }
  ...
  static bool cfa_val_offset(void* ptr, dwarf_loc_regs_t* loc_regs) {
    ...
    (*loc_regs)[reg] = { .type = DWARF_LOCATION_VAL_OFFSET,
                         .values = { cfa->operands_[1] * cfa->fde_->cie->data_alignment_factor } };
    return true;
  }
  ...
  static bool cfa_val_offset_sf(void* ptr, dwarf_loc_regs_t* loc_regs) {
    ...
    AddressType reg = cfa->operands_[0];
    if (sizeof(AddressType) == 4) {
      int32_t value = static_cast<int32_t>(cfa->operands_[1]) * cfa->fde_->cie->data_alignment_factor;
      (*loc_regs)[reg] = { .type = DWARF_LOCATION_VAL_OFFSET,
                           .values = { static_cast<uint64_t>(value) } };
    } else {
      int64_t value = static_cast<int64_t>(cfa->operands_[1]) * cfa->fde_->cie->data_alignment_factor;
      (*loc_regs)[reg] = { .type = DWARF_LOCATION_VAL_OFFSET,
                           .values = { static_cast<uint64_t>(value) } };
    }
    return true;
