...
class Elf {
 public:
  ...
  bool Step(uint64_t rel_pc, Regs* regs, Memory* process_memory) {
    ...
    if (interface_->Step(rel_pc, regs, process_memory)) {
      return true;
    }
    if (gnu_debugdata_interface_.get() != nullptr) {
      // TODO: Figure out the best way to do this. The rel pc for data
      // in this section is a bit wrong. It might be better to have
      // a second call where we can send in a different relative pc.
      rel_pc += gnu_debugdata_interface_->load_bias();
      return gnu_debugdata_interface_->Step(rel_pc, regs, process_memory);
    }
    return false;
  }
