...
void BenchmarkFunction(benchmark::State& state, std::vector<uint64_t> offsets, std::string elf_file,
                       bool expect_found) {
#if defined(__BIONIC__)
...
void BenchmarkFunction(benchmark::State& state, uint64_t pc, std::string elf_file,
                       bool expect_found) {
  BenchmarkFunction(state, std::vector<uint64_t>{pc}, elf_file, expect_found);
}
