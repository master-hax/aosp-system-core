...
void DoUnwind(pid_t pid) {
  ...
  std::shared_ptr<unwindstack::Memory> remote_memory = unwindstack::Memory::FromPid(pid);
  for (size_t frame_num = 0; frame_num < 64; frame_num++) {
    ...
    unwindstack::Elf* elf = map_info->GetElf(remote_memory, true);
    ...
    if (!elf->Step(rel_pc + map_info->elf_offset, regs, remote_memory.get())) {
      break;
    }
  }
}
