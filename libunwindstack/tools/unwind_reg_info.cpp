...
void PrintRegInformation(DwarfSection* section, Memory* memory, uint64_t pc) {
  ...
  for (auto& entry : loc_regs) {
    ...
    switch (loc->type) {
      ...
      case DWARF_LOCATION_EXPRESSION: {
        ...
        printf("EXPRESSION %" PRIx64 "-%" PRIx64 "\n    ", start, end);
        for (uint64_t i = start; i < end; i++) {
          uint8_t byte;
          if (!memory->Read(start + i, &byte, sizeof(byte))) {
            printf(" XX");
          } else {
            printf(" %02x", byte);
          }
      ...
      case DWARF_LOCATION_VAL_EXPRESSION: {
        uint64_t start = loc->values[1];
        uint64_t end = start + loc->values[0];
        printf("VAL EXPRESSION %" PRIx64 "-%" PRIx64 "\n    ", start, end);
        for (uint64_t i = start; i < end; i++) {
          uint8_t byte;
          if (!memory->Read(start + i, &byte, sizeof(byte))) {
            printf(" XX");
...
int GetInfo(const char* file, uint64_t pc) {
  ...
  if (section != nullptr) {
    printf("\neh_frame:\n");
    PrintRegInformation(section, memory, pc - load_bias);
  } else {
  ...
  if (gnu_debugdata_interface != nullptr) {
    ...
    if (section != nullptr) {
      ...
      PrintRegInformation(section, gnu_debugdata_interface->memory(), pc);
      printf("\n");
    }
