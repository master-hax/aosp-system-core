...
bool SaveRegs(unwindstack::Regs* regs) {
  ...
  regs->IterateRegisters(
      [&fp](const char* name, uint64_t value) { fprintf(fp, "%s: %" PRIx64 "\n", name, value); });
  fclose(fp);
...
bool SaveStack(pid_t pid, uint64_t sp_start, uint64_t sp_end) {
  ...
  size_t bytes = fwrite(&sp_start, 1, sizeof(sp_start), fp);
  ...
  bytes = fwrite(buffer.data(), 1, buffer.size(), fp);
...
bool CreateElfFromMemory(std::shared_ptr<unwindstack::Memory>& memory, map_info_t* info) {
  ...
  FILE* output = fopen(cur_name.c_str(), "w+");
  if (output == nullptr) {
  ...
  size_t bytes_written = fwrite(buffer.data(), 1, bytes, output);
...
bool CopyElfFromFile(map_info_t* info) {
  FILE* fp = fopen(info->name.c_str(), "r");
  if (fp == nullptr) {
    return false;
  }
  ...
  size_t bytes;
  while ((bytes = fread(buffer.data(), 1, buffer.size(), fp)) > 0) {
    size_t bytes_written = fwrite(buffer.data(), 1, bytes, output);
    if (bytes_written != bytes) {
...
int SaveData(pid_t pid) {
  ...
  for (auto frame : unwinder.frames()) {
    ...
    if (maps_by_start.count(frame.map_start) == 0) {
      ...
      if (!CopyElfFromFile(info)) {
        // Try to create the elf from memory.
  ...
  for (auto& element : sorted_maps) {
    ...
    fprintf(fp, "%" PRIx64 "-%" PRIx64 " r-xp %" PRIx64 " 00:00 0", map.start, map.end, map.offset);
    if (!map.name.empty()) {
      fprintf(fp, "   %s", map.name.c_str());
    }
    fprintf(fp, "\n");
  }
  fclose(fp);
...
int main(int argc, char** argv) {
  ...
  // If there are any failures during SaveData, file descriptors might leak.
  // This is fine since the program will exit on any failure. The only
  // reason to return is to detach from the remote process and set the
  // return code.
