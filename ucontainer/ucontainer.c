...
static char exe_path[_POSIX_PATH_MAX];
...
static int stop() {
    ...
    if ((sefd = open(SEPATH, O_RDWR | O_CLOEXEC)) < 0) {
        perror(str("fail to open %s\n", SEPATH));
        return -1;
    }
    if ((ret = write(sefd, "0", strlen("0"))) < 0) {
        perror(str("fail to write %s\n", SEPATH));
        return ret;
    }
    close(sefd);
...
static int start() {
    ...
    for (i = 0; i < 4; i++) {
        ...
        if (!exists("/dev/block/zram%d", i)) {
            ...
            if (mknod(str("/dev/block/zram%d", i), S_IFBLK, dev)) {
                perror(str("fail to mknod /dev/block/zram%d\n", i));
                return -1;
            }
        }
        if (!exists("/mnt/zram%d", i) && mkdir(str("/mnt/zram%d", i), 0700) != 0) {
            fprintf(stderr, "fail to mkdir /mnt/zram%d\n", i);
            return -1;
        }
