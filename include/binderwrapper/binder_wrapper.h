...
class BinderWrapper {
 public:
  ...
  // with the real binder system), returning true on success.
  static bool Create();
  ...
  // Like GetService() but waits for a few seconds if the service isn't yet
  // registered.
  virtual sp<IBinder> WaitForService(const std::string& service_name) = 0;
