...
// The command used to start the program, such as "/system/bin/dalvikvm". If InitLogging hasn't
// been performed then just returns "unknown"
extern const char* ProgramInvocationName();

// A short version of the command used to start the ART runtime, such as "dalvikvm". If InitLogging
// hasn't been performed then just returns "unknown"
...
#define LOG(severity) \
    ::android::utils::LogMessage(__FILE__, __LINE__, ::android::utils::severity, -1).stream()

// A variant of LOG that also logs the current errno value. To be used when library calls fail.
#define PLOG(severity) \
    ::android::utils::LogMessage(__FILE__, __LINE__, ::android::utils::severity, errno).stream()
...
#define VLOG(module)                                                        \
    if (VLOG_IS_ON(module)) {                                               \
        ::android::utils::LogMessage(__FILE__, __LINE__, INFO, -1).stream() \
    }

// Return the stream associated with logging for the given module.
#define VLOG_STREAM(module) ::android::utils::LogMessage(__FILE__, __LINE__, INFO, -1).stream()
...
#define CHECK(x)                                                                               \
    if (UNLIKELY(!(x))) {                                                                      \
        ::android::utils::LogMessage(__FILE__, __LINE__, ::android::utils::FATAL, -1).stream() \
            << "Check failed: " #x << " "                                                      \
    }

// Helper for CHECK_xx(x,y) macros.
#define CHECK_OP(LHS, RHS, OP)                                                                   \
    for (auto _values = ::android::utils::MakeEagerEvaluator(LHS, RHS);                          \
...
#define CHECK_STROP(s1, s2, sense)                                                              \
    if (UNLIKELY((strcmp(s1, s2) == 0) != sense)) {                                             \
        LOG(::android::utils::FATAL) << "Check failed: "                                        \
                                     << "\"" << s1 << "\"" << (sense ? " == " : " != ") << "\"" \
                                     << s2 << "\""                                              \
    }
...
#define CHECK_PTHREAD_CALL(call, args, what)                                  \
    do {                                                                      \
        ...
        if (rc != 0) {                                                        \
            errno = rc;                                                       \
            PLOG(::android::utils::FATAL) << #call << " failed for " << what; \
        }                                                                     \
    } while (false)
...
#define CHECK_CONSTEXPR(x, out, dummy) \
    (UNLIKELY(!(x))) ? (LOG(::android::utils::FATAL) << "Check failed: " << #x out, dummy):
...
#define DCHECK(x) \
    if (::android::utils::kEnableDChecks) CHECK(x)
#define DCHECK_EQ(x, y) \
    if (::android::utils::kEnableDChecks) CHECK_EQ(x, y)
#define DCHECK_NE(x, y) \
    if (::android::utils::kEnableDChecks) CHECK_NE(x, y)
#define DCHECK_LE(x, y) \
    if (::android::utils::kEnableDChecks) CHECK_LE(x, y)
#define DCHECK_LT(x, y) \
    if (::android::utils::kEnableDChecks) CHECK_LT(x, y)
#define DCHECK_GE(x, y) \
    if (::android::utils::kEnableDChecks) CHECK_GE(x, y)
#define DCHECK_GT(x, y) \
    if (::android::utils::kEnableDChecks) CHECK_GT(x, y)
#define DCHECK_STREQ(s1, s2) \
    if (::android::utils::kEnableDChecks) CHECK_STREQ(s1, s2)
#define DCHECK_STRNE(s1, s2) \
    if (::android::utils::kEnableDChecks) CHECK_STRNE(s1, s2)
#if defined(NDEBUG)
...
#define EAGER_PTR_EVALUATOR(T1, T2)                                                          \
    ...
    struct EagerEvaluator<T1, T2> {                                                          \
        EagerEvaluator(T1 l, T2 r)                                                           \
            : lhs(reinterpret_cast<const void*>(l)), rhs(reinterpret_cast<const void*>(r)) { \
        }                                                                                    \
        const void* lhs;                                                                     \
        const void* rhs;                                                                     \
    }
