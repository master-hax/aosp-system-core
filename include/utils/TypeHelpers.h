...
move_forward_type(TYPE* d, const TYPE* s, size_t n = 1) {
    // TODO(b/31595853): Vector and SortedVector have the has_trivial_move
    // trait, but clang disagrees that they are trivial when
    // -Wdynamic-class-memaccess is enabled.  If the trait is removed the
    // reinterpret_casts below should also be removed.
    memmove(reinterpret_cast<void*>(d), reinterpret_cast<const void*>(s), n*sizeof(TYPE));
}
