...
// protocol, these smart pointers work. We have several instances of that in
// the platform, where sp<> is used on non-RefBase objects. sp != RefBase.
...
// and it's hard to find -- except it's in fact really easy because RefBase has
...
// In the case of weak pointers, thos occurs because an object that has never been
...
// Use of wp::unsafe_get() for any purpose other than debugging is probably wrong.
// Unless you somehow know that there is a longer-lived sp<> to the same object, it
// may well return a pointer to a deallocated object that has since been reallocated
// for a different purpose. (And if you know there is a longer-lived sp<>, why not use
// an sp<> directly?) A wp<> should only be dereferenced by using promote().

// Extra Features:

// RefBase::extendObjectLifetime() can be used to prevent destruction of the
// object while there are still weak reference. RefBase::attemptIncStrong(),
// along with wp::promote() can be used to try to get back a strong reference
// from a weak one. Except for promote(), this is used only in very special
// cases in the binder code.
...
// to THE SAME sp<> or wp<>.  In effect, theire thread-safety properties are
