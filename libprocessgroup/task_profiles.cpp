...
TaskProfiles::TaskProfiles() {
    ...
    std::string sys_profiles_path;

    // load API-level specific system task profiles if available
    if (api_level > 0) {
        sys_profiles_path = android::base::StringPrintf(TEMPLATE_TASK_PROFILE_API_FILE, api_level);
        if (access(sys_profiles_path.c_str(), F_OK)) {
            sys_profiles_path = TASK_PROFILE_DB_FILE;
        }
    } else {
        sys_profiles_path = TASK_PROFILE_DB_FILE;
    }
