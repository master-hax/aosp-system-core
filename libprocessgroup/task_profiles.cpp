...
bool ProfileAttribute::GetPathForTask(int tid, std::string* path) const {
    ...
    std::string subgroup;
    if (!controller_->GetProcessGroup(tid, &subgroup)) {
        return false;
    }

    if (subgroup.empty()) {
        *path = StringPrintf("%s/%s", controller_->GetPath(), file_name_.c_str());
    } else {
        *path = StringPrintf("%s/%s/%s", controller_->GetPath(), subgroup.c_str(),
                             file_name_.c_str());
    }
    return true;
}
...
int SetAttributeAction::ExecuteForTask(int tid) const {
    ...
    if (!attr_->GetPathForTask(tid, &path)) {
...
bool SetCgroupAction::AddTidToCgroup(int tid, int fd) {
    ...
    *ptr = '\0';
    ...
    if (write(fd, ptr, end - ptr) < 0) {
         ...
         */
        if (errno != ESRCH) {
            PLOG(ERROR) << "JoinGroup failed to write '" << ptr << "' (" << strerror(errno)
                        << "); fd=" << fd;
            return false;
        }
    }
...
int TaskProfile::ExecuteForProcess(uid_t uid, pid_t pid) const {
    for (const auto& element : elements_) {
        int result = element->ExecuteForProcess(uid, pid);
        if (result != 0) {
            return result;
        }
    }
...
int TaskProfile::ExecuteForTask(int tid) const {
    ...
    for (const auto& element : elements_) {
        int result = element->ExecuteForTask(tid);
        if (result != 0) {
            return result;
        }
    }
