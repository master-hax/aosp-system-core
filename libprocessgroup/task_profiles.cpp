...
bool SetAttributeAction::ExecuteForTask(int tid) const {
    ...
    // Call access() before WriteStringToFile() because otherwise the call to access() would
    // overwrite errno.
    const bool writeable = access(path.c_str(), W_OK) == 0;
    if (!writeable && optional_) {
        LOG(VERBOSE) << "Did not write '" << value_ << "' into " << path << " (not writeable)";
        return true;
    }
    if (!WriteStringToFile(value_, path)) {
        if (!optional_ || writeable) {
