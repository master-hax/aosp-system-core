...
std::optional<unsigned long> readUnsignedLong(const std::string& str) {
    char* end;
    const unsigned long result = strtoul(str.c_str(), &end, 10);
    if (end > str.c_str()) {
        return result;
    }
    return std::nullopt;
    ...
    for (Json::Value::ArrayIndex i = 0; i < profiles_val.size(); ++i) {
        ...
        for (Json::Value::ArrayIndex act_idx = 0; act_idx < actions.size(); ++act_idx) {
            ...
            if (action_name == "JoinCgroup") {
                ...
                std::optional<unsigned long> slack = readUnsignedLong(slack_string);
                if (slack) {
                ...
                if (SetSchedulerPolicyAction::isNormalPolicy(policy)) {
                    ...
                    std::optional<unsigned long> virtual_priority =
                            readUnsignedLong(priority_string);
                    if (virtual_priority) {
                        int priority;
