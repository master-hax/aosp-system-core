...
void ProfileVariable::Reset(const std::string& name, const std::string& value) {
    name_ = name;
    value_ = value;
}
...
bool TaskProfiles::Load(const CgroupMap& cg_map, const std::string& file_name) {
    ...
    for (Json::Value::ArrayIndex i = 0; i < profile_variables.size(); ++i) {
        ...
        if (inserted) {
            LOG(WARNING) << "Variable " << name
                         << " override value was not provided. Defaulting to value " << value;
        }
    }
            ...
            if (action_name == "JoinCgroup") {
                ...
                if (!attr_variable.empty()) {
                    ...
                    if (iter != variables_.end()) {
                        attr_value = iter->second.get()->value();
                    } else {
...
bool TaskProfiles::SetProfileVariables(
...
bool TaskProfiles::SetProfileVariable(const std::pair<std::string, std::string>& profile_variable) {
    bool success = true;
    auto [it, inserted] = variables_.try_emplace(
            profile_variable.first,
            std::make_unique<ProfileVariable>(profile_variable.first, profile_variable.second));
    if (!inserted) {
        PLOG(WARNING) << "Failed to save variable \"" << profile_variable.first << "\"";
        success = false;
    }
    return success;
}
