...
TaskProfiles TaskProfiles::instance;
std::once_flag TaskProfiles::initFlag;
...
int ProfileAttribute::GetPathForTask(int tid, std::string& path) const {
    ...
    if (subgroup.empty()) {
        path = StringPrintf("%s/%s",
            CgroupMap::GetControllerPath(controller),
            file_name.c_str());
    } else {
        path = StringPrintf("%s/%s/%s",
            CgroupMap::GetControllerPath(controller),
            subgroup.c_str(), file_name.c_str());
    }
    return 0;
}

int SetClampsAction::ExecuteForProcess(uid_t /*uid*/, pid_t /*pid*/) const {
    // TODO:
    LOG(WARNING) << "SetClampsAction::ExecuteForProcess is not supported";
    return -1;
}

int SetClampsAction::ExecuteForTask(int /*tid*/) const {
    // TODO:
...
int SetTimerSlackAction::ExecuteForTask(int tid) const {
    set_timerslack_ns(__sys_supports_timerslack == 1, tid, slack);
    return 0;
}

int SetAttributeAction::ExecuteForProcess(uid_t /*uid*/, pid_t pid) const {
...
int SetAttributeAction::ExecuteForTask(int tid) const {
    ...
    res = attr->GetPathForTask(tid, path);
    if (res < 0) {
...
SetCgroupAction::SetCgroupAction(const struct cgroup_controller *c,
                           const std::string& p) : controller(c), path(p) {
...
int SetCgroupAction::ExecuteForProcess(uid_t uid, pid_t pid) const {
    ...
    if (fd < 0) {
        ...
        std::string procs_path = CgroupMap::GetProcsFilePath(
            controller, path.c_str(), uid, pid);
...
int SetCgroupAction::ExecuteForTask(int tid) const {
    std::string tasks_path = CgroupMap::GetTasksFilePath(controller, path.c_str());
    int res;
...
int TaskProfiles::Load(const CgroupMap& cg_map) {
    ...
    if (!android::base::ReadFileToString(TASK_PROFILE_DB_FILE, &json_doc)) {
        LOG(ERROR) << "Failed to read task profiles from "
                   << TASK_PROFILE_DB_FILE;
        return -1;
    }
    ...
    if (!reader.parse(json_doc, root)) {
        LOG(ERROR) << "Failed to parse task profiles: "
...
void TaskProfiles::Init() {
    ...
    int ret = instance.Load(CgroupMap::GetInstance());
    if (ret < 0) {
       LOG(ERROR) << "TaskProfiles::Load called for ["
                  << getpid() << "] returns " << ret;
    }
}

const TaskProfile* TaskProfiles::GetProfile(const std::string& name) const {
    auto iter = profiles.find(name);
