...
SetCgroupAction::SetCgroupAction(const CgroupController& c, const std::string& p, bool cache_fds)
...
TaskProfiles& TaskProfiles::GetInstance(bool cache_fds) {
    // Deliberately leak this object to avoid a race between destruction on
    // process exit and concurrent access from another thread.
    static auto* instance = new TaskProfiles(cache_fds);
    return *instance;
}

TaskProfiles::TaskProfiles(bool cache_fds) {
    // load system task profiles
    if (!Load(CgroupMap::GetInstance(), TASK_PROFILE_DB_FILE, cache_fds)) {
    ...
    if (!access(TASK_PROFILE_DB_VENDOR_FILE, F_OK) &&
        !Load(CgroupMap::GetInstance(), TASK_PROFILE_DB_VENDOR_FILE, cache_fds)) {
        LOG(ERROR) << "Loading " << TASK_PROFILE_DB_VENDOR_FILE << " for [" << getpid()
                   << "] failed";
    }
}

bool TaskProfiles::Load(const CgroupMap& cg_map, const std::string& file_name, bool cache_fds) {
    std::string json_doc;
    ...
    for (Json::Value::ArrayIndex i = 0; i < profiles_val.size(); ++i) {
        ...
        for (Json::Value::ArrayIndex act_idx = 0; act_idx < actions.size(); ++act_idx) {
            ...
            if (action_name == "JoinCgroup") {
                ...
                auto controller = cg_map.FindController(controller_name);
                if (controller.HasValue()) {
                    profile->Add(std::make_unique<SetCgroupAction>(controller, path, cache_fds));
                } else {
