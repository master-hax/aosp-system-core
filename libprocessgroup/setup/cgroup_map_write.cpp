...
static bool ActivateController(const format::CgroupController* controller) {
    std::string subtree_control_path = std::string(controller->path()) + "/cgroup.subtree_control";
    unique_fd fd(TEMP_FAILURE_RETRY(open(subtree_control_path.c_str(), O_WRONLY | O_CLOEXEC, 0)));
    if (fd < 0) {
        PLOG(ERROR) << "open() failed for " << subtree_control_path;
        return false;
    }

    std::string control_command = std::string("+") + controller->name();
    int ret = TEMP_FAILURE_RETRY(write(fd, control_command.c_str(), control_command.length()));
    if (ret < 0) {
        PLOG(ERROR) << "write() failed for " << subtree_control_path;
        return false;
    }
...
static void MergeCgroupToDescriptors(std::map<std::string, CgroupDescriptor>* descriptors,
    ...
    if (!cgroup["NeedsActivation"].isNull() && cgroup["NeedsActivation"].asBool())
    ...
    if (controller->version() == 2) {
        ...
        if (!strcmp(controller->name(), CGROUPV2_CONTROLLER_NAME)) {
            ...
            if ((controller->flags() & CGROUPRC_CONTROLLER_FLAG_NEEDS_ACTIVATION) &&
                ...
                LOG(ERROR) << "Failed to activatre controller " << controller->name();
                return false;
            }
        }
    } else {
