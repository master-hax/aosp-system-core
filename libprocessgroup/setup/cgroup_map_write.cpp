...
static ActivationResult MountV2CgroupController(const CgroupDescriptor& descriptor) {
    ...
    if (!ChangeDirModeAndOwner(controller->path(), descriptor.mode(), descriptor.uid(),
                               descriptor.gid())) {
        LOG(ERROR) << "Change or ownership or mode failed for " << controller->name() << " cgroup";
        return ActivationResult::kError;
    }

    return ActivationResult::kSuccess;
}

static ActivationResult ActivateV2CgroupController(const CgroupDescriptor& descriptor) {
    const format::CgroupController* controller = descriptor.controller();
...
static ActivationResult MountV1CgroupController(const CgroupDescriptor& descriptor) {
    const format::CgroupController* controller = descriptor.controller();
    ...
    return res == 0 ? ActivationResult::kSuccess : ActivationResult::kNoSuchController;
}
...
static bool SetupCgroup(const CgroupDescriptor& descriptor) {
    ...
    switch (result) {
        case ActivationResult::kError:
            return false;
        case ActivationResult::kSuccess:
            return true;
        case ActivationResult::kNoSuchController:
            break;
    }
