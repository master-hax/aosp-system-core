...
static bool ReadDescriptorsFromFile(const std::string& file_name,
    ...
    if (force_memcg_v2 && !memcgv2_present) {
        ...
        MergeCgroupToDescriptors(descriptors, memcgv2, "memory",
            root_path.empty() ? "/sys/fs/cgroup" : root_path, 2);
    }
...
static bool ReadDescriptors(std::map<std::string, CgroupDescriptor>* descriptors) {
    ...
    if (force_memcg_v2 && MEMCGDisabled().value_or(false)) {
        LOG(WARNING) << "Memcg forced to v2 hierarchy while memcg is disabled by kernel command line!";
    }
    if (force_memcg_v2 && !FreezerOnV2(descriptors)) {
        LOG(WARNING) << "Memcg forced to v2 hierarchy while freezer is not enabled on the v2 hierarchy! "
                     << "Vendor has modified Android, and kernel memory use will be higher than intended.";
    }
    if (force_memcg_v2 && kernelVersion() < std::make_tuple(6, 1)) {
        LOG(WARNING) << "Memcg forced to v2 hierarchy on a kernel < 6.1. "
                     << "This will work, but performance will suffer due to lack of backported patches.";
    }
    ...
    if (controller->flags() & CGROUPRC_CONTROLLER_FLAG_NEEDS_ACTIVATION) {
        ...
        if (!base::WriteStringToFile(str, path)) {
            if (IsOptionalController(controller)) {
                PLOG(INFO) << "Failed to activate optional controller " << controller->name() << " at " << path;
                return true;
            }
