...
[[deprecated("Retained to support old vendor images which have not yet updated their libprocessgroup")]]
static bool WriteRcFile(const CgroupDescriptorMap& descriptors) {
    struct CgroupFile {
        uint32_t version_;
    ...
    android::base::unique_fd fd(TEMP_FAILURE_RETRY(open(CGROUPS_RC_PATH,
    ...
    for (const auto& [name, descriptor] : descriptors) {
        ret = TEMP_FAILURE_RETRY(
                write(fd, descriptor.controller(), sizeof(CgroupController)));
        if (ret < 0) {
            PLOG(ERROR) << "write() failed for " << CGROUPS_RC_PATH;
            return false;
        }
...
bool CgroupSetup() {
    ...
    // Generate CGROUPS_RC_FILE for old vendor images. This is no longer used by libprocessgroup,
