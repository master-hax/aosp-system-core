...
bool Cgroupv2HierarchyActive() {
    std::ifstream is("/proc/self/cgroup");
    std::string line;

    while (std::getline(is, line)) {
        LOG(INFO) << "Read " << line;
        if (line.compare(0, 2, "0:") == 0) {
            return true;
        }
    }
    return false;
...
TEST_P(SetAttributeFixture, SetAttribute) {
    if (!Cgroupv2HierarchyActive()) {
        LOG(INFO) << "Skipping this test since the cgroup v2 hierarchy is not active";
        GTEST_SKIP();
    }
