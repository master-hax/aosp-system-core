...
// Return json_name of the field. If it is not set, return the name of the field.
        ...
        if (field_descriptor->is_repeated()) {
            ...
            if (json_value.size() != static_cast<size_t>(fields.size())) {
                *error << base::Join(*path, ".") << ": JSON list has size " << json_value.size()
                       << " but message has size " << fields.size()
...
bool EqReformattedJson(const std::string& json, google::protobuf::Message* scratch_space,
    ...
    if (old_json != new_json) {
        std::stringstream ss;
        ss << "Formatted JSON tree does not match source. Possible reasons include: \n"
              ...
              "- JSON keys are re-formatted to be lowerCamelCase. To fix, define json_name "
              "option\n"
