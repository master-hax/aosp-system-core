...
// If Prototype field name as keys while json_name is present, AllFieldsAreKnown should return
// false.
...
// AllFieldsAreKnown should return false. Although the lowerCamelCase name is a valid
// key accepted by Protobuf's JSON parser, we explicitly disallow the behavior.
...
TEST_F(EmbeddedJsonKeyTest, BazQux) {
    auto object = TestEmbeddedError("{\"no_json_name\": {\"bazQux\": \"test\"}}", "bazQux");
    ASSERT_TRUE(object.ok()) << object.error();
...
INSTANTIATE_TEST_SUITE_P(, ScalarTestError, ::testing::ValuesIn(gScalarTestErrorParams));
