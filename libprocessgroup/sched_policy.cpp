    ...
    switch (policy) {
        case SP_BACKGROUND:
            return SetTaskProfiles(
                    tid, std::vector<std::string>{TP_HighEnergySaving, TP_ProcessCapacityLow});
        case SP_FOREGROUND:
        case SP_AUDIO_APP:
        case SP_AUDIO_SYS:
            return SetTaskProfiles(
                    tid, std::vector<std::string>{TP_HighPerformance, TP_ProcessCapacityHigh});
        case SP_TOP_APP:
            return SetTaskProfiles(
                    tid, std::vector<std::string>{TP_MaxPerformance, TP_ProcessCapacityMax});
        case SP_SYSTEM:
            return SetTaskProfiles(tid, std::vector<std::string>{TP_ServiceCapacityLow});
        case SP_RESTRICTED:
            return SetTaskProfiles(tid, std::vector<std::string>{TP_ServiceCapacityRestricted});
        default:
            break;
    }
...
int set_sched_policy(int tid, SchedPolicy policy) {
    ...
    switch (policy) {
        case SP_BACKGROUND:
            return SetTaskProfiles(
                    tid, std::vector<std::string>{TP_HighEnergySaving, TP_TimerSlackHigh});
        case SP_FOREGROUND:
        case SP_AUDIO_APP:
        case SP_AUDIO_SYS:
            return SetTaskProfiles(
                    tid, std::vector<std::string>{TP_HighPerformance, TP_TimerSlackNormal});
        case SP_TOP_APP:
            return SetTaskProfiles(
                    tid, std::vector<std::string>{TP_MaxPerformance, TP_TimerSlackNormal});
        case SP_RT_APP:
            return SetTaskProfiles(
                    tid, std::vector<std::string>{TP_RealtimePerformance, TP_TimerSlackNormal});
        default:
            return SetTaskProfiles(tid, std::vector<std::string>{TP_TimerSlackNormal});
    }
