...
int set_cpuset_policy(pid_t tid, SchedPolicy policy) {
    ...
    switch (policy) {
        ...
        case SP_FOREGROUND_MW:
            return SetTaskProfiles(tid, {"CPUSET_SP_FOREGROUND_MW"}, true) ? 0 : -1;
        default:
            break;
    }
...
int set_sched_policy(pid_t tid, SchedPolicy policy) {
    ...
    switch (policy) {
        ...
        case SP_RT_APP:
            ...
            break;
        case SP_FOREGROUND_MW:
            SLOGD("MW  tid %d (%s)", tid, thread_name);
            break;
        default:
            SLOGD("??? tid %d (%s)", tid, thread_name);
            break;
    ...
    switch (policy) {
        ...
        case SP_FOREGROUND_MW:
            return SetTaskProfiles(tid, {"SCHED_SP_FOREGROUND_MW"}, true) ? 0 : -1;
        default:
...
static int get_sched_policy_from_group(const std::string& group, SchedPolicy* policy) {
    ...
    } else if (group == "foreground_mw") {
        *policy = SP_FOREGROUND_MW;
    } else {
        errno = ERANGE;
        return -1;
    }
...
const char* get_sched_policy_name(SchedPolicy policy) {
    ...
    static const char* const kSchedPolicyNames[] = {
            ...
            [SP_RT_APP] = "rt",     [SP_RESTRICTED] = "rs", [SP_FOREGROUND_MW] = "mw",
    };
