    ...
    switch (policy) {
        case SP_BACKGROUND:
            return SetTaskProfiles(
                    tid, std::vector<std::string>{"HighEnergySaving", "ProcessCapacityLow", "TimerSlackHigh"});
        case SP_FOREGROUND:
        case SP_AUDIO_APP:
        case SP_AUDIO_SYS:
            return SetTaskProfiles(
                    tid, std::vector<std::string>{"HighPerformance", "ProcessCapacityHigh", "TimerSlackNormal"});
        case SP_TOP_APP:
            return SetTaskProfiles(
                    tid, std::vector<std::string>{"MaxPerformance", "ProcessCapacityMax", "TimerSlackNormal"});
        case SP_SYSTEM:
            return SetTaskProfiles(tid, std::vector<std::string>{"ServiceCapacityLow", "TimerSlackNormal"});
        case SP_RESTRICTED:
            return SetTaskProfiles(tid, std::vector<std::string>{"ServiceCapacityRestricted", "TimerSlackNormal"});
        default:
            break;
    }
...
int set_sched_policy(int tid, SchedPolicy policy) {
    ...
    switch (policy) {
        case SP_BACKGROUND:
            return SetTaskProfiles(
                    tid, std::vector<std::string>{"HighEnergySaving", "TimerSlackHigh"});
        case SP_FOREGROUND:
        case SP_AUDIO_APP:
        case SP_AUDIO_SYS:
            return SetTaskProfiles(
                    tid, std::vector<std::string>{"HighPerformance", "TimerSlackNormal"});
        case SP_TOP_APP:
            return SetTaskProfiles(
                    tid, std::vector<std::string>{"MaxPerformance", "TimerSlackNormal"});
        case SP_RT_APP:
            return SetTaskProfiles(
                    tid, std::vector<std::string>{"RealtimePerformance", "TimerSlackNormal"});
        default:
            return SetTaskProfiles(tid, std::vector<std::string>{"TimerSlackNormal"});
    }
...
static int getCGroupSubsys(int tid, const char* subsys, std::string& subgroup) {
    ...
    if (!controller->GetProcessGroup(tid, &subgroup)) {
