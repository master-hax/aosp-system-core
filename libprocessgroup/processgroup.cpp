...
static std::vector<std::string_view> StringVectorToSpan(const std::vector<std::string>& v) {
    std::vector<std::string_view> s;
    s.reserve(v.size());
    for (int i = 0; i < v.size(); ++i) {
        s.emplace_back(std::string_view(v[i]));
    }
    return s;
}

bool SetProcessProfiles(uid_t uid, pid_t pid, const std::vector<std::string>& profiles) {
    return TaskProfiles::GetInstance().SetProcessProfiles(uid, pid, StringVectorToSpan(profiles),
...
bool SetProcessProfiles(uid_t uid, pid_t pid, std::initializer_list<std::string_view> profiles) {
    return TaskProfiles::GetInstance().SetProcessProfiles(
            uid, pid, std::span<const std::string_view>(profiles), &TaskProfile::ExecuteForProcess,
            false);
}
...
bool SetTaskProfiles(int tid, const std::vector<std::string>& profiles, bool use_fd_cache) {
    return TaskProfiles::GetInstance().SetTaskProfiles(tid, StringVectorToSpan(profiles),
                                                       &TaskProfile::ExecuteForTask, use_fd_cache);
}
