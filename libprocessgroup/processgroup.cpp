...
bool IsCgroupFileSystem(const std::string& system) {
    return CgroupMap::IsCgroupFileSystem(system);
}
...
bool CgroupGetControllerPath(const std::string& cgroup_name, std::string* path) {
    ...
    if (path == nullptr || controller == nullptr) return false;
...
bool CgroupGetAttributePath(const std::string& attr_name, std::string* path) {
    ...
    if (path == nullptr || attr == nullptr) return false;

    *path = StringPrintf("%s/%s", attr->GetController()->GetPath(), attr->GetFileName().c_str());
    return true;
}
