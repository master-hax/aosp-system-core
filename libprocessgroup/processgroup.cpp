...
bool CgroupGetAttributePath(const std::string& attr_name, std::string& path) {
    ...
    path = StringPrintf("%s/%s",
        CgroupMap::GetControllerPath(attr->GetController()),
...
static bool isMemoryCgroupSupported() {
    ...
    static bool memcg_supported =
        (CgroupMap::GetInstance().FindController("memory") != nullptr);

    return memcg_supported;
}

int SetProcessProfiles(uid_t uid, pid_t pid,
...
void removeAllProcessGroups()
{
    ...
    for (std::string cgroup_root_path : cgroups) {
        std::unique_ptr<DIR, decltype(&closedir)> root(
            opendir(cgroup_root_path.c_str()), closedir);
        if (root == NULL) {
            ...
            while ((dir = readdir(root.get())) != nullptr) {
                ...
                auto path = StringPrintf("%s/%s", cgroup_root_path.c_str(),
                    dir->d_name);
...
static bool SetProcessGroupValue(int tid, const std::string& attr_name,
                                 int64_t value) {
