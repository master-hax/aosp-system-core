...
class ProcessGroup {
  public:
    ...
    // Return positive number and sets pid = next pid in process group on success
    // Returns 0 if there are no pids left in the process group
    // Returns -errno if an error was encountered
    int GetOneAppProcess(pid_t* pid);

  private:
    // Returns positive number of byted filled on success
...
static int doKillProcessGroupOnce(uid_t uid, int initialPid, int signal) {
    ...
    if (!process_group.Open(uid, initialPid)) {
        PLOG(WARNING) << "failed to open process group uid " << uid << " pid " << initialPid;
        return -errno;
    }
...
static int killProcessGroup(uid_t uid, int initialPid, int signal, int retries) {
    ...
    if (processes < 0) {
        PLOG(ERROR) << "Error encountered killing process group uid " << uid << " pid "
                    << initialPid;
        return -1;
    }
    ...
    if (retries == 0) {
        ...
        // Reporting anything regarding the number of 'processes' here does not make sense.
        return 0;
    }
    ...
    if (processes == 0) {
        ...
        LOG(ERROR) << "Failed to kill process group uid " << uid << " pid " << initialPid << " in "
                   << static_cast<int>(ms) << "ms, " << processes << " processes remain";
        return -1;
    }
}
