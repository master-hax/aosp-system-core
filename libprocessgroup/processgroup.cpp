...
static void pollOnEvents(int events_fd, const std::chrono::steady_clock::time_point& until) {
    ...
    int ret = 0;
    if (start < until)
        ret = TEMP_FAILURE_RETRY(poll(&fds, 1, toMillisec(until - start).count()));
...
// Returns 1 if the cgroup is populated
// Returns 0 if the cgroup is not populated
// Returns -1 upon error
static int cgroupIsPopulated(int events_fd) {
...
static int KillProcessGroup(
    ...
    do {
        int populated;
        while ((populated = cgroupIsPopulated(events_fd.get())) == 1 &&
    ...
    } while (ret && errno == EBUSY && !once && std::chrono::steady_clock::now() < until);
