...
// Collect all the process group IDs and process IDs associated with the cgroup identified by the
// function arguments.
static CollectPidsResult CollectPids(const char* hierarchy_root, uid_t uid, int initialPid) {
    ...
    const std::string path =
            StringPrintf("%s%s", ConvertUidPidToPath(hierarchy_root, uid, initialPid).c_str(),
                         PROCESSGROUP_CGROUP_PROCS_FILE);
    std::unique_ptr<FILE, decltype(&fclose)> fd(fopen(path.c_str(), "re"), fclose);
    if (!fd) {
        if (errno == ENOENT) {
            // This happens when the process is already dead or if, as the result of a bug, it has
            // been migrated to another cgroup and its cgroup has been removed.
    ...
    while (fscanf(fd.get(), "%d\n", &pid) == 1) {
        if (pid <= 0) {
            // Should never happen.
            LOG(WARNING) << "Invalid PID in cgroup.procs: " << pid;
            continue;
        }
        result.processes++;
...
// Returns -1 on error
static int DoKillProcessGroupOnce(const char* hierarchy_root, uid_t uid, int initialPid,
                                  int signal) {
    // We separate all of the pids in the cgroup into those pids that are also the leaders of
    // process groups (stored in the pgids set) and those that are not (stored in the pids set).

    const CollectPidsResult result = CollectPids(hierarchy_root, uid, initialPid);
