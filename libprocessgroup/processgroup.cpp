...
static int setProcessGroupValue(uid_t uid, int pid, const char *fileName, long long value) {
    char path[PROCESSGROUP_MAX_PATH_LEN] = {0};
    int ret = 0;
    ...
    int len = snprintf(data, sizeof(data), "%lld", value);
...
int setProcessGroupSoftLimit(uid_t uid, int pid, long long soft_limit_in_bytes) {
    return setProcessGroupValue(uid, pid, "/memory.soft_limit_in_bytes", soft_limit_in_bytes);
}

int setProcessGroupLimit(uid_t uid, int pid, long long limit_in_bytes) {
