...
CgroupController::CgroupController(uint32_t version, const std::string& name, const std::string& path) {
    version_ = version;
...
CgroupDescriptor::CgroupDescriptor(uint32_t version, const std::string& name, const std::string& path, mode_t mode,
    const std::string& uid, const std::string& gid) : controller_(version, name, path), mode_(mode),
    uid_(uid), gid_(gid) {}
...
void CgroupMap::Print() {
    ...
    for (int i = 0; i < cg_file_data_->controller_count_; i++, controller++) {
        LOG(INFO) << "\t" << controller->name() << " ver " << controller->version()
...
bool CgroupMap::ReadDescriptors(std::map<std::string, CgroupDescriptor>* descriptors) {
    ...
    for (Json::Value::ArrayIndex i = 0; i < cgroups.size(); ++i) {
        ...
        descriptors->emplace(std::make_pair(name,
            CgroupDescriptor(1, name,
                cgroups[i]["Path"].asString(),
                cgroups[i]["Mode"].asInt(),
                cgroups[i]["UID"].asString(),
                cgroups[i]["GID"].asString())));
    }

    Json::Value cgroups2 = root["Cgroups2"];
    descriptors->emplace(std::make_pair(CGROUPV2_CONTROLLER_NAME,
        CgroupDescriptor(2, CGROUPV2_CONTROLLER_NAME,
            cgroups2["Path"].asString(),
            cgroups2["Mode"].asInt(),
            cgroups2["UID"].asString(),
