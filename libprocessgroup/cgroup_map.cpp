...
static bool do_mkdir(const std::string& path, mode_t mode, const std::string& uid,
...
bool CgroupMap::SetupCgroup(const CgroupDescriptor& descriptor) {
    const CgroupController* controller = descriptor.controller();

    // mkdir <path> [mode] [owner] [group]
    if (!do_mkdir(controller->path(), descriptor.mode(), descriptor.uid(), descriptor.gid())) {
...
bool CgroupMap::SetupCgroups() {
    ...
    if (!CgroupMap::ReadDescriptors(&descriptors)) {
    ...
    for (const auto& [name, descriptor] : descriptors) {
        if (!SetupCgroup(descriptor)) {
            PLOG(ERROR) << "Failed to setup " << name << " cgroup";
            return false;
        }
    }

    // mkdir <CGROUPS_RC_DIR> 0711 system system
    if (!do_mkdir(CGROUPS_RC_DIR, 0711, "system", "system")) {
    ...
    if (!CgroupMap::WriteRcFile(descriptors)) {
