...
void TestNewlineSplitter(const std::string& input,
  ...
  SplitByLines(input.c_str(), logger_function, nullptr);

  EXPECT_EQ(expected_output, output);
}

void TestLogdChunkSplitter(const std::string& tag, const std::string& input,
                           const std::vector<std::string>& expected_output) {
  std::vector<std::string> output;
  ...
  SplitByLogdChunks(MAIN, ERROR, tag.c_str(), "file", 1000, input.c_str(), logger_function,
                    nullptr);
