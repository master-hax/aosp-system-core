...
TEST(process_info, process_info_smoke) {
  android::base::ProcessInfo self;
  ASSERT_TRUE(android::base::GetProcessInfo(gettid(), &self));
...
TEST(process_info, process_info_proc_pid_fd_smoke) {
  android::base::ProcessInfo self;
  int fd = open(android::base::StringPrintf("/proc/%d", gettid()).c_str(),
                O_DIRECTORY | O_RDONLY);
  ASSERT_NE(-1, fd);
  ASSERT_TRUE(android::base::GetProcessInfoFromProcPidFd(fd, &self));
#if defined(__LP64__)
  ASSERT_EQ("libbase_test64", self.name);
#else
  ASSERT_EQ("libbase_test32", self.name);
#endif
  ASSERT_EQ(gettid(), self.tid);
  ASSERT_EQ(getpid(), self.pid);
  ASSERT_EQ(getppid(), self.ppid);
...
TEST(process_info, process_tids_smoke) {
  ...
  std::thread([main_tid]() {
...

    {
      ...
      ASSERT_TRUE(android::base::GetProcessTids(getpid(), &vec));
    ...
    {
      std::set<pid_t> set;
      ASSERT_TRUE(android::base::GetProcessTids(getpid(), &set));
