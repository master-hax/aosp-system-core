...
TEST(process_info, process_info_smoke) {
  ...
  ASSERT_TRUE(android::base::GetProcessInfo(&self, gettid()));
...
TEST(process_info, process_info_proc_pid_fd_smoke) {
  ...
  ASSERT_NE(-1, fd);
  ASSERT_TRUE(android::base::GetProcessInfoFromProcPidFd(&self, fd));
  ASSERT_EQ(gettid(), self.tid);
  ASSERT_EQ(getpid(), self.pid);
  ASSERT_EQ(getppid(), self.ppid);
  ASSERT_EQ(getuid(), self.uid);
...
TEST(process_info, process_tids_smoke) {
  ...
  std::thread([main_tid]() {
...

    {
      ...
      ASSERT_TRUE(android::base::GetProcessTids(&vec, getpid()));
    ...
    {
      std::set<pid_t> set;
      ASSERT_TRUE(android::base::GetProcessTids(&set, getpid()));
