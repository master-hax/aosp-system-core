...
TEST(result, result_success) {
  ...
  EXPECT_EQ(Nothing(), *result);
  EXPECT_EQ(Nothing(), result.value());
}

TEST(result, result_success_rvalue) {
  // Nothing() doesn't actually create a Result<Nothing> object, but rather an object that can be
  // implicitly constructed into a Result<Nothing> object.

  auto MakeRvalueSuccessResult = []() -> Result<Nothing> { return Nothing(); };
  ASSERT_TRUE(MakeRvalueSuccessResult());
  ASSERT_TRUE(MakeRvalueSuccessResult().has_value());

  EXPECT_EQ(Nothing(), *MakeRvalueSuccessResult());
  EXPECT_EQ(Nothing(), MakeRvalueSuccessResult().value());
}

TEST(result, result_error) {
  Result<Nothing> result = Error() << "failure" << 1;
  ASSERT_FALSE(result);
...
TEST(result, result_error_rvalue) {
  ...
  auto MakeRvalueErrorResult = []() -> Result<Nothing> { return Error() << "failure" << 1; };
...
TEST(result, result_errno_error) {
  ...
  errno = test_errno;
  Result<Nothing> result = ErrnoError() << "failure" << 1;
...
TEST(result, result_errno_error_no_text) {
  constexpr int test_errno = 6;
  errno = test_errno;
  Result<Nothing> result = ErrnoError();
...
TEST(result, result_error_from_other_result) {
  ...
  Result<Nothing> result = Error() << error_text;
...
TEST(result, result_errno_error_through_ostream) {
  ...
  constexpr int test_errno = 6;
  errno = 6;
  Result<Nothing> result = ErrnoError() << error_text;
...
TEST(result, result_result_with_success) {
  auto return_result_result_with_success = []() -> Result<Result<Nothing>> {
...
TEST(result, result_result_with_failure) {
  auto return_result_result_with_error = []() -> Result<Result<Nothing>> {
    return Result<Nothing>(ResultError("failure string", 6));
  };
