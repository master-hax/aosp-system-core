...
// Logs a message to logcat with the system log ID on Android otherwise to
// stderr. If the severity is FATAL it also causes an abort.
#define SLOG(severity)                                                     \
  ::android::base::LogMessage(__FILE__, __LINE__, ::android::base::SYSTEM, \
                              ::android::base::severity, -1).stream()
...
class LogMessage {
 public:
  ...
  static void LogLineLowStack(const char* file, unsigned int line, LogId id,
                              LogSeverity severity, const char* msg);
