...
// Variant of LOG that logs when verbose logging is enabled for a module. For
// example:
//     VLOG(jni) << "A JNI operation was performed";
#define VLOG(module)                                                   \
  if (VLOG_IS_ON(module)) {                                            \
    ::android::base::LogMessage(__FILE__, __LINE__, INFO, -1).stream() \
  }
...
#define CHECK_STROP(s1, s2, sense)                                         \
  if (UNLIKELY((strcmp(s1, s2) == 0) != sense)) {                          \
    LOG(::android::base::FATAL) << "Check failed: "                        \
                                << "\"" << s1 << "\""                      \
                                << (sense ? " == " : " != ") << "\"" << s2 \
                                << "\""                                    \
  }
...
#define CHECK_PTHREAD_CALL(call, args, what)                           \
  do {                                                                 \
    ...
    if (rc != 0) {                                                     \
      errno = rc;                                                      \
      PLOG(::android::base::FATAL) << #call << " failed for " << what; \
    }                                                                  \
  } while (false)
...
#define CHECK_CONSTEXPR(x, out, dummy)                                     \
  (UNLIKELY(!(x)))                                                         \
      ? (LOG(::android::base::FATAL) << "Check failed: " << #x out, dummy) \
      :
