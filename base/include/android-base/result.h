...
//
// Error and ErrnoError also accept the format string syntax of the fmblib (https://fmt.dev).
// Error("{} errors", num) is equivalent to Error() << num << " errors".
//
...
class Error {
 public:
  ...
  Error(const char* fmt, const Args&... args) : Error(false, 0, fmt::format(fmt, args...)) {}
  ...
  template <typename... Args>
  friend Error ErrnoError(const char* fmt, const Args&... args);
...
template <typename... Args>
inline Error ErrnoError(const char* fmt, const Args&... args) {
