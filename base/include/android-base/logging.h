...
namespace detail {
struct AbortOnDestruction {
  ~AbortOnDestruction() __attribute__((noreturn)) { abort(); }
  explicit operator bool() const { return false; }
};
}  // namespace detail
// Provides an expression that evaluates to the truthiness of `x`, automatically
// aborting if `c` is true.
#define ABORT_AFTER_LOG_EXPR_IF(c, x) \
  (((c) && ::android::base::detail::AbortOnDestruction()) || bool(x))
#else
#define ABORT_AFTER_LOG_FATAL
#define ABORT_AFTER_LOG_EXPR_IF(c, x) (bool(x))
#endif
#define ABORT_AFTER_LOG_FATAL_EXPR(x) ABORT_AFTER_LOG_EXPR_IF(1, x)
...
#define LOGGING_PREAMBLE(severity)                                                      \
  (WOULD_LOG(severity) &&                                                               \
   ABORT_AFTER_LOG_EXPR_IF((SEVERITY_LAMBDA(severity)) == ::android::base::FATAL, 1) && \
...
#define CHECK(x)                                                                \
  LIKELY((x)) || ABORT_AFTER_LOG_FATAL_EXPR(0) ||                               \
