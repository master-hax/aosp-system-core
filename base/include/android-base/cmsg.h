...
ssize_t ReceiveFileDescriptorVector(int sock, void* data, size_t len,
...
// Helper for ReceiveFileDescriptorVector that constructs a std::vector for you, e.g.:
...
ssize_t ReceiveFileDescriptors(int sock, void* data, size_t len, Args&&... received_fds) {
  ...
  return ReceiveFileDescriptorVector(sock, data, len, &fds);
}
