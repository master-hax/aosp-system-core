...
class unique_fd_impl final {
 ...
 private:
  ...
  static auto tag(int fd, void* tag_value, const T*) -> decltype(T::Tag(fd, tag_value), void()) {
    T::Tag(fd, tag_value);
  }

  static void tag(long, void*, const Closer*) {
    // No-op.
  }

  void tag(void* tag_value) {
    if (value_ != -1) {
      tag(value_, tag_value, static_cast<Closer*>(nullptr));
    }
  }
