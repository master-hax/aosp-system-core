...
TEST_F(NullFileTest, SetLength) {
  ...
  ASSERT_FALSE(f.SetLength(-1));
  ASSERT_EQ(EINVAL, errno);
  // ...but everything else is fine.
  ASSERT_TRUE(f.SetLength(0));
  ASSERT_TRUE(f.SetLength(128));
}
...
TEST_F(NullFileTest, GetLength) {
  ...
  ASSERT_EQ(static_cast<ssize_t>(content.size()),
            f.Write(content.data(), content.size(), 0));
  ASSERT_EQ(0, f.GetLength());
}
...
TEST_F(NullFileTest, Write) {
  ...
  ASSERT_EQ(-1, f.Write(content.data(), content.size(), -128));
  ASSERT_EQ(EINVAL, errno);
  // But you can write anywhere else...
  ASSERT_EQ(static_cast<ssize_t>(content.size()),
            f.Write(content.data(), content.size(), 0));
  ASSERT_EQ(static_cast<ssize_t>(content.size()),
            f.Write(content.data(), content.size(), 128));
