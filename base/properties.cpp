...
static void UpdateTimeSpec(timespec& ts, std::chrono::milliseconds relative_timeout,
  ...
  if (relative_timeout.count() < 0) {
    // Either there was an overflow when converting to milliseconds or we were passed a negative
    // value.  In any case, default to an infinite timeout
    relative_timeout = std::chrono::milliseconds::max();
  }
