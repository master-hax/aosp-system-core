...
#define aligned_alloca(alignment, size)                                      \
  ({                                                                         \
    char* alloc = static_cast<char*>(alloca(size + alignment - 1));          \
    uintptr_t misalignment = reinterpret_cast<uintptr_t>(alloc) % alignment; \
    if (misalignment) {                                                      \
      alloc += alignment - misalignment;                                     \
    }                                                                        \
    alloc;                                                                   \
  })
...
ssize_t SendFileDescriptorVector(int sockfd, const void* data, size_t len,
  ...
  char* cmsg_buf = aligned_alloca(alignof(struct cmsghdr), cmsg_space);
...
ssize_t ReceiveFileDescriptorVector(int sockfd, void* data, size_t len,
