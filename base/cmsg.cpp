...
ssize_t ReceiveFileDescriptors(int sockfd, void* data, size_t data_len,
  ...
  ssize_t rc = recvmsg(sockfd, &msg, MSG_TRUNC | MSG_CTRUNC | MSG_CMSG_CLOEXEC);
  if (rc == -1) {
    return -1;
  }
  ...
  if (fds->size() != received_fds.size()) {
    LOG(ERROR) << "received too " << (fds->size() > received_fds.size() ? "few" : "many")
               << " file descriptors, expected " << fds->size() << ", received "
               << received_fds.size();
    errno = fds->size() > received_fds.size() ? ENOMSG : ERANGE;
    return -1;
  }
