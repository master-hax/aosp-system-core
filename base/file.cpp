...
bool ReadFdToString(int fd, std::string* content, size_t max_size, milliseconds relative_timeout) {
  ...
  // Allow at least 5 seconds to retrieve continuously flowing content,
  // which should handily also deal with reading normal files Fully.
  static constexpr milliseconds zero(0);
  milliseconds timeout_threshold = std::min(relative_timeout - seconds(5), zero);
  ...
  for (;;) {
    ...
    if (!skip_poll) {
      ...
      if (rc == SOCKET_ERROR) {
        ...
        return remaining_timeout > timeout_threshold;
      }
#else
      auto rc = poll(&pfd, 1, std::max(remaining_timeout, zero).count());
      if ((rc == -1) && (errno == EINTR)) continue;
      if (rc < 0) return false;
      if (rc == 0) return remaining_timeout > timeout_threshold;
#endif
    }
    ...
    if (!skip_poll && (remaining_timeout <= timeout_threshold)) return false;
  }
}
