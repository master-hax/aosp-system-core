...
void LogdLogger::operator()(LogId id, LogSeverity severity, const char* tag,
  ...
  if (liblog_functions) {
    ...
    liblog_functions->__android_log_logd_logger(&log_message, message);
  } else {
...
void SetLogger(LogFunction&& logger) {
  ...
  if (liblog_functions) {
    ...
    liblog_functions->__android_log_set_logger([](const struct __android_log_message* log_message,
      ...
      function(log_id, severity, log_message->tag, log_message->file, log_message->line, message);
    });
...
void LogMessage::LogLine(const char* file, unsigned int line, LogSeverity severity, const char* tag,
  ...
  if (liblog_functions) {
    __android_log_message log_message = {
        sizeof(__android_log_message), LOG_ID_DEFAULT, priority, tag, file, line};
    liblog_functions->__android_log_write_log_message(&log_message, message);
  } else {
    if (tag == nullptr) {
      std::lock_guard<std::recursive_mutex> lock(TagLock());
      if (gDefaultTag == nullptr) {
