...
static void InitLoggingImpl(char* argv[], AbortFunction&& aborter) {
...
void InitLogging(char* argv[], LogFunction&& logger, AbortFunction&& aborter) {
  ...
  InitLoggingImpl(argv, std::move(aborter));
}

static void SetLogger(std::function<void(LogId, LogSeverity, const char*, const char*, unsigned int,
                                         const char*)>&& logger) {
  SetLogger([logger](const LoggerData& logger_data, const char* message) {
    auto log_id = log_id_tToLogId(logger_data.buffer_id);
    auto severity = PriorityToLogSeverity(logger_data.priority);

    logger(log_id, severity, logger_data.tag, logger_data.file, logger_data.line, message);
  });
}

// The logger API has been deprecated, but this InitLogging function must exist for prebuilts.
void InitLogging(char* argv[],
                 std::function<void(LogId, LogSeverity, const char*, const char*, unsigned int,
                                    const char*)>&& logger,
