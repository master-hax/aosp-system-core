...
static std::mutex& TagLock() {
  static auto& tag_lock = *new std::mutex();
  return tag_lock;
}
static std::string* default_tag;
std::string GetDefaultTag() {
  std::lock_guard<std::mutex> lock(TagLock());
...
void SetDefaultTag(const std::string& tag) {
  ...
  if (!tag.empty()) {
    default_tag = new std::string(tag);
  }
}
...
void LogMessage::LogLine(const char* file, unsigned int line, LogId id, LogSeverity severity,
  ...
  if (tag == nullptr) {
    std::lock_guard<std::mutex> lock(TagLock());
    if (default_tag == nullptr) {
      default_tag = new std::string(getprogname());
    }
    Logger()(id, severity, default_tag->c_str(), file, line, message);
  } else {
