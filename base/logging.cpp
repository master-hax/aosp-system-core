...
void DefaultAborter(const char* abort_message) {
  ...
  // stack-based buffer so that it can appear in tombstones / minidumps.
  // Choosing a small-ish buffer size since the tombstone will only display the
  // 256 bytes following the stack pointer. Breakpad will also only pick up the
  // first few kilobytes of each stack, so that will prevent this buffer from
