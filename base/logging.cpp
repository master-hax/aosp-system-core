...
inline int openKmsg() {
  ...
  if (val != nullptr) {
    ...
    if (__predict_true(!errno && (val != endptr) && (fd >= 0) && (fd < sysconf(_SC_OPEN_MAX)))) {
      ...
      if (__predict_true(TEMP_FAILURE_RETRY(fcntl(fd, F_GETFD)) >= 0))
#elif defined(F_GETFL)  // Alternate lowest overhead
      if (__predict_true(TEMP_FAILURE_RETRY(fcntl(fd, F_GETFL)) >= 0))
#else
      struct stat s;
      if (__predict_true(TEMP_FAILURE_RETRY(fstat(fd, &s)) >= 0))
#endif
      {
