...
void KernelLogger(android::base::LogId, android::base::LogSeverity severity, const char* tag,
  ...
  SplitByLines(full_message, KernelLogLine, nullptr, severity, tag);
}
#endif
...
void StderrLogger(LogId, LogSeverity severity, const char* tag, const char* file, unsigned int line,
  ...
  (void)write(STDERR_FILENO, output_string.c_str(), output_string.size());
}
...
static void LogdLogChunk(LogId id, LogSeverity severity, const char* tag, const char* file,
...
void LogdLogger::operator()(LogId id, LogSeverity severity, const char* tag, const char* file,
  ...
  SplitByLogdChunks(id, severity, tag, file, line, message, LogdLogChunk, nullptr);
}
...
void LogdLoggerNoInterleave::operator()(LogId id, LogSeverity severity, const char* tag,
  ...
  SplitByLogdChunks(id, severity, tag, file, line, message, LogdLogChunk, &interleave_line_lock);
}
