...
#if defined(__linux__)
static void KernelLogLine(android::base::LogId, android::base::LogSeverity severity,
                          const char* tag, const char*, unsigned int, const char* msg, int length) {
  // clang-format off
...
void KernelLogger(android::base::LogId log_id, android::base::LogSeverity severity, const char* tag,
                  const char* file, unsigned int line, const char* full_message) {
  SplitByLines(log_id, severity, tag, file, line, full_message, KernelLogLine, nullptr);
}
#endif

void StderrLogLine(LogId, LogSeverity severity, const char* tag, const char* file,
                   unsigned int line, const char* message, int length) {
  struct tm now;
  ...
  if (file != nullptr) {
    fprintf(stderr, "%s %c %s %5d %5" PRIu64 " %s:%u] %.*s\n", tag ? tag : "nullptr", severity_char,
            timestamp, getpid(), GetThreadId(), file, line, length, message);
  } else {
    fprintf(stderr, "%s %c %s %5d %5" PRIu64 " %.*s\n", tag ? tag : "nullptr", severity_char,
            timestamp, getpid(), GetThreadId(), length, message);
  }
}

void StderrLogger(LogId log_id, LogSeverity severity, const char* tag, const char* file,
                  unsigned int line, const char* full_message) {
  SplitByLines(log_id, severity, tag, file, line, full_message, StderrLogLine, nullptr);
}

void StderrLoggerNoInterleave(LogId log_id, LogSeverity severity, const char* tag, const char* file,
                              unsigned int line, const char* full_message) {
  static auto& interleave_line_lock = *new std::mutex();
  SplitByLines(log_id, severity, tag, file, line, full_message, StderrLogLine,
               &interleave_line_lock);
}
...
static void LogdLogLine(LogId id, LogSeverity severity, const char* tag, const char* file,
                        unsigned int line, const char* message, int length) {
  int32_t lg_id = LogIdTolog_id_t(id);
  int32_t priority = LogSeverityToPriority(severity);

  char modified_message[LOGGER_ENTRY_MAX_PAYLOAD];
  if (priority == ANDROID_LOG_FATAL && file != nullptr) {
    snprintf(modified_message, sizeof(modified_message), "%s:%u] %.*s", file, line, length,
             message);
    message = modified_message;
  } else if (length != -1) {
    snprintf(modified_message, sizeof(modified_message), "%.*s", length, message);
    message = modified_message;
  }
...
void LogdLogger::operator()(LogId id, LogSeverity severity, const char* tag, const char* file,
  ...
  SplitByLogdLines(id, severity, tag, file, line, message, LogdLogLine, nullptr);
}

LogdLoggerNoInterleave::LogdLoggerNoInterleave(LogId default_log_id)
    : default_log_id_(default_log_id) {}

void LogdLoggerNoInterleave::operator()(LogId id, LogSeverity severity, const char* tag,
  ...
  static auto& interleave_line_lock = *new std::mutex();
  SplitByLogdLines(id, severity, tag, file, line, message, LogdLogLine, &interleave_line_lock);
}
