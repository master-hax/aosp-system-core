...
#ifdef __ANDROID__
static const std::optional<LibLogFunctions>& GetLibLogFunctions() {
...
static LogSeverity PriorityToLogSeverity(android_LogPriority priority) {
  switch (priority) {
...
void LogdLogger::operator()(LogId id, LogSeverity severity, const char* tag,
  ...
  auto& liblog_functions = GetLibLogFunctions();
  if (liblog_functions) {
    __android_logger_data logger_data = {sizeof(__android_logger_data),     lg_id, priority, tag,
                                         static_cast<const char*>(nullptr), 0};
    liblog_functions->__android_log_logd_logger(&logger_data, log_message, nullptr);
  } else {
...
void SetLogger(LogFunction&& logger) {
  auto& liblog_functions = GetLibLogFunctions();
  if (liblog_functions) {
    auto* logger_function = new LogFunction(logger);
    auto release_function = [](void* data) { delete static_cast<LogFunction*>(data); };
    liblog_functions->__android_log_set_logger(
        [](const struct __android_logger_data* logger_data, const char* message, void* data) {
          auto log_id = log_id_tToLogId(logger_data->buffer_id);
          auto severity = PriorityToLogSeverity(logger_data->priority);

          auto& fn = *static_cast<LogFunction*>(data);

          fn(log_id, severity, logger_data->tag, logger_data->file, logger_data->line, message);
        },
        logger_function, release_function);
  } else {
...
void SetAborter(AbortFunction&& aborter) {
  auto& liblog_functions = GetLibLogFunctions();
  if (liblog_functions) {
    auto* abort_function = new AbortFunction(aborter);
    auto release_function = [](void* data) { delete static_cast<AbortFunction*>(data); };
    __android_log_set_aborter(
        [](const char* abort_message, void* data) {
          auto& fn = *static_cast<AbortFunction*>(data);
          fn(abort_message);
        },
        abort_function, release_function);
  } else {
    std::lock_guard<std::mutex> lock(LoggingLock());
...
void LogMessage::LogLine(const char* file, unsigned int line, LogSeverity severity, const char* tag,
  ...
  if (tag == nullptr) {
    ...
    if (liblog_functions) {
      __android_log_write_logger_data({sizeof(__android_logger_data), LOG_ID_DEFAULT, priority,
    ...
    if (liblog_functions) {
      __android_log_write_logger_data(
          {sizeof(__android_logger_data), LOG_ID_DEFAULT, priority, tag, file, line}, message);
    } else {
