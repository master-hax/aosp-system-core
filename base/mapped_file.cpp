...
MappedFile* MappedFile::FromFd(int fd, off64_t offset, size_t length, int prot) {
  ...
  return new MappedFile{static_cast<char*>(base), length, slop, handle};
#else
  void* base = mmap(nullptr, file_length, prot, MAP_SHARED, fd, file_offset);
  return (base == MAP_FAILED) ? nullptr : new MappedFile{static_cast<char*>(base), length, slop};
#endif
}
