...
bool WideToUTF8(const wchar_t* utf16, const size_t size, std::string* utf8) {
  ...
  // This uses &string[0] to get write-access to the entire string buffer which
  // may be assuming that the chars are all contiguous, but it seems to work and
  // saves us the hassle of using a temporary std::vector<char>.
...
bool UTF8ToWide(const char* utf8, const size_t size, std::wstring* utf16) {
  ...
  // This uses &string[0] to get write-access to the entire string buffer which
  // may be assuming that the chars are all contiguous, but it seems to work and
  // saves us the hassle of using a temporary std::vector<wchar_t>.
