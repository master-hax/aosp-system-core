...
TEST_P(CmsgTest, stream_fd_association) {
  ...
  ASSERT_EQ(2, SendFileDescriptors(send.get(), "bc", 2, std::move(fd1)));
  ASSERT_EQ(1, SendFileDescriptors(send.get(), "d", 1, std::move(fd2)));
  char buf[2];
  ASSERT_EQ(2, TEMP_FAILURE_RETRY(read(recv.get(), buf, 2)));
  ASSERT_EQ(0, memcmp(buf, "ab", 2));

  unique_fd received1;
  ssize_t rc = ReceiveFileDescriptors(recv.get(), buf, 1, &received1);
  ASSERT_EQ(1, rc);
  ASSERT_EQ('c', buf[0]);
  ASSERT_EQ(-1, received1.get());
...
TEST_P(CmsgTest, separate_fds_no_coalescing) {
  ...
  ASSERT_EQ(1, ReceiveFileDescriptors(recv.get(), buf, 2, &received2, &received1));
  ASSERT_EQ(ino2, GetInode(received2.get()));
  ASSERT_EQ(-1, received1.get());
}
