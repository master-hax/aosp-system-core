...
#ifndef BASE_RANDOM_ACCESS_FILE_TEST_H
#define BASE_RANDOM_ACCESS_FILE_TEST_H
...
using android::base::RandomAccessFile;
...
class RandomAccessFileTest : public testing::Test {
 protected:
  ...
  void TestReadContent(const std::string& content, RandomAccessFile* file) {
    ...
    ASSERT_EQ(-1, file->Read(buf.get(), 0, -123));
    ...
    ASSERT_EQ(static_cast<ssize_t>(short_request),
              file->Read(buf.get(), short_request, 0));
  ...
  void TestSetLength() {
    ...
    ASSERT_EQ(static_cast<ssize_t>(content.size()),
              file->Write(content.data(), content.size(), 0));
    ASSERT_EQ(static_cast<off_t>(content.size()), file->GetLength());

    // Can't give a file a negative length.
    ASSERT_FALSE(file->SetLength(-123));
    ASSERT_EQ(EINVAL, errno);

    // Can truncate the file.
    off_t new_length = 2;
    ASSERT_TRUE(file->SetLength(new_length));
  ...
  void TestWrite() {
    ...
    ASSERT_EQ(-1, file->Write(content.data(), 0, -123));
    ASSERT_EQ(EINVAL, errno);
...
#endif  // BASE_RANDOM_ACCESS_FILE_TEST_H
