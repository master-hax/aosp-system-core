...
TEST_F(MappedFileTest, DestructorDoesNotReclose) {
  FILE* f = tmpfile();
  CHECK(f != nullptr);
  errno = 0;
  {
...
TEST_F(MappedFileTest, WriteMappedReadWrite) {
  ...
  EXPECT_EQ(-1, file.Write(kContent.c_str(), 0, -123));
  ...
  ASSERT_EQ(-1, file.Write(kContent.c_str(), 1, 1));
