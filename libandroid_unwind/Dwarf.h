...
class DwarfBase {
 ...
 protected:
  ...
  std::unordered_map<uint64_t, std::unordered_map<uint8_t, DwarfLocation>> cie_regs_;
};
...
class Dwarf : public DwarfBase {
 public:
  ...
  bool EvalCfa(uint64_t pc) {
    ...
    return EvalCfa(pc, &cie, &fde);
  }

  bool EvalCfa(uint64_t pc, DwarfCIE* cie, DwarfFDE* fde) {
    DwarfCfa<AddressType> cfa(&memory_, cie, fde);

    auto reg_entry = cie_regs_.find(fde->cie_offset);
    if (reg_entry == cie_regs_.end() || g_LoggingEnabled) {
      if (!cfa.Eval(pc, cie->cfa_instructions_offset, cie->cfa_instructions_end)) {
        return false;
      }
      cie_regs_[fde->cie_offset] = cfa.regs();
    }
    cfa.set_cie_regs(&cie_regs_[fde->cie_offset]);
    return cfa.Eval(pc, fde->cfa_instructions_offset, fde->cfa_instructions_end);
  }
