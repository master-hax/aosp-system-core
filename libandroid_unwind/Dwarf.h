...
class Dwarf : public DwarfBase {
 public:
  ...
  bool GetCfaLocationInfo(uint64_t pc) {
    uint64_t fde_offset;
    ...
    return GetCfaLocationInfo(pc, &cie, &fde);
  }

  bool GetCfaLocationInfo(uint64_t pc, DwarfCIE* cie, DwarfFDE* fde) {
    DwarfCfa<AddressType> cfa(&memory_, cie, fde);

    auto reg_entry = cie_loc_regs_.find(fde->cie_offset);
    if (reg_entry == cie_loc_regs_.end() || (g_LoggingFlags & LOGGING_FLAG_ENABLE_OP)) {
      if (!cfa.GetLocationInfo(pc, cie->cfa_instructions_offset, cie->cfa_instructions_end)) {
        return false;
      }
      cie_loc_regs_[fde->cie_offset] = cfa.loc_regs();
    }
    cfa.set_cie_loc_regs(&cie_loc_regs_[fde->cie_offset]);
    return cfa.GetLocationInfo(pc, fde->cfa_instructions_offset, fde->cfa_instructions_end);
  }
