...
class Regs {
 public:
  Regs(uint16_t pc_reg, uint16_t sp_reg, uint16_t last_reg)
      : pc_reg_(pc_reg), sp_reg_(sp_reg), last_reg_(last_reg) {}
  virtual ~Regs() = default;

  uint16_t pc_reg() { return pc_reg_; }
  uint16_t sp_reg() { return sp_reg_; }
  uint16_t last_reg() { return last_reg_; }
...
class RegsTmpl : public Regs {
 public:
  RegsTmpl(uint16_t pc_reg, uint16_t sp_reg, uint16_t last_reg, void* reg_mem)
      : Regs(pc_reg, sp_reg, last_reg), regs_(reinterpret_cast<AddressType*>(reg_mem)) {}
...
class Regs32 : public RegsTmpl<uint32_t> {
 public:
  Regs32(uint16_t pc_reg, uint16_t sp_reg, uint16_t last_reg, void* reg_mem)
      : RegsTmpl(pc_reg, sp_reg, last_reg, reg_mem) {}
  virtual ~Regs32() = default;
};

class Regs64 : public RegsTmpl<uint64_t> {
 public:
  Regs64(uint16_t pc_reg, uint16_t sp_reg, uint16_t last_reg, void* reg_mem)
      : RegsTmpl(pc_reg, sp_reg, last_reg, reg_mem) {}
