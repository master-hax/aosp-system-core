...
class Memory {
 public:
  ...
  virtual bool Read(umaxptr_t offset, void* dst, size_t size) = 0;

  inline bool Read(umaxptr_t offset, void* start, void* field, size_t size) {
    return Read(offset + reinterpret_cast<uintptr_t>(field) - reinterpret_cast<uintptr_t>(start),
                field, size);
  }
  inline bool Read32(umaxptr_t offset, uint32_t* dst) {
    return Read(offset, dst, sizeof(uint32_t));
  }
  inline bool Read64(umaxptr_t offset, uint64_t* dst) {
...
class MemoryFileAtOffset : public Memory {
 public:
  ...
  bool Init(const std::string& file, umaxptr_t offset);

  bool Read(umaxptr_t offset, void* dst, size_t size) override;
...
class MemoryMapLocal : public Memory {
 public:
  MemoryMapLocal() = default;
  virtual ~MemoryMapLocal();
