...
class MemoryFake : public Memory {
 public:
  ...
  void OverwriteData(uint64_t addr, uint32_t value);

  void SetMemory(uint64_t offset, std::vector<uint8_t> values);
  void SetMemory(uint64_t offset, const void* memory, size_t length);
