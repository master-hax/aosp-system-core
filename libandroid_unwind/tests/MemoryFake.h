...
class MemoryFake : public Memory {
 public:
  ...
  bool Read(umaxptr_t addr, void* buffer, size_t size) override;

  // Address must be 32 bit aligned.
  void SetData(umaxptr_t addr, uint32_t value);
  void OverwriteData(umaxptr_t addr, uint32_t value);

  void SetMemory(umaxptr_t offset, const void* memory, size_t length);

  void Clear() { data_.clear(); }

 private:
  std::unordered_map<umaxptr_t, uint32_t> data_;
};
...
class MemoryFakeAlwaysReadZero : public Memory {
 public:
  ...
  bool Read(umaxptr_t, void* buffer, size_t size) override {
