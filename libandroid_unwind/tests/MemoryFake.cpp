...
void MemoryFake::SetData(umaxptr_t addr, uint32_t value) {
  if (addr & 0x3) {
...
void MemoryFake::OverwriteData(umaxptr_t addr, uint32_t value) {
  if (addr & 0x3) {
    printf("Addr is not aligned to a 32 bit boundary: 0x%" PRIx64 "\n", addr);
    abort();
  }
...
void MemoryFake::SetMemory(umaxptr_t offset, const void* source, size_t length) {
  const uint8_t* memory = reinterpret_cast<const uint8_t*>(source);
  for (size_t i = 0; i < length; i += sizeof(uint32_t), offset += sizeof(uint32_t)) {
    uint32_t data;
    memcpy(&data, &memory[i], sizeof(data));
    SetData(offset, data);
  }
...
bool MemoryFake::Read(umaxptr_t addr, void* memory, size_t size) {
  uint8_t* buffer = reinterpret_cast<uint8_t*>(memory);
  umaxptr_t aligned_addr = addr & ~0x3;
