...
class ArmUnwindExtractTest : public ::testing::Test {
 protected:
  virtual void SetUp() {
    ResetLogs();
    memory_.Clear();
    exidx_ = new ArmUnwind(StateArm{}, &memory_);
    data_ = exidx_->data();
    data_->clear();
...
TEST_F(ArmUnwindExtractTest, cant_unwind) {
...
TEST_F(ArmUnwindExtractTest, compact) {
...
TEST_F(ArmUnwindExtractTest, second_read_compact_personality_1_2) {
  memory_.SetData(0x5000, 0x1234);
...
TEST_F(ArmUnwindExtractTest, second_read_compact_personality_illegal) {
  memory_.SetData(0x5000, 0x7ffa1e48);
  memory_.SetData(0x5004, 0x1230);
  memory_.SetData(0x6234, 0x832132b0);
  ...
  memory_.SetData(0x5000, 0x7ffa1e48);
  memory_.SetData(0x5004, 0x1230);
  memory_.SetData(0x6234, 0x842132b0);
  ASSERT_FALSE(exidx_->ExtractEntry(0x5000));
  ASSERT_EQ(ARM_STATUS_INVALID_PERSONALITY, exidx_->status());
}

TEST_F(ArmUnwindExtractTest, second_read_offset_is_negative) {
  memory_.SetData(0x5000, 0x7ffa1e48);
  memory_.SetData(0x5004, 0x7fffb1e0);
  memory_.SetData(0x1e4, 0x842132b0);
  ASSERT_FALSE(exidx_->ExtractEntry(0x5000));
  ASSERT_EQ(ARM_STATUS_INVALID_PERSONALITY, exidx_->status());
}

TEST_F(ArmUnwindExtractTest, second_read_not_compact) {
  memory_.SetData(0x5000, 0x1234);
  memory_.SetData(0x5004, 0x00001230);
...
TEST_F(ArmUnwindExtractTest, read_failures) {
  ASSERT_FALSE(exidx_->ExtractEntry(0x5000));
...
TEST_F(ArmUnwindExtractTest, malformed) {
  memory_.SetData(0x5000, 0x100);
  memory_.SetData(0x5004, 0x100);
  memory_.SetData(0x5104, 0x1);
