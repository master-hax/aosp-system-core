...
TEST_F(MemoryByPidTest, local_read_string) {
  ...
  std::vector<uint8_t> dst(1024);
  std::string dst_name;
  ASSERT_TRUE(local.ReadString(reinterpret_cast<uintptr_t>(name.c_str()), &dst_name));
  ASSERT_EQ("string_in_memory", dst_name);

  ASSERT_TRUE(local.ReadString(reinterpret_cast<uintptr_t>(&name[7]), &dst_name));
  ASSERT_EQ("in_memory", dst_name);

  ASSERT_TRUE(local.ReadString(reinterpret_cast<uintptr_t>(&name[7]), &dst_name, 10));
  ASSERT_EQ("in_memory", dst_name);

  ASSERT_FALSE(local.ReadString(reinterpret_cast<uintptr_t>(&name[7]), &dst_name, 9));
}
...
TEST_F(MemoryByPidTest, range_read_string) {
  std::unique_ptr<MemoryByPidRange> memory;
  std::string name("0123456789");
  uint64_t start = reinterpret_cast<uintptr_t>(name.c_str());
  std::string dst_name;

  // Verify a read past the range fails.
  memory.reset(new MemoryByPidRange(getpid(), 0, start + 5));
  ASSERT_FALSE(memory->ReadString(start, &dst_name));

  // Verify the range going to the end of the string works.
  memory.reset(new MemoryByPidRange(getpid(), 0, start + name.size() + 1));
  ASSERT_TRUE(memory->ReadString(start, &dst_name));
  ASSERT_EQ("0123456789", dst_name);

  // Verify the range set to one byte less than the end of the string fails.
  memory.reset(new MemoryByPidRange(getpid(), 0, start + name.size()));
  ASSERT_FALSE(memory->ReadString(start, &dst_name));
}
