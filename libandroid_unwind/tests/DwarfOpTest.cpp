...
class DwarfOpTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    mem_.reset(new DwarfMemory<TypeParam>(&op_memory_));
  ...
  std::unique_ptr<DwarfMemory<TypeParam>> mem_;
...
TYPED_TEST_P(DwarfOpTest, eval) {
  ...
  ASSERT_TRUE(this->op_->Eval(0, 0, DWARF_VERSION_MAX));
  ...
  ASSERT_TRUE(this->op_->Eval(0, 7, DWARF_VERSION_MAX));
  ASSERT_EQ(DWARF_ERROR_NONE, this->op_->last_error());
...
TYPED_TEST_P(DwarfOpTest, op_deref_size) {
  ...
  for (size_t i = 1; i < sizeof(TypeParam); i++) {
    this->op_memory_.SetMemory(0, std::vector<uint8_t>{0x0a, 0x10, 0x20, 0x94, static_cast<uint8_t>(i)});
    ASSERT_TRUE(this->op_->Eval(0, 4, DWARF_VERSION_MAX))
        << "Failed at size " << i;
    ASSERT_EQ(1U, this->op_->StackSize())
        << "Failed at size " << i;
    ASSERT_EQ(0x94, this->op_->cur_op())
        << "Failed at size " << i;
    TypeParam expected_value = 0;
    memcpy(&expected_value, &value, i);
    ASSERT_EQ(expected_value, this->op_->StackAt(0))
        << "Failed at size " << i;
  }
...
TYPED_TEST_P(DwarfOpTest, compare_opcode_stack_error) {
  ...
  for (uint8_t opcode = 0x29; opcode <= 0x2e; opcode++) {
    ...
    ASSERT_FALSE(this->op_->Eval(0, 0, DWARF_VERSION_MAX));
    ASSERT_EQ(opcode, this->op_->cur_op());
    ASSERT_EQ(DWARF_ERROR_STACK_INDEX_NOT_VALID, this->op_->last_error());

    ASSERT_FALSE(this->op_->Eval(1, 3, DWARF_VERSION_MAX));
    ASSERT_EQ(opcode, this->op_->cur_op());
    ASSERT_EQ(1U, this->op_->StackSize());
    ASSERT_EQ(DWARF_ERROR_STACK_INDEX_NOT_VALID, this->op_->last_error());
  }
...
TYPED_TEST_P(DwarfOpTest, compare_opcodes) {
  ...
  for (size_t i = 0; i < expected.size(); i += 4) {
    ...
    ASSERT_TRUE(this->op_->Eval(0, 14, DWARF_VERSION_MAX))
...
TYPED_TEST_P(DwarfOpTest, op_lit) {
  ...
  for (size_t i = 0; i < opcode_buffer.size(); i++) {
    ...
    ASSERT_TRUE(this->op_->Eval(i, i, DWARF_VERSION_MAX))
        << "Failed op: 0x" << std::hex << op;
    ASSERT_EQ(op, this->op_->cur_op());
    ASSERT_EQ(1U, this->op_->StackSize())
        << "Failed op: 0x" << std::hex << op;
    ASSERT_EQ(op - 0x30U, this->op_->StackAt(0))
        << "Failed op: 0x" << std::hex << op;
  }
}
...
TYPED_TEST_P(DwarfOpTest, op_reg) {
  ...
  for (size_t i = 0; i < opcode_buffer.size(); i++) {
    ...
    ASSERT_EQ(1U, this->op_->StackSize())
        << "Failed op: 0x" << std::hex << op;
    ASSERT_EQ(op - 0x50U, this->op_->StackAt(0))
        << "Failed op: 0x" << std::hex << op;
  }
}
...
TYPED_TEST_P(DwarfOpTest, op_regx) {
  ...
  ASSERT_TRUE(this->op_->Eval(2, 4, DWARF_VERSION_MAX));
  ASSERT_EQ(0x90, this->op_->cur_op());
  ASSERT_TRUE(this->op_->is_register());
  ASSERT_EQ(1U, this->op_->StackSize());
...
TYPED_TEST_P(DwarfOpTest, op_breg) {
  ...
  std::vector<TypeParam> mem(32);
  for (size_t i = 0; i < mem.size(); i++) {
    mem[i] = i + 10;
  }
  RegsTmpl<TypeParam> regs(0, 0, 32, mem.data());
  ...
  for (uint32_t op = 0x70; op <= 0x8f; op++) {
    // Positive value added to register.
    ASSERT_TRUE(this->op_->Eval(offset, offset + 1, DWARF_VERSION_MAX))
        << "Failed op: 0x" << std::hex << op;
    ASSERT_EQ(op, this->op_->cur_op());
    ASSERT_EQ(1U, this->op_->StackSize())
        << "Failed op: 0x" << std::hex << op;
    ASSERT_EQ(op - 0x70 + 10 + 0x12, this->op_->StackAt(0))
        << "Failed op: 0x" << std::hex << op;
    offset += 2;
    ...
    ASSERT_EQ(1U, this->op_->StackSize())
        << "Failed op: 0x" << std::hex << op;
    ASSERT_EQ(op - 0x70 + 10 - 2, this->op_->StackAt(0))
        << "Failed op: 0x" << std::hex << op;
    offset += 2;
  }
}
...
TYPED_TEST_P(DwarfOpTest, op_breg_invalid_register) {
  ...
  std::vector<TypeParam> mem(16);
  for (size_t i = 0; i < mem.size(); i++) {
    mem[i] = i + 10;
  }
  RegsTmpl<TypeParam> regs(0, 0, 16, mem.data());
  ...
  ASSERT_FALSE(this->op_->Eval(2, 3, DWARF_VERSION_MAX));
  ASSERT_EQ(DWARF_ERROR_ILLEGAL_VALUE, this->op_->last_error());
}
...
TYPED_TEST_P(DwarfOpTest, op_bregx) {
  ...
  std::vector<TypeParam> mem(10);
  mem[5] = 0x45;
  mem[6] = 0x190;
  RegsTmpl<TypeParam> regs(0, 0, mem.size(), mem.data());
  this->op_->set_regs(&regs);
  ...
  ASSERT_TRUE(this->op_->Eval(3, 6, DWARF_VERSION_MAX));
  ASSERT_EQ(0x92, this->op_->cur_op());
  ASSERT_EQ(1U, this->op_->StackSize());
  ASSERT_EQ(0x90U, this->op_->StackAt(0));

  ASSERT_FALSE(this->op_->Eval(7, 11, DWARF_VERSION_MAX));
  ASSERT_EQ(DWARF_ERROR_ILLEGAL_VALUE, this->op_->last_error());
}
...
REGISTER_TYPED_TEST_CASE_P(DwarfOpTest,
                           decode,
                           eval,
                           illegal_opcode,
                           illegal_in_version3,
