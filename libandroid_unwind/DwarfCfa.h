...
class DwarfCfa {
 public:
  ...
  bool GetLocationInfo(uint64_t pc, uint64_t start_offset, uint64_t end_offset) {
    if (cie_loc_regs_) {
      loc_regs_ = *cie_loc_regs_;
    } else {
      loc_regs_.clear();
    }
    loc_regs_[CFA_REG] = init_cfa_location_;
    ...
    while ((cfa_offset = memory_->cur_offset()) < end_offset && cur_pc_ <= pc) {
      ...
      case 2:
      {
        ...
        loc_regs_[cfa_low] = { .type = DWARF_LOCATION_OFFSET,
      ...
      case 3:
      {
        ...
        if (reg_entry == cie_loc_regs_->end()) {
          loc_regs_.erase(cfa_low);
        } else {
          loc_regs_[cfa_low] = reg_entry->second;
        }
        break;
      }
      case 0:
  ...
  const DwarfLocation& cfa_location() { return loc_regs_[CFA_REG]; }
  AddressType cfa_offset() { return loc_regs_[CFA_REG].values[1]; }

  const dwarf_loc_regs_t& loc_regs() { return loc_regs_; }

  void set_cie_loc_regs(const dwarf_loc_regs_t* cie_loc_regs) { cie_loc_regs_ = cie_loc_regs; }
  void set_init_cfa_location(const DwarfLocation& location) { init_cfa_location_ = location; }
 ...
 private:
  ...
  DwarfLocation init_cfa_location_ = { .type = DWARF_LOCATION_UNDEFINED };
  ...
  static bool cfa_offset(void* ptr) {
    ...
    cfa->loc_regs_[reg] = { .type = DWARF_LOCATION_OFFSET, .values = { cfa->operands_[1] } };
    return true;
  }
  ...
  static bool cfa_restore(void* ptr) {
    ...
    if (reg_entry == cfa->cie_loc_regs_->end()) {
      cfa->loc_regs_.erase(reg);
    } else {
      cfa->loc_regs_[reg] = reg_entry->second;
    }
    return true;
  }
  ...
  static bool cfa_undefined(void* ptr) {
    DwarfCfa* cfa = reinterpret_cast<DwarfCfa*>(ptr);
    AddressType reg = cfa->operands_[0];
    auto reg_entry = cfa->loc_regs_.find(reg);
    if (reg_entry != cfa->loc_regs_.end()) {
      cfa->loc_regs_.erase(reg);
    }
    return true;
  }
  ...
  static bool cfa_same_value(void* ptr) {
    DwarfCfa* cfa = reinterpret_cast<DwarfCfa*>(ptr);
    AddressType reg = cfa->operands_[0];
    cfa->loc_regs_[reg] = { .type = DWARF_LOCATION_SAME };
    return true;
  }
  ...
  static bool cfa_register(void* ptr) {
    ...
    cfa->loc_regs_[reg] = { .type = DWARF_LOCATION_REGISTER, .values = { reg_dst } };
    return true;
  }

  static bool cfa_remember_state(void* ptr) {
    DwarfCfa* cfa = reinterpret_cast<DwarfCfa*>(ptr);
    cfa->loc_reg_state_.push(cfa->loc_regs_);
    return true;
  }
  ...
  static bool cfa_restore_state(void* ptr) {
    ...
    cfa->loc_regs_ = cfa->loc_reg_state_.top();
  ...
  static bool cfa_def_cfa(void* ptr) {
    DwarfCfa* cfa = reinterpret_cast<DwarfCfa*>(ptr);
    cfa->loc_regs_[CFA_REG] = { .type = DWARF_LOCATION_REGISTER,
  ...
  static bool cfa_def_cfa_offset(void* ptr) {
    ...
    auto cfa_location = cfa->loc_regs_.find(CFA_REG);
    if (cfa_location->second.type != DWARF_LOCATION_REGISTER) {
  ...
  static bool cfa_def_cfa_expression(void* ptr) {
    DwarfCfa* cfa = reinterpret_cast<DwarfCfa*>(ptr);
    cfa->loc_regs_[CFA_REG] = { .type = DWARF_LOCATION_EXPRESSION,
  ...
  static bool cfa_expression(void* ptr) {
    DwarfCfa* cfa = reinterpret_cast<DwarfCfa*>(ptr);
    AddressType reg = cfa->operands_[0];
    cfa->loc_regs_[reg] = { .type = DWARF_LOCATION_EXPRESSION,
  ...
  static bool cfa_offset_extended_sf(void* ptr) {
    ...
    if (sizeof(AddressType) == 4) {
      ...
      cfa->loc_regs_[reg] = { .type = DWARF_LOCATION_OFFSET,
  ...
  static bool cfa_def_cfa_sf(void* ptr) {
    DwarfCfa* cfa = reinterpret_cast<DwarfCfa*>(ptr);
    AddressType offset = cfa->operands_[1] * cfa->cie_->data_alignment_factor;
    cfa->loc_regs_[CFA_REG] = { .type = DWARF_LOCATION_REGISTER,
                                .values = { cfa->operands_[0], offset } };
    return true;
  }
  ...
  static bool cfa_val_offset(void* ptr) {
    DwarfCfa* cfa = reinterpret_cast<DwarfCfa*>(ptr);
    AddressType reg = cfa->operands_[0];
    cfa->loc_regs_[reg] = { .type = DWARF_LOCATION_VAL_OFFSET,
  ...
  static bool cfa_val_expression(void* ptr) {
    DwarfCfa* cfa = reinterpret_cast<DwarfCfa*>(ptr);
    AddressType reg = cfa->operands_[0];
    cfa->loc_regs_[reg] = { .type = DWARF_LOCATION_VAL_EXPRESSION,
                            .values = { cfa->operands_[1], cfa->memory_->cur_offset() } };
    return true;
  }
