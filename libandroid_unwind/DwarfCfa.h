...
class DwarfCfa {
 public:
      ...
      case 0:
        {
          ...
          if (g_LoggingEnabled) {
            ...
            if (((cur_offset - cfa_offset + 2) % 10) != 0) {
              log("%s", raw_data.c_str());
            }
  ...
  const std::unordered_map<uint8_t, DwarfLocation>& regs() { return regs_; }

  void set_cie_regs(const std::unordered_map<uint8_t, DwarfLocation>* cie_regs) { cie_regs_ = cie_regs; }
 ...
 private:
  ...
  AddressType cur_pc_;
  DwarfLocation cfa_location_;
  AddressType cfa_offset_;
  std::unordered_map<uint8_t, DwarfLocation> regs_;
  const std::unordered_map<uint8_t, DwarfLocation>* cie_regs_ = nullptr;
  std::vector<AddressType> operands_;
  std::stack<std::unordered_map<uint8_t, DwarfLocation>> reg_state_;
  ...
  static bool cfa_restore(void* ptr) {
    ...
    if (reg_entry == cfa->cie_regs_->end()) {
      ...
      //cfa->regs_[reg] = (*cfa->cie_regs_)[reg];
    }
    return true;
  }
  ...
  static bool cfa_def_cfa_offset(void* ptr) {
    DwarfCfa* cfa = reinterpret_cast<DwarfCfa*>(ptr);
    cfa->cfa_offset_ = cfa->operands_[0] * cfa->cie_->data_alignment_factor;
    return true;
  }
  ...
  static bool cfa_def_cfa_offset_sf(void* ptr) {
    DwarfCfa* cfa = reinterpret_cast<DwarfCfa*>(ptr);
    if (sizeof(AddressType) == 4) {
      int32_t value = static_cast<int32_t>(cfa->operands_[0]);
      cfa->cfa_offset_ = value * cfa->cie_->data_alignment_factor;
    } else {
      int64_t value = static_cast<int64_t>(cfa->operands_[0]);
      cfa->cfa_offset_ = value * cfa->cie_->data_alignment_factor;
    }
    return true;
  }
