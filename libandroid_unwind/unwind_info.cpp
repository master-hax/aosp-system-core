...
void DumpArm(Elf* elf) {
  ...
  ElfInterfaceArm* arm = elf_interface->arm();
  if (arm == nullptr) {
...
void DumpEhFrame(Elf* elf) {
  ...
  if (!elf_interface->ReadEhFrame<AddressType>()) {
  ...
  for (auto info : *eh_frame) {
    ...
    printf("  PC 0x%" PRIx64 "\n", info.pc + load_bias);
    if (!eh_frame->GetCfaLocationInfo(fde.start_pc + fde.pc_length - 1, &cie, &fde)) {
