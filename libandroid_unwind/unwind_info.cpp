...
void DumpEhFrame(Elf* elf) {
  ...
  for (auto info : *eh_frame) {
    ...
    printf("  PC 0x%" PRIx64, info.pc + load_bias);
    std::string name;
    if (elf->GetFunctionName(info.pc, &name) && !name.empty()) {
      printf(" <%s>", name.c_str());
    }
    printf("\n");
    if (!eh_frame->Log(2, fde.start_pc + fde.pc_length - 1, &cie, &fde)) {
...
int main(int argc, char** argv) {
  ...
  std::string name;
  if (elf.GetFunctionName(0x54061, &name)) {
