...
class ElfInterfaceArm : public ElfInterface32 {
 public:
  ...
  void ClearCache() override { addrs_.clear(); }

  bool Step(uint64_t rel_pc, Regs* regs, Memory* process_memory) override;

  bool StepExidx(uint64_t rel_pc, Regs* regs, Memory* process_memory);

  void AdjustPc(Regs* regs, MapInfo* map_info) override;
