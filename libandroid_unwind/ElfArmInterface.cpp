...
ElfArmInterface::ElfArmInterface(Memory* memory, uint64_t start_offset, uint64_t size)
    : memory_(memory), start_offset_(start_offset), total_entries_(size / 8) {
}

// Ip must have already been adjusted for any load bias.
bool ElfArmInterface::FindEntry(arm_ptr_t ip, uint64_t* entry_offset) {
...
bool ElfArmInterface::GetPrel31Addr(arm_ptr_t offset, arm_ptr_t* addr) {
  uint32_t data;
