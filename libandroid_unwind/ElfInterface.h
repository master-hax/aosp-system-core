  ...
  bool ProcessProgramHeaders() override {
    ...
    for (size_t i = 0; i < ehdr.e_phnum; i++, offset += sizeof(PhdrType)) {
      ...
      case PT_LOAD:
      {
        ...
        if (!memory_->Read(offset + field_offset,
                           reinterpret_cast<uint8_t*>(&load_info.table_offset),
                           sizeof(load_info.table_offset))) {
          return false;
        }
        field_offset =
            reinterpret_cast<uintptr_t>(&phdr.p_offset) - reinterpret_cast<uintptr_t>(&phdr);
        if (!memory_->Read(offset + field_offset, reinterpret_cast<uint8_t*>(&load_info.offset),
                           sizeof(load_info.offset))) {
          return false;
        }
        field_offset =
            reinterpret_cast<uintptr_t>(&phdr.p_memsz) - reinterpret_cast<uintptr_t>(&phdr);
        if (!memory_->Read(offset + field_offset, reinterpret_cast<uint8_t*>(&load_info.memsize),
                           sizeof(load_info.memsize))) {
          return false;
        }
