...
class ElfInterfaceBase {
 public:
  ...
  virtual bool Step(Regs* regs, Memory* process_memory) = 0;
...
class ElfInterface32 : public ElfInterface<Elf32_Ehdr, Elf32_Phdr, Elf32_Dyn> {
 public:
  ...
  bool Step(Regs* regs, Memory* process_memory) override {
    ...
    uint64_t fde_offset;
    if (!eh_frame->GetFdeOffset(regs32->pc(), &fde_offset)) {
      return false;
    }
    DwarfCIE cie;
    DwarfFDE fde;
    ...
    if (!eh_frame->GetCfaLocationInfo(regs32->pc(), &cie, &fde, &loc_regs)) {
      return false;
    }
...
class ElfInterface64 : public ElfInterface<Elf64_Ehdr, Elf64_Phdr, Elf64_Dyn> {
 public:
  ...
  bool Step(Regs*, Memory*) override {
    return true;
  }
};
