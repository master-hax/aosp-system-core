...
bool ArmExidx::ExtractEntry(arm_ptr_t entry) {
  data_.clear();
  ...
  if (data & (1UL << 31)) {
    // Compact entry.
    uint8_t personality = (data >> 24) & 0xf;
    ...
    // Ignore the personality routine value, we aren't going to use it.
    addr += 4;
    if (!memory_->Read32(addr, &data)) {
      status_ = ARM_STATUS_READ_FAILED;
      return false;
