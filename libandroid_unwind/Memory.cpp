...
bool MemoryMapRemote::Init(pid_t pid) {
  pid_ = pid;
  return true;
}

bool MemoryMapRemote::Read(uint64_t offset, void* dst, size_t size) {
  ...
  return static_cast<size_t>(process_vm_readv(pid_, &local_io, 1, &remote_io, 1, 0)) == size;
}
