...
bool MemoryFileAtOffset::Init(const std::string& file, umaxptr_t offset) {
  ...
  if (offset >= static_cast<umaxptr_t>(buf.st_size)) {
    return false;
  }

  offset_ = offset & (getpagesize() - 1);
  umaxptr_t aligned_offset = offset & ~(getpagesize() - 1);
...
bool MemoryFileAtOffset::Read(umaxptr_t offset, void* dst, size_t size) {
...
bool MemoryMapLocal::Read(umaxptr_t offset, void* dst, size_t size) {
