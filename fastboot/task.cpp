...
FlashSuperLayoutTask::FlashSuperLayoutTask(const std::string& super_name, SparsePtr s)
    : super_name_(super_name), s_(std::move(s)) {}
...
std::unique_ptr<FlashSuperLayoutTask> FlashSuperLayoutTask::Initialize(
    ...
    os_images.erase(std::remove_if(os_images.begin(), os_images.end(), remove_if_callback),
                    os_images.end());
    return std::make_unique<FlashSuperLayoutTask>(super_name, std::move(s));
}
