...
bool FlashTask::IsDynamicParitition(const ImageSource* source, FlashTask* task) {
...
bool OptimizedFlashSuperTask::CanOptimize(const ImageSource* source,
    ...
    for (size_t i = 0; i < tasks.size(); i++) {
        if (auto reboottask = tasks[i]->AsRebootTask()) {
            if (reboottask->ToString() == "reboot fastboot") {
                if (i < tasks.size() - 2) {
                    if (auto updatesupertask = tasks[i + 1]->AsUpdateSuperTask()) {
                        // at least one dynamic partiton flash task exists, thus fulfilling our
                        // pattern match
                        if (auto flash_task = tasks[i + 2]->AsFlashTask()) {
                            if (FlashTask::IsDynamicParitition(source, flash_task)) return true;
                        }
                    }
                }
            }
