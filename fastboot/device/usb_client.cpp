...
constexpr int kFbFfsNumBufs = 16;
constexpr int FbFfsBufSize = 32768;
...
static bool InitFunctionFs(usb_handle* h) {
    ssize_t ret;
    PLOG(INFO) << "initializing functionfs";

    if (h->control < 0) {  // might have already done this before
        PLOG(INFO) << "opening control endpoint " << kUsbFfsFastbootEp0;
...
ClientUsbTransport::ClientUsbTransport()
    : handle_(std::unique_ptr<usb_handle>(create_usb_handle(kFbFfsNumBufs, FbFfsBufSize))) {
...
ssize_t ClientUsbTransport::Read(void* data, size_t len) {
    ...
    while (bytes_read_total < static_cast<ssize_t>(len)) {
        int bytes_to_read = std::min(len - bytes_read_total,
                                     static_cast<unsigned long>(kFbFfsNumBufs * FbFfsBufSize));
        int bytes_read_now = handle_->read(handle_.get(), char_data, bytes_to_read);
        if (bytes_read_now < 0) {
            return bytes_read_now;
        }
        bytes_read_total += bytes_read_now;
        char_data += bytes_to_read;
...
ssize_t ClientUsbTransport::Write(const void* data, size_t len) {
    ...
    while (bytes_written_total < static_cast<ssize_t>(len)) {
        int bytes_to_write = std::min(len - bytes_written_total,
                                      static_cast<unsigned long>(kFbFfsNumBufs * FbFfsBufSize));
        int bytes_written_now = handle_->write(handle_.get(), data, len);
        if (bytes_written_now < 0) {
            return bytes_written_now;
        }
        bytes_written_total += bytes_written_now;
        char_data += bytes_to_write;
