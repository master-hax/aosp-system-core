...
// TODO(b/79107699): Use health HAL to get battery voltage once health 2.0 reaches AOSP.
bool getBatteryVoltage(int64_t* battery_voltage) {
    std::unique_ptr<DIR, decltype(&closedir)> dir(opendir(POWER_SUPPLY_SYSFS_PATH), closedir);
    if (!dir) {
        PLOG(ERROR) << "Unable to get battery voltage";
        return false;
    }
    ...
    while ((dp = readdir(dir.get())) != nullptr) {
        ...
        std::string type_path =
                android::base::StringPrintf("%s/%s/type", POWER_SUPPLY_SYSFS_PATH, dp->d_name);
        ...
        if (android::base::ReadFileToString(type_path, &type_value)) {
            ...
            if (type_value == type_battery) {
                std::string voltage_path = android::base::StringPrintf(
                        "%s/%s/voltage_now", POWER_SUPPLY_SYSFS_PATH, dp->d_name);
                std::string voltage_value;
                if (android::base::ReadFileToString(voltage_path, &voltage_value)) {
                    voltage_value = android::base::Trim(voltage_value);
