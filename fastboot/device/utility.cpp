...
bool OpenPartition(FastbootDevice* device, const std::string& name, PartitionHandle* handle,
    ...
    int flags_ = (read ? O_RDONLY : O_WRONLY);
    flags_ |= (O_EXCL | O_CLOEXEC | O_BINARY);
    flags_ |= flags;
    unique_fd fd(TEMP_FAILURE_RETRY(open(handle->path().c_str(), flags_)));
    if (fd < 0) {
...
bool OpenPartition(FastbootDevice* device, const std::string& name, PartitionHandle* handle,
                   bool read) {
    return OpenPartition(device, name, handle, 0, read);
}
