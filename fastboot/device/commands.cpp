...
bool DownloadHandler(FastbootDevice* device, const std::vector<std::string>& args) {
    ...
    unsigned int size = strtoul(args[1].c_str(), nullptr, 16);
    ...
    if (device->HandleData(device->GetDownloadData(), true)) {
...
bool ShutDownHandler(FastbootDevice* device) {
    bool result = device->WriteStatus(FastbootResult::OKAY, "Shutting down");
...
bool RebootHandler(FastbootDevice* device) {
    bool result = device->WriteStatus(FastbootResult::OKAY, "Rebooting");
...
bool RebootBootloaderHandler(FastbootDevice* device) {
    bool result = device->WriteStatus(FastbootResult::OKAY, "Rebooting bootloader");
...
bool RebootFastbootHandler(FastbootDevice* device) {
    bool result = device->WriteStatus(FastbootResult::OKAY, "Rebooting fastboot");
