...
struct VariableHandlers {
    ...
    using GetAllArgs = std::function<std::vector<std::vector<std::string>>(FastbootDevice*)>;

    GetVarHandler get;
    GetAllArgs get_all_args;
};
...
static void GetAllVars(FastbootDevice* device, const std::string& name,
    ...
    for (const auto& args : all_args) {
        ...
        std::string argstring = android::base::Join(args, ":");
        device->WriteInfo(android::base::StringPrintf("%s:%s:%s", name.c_str(), argstring.c_str(),
                                                      message.c_str()));
    }
}
...
bool GetVarHandler(FastbootDevice* device, const std::vector<std::string>& args) {
    const std::unordered_map<std::string, VariableHandlers> kVariableMap = {
            ...
            {FB_VAR_HAS_SLOT, {GetHasSlot, nullptr}},
            {FB_VAR_SLOT_SUCCESSFUL, {GetSlotSuccessful, nullptr}},
            {FB_VAR_SLOT_UNBOOTABLE, {GetSlotUnbootable, nullptr}},
            {FB_VAR_PARTITION_SIZE, {GetPartitionSize, GetAllPartitions}},
            {FB_VAR_IS_LOGICAL, {GetPartitionIsLogical, GetAllPartitions}},
