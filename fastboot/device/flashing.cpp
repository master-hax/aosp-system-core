...
static void MoveAVBFooter(std::vector<char>& data, const uint64_t block_device_size) {
    ...
    for (int idx = 0; idx < AVB_FOOTER_MAGIC_LEN; idx++) {
        footer.push_back(data[footer_offset + idx]);
    }
    if (0 != footer.compare(AVB_FOOTER_MAGIC)) {
        return;
    }

    // move AVB footer from end of data to end of block device
    data.resize(block_device_size, 0);
    for (int idx = 0; idx < AVB_FOOTER_SIZE; idx++) {
        data[block_device_size - 1 - idx] = data[footer_offset + AVB_FOOTER_SIZE - 1 - idx];
    }
...
int Flash(FastbootDevice* device, const std::string& partition_name) {
    ...
    if (data.size() > block_device_size) {
        ...
        MoveAVBFooter(data, block_device_size);
    }
