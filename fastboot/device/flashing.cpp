...
std::optional<std::string> blockdev_search(const std::string& name) {
...
int GetPartitionDevice(std::string name) {
    auto path = blockdev_search(name);
    if (path) {
        return TEMP_FAILURE_RETRY(open(path->c_str(), O_WRONLY | O_EXCL));
    }
    return -1;
}
...
int flash_raw_data(int fd, const std::vector<char>& downloaded_data) {
