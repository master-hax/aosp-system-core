...
int FlashRawDataChunk(PartitionHandle *handle, const char* data, size_t len) {
    ...
    while (ret < len) {
        ...
        if (this_len & 0xFFF) {
            if (handle->reset_fd(O_WRONLY) != true) {
...
int Flash(FastbootDevice* device, const std::string& partition_name) {
    std::unique_ptr<PartitionHandle> handle = std::make_unique<PartitionHandle>();
    if (!handle) {
        return -ENOMEM;
    }

    if (!OpenPartition(device, partition_name, handle.get(), O_WRONLY | O_DIRECT)) {
        return -ENOENT;
    }
    ...
    int result = FlashBlockDevice(handle.get(), data);
    sync();
    return result;
}
