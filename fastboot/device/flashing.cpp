...
bool UpdateSuper(FastbootDevice* device, const std::string& partition_name, bool wipe) {
    ...
    // may contain partitions created by the user. Instead, we create a zero-
    ...
    for (uint32_t i = 0; i < new_metadata->geometry.metadata_slot_count; i++) {
        if (!UpdatePartitionTable(super.value(), *new_metadata.get(), i)) {
