...
static int usb_ffs_do_aio(usb_handle* h, T* const data, int len) {
    ...
    const bool zero_packet = read && h->reads_zero_packets && (len % h->io_size) % packet_size == 0;

    const int num_requests = DivRoundup<int>(len, h->io_size) + static_cast<size_t>(zero_packet);
    auto cur_data = data;
