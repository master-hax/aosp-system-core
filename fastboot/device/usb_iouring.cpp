...
static int usb_ffs_do_aio(usb_handle* h, T* const data, int len) {
    ...
    const bool zero_packet = buf_len % packet_size == 0 && read && h->reads_zero_packets;

    const int num_requests = DivRoundup<size_t>(len, h->io_size) + static_cast<size_t>(zero_packet);
    auto cur_data = data;

    for (int i = num_requests - static_cast<size_t>(zero_packet); i > 0; i--) {
    ...
    if (zero_packet) {
        prep_async_io<read>(&h->ring, aiob->fd, cur_data, packet_size, 0);
    }
    int ret = io_uring_submit(&h->ring);
    if (ret <= 0) {
    ...
    for (int i = 0; i < num_requests; ++i) {
        ...
        const auto ret = io_uring_wait_cqe(&h->ring, &cqe);
