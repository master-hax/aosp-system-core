...
std::vector<uint8_t> fastboot_partition_table_data(
  ...
  std::vector<uint8_t> data(sizeof(PartitionTableHeader) +
              part_table.GetNumOfPartitions() * sizeof(PartitionHeader));
  ...
  auto tmp = reinterpret_cast<PartitionHeader *>
                              (&data[0] + sizeof table_header);
  ...
  for (auto& p : part_table.GetPartitions()) {
    ...
    if (p.IsExtended())
      part_header.extend = 1;
    part_header.attr = 0;
    if (p.IsBootable())
      part_header.attr |= GPT_ATTR_BOOTABLE;
    if (p.IsReadOnly())
      part_header.attr |= GPT_ATTR_RO;
