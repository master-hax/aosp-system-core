...
std::vector<uint8_t> partition_table::serialize() const {
  ...
  /* send all data as little endian */
#if __BYTE_ORDER__ == __ORDER_BIG_ENDIAN__
  tmp->magic = __builtin_bswap32(header.magic);
  tmp->version = __builtin_bswap32(header.version);
  tmp->lun = __builtin_bswap32(header.lun);
  tmp->type = __builtin_bswap32(header.type);
  tmp->num = __builtin_bswap32(header.num);
#else
  tmp->magic = header.magic;
  tmp->version = header.version;
  tmp->lun = header.lun;
  tmp->type = header.type;
  tmp->num = header.num;
#endif
  std::memcpy(&tmp->disk_guid, header.disk_guid, sizeof header.disk_guid);
