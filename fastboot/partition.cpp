...
std::vector<uint8_t> partition::serialize() const
{
  ...
  tmp->size = htole64(size);
  tmp->attr = htole64(attr);
  tmp->extend = htole32(extend);
...
std::vector<uint8_t> partition_table::serialize() const {
  ...
  /* send all data as little endian */
  tmp->magic = htole32(header.magic);
  tmp->version = htole32(header.version);
  tmp->lun = htole32(header.lun);
  tmp->type = htole32(header.type);
  tmp->num = htole32(header.num);
