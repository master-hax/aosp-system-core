...
std::vector<uint8_t> fastboot_partition_table_data(const PartitionTable& part_table) {
    std::vector<uint8_t> data(sizeof(PartitionTableHeader) +
                              part_table.partitions.size() * sizeof(PartitionHeader));
    ...
    auto tmp = reinterpret_cast<PartitionHeader*>(&data[0] + sizeof table_header);
    ...
    for (auto& p : part_table.partitions) {
        ...
        if (p.erase_block_align) part_header.attr |= FbPartAttr::kAttrEraseBlockAlign;
