...
// Match is successful if |arg| is a zero-padded version of |actual|.
MATCHER_P(IsPadded, actual, (negation ? "is" : "isn't") + " zero-padded of expected value"s) {
    if (arg.size() < actual.size()) return false;
    if (0 != memcmp(arg.data(), actual.data(), actual.size())) return false;
    auto remainder = std::string_view(arg).substr(actual.size());
...
MATCHER_P(MemEq, actual, (negation ? "is" : "isn't") + " expected value"s) {
    if (arg.size() != actual.size()) return false;
    return 0 == memcmp(arg.data(), actual.data(), actual.size());
}
...
TEST_P(RepackVendorBootImgTest, ReplaceUnknown) {
    ...
    if (GetParam().expected_header_version == 3) {
        EXPECT_ERROR(res, Eq("Require vendor boot header 4 but is 3"));
