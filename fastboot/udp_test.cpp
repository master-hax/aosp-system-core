...
TEST_F(UdpTest, SequenceIncrementCheck) {
    for (uint16_t seq : kTestSequenceNumbers) {
        ...
        for (uint16_t i = 1; i < 10; ++i) {
            mock_socket_->ExpectSend(FastbootPacket(seq + i, "foo"));
            mock_socket_->AddReceive(FastbootPacket(seq + i, "bar"));
...
TEST_F(UdpTest, ReadAndWriteSmallPackets) {
    ...
    mock_socket_->AddReceive(FastbootPacket(1, "bar"));

    EXPECT_TRUE(Write("foo"));
    EXPECT_TRUE(Read("bar"));

    mock_socket_->ExpectSend(FastbootPacket(2, "12345 67890"));
    mock_socket_->AddReceive(FastbootPacket(2));
    mock_socket_->ExpectSend(FastbootPacket(3, "\x01\x02\x03\x04\x05"));
    ...
    mock_socket_->AddReceive(FastbootPacket(4, "foo bar baz"));
    mock_socket_->ExpectSend(FastbootPacket(5));
    mock_socket_->AddReceive(FastbootPacket(5, "\x01\x02\x03\x04\x05"));
...
// Tests breaking a single UDP packet into multiple fastboot reads. This would indicate that the
// device sent a fastboot message larger than the engine was prepared to receive, which should
// never happen if the device obeys the fastboot protocol, but we should still handle it gracefully.
TEST_F(UdpTest, MultipleReadPacket) {
    ...
    EXPECT_TRUE(Read("bar"));
    EXPECT_TRUE(Read("baz"));

    // We've finished the sent data so the next Read() should try to receive more data.
    mock_socket_->ExpectSend(FastbootPacket(2));
    mock_socket_->AddReceive(FastbootPacket(2, "new data"));

    EXPECT_TRUE(Read("new data"));
}
