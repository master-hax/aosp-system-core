  ...
  protected:
    ...
    void SetUp() override {
        ...
        for (int i = 0; i < MAX_USB_TRIES && !transport; i++) {
            transport = std::unique_ptr<UsbTransport>(usb_open(matcher, USB_TIMEOUT));
...
TEST_F(Fuzz, DownloadInvalid) {
    ...
    std::string cmd("download:01000000\0", 18);
    EXPECT_EQ(fb->RawCommand(cmd), DEVICE_FAIL);
    ASSERT_TRUE(UsbStillAvailible());
    std::string cmd2("download:01000000\0dkjfvijafdaiuybgidabgybr", 18);
