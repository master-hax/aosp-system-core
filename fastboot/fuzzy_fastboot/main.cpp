  ...
  public:
    ...
    virtual ssize_t Read(void* data, size_t len) override {
        ...
        if (ret < 0) {
            ...
            std::pair<const TransferType, const std::vector<char>> t(READ_ERROR, std::move(buf));
    ...
    virtual ssize_t Write(const void* data, size_t len) override {
        ...
        if (ret != len) {
            const char* err = strerror(errno);
            std::vector<char> buf(err, err + strlen(err));
            std::pair<const TransferType, const std::vector<char>> t(WRITE_ERROR, std::move(buf));
            transfers_.push_back(t);
            return ret;
        }
    ...
    virtual int Reset() override {
        ...
        std::vector<char> buf;
        std::pair<const TransferType, const std::vector<char>> t(RESET, std::move(buf));
        transfers_.push_back(t);
        ProcessSerial();
        return ret;
    }
    ...
    std::string CreateTrace() {
        ...
        for (const auto& tup : transfers_) {
            const std::vector<char>& cbuf = tup.second;
    ...
    void ProcessSerial() {
        ...
        if (select(serial_fd_ + 1, &set, NULL, NULL, &timeout) <= 0) {
            return;
        }
...
class FastBootTest : public testing::Test {
  ...
  protected:
    ...
    void ChangeLockState(bool unlock, bool assert_change = true) {
        ...
        std::string resp;
        EXPECT_EQ(fb->GetVar("unlocked", &resp), SUCCESS);
        ...
        if ((unlock && resp == "no") || (!unlock && resp == "yes")) {
            ...
            fb = std::unique_ptr<FastBootDriver>(
                    new FastBootDriver(transport.get(), [](std::string&) {}, true));
            if (assert_change) {
                ASSERT_EQ(fb->GetVar("unlocked", &resp), SUCCESS);
...
TEST_F(UnlockPermissions, DownloadFlash) {
    ...
    std::vector<std::tuple<std::string, uint32_t>> parts;
    EXPECT_EQ(fb->Partitions(&parts), SUCCESS);
    // EXPECT_EQ(fb->Flash(std::get<0>(parts.front())), DEVICE_SUCCESS);
}
...
TEST_F(LockPermissions, DownloadFlash) {
    ...
    for (const auto tup : parts) {
        ...
        EXPECT_GT(resp.size(), 0);  // meaningful error message
    }
}
...
TEST_F(LockPermissions, SetActive) {
    ...
    std::string resp;
    EXPECT_EQ(fb->GetVar("slot-count", &resp), SUCCESS);
...
TEST_F(LockPermissions, Boot) {
    ...
    EXPECT_EQ(fb->Download(buf), SUCCESS);
    std::string resp;
    EXPECT_EQ(fb->Boot(&resp), DEVICE_FAIL);
...
TEST_F(Fuzz, CommandTooLarge) {
    for (const std::string& s : CMDS) {
        ...
        ASSERT_TRUE(UsbStillAvailible());
        std::string resp;
        EXPECT_EQ(fb->GetVar("product", &resp), SUCCESS);  // make sure still responsive
    }
}
...
TEST_F(Fuzz, USBResetCommandSpam) {
    ...
    do {
        ...
        ASSERT_EQ(transport->Reset(), 0);
        EXPECT_EQ(fb->GetVarAll(&all), SUCCESS);
        EXPECT_EQ(fb->GetVar("product", &resp), SUCCESS);  // make sure still responsive
        elapsed = std::chrono::high_resolution_clock::now() - start;
...
TEST_F(Fuzz, USBResetAfterDownload) {
    std::vector<char> buf;
    buf.resize(1000000);
    EXPECT_EQ(DownloadCommand(buf.size()), SUCCESS);
    EXPECT_EQ(transport->Reset(), 0);
    std::vector<std::string> all;
    EXPECT_EQ(fb->GetVarAll(&all), SUCCESS);
}
