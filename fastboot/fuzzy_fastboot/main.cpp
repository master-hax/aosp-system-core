...
class FastBootTest : public testing::Test {
  ...
  protected:
    ...
    void ChangeLockState(bool unlock) {
        std::string resp;
        ...
        if ((unlock && resp == "no") || (!unlock && resp == "yes")) {
            ...
            transport->Close();
            transport.reset();
            printf("PLEASE CONFIRM '%sing' BOOTLOADER ON DEVICE, THEN PRESS ANY KEY", cmd.c_str());
            getchar();
...
TEST_F(LockPermissions, DownloadFlash) {
    ...
    std::vector<std::tuple<std::string, uint32_t>> parts;
    EXPECT_EQ(fb->Partitions(&parts), SUCCESS);
    EXPECT_EQ(fb->Flash(std::get<0>(parts.front())), DEVICE_FAIL);
}
