...
#ifdef WIN32
static int exec_e2fs_cmd(const char* /*path*/, char* const argv[], char* const envp[]) {
    std::string cmd;
    int i = 0;
...
static int generate_ext4_image(const char* fileName, long long partSize,
    ...
    int ret = exec_e2fs_cmd(mke2fs_args[0], const_cast<char**>(mke2fs_args.data()),
                            const_cast<char**>(mke2fs_envp.data()));
    if (ret != 0) {
    ...
    ret = exec_e2fs_cmd(e2fsdroid_args[0], const_cast<char**>(e2fsdroid_args.data()), nullptr);
    if (ret != 0) {
                               ...
                               unsigned /* unused */, unsigned /* unused */)
{
    ...
    int ret = exec_e2fs_cmd(mkf2fs_args[0], const_cast<char**>(mkf2fs_args.data()), nullptr);
    if (ret != 0) {
