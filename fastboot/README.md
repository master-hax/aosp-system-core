...
The various currently defined commands are:
    ...
    partition          Create partition table using a xml file. Plase see
                       section Partition Table Formart for details.
...
## Partition Table Formart
Fastboot supports "partition" command which will parse a xml file and create
a package message that will be sent to the client side. Client side needs to
...
This is the format of the XML file:
  <!DOCTYPE storage [
    ...
    <!ATTLIST volune  type          CDATA #IMPLIED>
...
Attribute type: memory technology, either "ufs", or "emmc". ufs is not present

### Element volume
represents addressible physical instance on storage device
...
Attribute lun:    unique id of the physical instance for emmc device this is
                  ...
                  is UFS LUN# (0..7). if not presnet volume will be enumerated
...
Attribute size_in_kb: size of partition in units of kylobytes (1024 bytes),
                      decimal integer
...
<storage>
  <volume lun="2" >
    ...
    <partition label="part2B" size_in_kb="1024" type="DEA0BA2C-CBDD-4805-B4F 9-F428251C3E98" bootable="false" readonly="true" />
  </volume>
