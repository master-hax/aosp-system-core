...
```xml
  <!DOCTYPE storage [
    ...
    <!ATTLIST partition erase-block-align CDATA #IMPLIED>
  ]>
```
...
### Element partition
    ...
    Attribute erase-block-align: whether partition has to be aligned on the
                                 (device specific) erase block boundary; boolean
                                 (true/false)
