...
static string get_full_file_path(string file) {
  // files are in the same directory as the final test
  char cdir[PATH_MAX];
  auto count = readlink("/proc/self/exe", cdir, PATH_MAX);
  if (count == -1) {
    fprintf(stderr, "error getting executable path\n");
    return "";
  }
  // readlink doest not append null char
  cdir[count < PATH_MAX ? count : PATH_MAX - 1] = '\0';
  string dir(cdir);
  size_t found = dir.find_last_of("/");
  return dir.substr(0, found + 1) + file;
}
...
TEST(libstorage, parse_error_no_lable) {
