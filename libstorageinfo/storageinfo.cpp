...
class Parser {
  public:
    bool ParseXml(StorageInfo* info, const string& fname);
  ...
  private:
    ...
    static bool ParsePartitionTable(const char**, PartitionTable&, int);
    static bool ParsePartition(const char**, Partition&);
};

bool Parser::ParsePartition(const char** attr, Partition& part) {
...
bool Parser::ParsePartitionTable(const char** attr, PartitionTable& table, int lun) {
...
bool Parser::ParseXml(StorageInfo* info, const string& fname) {
    stor_info_ = info;
...
std::vector<Partition> StorageInfo::GetPartitionsByGroup(const string& name) const {
...
std::vector<PartitionTable> StorageInfo::GetPartitionTablesByGroup(const string& name) const {
...
std::unique_ptr<StorageInfo> StorageInfo::NewStorageInfo(const string& fname) {
    auto info = std::unique_ptr<StorageInfo>(new StorageInfo());
    return Parser().ParseXml(info.get(), fname) ? std::move(info) : nullptr;
}
