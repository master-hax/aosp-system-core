...
static void rotateLogs(android_logcat_context_internal* context)
{
    ...
    if (context->stderr_stdout && context->error) {
        if (context->output && (context->outFD == fileno(context->output))) {
            context->outFD = -1;
        } else if (context->outFD == context->output_fd) {
            context->outFD = -1;
        }
    }
    if (context->error && (context->outFD == fileno(context->error))) {
        context->outFD = -1;
    } else if (context->outFD == context->error_fd) {
        context->outFD = -1;
    }
    close(context->outFD);
    ...
    for (int i = context->maxRotatedLogs ; i > 0 ; i--) {
        ...
        if (i - 1 == 0) {
            ...
            file0 = android::base::StringPrintf("%s.%.*d",
    ...
    context->outFD = openLogFile(context->outputFileName);
...
void printBinary(android_logcat_context_internal* context, struct log_msg* buf)
{
    ...
    TEMP_FAILURE_RETRY(write(context->outFD, buf, size));
}
                          ...
                          log_device_t* dev, struct log_msg* buf)
{
    ...
    if (android_log_shouldPrintLine(context->logformat,
    ...
    if (!dev->printed || printDividers) {
        if (context->devCount > 1 && !context->printBinary) {
            ...
            snprintf(buf, sizeof(buf), "--------- %s %s\n",
                     ...
                     dev->device);
            if (write(context->outFD, buf, strlen(buf)) < 0) {
                logcat_panic(context, HELP_FALSE, "output error");
                return;
            }
        }
...
static void setupOutputAndSchedulingPolicy(
    ...
    if (context->outputFileName == NULL) {
        context->outFD = fileno(context->output);
        return;
    }
    ...
    if (blocking) {
        ...
        if ((sched_setscheduler((pid_t) 0, SCHED_BATCH, &param) < 0) &&
                context->error) {
            fprintf(context->error, "failed to set to batch scheduler\n");
        }
    ...
    struct stat statbuf;
    if (fstat(context->outFD, &statbuf) == -1) {
        close(context->outFD);
        ...
        switch (ret) {
            ...
            case 'm': {
                ...
                if (!getSizeTArg(optarg, &context->maxCount)) {
                    logcat_panic(context, HELP_FALSE, "-%c \"%s\" isn't an "
            ...
            case 'Q':
                ...
                // androidboot.logcat= on the kernel command line. If something
                // is found, it extracts a log filter and uses it to run the
                // program. If nothing is found, the program should quit
                // immediately.
...
#define CONSOLE_OPTION "androidboot.console="
                {
                    ...
                    const char* logcat = strstr(cmdline.c_str(),
                                                KERNEL_OPTION);
                    ...
                    const char* console = strstr(cmdline.c_str(),
                                                 CONSOLE_OPTION );
                    if (!console) break;
...
int android_logcat_destroy(android_logcat_context* ctx) {
    ...
    *ctx = NULL;
    ...
    if (context->output && (context->output != stdout)) {
        if (context->outFD == fileno(context->output)) context->outFD = -1;
        fclose(context->output);
        context->output = NULL;
        context->output_fd = -1;
    }
