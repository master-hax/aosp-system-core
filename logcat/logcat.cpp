...
class Logcat {
  ...
  private:
    ...
    bool RegexOk(const AndroidLogEntry& entry);
...
static void logcat_panic(enum helpType showHelp, const char* fmt, ...) __printflike(2, 3)
...
void Logcat::RotateLogs() {
    ...
    for (int i = max_rotated_logs_; i > 0; i--) {
        std::string file1 = StringPrintf("%s.%.*d", output_file_name_, maxRotationCountDigits, i);

        std::string file0;
        if (!(i - 1)) {
            file0 = output_file_name_;
        } else {
            file0 = StringPrintf("%s.%.*d", output_file_name_, maxRotationCountDigits, i - 1);
        }
...
void Logcat::PrintBinary(struct log_msg* buf) {
    size_t size = buf->len();

    TEMP_FAILURE_RETRY(write(output_fd_.get(), buf, size));
}

bool Logcat::RegexOk(const AndroidLogEntry& entry) {
    if (!regex_) return true;

    return std::regex_search(entry.message, entry.message + entry.messageLen, *regex_);
}
...
int Logcat::SetLogFormat(const char* format_string) {
    ...
    format = android_log_formatFromString(format_string);
        ...
        switch (c) {
            case 0:
                ...
                if (long_options[option_index].name == pid_str) {
                    if (pid != 0) {
                        logcat_panic(HELP_TRUE, "Only supports one PID argument.\n");
                    }
            ...
            case 'm': {
                if (!getSizeTArg(optarg, &max_count_)) {
                    logcat_panic(HELP_FALSE, "-%c \"%s\" isn't an integer greater than zero\n", c,
                                 optarg);
                }
            } break;
            ...
            case 'v': {
                ...
                while (!!(arg = strtok_r(arg, delimiters, &sv))) {
                    ...
                    if (err < 0) {
                        logcat_panic(HELP_FORMAT, "Invalid parameter \"%s\" to -v\n", arg);
                    }
                    arg = nullptr;
...
#define QEMU_CMDLINE "qemu.cmdline"
    ...
    if (forceFilters.size()) {
        ...
        if (!!env_tags_orig) {
            ...
            if (err < 0) {
                logcat_panic(HELP_TRUE, "Invalid filter expression in ANDROID_LOG_TAGS\n");
            }
        }
    } else {
            ...
            if (output_file_name_) {
                ...
                for (int i = max_rotated_logs_; i >= 0; --i) {
                    ...
                    if (!i) {
                        ...
                        file = StringPrintf("%s.%.*d", output_file_name_, maxRotationCountDigits,
                                            i);
                    }
    ...
    if (getSizeFail) {
        logcat_panic(HELP_FALSE, "failed to get the readable '%s' log size", getSizeFail);
    }
