...
General options:
  ...
  -c, --clear                 Clear (flush) the entire log and exit.
                              if -f is specified, clear the fileset instead.
                              if -L is specified, clear the persistent log instead.
...
Formatting:
  -v, --format=<format>       Sets log print format verb and adverbs, where <format> is one of:
                              ...
                              Individually flagged modifying adverbs can be added:
...
Outfile files:
  ...
  --id=<id>                   If the signature <id> for logging to file changes, then clear the
                              fileset and continue.

Logd control:
 These options send a control message to logd, print its return message if applicable, then exit.
 They are incompatible with -L, as there is nothing to control about persistent logs.
...
Filtering:
  ...
  --print                     Paired with --regex and --max-count to let content bypass regex filter
                              but still stop at number of matches.
...
If this happened at top of tree, file a bug.
If this is a local build and you have added significant logging, look into using the -G option to
increase log buffer sizes to handle the additional logging.
)init");
        }
