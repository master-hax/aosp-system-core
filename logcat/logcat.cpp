...
static bool getUintArg(char *ptr, unsigned int *val, unsigned int min = 0,
        ...
        switch(ret) {
            ...
            case 'T':
                if (strspn(optarg, "0123456789") != strlen(optarg)) {
                    ...
                    if (!getUintArg(optarg, &tail_lines, 1) == -1) {
                        fprintf(stderr,
            ...
            case 'r':
                if (!getUintArg(optarg, &g_logRotateSizeKBytes, 1) == -1) {
                    logcat_panic(true, "Invalid parameter %s to -r\n", optarg);
                }
            break;

            case 'n':
                if (!getUintArg(optarg, &g_maxRotatedLogs, 1) == -1) {
...
#define  CONSOLE_OPTION "androidboot.console="
    ...
    if (setPruneList) {
        ...
        char *buf = NULL;
        /*extra 31 bytes are needed by  android_logger_set_prune_list */
        if (!asprintf(&buf, "%-*s", len + 31, setPruneList)) {
            buf[len] = '\0';
        }

        if (!buf || android_logger_set_prune_list(logger_list, buf, len + 32)) {
