...
static unsigned int g_logRotateSizeKBytes = 0;
// 0 means "unbounded"
static unsigned int g_maxRotatedLogs = DEFAULT_MAX_ROTATED_LOGS;
static int g_outFD = -1;
static unsigned int g_outByteCount = 0;
static int g_printBinary = 0;
static int g_devCount = 0;                              // >1 means multiple

static void logcat_exit(int code, bool showHelp, const char *fmt, ...);
...
static void rotateLogs()
{
    ...
    if (g_outFD < 0) {
        logcat_exit(-1, false, "couldn't open output file");
    }
...
static void processBuffer(log_device_t* dev, struct log_msg *buf)
{
    ...
    if (android_log_shouldPrintLine(g_logformat, entry.tag, entry.priority)) {
        ...
        if (bytesWritten < 0) {
            logcat_exit(-1, false, "output error");
        }
    }
...
static void setupOutput()
{
    ...
    if (g_outputFileName == NULL) {
        ...
        if (fstat(g_outFD, &statbuf) == -1) {
            close(g_outFD);
            logcat_exit(-1, false, "couldn't get output file stat\n");
        }
...
/*Srting to unsigned int, returns -1 if it fails*/
static int strtouia(char *ptr, unsigned int *val, unsigned int min = 0,
                     unsigned int max = UINT_MAX)
{
    char *endp;
    long long ret = strtoll(ptr, &endp, 0);

    if (endp[0] != '\0') {
        return -1;
    }

    if (ret > (long long) max || ret < (long long) min) {
        return -1;
    }
        ...
        switch(ret) {
            ...
            case 'T':
                if (strspn(optarg, "0123456789") != strlen(optarg)) {
                    ...
                    if (!cp) {
                        logcat_exit(EXIT_FAILURE, false,
                    ...
                    if (strtouia(optarg, &tail_lines, 1) == -1) {
                        fprintf(stderr,
            ...
            case 'r':
                if (strtouia(optarg, &g_logRotateSizeKBytes, 1) == -1) {
                    logcat_exit(-1, true, "Invalid parameter %s to -r\n", optarg);
                }
            break;

            case 'n':
                if (strtouia(optarg, &g_maxRotatedLogs, 1) == -1) {
                    logcat_exit(-1, true, "Invalid parameter %s to -n\n", optarg);
                }
            break;
            ...
            case 'v':
                ...
                if (err < 0) {
                    logcat_exit(-1, true, "Invalid parameter %s to -v\n", optarg);
                }
...
#define  CONSOLE_OPTION "androidboot.console="
            ...
            case ':':
                logcat_exit(-1, true, "Option -%c needs an argument\n", optopt);
                break;

            default:
                logcat_exit(-1, true, "Unrecognized Option %c\n", optopt);
                break;
        }
    }
    ...
    if (g_logRotateSizeKBytes != 0 && g_outputFileName == NULL) {
        logcat_exit(-1, true, "-r requires -f as well\n");
    }
    ...
    if (forceFilters) {
        ...
        if (err < 0) {
            logcat_exit(0, false, "Invalid filter expression in -logcat option\n");
        }
    } else if (argc == optind) {
        ...
        if (env_tags_orig != NULL) {
            ...
            if (err < 0) {
                logcat_exit(-1, true, "Invalid filter expression in"
                                    " ANDROID_LOG_TAGS\n");
            }
        }
    } else {
        ...
        for (int i = optind ; i < argc ; i++) {
            ...
            if (err < 0) {
                logcat_exit(-1, true, "Invalid filter expression '%s'\n", argv[i]);
            }
        }
    }
    ...
    while (dev) {
        ...
        if (!dev->logger) {
            logcat_exit(EXIT_FAILURE, false, "Unable to open log device '%s'\n",
                        dev->device);
        }
        ...
        if (clearLog) {
            ...
            if (ret) {
                logcat_exit(EXIT_FAILURE, false, "failed to clear the log");
            }
        }

        if (setLogSize && android_logger_set_log_size(dev->logger, setLogSize)) {
            logcat_exit(EXIT_FAILURE, false, "failed to set the log size");
        }
        ...
        if (getLogSize) {
            ...
            if (size < 0) {
                logcat_exit(EXIT_FAILURE, false, "failed to get the log size");
            }

            readable = android_logger_get_log_readable_size(dev->logger);
            if (readable < 0) {
                logcat_exit(EXIT_FAILURE, false, "failed to get the readable"
                                                 " log size");
            }
    ...
    if (setPruneList) {
        ...
        char *buf;
        if (asprintf(&buf, "%*s", len, setPruneList)) {
            buf = NULL;
        }

        if (!buf || android_logger_set_prune_list(logger_list, buf, len)) {
            logcat_exit(EXIT_FAILURE, false, "failed to set the prune list");
        }
    ...
    if (printStatistics || getPruneList) {
        ...
        if (!buf) {
            logcat_exit(EXIT_FAILURE, false, "failed to read data");
        }
    ...
    while (1) {
        ...
        if (ret == 0) {
            logcat_exit(EXIT_FAILURE, false, "read: unexpected EOF!\n");
        }
        ...
        if (ret < 0) {
            ...
            if (ret == -EINVAL) {
                logcat_exit(EXIT_FAILURE, false, "read: unexpected length.\n");
            }
            logcat_exit(EXIT_FAILURE, false, "logcat read failure");
        }
