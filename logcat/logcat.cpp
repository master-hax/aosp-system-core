...
static void setupOutput()
{
    ...
    if (g_outputFileName == NULL) {
        ...
        if (statbuf.st_size > SIZE_MAX || statbuf.st_size < 0) {
            close(g_outFD);
        ...
        switch(ret) {
            ...
            case 'T':
                if (strspn(optarg, "0123456789") != strlen(optarg)) {
                    ...
                    if (!getSizeTArg(optarg, &tail_lines, 1) == -1) {
                        fprintf(stderr,
            ...
            case 'r':
                if (!getSizeTArg(optarg, &g_logRotateSizeKBytes, 1) == -1) {
                    logcat_panic(true, "Invalid parameter %s to -r\n", optarg);
                }
            break;

            case 'n':
                if (!getSizeTArg(optarg, &g_maxRotatedLogs, 1) == -1) {
...
#define  CONSOLE_OPTION "androidboot.console="
    ...
    if (setPruneList) {
        ...
        if (asprintf(&buf, "%-*s", len + 31, setPruneList) > 0) {
            buf[len] = '\0';
            if (android_logger_set_prune_list(logger_list, buf, len + 32)) {
