...
class Logcat {
  ...
  private:
    ...
    void MaybePrintStart(log_id_t log_id, bool print_dividers);
...
void Logcat::MaybePrintStart(log_id_t log_id, bool print_dividers) {
...
void ReportErrorName(const std::string& name, bool block_security,
                     std::vector<std::string>* errors) {
    if (!block_security || name != "security") {
...
#define QEMU_CMDLINE "qemu.cmdline"
    ...
    for (int i = LOG_ID_MIN; i < LOG_ID_MAX; ++i) {
        ...
        if (logger == nullptr) {
            ReportErrorName(buffer_name, all_selected, &open_device_failures);
            continue;
        }
            ...
            if (output_file_name_) {
                ...
                for (int i = max_rotated_logs_; i >= 0; --i) {
                    ...
                    if (!file.length()) {
                        ...
                        ReportErrorName(buffer_name, all_selected, &clear_failures);
                        break;
                    }
        ...
        if (setLogSize) {
            if (android_logger_set_log_size(logger, setLogSize)) {
                ReportErrorName(buffer_name, all_selected, &set_size_failures);
            }
        }
        ...
        if (getLogSize) {
            ...
            if (size < 0 || readable < 0) {
                ReportErrorName(buffer_name, all_selected, &get_size_failures);
            } else {
    ...
    while (!max_count_ || print_count_ < max_count_) {
        ...
        MaybePrintStart(log_msg.id(), printDividers);
