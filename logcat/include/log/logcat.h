...
class android_logcat {
  ...
  public:
    ...
    ~android_logcat() {
      // likely an over-pclose, but required if caller
      // did not explictly call pclose() method.
      android_logcat_pclose(&_ctx, _fp);
    }
    ...
    // Added for completeness, straight static inline method
    // punt to android_logcat_system() C interface call.
