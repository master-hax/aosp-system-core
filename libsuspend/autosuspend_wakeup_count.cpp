...
static void* suspend_thread_func(void* arg __attribute__((unused))) {
    ...
    while (1) {
        ...
        ReadFileToString(sys_power_wakeup_count, &wakeup_count);
        ...
        ret = WriteStringToFile(wakeup_count, sys_power_wakeup_count);
        if (ret == 0) {
            PLOG(ERROR) << "error writing to " << sys_power_wakeup_count;
        } else {
            LOG(VERBOSE) << "write " << sleep_state << " to " << sys_power_state;
            success = WriteStringToFile(sleep_state, sys_power_state);
