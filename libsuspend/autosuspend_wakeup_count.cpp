...
static int forcesuspend_init(void) {
    static bool forcesuspend_is_init = false;

    if (forcesuspend_is_init) {
        return 0;
    }
    ...
    LOG(INFO) << "forcesuspend_init success";
    forcesuspend_is_init = true;
    return 0;
}
...
static int autosuspend_init(void) {
    ...
    int ret = forcesuspend_init();
    if (ret < 0) {
        LOG(ERROR) << "error in forcesuspend_init";
