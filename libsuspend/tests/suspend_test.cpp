...
TEST(suspend, auto) {
    ...
    if (isWakeLock("development")) {
        EXPECT_EQ(WriteStringToFile("development", "/sys/power/wake_unlock"), true);
    }
    ...
    bool adbd_running = Trim(readCommand("getprop init.svc.adbd")) == "running";
    if (adbd_running) {
    ...
    if (usb_hal.length()) {
        ...
        EXPECT_TRUE(SetProperty("ctl.stop", usb_hal));
        EXPECT_TRUE(WaitForProperty("init.svc." + usb_hal, "stopped", 1s));
    }

    // Switch port to host (source) mode
    static const std::string port_file = "/sys/class/typec/port0/port_type";
    std::string port_type = Trim(readCommand(
        ("sed -n 's/.*[[]\\(.*\\)[]].*/\\1/p' " + port_file + " 2>/dev/null >/dev/null").c_str()));
    if (port_type == "source") {
        port_type = "";
    } else {
        GTEST_LOG_(INFO) << "USB temporarily set to host mode.";
        WriteStringToFile("source", port_file);
    }
    ...
    if (port_type.length()) {
        WriteStringToFile(port_type, port_file);
    }

    // Restart USB HAL daemon
    if (usb_hal.length()) {
        EXPECT_TRUE(SetProperty("ctl.start", usb_hal));
    }
