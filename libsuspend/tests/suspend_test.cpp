...
bool isWakeLock(const char* name) {
    ...
    for (auto& l : locks) {
        if ((name == nullptr) && (l.length() > 0)) return true;
        if ((name != nullptr) && (l == name)) return true;
    }
    return false;
}
...
TEST(suspend, auto) {
    ...
    close(0);
    ASSERT_EQ(readCommand("ls -l /proc/self/fd").find(" -> /dev/pts/"), std::string::npos);
    ...
    size_t pos;
    ASSERT_NE(pos = dmesg.find(marker), std::string::npos);
    dmesg.erase(0, pos + marker.length());

    EXPECT_NE(dmesg.find("PM: suspend entry "), std::string::npos);
    EXPECT_NE(dmesg.find("PM: suspend exit "), std::string::npos);
}
