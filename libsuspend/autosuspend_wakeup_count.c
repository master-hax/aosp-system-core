...
#define SUSPEND_RESULT_SUCCESS (1)               // Suspend was successful
#define SUSPEND_RESULT_ATTEMPTED (0)             // Suspend was attempted but didn't happen
#define SUSPEND_RESULT_MUTEX_NOT_AVAILABLE (-1)  // Suspend couldn't get mutex
#define SUSPEND_RESULT_WAKECOUNT_ERROR (-2)      // Suspend wakecount isn't correct

static int state_fd;
static int wakeup_count_fd;
static pthread_t suspend_thread;
static sem_t autosuspend_lockout;
static sem_t suspend_mutex;
...
static int do_suspend(void) {
    ...
    ALOGV("%s: get suspend mutex", __func__);
    ret = sem_wait(&suspend_mutex);
    if (ret < 0) {
        strerror_r(errno, buf, sizeof(buf));
        ALOGE("Error getting suspend mutex: %s", buf);
        return SUSPEND_RESULT_MUTEX_NOT_AVAILABLE;
    }
    ...
    if (ret < 0) {
        ...
        if (ret >= 0) {
            success = true;
            ret_val = SUSPEND_RESULT_SUCCESS;
        } else {
            success = false;
            ret_val = SUSPEND_RESULT_ATTEMPTED;
        }
...
exit:
    ...
    ret = sem_post(&suspend_mutex);
    if (ret < 0) {
        strerror_r(errno, buf, sizeof(buf));
        ALOGE("Error releasing mutex: %s", buf);
    }
    return ret_val;
}
...
struct autosuspend_ops* autosuspend_wakeup_count_init(void) {
    ...
    ret = sem_init(&suspend_mutex, 0, 1);
    if (ret < 0) {
        strerror_r(errno, buf, sizeof(buf));
        ALOGE("Error creating semaphore: %s", buf);
        goto err_suspend_mutex_init;
    }
...
err_pthread_create:
    sem_destroy(&suspend_mutex);
err_suspend_mutex_init:
