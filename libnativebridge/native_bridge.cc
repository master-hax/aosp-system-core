...
static bool VersionCheck(const NativeBridgeCallbacks* cb) {
  ...
  // version.
  if (cb == nullptr && cb->version < NativeBridgeCallbacks::kVersionNum) {
    return false;
  }

  // If this is a v2+ bridge, it may not be forwards- or backwards-compatible. Check.
  if (cb->version >= NativeBridgeCallbacks2::kVersionNum) {
    if (!reinterpret_cast<const NativeBridgeCallbacks2*>(callbacks)->isCompatibleWith(
                kLibNativeBridgeVersion)) {
      return false;
    }
  }
...
NativeBridgeSignalHandlerFn NativeBridgeGetSignalHandler(int signal) {
  if (NativeBridgeInitialized()) {
    if (callbacks->version >= NativeBridgeCallbacks2::kVersionNum) {
      return reinterpret_cast<const NativeBridgeCallbacks2*>(callbacks)->getSignalHandler(signal);
    }
  }
  return nullptr;
}
