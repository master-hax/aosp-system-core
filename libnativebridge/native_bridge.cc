...
static const char* kCodeCache = "code_cache";
...
void PreInitializeNativeBridge(const char* app_data_dir_in, uid_t uid, gid_t gid) {
  ...
  if (len < 1000) {  // 1024B buffer, needs name for "/code_cache/cpuinfo", rounded up.
    // Make sure the cpuinfo file exists.
    char cpuinfo_path[1024];
    strncpy(cpuinfo_path, app_data_dir, len);
    cpuinfo_path[len] = '/';
    const size_t cc_len = strlen(kCodeCache);
    strncpy(cpuinfo_path + len + 1, kCodeCache, cc_len);
    cpuinfo_path[len + 1 + cc_len] = '/';
    const size_t ci_len = strlen(kCpuinfo);
    strncpy(cpuinfo_path + len + 1 + cc_len + 1, kCpuinfo, ci_len);

    // Open RD | CREATE to make sure it exists, but leave existing file untouched.
    mode_t mode = S_IRUSR | S_IWUSR | S_IRGRP;
    int fd = open(cpuinfo_path, O_RDONLY | O_CREAT, mode);
    if (fd != -1) {
      close(fd);

      // Bind-mount.
      if (TEMP_FAILURE_RETRY(mount("/proc/cpuinfo", cpuinfo_path,
                                   nullptr, MS_BIND, nullptr)) == -1) {
        ALOGW("Failed to bind-mount %s as /proc/cpuinfo: %d", cpuinfo_path, errno);
      }

      // Give the file to the process.
      if (chown(cpuinfo_path, uid, gid) != 0) {
        ALOGW("Failed to give %s to process: %d", cpuinfo_path, errno);
      }
    } else {
      ALOGW("Could not create/open cpuinfo at %s: %d", cpuinfo_path, errno);
    }
  }
}
...
static void SetSupportedAbis(JNIEnv* env, jclass build_class, const char* field,
  ...
  if (value != nullptr) {
    ...
    if (field_id != nullptr) {
      ...
      jstring str2 = nullptr;
      if (value2 != nullptr) {
        str2 = env->NewStringUTF(value2);
        if (str2 == nullptr) {
          env->ExceptionClear();
          ALOGW("Could not create string %s.", value2);
          return;
        }
      }

      jobjectArray array = env->NewObjectArray(value2 != nullptr ? 2 : 1,
      ...
      if (str2 != nullptr) {
        // Value 1 is already set by NewObjectArray.
        env->SetObjectArrayElement(array, 1, str2);
      }
...
static void SetupEnvironment(NativeBridgeCallbacks* callbacks, JNIEnv* env, const char* isa) {
  ...
  if (env_values->cpu_abi != nullptr || env_values->cpu_abi2 != nullptr) {
    ...
    if (bclass_id != nullptr) {
      ...
      SetSupportedAbis(env, bclass_id, "SUPPORTED_ABIS", env_values->cpu_abi, env_values->cpu_abi2);
    } else {
