...
#define COPY_BUF_SIZE ((int64_t)(1024U*1024U))
static char *copybuf;
...
static void verbose_error(bool verbose, int err, const char *fmt, ...)
{
	if (verbose) {
		std::string msg;
		if (err == -EOVERFLOW) msg = "EOF while reading file";
		else if (err == -EINVAL) msg = "Invalid sparse file format";
		else if (err == -ENOMEM) msg = "Failed allocation while reading file";
		else msg = android::base::StringPrintf("Unknown error %d", err);
