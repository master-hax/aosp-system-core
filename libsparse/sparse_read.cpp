...
static void verbose_error(bool verbose, int err, const char *fmt, ...)
{
	...
	std::string msg =
	  (err == -EOVERFLOW) ? "EOF while reading file" :
	  (err == -EINVAL)    ? "Invalid sparse file format" :
	  (err == -ENOMEM)    ? "Failed allocation while reading file" :
	  android::base::StringPrintf("Unknown error %d", err);
