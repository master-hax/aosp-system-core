...
static int process_chunk(struct sparse_file* s, SparseFileSource* source, unsigned int chunk_hdr_sz,
    ...
    switch (chunk_header->chunk_type) {
        case CHUNK_TYPE_RAW:
            ret = process_raw_chunk(s, chunk_data_size, source, chunk_header->chunk_sz, cur_block,
                                    crc_ptr);
            if (ret < 0) {
...
static int sparse_file_read_sparse(struct sparse_file* s, SparseFileSource* source, bool crc) {
    ...
    for (i = 0; i < sparse_header.total_chunks; i++) {
        ...
        ret =
            process_chunk(s, source, sparse_header.chunk_hdr_sz, &chunk_header, cur_block, crc_ptr);
        if (ret < 0) {
            return ret;
        }
