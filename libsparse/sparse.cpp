...
int sparse_file_callback(struct sparse_file* s, bool sparse, bool crc,
  ...
  chunks = sparse_count_chunks(s);
  out = output_file_open_callback(write, nullptr, nullptr, nullptr,
          priv, s->block_size, s->len, false, sparse, chunks, crc);
...
int sparse_file_callback_typed(struct sparse_file *s, bool sparse,
		...
		int (*skip_write)(void *priv, int64_t len),
        void *priv) {
  int ret;
  int chunks;
  struct output_file* out;
  ...
  chunks = sparse_count_chunks(s);
  out = output_file_open_callback(data_write, fd_write, fill_write, skip_write,
          priv, s->block_size, s->len, false, sparse, chunks, false);
...
int sparse_file_foreach_chunk(struct sparse_file* s, bool sparse, bool crc,
  ...
  chunks = sparse_count_chunks(s);
  out = output_file_open_callback(foreach_chunk_write, nullptr, nullptr, nullptr,
          &chk, s->block_size, s->len, false, sparse, chunks, crc);
...
int64_t sparse_file_len(struct sparse_file* s, bool sparse, bool crc) {
  ...
  out = output_file_open_callback(out_counter_write, nullptr, nullptr, nullptr,
          &count, s->block_size, s->len, false, sparse, chunks, crc);
  if (!out) {
    return -1;
  }
...
static struct backed_block* move_chunks_up_to_len(struct sparse_file* from, struct sparse_file* to,
  ...
  out_counter = output_file_open_callback(out_counter_write, nullptr, nullptr, nullptr,
          &count, to->block_size, to->len, false, true, 0, false);
  if (!out_counter) {
    return NULL;
  }
