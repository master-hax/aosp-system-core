...
static int callback_file_skip(struct output_file* out, int64_t off) {
  ...
  while (off > 0) {
    ...
    ret = outc->data_write(outc->priv, NULL, to_write);
    if (ret < 0) {
      return ret;
    }
    off -= to_write;
...
static int write_sparse_data_chunk_variant(struct output_file* out, unsigned int len, std::variant<int, void*> data) {
  chunk_header_t chunk_header;
  ...
  if (std::holds_alternative<int>(data)) {
    ...
    ret = out->ops->write_fd(out, std::get<int>(data), len);
  } else {
    ret = out->ops->write(out, std::get<void*>(data), len);
    if (out->use_crc) {
      out->crc32 = sparse_crc32(out->crc32, std::get<void*>(data), len);
    }
  }
  if (ret < 0) return -1;
  if (zero_len) {
...
static int write_sparse_data_chunk(struct output_file* out, unsigned int len, void* data) {
    return write_sparse_data_chunk_variant(out, len, data);
}

static int write_sparse_fd_chunk(struct output_file* out, unsigned int len, int fd) {
    return write_sparse_data_chunk_variant(out, len, fd);
}
...
static int write_normal_data_chunk(struct output_file* out, unsigned int len, void *data) {
  return write_normal_data_chunk_variant(out, len, data);
}

static int write_normal_fd_chunk(struct output_file* out, unsigned int len, int fd) {
  return write_normal_data_chunk_variant(out, len, fd);
}
