...
struct sparse_file_ops {
  ...
  int (*write_fill_chunk)(struct output_file* out, int64_t len, uint32_t fill_val);
...
static int write_sparse_fill_chunk(struct output_file* out, int64_t len, uint32_t fill_val) {
  chunk_header_t chunk_header;
  int64_t rnd_up_len;
  int count;
  int ret;
...
static int write_sparse_data_chunk(struct output_file* out, int64_t len, void* data) {
  chunk_header_t chunk_header;
...
static int write_normal_data_chunk(struct output_file* out, int64_t len, void* data) {
  int ret;
...
static int write_normal_fill_chunk(struct output_file* out, int64_t len, uint32_t fill_val) {
  int ret;
  unsigned int i;
  int64_t write_len;
  ...
  while (len) {
    write_len = std::min(len, (int64_t)out->block_size);
...
int write_fd_chunk(struct output_file* out, int64_t len, int fd, int64_t offset) {
...
int write_file_chunk(struct output_file* out, int64_t len, const char* file, int64_t offset) {
  int ret;
