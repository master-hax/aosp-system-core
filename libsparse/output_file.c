...
struct sparse_file_ops {
	...
	int (*write_fill_chunk)(struct output_file *out, ssize_t len,
			uint32_t fill_val);
	int (*write_skip_chunk)(struct output_file *out, off_t len);
...
{
	int ret;
	ssize_t rnd_up_len = ALIGN(len, out->block_size);
		...
		uint32_t fill_val)
{
	...
	while (len) {
		write_len = min((uint32_t)len, out->block_size);
...
static int write_normal_skip_chunk(struct output_file *out, off_t len)
{
