...
struct output_file_ops {
	...
	int (*write)(struct output_file *, void *, unsigned int);
...
static int file_write(struct output_file *out, void *data, unsigned int len)
{
	int ret;
	struct output_file_normal *outn = to_output_file_normal(out);
...
static int gz_file_write(struct output_file *out, void *data, unsigned int len)
{
	int ret, errnum;
	struct output_file_gz *outgz = to_output_file_gz(out);
	...
	while (len > 0) {
		...
		if (ret == 0) {
			error("gzwrite %s", gzerror(outgz->gz_fd, &errnum));
			return -1;
		}
		len -= ret;
		data = (char *)data + ret;
...
static int callback_file_write(struct output_file *out, void *data, unsigned int len)
{
	struct output_file_callback *outc = to_output_file_callback(out);
