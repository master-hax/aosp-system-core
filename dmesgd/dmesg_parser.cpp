...
DmesgParser::DmesgParser() {
    ...
    this->register_pattern_ = std::regex("([ _][Rx]..|raw): [0-9a-f]{16}");
 ...
 * following actions:
 *  1. Drop the entire line, if it contains process or hardware name:
...
std::string DmesgParser::StripSensitiveData(const std::string& line) const {
    for (auto skip : {"Comm: ", "Hardware name: "}) {
        if (line.find(skip) != std::string::npos) return "";
    }

    if (std::regex_search(line, register_pattern_)) return "";
