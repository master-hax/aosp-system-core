...
bool NetlinkEvent::parseIfAddrMessage(const struct nlmsghdr *nh) {
    ...
    for (rta = IFA_RTA(ifaddr); RTA_OK(rta, len); rta = RTA_NEXT(rta, len)) {
        if (rta->rta_type == IFA_ADDRESS) {
            ...
            // Get the low byte of the flags here, and the rest from the IFA_FLAGS attribute, in
            // whatever order they appear. In practice IFA_FLAGS is always after IFA_ADDRESS, so
            // this isn't necessary, but it's technically more correct.
            ...
            flags = flags | (*(uint32_t*)RTA_DATA(rta) & 0xffffff00);
        }
    }
