...
void* getLibHandle() {
  ...
  LOG_FATAL_IF(handle == nullptr, "Failed to load libnativeloader.so:" + strerror(errno));
  return handle;
}
...
FuncPtr getFuncPtr(const char* functionName) {
  ...
  LOG_FATAL_IF(f == nullptr, "Failed to get address of " + functionName + ": " + strerror(errno));
  return f;
}
