...
#define LOG_TAG "libnativeloader_shim"
#include <log/log.h>
...
void* getLibHandle() {
  ...
  LOG_FATAL_IF(handle == nullptr, "Failed to load libnativeloader.so: %s", strerror(errno));
  return handle;
}

template <typename FuncPtr>
FuncPtr getFuncPtr(const char* functionName) {
  auto f = reinterpret_cast<FuncPtr>(dlsym(getLibHandle(), functionName));
  LOG_FATAL_IF(f == nullptr, "Failed to get address of %s: %s", functionName, strerror(errno));
  return f;
}
