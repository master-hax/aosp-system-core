...
class LibraryNamespaces {
 public:
  ...
  android_namespace_t* GetOrCreate(JNIEnv* env, jobject class_loader,
                                   ...
                                   jstring java_permitted_when_isolated_path) {
    ScopedUtfChars library_path(env, java_library_path);

    std::string permitted_when_isolated_path;
    if (java_permitted_when_isolated_path != nullptr) {
      ScopedUtfChars path(env, java_permitted_when_isolated_path);
      permitted_when_isolated_path = path.c_str();
    }
    ...
    android_namespace_t* ns =
            android_create_namespace("classloader-namespace",
                                     ...
                                     library_path.c_str(),
                                     true,
                                     java_permitted_when_isolated_path != nullptr ?
                                         permitted_when_isolated_path.c_str() :
                                         nullptr);
...
void* OpenNativeLibrary(JNIEnv* env, int32_t target_sdk_version, const char* path,
  ...
  android_namespace_t* ns =
      g_namespaces->GetOrCreate(env, class_loader, java_library_path,
                                java_permitted_when_isolated_path);
  ...
  UNUSED(env, target_sdk_version, class_loader,
         java_library_path, java_permitted_when_isolated_path);
  return dlopen(path, RTLD_NOW);
#endif
}
