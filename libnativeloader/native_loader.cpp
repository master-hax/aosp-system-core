...
static const std::function<bool(const std::string&, std::string*)> no_check =
...
class LibraryNamespaces {
 public:
  ...
  void Initialize() {
    ...
    std::string error_msg;
    LOG_ALWAYS_FATAL_IF(
        !ReadConfig(public_native_libraries_system_config, &sonames, no_check, &error_msg),
    ...
    if (dir != nullptr) {
      ...
      while ((ent = readdir(dir.get())) != nullptr) {
        ...
        if (android::base::StartsWith(filename, kPublicNativeLibrariesExtensionConfigPrefix) &&
          ...
          const size_t start = strlen(kPublicNativeLibrariesExtensionConfigPrefix);
          const size_t end = filename.size() - strlen(kPublicNativeLibrariesExtensionConfigSuffix);
          ...
          LOG_ALWAYS_FATAL_IF(
              !ReadConfig(config_file_path, &sonames,
                          [&company_name](const std::string& trimmed_line, std::string* error_msg) {
                            if (android::base::StartsWith(trimmed_line, "lib") &&
                                android::base::EndsWith(
                                    trimmed_line, std::string("." + company_name + ".so").c_str())) {
                              return true;
                            } else {
                              *error_msg = "Library name \"" + trimmed_line +
    ...
    sonames.clear();
    ReadConfig(llndk_native_libraries_system_config, &sonames, no_check);
    system_llndk_libraries_ = base::Join(sonames, ':');

    sonames.clear();
    ReadConfig(vndksp_native_libraries_system_config, &sonames, no_check);
    system_vndksp_libraries_ = base::Join(sonames, ':');

    sonames.clear();
    // This file is optional, quietly ignore if the file does not exist.
    ReadConfig(kPublicNativeLibrariesVendorConfig, &sonames, no_check, nullptr);
 ...
 private:
  bool ReadConfig(const std::string& configFile, std::vector<std::string>* sonames,
                  const std::function<bool(const std::string& /* trimmed_line */,
                                           std::string* /* error_msg */)>& check_line,
    ...
    for (auto& line : lines) {
      ...
      if (check_line(trimmed_line, error_msg)) {
