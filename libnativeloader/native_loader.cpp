...
class LibraryNamespaces {
 public:
  ...
  bool Create(JNIEnv* env,
    ...
    bool is_native_bridge;

    if (found_parent_namespace) {
      is_native_bridge = !parent_ns.is_android_namespace();
    } else {
      is_native_bridge = NativeBridgeIsSupported(library_path.c_str());
    }
 ...
 private:
  ...
  bool InitPublicNamespace(const char* library_path, std::string* error_msg) {
    ...
    bool is_native_bridge = NativeBridgeIsSupported(library_path);
...
#if defined(__ANDROID__)
static void* nbLoadLibraryWrapper(const char* path, native_bridge_namespace_t* ns) {
  if (ns != nullptr) {
...
void* OpenNativeLibrary(JNIEnv* env,
  ...
  if (ns.is_android_namespace()) {
    ...
    void* handle = nbLoadLibraryWrapper(path, ns.get_native_bridge_ns());
    if (handle == nullptr) {
      *error_msg = dlerror();
    } else {
  ...
  if (handle == nullptr) {
    ...
    if (handle != nullptr) {
      *needs_native_bridge = true;
      ...
      // in this case we assume that the correct error_msg is coming from dlerror();
      *error_msg = dlerror();
    }
  }
  return handle;
