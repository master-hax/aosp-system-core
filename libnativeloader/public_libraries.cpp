...
void ReadExtensionLibraries(const char* dirname, std::vector<std::string>* sonames) {
  ...
  if (dir != nullptr) {
    ...
    while (struct dirent* ent = readdir(dir.get())) {
      ...
      if (android::base::StartsWith(filename, kExtendedPublicLibrariesFilePrefix) &&
          android::base::EndsWith(filename, kExtendedPublicLibrariesFileSuffix)) {
        const size_t start = sizeof(kExtendedPublicLibrariesFilePrefix) - 1;
        const size_t end = filename.size() - (sizeof(kExtendedPublicLibrariesFileSuffix) - 1);
