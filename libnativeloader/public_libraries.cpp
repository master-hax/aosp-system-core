...
const std::vector<const std::string> kArtPublicLibraries = {
    "libicuuc.so",
    "libicui18n.so",
};
...
static std::string InitDefaultPublicLibraries() {
  ...
  for (const std::string& lib_name : kArtPublicLibraries) {
...
static std::string InitArtPublicLibraries() {
  CHECK(sizeof(kArtPublicLibraries) > 0);
  std::string list = android::base::Join(kArtPublicLibraries, ":");
