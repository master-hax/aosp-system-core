...
// gmock interface for platform APIs on libdl and libnativebridge
class Platform {
 public:
  virtual ~Platform() {}

  // libdl APIs
  virtual void* dlopen(const char*, int) = 0;
  virtual int dlclose(void*) = 0;
  virtual char* dlerror(void) = 0;
  virtual bool android_init_anonymous_namespace(const char*, const char*) = 0;
  virtual struct android_namespace_t* android_create_namespace(const char*, const char*,
                                                               const char*, uint64_t, const char*,
                                                               struct android_namespace_t*) = 0;
  virtual bool android_link_namespaces(struct android_namespace_t*, struct android_namespace_t*,
                                       const char*) = 0;
  virtual struct android_namespace_t* android_get_exported_namespace(const char*) = 0;
  virtual void* android_dlopen_ext(const char*, int, const android_dlextinfo*) = 0;

  // libnativebridge APIs
  virtual bool NativeBridgeInitialized() = 0;
  virtual bool NativeBridgeInitAnonymousNamespace(const char*, const char*) = 0;
  ...
  virtual struct native_bridge_namespace_t* NativeBridgeGetExportedNamespace(const char*) = 0;
  virtual struct native_bridge_namespace_t* NativeBridgeCreateNamespace(
      const char*, const char*, const char*, uint64_t, const char*,
      struct native_bridge_namespace_t*) = 0;
  virtual bool NativeBridgeLinkNamespaces(struct native_bridge_namespace_t*,
                                          struct native_bridge_namespace_t*, const char*) = 0;
  virtual void* NativeBridgeLoadLibraryExt(const char*, int, struct native_bridge_namespace_t*) = 0;

  // To mock "ClassLoader Object.getParent()"
  virtual const char* GetParentClassLoader(const char*) = 0;
};

// The mock does not actually create namespace object. But simply casts the pointer to the
...
static std::unordered_map<std::string, struct android_namespace_t*> namespaces = {
    {"platform", TO_ANDROID_NAMESPACE("platform")}, {"default", TO_ANDROID_NAMESPACE("default")},
    {"runtime", TO_ANDROID_NAMESPACE("runtime")},   {"sphal", TO_ANDROID_NAMESPACE("sphal")},
    {"vndk", TO_ANDROID_NAMESPACE("vndk")},
};
...
class MockPlatform : public Platform {
 public:
  MockPlatform(bool is_bridged) : is_bridged_(is_bridged) {
    ...
    ON_CALL(*this, android_get_exported_namespace(_)).WillByDefault(Invoke([
    ](const char* name) -> struct android_namespace_t* {
  ...
  MOCK_METHOD2(android_init_anonymous_namespace, bool(const char*, const char*));
  MOCK_METHOD6(android_create_namespace,
               struct android_namespace_t*(const char*, const char*, const char*, uint64_t,
                                           const char*, struct android_namespace_t*));
  MOCK_METHOD3(android_link_namespaces,
               bool(struct android_namespace_t*, struct android_namespace_t*, const char*));
  MOCK_METHOD1(android_get_exported_namespace, struct android_namespace_t*(const char*));
  MOCK_METHOD3(android_dlopen_ext, void*(const char*, int, const android_dlextinfo*));

  // Mocking libnativebridge APIs
  MOCK_METHOD0(NativeBridgeInitialized, bool());
  MOCK_METHOD2(NativeBridgeInitAnonymousNamespace, bool(const char*, const char*));
  ...
  MOCK_METHOD1(NativeBridgeGetExportedNamespace, struct native_bridge_namespace_t*(const char*));
  MOCK_METHOD6(NativeBridgeCreateNamespace,
               struct native_bridge_namespace_t*(const char*, const char*, const char*, uint64_t,
                                                 const char*, struct native_bridge_namespace_t*));
  MOCK_METHOD3(NativeBridgeLinkNamespaces, bool(struct native_bridge_namespace_t*,
                                                struct native_bridge_namespace_t*, const char*));
  MOCK_METHOD3(NativeBridgeLoadLibraryExt,
               void*(const char*, int, struct native_bridge_namespace_t*));

  // Mocking "ClassLoader Object.getParent()"
  MOCK_METHOD1(GetParentClassLoader, const char*(const char*));

 private:
  bool is_bridged_;
};

std::unique_ptr<MockPlatform> mock;
...
bool android_init_anonymous_namespace(const char* sonames, const char* search_path) {
  return mock->android_init_anonymous_namespace(sonames, search_path);
}
struct android_namespace_t* android_create_namespace(const char* name, const char* ld_library_path,
  ...
  return mock->android_create_namespace(name, ld_library_path, default_library_path, type,
...
bool android_link_namespaces(struct android_namespace_t* from, struct android_namespace_t* to,
                             const char* sonames) {
  return mock->android_link_namespaces(from, to, sonames);
}
struct android_namespace_t* android_get_exported_namespace(const char* name) {
  return mock->android_get_exported_namespace(name);
}

void* android_dlopen_ext(const char* filename, int flags, const android_dlextinfo* info) {
  return mock->android_dlopen_ext(filename, flags, info);
}
...
struct native_bridge_namespace_t* NativeBridgeGetExportedNamespace(const char* name) {
  return mock->NativeBridgeGetExportedNamespace(name);
}
struct native_bridge_namespace_t* NativeBridgeCreateNamespace(
  ...
  return mock->NativeBridgeCreateNamespace(name, ld_library_path, default_library_path, type,
                                           permitted_when_isolated_path, parent);
}
bool NativeBridgeLinkNamespaces(struct native_bridge_namespace_t* from,
  ...
  return mock->NativeBridgeLinkNamespaces(from, to, sonames);
}
void* NativeBridgeLoadLibraryExt(const char* libpath, int flag,
                                 struct native_bridge_namespace_t* ns) {
  return mock->NativeBridgeLoadLibraryExt(libpath, flag, ns);
}
...
bool NativeBridgeInitAnonymousNamespace(const char* public_ns_sonames,
  ...
  return mock->NativeBridgeInitAnonymousNamespace(public_ns_sonames, anon_ns_library_path);
}
...
// Only a few JNI methods that are actually used in libnativeloader are implemented.
JNINativeInterface* CreateJNINativeInterface() {
  ...
  inf->CallObjectMethodV = [](JNIEnv*, jobject obj, jmethodID mid, va_list) -> jobject {
    if (strcmp("getParent", reinterpret_cast<const char*>(mid)) == 0) {
      // GetParentClassLoader can be a valid jobject or nullptr if there is
      // no parent classloader.
      const char* ret = mock->GetParentClassLoader(reinterpret_cast<const char*>(obj));
...
class NativeLoaderTest : public ::testing::Test {
 protected:
  void SetUp() override {
    mock = std::make_unique<NiceMock<MockPlatform>>(bridged);
...
class NativeLoader : public NativeLoaderTest<false> {};
class BridgedLoader : public NativeLoaderTest<true> {};

/////////////////////////////////////////////////////////////////

TEST_F(NativeLoader, InitializeLoadsDefaultPublicLibraries) {
  SetExpectation();
  RunTest();
}

TEST_F(BridgedLoader, InitializeLoadsDefaultPublicLibraries) {
  SetExpectation();
  RunTest();
}
...
template <bool bridged>
class NativeLoaderTest_Create : public NativeLoaderTest<bridged> {
 protected:
  void SetExpectation() {
    NativeLoaderTest<bridged>::SetExpectation();

    ON_CALL(*mock, GetParentClassLoader(StrEq(class_loader))).WillByDefault(Return(nullptr));

    EXPECT_CALL(*mock, NativeBridgeIsPathSupported(_)).Times(AnyNumber());
    EXPECT_CALL(*mock, NativeBridgeInitialized()).Times(AnyNumber());

    if (!bridged) {
      EXPECT_CALL(*mock, android_init_anonymous_namespace(StrEq(default_public_libraries()),
                                                          StrEq(library_path)))
          .WillOnce(Return(true));
      EXPECT_CALL(*mock,
                  android_create_namespace(
                      StrEq(expected_namespace_name), nullptr, StrEq(expected_library_path),
                      expected_namespace_flags, StrEq(expected_permitted_path),
                      NsEq(android_get_exported_namespace(expected_parent_namespace.c_str()))))
          .WillOnce(Return(TO_ANDROID_NAMESPACE(dex_path.c_str())));
      if (expected_link_with_platform_ns) {
        EXPECT_CALL(*mock, android_link_namespaces(_, NsEq("platform"),
                                                   StrEq(expected_shared_libs_to_platform_ns)))
            .WillOnce(Return(true));
      }
      if (expected_link_with_runtime_ns) {
        EXPECT_CALL(*mock, android_link_namespaces(_, NsEq("runtime"),
                                                   StrEq(expected_shared_libs_to_runtime_ns)))
            .WillOnce(Return(true));
      }
      if (expected_link_with_sphal_ns) {
        EXPECT_CALL(*mock, android_link_namespaces(_, NsEq("sphal"),
                                                   StrEq(expected_shared_libs_to_sphal_ns)))
            .WillOnce(Return(true));
      }
      if (expected_link_with_vndk_ns) {
        EXPECT_CALL(
            *mock, android_link_namespaces(_, NsEq("vndk"), StrEq(expected_shared_libs_to_vndk_ns)))
            .WillOnce(Return(true));
      }
      if (expected_link_with_default_ns) {
        EXPECT_CALL(*mock, android_link_namespaces(_, NsEq("default"),
  ...
  void RunTest() {
    NativeLoaderTest<bridged>::RunTest();
  ...
  JNIEnv* env() { return NativeLoaderTest<bridged>::env; }

  // test inputs (initialized to the default values)
...
class NativeLoader_Create : public NativeLoaderTest_Create<false> {};
class BridgedLoader_Create : public NativeLoaderTest_Create<true> {};

TEST_F(NativeLoader_Create, DownloadedApp) {
  SetExpectation();
  RunTest();
}

TEST_F(NativeLoader_Create, BundledSystemApp) {
...
TEST_F(NativeLoader_Create, BundledVendorApp) {
...
TEST_F(NativeLoader_Create, UnbundledVendorApp) {
  dex_path = "/vendor/app/foo/foo.apk";
  is_shared = false;
...
TEST_F(NativeLoader_Create, BundledProductApp_pre30) {
...
TEST_F(NativeLoader_Create, BundledProductApp_post30) {
  dex_path = "/product/app/foo/foo.apk";
  is_shared = true;
...
TEST_F(NativeLoader_Create, UnbundledProductApp_pre30) {
  dex_path = "/product/app/foo/foo.apk";
  is_shared = false;
  SetExpectation();
  RunTest();
...
TEST_F(NativeLoader_Create, UnbundledProductApp_post30) {
  dex_path = "/product/app/foo/foo.apk";
  is_shared = false;
  target_sdk_version = 30;
...
TEST_F(NativeLoader_Create, TwoApks) {
  SetExpectation();
  ...
  // app's classloader.
  ON_CALL(*mock, GetParentClassLoader(StrEq(second_app_class_loader)))
      .WillByDefault(Return(class_loader.c_str()));

  // namespace for the second app is created. Its parent is set to the namespace
  // of the first app.
  EXPECT_CALL(*mock, android_create_namespace(
                         StrEq(expected_namespace_name), nullptr, StrEq(second_app_library_path),
                         expected_namespace_flags, StrEq(expected_second_app_permitted_path),
                         NsEq(dex_path.c_str())))
      .WillOnce(Return(TO_ANDROID_NAMESPACE(second_app_dex_path.c_str())));
  EXPECT_CALL(*mock, android_link_namespaces(NsEq(second_app_dex_path.c_str()), _, _))
