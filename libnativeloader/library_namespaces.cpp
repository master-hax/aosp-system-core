...
NativeLoaderNamespace* LibraryNamespaces::Create(JNIEnv* env, uint32_t target_sdk_version,
  ...
  NativeLoaderNamespace app_ns(namespace_name, library_path, permitted_path, parent_ns, is_shared,
                               target_sdk_version < 24 /* is_greylist_enabled */);
  if (app_ns.IsNil()) {
  ...
  bool is_android_ns = app_ns.IsAndroidNamespace();

  if (apk_origin == APK_ORIGIN_VENDOR || apk_origin == APK_ORIGIN_PRODUCT) {
    LOG_FATAL_IF(!is_android_ns,
                 "Unbundled apk in vendor or product must not use translated architecture");
  }

  auto platform_ns = NativeLoaderNamespace::GetPlatformNamespace(is_android_ns);
  ...
  auto runtime_ns =
      NativeLoaderNamespace::GetExportedNamespace(kRuntimeNamespaceName, is_android_ns);
  ...
  auto vndk_ns = NativeLoaderNamespace::GetExportedNamespace(kVndkNamespaceName, is_android_ns);
  if (!vndk_ns.IsNil() && !vndksp_libraries().empty()) {
    if (!app_ns.Link(vndk_ns, vndksp_libraries())) {
      *error_msg = app_ns.GetError();
      return nullptr;
    }
  }
  ...
  auto vendor_ns = NativeLoaderNamespace::GetExportedNamespace(kVendorNamespaceName, is_android_ns);
