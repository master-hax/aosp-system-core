...
bool ShellProtocol::Write(Id id, size_t length_or_exit_code) {
    buffer_[0] = id;
    length_t length = length_or_exit_code;
    if (id == kIdExit) {
        data()[0] = length_or_exit_code;
        length = 1;
    }

    memcpy(&buffer_[1], &length, sizeof(length));
