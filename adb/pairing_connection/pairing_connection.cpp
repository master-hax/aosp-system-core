...
using TlsError = ssl::TlsConnection::TlsError;
...
bool PairingConnectionCtx::SetupTlsConnection(int fd) {
    switch (role_) {
        case Role::Server:
            tls_ = ssl::TlsConnection::Create(
                    ssl::TlsConnection::Role::Server,
                    std::string_view(reinterpret_cast<const char*>(cert_.data()), cert_.size()),
                    std::string_view(reinterpret_cast<const char*>(priv_key_.data()),
                                     priv_key_.size()));
            break;
        case Role::Client:
            tls_ = ssl::TlsConnection::Create(
                    ssl::TlsConnection::Role::Client,
                    std::string_view(reinterpret_cast<const char*>(cert_.data()), cert_.size()),
                    std::string_view(reinterpret_cast<const char*>(priv_key_.data()),
                                     priv_key_.size()));
            break;
    ...
    if (tls_->DoHandshake(fd) != TlsError::Success) {
...
bool PairingConnectionCtx::DoExchangeMsgs() {
    ...
    pairing_auth_get_msg(auth_.get(), msg.data());
...
bool PairingConnectionCtx::DoExchangePeerInfo() {
    ...
    their_info_.info[kMaxPeerInfoSize - 1] = '\0';
