...
class TlsConnectionImpl : public TlsConnection {
  public:
    ...
    TlsError DoHandshake(borrowed_fd fd) override;
...
TlsConnection::TlsError TlsConnectionImpl::DoHandshake(borrowed_fd fd) {
    int err = -1;
...
std::unique_ptr<TlsConnection> TlsConnection::Create(TlsConnection::Role role,
    ...
    return std::make_unique<TlsConnectionImpl>(role, cert, priv_key);
}
