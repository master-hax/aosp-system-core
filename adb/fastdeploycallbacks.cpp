...
static void appendBuffer(std::vector<unsigned char>* buffer, const char* input, int length) {
    if (buffer != NULL) {
        // buffer->insert(buffer->end(), length, *(const unsigned char*)input);
        buffer->insert(buffer->end(), input, input + length);
    }
}

DeployAgentFileCallback::DeployAgentFileCallback(FILE* outputFile,
                                                 std::vector<unsigned char>* errBuffer,
                                                 int* statusCode) {
...
DeployAgentBufferCallback::DeployAgentBufferCallback(std::vector<unsigned char>* outBuffer,
                                                     std::vector<unsigned char>* errBuffer,
                                                     int* statusCode) {
