...
bool set_tcp_keepalive(int fd, int interval_sec) {
    ...
    // Idle time before sending the first keepalive: TCP_KEEPIDLE on Linux, TCP_KEEPALIVE on Mac.
    ...
    // Time between keepalives: TCP_KEEPINTVL. Mac supports this since Mountain Lion (10.8),
    // anything before that will use the default system value which is usually 75 seconds.
    ...
    // Number of keepalive failures to allow before dropping the connection: TCP_KEEPCNT. Also
    // only available on Mac since 10.8.
