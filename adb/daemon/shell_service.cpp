...
unique_fd StartSubprocess(const char* name, const char* terminal_type, SubprocessType type,
    ...
    unique_fd error_fd;
    unique_fd fd =
            StartSubprocess(name, terminal_type, type, protocol, make_pty_raw, protocol, &error_fd);
    if (fd == -1) {
        return error_fd;
    }
    return fd;
...
unique_fd StartSubprocess(const char* name, const char* terminal_type, SubprocessType type,
    ...
    auto subprocess =
            std::make_unique<Subprocess>(name, terminal_type, type, protocol, make_pty_raw);
    if (!subprocess) {
