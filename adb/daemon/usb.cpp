...
struct UsbFfsConnection : public Connection {
  ...
  private:
    void StartMonitor() {
        ...
        // becomes disabled. Work around this by listening for functionfs lifecycle events
        // on a different thread, and then sending interruption signals to the worker thread
        // until it dies.
                ...
                switch (event.type) {
                    case FUNCTIONFS_ENABLE:
                        if (!started) {
                            ...
                            LOG(FATAL) << "received FUNCTIONFS_ENABLE while already running?";
                        }
                        break;
    ...
    void WaitForEvents() {
        ...
        int rc = io_getevents(aio_context_, 0, kMaxEvents, events, &timeout);
        if (rc == -1) {
