...
struct TransferId {
    static constexpr uint64_t DIRECTION_READ = 0;
    static constexpr uint64_t DIRECTION_WRITE = 1;

    uint64_t direction : 1;
    uint64_t id : 63;

    TransferId() : TransferId(0, 0) {}
    TransferId(uint64_t direction, uint64_t id) : direction(direction), id(id) {}
    ...
    static TransferId read(uint64_t id) { return TransferId(DIRECTION_READ, id); }

    static TransferId write(uint64_t id) { return TransferId(DIRECTION_WRITE, id); }
...
struct UsbFfsConnection : public Connection {
    UsbFfsConnection(unique_fd read, unique_fd write, std::promise<void> destruction_notifier)
        : stopped_(false),
        ...
        if (io_setup(kUsbReadQueueDepth + kUsbWriteQueueDepth, &aio_context_) != 0) {
            PLOG(FATAL) << "failed to create AIO context";
        }
    }
  ...
  private:
    void Run() {
        adb_thread_setname("UsbFfsConnection");
        ...
        if (io_destroy(aio_context_) != 0) {
            PLOG(FATAL) << "failed to destroy AIO context";
        }
        aio_context_ = 0;
    ...
    void WaitForEvents() {
        ...
        int rc = TEMP_FAILURE_RETRY(io_getevents(aio_context_, 0, kMaxEvents, events, &timeout));
        if (rc == -1) {
            PLOG(FATAL) << "io_getevents failed while reading";
        }
            ...
            if (event.res < 0) {
                std::string error =
                        StringPrintf("%s %" PRIu64 " failed with error %s",
                                     id.direction == TransferId::DIRECTION_READ ? "read" : "write",
                                     id.id, strerror(-event.res));
                HandleError(error);
                return;
            }

            if (id.direction == TransferId::DIRECTION_READ) {
    ...
    void SubmitRead(IoBlock* block) {
        ...
        if (io_submit(aio_context_, 1, &iocb) != 1) {
    ...
    void SubmitWrites() REQUIRES(write_mutex_) {
        ...
        int rc = io_submit(aio_context_, writes_to_submit, iocbs);
        if (rc == -1) {
...
static void usb_ffs_open_thread() {
    ...
    while (true) {
        ...
        transport->SetConnection(std::make_unique<UsbFfsConnection>(
                std::move(bulk_out), std::move(bulk_in), std::move(destruction_notifier)));
