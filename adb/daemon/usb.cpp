...
bool init_functionfs(struct usb_handle* h) {
    ...
    // File transfer packets don't get larger than SYNC_DATA_MAX, double to allow for overhead
    h->max_rw = SYNC_DATA_MAX * 2;
    while (h->max_rw >= USB_FFS_BULK_SIZE) {
        if (ioctl(h->bulk_in, FUNCTIONFS_ENDPOINT_ALLOC, static_cast<__u32>(h->max_rw)) ||
            ioctl(h->bulk_out, FUNCTIONFS_ENDPOINT_ALLOC, static_cast<__u32>(h->max_rw))) {
            h->max_rw /= 2;
        } else {
            return true;
        }
