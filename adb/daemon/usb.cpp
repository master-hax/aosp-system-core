...
static constexpr size_t kUsbWriteSize = 16 * 4096;
...
struct UsbFfsConnection : public Connection {
    ...
    virtual bool Write(std::unique_ptr<apacket> packet) override final {
        ...
        if (!packet->payload.empty()) {
            ...
            while (len > kUsbWriteSize) {
                write_requests_.push_back(
                        CreateWriteBlock(payload, offset, kUsbWriteSize, next_write_id_++));
                len -= kUsbWriteSize;
                offset += kUsbWriteSize;
            }
            write_requests_.push_back(
                    CreateWriteBlock(std::move(payload), offset, len, next_write_id_++));
        }
        SubmitWrites();
        return true;
    }
