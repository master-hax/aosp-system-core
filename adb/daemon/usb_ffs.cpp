...
bool open_functionfs(android::base::unique_fd* control, android::base::unique_fd* bulk_out,
    ...
    if (control->get() < 0) {  // might have already done this before
        ...
        ret = adb_write(control->get(), &v2_descriptor, sizeof(v2_descriptor));
        if (ret < 0) {
            ...
            ret = adb_write(control->get(), &v1_descriptor, sizeof(v1_descriptor));
            if (ret < 0) {
        ...
        ret = adb_write(control->get(), &strings, sizeof(strings));
        if (ret < 0) {
