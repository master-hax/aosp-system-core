...
const char kWifiEnabledProp[] = "persist.sys.adb.wifi";
...
static void start_wifi_enabled_observer() {
    std::thread([]() {
        ...
        while (true) {
            ...
            LOG(INFO) << "Waiting for persist.sys.adb.wifi=" << toggled_val;
            if (WaitForProperty(kWifiEnabledProp, toggled_val)) {
                wifi_enabled = !wifi_enabled;
                LOG(INFO) << "persist.sys.adb.wifi changed to " << toggled_val;
                if (wifi_enabled) {
