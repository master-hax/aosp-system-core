...
const char kWifiEnabledProp[] = "sys.adb.wifi";
...
// Watches for the sys.adb.wifi property to toggle the TlsServer
static void start_wifi_enabled_observer() {
    std::thread([]() {
        bool wifi_enabled = false;
        while (true) {
            std::string toggled_val = wifi_enabled ? "0" : "1";
            LOG(INFO) << "Waiting for sys.adb.wifi=" << toggled_val;
            if (WaitForProperty(kWifiEnabledProp, toggled_val)) {
                wifi_enabled = !wifi_enabled;
                LOG(INFO) << "sys.adb.wifi changed to " << toggled_val;
                if (wifi_enabled) {
