...
TlsServer::~TlsServer() {
    ...
    // TODO: set a system property to hide the port
    // (persist.adb.wifi.port="")
...
bool TlsServer::Start() {
    ...
    // TODO: set a system property to show the port
    // (persist.adb.wifi.port=port_)
...
static void adb_disconnected(void* unused, atransport* t) {
    ...
    adbd_tls_device_disconnected(auth_ctx, kAdbTransportTypeWifi, t->auth_id);
}
...
// TODO: toggle the tls server via system property instead, since the framework
// is not communicating with us when ro.adb.secure=0.
//
// Furthermore, also need to only ctl.stop adbd when sys.usb.config does not
// contain adb and a new prop (sys.adb.wifi?) is 0.
...
void adbd_wifi_secure_connect(atransport* t) {
    ...
    t->auth_id = adbd_tls_device_connected(auth_ctx, kAdbTransportTypeWifi, t->auth_key.data(),
                                           t->auth_key.size());
}
#endif /* !HOST */
