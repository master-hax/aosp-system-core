...
static void setup_mdns_thread() {
    ...
    for (int i = 0; i < kNumADBDNSServices; i++) {
        auto error = DNSServiceRegister(
                &mdns_refs[i], 0, 0, hostname.c_str(),
                kADBDNSServices[i], nullptr, nullptr,
                htobe16((uint16_t)port), 0, nullptr,
                mdns_callback, nullptr);

        if (error != kDNSServiceErr_NoError) {
            LOG(ERROR) << "Could not register mDNS service " << kADBDNSServices[i] << ", error (" << error << ").";
