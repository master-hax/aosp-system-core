...
static void register_mdns_service(int index, int port, const std::string service_name) {
    ...
    if (kADBDNSServiceTxtRecords[index]) {
        size_t txtRecordStringLength =
            strlen(kADBDNSServiceTxtRecords[index]);
        ...
        memcpy(txtRecord.data() + 1,
               kADBDNSServiceTxtRecords[index],
               txtRecordStringLength);
    }

    auto error = DNSServiceRegister(
            &mdns_refs[index], 0, 0, service_name.c_str(),
            kADBDNSServices[index], nullptr, nullptr,
            htobe16((uint16_t)port),
            (uint16_t)txtRecord.size(),
            txtRecord.empty() ? nullptr : txtRecord.data(),
            mdns_callback, nullptr);

    if (error != kDNSServiceErr_NoError) {
        LOG(ERROR) << "Could not register mDNS service " << kADBDNSServices[index] << ", error (" << error << ").";
        mdns_registered[index] = false;
    }

    mdns_registered[index] = true;

    LOG(INFO) << "adbd mDNS service " <<
        kADBDNSServices[index] << " registered: " <<
        mdns_registered[index];
}
...
void register_adb_secure_pairing_service(int port) {
    std::thread([port]() {
        ...
        register_mdns_service(
            kADBSecurePairingServiceRefIndex, port, service_name);
    }).detach();
}
...
void register_adb_secure_connect_service(int port) {
    std::thread([port]() {
        ...
        register_mdns_service(
            kADBSecureConnectServiceRefIndex, port, service_name);
    }).detach();
}
