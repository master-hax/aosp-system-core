...
int init_jdwp(void) {
    std::thread([]() {
        ...
        adbconnection::Listen([](unique_fd fd, pid_t pid) {
            LOG(INFO) << "jdwp conenction from " << pid;
            int released_fd = fd.release();
            fdevent_run_on_main_thread([released_fd, pid] {
                unique_fd fd(released_fd);
                auto proc = std::make_unique<JdwpProcess>(std::move(fd), pid);
                if (!proc) {
