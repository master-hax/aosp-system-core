...
void remount_service(int fd, void* cookie) {
    ...
    // Reboot now if we need an fsck and the user requested it.
    if (user_requested_reboot) {
        ...
        WriteFdExactly(fd, "Skipping reboot (-R), no fsck needed.\n");
    }
    ...
    if ((system_verified || vendor_verified) && dedup.empty()) {
