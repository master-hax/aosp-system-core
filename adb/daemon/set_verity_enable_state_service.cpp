...
void run_adb_root(int fd) {
    if (getuid() != 0) WriteFdFmt(fd, "Maybe run adb root?\n");
}
...
static bool set_verity_enabled_state(int fd, const char* block_device, const char* mount_point,
    ...
    if (!(enable ? fs_mgr_overlayfs_teardown(fstab, mount_point)
                 : fs_mgr_overlayfs_setup(fstab, mount_point))) {
...
static bool overlayfs_setup(int fd, bool enable) {
    ...
    auto change = false;
    if (!(enable ? fs_mgr_overlayfs_teardown(fstab, nullptr, &change)
                 : fs_mgr_overlayfs_setup(fstab, nullptr, &change))) {
        WriteFdFmt(fd, "Overlayfs %s failed.\n", enable ? "teardown" : "setup");
        run_adb_root(fd);
    }
    return change;
}
