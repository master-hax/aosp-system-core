...
static bool send_file_compressed(borrowed_fd s, unique_fd fd, uint32_t* timestamp) {
    syncmsg msg;
...
static bool send_file_uncompressed(borrowed_fd s, unique_fd fd, uint32_t* timestamp) {
    syncmsg msg;
...
static bool handle_send_file(borrowed_fd s, const char* path, uint32_t* timestamp, uid_t uid,
...

    {
        ...
        bool result;
        if (compressed) {
            result = send_file_compressed(s, std::move(fd), timestamp);
        } else {
            result = send_file_uncompressed(s, std::move(fd), timestamp);
        }
...
static bool recv_uncompressed(borrowed_fd s, unique_fd file) {
    syncmsg msg;
    msg.data.id = ID_DATA;
...
static bool recv_impl(borrowed_fd s, const char* path, bool compressed, std::vector<char>& buffer) {
    ...
    if (compressed) {
        return recv_compressed(s, std::move(fd));
    } else {
        return recv_uncompressed(s, std::move(fd));
    }
}
