...
class FileSyncPreparer {
  public:
    FileSyncPreparer() : saved_ns_fd_(-1) {
        ...
        if (ns_fd == -1) {
            PLOG(ERROR) << "Failed to save mount namespace";
            return;
        }

        // Note: this is for the current thread only
        if (unshare(CLONE_NEWNS) != 0) {
            PLOG(ERROR) << "Failed to clone mount namespace";
            return;
        }
        ...
        if (mount("/system", "/system", nullptr, MS_BIND, nullptr) == -1) {
            PLOG(ERROR) << "Failed to bind-mount /system to /system";
        }
    }
