...
int local_connect_arbitrary_ports(int console_port, int adb_port) {
    ...
    if (fd >= 0) {
        ...
        std::string serial =
            android::base::StringPrintf("emulator-%d", console_port);
 ...
 * independent from the network, we use here 'adb' QEMUD service to transfer
 * data
 ...
 * - Connect with adb QEMUD service. In this phase a handle to 'adb' QEMUD
 * service
 ...
 *   service is registered as the transport, and a 'start' request is sent to
 * the
