...
void connect_device(const std::string& address, std::string* response) {
    ...
    auto reconnect = [address](atransport* t) {
        ...
        return init_socket_transport(t, std::move(fd), port, 0, true) >= 0
...
int init_socket_transport(atransport* t, unique_fd fd, int adb_port, int local, bool is_reconnect) {
    int fail = 0;
...
#if ADB_HOST
        {
            ...
            if (is_reconnect) {
                if (t->id != id) {
                    ...
                    // connection.
                    D("New secure wifi connection detected (id=%lu). Not connecting to old "
                      "connection (id=%lu).",
                      id, t->id);
                    return -1;
                }
                ...
                if (id != 0) {
                    std::string err;
                    auto* existing =
                            acquire_one_transport(kTransportAny, nullptr, id, nullptr, &err, true);
                    if (existing != nullptr) {
                        // Force close the connection, because it may be in a
                        // state of successful reconnection, but unstarted
                        // connection.
                        existing->Kick();
                        auto* conn = reinterpret_cast<BlockingConnectionAdapter*>(
                                existing->connection().get());
                        conn->underlying_->Close();
                    }
                }
                // Save the new connection
                D("Adding new secure wifi connection (serial=%s, id=%lu)", t->serial.c_str(),
                  t->id);
