...
int init_socket_transport(atransport* t, unique_fd fd, int adb_port, int local) {
    ...
    if (local) {
        ...
        t->SetConnection(
                std::make_unique<BlockingConnectionAdapter>(std::move(emulator_connection), false));
        std::lock_guard<std::mutex> lock(local_transports_lock);
    ...
    t->SetConnection(
            std::make_unique<BlockingConnectionAdapter>(std::move(fd_connection), t->use_tls));
    return fail;
}
