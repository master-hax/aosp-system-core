...
std::tuple<unique_fd, int, std::string> tcp_connect(const std::string& address,
    ...
    if (android::base::StartsWith(address, "vsock:")) {
        ...
        if (!android::base::ParseUint(fragments[1], &cid)) {
            *response = android::base::StringPrintf("could not parse vsock cid in '%s'",
                                                    address.c_str());
            return std::make_tuple(unique_fd(), port, serial);
        }
        if (fragments.size() == 3 && !android::base::ParseInt(fragments[2], &port)) {
            *response = android::base::StringPrintf("could not parse vsock port in '%s'",
                                                    address.c_str());
            return std::make_tuple(unique_fd(), port, serial);
        }
        fd = unique_fd(socket(AF_VSOCK, SOCK_STREAM, 0));
        if (fd == -1) {
    ...
    std::string error;
    if (fd == -1) {
        fd = unique_fd(network_connect(host.c_str(), port, SOCK_STREAM, 10, &error));
    }
    if (fd == -1) {
