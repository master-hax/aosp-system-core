...
static void client_socket_thread(std::string) {
...
void local_init(std::string addr) {
#if ADB_HOST
    D("transport: local client init");
    std::thread(client_socket_thread, std::move(addr)).detach();
    adb_local_transport_max_port_env_override();
#elif !defined(__ANDROID__)
    // Host adbd.
    D("transport: local server init");
    std::thread(server_socket_thread, adb_listen, std::move(addr)).detach();
#else
    D("transport: local server init");
    // For the adbd daemon in the system image we need to distinguish
    // between the device, and the emulator.
    if (addr.starts_with("tcp:") && use_qemu_goldfish()) {
        std::thread(qemu_socket_thread, std::move(addr)).detach();
    } else {
        std::thread(server_socket_thread, adb_listen, std::move(addr)).detach();
    }
#endif // !ADB_HOST
