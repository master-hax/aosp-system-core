...
void connect_device(const std::string& address, std::string* response) {
    ...
    int port;
    std::string serial;
    socket_spec_connect(&fd, address, &port, &serial, response);
    if (fd.get() == -1) {
        return;
    }
...
void local_init(std::string addr, int* port) {
    std::string error;
    if (!get_host_socket_spec_port(addr, port, &error)) {
        return;
    }
#if ADB_HOST
    ...
    if (use_qemu_goldfish()) {
        std::thread(qemu_socket_thread, addr, *port).detach();
    } else {
        std::thread(server_socket_thread, adb_listen, addr).detach();
    }
#endif // !ADB_HOST
