...
// set to "1" for ranchu and goldfish images.
static bool use_qemu_goldfish(void) {
    static constexpr char kQemuKernelProperty[] = "ro.kernel.qemu";
    static constexpr char kAdbTransportProperty[] = "service.adb.transport";
    static constexpr char kAdbTransportGoldfish[] = "goldfish";
    ...
    if (android::base::GetBoolProperty(kQemuKernelProperty, false)) {
        return true;
    }
    // If service.adb.transport is present and is set to "goldfish", use the
    // QEMUD pipe.
    if (android::base::GetProperty(kAdbTransportProperty, "") == kAdbTransportGoldfish) {
        return true;
    }
    return false;
}
