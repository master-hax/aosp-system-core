...
static void AddDevice(usb_handle* handle) {
    handle->mark = true;
    std::lock_guard<std::mutex> lock(g_usb_handles_mutex);
    g_usb_handles.push_back(handle);
}
...
AndroidInterfaceAdded(io_iterator_t iterator)
{
    ...
    while ((usbInterface = IOIteratorNext(iterator))) {
        ...
        AddDevice(handle);
        register_usb_transport(handle, serial, devpath.c_str(), 1);
    }
}
