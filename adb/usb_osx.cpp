...
static IOReturn
ClearPipeStallBothEnds(IOUSBInterfaceInterface190 **interface, UInt8 bulkEp) {
    ...
    return (*interface)->ClearPipeStallBothEnds(interface, bulkEp);
}
    ...
    for (endpoint = 0; endpoint <= interfaceNumEndpoints; endpoint++) {
        ...
        if (kIOReturnSuccess == kr) {
            ...
            if (kUSBIn == direction) {
                ...
                if (ClearPipeStallBothEnds(interface, handle->bulkIn) != kIOReturnSuccess) {
                    DBG("ERR: Could not clear input pipe; result %x, skipping interface...\n", kr);
            ...
            if (kUSBOut == direction) {
                ...
                if (ClearPipeStallBothEnds(interface, handle->bulkOut) != kIOReturnSuccess) {
                    DBG("ERR: Could not clear output pipe; result %x, skipping interface...\n", kr);
                    goto err_get_pipe_props;
                }
            }
