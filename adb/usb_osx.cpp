...
AndroidInterfaceAdded(void *refCon, io_iterator_t iterator)
{
    ...
    while ((usbInterface = IOIteratorNext(iterator))) {
        ...
        if (result || !iface) {
            LOG(ERROR) << "Couldn't query the interface (" << std::hex << (int) result << ")";
            continue;
        }
        ...
        (*plugInInterface)->Release(plugInInterface);
        if (result || !dev) {
            LOG(ERROR) << "Couldn't create a device interface (" << std::hex << (int) result << ")";
            continue;
        }
	...
	if (serialIndex > 0) {
        ...
        LOG(INFO) << "Found vid=" << std::hex << vendor
                  << " pid=" << std::hex << product
        ...
        usb_handle* handle = CheckInterface((IOUSBInterfaceInterface190**)iface,
                                            vendor, product);
        if (handle == NULL) {
            LOG(ERROR) << "Could not find device interface: " << std::hex << kr;
            (*iface)->Release(iface);
            continue;
        }
...
static bool ClearPipeStallBothEnds(IOUSBInterfaceInterface190** interface, UInt8 bulkEp) {
    ...
    if (rc != kIOReturnSuccess) {
        LOG(ERROR) << "Could not clear pipe stall both ends: " << std::hex << (int) rc;
        return false;
    }
    return true;
}
...
CheckInterface(IOUSBInterfaceInterface190 **interface, UInt16 vendor, UInt16 product)
{
    ...
    if (kr != kIOReturnSuccess) {
        LOG(ERROR) << "Could not open interface: " << std::hex << (int) kr;
        return NULL;
    }
    ...
    if (kr != kIOReturnSuccess) {
        LOG(ERROR) << "Unable to get number of endpoints: " << std::hex << (int) kr;
        goto err_get_num_ep;
    }
