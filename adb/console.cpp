...
int adb_send_emulator_command(int argc, const char** argv, const char* serial) {
    ...
    if (adb_write(fd, disconnect_command, sizeof(disconnect_command)) == -1) {
        LOG(ERROR) << "Could not finalize emulator command";
        // We're in a pretty weird state at this point. Continue trying to flush
        // the socket anyway.
    }
