...
def make_random_device_files(adb, in_dir, num_files, rand_size=True):
    ...
    for i in range(num_files):
        ...
        dev_md5, _ = adb.shell("md5 {}".format(full_path)).split()
...
class AdbFile(unittest.TestCase):
    ...
    def pull_dir_with_device(self, device):
        ...
        try:
            ...
            for device_full_path in temp_files.keys():
                host_path = host_dir + "/" + temp_files[device_full_path].base_name
            ...
            for dev_file in temp_files.values():
                host_path = host_dir + "/" + dev_file.base_name
                os.remove(host_path)
            adb.shell_nocheck("rm -r {}".format(AdbFile.DEVICE_TEMP_DIR))
            if host_dir:
