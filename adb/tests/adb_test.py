...
def call_checked(cmd_str):
    ...
    trace(cmd_str)
    return subprocess.check_output(shlex.split(cmd_str), stderr=subprocess.STDOUT)
...
def make_random_device_files(adb, in_dir, num_files, rand_size=True):
    ...
    for i in range(num_files):
        ...
        adb.shell("dd if=/dev/urandom of={} bs={} count=1".format(full_path, size))
...
class AdbWrapper(object):
    ...
    def forward(self, local, remote):
        return call_checked(self.adb_cmd + "forward {} {}".format(local, remote))
    ...
    def reverse(self, remote, local):
        return call_checked(self.adb_cmd + "reverse {} {}".format(remote, local))

    def reverse_remove_all(self):
        return call_checked(self.adb_cmd + "reverse --remove-all")

    def reverse_remove(self, remote):
        return call_checked(self.adb_cmd + "reverse --remove {}".format(remote))
...
class AdbFile(unittest.TestCase):
    ...
    def push_with_device(self, device):
        ...
        with tempfile.NamedTemporaryFile(mode="w") as tmp:
            ...
            try:
                ...
                dev_md5, _ = adb.shell("md5sum {}".format(AdbFile.DEVICE_TEMP_FILE)).split()
    ...
    def pull_dir_with_device(self, device):
        ...
        temp_files = {}
        host_dir = None
        """Pull a randomly generated directory of files from specified device"""
        try:
            ...
            temp_files = make_random_device_files(adb, in_dir=AdbFile.DEVICE_TEMP_DIR, num_files=32)

            adb.pull(remote=AdbFile.DEVICE_TEMP_DIR, local=host_dir)

            for device_full_path in temp_files:
                host_path = os.path.join(host_dir, temp_files[device_full_path].base_name)
                with open(host_path) as host_file:
                    host_md5 = compute_md5(host_file.read())
                    self.assertEqual(host_md5, temp_files[device_full_path].md5)
        finally:
    ...
    def sync_dir_with_device(self, device):
        ...
        try:
            ...
            for host_full_path in temp_files.keys():
                ...
                dev_md5, _ = adb.shell("md5sum {}".format(device_full_path)).split()
