...
class Subprocess {
  ...
  private:
    ...
    int OpenPtyChildFd(const char* pts_name);
    ...
    ShellProtocol *input_= nullptr, *output_ = nullptr;
...
bool Subprocess::ForkAndExec() {
    ...
    if (pid_ == 0) {
        ...
        if (type_ == SubprocessType::kPty) {
            child_stdinout_sfd.Reset(OpenPtyChildFd(pts_name));
        }
        ...
        if (is_interactive()) {
            execl(SHELL_COMMAND, SHELL_COMMAND, "-", nullptr);
        } else {
            execl(SHELL_COMMAND, SHELL_COMMAND, "-c", command_.c_str(), nullptr);
        }
        PLOG(FATAL) << "exec '" << SHELL_COMMAND << "' failed";
    }
    ...
    if (protocol_ == SubprocessProtocol::kNone) {
        ...
        input_ = new ShellProtocol(protocol_sfd_.fd());
        output_ = new ShellProtocol(protocol_sfd_.fd());
        if (!input_ || !output_) {
...
int Subprocess::OpenPtyChildFd(const char* pts_name) {
    int child_fd = adb_open(pts_name, O_RDWR | O_CLOEXEC);
    if (child_fd == -1) {
        PLOG(ERROR) << "child failed to open pseudo-term slave " << pts_name;
        exit(-1);
    }
...
void Subprocess::WaitForExit() {
    ShellProtocol::exit_code_t exit_code = 1;
    ...
    if (protocol_sfd_.valid()) {
        if (output_->Write(ShellProtocol::kIdExit, exit_code)) {
