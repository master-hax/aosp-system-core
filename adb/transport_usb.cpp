...
constexpr size_t MAX_USB_BULK_PACKET_SIZE = 1024u;
...
static int UsbReadPayload(usb_handle* h, apacket* p) {
    ...
    while (need_size > 0u) {
        int n = 0;
        if (need_size >= MAX_USB_BULK_PACKET_SIZE ||
            data_pos + MAX_USB_BULK_PACKET_SIZE <= sizeof(p->data)) {
            // Read directly to p->data.
            size_t direct_read_size = need_size;
            if (need_size % MAX_USB_BULK_PACKET_SIZE) {
                direct_read_size += MAX_USB_BULK_PACKET_SIZE - need_size % MAX_USB_BULK_PACKET_SIZE;
            }
