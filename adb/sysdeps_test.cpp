...
TEST_F(sysdeps_poll, duplicate_fd) {
    ...
    // Windows's WSAPoll seems to return the number of unique file descriptors that have events, not
    // the number of pollfd structs with nonzero events. This probably isn't a problem for adb,
    // since we don't allow the same FD to be registered more than once in fdevent.
    EXPECT_EQ(2, adb_poll(pfd, 2, 0));
    EXPECT_EQ(POLLRDNORM, pfd[0].revents);
