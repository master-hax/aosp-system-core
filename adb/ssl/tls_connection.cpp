...
class TlsConnectionImpl : public TlsConnection {
  public:
    ...
    virtual void SetCertVerifyCallback(CertVerifyCb cb, void* opaque) override;
    ...
    virtual bool ReadFully(void* buf, int size) override;
...
bool TlsConnectionImpl::ReadFully(void* buf, int size) {
    CHECK_GT(size, 0);
    if (!ssl_) {
        LOG(ERROR) << "Tried to read on a null SSL connection";
        return false;
