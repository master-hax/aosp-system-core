...
TEST_F(AdbCAListTest, CreateCAIssuerFromEncodedKey_KeySize) {
    // Ensure a crash if the the key is beyond the max size.
    ASSERT_DEATH(
            {
                std::string too_big(GetAdbKeyMaxSize() + 1, 'a');
                auto res = CreateCAIssuerFromEncodedKey(too_big);
            },
            "");
    // Ensure a crash if the key is empty
    ASSERT_DEATH({ auto res = CreateCAIssuerFromEncodedKey(""); }, "");
    // Valid key sizes
    {
        std::string key(GetAdbKeyMaxSize(), 'a');
        auto res = CreateCAIssuerFromEncodedKey(key);
...
TEST_F(AdbCAListTest, Smoke) {
    {
        std::string key = "A45BC1FF6C89B";
        ASSERT_LE(key.size(), GetAdbKeyMaxSize());
