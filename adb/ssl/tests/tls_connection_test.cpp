    ...
    void StartClientHandshakeAsync(bool expect_success) {
        client_thread_ = std::thread([=]() {
            if (expect_success) {
                ...
                EXPECT_NE(client_->DoHandshake(client_fd_.get()), TlsError::Success);
            }
        });
    }
...
TEST_F(AdbWifiTlsConnectionTest, NoTrustedCertificates) {
    ...
    EXPECT_NE(server_->DoHandshake(server_fd_.get()), TlsError::Success);
    WaitForClientConnection();
...
TEST_F(AdbWifiTlsConnectionTest, SetCertVerifyCallback_ClientAcceptsServerRejects_PostHSCheck) {
    ...
    client_thread_ = std::thread([&]() {
        EXPECT_EQ(client_->DoHandshake(client_fd_.get()), TlsError::PeerRejectedCertificate);
    });
...
TEST_F(AdbWifiTlsConnectionTest, SetCertVerifyCallback_ClientRejectsServerAccepts) {
    ...
    client_thread_ = std::thread([&]() {
        EXPECT_EQ(client_->DoHandshake(client_fd_.get()), TlsError::CertificateRejected);
    });

    // Server handshake should fail
    EXPECT_EQ(server_->DoHandshake(server_fd_.get()), TlsError::PeerRejectedCertificate);
    WaitForClientConnection();
}
