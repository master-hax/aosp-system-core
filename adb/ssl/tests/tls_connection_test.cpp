...
class AdbWifiTlsConnectionTest : public testing::Test {
  protected:
    ...
    void SetupClientConnectionAsync(bool use_cert_verify, TlsError expected) {
        client_thread_ = std::thread([=]() {
            client_->EnableCertificateVerification(use_cert_verify);
            EXPECT_EQ(client_->DoHandshake(client_fd_.get()), expected);
        });
    }
...
TEST_F(AdbWifiTlsConnectionTest, NoCertificateVerification) {
    ...
    SetupClientConnectionAsync(false, TlsError::Success);

    // Handshake should succeed
    EXPECT_EQ(server_->DoHandshake(server_fd_.get()), TlsError::Success);
    WaitForClientConnection();
...
TEST_F(AdbWifiTlsConnectionTest, NoTrustedCertificates) {
    ...
    SetupClientConnectionAsync(true, TlsError::CertificateRejected);

    // Handshake should not succeed
    ASSERT_EQ(server_->DoHandshake(server_fd_.get()), TlsError::CertificateRejected);
    WaitForClientConnection();
...
TEST_F(AdbWifiTlsConnectionTest, CertificateVerifyIsDefault) {
    // Test whether certificate verification is the default
    SetupClientConnectionAsync(true, TlsError::CertificateRejected);
...
TEST_F(AdbWifiTlsConnectionTest, AddTrustedCertificates) {
    ...
    SetupClientConnectionAsync(true, TlsError::Success);
...
TEST_F(AdbWifiTlsConnectionTest, AddTrustedCertificates_ClientWrongCert) {
    ...
    EXPECT_EQ(server_->DoHandshake(server_fd_.get()), TlsError::CertificateRejected);
    WaitForClientConnection();
...
TEST_F(AdbWifiTlsConnectionTest, SetCertVerifyCallback_EnableCertificateVerificationOn) {
    // Certificate verification is on by default. Setting a custom certificate
    // verification function will not take effect unless it is turned off.
    client_->SetCertVerifyCallback([](X509_STORE_CTX*, void*) { return 1; }, nullptr);
    server_->SetCertVerifyCallback([](X509_STORE_CTX*, void*) { return 1; }, nullptr);
    SetupClientConnectionAsync(true, TlsError::CertificateRejected);
...
TEST_F(AdbWifiTlsConnectionTest, SetCertVerifyCallback_EnableCertificateVerificationOff) {
    // Certificate verification is on by default. Setting a custom certificate
    // verification function will not take effect unless it is turned off.
    client_->SetCertVerifyCallback([](X509_STORE_CTX*, void*) { return 1; }, nullptr);
    server_->SetCertVerifyCallback([](X509_STORE_CTX*, void*) { return 1; }, nullptr);
    ...
    // Handshake should succeed
    ASSERT_EQ(server_->DoHandshake(server_fd_.get()), TlsError::Success);
    WaitForClientConnection();

    // Client write, server read should succeed
    EXPECT_TRUE(client_->WriteFully(
            std::string_view(reinterpret_cast<const char*>(msg_.data()), msg_.size())));
    auto data = server_->ReadFully(msg_.size());
    EXPECT_EQ(data, msg_);

    // Client read, server write should succeed
    EXPECT_TRUE(server_->WriteFully(
            std::string_view(reinterpret_cast<const char*>(msg_.data()), msg_.size())));
    data = client_->ReadFully(msg_.size());
    EXPECT_EQ(data, msg_);
...
TEST_F(AdbWifiTlsConnectionTest, SetCertVerifyCallback_ClientAcceptsServerRejects) {
    // Client sets the custom callback to accept any certificate,
    // Server rejects all certificates.
    client_->SetCertVerifyCallback([](X509_STORE_CTX*, void*) { return 1; }, nullptr);
    server_->SetCertVerifyCallback([](X509_STORE_CTX*, void*) { return 0; }, nullptr);
    server_->EnableCertificateVerification(false);
...
TEST_F(AdbWifiTlsConnectionTest, EnableClientPostHandshakeCheck_ClientWrongCert) {
    ...
    SetupClientConnectionAsync(false, TlsError::CertificateRejected);
...
TEST_F(AdbWifiTlsConnectionTest, SetClientCAList_Smoke) {
    ...
    client_thread_ = std::thread([&]() {
        client_->EnableCertificateVerification(false);
        client_->SetCertificateCallback(
                [](SSL* ssl, void* opaque) -> int {
                    auto caIssuers = *reinterpret_cast<std::vector<CAIssuer>*>(opaque);
                    ...
                    EXPECT_EQ(caIssuers.size(), num_names);
                    ...
                    size_t idx = 0;
                    for (auto& issuer : caIssuers) {
                        auto* name = sk_X509_NAME_value(received, idx++);
                        for (auto& attr : issuer) {
                            EXPECT_EQ(
                                    X509_NAME_get_text_by_NID(
                                            name, attr.nid, reinterpret_cast<char*>(buf), buf_size),
                                    attr.val.size());
                ...
                &kCAIssuers);
        // Client handshake should succeed
        EXPECT_EQ(client_->DoHandshake(client_fd_.get()), TlsError::Success);
    });
    ...
    client_thread_ = std::thread([&]() {
        ...
        client_->SetCertificateCallback(
                [](SSL* ssl, void* opaque) -> int {
                    ...
                    // trusted one here.
                    auto* keyhash = reinterpret_cast<std::string*>(opaque);
                    const STACK_OF(X509_NAME)* received = SSL_get_client_CA_list(ssl);
                    EXPECT_NE(received, nullptr);
                    const size_t num_names = sk_X509_NAME_num(received);
