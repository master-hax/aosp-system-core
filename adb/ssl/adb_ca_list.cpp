...
std::string GetX509NameTextByNid(X509_NAME* name, int nid) {
...
std::string ParseEncodedKeyFromCAIssuer(X509_NAME* issuer) {
    ...
    std::string buf = GetX509NameTextByNid(issuer, kAdbKeyIdentifierNid);
    if (buf.empty() || !android::base::StartsWith(buf, kAdbKeyIdentifierPrefix)) {
        return "";
    }
