...
class TlsConnection {
  public:
    ...
    // Enable the CertificateRequest query from the server, with the list of
    // certificate authorities, |ca_list| that are accepted. This API is for
    // server only. This CA issuer list will be sent to the client in the
    // CertificateRequest.
    ...
    // for more details.
    virtual void SetCertificateCallback(SetCertCb cb, void* opaque) = 0;
    ...
    virtual TlsError DoHandshake(int fd) = 0;
