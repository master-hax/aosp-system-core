...
void atransport::AddDisconnect(adisconnect* disconnect) {
    if (std::find(disconnects_.begin(), disconnects_.end(), disconnect) == disconnects_.end()) {
...
void atransport::RunDisconnects() {
    // alistener may call RemoveDisconnect in the callback function,
    // so copy to another place before calling callbacks.
    std::list<adisconnect*> v(disconnects_);
    for (auto& disconnect : v) {
