...
static std::string dump_packet(const char* name, const char* func, apacket* p) {
    ...
    std::string result =
        StringPrintf("%s: %s: [%s] arg0=%s arg1=%s (len=%d) ", name, func, cmd, arg0, arg1, len);
...
static void read_transport_thread(void* _t) {
    ...
    adb_thread_setname(StringPrintf("<-%s", (t->serial != nullptr ? t->serial : "transport")));
...
// write_transport thread gets packets sent by the main thread (through
// send_packet()),
...
atransport* acquire_one_transport(TransportType type, const char* serial, bool* is_ambiguous,
    ...
    if (serial) {
        *error_out = StringPrintf("device '%s' not found", serial);
...
std::string FeatureSetToString(const FeatureSet& features) {
    return Join(features, kFeatureStringDelimiter);
}
...
FeatureSet StringToFeatureSet(const std::string& features_string) {
    ...
    auto names = Split(features_string, {kFeatureStringDelimiter});
...
bool atransport::MatchesTarget(const std::string& target) const {
    if (serial) {
        if (target == serial) {
            ...
            if (StartsWith(target, "tcp:") || StartsWith(target, "udp:")) {
                local_target_ptr += 4;
            }
            ...
            int serial_port = -1;
            if (ParseNetAddress(serial, &serial_host, &serial_port, nullptr, &error)) {
                // |target| may omit the port to default to ours.
                std::string target_host;
                int target_port = serial_port;
                if (ParseNetAddress(local_target_ptr, &target_host, &target_port, nullptr, &error) &&
...
static void append_transport(const atransport* t, std::string* result, bool long_listing) {
    ...
    if (!long_listing) {
        ...
        *result += t->connection_state_name();
        ...
        StringAppendF(result, "%-22s %s", serial, t->connection_state_name().c_str());
