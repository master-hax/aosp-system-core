...
class ReconnectHandler {
  ...
  private:
    ...
    struct ReconnectAttempt {
        ...
        std::chrono::steady_clock::time_point reconnect_timestamp;
        ...
        bool operator<(const ReconnectAttempt& rhs) const {
            ...
            return reconnect_timestamp > rhs.reconnect_timestamp;
        }
    };
...
void ReconnectHandler::TrackTransport(atransport* transport) {
    check_main_thread();
    {
        ...
        auto reconnect_timestamp = std::chrono::steady_clock::now() + 250ms;
        reconnect_queue_.emplace(
                ReconnectAttempt{transport, reconnect_timestamp, ReconnectHandler::kMaxAttempts});
    }
    reconnect_cv_.notify_one();
}
    ...
    while (true) {
        ReconnectAttempt attempt;
        {
            ...
            if (!reconnect_queue_.empty()) {
                ...
                //        switch to libc++.
                reconnect_cv_.wait_until(lock, reconnect_queue_.top().reconnect_timestamp);
            } else {
            ...
            if (reconnect_queue_.top().reconnect_timestamp > now) {
                continue;
            }
