...
void kick_all_tcp_devices() {
    ...
    for (auto& t : transport_list) {
        ...
        if (t->type == kTransportLocal && t->adb_port == 0) {
            // It break the output thread of this transport out of any read, then
            // the output thread will notice the main thread to make this transport
            // offline. The main thread will notice the input thread to exit.
            // At last, this transport will be closed and freed.
            kick_transport_locked(t);
        }
    }
    adb_mutex_unlock(&transport_lock);
