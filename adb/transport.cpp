...
static atransport transport_list = {
    .next = &transport_list, .prev = &transport_list,
};

static atransport pending_list = {
    .next = &pending_list, .prev = &pending_list,
};
...
static void dump_packet(const char* name, const char* func, apacket* p) {
    ...
    D("%s: %s: [%s] arg0=%s arg1=%s (len=%d) ", name, func, cmd, arg0, arg1,
      len);
    dump_hex(p->data, len);
}
...
static void* input_thread(void* _t) {
    ...
    // this is necessary to avoid a race condition that occured when a transport
    // closes
...
static int device_tracker_send(device_tracker* tracker,
    ...
    snprintf(reinterpret_cast<char*>(p->data), 5, "%04x",
...
asocket* create_device_tracker(void) {
    device_tracker* tracker =
        reinterpret_cast<device_tracker*>(calloc(1, sizeof(*tracker)));
...
static void transport_registration_func(int _fd, unsigned ev, void* data) {
    ...
    if (t->connection_state != CS_NOPERM) {
        ...
        fdevent_install(&(t->transport_fde), t->transport_socket,
...
retry:
    if (error_out)
        *error_out =
            android::base::StringPrintf("device '%s' not found", serial);
...
static void append_transport(atransport* t, std::string* result,
                             bool long_listing) {
