...
bool FdConnection::doTlsHandshake(atransport* t) {
    ...
    PeerInfo sys_info = {};
    memcpy(sys_info.guid, sys_guid.data(), std::min(adbwifi::pairing::kPeerGuidLength, sys_guid.size()));
    memcpy(sys_info.name, sys_name.data(), std::min(adbwifi::pairing::kPeerNameLength, sys_name.size()));
    if (!tls_->writeFully(std::string_view(reinterpret_cast<const char*>(&sys_info), sizeof(PeerInfo)))) {
        return false;
    }
