...
namespace UsbTransportConstants {
constexpr size_t HEARTBEAT_INTERVAL_IN_SEC = 1;
constexpr size_t HEARTBEAT_FAILED_COUNT_BEFORE_INVALID = 2;
}

namespace EmulatorTransportConstants {
constexpr size_t HEARTBEAT_INTERVAL_IN_SEC = 1;
constexpr size_t HEARTBEAT_FAILED_COUNT_BEFORE_INVALID = 3;
}

namespace TcpTransportConstants {
constexpr size_t HEARTBEAT_INTERVAL_IN_SEC = 1;
constexpr size_t HEARTBEAT_FAILED_COUNT_BEFORE_INVALID = 10;
}
...
static void kick_invalid_transports() {
    ...
    for (auto& t : transport_list) {
        if (t->connection_state == kCsOffline) {
            ...
            if (duration >= t->GetInitialConnectionTimeout()) {
...
time_interval atransport::GetInitialConnectionTimeout() {
    // The initial connection timeout period should be at least as long as
    // the heartbeat timeout period.
    return std::max(time_interval(3),
                    heartbeat_interval_ * heartbeat_failed_count_before_invalid_);
}
    ...
    if (open_duration > heartbeat_interval_) {
        if (heartbeat_close_packet_received_time_ > heartbeat_open_packet_sent_time_) {
            ...
            if (++heartbeat_failed_count_ >= heartbeat_failed_count_before_invalid_) {
                time_interval no_response_duration =
                    std::chrono::duration_cast<time_interval>(
                        now - std::max(heartbeat_close_packet_received_time_, created_time_));
...
int register_socket_transport(int s, const char *serial, int port, int local) {
    atransport* t;
    if (local) {
        t = new atransport(EmulatorTransportConstants::HEARTBEAT_INTERVAL_IN_SEC,
                           EmulatorTransportConstants::HEARTBEAT_FAILED_COUNT_BEFORE_INVALID);
    } else {
        t = new atransport(TcpTransportConstants::HEARTBEAT_INTERVAL_IN_SEC,
                           TcpTransportConstants::HEARTBEAT_FAILED_COUNT_BEFORE_INVALID);
    }
...
void register_usb_transport(usb_handle* usb, const char* serial,
    ...
    atransport* t = new atransport(UsbTransportConstants::HEARTBEAT_INTERVAL_IN_SEC,
                                   UsbTransportConstants::HEARTBEAT_FAILED_COUNT_BEFORE_INVALID);
