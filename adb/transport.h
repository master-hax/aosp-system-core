...
struct BlockingConnectionAdapter : public Connection {
  ...
  private:
    void StartReadFirstPacketThread() REQUIRES(mutex_);
    void StartBlockingReadThread() REQUIRES(mutex_);

    bool started_ GUARDED_BY(mutex_) = false;
    bool stopped_ GUARDED_BY(mutex_) = false;

    std::unique_ptr<BlockingConnection> underlying_;
    std::thread detection_thread_ GUARDED_BY(mutex_);
...
struct FdConnection : public BlockingConnection {
  ...
  private:
    ...
    unique_fd fd_;
    std::unique_ptr<adb::ssl::TlsConnection> tls_;
};
