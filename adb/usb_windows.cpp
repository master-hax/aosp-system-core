...
usb_handle* do_usb_open(const wchar_t* interface_name) {
    ...
    ret->adb_read_pipe = AdbOpenDefaultBulkReadEndpoint(
        ret->adb_interface, AdbOpenAccessTypeReadWrite,
    ...
    if (NULL != ret->adb_read_pipe) {
        ...
        if (NULL != ret->adb_write_pipe) {
            ...
            AdbGetInterfaceName(ret->adb_interface, NULL, &name_len, true);
            if (0 != name_len) {
...
int usb_write(usb_handle* handle, const void* data, int len) {
    ...
    if (NULL != handle) {
        // Perform write
        ret = AdbWriteEndpointSync(handle->adb_write_pipe, (void*)data,
                                   (unsigned long)len, &written, time_out);
        int saved_errno = GetLastError();
        ...
        if (ret) {
            ...
            if (written == (unsigned long)len) {
                if (handle->zero_mask && (len & handle->zero_mask) == 0) {
                    ...
                    AdbWriteEndpointSync(handle->adb_write_pipe, (void*)data, 0,
                                         &written, time_out);
                }
                return 0;
            }
            ...
            if (saved_errno == ERROR_INVALID_HANDLE) usb_kick(handle);
        }
        errno = saved_errno;
    } else {
...
int usb_read(usb_handle* handle, void* data, int len) {
    ...
    if (NULL != handle) {
        while (len > 0) {
            ...
            ret = AdbReadEndpointSync(handle->adb_read_pipe, data,
                                      (unsigned long)xfer, &read, time_out);
            int saved_errno = GetLastError();
...
int recognized_device(usb_handle* handle) {
    ...
    if (is_adb_interface(device_desc.idVendor, device_desc.idProduct,
                         ...
                         interf_desc.bInterfaceSubClass,
...
void find_devices() {
    ...
    while (AdbNextInterface(enum_handle, next_interface, &entry_buffer_size)) {
        ...
        for (copy_name = interf_name; L'\0' != *wchar_name;
        ...
        if (!known_device(interf_name)) {
            ...
            if (NULL != handle) {
                ...
                if (recognized_device(handle)) {
                    ...
                    if (AdbGetSerialNumber(handle->adb_interface, serial_number,
                                           &serial_number_len, true)) {
                        // Lets make sure that we don't duplicate this device
                        if (register_new_device(handle)) {
                            register_usb_transport(handle, serial_number, NULL,
                                                   1);
                        } else {
