...
static void* _power_notification_thread(void* unused) {
  ...
  if (!instance) {
    ...
    fatal("GetModuleHandleW failed: %ld", GetLastError());
  }
  ...
  if (!RegisterClassExW(&wndclass)) {
    fatal("RegisterClassExW failed: %ld", GetLastError());
  }

  if (!CreateWindowExW(WS_EX_NOACTIVATE, kPowerNotificationWindowClassName,
                       L"ADB Power Notification Window", WS_POPUP, 0, 0, 0, 0,
                       NULL, NULL, instance, NULL)) {
    fatal("CreateWindowExW failed: %ld", GetLastError());
  }
...
usb_handle* do_usb_open(const wchar_t* interface_name) {
  ...
  if (NULL == ret->adb_interface) {
    D("AdbCreateInterfaceByName failed: %ld\n", GetLastError());
    goto fail;
  }
  ...
  if (NULL == ret->adb_read_pipe) {
    D("AdbOpenDefaultBulkReadEndpoint failed: %ld\n", GetLastError());
    goto fail;
  }
  ...
  if (NULL == ret->adb_write_pipe) {
    D("AdbOpenDefaultBulkWriteEndpoint failed: %ld\n", GetLastError());
    goto fail;
  }
  ...
  AdbGetInterfaceName(ret->adb_interface,
                      ...
                      true);
  if (0 == name_len) {
    D("AdbGetInterfaceName returned name length of zero: %ld\n",
      GetLastError());
    goto fail;
  }
  ...
  if (!AdbGetInterfaceName(ret->adb_interface,
                           ...
                           &name_len,
                           true)) {
    D("AdbGetInterfaceName failed: %ld\n", GetLastError());
    goto fail;
  }
...
int usb_write(usb_handle* handle, const void* data, int len) {
  ...
  if (!AdbWriteEndpointSync(handle->adb_write_pipe,
                            ...
                            &written,
                            time_out)) {
    D("AdbWriteEndpointSync failed: %ld\n", GetLastError());
    err = EIO;
    goto fail;
  }
  ...
  if (handle->zero_mask && (len & handle->zero_mask) == 0) {
    ...
    if (!AdbWriteEndpointSync(handle->adb_write_pipe,
                              ...
                              &written,
                              time_out)) {
      D("AdbWriteEndpointSync of zero length packet failed: %ld\n",
        GetLastError());
      err = EIO;
      goto fail;
    }
...
int usb_read(usb_handle *handle, void* data, int len) {
  ...
  while (len > 0) {
    ...
    if (!AdbReadEndpointSync(handle->adb_read_pipe,
                             data,
                             (unsigned long)xfer,
                             &read,
                             time_out)) {
      D("AdbReadEndpointSync failed: %ld\n", GetLastError());
      err = EIO;
      goto fail;
    }
...
static void _adb_close_handle(ADBAPIHANDLE adb_handle) {
  if (!AdbCloseHandle(adb_handle)) {
    D("AdbCloseHandle(%p) failed: %ld\n", adb_handle, GetLastError());
  }
}
...
int recognized_device(usb_handle* handle) {
  ...
  if (!AdbGetUsbDeviceDescriptor(handle->adb_interface,
                                 &device_desc)) {
    D("AdbGetUsbDeviceDescriptor failed: %ld\n", GetLastError());
    return 0;
  }
  ...
  if (!AdbGetUsbInterfaceDescriptor(handle->adb_interface,
                                    &interf_desc)) {
    D("AdbGetUsbInterfaceDescriptor failed: %ld\n", GetLastError());
    return 0;
  }
  ...
  if (is_adb_interface(device_desc.idVendor, device_desc.idProduct,
    ...
    if(interf_desc.bInterfaceProtocol == 0x01) {
      ...
      if (AdbGetEndpointInformation(handle->adb_interface, 0, &endpoint_info)) {
        ...
        D("AdbGetEndpointInformation failed: %ld\n", GetLastError());
      }
    }
