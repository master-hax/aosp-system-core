...
static void usb_adb_kick(usb_handle *h) {
    ...
    // If we close h->fd, the file handler number will be reused to open other
    // files, and the read/write thread may operate on the wrong file. So instead
    // we set the kicked flag and reopen h->fd to a dummy file here. After read/write
    // thread finished, we close h->fd in usb_adb_close().
    h->kicked = true;
