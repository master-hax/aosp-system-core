...
static int usb_added_cb(const char* devname, void* cb) {
//            DBGX("[ scanning %s ]\n", devname);

    // TODO: rewrite this using libusbhost.
    ...
    size_t desclength = adb_read(fd, devdesc, sizeof(devdesc));
    ...
    unsigned vid = device->idVendor;
    unsigned pid = device->idProduct;
    ...
    while (bufptr < bufend) {
        ...
        if (type == USB_DT_INTERFACE) {
            ...
            if (interface->bNumEndpoints == 2 &&
                ...
                register_device(devname, devpath,
...
static int usb_removed_cb(const char* devname, void* unused) {
    usb_handle *usb;

    adb_mutex_lock(&usb_lock);
    for (usb = handle_list.next; usb != &handle_list; usb = usb->next) {
        if (!strcmp(usb->fname, devname)) {
            usb_kick(usb);
            return 0;
        }
    }
...
void* device_poll_thread(void* unused)
{
    ...
    usb_host_run(ctx, usb_added_cb, usb_removed_cb, NULL, NULL);
    return NULL;
}
