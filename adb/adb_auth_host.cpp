...
{
    uint8_t* rsa_public_key = nullptr;
    FILE *outfile = NULL;
    ...
    rsa_public_key =
        new uint8_t[ANDROID_PUBKEY_ENCODED_SIZE_FOR_KEY(private_key)];
    if (!rsa_public_key) {
    ...
    if (!android_pubkey_encode(private_key, rsa_public_key,
...
#if defined(OPENSSL_IS_BORINGSSL)
    if (!EVP_EncodedLength(&encoded_length, sizeof(rsa_public_key))) {
    ...
    encoded_length = 1 + ((sizeof(rsa_public_key) + 2) / 3 * 4);
#endif
    ...
    encoded_length =
        EVP_EncodeBlock(encoded, rsa_public_key, sizeof(rsa_public_key));
