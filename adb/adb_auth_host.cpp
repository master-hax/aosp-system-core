...
static std::string get_user_info() {
...
#if !defined(_WIN32)
    char buf[64];
    if (gethostname(buf, sizeof(buf)) != -1) hostname = buf;
#endif
    ...
    if (getlogin()) username = getlogin();
#endif

    return " " + username + "@" + hostname;
}

static bool write_public_keyfile(RSA* private_key, const char* private_key_path) {
    uint8_t binary_key_data[ANDROID_PUBKEY_ENCODED_SIZE];
    if (!android_pubkey_encode(private_key, binary_key_data, sizeof(binary_key_data))) {
        D("Failed to convert to public key");
        return false;
    }
    ...
    std::string path(android::base::StringPrintf("%s.pub", private_key_path));
