...
static bool for_each_public_key(std::function<bool(RSA*)> callback) {
    ...
    for (auto path : key_paths) {
        if (access(path, R_OK) == 0) {
            ...
            for (auto line : android::base::Split(content, "\n")) {
                ...
                bool callback_result = callback(key);

                RSA_free(key);

                if (callback_result) return true;
            }
        }
    }
    return false;
...
bool adb_auth_verify(uint8_t* token, size_t token_size, uint8_t* sig, int sig_len) {
    return for_each_public_key([&](RSA* key) {
      return RSA_verify(NID_sha1, token, token_size, sig, sig_len, key);
    });
}
