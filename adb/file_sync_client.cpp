...
class SyncConnection {
  public:
    SyncConnection() : total_bytes_(0), start_time_ms_(CurrentTimeMs()), expected_total_bytes_(0) {
    ...
    void ReportProgress(const char* file, uint64_t file_copied_bytes, uint64_t file_total_bytes) {
        ...
        if (expected_total_bytes_ != 0) {
            ...
            if (overall_percentage < 100) {
                snprintf(overall_percentage_str, sizeof(overall_percentage_str), "%3d%%",
                         overall_percentage);
            }
        }
        ...
        if (file_copied_bytes > file_total_bytes || file_total_bytes == 0) {
            ...
            Printf("[%s] %s: %" PRId64 "/?", overall_percentage_str, file, file_copied_bytes);
        } else {
            int file_percentage = static_cast<int>(file_copied_bytes * 100 / file_total_bytes);
            Printf("[%s] %s: %d%%", overall_percentage_str, file, file_percentage);
        }
    }
    ...
    void ComputeTarget(const std::vector<copyinfo>& file_list) {
