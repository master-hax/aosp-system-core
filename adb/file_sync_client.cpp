...
bool do_sync_pull(const std::vector<const char*>& srcs, const char* dst,
    ...
    if (stat(dst, &st)) {
        ...
        if (srcs.size() != 1 || errno != ENOENT) {
            sc.Error("adb: cannot stat '%s': %s", dst, strerror(errno));
            return false;
        }
    }
    ...
    for (const char* src_path : srcs) {
        ...
        if (mode == 0) {
            sc.Error("adb: remote object '%s' does not exist", src_path);
            success = false;
            continue;
        }
        ...
        if (S_ISREG(mode) || S_ISLNK(mode) || S_ISCHR(mode) || S_ISBLK(mode)) {
            ...
            sc.Error("adb: remote object '%s' not a file or directory",
                     src_path);
            success = false;
            continue;
        }
