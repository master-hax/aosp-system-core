...
bool do_sync_push(const char* src, const char* dst) {
    std::vector<const char*> srcs;
    srcs.push_back(src);
    return do_sync_push(srcs, dst);
}
...
bool do_sync_push(const std::vector<const char*>& srcs, const char* dst) {
    ...
    if (!dst_isdir) {
        if (srcs.size() > 1) {
            sc.Error("adb: target '%s' is not a directory", dst);
            return false;
        } else {
            size_t dst_len = strlen(dst);
            if (dst[dst_len - 1] == '/') {
                sc.Error("adb: failed to access '%s': Not a directory", dst);
                return false;
            }
        }
    }
    ...
    for (const char* src_path : srcs) {
        ...
        struct stat st;
        if (stat(src_path, &st)) {
            sc.Error("adb: cannot stat '%s': %s", src_path, strerror(errno));
            success = false;
            continue;
        }
...
bool do_sync_pull(const std::vector<const char*>& srcs, const char* dst,
    ...
    if (!dst_isdir) {
        if (srcs.size() > 1) {
            fprintf(stderr, "adb: target '%s' is not a directory\n", dst);
            return false;
        } else {
            size_t dst_len = strlen(dst);
            if (dst[dst_len - 1] == '/') {
                fprintf(stderr, "adb: failed to access '%s': Not a directory\n",
                        dst);
                return false;
            }
        }
