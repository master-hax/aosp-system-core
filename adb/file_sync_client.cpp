...
class SyncConnection {
  ...
  private:
    ...
    void QueryMax() {
        ...
        std::string error;
        int fd = adb_connect("features", &error);
        if (fd < 0) {
            // Assume this is an old adbd we're talking to.
            // If not, we'll detect and report the error when we try to sync.
            return;
        }

        std::string features;
        if (!ReadProtocolString(fd, &features, &error)) {
            return;
        }

        for (const auto& feature : android::base::Split(features, ",")) {
            fprintf(stderr, "host:feature='%s'\n", feature.c_str());
        }
    }
...
static bool sync_start_stat(SyncConnection& sc, const char* path) {
    ...
    return WriteFdExactly(sc.fd, &msg.req, sizeof(msg.req)) && WriteFdExactly(sc.fd, path, len);
}
                      ...
                      unsigned mtime, mode_t mode, bool show_progress)
{
    ...
    if (!WriteFdExactly(sc.fd, &msg.req, sizeof(msg.req)) ||
        !WriteFdExactly(sc.fd, rpath, len) || !WriteFdExactly(sc.fd, tmp, r)) {
        goto fail;
    }
...
static int sync_recv(SyncConnection& sc, const char* rpath, const char* lpath, bool show_progress) {
    ...
    msg.req.id = ID_RECV;
    msg.req.namelen = htoll(len);
    if (!WriteFdExactly(sc.fd, &msg.req, sizeof(msg.req)) || !WriteFdExactly(sc.fd, rpath, len)) {
        return -1;
    }
