...
bool do_sync_push(const char* src, const char* dst, bool show_progress) {
    std::vector<const char*> srcs;
    srcs.push_back(src);
    return do_sync_push(srcs, dst, show_progress);
}
...
bool do_sync_pull(const std::vector<const char*>& srcs, const char* dst,
    ...
    for (const char* src_path : srcs) {
        ...
        if (S_ISREG(mode) || S_ISLNK(mode) || S_ISCHR(mode) || S_ISBLK(mode)) {
            ...
            if (!sync_recv(sc, src_path, dst_path, show_progress)) {
                success = false;
                continue;
            } else {
