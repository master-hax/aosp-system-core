...
static void END() {
    ...
    fprintf(stderr, "%lld KB/s (%lld bytes in %lld.%03llds)\n",
            ((total_bytes * 1000000LL) / t) / 1024LL, total_bytes,
            (t / 1000000LL), (t % 1000000LL) / 1000LL);
}

static const char* transfer_progress_format =
...
static int sync_ls(int fd, const char* path, sync_ls_cb func, void* cookie) {
    ...
    for (;;) {
        ...
        func(ltohl(msg.dent.mode), ltohl(msg.dent.size), ltohl(msg.dent.time),
             buf, cookie);
    }
...
static int sync_readtime(int fd, const char* path, unsigned int* timestamp,
                         unsigned int* mode) {
    syncmsg msg;
    int len = strlen(path);
...
static int sync_finish_readtime(int fd, unsigned int* timestamp,
    ...
    if (!ReadFdExactly(fd, &msg.stat, sizeof(msg.stat))) return -1;

    if (msg.stat.id != ID_STAT) return -1;
...
static int write_data_buffer(int fd, char* file_buffer, int size,
                             syncsendbuf* sbuf, int show_progress) {
    int err = 0;
    int total = 0;
...
static int sync_send(int fd, const char* lpath, const char* rpath,
    ...
    if (!WriteFdExactly(fd, &msg.data, sizeof(msg.data))) goto fail;

    if (!ReadFdExactly(fd, &msg.status, sizeof(msg.status))) return -1;
...
static int sync_recv(int fd, const char* rpath, const char* lpath,
                     int show_progress) {
    syncmsg msg;
    int len;
    int lfd = -1;
...
static copyinfo* mkcopyinfo(const char* spath, const char* dpath,
    ...
    copyinfo* ci =
        reinterpret_cast<copyinfo*>(malloc(sizeof(copyinfo) + ssize + dsize));
    if (ci == 0) {
...
static int local_build_list(copyinfo** filelist, const char* lpath,
                            const char* rpath) {
    DIR* d;
    struct dirent* de;
    struct stat st;
...
static int copy_local_dir_remote(int fd, const char* lpath, const char* rpath,
    ...
    if (checktimestamps) {
        ...
        for (ci = filelist; ci != 0; ci = ci->next) {
            ...
            if (sync_finish_readtime(fd, &timestamp, &mode, &size)) return 1;
            if (size == ci->size) {
    ...
    for (ci = filelist; ci != 0; ci = next) {
        ...
        if (ci->flag == 0) {
            fprintf(stderr, "%spush: %s -> %s\n", listonly ? "would " : "",
                    ci->src, ci->dst);
            if (!listonly && sync_send(fd, ci->src, ci->dst, ci->time, ci->mode,
                                       0 /* no show progress */)) {
                return 1;
            }
            pushed++;
    ...
    if (S_ISDIR(st.st_mode)) {
        ...
        if ((mode != 0) && S_ISDIR(mode)) {
            ...
            */
            ...
            const char* name =
                (slash == std::string::npos) ? lpath : lpath + slash + 1;
...
static int remote_build_list(int syncfd, copyinfo** filelist, const char* rpath,
                             const char* lpath) {
...
static char* add_slash_to_path(const char* path) {
    if (path[strlen(path) - 1] != '/') {
        ...
        if (path_with_slash == NULL) return NULL;
...
int do_sync_pull(const char* rpath, const char* lpath, int show_progress,
    ...
    if (S_ISREG(mode) || S_ISLNK(mode) || S_ISCHR(mode) || S_ISBLK(mode)) {
        if (stat(lpath, &st) == 0) {
            if (S_ISDIR(st.st_mode)) {
                ...
                */
                ...
                const char* name =
                    (slash == std::string::npos) ? rpath : rpath + slash + 1;
        ...
        if (sync_recv(fd, rpath, lpath, show_progress)) {
            ...
            if (copy_attrs && set_time_and_mode(lpath, time, mode)) return 1;
            END();
            sync_quit(fd);
            return 0;
        }
...
int do_sync_sync(const std::string& lpath, const std::string& rpath,
                 bool list_only) {
