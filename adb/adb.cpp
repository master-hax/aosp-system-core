...
void send_connect(atransport* t) {
    ...
    if (connection_str.length() > MAX_PAYLOAD) {
        // Truncating the connection banner may potentially cut off some
        // features but we can try to keep going so that adb may be at least
        // partially functional.
        LOG(WARNING) << "Connection string is too long for packet and will be "
                        "truncated.";
        connection_str.resize(MAX_PAYLOAD);
    }
