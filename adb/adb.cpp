...
void start_device_log(void) {
    ...
    std::string path =
        android::base::StringPrintf("/data/adb/adb-%s-%d", timestamp, getpid());
    int fd =
        unix_open(path.c_str(), O_WRONLY | O_CREAT | O_TRUNC | O_CLOEXEC, 0640);
    if (fd == -1) {
        return;
    }
...
void print_packet(const char* label, apacket* p) {
    ...
    fprintf(stderr, "%s: %s %08x %08x %04x \"", label, tag, p->msg.arg0,
            p->msg.arg1, p->msg.data_length);
...
void parse_banner(const char* banner, atransport* t) {
    ...
    if (pieces.size() > 2) {
        ...
        for (auto& prop : android::base::Split(props, ";")) {
            // The list of properties was traditionally ;-terminated rather than
            // ;-separated.
            ...
            if (t->online && p->msg.arg0 != 0 && p->msg.arg1 != 0) {
                if ((s = find_local_socket(p->msg.arg1, 0))) {
                    if (s->peer == 0) {
                        ...
                        D("Invalid A_OKAY(%d,%d), expected A_OKAY(%d,%d) on "
                          "transport %s\n",
                          p->msg.arg0, p->msg.arg1, s->peer->id, p->msg.arg1,
                          t->serial);
                    }
                }
            }
        ...
        case A_CLSE: /* CLOSE(local-id, remote-id, "") or CLOSE(0, remote-id,
            ...
            if (t->online && p->msg.arg1 != 0) {
                if ((s = find_local_socket(p->msg.arg1, p->msg.arg0))) {
                    /* According to protocol.txt, p->msg.arg0 might be 0 to
                     *indicate
                     * a failed OPEN only. However, due to a bug in previous ADB
                     * versions, CLOSE(0, remote-id, "") was also used for
                     *normal
                     * CLOSE() operations.
                     *
                     * This is bad because it means a compromised adbd could
                     * send packets to close connections between the host and
                     * other devices. To avoid this, only allow this if the
                     *local
                     * socket has a peer on the same transport.
                     */
                    if (p->msg.arg0 == 0 && s->peer && s->peer->transport != t) {
                        D("Invalid A_CLSE(0, %u) from transport %s, expected "
                          "transport %s\n",
...
int launch_server(int server_port) {
    ...
    // fd[0] will be parent's end, and fd[1] will get mapped to stderr in the
    // child.
    if (pipe(fd)) {
    ...
    if (pid == 0) {
        ...
        // child process
        int result =
            execl(path, "adb", "-P", str_port, "fork-server", "server", NULL);
...
/* Get substrings to the left and right of the first occurence of the delimiter.
 * Concatenating left_of + delimiter + right_of would give back the original
 * string if it contained the delimiter or appends the delimiter to the original
 * string if it did not contain the delimiter.
 */
...
int handle_forward_request(const std::string& service, TransportType type,
                           const std::string& serial, int reply_fd) {
    ...
    if (StartsWith(service, "forward:") ||
        ...
        transport =
            acquire_one_transport(CS_ANY, type, serial.c_str(), &error_msg);
        if (!transport) {
        ...
        InstallStatus r;
        if (createForward) {
            r = install_listener(local.c_str(), remote.c_str(), transport,
                                 no_rebind);
        } else {
        ...
        switch (r) {
            ...
            case INSTALL_STATUS_CANNOT_BIND:
                message = android::base::StringPrintf(
                    "cannot bind to socket: %s", strerror(errno));
                break;
            case INSTALL_STATUS_CANNOT_REBIND:
                message = android::base::StringPrintf(
                    "cannot rebind existing socket: %s", strerror(errno));
                break;
...
int handle_host_request(const std::string& service, TransportType type,
                        const std::string& serial, int reply_fd, asocket* s) {
    if (service == "kill") {
    ...
    // "transport:" is used for switching transport with a specified serial
    // number
    // "transport-usb:" is used for switching transport to the only USB
    // transport
    // "transport-local:" is used for switching transport to the only local
    // transport
    ...
    if (StartsWith(service, "transport")) {
        ...
        transport = acquire_one_transport(CS_ANY, type,
                                          transport_serial.c_str(), &error_msg);
    ...
    if (service == "devices") {
        bool long_listing = StartsWith(service.substr(7), "-l");
    ...
    if (service == "version") {
        SendOkay(reply_fd);
        SendProtocolString(
            reply_fd, android::base::StringPrintf("%04x", ADB_SERVER_VERSION));
        return 0;
    }
