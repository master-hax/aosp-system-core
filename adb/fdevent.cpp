...
static std::string dump_fdevent(const fdevent& fde) {
    std::string state;
...
void fdevent_destroy(fdevent *fde)
{
    if(fde == 0) return;
    if(!(fde->state & FDE_CREATED)) {
        LOG(FATAL) << "destroy fde not created by fdevent_create(): " << dump_fdevent(*fde);
    }
    fdevent_remove(fde);
    free(fde);
}
...
void fdevent_install(fdevent* fde, int fd, fd_func func, void* arg) {
    ...
    poll_node_map.emplace(fde->fd, PollNode(fde));
    LOG(DEBUG) << "fdevent_install " << dump_fdevent(*fde);
}

void fdevent_remove(fdevent* fde) {
    LOG(DEBUG) << "fdevent_remove " << dump_fdevent(*fde);
    if (fde->state & FDE_ACTIVE) {
...
void fdevent_set(fdevent* fde, unsigned events) {
    ...
    if (fde->state & FDE_ACTIVE) {
        ...
        LOG(DEBUG) << "fdevent_set: " << dump_fdevent(*fde) << ", events = " << events;
...
static std::string dump_pollfds(const std::vector<pollfd>& pollfds) {
    ...
    for (auto& pollfd : pollfds) {
        ...
        result += android::base::StringPrintf(" %d(%s)", pollfd.fd, op.c_str());
    }
    return result;
}
...
static void fdevent_process() {
    ...
    PLOG(DEBUG) << "poll(), pollfds =" << dump_pollfds(pollfds);
    int ret = poll(&pollfds[0], pollfds.size(), -1);
    PLOG(DEBUG) << "poll(), ret = " << ret;
    for (auto& pollfd : pollfds) {
        unsigned events = 0;
        ...
        if (events != 0) {
            ...
            fde->events |= events;
            LOG(DEBUG) << dump_fdevent(*fde) << ", got events " << std::hex << events;
            fde->state |= FDE_PENDING;
...
static void fdevent_call_fdfunc(fdevent* fde)
{
    ...
    fde->state &= (~FDE_PENDING);
    LOG(DEBUG) << "fdevent_call_fdfunc " << dump_fdevent(*fde);
                                       ...
                                       void* /* userdata */)
{
    ...
    if (ev & FDE_READ) {
        ...
        LOG(DEBUG) << "subproc_fde " << dump_fdevent(*subproc_fde);
