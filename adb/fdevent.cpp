...
static std::unordered_map<int, std::chrono::steady_clock::time_point> g_silent_map;
...
void fdevent_install(fdevent* fde, int fd, fd_func func, void* arg) {
    ...
    g_silent_map.emplace(fde->fd, std::chrono::steady_clock::now());
...
static void fdevent_update(fdevent* fde, unsigned events) {
    ...
    if (events & (FDE_READ | FDE_WRITE)) {
        ...
        g_silent_map[fde->fd] = std::chrono::steady_clock::now();
    }
}
...
static void fdevent_check_silent_fd(std::vector<pollfd>* pollfds) {
    ...
    for (auto& pollfd : *pollfds) {
        if (pollfd.events == 0 && pollfd.revents == 0 &&
            ...
            const auto& start_time = g_silent_map[pollfd.fd];
            if (std::chrono::duration_cast<std::chrono::duration<double>>(
                    cur_time - start_time).count() > g_silent_fd_check_interval_in_sec) {
