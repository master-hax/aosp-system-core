...
struct Decoder {
    ...
    bool Finish() {
        bool f = true;
        std::swap(f, finished_);
        if (f) {
            LOG(ERROR) << "Decoder::Finish called while already finished?";
            return false;
        }
        return true;
    }
...
struct NullDecoder final : public Decoder {
    ...
    virtual DecodeResult Decode(std::span<char>* output) override final {
...
struct NullEncoder final : public Encoder {
    ...
    virtual EncodeResult Encode(Block* output) final {
        output->clear();
...
struct BrotliEncoder final : public Encoder {
    ...
    virtual EncodeResult Encode(Block* output) override final {
        output->clear();
...
struct LZ4Encoder final : public Encoder {
    ...
    bool OutputReady() { return output_buffer_.size() > output_block_size_ || lz4_finalized_; }
    ...
    virtual EncodeResult Encode(Block* output) override final {
        ...
        // Input size chosen to be the maximum size .
