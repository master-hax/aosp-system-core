...
static int jdwp_socket_enqueue(asocket* s, std::vector<char>) {
...
static void jdwp_socket_ready(asocket* s) {
     ...
     */
    if (!jdwp->pass) {
        std::vector<char> data(s->get_max_payload());
        size_t len = jdwp_process_list(data.data(), data.size());
        data.resize(len);
...
static int jdwp_tracker_enqueue(asocket* s, std::vector<char>) {
