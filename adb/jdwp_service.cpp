...
static void jdwp_process_free(JdwpProcess* proc) {
    if (proc) {
        int n;
...
static JdwpProcess* jdwp_process_alloc(int socket) {
    JdwpProcess* proc = reinterpret_cast<JdwpProcess*>(calloc(1, sizeof(*proc)));
...
static void jdwp_process_event(int socket, unsigned events, void* _proc) {
    ...
    if (events & FDE_READ) {
        if (proc->pid < 0) {
            ...
            /* the pid was read, if we get there it's probably because the
             * connection
            ...
            for (;;) {
                ...
                if (len <= 0) {
                    ...
                    D("ignoring unexpected JDWP %d control socket activity (%d "
                      "bytes)\n",
                      proc->pid, len);
                }
            }
...
static int jdwp_control_init(JdwpControl* control, const char* sockname,
                             int socknamelen) {
