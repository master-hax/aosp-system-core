...
static void jdwp_process_event(int socket, unsigned events, void* _proc) {
    ...
    if (events & FDE_READ) {
        if (proc->pid < 0) {
            ...
            static constexpr size_t read_len = 4;
            char* p = proc->in_buf + proc->in_len;
            size_t size = read_len - proc->in_len;
...
static void jdwp_control_event(int s, unsigned events, void* _control) {
    ...
    if (events & FDE_READ) {
        ...
        int s = TEMP_FAILURE_RETRY(adb_socket_accept(control->listen_socket, addrp, &addrlen));
        if (s < 0) {
