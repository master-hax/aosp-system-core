...
static const FHClassRec _fh_file_class = {_fh_file_init,  _fh_file_close,
                                          _fh_file_lseek, _fh_file_read,
                                          _fh_file_write, _fh_file_hook};
...
static const FHClassRec _fh_socket_class = {_fh_socket_init,  _fh_socket_close,
                                            _fh_socket_lseek, _fh_socket_read,
                                            _fh_socket_write, _fh_socket_hook};

#define assert(cond)                                                    \
    do {                                                                \
        if (!(cond))                                                    \
            fatal("assertion failed '%s' on %s:%ld\n", #cond, __FILE__, \
                  __LINE__);                                            \
    } while (0)
...
void* load_file(const char* fn, unsigned* _sz) {
    ...
    file = CreateFile(fn, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, 0,
                      NULL);
...
int adb_open(const char* path, int options) {
    ...
    f->fh_handle = CreateFile(path, desiredAccess, shareMode, NULL,
...
int socket_loopback_client(int port, int type) {
    ...
    snprintf(f->name, sizeof(f->name), "%d(lo-client:%s%d)", _fh_to_int(f),
             type != SOCK_STREAM ? "udp:" : "", port);
    D("socket_loopback_client: port %d type %s => fd %d\n", port,
      type != SOCK_STREAM ? "udp" : "tcp", _fh_to_int(f));
    return _fh_to_int(f);
}
...
int socket_loopback_server(int port, int type) {
    ...
    snprintf(f->name, sizeof(f->name), "%d(lo-server:%s%d)", _fh_to_int(f),
             type != SOCK_STREAM ? "udp:" : "", port);
...
int socket_network_client(const char* host, int port, int type) {
    ...
    snprintf(f->name, sizeof(f->name), "%d(net-client:%s%d)", _fh_to_int(f),
             type != SOCK_STREAM ? "udp:" : "", port);
...
int socket_inaddr_any_server(int port, int type) {
    ...
    snprintf(f->name, sizeof(f->name), "%d(any-server:%s%d)", _fh_to_int(f),
             type != SOCK_STREAM ? "udp:" : "", port);
...
int adb_socket_accept(int serverfd, struct sockaddr* addr, socklen_t* addrlen) {
    ...
    if (!fh) {
        D("adb_socket_accept: not enough memory to allocate accepted socket "
          "descriptor\n");
        return -1;
    }
    ...
    snprintf(fh->name, sizeof(fh->name), "%d(accept:%s)", _fh_to_int(fh),
             serverfh->name);
    D("adb_socket_accept on fd %d returns fd %d\n", serverfd, _fh_to_int(fh));
    return _fh_to_int(fh);
}

int adb_setsockopt(int fd, int level, int optname, const void* optval,
                   socklen_t optlen) {
    ...
    return setsockopt(fh->fh_socket, level, optname,
...
static int bip_buffer_write(BipBuffer bip, const void* src, int len) {
    ...
    while (!bip->can_write) {
        ...
        if (ret != WAIT_OBJECT_0) { /* buffer probably closed */
            D("bip_buffer_write: error %d->%d WaitForSingleObject returned %d, "
              "error %ld\n",
...
Exit:
    ...
    BIPD(
        ("bip_buffer_write: exit %d->%d count %d (as=%d ae=%d be=%d cw=%d "
         "cr=%d\n",
         bip->fdin, bip->fdout, count, bip->a_start, bip->a_end, bip->b_end,
         bip->can_write, bip->can_read));
    LeaveCriticalSection(&bip->lock);
...
static int bip_buffer_read(BipBuffer bip, void* dst, int len) {
    ...
    while (!bip->can_read) {
        ...
        if (ret != WAIT_OBJECT_0) { /* probably closed buffer */
            D("bip_buffer_read: error %d->%d WaitForSingleObject returned %d, "
              "error %ld\n",
              bip->fdin, bip->fdout, ret, GetLastError());
            return 0;
        }
    ...
    bip->a_start += avail;
    if (bip->a_start < bip->a_end) goto Exit;
...
Exit:
    ...
    BIPD(
        ("bip_buffer_read: exit %d->%d count %d (as=%d ae=%d be=%d cw=%d "
         "cr=%d\n",
         bip->fdin, bip->fdout, count, bip->a_start, bip->a_end, bip->b_end,
         bip->can_write, bip->can_read));
    LeaveCriticalSection(&bip->lock);
...
static const FHClassRec _fh_socketpair_class = {
    _fh_socketpair_init, _fh_socketpair_close, _fh_socketpair_lseek,
...
static void dump_fde(fdevent* fde, const char* info) {
    fprintf(stderr, "FDE #%03d %c%c%c %s\n", fde->fd,
            ...
            fde->state & FDE_ERROR ? 'E' : ' ', info);
}
#else
#define dump_fde(fde, info) \
    do {                    \
    } while (0)
#endif
...
static fdevent list_pending = {
    .next = &list_pending, .prev = &list_pending,
};
...
static void event_hook_free(EventHook hook) {
    hook->fh = NULL;
    hook->wanted = 0;
    hook->ready = 0;
...
static void event_hook_signal(EventHook hook) {
    FH f = hook->fh;
    int fd = _fh_to_int(f);
...
static EventHook* event_looper_find_p(EventLooper looper, FH fh) {
    EventHook* pnode = &looper->hooks;
    EventHook node = *pnode;
    for (;;) {
        if (node == NULL || node->fh == fh) break;
        pnode = &node->next;
        node = *pnode;
    }
    return pnode;
...
static void event_looper_hook(EventLooper looper, int fd, int events) {
    FH f = _fh_from_int(fd);
    EventHook* pnode;
    EventHook node;
...
static void event_looper_unhook(EventLooper looper, int fd, int events) {
    FH fh = _fh_from_int(fd);
 ...
 * handle array into chunks with the chunk size less than 64, and fire up as
 *many
 * waiting threads as there are chunks. Then each thread would wait on a chunk
 *of
...
typedef struct WaitForAllParam {
    /* A handle to an event to signal when waiting is over. This handle is
     * shared
     ...
     * the original array. This pointer is shared accross all the waiting
     * threads
...
static unsigned __stdcall _in_waiter_thread(void* arg) {
    ...
    res = WaitForMultipleObjects(param->handles_count + 1, wait_on, FALSE,
                                 INFINITE);
    if (res > 0 && res < (param->handles_count + 1)) {
        /* One of the original handles got signaled. Save its absolute index
         * into
         * the output variable. */
 ...
 */
static int _wait_for_all(HANDLE* handles, int handles_count) {
    WaitForAllParam* threads;
    HANDLE main_event;
    int chunks, chunk, remains;

    /* This variable is going to be accessed by several threads at the same
     * time,
     * this is bound to fail randomly when the core is run on multi-core
     * machines.
     ...
     *    InterlockedCompareExchange() which always provide a full memory
     * barrier
     *    on Win32.
     */
    ...
    for (chunk = 0; chunk < chunks; chunk++) {
        ...
        threads[chunk].thread = (HANDLE)_beginthreadex(
            NULL, 0, _in_waiter_thread, &threads[chunk], 0, NULL);
        if (threads[chunk].thread == NULL) {
            /* Unable to create a waiter thread. Collapse. */
            D("Unable to create a waiting thread %d of %d. errno=%d", chunk,
              chunks, errno);
            chunks = chunk;
            SetEvent(main_event);
            break;
...
static void fdevent_connect(fdevent* fde) {
    ...
    if (events != 0) event_looper_hook(looper, fde->fd, events);
}
...
static void fdevent_disconnect(fdevent* fde) {
    ...
    if (events != 0) event_looper_unhook(looper, fde->fd, events);
}
...
static void fdevent_process() {
    ...
    if (!gotone) {
        ...
        for (hook = looper->hooks; hook; hook = hook->next) {
            ...
            if (hook->h != INVALID_HANDLE_VALUE) {
                ...
                for (nn = 0; nn < looper->htab_count; nn++) {
                    if (looper->htab[nn] == hook->h) goto DontAdd;
                }
        ...
        do {
            ...
            if (looper->htab_count > MAXIMUM_WAIT_OBJECTS) {
                ...
                wait_ret = WaitForMultipleObjects(
                    looper->htab_count, looper->htab, FALSE, INFINITE);
            }
            if (wait_ret == (int)WAIT_FAILED) {
                ...
                /* according to Cygwin, some objects like consoles wake up on
                 * "inappropriate" events
                 * like mouse movements. we need to filter these with the
                 * "check" function
                 */
        ...
        for (hook = looper->hooks; hook; hook = hook->next) {
            if (hook->stop) hook->stop(hook);
        }
    }

    for (hook = looper->hooks; hook; hook = hook->next) {
        if (hook->peek && hook->peek(hook)) event_hook_signal(hook);
    }
}
...
static void _event_socket_verify(EventHook hook, WSANETWORKEVENTS* evts) {
    if (evts->lNetworkEvents & (FD_READ | FD_ACCEPT | FD_CLOSE)) {
        if (hook->wanted & FDE_READ) hook->ready |= FDE_READ;
    ...
    if (evts->lNetworkEvents & (FD_WRITE | FD_CONNECT | FD_CLOSE)) {
        if (hook->wanted & FDE_WRITE) hook->ready |= FDE_WRITE;
        if ((evts->iErrorCode[FD_WRITE] != 0) && hook->wanted & FDE_ERROR)
            hook->ready |= FDE_ERROR;
    }
    if (evts->lNetworkEvents & FD_OOB) {
        if (hook->wanted & FDE_ERROR) hook->ready |= FDE_ERROR;
    }
}
...
static int _socket_wanted_to_flags(int wanted) {
    int flags = 0;
    if (wanted & FDE_READ) flags |= FD_READ | FD_ACCEPT | FD_CLOSE;

    if (wanted & FDE_WRITE) flags |= FD_WRITE | FD_CONNECT | FD_CLOSE;

    if (wanted & FDE_ERROR) flags |= FD_OOB;
...
static int _event_socket_peek(EventHook hook) {
    ...
    if (!WSAEnumNetworkEvents(fh->fh_socket, NULL, &evts)) {
        _event_socket_verify(hook, &evts);
        if (hook->ready) ResetEvent(hook->h);
    }
...
static void _event_socketpair_prepare(EventHook hook) {
    ...
    if (hook->wanted & FDE_READ && rbip->can_read) hook->ready |= FDE_READ;

    if (hook->wanted & FDE_WRITE && wbip->can_write) hook->ready |= FDE_WRITE;
}
...
static bool _get_interesting_input_record_uncached(
    const HANDLE console, INPUT_RECORD* const input_record) {
    for (;;) {
        DWORD read_count = 0;
...
// written.
static size_t _get_char_ignoring_modifier(
    char* const ch, const KEY_EVENT_RECORD* const key_event,
...
static __inline__ size_t _get_non_control_char(
    char* const ch, const KEY_EVENT_RECORD* const key_event,
...
// Get without Alt.
static __inline__ size_t _get_non_alt_char(
    char* const ch, const KEY_EVENT_RECORD* const key_event,
    const DWORD control_key_state) {
    return _get_char_ignoring_modifier(ch, key_event, control_key_state,
                                       VK_MENU);
        ...
        if (_is_enhanced_key(control_key_state)) {
            switch (vk) {
                case VK_RETURN:  // Enter key on keypad
                    ...
                    // gnome-terminal currently sends SS3 "F" and SS3 "H", but
                    // that
                    // will be fixed soon to match xterm which sends CSI "F" and
                    // CSI "H".
                    // https://bugzilla.redhat.com/show_bug.cgi?id=1119764
                    MATCH(VK_END, CSI "F");
                    MATCH(VK_HOME, CSI "H");
            ...
            switch (vk) {
                ...
                case VK_TAB:
                    ...
                    // Number 5 key in keypad when NumLock is off, or if NumLock
                    // is
                ...
                {
                    seqbuflen =
                        _get_non_alt_char(seqbuf, key_event, control_key_state);
