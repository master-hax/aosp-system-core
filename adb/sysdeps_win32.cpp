...
int adb_socket(int domain, int type, int protocol) {
    ...
    if (s == INVALID_SOCKET) {
        const DWORD err = WSAGetLastError();
        auto error = android::base::StringPrintf(
...
ssize_t adb_sendmsg(borrowed_fd fd, const struct adb_msghdr* msg, int flags) {
    ...
    auto ret = WSASendMsg(fh->fh_socket, &wmsg, 0, &dwBytes, NULL, NULL);
    if (ret == SOCKET_ERROR) {
        const DWORD err = WSAGetLastError();
