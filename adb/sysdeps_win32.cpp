...
int  adb_open(const char*  path, int  options)
{
    ...
    if ( f->fh_handle == INVALID_HANDLE_VALUE ) {
        ...
        switch (err) {
            ...
            default:
                D("unknown error: %s", android::base::SystemErrorString(err).c_str());
                errno = ENOENT;
                return -1;
        }
    }
...
static void _fh_socket_init( FH  f ) {
    ...
    if (f->event == WSA_INVALID_EVENT) {
        D("WSACreateEvent failed: %s", android::base::SystemErrorString(WSAGetLastError()).c_str());
...
static int _fh_socket_close( FH  f ) {
    if (f->fh_socket != INVALID_SOCKET) {
        ...
        if (shutdown(f->fh_socket, SD_BOTH) == SOCKET_ERROR) {
            ...
            D("socket shutdown failed: %s",
              android::base::SystemErrorString(WSAGetLastError()).c_str());
#endif
        }
    ...
    if (f->event != NULL) {
        if (!CloseHandle(f->event)) {
            D("CloseHandle failed: %s", android::base::SystemErrorString(GetLastError()).c_str());
        }
        f->event = NULL;
    }
    f->mask = 0;
    ...
    if (result == SOCKET_ERROR) {
        ...
        if (err != WSAEWOULDBLOCK) {
            D("recv fd %d failed: %s", _fh_to_int(f),
              android::base::SystemErrorString(err).c_str());
        }
    ...
    if (!_winsock_init) {
        ...
        if (rc != 0) {
            fatal("adb: could not initialize Winsock: %s",
                  android::base::SystemErrorString(rc).c_str());
        }
        _winsock_init = 1;
...
int network_connect(const std::string& host, int port, int type, int timeout, std::string* error) {
    ...
    if (getaddrinfo(host.c_str(), port_str, &hints, &addrinfo_ptr) != 0) {
        *error = android::base::StringPrintf(
                ...
                port_str, android::base::SystemErrorString(WSAGetLastError()).c_str());
        D("%s", error->c_str());
        return -1;
    }
...
int  adb_socket_accept(int  serverfd, struct sockaddr*  addr, socklen_t  *addrlen)
{
    ...
    if (fh->fh_socket == INVALID_SOCKET) {
        ...
        LOG(ERROR) << "adb_socket_accept: accept on fd " << serverfd <<
                      " failed: " + android::base::SystemErrorString(err);
        _socket_set_errno( err );
        return -1;
    }
...
int  adb_setsockopt( int  fd, int  level, int  optname, const void*  optval, socklen_t  optlen )
{
    ...
    if ( result == SOCKET_ERROR ) {
        ...
        D("adb_setsockopt: setsockopt on fd %d level %d optname %d failed: %s\n",
          fd, level, optname, android::base::SystemErrorString(err).c_str());
        _socket_set_errno( err );
        result = -1;
    }
    return result;
...
int  adb_shutdown(int  fd)
{
    ...
    if (shutdown(f->fh_socket, SD_BOTH) == SOCKET_ERROR) {
        const DWORD err = WSAGetLastError();
        D("socket shutdown fd %d failed: %s", fd, android::base::SystemErrorString(err).c_str());
        _socket_set_errno(err);
        return -1;
    }
    return 0;
