...
_fh_alloc( FHClass  clazz )
{
    ...
    for (int nn = 0; nn < WIN32_MAX_FHS; nn++) {
        ...
        if (_win32_fh_next == WIN32_MAX_FHS) {
            // close will always set _win32_fh_next to the lowest open file descriptor.
            break;
        }
...
extern int adb_poll(adb_pollfd* fds, size_t nfds, int timeout) {
    ...
    for (size_t i = 0; i < nfds; ++i) {
        ...
        if (!fh || !fh->used || fh->clazz != &_fh_socket_class) {
            ...
            sockets.push_back(fds[i]);
            sockets.back().fd = fh->u.socket;
...
int adb_socketpair(int sv[2]) {
    ...
    if (adb_getsockname(server, reinterpret_cast<sockaddr*>(&addr), &addr_len) < 0) {
        D("adb_socketpair: adb_getsockname failed: %s", strerror(errno));
        goto fail;
    }

    if (addr.sin_family != AF_INET) {
        D("adb_socketpair: unknown address family received: %d", AF_INET);
        errno = ECONNABORTED;
        goto fail;
    }
