...
int adb_pread(borrowed_fd fd, void* buf, int len, off64_t offset) {
    ...
    DWORD bytes_read;
    if (!::ReadFile(reinterpret_cast<HANDLE>(adb_get_os_handle(fd)), buf, static_cast<DWORD>(len),
...
int adb_pwrite(borrowed_fd fd, const void* buf, int len, off64_t offset) {
    ...
    if (!::WriteFile(reinterpret_cast<HANDLE>(adb_get_os_handle(fd)), buf, len, &countWritten,
                     &params)) {
    ...
    return static_cast<int>(countWritten);
}
...
intptr_t adb_get_os_handle(borrowed_fd fd) {
    FH f = _fh_from_int(fd, __func__);
    ...
    return f->clazz->_fh_get_os_handle(f);
}
