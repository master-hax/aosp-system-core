...
// Given a sequence of UTF-8 bytes (denoted by the iterator range [first, last)), return the number
// of bytes (from the beginning) that are complete UTF-8 sequences and append the remaining bytes to
// remaining_bytes. This is a template function so that the algorithm can operate on a raw byte
// array or an STL container.
template <class InputIterator>
static size_t ParseCompleteUTF8(const InputIterator first, const InputIterator last,
    ...
    for (InputIterator current_after = last; current_after != first; --current_after) {
        const InputIterator current = current_after - 1;
        const char ch = *current;
        const char kHighBit = 0x80;
        const char kTwoHighestBits = 0xC0;
...
static int _console_write_utf8(const char* const buf, const size_t buf_size, FILE* stream,
    ...
    if (g_console_output_buffer.empty()) {
        ...
        utf8 = buf;
        utf8_size = ParseCompleteUTF8(buf, buf + buf_size, &g_console_output_buffer);
    } else {
        // If g_console_output_buffer has a buffered up incomplete UTF-8 sequence, move it to
        // combined_buffer and append buf, then parse it all together.
        combined_buffer = std::move(g_console_output_buffer);
        combined_buffer.insert(combined_buffer.end(), buf, buf + buf_size);

        g_console_output_buffer.clear();    // reset after std::move().

        utf8 = combined_buffer.data();
        utf8_size = ParseCompleteUTF8(combined_buffer.begin(), combined_buffer.end(),
