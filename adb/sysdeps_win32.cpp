...
static size_t _console_fwrite(const void* ptr, size_t size, size_t nmemb,
    ...
    // to a complete UTF-16 char, so the output won't look right. For now we
    // ignore this problem because the alternative is that we'd have to parse
    // UTF-8 and buffer things up (doable). At least this is better than what
    // we had before -- always incorrect multi-byte UTF-8 output.
