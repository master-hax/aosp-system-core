...
// Register a dummy socket, so fdevet_loop() will not abort when previously registered
// local sockets are all closed.
...
// This test checks if we can close local socket in the following situation:
// The socket is not closing and has some packets. When it fails to write to
