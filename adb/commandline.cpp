...
static void help() {
    ...
    fprintf(stderr,
        ...
        "                                 Port 5555 is used by default if no port number\n"
        "                                 is specified. Using this command with no\n"
        "                                 additional arguments will disconnect from all\n"
        ...
        "                                   tcp:<port> (<local> may be \"tcp:0\" to pick\n"
        "                                   any open port)\n"
        ...
        "                                   tcp:<port> (<remote> may be \"tcp:0\" to\n"
        "                                   pick any open port)\n"
        "                                   localabstract:<unix domain socket name>\n"
        "                                   localreserved:<unix domain socket name>\n"
        "                                   localfilesystem:<unix domain socket name>\n"
        ...
        "  adb reverse --remove-all     - remove all reversed socket connections from\n"
        "                                 device\n"
        "  adb jdwp                     - list PIDs of processes hosting a JDWP transport\n"
        "  adb install [-lrtsdg] <file>\n"
        "                               - push this package file to the device and\n"
        "                                 install it\n"
        ...
        "                               - push these package files to the device and\n"
        "                                 install them\n"
        "                                 (-l: forward lock application)\n"
        "                                 (-r: replace existing application)\n"
        "                                 (-t: allow test packages)\n"
        ...
        "                                 <file>. If no -f option is supplied then the\n"
        "                                 data is written to \"backup.ab\" in the\n"
        "                                 current directory.\n"
        "                                 (-apk|-noapk enable/disable backup of the .apks\n"
        "                                    themselves in the archive; the default is\n"
        "                                    noapk.)\n"
        "                                 (-obb|-noobb enable/disable backup of any\n"
        "                                    installed apk expansion (aka .obb) files\n"
        "                                    associated with each application; the\n"
        "                                    default is noobb.)\n"
        "                                 (-shared|-noshared enable/disable backup of the\n"
        "                                    device's shared storage / SD card contents;\n"
        "                                    the default is noshared.)\n"
        "                                 (-all means to back up all installed\n"
        "                                    applications)\n"
        "                                 (-system|-nosystem toggles whether -all\n"
        "                                    automatically includes system applications;\n"
        "                                    the default is to include system apps)\n"
        "                                 (<packages...> is the list of applications to\n"
        "                                    be backed up.  If the -all or -shared flags\n"
        "                                    are passed, then the package list is\n"
        "                                    optional.  Applications explicitly given on\n"
        "                                    the command line will be included even if\n"
        "                                    -nosystem would ordinarily cause them to be\n"
        "                                    omitted.)\n"
        "\n"
        ...
        "  adb enable-verity            - re-enable dm-verity checking on USERDEBUG\n"
        "                                 builds\n"
        "  adb keygen <file>            - generate adb public/private key. The private\n"
        "                                 key is stored in <file>, and the public key is\n"
        "                                 stored in <file>.pub. Any existing files are\n"
        "                                 overwritten.\n"
        ...
        "                                 /oem (if present) partitions on the device\n"
        "                                 read-write\n"
        "  adb reboot [bootloader|recovery]\n"
        "                               - reboots the device, optionally into the\n"
        "                                 bootloader or recovery program.\n"
        "  adb reboot sideload          - reboots the device into the sideload mode in\n"
        "                                 recovery program (adb root required).\n"
        "  adb reboot sideload-auto-reboot\n"
        "                               - reboots into the sideload mode, then reboots\n"
        "                                 automatically after the sideload regardless of\n"
        "                                 the result.\n"
        ...
        "  adb tcpip <port>             - restarts the adbd daemon listening on TCP on\n"
        "                                 the specified port\n"
        "\n"
        "networking:\n"
        "  adb ppp <tty> [parameters]   - Run PPP over USB.\n"
        "                                 Note: you should not automatically start a\n"
        "                                 PPP connection.\n"
        "                                 <tty> refers to the tty for PPP stream.\n"
        "                                   Eg. dev:/dev/omap_csmi_tty1\n"
        "                                 [parameters] - Eg. defaultroute debug dump \n"
        "                                   local notty usepeerdns\n"
        "\n"
        "adb sync notes: adb sync [ <directory> ]\n"
        "                                 <directory> can be interpreted in several ways:\n"
        "                                  - If <directory> is not specified, /system,\n"
        "                                    /vendor (if present), /oem (if present) and\n"
        "                                    /data partitions will be updated.\n"
        "                                  - If it is \"system\", \"vendor\", \"oem\" or\n"
        "                                    \"data\", only the corresponding partition\n"
        "                                    is updated.\n"
        "\n"
        "internal debugging:\n"
        "  adb reconnect                  Kick current connection from host side and make\n"
        "                                 it reconnect.\n"
        "  adb reconnect device           Kick current connection from device side and\n"
        "                                 make it reconnect.\n"
        "environment variables:\n"
        "  ADB_TRACE                    - Print debug information. A comma separated list\n"
        "                                 of one or all of the following values:\n"
        "                                 adb, sockets, packets, rwx, usb, sync, sysdeps,\n"
        "                                 transport, jdwp\n"
