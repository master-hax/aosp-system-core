...
int adb_commandline(int argc, const char **argv) {
    ...
    if (!server_socket_str) {
        ...
        server_host_str = server_host_str ? server_host_str : "localhost";

        server_port_str = server_port_str ? server_port_str : getenv("ANDROID_ADB_SERVER_PORT");
        server_port_str = server_port_str ? server_port_str : DEFAULT_ADB_PORT_STR;

        char* temp = nullptr;
        if (asprintf(&temp, "tcp:%s:%s", server_host_str, server_port_str) < 0) {
