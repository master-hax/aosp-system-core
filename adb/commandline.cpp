...
static int install_app(TransportType t, const char* serial, int argc,
                       const char** argv);
static int install_multiple_app(TransportType t, const char* serial, int argc,
                                const char** argv);
static int uninstall_app(TransportType t, const char* serial, int argc,
                         const char** argv);
...
static std::string product_file(const char* extra) {
    ...
    return android::base::StringPrintf("%s%s%s", gProductOutPath.c_str(),
                                       OS_PATH_SEPARATOR_STR, extra);
}
...
static void help() {
    ...
    fprintf(
        ...
        "                                 ('-p' to display the transfer "
        "progress)\n"
        "  adb pull [-p] [-a] <remote> [<local>]\n"
        "                               - copy file/dir from device\n"
        "                                 ('-p' to display the transfer "
        "progress)\n"
        "                                 ('-a' means copy timestamp and "
        "mode)\n"
        ...
        "  adb forward --list           - list all forward socket "
        "connections.\n"
        "                                 the format is a list of lines with "
        ...
        "                               - same as 'adb forward <local> "
        "<remote>' but fails\n"
        "                                 if <local> is already forwarded\n"
        "  adb forward --remove <local> - remove a specific forward socket "
        "connection\n"
        "  adb forward --remove-all     - remove all forward socket "
        "connections\n"
        ...
        "                               - same as 'adb reverse <remote> "
        "<local>' but fails\n"
        "                                 if <remote> is already reversed.\n"
        "  adb reverse --remove <remote>\n"
        "                               - remove a specific reversed socket "
        "connection\n"
        "  adb reverse --remove-all     - remove all reversed socket "
        "connections from device\n"
        "  adb jdwp                     - list PIDs of processes hosting a "
        "JDWP transport\n"
        ...
        "directory.\n"
        "                                 (-apk|-noapk enable/disable backup "
        "of the .apks themselves\n"
        "                                    in the archive; the default is "
        "noapk.)\n"
        "                                 (-obb|-noobb enable/disable backup "
        "of any installed apk expansion\n"
        ...
        "                                    system applications; the default "
        "is to include system apps)\n"
        ...
        "                                    command line will be included "
        "even if -nosystem would\n"
        ...
        "permissions\n"
        "  adb unroot                   - restarts the adbd daemon without "
        "root permissions\n"
        "  adb usb                      - restarts the adbd daemon listening "
        "on USB\n"
        "  adb tcpip <port>             - restarts the adbd daemon listening "
        "on TCP on the specified port\n"
        "networking:\n"
        ...
        "only these debug tags are printed.\n");
}
...
static std::string format_host_command(const char* command, TransportType type,
                                       const char* serial) {
    if (serial) {
...
static int adb_download_buffer(const char* service, const char* fn,
                               const void* data, unsigned sz,
                               bool show_progress) {
    std::string error;
    int fd =
        adb_connect(android::base::StringPrintf("%s:%d", service, sz), &error);
    if (fd < 0) {
        fprintf(stderr, "error: %s\n", error.c_str());
        return -1;
    }
    ...
    int opt = CHUNK_SIZE;
    opt = adb_setsockopt(fd, SOL_SOCKET, SO_SNDBUF, (const void*)&opt,
                         sizeof(opt));
    ...
    while (sz > 0) {
        ...
        ptr += xfer;
        if (show_progress) {
            printf("sending: '%s' %4d%%    \r", fn,
                   (int)(100LL - ((100LL * sz) / (total))));
            fflush(stdout);
        }
    }
    if (show_progress) {
...
static int adb_sideload_host(const char* fn) {
    ...
    std::string service = android::base::StringPrintf("sideload-host:%d:%d", sz,
...
static bool wait_for_device(const char* service, TransportType t,
                            const char* serial) {
    // Was the caller vague about what they'd like us to wait for?
    // If so, check they weren't more specific in their choice of transport
    // type.
...
static int logcat(TransportType transport, const char* serial, int argc,
    ...
    std::string cmd =
        "shell:export ANDROID_LOG_TAGS=\"" + quoted + "\"; exec logcat";
...
static std::string find_product_out_path(const char* hint) {
    ...
    if (adb_dirstart(hint) != std::string::npos) {
        ...
        return android::base::StringPrintf("%s%s%s", cwd.c_str(),
                                           OS_PATH_SEPARATOR_STR, hint);
    }
...
int adb_commandline(int argc, const char** argv) {
    ...
    while (argc > 0) {
        if (!strcmp(argv[0], "server")) {
            ...
            /* this is a special flag used only when the ADB client launches the
             * ADB Server */
            is_daemon = 1;
            ...
            if (strlen(server_port_str) > 0) {
                ...
                if (server_port <= 0 || server_port > 65535) {
                    fprintf(stderr,
                            "adb: port number must be a positive number less "
                            "than 65536. Got \"%s\"\n",
                            server_port_str);
                    return usage();
                }
    ...
    if (!strcmp(argv[0], "devices")) {
        ...
        std::string query =
            android::base::StringPrintf("host:connect:%s", argv[1]);
        return adb_query_command(query);
        ...
        std::string query = android::base::StringPrintf(
            "host:disconnect:%s", (argc == 2) ? argv[1] : "");
        return adb_query_command(query);
    ...
    /* adb_command() wrapper commands */
        ...
        if (list) {
            ...
            std::string query =
                android::base::StringPrintf("%s:list-forward", host_prefix);
            return adb_query_command(query);
        }
        ...
        // Implement forward --remove <local>
            ...
            if (argc != 2) return usage();
            cmd = android::base::StringPrintf("%s:killforward:%s", host_prefix,
                                              argv[1]);
        }
        // Or implement one of:
        ...
        //    forward --no-rebind <local> <remote>
            ...
            cmd = android::base::StringPrintf("%s:%s:%s;%s", host_prefix,
    ...
    /* do_sync_*() commands */
        ...
        if (src != "" && src != "system" && src != "data" && src != "vendor" &&
            src != "oem") {
            return usage();
        }
        ...
        if (rc == 0 && (src.empty() || src == "vendor") &&
            directory_exists(vendor_src_path)) {
            rc = do_sync_sync(vendor_src_path, "/vendor", list_only);
        }
        if (rc == 0 && (src.empty() || src == "oem") &&
            directory_exists(oem_src_path)) {
    ...
    /* passthrough commands */
        ...
        return adb_query_command(
            format_host_command(argv[0], transport_type, serial));
    }
    /* other commands */
...
static int pm_command(TransportType transport, const char* serial, int argc,
                      const char** argv) {
...
static int uninstall_app(TransportType transport, const char* serial, int argc,
                         const char** argv) {
    ...
    if (argc == 3 && strcmp(argv[1], "-k") == 0) {
        printf(
            ...
            "signature, and fully uninstall it.\n"
...
static int install_app(TransportType transport, const char* serial, int argc,
                       const char** argv) {
...
static int install_multiple_app(TransportType transport, const char* serial,
    ...
    std::string cmd = android::base::StringPrintf(
        "exec:pm install-create -S %u", (unsigned)total_size);
#else
    std::string cmd = android::base::StringPrintf(
        "exec:pm install-create -S %" PRIu64, total_size);
#endif
...
#if defined(_WIN32)  // Remove when we're using clang for Win32.
        std::string cmd = android::base::StringPrintf(
            "exec:pm install-write -S %u %d %d_%s -", (unsigned)sb.st_size,
            session_id, i, get_basename(file));
#else
        std::string cmd = android::base::StringPrintf(
...
finalize_session:
    ...
    std::string service = android::base::StringPrintf(
        "exec:pm install-%s %d", success ? "commit" : "abandon", session_id);
