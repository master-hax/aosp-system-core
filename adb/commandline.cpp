...
static void help() {
    ...
    fprintf(stdout,
        ...
        "     -s,--sync only pushes files that are newer on the host than the device\n"
        " pull [-a] REMOTE... LOCAL\n"
    ...
    while (narg > 0) {
        if (ignore_flags || *arg[0] != '-') {
            ...
            if (!strcmp(*arg, "-p")) {
                ...
                *copy_attrs = true;
            } else if (!strcmp(*arg, "-s") || !strcmp(*arg, "--sync")) {
                if (sync != nullptr) {
                    *sync = true;
                }
            } else if (!strcmp(*arg, "--")) {
...
int adb_commandline(int argc, const char** argv) {
    ...
    /* do_sync_*() commands */
        ...
        parse_push_pull_args(&argv[optind], argc - 1, &srcs, &dst, &copy_attrs, &sync);
