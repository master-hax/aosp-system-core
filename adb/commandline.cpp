...
static void help() {
    ...
    fprintf(stdout,
        ...
        "     -p/--streaming: always stream APK directly into Package Manager\n"
        "     --nostreaming: always push APK to device and invoke Package Manager as separate steps\n"
        "     -f/--fastdeploy: force use of fast deployment (only valid with -r)\n"
        "     --nofastdeploy: prevent use of fast deployment (only valid with -r)\n"
        "     --forceagent: force update of deployment agent when using fast deploy\n"
        "     --datecheckagent: update deployment agent when local version is newer and using fast deploy\n"
        "     --versioncheckagent: update deployment agent when local version has different version code and using fast deploy\n"
        "     --uselocalagentbuild: locate agent files from local source build (instead of SDK location)\n"
