...
int adb_commandline(int argc, const char** argv) {
    ...
    if (!strcmp(argv[0], "devices")) {
        ...
        if (argc < 2) {
            ...
            return syntax_error("usage: adb devices [-l]");
        }
        ...
        if (argc != 2) return syntax_error("usage: adb connect <host>[:<port>]");

        std::string query = android::base::StringPrintf("host:connect:%s", argv[1]);
        return adb_query_command(query);
    }
    else if (!strcmp(argv[0], "disconnect")) {
        if (argc > 2) return syntax_error("usage: adb disconnect [<host>[:<port>]]");
        ...
        if (argc < 2) return syntax_error("usage: adb %s command", argv[0]);
    ...
    /* do_sync_*() commands */
        ...
        if (argc < 2) {
            ...
            return syntax_error("usage: adb sync [-l] [PARTITION]");
        }
    ...
    /* "adb /?" is a common idiom under Windows */
        ...
        if (argc == 1) {
            ...
            if (!strcmp(argv[1], "device")) {
                ...
                return syntax_error("usage: adb reconnect [device|offline]");
            }
        }
    }
                                ...
                                const char** argv)
{
    ...
    if (first_apk == -1) return syntax_error("No APK file on command line");
...
static int install_app_legacy(TransportType transport, const char* serial, int argc, const char** argv) {
    ...
    if (last_apk == -1) return syntax_error("No APK file on command line");
