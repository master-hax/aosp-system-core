...
static void handle_SIGWINCH(int) {
    // We can't ensure that SIGWINCH goes to the stdin read thread,
    // so we send SIGUSR1 directly to the right thread ourselves.
...
static void* stdin_read_thread_loop(void* x) {
    StdinReadArgs* args = reinterpret_cast<StdinReadArgs*>(x);
    int state = 0;
...
#if !defined(_WIN32)
    // The SIGWINCH catcher will send SIGUSR1 to this thread.
...
static int RemoteShell(bool use_shell_protocol, const std::string& type_arg,
    ...
    std::unique_ptr<StdinReadArgs> args(new StdinReadArgs);
    if (!args) {
    ...
    return read_and_dump(fd, use_shell_protocol);
}

static void adb_shell(int argc, const char** argv,
    ...
    exit(RemoteShell(use_shell_protocol, shell_type_arg, command));
}
...
int adb_commandline(int argc, const char **argv) {
    ...
    if (!strcmp(argv[0], "devices")) {
        ...
        adb_shell(argc, argv, transport_type, serial);
    }
