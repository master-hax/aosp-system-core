...
int adb_commandline(int argc, const char **argv) {
    ...
    if (!strcmp(argv[0], "devices")) {
        ...
        while (argc) {
            if (!strcmp(argv[0], "-T") || !strcmp(argv[0], "-t")) {
                if (features.count(kFeatureShellPtyArgs) == 0) {
    ...
    /* "adb /?" is a common idiom under Windows */
        ...
        // Only list the features common to us and the device so this list is
        // an accurate representation of what we can do. To get all device
        // features, whether we support them or not, use `adb devices -l`.
        FeatureSet features = GetFeatureSet(transport_type, serial);
