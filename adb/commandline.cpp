...
static bool is_android_build_top(const std::string& top) {
    return (adb_is_absolute_host_path(top.c_str()) &&
        directory_exists(top + OS_PATH_SEPARATOR_STR "out" OS_PATH_SEPARATOR_STR "target"));
}

static std::string find_top_from(const std::string& start_dir) {
    std::string result(start_dir);
    while (true) {
        if (is_android_build_top(result)) {
            return result;
        }

        char* s = adb_dirstop(result.c_str());
        if (s == nullptr) {
            return "";
        }

        result = result.erase(s - &result[0]);
    }
}

static std::string find_top() {
    char* top = getenv("ANDROID_BUILD_TOP");
    if (top != nullptr) {
        if (is_android_build_top(top)) {
    ...
    // The environment variable wasn't set. Walk up the tree from the CWD
    ...
    std::string result = find_top_from(cwd);
    if (result.empty()) {
        // If the CWD isn't under a plausible top, see if the executable is.
        char my_path[PATH_MAX];
        get_my_path(my_path, PATH_MAX);
        result = find_top_from(my_path);
    }
    return result;
}
