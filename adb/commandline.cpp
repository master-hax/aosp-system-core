...
int adb_commandline(int argc, const char **argv) {
    ...
    if (!server_socket_str) {
        ...
        if (server_port_str) {
            char* parsed_end;
            server_port = strtol(server_port_str, &parsed_end, 10);
            if (*parsed_end != '\0' || server_port <= 0 || server_port > 65535) {
                fprintf(stderr, "adb: port number must be a positive number less than 65536, got '%s'\n",
                        server_port_str);
                exit(1);
            }
        }
    ...
    if (!lockfile_acquire(&lockfile_contents)) {
        if (lockfile_contents != server_socket_str && is_local_socket_spec(server_socket_str)) {
