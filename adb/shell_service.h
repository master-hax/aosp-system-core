...
class ShellProtocol {
  public:
    // Protocol supports exit codes between 0-255.
    ...
    // Returns the exit code in the buffer. Only valid if id() returns kIdExit.
    exit_code_t exit_code() const { return data()[0]; }

    // Writes the packet currently in the buffer to the FD.
    //
    // For stdin/out/err packets pass the data length, for exit code packets
    // pass the desired exit code instead.
    //
    // Returns false if the FD closed or errored.
    bool Write(Id id, size_t length_or_exit_code);
