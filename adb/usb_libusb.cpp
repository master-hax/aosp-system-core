...
// Internal implementation of externally-opaque usb_handle type.
struct usb_handle {
    ...
    std::string serial;
    unique_device_handle device_handle;
    uint8_t interface;
    uint8_t bulk_in;
    uint8_t bulk_out;
    uint16_t zero_mask;
        ...
        for (ssize_t i = 0; i < device_count; ++i) {
            ...
            for (interface_num = 0; interface_num < config->bNumInterfaces; ++interface_num) {
                ...
                for (size_t endpoint_num = 0; endpoint_num < interface_desc.bNumEndpoints;
                    ...
                    const uint8_t direction = endpoint_addr & LIBUSB_ENDPOINT_DIR_MASK;
                    ...
                    if (direction == LIBUSB_ENDPOINT_IN && !found_in) {
                        ...
                        zero_mask = endpoint_desc.wMaxPacketSize;
                    }
                }
...
int usb_write(usb_handle* h, const void* d, int len) {
    ...
    unsigned char* data = reinterpret_cast<unsigned char*>(const_cast<void*>(d));
    bool need_zero_transfer = false;

    if (len & h->zero_mask) {
        need_zero_transfer = true;
    }
    ...
    while (len > 0) {
        sent = -1;
        rc = libusb_bulk_transfer(h->device_handle.get(), h->bulk_out, data, len, &sent, 0);
        LOG(DEBUG) << "usb_write wrote " << sent << " bytes";

        if (rc == 0) {
            len -= sent;
            data += sent;
        } else {
            LOG(WARNING) << "usb_write failed for '" << h->serial << "': " << libusb_error_name(rc);
            errno = EIO;
            return -1;
        }
    }

    if (need_zero_transfer) {
        rc = libusb_bulk_transfer(h->device_handle.get(), h->bulk_out, nullptr, 0, &sent, 0);
        if (rc != 0) {
            LOG(WARNING) << "usb_write failed while sending zero marker: " << libusb_error_name(rc);
            errno = EIO;
            return -1;
        }
    }
...
int usb_read(usb_handle* h, void* d, int len) {
    ...
    while (len > 0) {
        int received = -1;
        int rc = libusb_bulk_transfer(h->device_handle.get(), h->bulk_in, data, len, &received, 0);
        LOG(DEBUG) << "usb_read read " << received << "bytes";
        if (rc == 0) {
            len -= received;
            data += received;
        } else {
            LOG(WARNING) << "usb_read failed for '" << h->serial << "': " << libusb_error_name(rc);
            errno = EIO;
            return -1;
        }
    }
...
void usb_kick(usb_handle* h) {
    libusb_release_interface(h->device_handle.get(), h->interface);
    h->device_handle.reset();
}
