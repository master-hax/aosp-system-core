...
static void config_auto_connect_services() {
    // ADB_MDNS_AUTO_CONNECT_SRVS is a comma-delimited list of mdns services
    // that are allowed to auto-connect. By default, only allow "abd-tls-connect"
    ...
    const char* srvs = getenv("ADB_MDNS_AUTO_CONNECT_SRVS");
    if (!srvs) {
        return;
    }
    ...
    // E.g. ADB_MDNS_AUTO_CONNECT_SRVS=adb,adb-tls-connect would allow
