  ...
  public:
    ...
    bool ConnectIfInKeystore() {
          ...
          std::unique_ptr<void, void (*)(void*)> keystore_ctx(
                      keystore_new_ctx(adb_get_android_dir_path().c_str()),
                      keystore_delete_ctx);
          if (keystore_ctx == nullptr ||
              !keystore_contains_guid(keystore_ctx.get(), serviceName_.c_str())) {
              LOG(ERROR) << "Keystore context is null or serviceName["
                         << serviceName_ << "] not in keystore.";
              return false;
          }
    ...
    void Connect(const sockaddr* address) {
        ...
        if (!inet_ntop(sa_family_, const_cast<void*>(ip_addr_data_),
                       ip_addr_, sizeof(ip_addr_))) {
...
static int parse_version_from_txt_record(uint16_t txtLen,
                                         const unsigned char* txtRecord) {
    if (!txtLen) return -1;
    ...
    memcpy(str, txtRecord + 1 /* skip the length byte */,
           txtLen);
    ...
    if (parsedNumber == LONG_MIN ||
        parsedNumber == LONG_MAX) return -1;
...
static void DNSSD_API register_resolved_mdns_service(DNSServiceRef sdRef,
    ...
    auto resolved =
        new ResolvedService(discovered->ServiceName(), discovered->RegType(),
...
static void DNSSD_API on_service_browsed(DNSServiceRef sdRef,
                                         DNSServiceFlags flags,
                                         uint32_t interfaceIndex,
                                         DNSServiceErrorType errorCode,
                                         const char* serviceName,
...
void init_mdns_transport_discovery_thread(void) {
    ...
    for (int i = 0; i < kNumADBDNSServices; ++i) {
        errorCodes[i] =
            DNSServiceBrowse(&service_refs[i], 0, 0,
                             kADBDNSServices[i], nullptr,
                             on_service_browsed, nullptr);

        if (errorCodes[i] != kDNSServiceErr_NoError) {
            D("Got %d browsing for mDNS service %s.",
              errorCodes[i],
              kADBDNSServices[i]);
        }
