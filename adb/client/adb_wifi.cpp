...
void adb_wifi_pair_device(const std::string& host, const std::string& password,
    ...
    memcpy(system_info.guid, sys_guid.data(),
           std::min(adbwifi::pairing::kPeerGuidLength, sys_guid.size()));
    memcpy(system_info.name, sys_name.data(),
    ...
    auto client = PairingClient::create(pswd8, system_info, cert8, priv8, host);
    if (client == nullptr) {
    ...
    auto callback = [&](const adbwifi::pairing::PeerInfo* peer_info,
                        const adbwifi::pairing::PairingConnection::Data* cert, void* /* opaque */) {
        if (peer_info != nullptr && cert != nullptr) {
            // Save in the keystore
            auto key_info =
                    std::make_tuple(std::string(peer_info->guid), std::string(peer_info->name),
    ...
    if (*got_pairing) {
        // Try to auto-connect.
        adb_secure_connect_by_service_name(guid.c_str());
    }
}
