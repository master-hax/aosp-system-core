...
static bool copy_local_dir_remote(SyncConnection& sc, std::string lpath,
    ...
    // P shipped with a bug that causes directory creation as a side-effect of a push fail.
    // Work around this by explicitly doing a mkdir via shell. (b/110953234)
    ...
    if (CanUseFeature(sc.Features(), kFeatureShell2)) {
        ...
        auto it = directory_list.begin();
        std::string cmd = "mkdir";
        while (it != directory_list.end()) {
            std::string escaped_path = escape_arg(*it);
