...
std::optional<com::android::fastdeploy::APKMetaData> extract_metadata(const char* apkPath);
unique_fd install_patch(int argc, const char** argv);
unique_fd apply_patch_on_device(const char* outputPath);
int stream_patch(const char* apkPath, com::android::fastdeploy::APKMetaData metadata,
                 unique_fd patchFd);
