...
class AdbOspTaskRunner final : public openscreen::TaskRunner {
  ...
  private:
    std::optional<uint64_t> thread_id_;
    std::mutex mutex_;
    std::deque<std::pair<Task, openscreen::Clock::duration>> tasks_ GUARDED_BY(mutex_);
    std::deque<std::pair<Task, openscreen::Clock::duration>> running_tasks_;
