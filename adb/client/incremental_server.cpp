...
class IncrementalServer {
  ...
  private:
    ...
    bool Read(void* buffer, size_t* size, bool blocking);
...
bool IncrementalServer::Read(void* buffer, size_t* size, bool blocking) {
    ...
    if (!ReadFdExactly(adb_fd_, buffer, *size)) {
...
std::optional<RequestCommand> IncrementalServer::ReadRequest(bool blocking) {
    ...
    if (!Read(&commandBuf, &size, blocking)) {
        return {{EXIT}};
    }
