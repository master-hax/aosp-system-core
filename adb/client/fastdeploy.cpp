...
// https://stackoverflow.com/questions/14335494/how-to-deal-with-exit-time-destructor-warning-in-clang
static TFastDeployConfig& fastdeploy_config = *new TFastDeployConfig;
...
void fastdeploy_init(bool use_localagent, const std::string& adb_path) {
    memset(&fastdeploy_config, 0, sizeof(fastdeploy_config));
...
static bool patch_generator_command(std::string* output) {
    printf("patch_generator_command: config_initialized = %d\n", config_initialized);
