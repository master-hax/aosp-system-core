...
static bool get_agent_component_host_path(bool use_localagent, const char* adb_path,
    ...
    int path_len = strlen(adb_path);
    std::vector<char> adb_dir_buffer(path_len);
    strncpy(adb_dir_buffer.data(), adb_path, path_len);

    const char* adb_dir = dirname(adb_dir_buffer.data());
    if (adb_dir == nullptr) {
        return false;
    }
