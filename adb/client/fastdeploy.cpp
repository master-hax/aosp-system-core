...
static bool deploy_agent(bool check_time_stamps) {
    ...
    {
        TemporaryFile temp_agent;
        android::base::WriteFully(temp_agent.fd, kDeployAgent, sizeof(kDeployAgent));
        srcs.push_back(temp_agent.path);
        ...
        temp_agent.DoNotRemove();
    }
    FileDeleter temp_agent_remover(srcs.back());

    if (!do_sync_push(srcs, kDeviceAgentFile, check_time_stamps)) {
        error_exit("Failed to push fastdeploy agent to device.");
    }
    srcs.clear();
    {
        TemporaryFile temp_agent_script;
        android::base::WriteFully(temp_agent_script.fd, kDeployAgentScript,
                                  sizeof(kDeployAgentScript));
        srcs.push_back(temp_agent_script.path);

        temp_agent_script.DoNotRemove();
    }
    FileDeleter temp_agent_script_remover(srcs.back());

    if (!do_sync_push(srcs, kDeviceAgentScript, check_time_stamps)) {
...
int stream_patch(const char* apk_path, APKMetaData metadata, unique_fd patch_fd) {
    {
        TimeReporter reporter("create_patch");
    ...
    TimeReporter reporter("stream_patch");
