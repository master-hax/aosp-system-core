...
int get_device_api_level() {
    std::vector<unsigned char> sdkVersionOutputBuffer;
    std::vector<unsigned char> sdkVersionErrorBuffer;
    int statusCode;
    DeployAgentBufferCallback cb(&sdkVersionOutputBuffer, &sdkVersionErrorBuffer, &statusCode);
    int api_level = -1;

    int ret = send_shell_command("getprop ro.build.version.sdk", false, &cb);
    if (ret == 0 && statusCode == 0 && sdkVersionOutputBuffer.size() > 0) {
