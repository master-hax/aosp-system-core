...
static void help() {
    ...
    fprintf(stdout,
        ...
        "     -d: dry run: push files to device without storing to the filesystem\n"
...
int adb_commandline(int argc, const char** argv) {
    ...
    /* do_sync_*() commands */
        ...
        int opt;
        while ((opt = getopt(argc, const_cast<char**>(argv), "dlz:Z")) != -1) {
            switch (opt) {
                case 'd':
                    dry_run = true;
                    break;
                ...
                case 'Z':
                    ...
                    break;
                default:
                    error_exit("usage: adb sync [-d] [-l] [-z ALGORITHM] [-Z] [PARTITION]");
            }
        }
