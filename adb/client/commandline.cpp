...
static int adb_wipe_devices() {
    ...
    unique_fd fd(adb_connect(
            android::base::StringPrintf("rescue-wipe:%zu", WIPE_DEVICES_MESSAGE_SIZE), &error));
    if (fd < 0) {
...
int adb_commandline(int argc, const char** argv) {
    ...
    if (!strcmp(argv[0], "devices")) {
        ...
        // adb rescue wipe data
        if (argc != 3) error_exit("rescue requires three arguments");
        ...
        } else if (!strcmp(argv[1], "wipe") && !strcmp(argv[2], "data")) {
