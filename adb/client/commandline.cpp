...
static void help() {
    ...
    fprintf(stdout,
        ...
        "\n"
        "file transfer:\n"
        " push [--sync] [-Z] LOCAL... REMOTE\n"
        "     copy local files/directories to device\n"
        "     --sync: only push files that are newer on the host than the device\n"
        "     -Z: disable compression\n"
        " pull [-aZ] REMOTE... LOCAL\n"
        "     copy files/dirs from device\n"
        "     -a: preserve file timestamp and mode\n"
        "     -Z: disable compression\n"
        " sync [-lZ] [all|data|odm|oem|product|system|system_ext|vendor]\n"
...
int adb_commandline(int argc, const char** argv) {
    ...
    /* do_sync_*() commands */
        ...
        int opt;
        while ((opt = getopt(argc, const_cast<char**>(argv), "lZ")) != -1) {
            switch (opt) {
                case 'l':
                    list_only = true;
                    break;
                case 'Z':
                    ...
                    break;
                default:
                    error_exit("usage: adb sync [-lZ] [PARTITION]");
            }
        }
