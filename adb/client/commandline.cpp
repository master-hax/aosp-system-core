...
class TrackAppStreamsCallback : public DefaultStandardStreamsCallback {
  public:
    ...
    void OnStdout(const char* buffer, int length) override {
        ...
        oss << binary_proto.process_size() << " debuggable/profileable app process"
            << (binary_proto.process_size() > 1 ? "es are" : " is") << " running." << std::endl;
        OnStream(nullptr, stdout, oss.str().data(), oss.str().length());
