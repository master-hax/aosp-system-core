...
static int install_app_streamed(int argc, const char** argv, bool use_fastdeploy,
    ...
    if (!android::base::EndsWithIgnoreCase(file, ".apk") &&
        ...
        error(1, 0, "filename doesn't end with .apk or .apex: %s", file);
    }
    ...
    if (is_apex && !is_apex_supported()) {
        error(1, 0, ".apex is not supported on the target device");
    }

    if (is_apex && use_fastdeploy) {
        error(1, 0, "--fastdeploy doesn't support .apex files");
    }
...
static int install_app_legacy(int argc, const char** argv, bool use_fastdeploy,
    ...
    for (int i = argc - 1; i >= 0; i--) {
        if (android::base::EndsWithIgnoreCase(argv[i], ".apex")) {
            error(1, 0, "APEX packages are only compatible with Streamed Install");
        }
...
int install_multiple_app(int argc, const char** argv) {
    ...
    for (int i = argc - 1; i >= 0; i--) {
        ...
        if (android::base::EndsWithIgnoreCase(argv[i], ".apex")) {
            error(1, 0, "APEX packages are not compatible with install-multiple");
        }
