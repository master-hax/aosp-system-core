...
static int install_app_streamed(int argc, const char** argv, bool use_fastdeploy,
    ...
    if (use_fastdeploy == true) {
        ...
        const bool useAbb = can_use_feature(kFeatureAbb);
        std::string error;
        std::vector<std::string> cmd_args = {useAbb ? "package" : "exec:cmd package"};
        ...
        unique_fd remote_fd(useAbb ? send_abb_exec_command(cmd_args, &error)
                                   : adb_connect(android::base::Join(cmd_args, " "), &error));
        if (remote_fd < 0) {
