...
static unique_fd send_command(const std::vector<std::string>& cmd_args, bool use_abb_exec,
                              std::string* error) {
    if (use_abb_exec) {
...
static int install_app_streamed(int argc, const char** argv, bool use_fastdeploy) {
    ...
    unique_fd remote_fd = send_command(cmd_args, use_abb_exec, &error);
    if (remote_fd < 0) {
...
int install_multiple_app(int argc, const char** argv) {
    ...
    std::vector<std::string> cmd_args = {
            install_cmd, "install-create", "-S",
            android::base::StringPrintf("%" PRIu64, static_cast<uint64_t>(total_size))};
    ...
    char buf[BUFSIZ];
    {
        unique_fd fd = send_command(cmd_args, use_abb_exec, &error);
        if (fd < 0) {
...
finalize_session:
    ...
    {
        unique_fd fd = send_command(service_args, use_abb_exec, &error);
        if (fd < 0) {
