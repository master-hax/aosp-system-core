...
static std::pair<InstallMode, std::optional<InstallMode>> calculateInstallMode(
        InstallMode modeFromArgs, bool fastdeploy, std::optional<bool> incrementalRequested) {
    if (incrementalRequested.value_or(false)) {
        if (fastdeploy) {
            error_exit(
    ...
    if (incrementalRequested.value_or(true) && !is_abb_exec_supported()) {
        if (!incrementalRequested.has_value()) {
...
int install_app(int argc, const char** argv) {
    ...
    bool wait = false;
    std::optional<bool> incremental_requested = {};
    ...
    auto [primaryMode, fallbackMode] =
            calculateInstallMode(installMode, use_fastdeploy, incremental_requested);
    if (primaryMode == INSTALL_STREAM && best_install_mode() == INSTALL_PUSH) {
