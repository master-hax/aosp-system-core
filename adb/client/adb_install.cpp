...
static std::vector<const char*> parseFastDeployMode(
...
int install_app(int argc, const char** argv) {
    ...
    auto unused_argv = parse_install_mode({argv + 1, argv + argc}, &install_mode,
                                          &incremental_request, &incremental_wait);
    auto passthrough_argv =
            parseFastDeployMode(std::move(unused_argv), &use_fastdeploy, &agent_update_strategy);
