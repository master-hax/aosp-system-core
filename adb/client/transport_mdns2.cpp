...
#include "client/mdns_service_watcher.h"
...
void OnServiceReceiverResult(std::vector<std::reference_wrapper<const ServiceInfo>> infos,
    ...
    LOG(INFO) << "Endpoint state=" << static_cast<int>(state)
              ...
              << " addr=" << info.get().v4_address
              << " addrv6=" << info.get().v6_address
              << " total_serv=" << infos.size();

    switch (state) {
    case ServicesUpdatedState::EndpointCreated:
    case ServicesUpdatedState::EndpointUpdated:
        if (adb_DNSServiceShouldAutoConnect(info.get().service_name, info.get().instance_name)
                && info.get().v4_address) {
                ...
                LOG(INFO) << "Attempting to auto-connect to instance=" << info.get().instance_name
                          ...
                          << " addr4=%s" << info.get().v4_address << ":" << info.get().port;
                connect_device(
                        android::base::StringPrintf("%s.%s",
                            info.get().instance_name.c_str(),
                            info.get().service_name.c_str()), &response);
        }
        break;
    default:
        break;
...
//        }
        ...
        if (supported_address_families == discovery::Config::NetworkInfo::kNoAddressFamily) {
            LOG(INFO) << "Interface [" << interface << "] doesn't suppport ipv4 or ipv6."
                      << " Ignoring interface.";
            continue;
        }
    ...
    // TODO: NSEC optimizations don't work with the current MdnsResponder publisher. So disable it
    // until we migrate the publisher to openscreen code.
    // Disable NSEC record processing (b/168034815).
    config.ignore_nsec_responses = true;
...
void osp_cleanup() {
    ...
    // ShutDown() must be performed after destroying the serial delete pointer, as the deletion
    // happens on the task runner.
    if (PlatformClientPosix::GetInstance()) {
        if (PlatformClientPosix::GetInstance()->GetTaskRunner()->IsRunningOnTaskRunner()) {
            std::thread(PlatformClientPosix::ShutDown).detach();
        } else {
...
void StartDiscovery() {
    ...
    PlatformClientPosix::GetInstance()->GetTaskRunner()->PostTask([]() {
        ...
        // Will deadlock if on the TaskRunner thread.
        osp_cleanup();
...
void ForEachService(const std::unique_ptr<ServiceReceiver>& receiver,
                    std::string_view wanted_instance_name,
    ...
    for (const auto& s : services) {
        if (wanted_instance_name.empty() || s.get().instance_name == wanted_instance_name) {
            std::stringstream ss;
            ss << s.get().v4_address;
            cb(s.get().instance_name.c_str(),
               s.get().service_name.c_str(),
               ss.str().c_str(),
               s.get().port);
        }
    }
...
void init_mdns_transport_discovery(void) {
    // TODO: Enable via ADB_MDNS trace environment variable.
    openscreen::SetLogLevel(openscreen::LogLevel::kVerbose);
    // This will create a TaskRunner on a new thread
    PlatformClientPosix::Create(std::chrono::milliseconds(50), std::chrono::milliseconds(50));
    StartDiscovery();
}

bool adb_secure_connect_by_service_name(std::string_view instance_name) {
    if (g_using_bonjour) {
    ...
    auto cb = [&](const char* instance_name, const char* service_name, const char* ip_addr,
...
std::string mdns_check() {
    ...
    return "Openscreen discovery library";
}
