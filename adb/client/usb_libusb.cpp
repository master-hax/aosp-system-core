...
static bool check_adb_interface(const libusb_interface& interface, AdbInterface* adb_interface) {
    if (interface.num_altsetting != 1) {
        ...
        LOG(VERBOSE) << "skipping interface with unexpected num_altsetting";
        return false;
    }
    ...
    if (!is_adb_interface(interface_desc.bInterfaceClass, interface_desc.bInterfaceSubClass,
        ...
        LOG(VERBOSE) << "skipping non-adb interface";
        return false;
    }
    ...
    for (size_t endpoint_num = 0; endpoint_num < interface_desc.bNumEndpoints; ++endpoint_num) {
        ...
        if (transfer_type != LIBUSB_TRANSFER_TYPE_BULK) {
            LOG(VERBOSE) << "found non-bulk endpoint in interface";
            return false;
        }

        if (endpoint_is_output(endpoint_addr)) {
            if (found_out) {
                LOG(VERBOSE) << "found multiple bulk out endpoints in interface";
                return false;
            }
            ...
            if (found_in) {
                LOG(VERBOSE) << "found multiple bulk in endpoints in interface";
                return false;
            }
        ...
        if (adb_interface->packet_size == 0) {
            ...
            CHECK(adb_interface->packet_size == endpoint_packet_size);
        }
    }

    if (found_in && found_out) {
        LOG(VERBOSE) << "accepting potential adb interface";
        return true;
    }

    LOG(VERBOSE) << "rejecting potential adb interface: missing bulk endpoints "
                 << "(found_in = " << found_in << ", found_out = " << found_out << ")";
    return false;
}

static std::string get_device_serial(libusb_device_handle* device_handle) {
    std::string result;

    if (device_handle != nullptr) {
        libusb_device_descriptor device_desc;
        int rc = libusb_get_device_descriptor(libusb_get_device(device_handle), &device_desc);
        CHECK(rc == 0);
        ...
        rc = libusb_get_string_descriptor_ascii(device_handle, device_desc.iSerialNumber,
                                                ...
                                                result.length());
        if (rc == 0) {
            LOG(WARNING) << "received empty serial from device";
            return "unknown";
        } else if (rc < 0) {
            LOG(WARNING) << "failed to get serial from device" << libusb_error_name(rc);
            return std::string();
        }
        result.resize(rc);
        return result;
        ...
        if (!android::base::ReadFileToString(
                get_device_serial_path(libusb_get_device(device_handle)), &result)) {
...
static bool find_adb_interface(libusb_device* device, AdbInterface* adb_interface) {
    ...
    for (size_t interface_num = 0; interface_num < config->bNumInterfaces; ++interface_num) {
        LOG(VERBOSE) << "checking potential usb interface, device = " << device_address
...
static void process_device(libusb_device* device) {
    ...
    std::string device_serial = get_device_serial(handle.get());
