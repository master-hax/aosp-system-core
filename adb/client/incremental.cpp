...
static inline int32_t readInt32(borrowed_fd fd) {
    int32_t result;
...
static inline int32_t readBEInt32(borrowed_fd fd) {
    return int32_t(be32toh(readInt32(fd)));
}

static inline void appendBytesWithSize(borrowed_fd fd, std::vector<char>* bytes) {
    int32_t be_size = readInt32(fd);
...
static inline std::pair<std::vector<char>, int32_t> readIdSigHeaders(borrowed_fd fd) {
    std::vector<char> result;
    appendBytesWithSize(fd, &result);  // verityRootHash
    appendBytesWithSize(fd, &result);  // v3Digest
    appendBytesWithSize(fd, &result);  // pkcs7SignatureBlock
    auto tree_size = readBEInt32(fd);  // size of the verity tree
    return {std::move(result), tree_size};
}

static inline Size verityTreeSizeForFile(Size fileSize) {
    constexpr int INCFS_DATA_FILE_BLOCK_SIZE = 4096;
    constexpr int INCFS_MAX_MTREE_LEVELS = 8;
    ...
    Size tree_lvl_count[INCFS_MAX_MTREE_LEVELS] = {};
...
static std::pair<unique_fd, std::string> read_and_encode_signature(std::string signature_file) {
    ...
    auto [signature, tree_size] = readIdSigHeaders(fd);
    if (auto expected = verityTreeSizeForFile(st.st_size); tree_size != expected) {
        fprintf(stderr,
...
static unique_fd start_install(const std::vector<std::string>& files) {
    ...
    for (int i = 0, size = files.size(); i < size; ++i) {
        ...
        auto [signature_fd, signature] = read_and_encode_signature(file);
        if (!signature_fd.ok()) {
            return {};
        }
