...
static inline void appendBytes(borrowed_fd fd, std::vector<char>* bytes) {
    int32_t size = readBEInt32(fd);
    auto old_size = bytes->size();
    bytes->resize(old_size + size);
    ReadFully(fd, bytes->data() + old_size, size);
}

static inline std::pair<std::vector<char>, int32_t> readIdSigHeaders(borrowed_fd fd) {
    std::vector<char> result;
    appendBytes(fd, &result);          // verityRootHash
    appendBytes(fd, &result);          // v3Digest
    appendBytes(fd, &result);          // pkcs7SignatureBlock
