...
class ReconnectTest(DeviceTest):
    ...
    def test_reconnect_offline(self, args=['offline']):
        ...
        # TODO: adb kill-server should wait until the server exits?
        time.sleep(0.5)
        if self._get_device_state(serialno) != 'offline':
            print("This test is skipped because the device didn't go offline")
            return
        # 4. See if adb reconnect offline can bring the device online.
        subprocess.check_call(self.device.adb_cmd + ['reconnect'] + args)
        time.sleep(1)
        self.assertEqual(self._get_device_state(serialno), 'device')

    def test_reconnect(self):
        """Test `adb reconnect` on a offlined device."""
        self.test_reconnect_offline(args=[])
