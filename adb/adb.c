    ...
    case A_CLSE: /* CLOSE(local-id, remote-id, "") or CLOSE(0, remote-id, "") */
        if (t->online && p->msg.arg1 != 0) {
            if((s = find_local_socket(p->msg.arg1, p->msg.arg0))) {
                /* p->msg.arg0 might be 0 to indicate a failed OPEN. If this
                 * is the case, check that the socket isn't connected yet. */
                if (p->msg.arg0 == 0 && s->peer != NULL) {
                    D("Invalid A_CLSE(0, %u) from transport %s\n",
                      p->msg.arg1, t->serial);
                } else {
                    s->close(s);
                }
