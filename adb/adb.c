...
static int install_listener(const char *local_name, const char *connect_to, atransport* transport, int no_rebind)
{
    alistener *l;
...
int handle_host_request(char *service, transport_type ttype, char* serial, int reply_fd, asocket *s)
{
    ...
    if(!strncmp(service,"forward:",8) ||
       !strncmp(service,"killforward:",12)) {
        ...
        if (createForward) {
            sendfailmsg(reply_fd, (r == -1) ? "cannot rebind socket" : "cannot bind socket");
        } else {
