...
static bool patch_generator_command(bool use_localagent, const char* adb_path, std::string* output) {
    if (use_localagent) {
        //        const char* kGeneratorCommandPattern =
        //            "java -Xbootclasspath/a:%s/framework/host-libprotobuf-java-lite.jar -jar
        //            %s/framework/deploypatchgenerator.jar";
...
int create_patch(const char* apkPath, const char* metadataPath, const char* patchPath,
    ...
    std::string generatePatchCommand = android::base::StringPrintf(
        kGeneratePatchCommandPattern, patch_generator_command_string.c_str(), apkPath, metadataPath,
        patchPath);
...
int install_patch(const char* apkPath, const char* patchPath) {
    const std::string kAgentApplyCommandPattern = "/data/local/tmp/deployagent.sh apply %s %s";
