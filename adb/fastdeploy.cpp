...
static bool get_packagename_from_apk(const char* apkPath, std::string& output) {
    ...
    if (zipFile->getEntryInfo(entry, NULL, &manifest_len, NULL, NULL, NULL, NULL) == false) {
        return false;
    }

    void* manifest_data = malloc(manifest_len);
    if (zipFile->uncompressEntry(entry, manifest_data, manifest_len) == false) {
        free(manifest_data);
        return false;
    }

    ResXMLTree tree;
    if (tree.setTo(manifest_data, manifest_len) != NO_ERROR) {
        free(manifest_data);
        return false;
    }
    ...
    while ((code = tree.next()) != ResXMLParser::BAD_DOCUMENT &&
        ...
        switch (code) {
            case ResXMLParser::START_TAG: {
                ...
                std::string element_name_string =
                    get_string_from_utf16(element_name, element_name_length);
                if (element_name_string.compare("manifest") == 0) {
                    for (int i = 0; i < (int)tree.getAttributeCount(); i++) {
                        ...
                        std::string attribute_name_string =
                            get_string_from_utf16(attribute_name_text, attribute_name_length);

                        if (attribute_name_string.compare("package") == 0) {
