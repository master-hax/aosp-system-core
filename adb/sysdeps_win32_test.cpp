...
TEST(sysdeps_win32, unix_isatty) {
    ...
    for (auto flags : {O_RDONLY, O_RDWR}) {
        int temp_fd = unix_open(temp_file_name, flags | O_CREAT, S_IWUSR);
        EXPECT_TRUE(temp_fd >= 0);
        EXPECT_EQ(0, unix_isatty(temp_fd));
        EXPECT_EQ(0, unix_close(temp_fd));
    }
    EXPECT_EQ(0, unlink(temp_file_name));
