...
static int local_socket_enqueue(asocket* s, std::vector<char> data) {
...
static void local_socket_event_func(int fd, unsigned ev, void* _s) {
    ...
    */
    ...
    if (ev & FDE_READ) {
        ...
        std::vector<char> data(max_payload);
        char* x = data.data();
...
static int remote_socket_enqueue(asocket* s, std::vector<char> data) {
    ...
    // TODO: Convert apacket::data to a vector that we can move into.
...
static int smart_socket_enqueue(asocket* s, std::vector<char> data) {
#if ADB_HOST
...
#if ADB_HOST
    service = s->smart_socket_data.data() + 4;
