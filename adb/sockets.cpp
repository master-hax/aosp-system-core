...
void install_local_socket(asocket* s) {
    ...
    if (local_socket_next_id == 0) local_socket_next_id = 1;
...
asocket* create_local_service_socket(const char* name) {
    ...
    if (!strncmp(name, "root:", 5)) property_get("ro.debuggable", debug, "");

    if ((!strncmp(name, "root:", 5) && getuid() != 0 &&
         strcmp(debug, "1") == 0) ||
        (!strncmp(name, "unroot:", 7) && getuid() == 0) ||
        !strncmp(name, "usb:", 4) || !strncmp(name, "tcpip:", 6)) {
...
#if ADB_HOST
static asocket* create_host_service_socket(const char* name,
                                           const char* serial) {
    asocket* s;
...
static const struct prefix_struct {
    ...
    PREFIX("usb:"), PREFIX("product:"), PREFIX("model:"), PREFIX("device:"),
};
...
static int smart_socket_enqueue(asocket* s, apacket* p) {
    ...
    if (service) {
        ...
        ** is clean up.
        */
        if (handle_host_request(service, type, serial ? serial : "",
                                s->peer->fd, s) == 0) {
            /* XXX fail message? */
    ...
    if (s->transport == NULL) {
        ...
        s->transport =
            acquire_one_transport(CS_ANY, kTransportAny, NULL, &error_msg);
