    ...
    if (use_fastdeploy == true) {
        ...
        if (metadata_len <= 0) {
            ...
            if (create_patch_result != 0) {
                ...
                int apply_patch_result = install_patch(file, patchTmpPath.c_str());
...
int install_app(int argc, const char** argv) {
    ...
    for (int i = 1; i < argc; i++) {
        if (!strcmp(argv[i], "--streaming")) {
        ...
        } else if (!strcmp(argv[i], "--nostreaming")) {
            processedArgIndicies.push_back(i);
            ...
            processedArgIndicies.push_back(i);
            use_fastdeploy = true;
        } else if (!strcmp(argv[i], "--nofastdeploy")) {
            processedArgIndicies.push_back(i);
            use_fastdeploy = false;
        } else if (!strcmp(argv[i], "-f")) {
            processedArgIndicies.push_back(i);
            use_fastdeploy = true;
        } else if (!strcmp(argv[i], "--forceagent")) {
            processedArgIndicies.push_back(i);
            agent_update_strategy = FastDeploy_AgentUpdateAlways;
        } else if (!strcmp(argv[i], "--datecheckagent")) {
            processedArgIndicies.push_back(i);
            agent_update_strategy = FastDeploy_AgentUpdateNewerTimeStamp;
        } else if (!strcmp(argv[i], "--versioncheckagent")) {
            processedArgIndicies.push_back(i);
            agent_update_strategy = FastDeploy_AgentUpdateDifferentVersion;
        } else if (!strcmp(argv[i], "--uselocalagentbuild")) {
            processedArgIndicies.push_back(i);
            use_localagent = true;
        }
    ...
    std::vector<const char*> pm_argv;
    for (int i = 0; i < argc; i++) {
        if (std::find(processedArgIndicies.begin(), processedArgIndicies.end(), i) ==
            processedArgIndicies.end()) {
            pm_argv.push_back(argv[i]);
        }
    }

    if (use_fastdeploy == true) {
        bool agent_up_to_date = update_agent(agent_update_strategy, use_localagent, argv[0]);
    ...
    switch (installMode) {
        case INSTALL_PUSH:
            return install_app_legacy(pm_argv.size(), pm_argv.data(), use_fastdeploy,
                                      use_localagent, argv[0]);
        case INSTALL_STREAM:
            return install_app_streamed(pm_argv.size(), pm_argv.data(), use_fastdeploy,
                                        use_localagent, argv[0]);
