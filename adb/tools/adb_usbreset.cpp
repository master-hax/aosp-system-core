...
static void PrintLibusbError(int err, const char* prefix = "") {
    printf("%s%s", prefix, libusb_strerror(static_cast<libusb_error>(err)));
}
...
int main(int argc, char** argv) {
    ...
    if (std::get_if<std::monostate>(&selection)) {
        ...
        if (env) {
            ...
            fprintf(stderr, "usbreset: no device specified\n");
            Usage(1);
        }
    }
    ...
    if (libusb_init(&ctx) != LIBUSB_SUCCESS) {
        errx(1, "failed to initialize libusb");
    }
    ...
    for (int i = 0; i < device_count; ++i) {
        ...
        if (rc != 0) {
            PrintLibusbError(rc, "failed to get config descriptor: ");
            continue;
        }
        ...
        if (found_adb_interface) {
            ...
            if (rc != 0) {
                PrintLibusbError(rc, "failed to open device: ");
                continue;
            }
            ...
            if (rc < 0) {
                PrintLibusbError(rc, "failed to get device serial: ");
                continue;
            }
    ...
    for (auto& [serial, device_handle] : selected_devices) {
        int rc = libusb_reset_device(device_handle);
