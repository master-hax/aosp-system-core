...
void BM_Connection_PingPong(benchmark::State& state) {
    int fds[2];
    if (adb_socketpair(fds) != 0) {
        LOG(FATAL) << "failed to create socketpair";
    }
...
ADB_CONNECTION_BENCHMARK(BM_Connection_PingPong, ThreadPolicy::SameThread);
ADB_CONNECTION_BENCHMARK(BM_Connection_PingPong, ThreadPolicy::MainThread);
