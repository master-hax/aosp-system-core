...
public final class ApkArchive {
    ...
    private static final int LFH_OFFSET_TO_COMPRESSED_SIZE = 18;

    /**
     * Copy the compressed file content from apk into output.
     *
     * @param apk                   source APK channel
     * @param localFileHeaderOffset offset to Local File Header in the apk
     * @param output                where the file content will be written to
     * @return amount of data written aka compressed size of the file
     */
    public static long copyFileData(FileChannel apk, long localFileHeaderOffset,
            WritableByteChannel output) throws IOException {
        ByteBuffer lfhBuffer = ByteBuffer.allocate(LFH_SIZE).order(
                ByteOrder.LITTLE_ENDIAN);
        apk.read(lfhBuffer, localFileHeaderOffset);
        lfhBuffer.rewind();

        int signature = lfhBuffer.getInt();
        if (signature != LFH_SIGNATURE) {
            throw new IllegalArgumentException(
                    "Not a Local File header. Signature: 0x"
                            + Long.toHexString(signature & 0xffffffffL));
        }

        lfhBuffer.position(LFH_OFFSET_TO_COMPRESSED_SIZE);
        long compressedSize = lfhBuffer.getInt();
        /*long uncompressedSize = */
        lfhBuffer.getInt();
        long fileNameLength = lfhBuffer.getShort();
        long extraFieldLength = lfhBuffer.getShort();
        apk.transferTo(localFileHeaderOffset + LFH_SIZE + fileNameLength + extraFieldLength,
                compressedSize, output);
        return compressedSize;
    }

    final RandomAccessFile mFile;
    ...
    private Location findCDRecord(ByteBuffer buf) {
        ...
        if (buf.remaining() < CD_ENTRY_HEADER_SIZE_BYTES) {
            Log.e(TAG, "Input too short. Need at least " + CD_ENTRY_HEADER_SIZE_BYTES
    ...
    Location getCDLocation() throws IOException {
        ...
        if (eocdRecord < 0) {
            Log.e(TAG, "Unable to find End of Central Directory record.");
            return null;
        }
        ...
        if (location == null) {
            Log.e(TAG, "Unable to find Central Directory File Header.");
            return null;
        }
