...
size_t ApkArchive::CalculateLocalFileEntrySize(int64_t localFileHeaderOffset) const {
    // The local file header for a given entry. This duplicates information
    // present in the central directory of the archive. It is an error for
    // the information here to be different from the central directory
    // information for a given entry.
    static constexpr int kLFHFileHeaderMagic = 0x04034b50;
    ...
    if (lfh->lfh_signature != kLFHFileHeaderMagic) {
        fprintf(stderr, "Invalid Local File Header signature in file '%s' at offset %lld\n",
                path_.c_str(), (long long)localFileHeaderOffset);
        return {};
    }
    return sizeof(*lfh) + lfh->file_name_length + lfh->extra_field_length + lfh->compressed_size;
}
