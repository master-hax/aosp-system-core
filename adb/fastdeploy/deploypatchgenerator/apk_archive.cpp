...
off_t ApkArchive::FindEndOfCDRecord() const {
    ...
    auto mapped =
            MappedFile::FromOsHandle(adb_get_os_handle(fd_), readOffset, sizeToRead, PROT_READ);
...
ApkArchive::Location ApkArchive::GetCDLocation() {
    ...
    auto mapped = MappedFile::FromOsHandle(adb_get_os_handle(fd_), eocdRecord,
                                           cdEntryHeaderSizeBytes, PROT_READ);
...
ApkArchive::Location ApkArchive::GetSignatureLocation(off_t cdRecordOffset) {
    ...
    auto mapped = MappedFile::FromOsHandle(adb_get_os_handle(fd_), signatureOffset,
                                           endOfSignatureSize, PROT_READ);
...
std::string ApkArchive::ReadMetadata(Location loc) const {
    auto mapped = MappedFile::FromOsHandle(adb_get_os_handle(fd_), loc.offset, loc.size, PROT_READ);
...
size_t ApkArchive::CalculateLocalFileEntrySize(int64_t localFileHeaderOffset,
    ...
    auto lfhMapped = MappedFile::FromOsHandle(adb_get_os_handle(fd_), localFileHeaderOffset,
                                              sizeof(LocalFileHeader), PROT_READ);
    ...
    if (lfh->gpb_flags & kGPBDDFlagMask) {
        ...
        auto ddMapped =
                MappedFile::FromOsHandle(adb_get_os_handle(fd_), ddOffset,
                                         sizeof(uint32_t) + sizeof(DataDescriptor), PROT_READ);
