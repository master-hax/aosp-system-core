...
void DeployPatchGenerator::ReportSavings(const std::vector<SimpleEntry>& identicalEntries,
    ...
    float savingPercent = (float)(totalEqualBytes * 100.0f) / totalSize;
    fprintf(stderr, "Detected %d equal APK entries\n", totalEqualFiles);
    fprintf(stderr, "%ld bytes are equal out of %" PRIu64 " (%.2f%%)\n", totalEqualBytes, totalSize,
            savingPercent);
}

void DeployPatchGenerator::GeneratePatch(std::vector<SimpleEntry>& entriesToUseOnDevice,
                                         const char* localApkPath, borrowed_fd output) {
    android::base::unique_fd input(adb_open(localApkPath, O_RDONLY | O_CLOEXEC));
    size_t newApkSize = adb_lseek(input, 0L, SEEK_END);
    adb_lseek(input, 0L, SEEK_SET);
    std::sort(entriesToUseOnDevice.begin(), entriesToUseOnDevice.end(),
...
int DeployPatchGenerator::CreatePatch(const char* localApkPath, const char* deviceApkMetadataPath,
                                      borrowed_fd output) {
    std::string content;
