...
class DeployPatchGenerator {
  public:
     ...
     * the console.
     */
    DeployPatchGenerator(bool is_verbose) : is_verbose_(is_verbose) {}
    /**
     * Given a |localApkPath|, and the |deviceApkMetadataPath| from an installed APK this function
     * writes a patch to the given |output|.
     */
    int CreatePatch(const char* localApkPath, const char* deviceApkMetadataPath,
  ...
  private:
     ...
     */
    void GeneratePatch(std::vector<SimpleEntry>& entriesToUseOnDevice, const char* localApkPath,
                       android::base::borrowed_fd output);
