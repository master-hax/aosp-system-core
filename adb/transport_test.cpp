...
TEST(transport, disconnect_smoke) {
    atransport t;
...
static void RemoveDisconnectFunc(void* arg, atransport* t) {
    adisconnect* disconnect = reinterpret_cast<adisconnect*>(arg);
    t->RemoveDisconnect(disconnect);
}

TEST(transport, remove_disconnect_in_callback) {
    atransport t;
    adisconnect disconnect;
    disconnect.func = RemoveDisconnectFunc;
    disconnect.opaque = &disconnect;
    t.AddDisconnect(&disconnect);
    t.RunDisconnects();
}
