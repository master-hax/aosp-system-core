...
bool PairingClient::handleMsg(std::string_view msg,
    ...
    switch(dataType) {
        case DataType::PublicKey: {
            ...
            keystore_public_key_header(keystore_ctx.get(), &public_key_header);
            std::vector<char> public_key(keystore_max_certificate_size(keystore_ctx.get()));
            uint32_t public_key_sz = keystore_system_public_key(keystore_ctx.get(), public_key.data());
            if (public_key_sz == 0) {
                LOG(ERROR) << "Unable to retrieve the system's public certificate.";
                return false;
            }
            // The key may not be null-terminated. Ensure that it is.
            public_key.resize(public_key_sz + 1);
            public_key[public_key_sz] = '\0';
        ...
        case DataType::PairingRequest: {
            ...
            // The data may not be null-terminated. Ensure that it is.
            public_key.resize(header.payload + 1);
            public_key[header.payload] = '\0';

            valid = keystore_store_public_key(keystore_ctx.get(),
                                              &header,
                                              public_key.data());
            if (!valid) {
