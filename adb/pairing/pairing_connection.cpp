    ...
    while (true) {
        switch (mState) {
            case State::ExchangingKeys:
                ...
                if (tryReadPublicKey(theirKey)) {
                    LOG(INFO) << "Got public key";
                    mState = State::ExchangingHeaders;
                    mDataCallback(std::string_view(reinterpret_cast<const char*>(theirKey.data()),
                                                   theirKey.size()),
                                  DataType::PublicKey,
                                  mOpaque);
                    continue;
