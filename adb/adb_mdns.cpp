...
std::unordered_set<int>* g_autoconn_allowedlist = nullptr;
...
void config_auto_connect_services() {
    ...
    g_autoconn_allowedlist = new std::unordered_set<int>();
    ...
    std::string res;
    std::for_each(g_autoconn_allowedlist->begin(), g_autoconn_allowedlist->end(),
            [&](const int& i) {
...
bool adb_DNSServiceShouldAutoConnect(std::string_view reg_type, std::string_view service_name) {
    ...
    if (!index || (index != kADBTransportServiceRefIndex
