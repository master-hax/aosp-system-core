...
static bool handle_send_file(int s, const char* path, uid_t uid,
    ...
    while (true) {
        ...
        if (!ReadFdExactly(s, &buffer[0], len)) goto abort;
...
fail:
    // Keep reading packets until we get an ID_DONE.
    while (true) {
        unsigned int len;
        if (!ReadFdExactly(s, &msg.data, sizeof(msg.data))) goto fail;
        ...
        len = msg.data.size;
        if (len > buffer.size()) {
            D("handle_send_fail received oversized packet of length '%u' during failure", len);
            goto abort;
        }
