...
static int handle_send_file(int s, char* path, uid_t uid, gid_t gid,
    ...
    if (fd < 0 && errno == ENOENT) {
        if (mkdirs(path) != 0) {
            ...
            fd = adb_open_mode(path, O_WRONLY | O_CREAT | O_EXCL | O_CLOEXEC,
                               mode);
        }
    }
    ...
    for (;;) {
        ...
        if (!ReadFdExactly(s, buffer, len)) goto fail;
    ...
    if (fd >= 0) {
        ...
        msg.status.msglen = 0;
        if (!WriteFdExactly(s, &msg.status, sizeof(msg.status))) return -1;
    }
    return 0;
...
#if defined(_WIN32)
extern int handle_send_link(int s, char* path, char* buffer)
...
static int handle_send_link(int s, char* path, char* buffer) {
    ...
    if (!ReadFdExactly(s, &msg.data, sizeof(msg.data))) return -1;
    ...
    if (!ReadFdExactly(s, buffer, len)) return -1;
...
static int do_send(int s, char* path, char* buffer) {
    ...
    if (!tmp || errno) {
        ...
        do_unlink =
            lstat(path, &st) || S_ISREG(st.st_mode) || S_ISLNK(st.st_mode);
        if (do_unlink) {
            adb_unlink(path);
        }
    }
