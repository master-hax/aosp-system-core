...
static void update_capabilities(const char* path, uint64_t capabilities) {
    if (capabilities == 0) {
    ...
    // Not all filesystems support extended attributes, so we don't check for success.
    setxattr(path, XATTR_NAME_CAPS, &cap_data, sizeof(cap_data), 0);
}
...
static bool secure_mkdirs(const std::string& path) {
    ...
    for (const auto& path_component : path_components) {
        ...
        if (adb_mkdir(partial_path.c_str(), mode) == -1) {
            ...
            update_capabilities(partial_path.c_str(), capabilities);
        }
    }
    return true;
}
...
static bool handle_send_file(int s, const char* path, uid_t uid, gid_t gid, uint64_t capabilities,
    ...
    if (fd < 0) {
        ...
        update_capabilities(path, capabilities);
    }
