...
static void kick_disconnected_devices() {
    ...
    adb_mutex_lock(&usb_lock);
    // kick any devices in the device list that were not found in the device
    // scan
    for (usb = handle_list.next; usb != &handle_list; usb = usb->next) {
        if (usb->mark == 0) {
            usb_kick(usb);
...
static void find_usb_device(
    ...
    while ((de = readdir(busdir)) != 0) {
        ...
        while ((de = readdir(devdir))) {
            ...
            // should have device and configuration descriptors, and atleast two
            // endpoints
            ...
            while (bufptr < bufend) {
                ...
                if (type == USB_DT_INTERFACE) {
                    ...
                    if (interface->bNumEndpoints == 2 &&
                        ...
                        // we have a match.  now we just need to figure out
                        ...
                        register_device_callback(
                            devname, devpath, local_ep_in, local_ep_out,
...
static void register_device(const char* dev_name, const char* dev_path,
    ...
    */
    ...
    D("[ usb located new device %s (%d/%d/%d) ]\n", dev_name, ep_in, ep_out,
      interface);
    usb_handle* usb =
        reinterpret_cast<usb_handle*>(calloc(1, sizeof(usb_handle)));
