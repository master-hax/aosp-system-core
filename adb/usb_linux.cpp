...
// Returns a message describing what we think is the user's specific problem, or nullptr if we
// can't find anything useful.
static const char* GetPermissionProblem() {
    errno = 0;
    ...
    return "verify plugdev group membership";
}
...
std::string UsbNoPermissionsShortHelpText() {
    ...
    const char* problem = GetPermissionProblem();
    if (problem != nullptr) {
        help_text = android::base::StringPrintf("%s (%s)", help_text.c_str(), problem);
    }
...
std::string UsbNoPermissionsLongHelpText() {
    ...
    return android::base::StringPrintf(
            "%s.\nudev may be blocking access to this device, see [%s] for more information.",
