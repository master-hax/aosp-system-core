...
{
    std::unique_ptr<DIR, int(*)(DIR*)> bus_dir(opendir(base), closedir);
    if (!bus_dir) return;
    ...
    while ((de = readdir(bus_dir.get())) != 0) {
        ...
        std::string bus_name = android::base::StringPrintf("%s/%s", base, de->d_name);
...
static int usb_bulk_write(usb_handle* h, const void* data, int len) {
    ...
    while (true) {
        ...
        if (h->urb_out_busy == 0 && urb->status == 0) {
...
static int usb_bulk_read(usb_handle *h, void *data, int len)
{
    ...
    do {
        res = ioctl(h->fd, USBDEVFS_SUBMITURB, urb);
    ...
    for(;;) {
        ...
        int fd = h->fd;
        h->mutex.unlock();
        res = ioctl(fd, USBDEVFS_REAPURB, &out);
...
static void register_device(const char* dev_name, const char* dev_path,
    ...
    usb->writeable = 1;
    usb->urb_in_busy = usb->urb_out_busy = 0;
    usb->dead = false;
