...
check_endpoint(usb_handle *uh, int ep, int mode)
{
        ...
        if (uh->ep[index] < 0) {
                ...
                if (fd < 0)
                    D("can't open endpoint %s for bulk operations: %s",
                        epbuf, strerror(errno));
...
usb_bulk_write(usb_handle *uh, const void *data, int len)
{
        ...
        if (fd < 0) {
                D("failed to open endpoint %d for writing: %s\n",
                    UE_GET_ADDR(ep_addr), strerror(errno));
                return (-1);
        }
        ...
        if (ret < 0) {
                D("failed to set timeout: %s", strerror(errno));
        ...
        if (ret < 0)
                D("failed to write to bulk endpoint %s.%d: %s",
                    uh->devname, UE_GET_ADDR(ep_addr), strerror(errno));
...
usb_bulk_read(usb_handle *uh, void *data, int len)
{
        ...
        if (fd < 0) {
                D("failed to open endpoint %d for reading: %s\n",
                    UE_GET_ADDR(ep_addr), strerror(errno));
                return (-1);
        }
        ...
        if (ret < 0) {
                D("failed to set short transfer: %s", strerror(errno));
                /* continue */
        }
        ...
        if (ret < 0) {
                D("failed to read from bulk endpoint %s.%d: %s",
                    uh->devname, UE_GET_ADDR(ep_addr), strerror(errno));
        }
    ...
    int cindex, int iindex, int aindex, usb_handle *uh)
{
        ...
        if (ioctl(f, USB_GET_INTERFACE_DESC, &idesc) <0) {
                // D("ioctl USB_GET_INTERFACE_DESC failed: %s\n",
                //  strerror(errno));
                return (-1);
        }

        if (idesc.uid_desc.bNumEndpoints != 2)  {
                // D("Interface has not 2 endpoints, ignoring\n");
                return (-1);
        }
        ...
        for (e = 0; e < idesc.uid_desc.bNumEndpoints; e++) {
                ...
                if (ioctl(f, USB_GET_ENDPOINT_DESC, &edesc) < 0) {
                        // D("ioctl USB_GET_ENDPOINT_DESC failed: %s\n",
                        //  strerror(errno));
                        return (-1);
                }
            
                if (edesc.ued_desc.bmAttributes != UE_BULK) {
                        // D("endpoint is not bulk, ignoring\n");
                        return (0);
                }
...
fail:
        D("[ usb open %s error=%d, err_str = %s]\n",
            usb->devname,  errno,  strerror(errno));
...
check_usb_device_attach(struct usb_device_info *info)
{
        ...
        D("probing usb device %s [V: %04x P: %04x (%s %s rel. %s]\n",
            devname,
        ...
        if (fd < 0) {
                D("failed to open device for reading: %s", strerror(errno));
                return;
        }
        ...
        if (ioctl(fd, USB_GET_CONFIG_DESC, &cdesc) < 0) {
                D("failed to get configuration for device: %s", strerror(errno));
                return;
        }
        ...
        if (found == 1) {
                ...
                if (ioctl(fd, USB_GET_DEVICE_DESC, &ddesc) < 0) {
                        D("failed to get device descriptor: %s\n",  strerror(errno));
                        return;
                }
...
device_poll_thread(void* unused)
{
        ...
        if (fd == -1) {
                D("failed to open /dev/usb to fetch events");
                return (NULL);
        }
        ...
        for(;;) {
                if (poll_usb_events(fd) > 0) {
                        ...
                        fatal_errno("failed to poll usb event");
                        break;
                }
        }
