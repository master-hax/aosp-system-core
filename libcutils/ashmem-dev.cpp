...
/* For debugging: set to 1 for verbose debug logging */
 ...
 */
static std::atomic<int> memfd_supported = -1;
...
static int memfd_create_region(const char *name, size_t size)
{
    ...
    if (fd < 0) {
        ALOGE("memfd: create: name: %s, size: %zd, error fd %d, errno %d\n",
              name, size, fd, errno);
        goto error;
    }

   if (ftruncate(fd, size) == -1) {
        ALOGE("memfd: create: truncate error. name: %s, size: %zd, errno: %d\n",
              name, size, errno);
        goto error;
    }
