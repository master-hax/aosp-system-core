 ...
 *
 * Previously this function checked if memfd is suppported by checking if
...
static bool check_vendor_memfd_allowed() {
    std::string is_treblelized = android::base::GetProperty("ro.treble.enabled", "");

    return is_treblelized == "true";
}
