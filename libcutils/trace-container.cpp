...
#define WRITE_MSG_IN_CONTAINER_LOCKED(ph, sep_before_name, value_format, trackName, name, value) { \
    char buf[CONTAINER_ATRACE_MESSAGE_LENGTH]; \
    const char* trackName_sep = strlen(trackName) > 0 ? "|" : ""; \
    int pid = getpid(); \
    int tid = gettid(); \
    ...
    int len = snprintf( \
            ...
            pid, tid, ts, tts, trackName, trackName_sep, name, value); \
    if (len >= (int) sizeof(buf)) { \
        ...
        if (name_len > 0) { \
            ...
            len = snprintf( \
                    buf, sizeof(buf), \
                    ph "|%d|%d|%" PRIu64 "|%" PRIu64 sep_before_name "%s%s%.*s" value_format, \
                    pid, tid, ts, tts, trackName, trackName_sep, name_len, name, value); \
        } \
        else { \
...
#define WRITE_MSG_IN_CONTAINER(ph, sep_before_name, value_format, trackName, name, value) { \
    pthread_rwlock_rdlock(&atrace_container_sock_rwlock); \
    if (atrace_container_sock_fd != -1) { \
       WRITE_MSG_IN_CONTAINER_LOCKED(ph, sep_before_name, value_format, trackName, name, value); \
    } \
    pthread_rwlock_unlock(&atrace_container_sock_rwlock); \
}
