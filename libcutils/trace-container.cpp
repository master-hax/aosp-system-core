...
#define WRITE_MSG_IN_CONTAINER_LOCKED(ph, sep_before_name, value_format, track_name, name, value) { \
    ...
    if (len >= (int) sizeof(buf)) { \
        ...
        if (name_len > 0) { \
            ...
            int track_name_len = track_name[0] == '\0' ? 0 : strlen(track_name) - (len - strlen(name) - sizeof(buf)) - 2; \
            if (track_name_len <= 0){ \
