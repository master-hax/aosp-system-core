...
#if defined(__BIONIC__) && !defined(__ANDROID_RECOVERY__)
 ...
 **/
extern void atrace_seq_number_changed(uint32_t prev_seq_no, uint32_t seq_no);
...
#if defined(__BIONIC__) && !defined(__ANDROID_RECOVERY__)
    uint32_t seq_no = __system_property_serial(atrace_property_info);
#else
    uint32_t seq_no = 0;
#endif
