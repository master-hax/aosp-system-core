...
class TraceDevTest : public ::testing::Test {
 protected:
  void SetUp() override {
    tmp_file_.reset(new TemporaryFile);
    atrace_marker_fd = tmp_file_->fd;
  }

  void TearDown() override {
    tmp_file_.reset(nullptr);
    atrace_marker_fd = -1;
  }

  std::unique_ptr<TemporaryFile> tmp_file_;
};
...
TEST_F(TraceDevTest, atrace_begin_body_exact) {
  ...
  std::string name;
  for (size_t i = 0; i < ATRACE_MESSAGE_LENGTH - expected.length() - 1; i++) {
    name += '0' + (i % 10);
  }
...
TEST_F(TraceDevTest, atrace_begin_body_truncated) {
  std::string expected = android::base::StringPrintf("B|%d|", getpid());
  std::string name;
  for (size_t i = 0; i < 2 * ATRACE_MESSAGE_LENGTH; i++) {
    name += '0' + (i % 10);
  }
  atrace_begin_body(name.c_str());
...
TEST_F(TraceDevTest, atrace_async_begin_body_exact) {
  ...
  std::string name;
  for (size_t i = 0; i < ATRACE_MESSAGE_LENGTH - expected.length() - 7; i++) {
    name += '0' + (i % 10);
  }
...
TEST_F(TraceDevTest, atrace_int64_body_exact) {
  std::string expected = android::base::StringPrintf("C|%d|", getpid());
  std::string name;
  for (size_t i = 0; i < ATRACE_MESSAGE_LENGTH - expected.length() - 13; i++) {
    name += '0' + (i % 10);
  }
