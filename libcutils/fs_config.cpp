...
static bool fs_config_cmp(bool dir, const char* prefix, size_t len, const char* path,
...
#else
    if (PathMatchSpecA(input, pattern)) return true;
#endif
    ...
    const bool input_is_partition = is_partition(input, strlen(input));
#if !defined(_WIN32)
    return input_is_partition && fnmatch(pattern, input, fnm_flags) == 0;
#else
    return input_is_partition && PathMatchSpecA(input, pattern);
#endif
}
#ifndef __ANDROID_VNDK__
