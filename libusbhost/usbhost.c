...
/* returns the ucs-2 encoded string descriptor in a malloc()'d buffer.
 *
 ...
 *  ucs2_out - must be null on call. Will be filled in with a buffer on success.
 *           if this is non-null on return, it must be free()d.
 *           NOT GUARANTEED NULL TERMINATED.
 *  buffer_size - size of ucs-2 string in ucs2_out.
 */
int usb_device_get_string_ucs2(struct usb_device* device, int id, int timeout, void** ucs2_out,
    ...
    char buffer[MAX_STRING_DESCRIPTOR_LENGTH];
    int i, result;
    int languageCount = 0;
    ...
    for (i = 1; i <= languageCount; i++) {
        ...
        result = usb_device_control_transfer(device,
                ...
                timeout);
        if (result >= 2) {  // a result of 1 is illegal. first entry is a 2-byte int.
            int descriptor_len = result - 2;
            char* out = malloc(descriptor_len + 1);  // guaranteed at least 1.
            if (out == NULL) {
                return -1;
            }
            memcpy(out, (char*)(buffer + 2), descriptor_len);
            out[descriptor_len] = '\0';  // guaranteed at least zero;
            *ucs2_out = (void*)out;
            *buffer_size = descriptor_len;
            return 0;
        }
    }
