...
static void watch_existing_subdirs(struct usb_host_context *context,
                                   int *wdd, int *wds, int wd_count)
{
    char path[100];
    int i, ret;

    *wdd = inotify_add_watch(context->fd, USB_FS_DIR, IN_CREATE | IN_DELETE);
    if (*wdd < 0)
        return;
                  ...
                  void *client_data)
{
    ...
    memset(context->wds, 0, sizeof(context->wds));
    /* watch the root for new subdirectories */
    context->wdd = inotify_add_watch(context->fd, USB_FS_DIR, IN_CREATE | IN_DELETE);
    if (context->wdd < 0) {
    ...
    watch_existing_subdirs(context, &context->wdd, context->wds, MAX_USBFS_WD_COUNT);
...
int usb_host_read_event(struct usb_host_context *context)
{
    ...
    if (ret >= (int)sizeof(struct inotify_event)) {
        ...
        if (wd == context->wdd) {
            ...
            for (i = 0; (i < MAX_USBFS_WD_COUNT) && !done; i++) {
                if (wd == context->wds[i]) {
                    j = 0;
                    while ( j < ret ) {
                        /*Parse events*/
