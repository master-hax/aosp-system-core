...
static std::string CopySharedLibrary(const char* tmp_dir) {
  ...
  std::string tmp_so_name(std::string(tmp_dir) + "/libbacktrace_test.so");
  std::string cp_cmd =
...
TEST(libbacktrace, check_unreadable_elf_local) {
  TemporaryDir td;
  std::string tmp_so_name(CopySharedLibrary(td.path));
