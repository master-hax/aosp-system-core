...
bool FindFuncInBacktrace(Backtrace* backtrace, uintptr_t test_func) {
  backtrace_map_t map;
  ...
  // The called function should never be last frame, that would be a bug.
  size_t frame_num = 0;
  for (Backtrace::const_iterator it = backtrace->begin(); it != backtrace->end(); ++it) {
...
void VerifyUnreadableElfBacktrace(Backtrace* backtrace, uintptr_t test_func) {
  ASSERT_GT(backtrace->NumFrames(), static_cast<size_t>(0))
    << DumpFrames(backtrace);
  ASSERT_LT(backtrace->NumFrames(), static_cast<size_t>(MAX_BACKTRACE_FRAMES))
    << DumpFrames(backtrace);
  ...
  backtrace_map_t map;
  backtrace->FillInMap(test_func, &map);
  ASSERT_TRUE(BacktraceMap::IsValid(map)) << DumpFrames(backtrace);
...
void VerifyUnreadableElfBacktrace(uintptr_t test_func) {
    ...
    VerifyUnreadableElfBacktrace(backtrace.get(), test_func);
}
...
TEST(libbacktrace, unwind_through_unreadable_elf_remote) {
  ...
  while (!done) {
    ...
    if (FindFuncInBacktrace(backtrace.get(), reinterpret_cast<uintptr_t>(test_func))) {
      VerifyUnreadableElfBacktrace(backtrace.get(), reinterpret_cast<uintptr_t>(test_func));
      done = true;
    }
