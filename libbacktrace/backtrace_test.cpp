...
void VerifyMaxDump(const backtrace_t* backtrace) {
  ...
  ASSERT_TRUE(NULL != backtrace->frames[MAX_BACKTRACE_FRAMES-1].proc_name);
  ASSERT_STREQ(backtrace->frames[MAX_BACKTRACE_FRAMES-1].proc_name,
               "test_recursive_call");
}
...
void VerifyIgnoreFrames(
  ...
  for (size_t i = 0; i < bt_ign2->num_frames; i++) {
    ...
    if (!check && bt_ign2->frames[i].proc_name &&
        strcmp(bt_ign2->frames[i].proc_name, cur_proc) == 0) {
      check = true;
    }
  }
}
