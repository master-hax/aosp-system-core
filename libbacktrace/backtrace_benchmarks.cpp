...
static bool CountMaps(pid_t pid, size_t* num_maps) {
  ...
  if (pid == getpid()) {
    ...
    fd = open((std::string("/proc/") + std::to_string(pid) + "/maps").c_str(), O_RDONLY);
  }
  if (fd == -1) {
...
static void CreateMap(benchmark::State& state, BacktraceMap* (*map_func)(pid_t, bool)) {
  ...
  // And to create a number of maps.
  pid_t pid;
  if ((pid = fork()) == 0) {
    size_t num_maps;
    ...
    for (size_t i = num_maps; i < kNumMaps; i++) {
      ...
      // Alternate page type to make sure the new maps are added for each call.
      if ((i % 2) == 0) {
        flags |= PROT_EXEC;
      }
...
BENCHMARK(BM_create_map)->Arg(1000);

static void BM_create_map_new(benchmark::State& state) {
  CreateMap(state, BacktraceMap::CreateNew);
}
BENCHMARK(BM_create_map_new)->Arg(1000);

#if 0
static void BM_unwind_local_same_map(benchmark::State& state) {
  while (state.KeepRunning()) {
  }
}
BENCHMARK(BM_unwind_local_same_map);

static void BM_unwind_remote_same_map(benchmark::State& state) {
  while (state.KeepRunning()) {
  }
}
BENCHMARK(BM_unwind_remote_same_map);
