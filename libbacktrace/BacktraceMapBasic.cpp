...
BacktraceMapBasic::BacktraceMapBasic(pid_t pid, const std::vector<backtrace_map_t>& maps)
    : BacktraceMap(pid) {
  maps_.insert(maps_.begin(), maps.begin(), maps.end());
...
void BacktraceMapBasic::FillIn(uintptr_t addr, backtrace_map_t* map) {
  ScopedBacktraceMapIteratorLock lock(this);
  for (auto cur_map : *this) {
    if (addr >= cur_map.start && addr < cur_map.end) {
      *map = cur_map;
      return;
    }
  }
...
bool BacktraceMapBasic::ParseLine(const char* line, backtrace_map_t* map) {
  uint64_t start;
  uint64_t end;
  char permissions[5];
  int name_pos;
  ...
  // 012345678901234567890123456789012345678901234567890123456789
  // 0         1         2         3         4         5
  if (sscanf(line, "%" SCNx64 "-%" SCNx64 " %4s %*x %*x:%*x %*d %n", &start, &end, permissions,
             &name_pos) != 3) {
#endif
    return false;
  }
