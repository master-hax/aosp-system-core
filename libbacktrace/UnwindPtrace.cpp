...
bool UnwindPtrace::Unwind(size_t num_ignore_frames) {
  ...
  if (ret < 0) {
    ALOGW("UnwindPtrace::Unwind: unw_init_remote failed %d", ret);
    return false;
  }
  ...
  do {
    ...
    if (ret < 0) {
      ALOGW("UnwindPtrace::Unwind: Failed to read IP %d", ret);
      break;
    }
    unw_word_t sp;
    ret = unw_get_reg(&cursor, UNW_REG_SP, &sp);
    if (ret < 0) {
      ALOGW("UnwindPtrace::Unwind: Failed to read SP %d", ret);
      break;
    }
