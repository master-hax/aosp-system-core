...
UnwindProcess::UnwindProcess() : addr_space_(NULL), upt_info_(NULL) {
}

UnwindProcess::~UnwindProcess() {
  if (upt_info_) {
...
bool UnwindProcess::Unwind(size_t num_ignore_frames) {
  addr_space_ = unw_create_addr_space(&_UPT_accessors, 0);
  if (!addr_space_) {
    ALOGW("UnwindProcess::Unwind: unw_create_addr_space failed.\n");
    return false;
  }

  upt_info_ = reinterpret_cast<struct UPT_info*>(_UPT_create(backtrace_obj_->Tid()));
  if (!upt_info_) {
    ALOGW("UnwindProcess::Unwind: Failed to create upt info.\n");
    return false;
  }
  ...
  if (ret < 0) {
    ALOGW("UnwindProcess::Unwind: unw_init_remote failed %d\n", ret);
    return false;
  }
  ...
  do {
    ...
    if (ret < 0) {
      ALOGW("UnwindProcess::Unwind: Failed to read IP %d\n", ret);
      break;
    }
    unw_word_t sp;
    ret = unw_get_reg(&cursor, UNW_REG_SP, &sp);
    if (ret < 0) {
      ALOGW("UnwindProcess::Unwind: Failed to read SP %d\n", ret);
      break;
    }
    ...
    if (num_ignore_frames == 0) {
      ...
      uintptr_t map_start;
      frame->map_name = backtrace_obj_->GetMapInfo(frame->pc, &map_start);
      if (frame->map_name) {
...
char* UnwindProcess::GetProcName(uintptr_t pc, uintptr_t* offset) {
  *offset = 0;
  char buf[512];
  unw_word_t value;
...
//-------------------------------------------------------------------------
Backtrace* CreateProcessObj(pid_t pid, pid_t tid) {
  return new BacktraceProcess(new UnwindProcess(), pid, tid);
}
