...
bool CorkscrewCommon::GenerateFrameData(
  ...
  for (size_t i = 0; i < data->num_frames; i++) {
    ...
    std::string proc_name = backtrace_obj_->GetProcName(frame->pc, &frame->proc_offset);
    if (!proc_name.empty()) {
      frame->proc_name = strdup(proc_name.c_str());
    }
  }
  return true;
}
...
std::string CorkscrewCurrent::GetProcName(uintptr_t pc, uintptr_t* offset) {
  *offset = 0;
...
std::string CorkscrewPtrace::GetProcName(uintptr_t pc, uintptr_t* offset) {
