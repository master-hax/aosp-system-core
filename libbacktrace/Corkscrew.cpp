...
bool CorkscrewCommon::GenerateFrameData(
  ...
  for (size_t i = 0; i < data->num_frames; i++) {
    ...
    frame->proc_offset = 0;
    frame->proc_name = backtrace_obj_->GetProcName(frame->pc, &frame->proc_offset);
  }
  return true;
}
...
char* CorkscrewCurrent::GetProcName(uintptr_t pc, uintptr_t* offset) {
  *offset = 0;
  ...
  return Backtrace::Demangle(symbol_name);
}
...
char* CorkscrewPtrace::GetProcName(uintptr_t pc, uintptr_t* offset) {
...
Backtrace* CreateThreadObj(pid_t tid) {
  CorkscrewThread* thread_impl = new CorkscrewThread();
  return new BacktraceThread(thread_impl, thread_impl, tid);
}
