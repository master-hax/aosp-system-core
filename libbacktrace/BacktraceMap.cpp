...
// 0         1         2         3         4         5
#define SCANF_FORMAT  "%*21c %lx-%lx [%*13c] %3c/%*3c SM=%*3c  %n"
...
// 012345678901234567890123456789012345678901234567890123456789
// 0         1         2         3         4         5
#define SCANF_FORMAT   "%lx-%lx %4s %*x %*x:%*x %*d%n"
...
const backtrace_map_t* BacktraceMap::Find(uintptr_t addr) {
  for (std::vector<backtrace_map_t>::const_iterator it = begin();
       it != end(); ++it) {
...
#if defined(__APPLE__)
  char cmd[sizeof(pid_t)*4 + 1 + sizeof("vmmap -w -resident -submap -allSplitLibs -interleaved ")];
#else
  char path[sizeof(pid_t)*4 + 1 + sizeof("/proc//maps")];
#endif
  char line[1024];
