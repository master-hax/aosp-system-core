...
static int ReadData(int fd, unsigned long place, uint64_t *data) {
  if (lseek(fd, place * sizeof(uint64_t), SEEK_SET) == (off_t)-1) {
    return -1;
  }
...
size_t GetPssBytes() {
  ...
  while (fgets(line, sizeof(line), maps)) {
    ...
    for (size_t page = start/pagesize; page < end/pagesize; page++) {
      uint64_t data;
      if (ReadData(pagemap_fd, page, &data) != 0) {
        continue;
      }

      if (PAGEMAP_PRESENT(data) && !PAGEMAP_SWAPPED(data)) {
        uint64_t count;
        if (ReadData(pagecount_fd, PAGEMAP_PFN(data), &count) == 0) {
