...
bool BacktraceOffline::FindProcInfo(unw_addr_space_t addr_space, uint64_t ip,
  ...
  // In .ARM.exidx and .eh_frame_hdr, we only know the start address of each function,
  // so we don't know the end address of last function. Although we can read symbol
  // table to get its function size, but that operation is expensive. So here we just
  // estimate a max size for the end function.
  constexpr int MAX_SIZE_OF_END_FUNCTION = 0xffff;
  if (debug_frame->has_arm_exidx && test_option_ != TEST_OPTION_NOT_USE_ARM_EXIDX) {
    ...
    if (ip_vaddr >= func_vaddrs[0] && ip_vaddr < func_vaddrs.back() + MAX_SIZE_OF_END_FUNCTION) {
  ...
  if (debug_frame->has_eh_frame) {
    if (ip_vaddr >= debug_frame->eh_frame.min_func_vaddr &&
        ip_vaddr < debug_frame->eh_frame.max_func_vaddr + MAX_SIZE_OF_END_FUNCTION) {
      // TODO: Will it be simpler to search by ourselves?
      // Calculate the addresses where .eh_frame_hdr and .eh_frame stay when the process was
      // running.
  ...
  for (auto it = elf->section_begin(); it != elf->section_end(); ++it) {
    ...
    if (name) {
      ...
      if (s == ".debug_frame") {
        ...
        result->eh_frame.eh_frame_hdr_vaddr = it->sh_addr;
        llvm::ErrorOr<llvm::ArrayRef<uint8_t>> data = elf->getSectionContents(&*it);
        if (data) {
          result->eh_frame.eh_frame_hdr_data.insert(
              result->eh_frame.eh_frame_hdr_data.end(),
              data->data(), data->data() + data->size());

          uint64_t fde_table_offset;
          if (GetFdeTableOffsetInEhFrameHdr(result->eh_frame.eh_frame_hdr_data,
                                             &fde_table_offset)) {
            result->eh_frame.fde_table_offset_in_eh_frame_hdr = fde_table_offset;
            if (fde_table_offset + 8 <= data->size()) {
              intptr_t eh_frame_hdr_vaddr = it->sh_addr;
              int32_t sdata;
              uint8_t* p = result->eh_frame.eh_frame_hdr_data.data() + fde_table_offset;
              memcpy(&sdata, p, 4);
              result->eh_frame.min_func_vaddr = eh_frame_hdr_vaddr + sdata;
              p = result->eh_frame.eh_frame_hdr_data.data() + data->size() - 8;
              memcpy(&sdata, p, 4);
              result->eh_frame.max_func_vaddr = eh_frame_hdr_vaddr + sdata;
              has_eh_frame_hdr = true;
            }
          }
        }
      } else if (s == ".eh_frame") {
        result->eh_frame.eh_frame_vaddr = it->sh_addr;
        llvm::ErrorOr<llvm::ArrayRef<uint8_t>> data = elf->getSectionContents(&*it);
        if (data) {
          result->eh_frame.eh_frame_data.insert(result->eh_frame.eh_frame_data.end(),
                                                data->data(), data->data() + data->size());
          has_eh_frame = true;
        }
      } else if (s == ".ARM.exidx") {
          ...
          if (entry_count > 0u) {
            ...
            for (auto& entry : result->arm_exidx.exidx_data) {
              result->arm_exidx.func_vaddr_array.push_back(
                  (entry.func_offset + vaddr) & 0x7fffffff);
              vaddr += 8;
            }
