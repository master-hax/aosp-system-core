...
llvm::object::OwningBinary<llvm::object::Binary> OpenEmbeddedElfFile(const std::string& filename) {
  ...
  android::base::unique_fd fd(open(apk_file.c_str(), O_RDONLY | O_BINARY));
  if (fd.get() == -1) {
    return ret;
  }
  ZipArchiveHandle handle;
  if (OpenArchiveFd(fd.get(), apk_file.c_str(), &handle, false) != 0) {
    return ret;
  }
  ScopedZiparchiveHandle scoped_handle(handle);
  ZipEntry zentry;
  if (FindEntry(handle, ZipString(elf_file.c_str()), &zentry) != 0) {
    return ret;
  }
  if (zentry.method != kCompressStored || zentry.compressed_length != zentry.uncompressed_length) {
    return ret;
  }
  auto buffer_or_err = llvm::MemoryBuffer::getOpenFileSlice(fd.get(), apk_file,
                                                            zentry.uncompressed_length, zentry.offset);
  if (!buffer_or_err) {
    return ret;
  }
