...
llvm::object::OwningBinary<llvm::object::Binary> OpenEmbeddedElfFile(const std::string& filename) {
  ...
  std::string err_msg;
  std::unique_ptr<ZipArchive> archive(ZipArchive::Open(apk_file.c_str(), &err_msg));
  if (archive == nullptr) {
    BACK_LOGW("failed to open archive %s: %s", apk_file.c_str(), err_msg.c_str());
    return ret;
  }
  ZipEntry zentry;
  if (FindEntry(archive.get(), ZipString(elf_file.c_str()), &zentry) != 0) {
    return ret;
  }
  if (zentry.method != kCompressStored || zentry.compressed_length != zentry.uncompressed_length) {
    return ret;
  }
  auto buffer_or_err = llvm::MemoryBuffer::getOpenFileSlice(
                            GetFileDescriptor(archive.get()), apk_file,
