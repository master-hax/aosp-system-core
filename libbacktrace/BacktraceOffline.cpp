...
bool BacktraceOffline::Unwind(size_t num_ignore_frames, ucontext_t* context) {
  if (context == nullptr) {
    BACK_LOGW("The offline context is needed for offline backtracing.");
    return false;
  }
  context_ = context;
...
class DebugFrameScopedGuard {
 public:
  DebugFrameScopedGuard(DebugFrameInfo* debug_frame) : debug_frame_(debug_frame) {
  }

  ~DebugFrameScopedGuard() {
...
bool BacktraceOffline::FindProcInfo(unw_addr_space_t addr_space, uint64_t ip,
  ...
  DebugFrameScopedGuard guard((cache_file_ ? nullptr : debug_frame));
