...
class BacktraceOffline : public Backtrace {
 public:
  BacktraceOffline(pid_t pid, pid_t tid, BacktraceMap* map, const StackInfo& stack, bool cache_file)
      : Backtrace(pid, tid, map), cache_file_(cache_file), context_(nullptr) {
    stack_space_.start = stack.stack_addr;
    stack_space_.end = stack.stack_addr + stack.stack_size;
    stack_space_.data = stack.stack_data;
  }
