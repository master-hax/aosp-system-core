...
static void FillInDexFrame(UnwindStackMap* stack_map, uint64_t dex_pc,
  ...
  // so we need to try the subsequent map to find the .odex file.
  ...
  // Get the start of the symbol, which is the start of the dex file.
  UnwindDexFile* dex_file =
      stack_map->GetDexFile(rel_dex_pc - dex_offset - entry->elf_offset + info->start, info);
  if (dex_file != nullptr) {
