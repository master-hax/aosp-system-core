...
static bool Unwind(pid_t pid, unwindstack::Memory* memory, unwindstack::Regs* regs,
  ...
  do {
    ...
    if (num_ignore_frames == 0 && !skip_frame) {
      ...
      frame->map.load_base = elf->GetLoadBias();
      frame->map.flags = map_info->flags;
      frame->map.name = map_info->name;

      frame->func_name = GetFunctionName(pid, back_map, frame->pc, &frame->func_offset);
      if (num_frames > 0) {
...
UnwindStackCurrent::UnwindStackCurrent(BacktraceMap* map)
    : BacktraceCurrent(getpid(), getpid(), map), memory_(new unwindstack::MemoryLocal) {}
...
Backtrace* Backtrace::CreateNew(pid_t pid, pid_t tid, BacktraceMap* map) {
  ...
  if (pid == getpid()) {
    return new UnwindStackCurrent(map);
  } else {
