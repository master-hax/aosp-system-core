...
bool UnwindCurrent::Unwind(size_t num_ignore_frames) {
  ...
  if (ret < 0) {
    ALOGW("UnwindCurrent::Unwind: unw_getcontext failed %d", ret);
    return false;
  }
...
bool UnwindCurrent::UnwindFromContext(size_t num_ignore_frames, bool resolve) {
  ...
  if (ret < 0) {
    ALOGW("UnwindCurrent::UnwindFromContext: unw_init_local failed %d", ret);
    return false;
  }
  ...
  do {
    ...
    if (ret < 0) {
      ALOGW("UnwindCurrent::UnwindFromContext: Failed to read IP %d", ret);
      break;
    }
    unw_word_t sp;
    ret = unw_get_reg(cursor, UNW_REG_SP, &sp);
    if (ret < 0) {
      ALOGW("UnwindCurrent::UnwindFromContext: Failed to read SP %d", ret);
      break;
    }
