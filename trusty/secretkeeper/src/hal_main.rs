...
use authgraph_hal::{channel::SerializedChannel, service};
use log::{error, info};
use std::{
    ffi::CString,
    panic,
...
const TIPC_SERVICE_NAME: &str = "com.android.trusty.secretkeeper";

// TODO: move to ISecretkeeper
static AIDL_SERVICE_NAME: &str = "android.hardware.security.authgraph.IAuthGraphKeyExchange";
...
impl SerializedChannel for TipcChannel {
    ...
    fn execute(&mut self, req_data: &[u8]) -> binder::Result<Vec<u8>> {
        self.0.send(req_data).map_err(|e| {
...
fn inner_main() -> Result<(), HalServiceError> {
    ...
    let connection =
        trusty::TipcChannel::connect(DEFAULT_DEVICE, TIPC_SERVICE_NAME).map_err(|e| {
            HalServiceError(format!(
                "Failed to connect to Trusty Secretkeeper TA because of {:?}.",
                e
            ))
        })?;
    ...
    // TODO: shift to Secretkeeper
    let service = service::AuthGraphService::new_as_binder(tipc_channel.clone());
    let service_name = format!("{}/{}", AIDL_SERVICE_NAME, SERVICE_INSTANCE);
