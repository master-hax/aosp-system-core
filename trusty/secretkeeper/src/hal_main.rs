...
fn inner_main() -> Result<(), HalServiceError> {
    ...
    let ag_tipc_channel = Arc::new(Mutex::new(TipcChannel(ag_connection)));
    ...
    let _sk_tipc_channel = Arc::new(Mutex::new(TipcChannel(sk_connection)));

    // Register the AIDL service
    // TODO: shift to Secretkeeper
    let service = service::AuthGraphService::new_as_binder(ag_tipc_channel.clone());
