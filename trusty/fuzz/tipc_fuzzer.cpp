...
static TrustyApp trustyApp(TIPC_DEV, TRUSTY_APP_PORT);
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    auto ret = trustyApp.Write(data, size);
    if (!ret.ok()) {
        return -1;
    }

    ret = trustyApp.Read(&buf, sizeof(buf));
    if (!ret.ok()) {
        return -1;
    }

    // Reconnect to ensure that the service is still up
    trustyApp.Disconnect();
    ret = trustyApp.Connect();
    if (!ret.ok()) {
        std::cerr << ret.error() << std::endl;
