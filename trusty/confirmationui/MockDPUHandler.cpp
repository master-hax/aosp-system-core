...
HandleCmdResult DPUHandler::HandleAllocateBuffer(const uint64_t req_buffer_len, struct secure_dpu_allocate_buffer_rsp* args) {
    args->buffer_phy_addr = 0x12345678;
    args->buffer_len = req_buffer_len;
    return HandleCmdResult::Success;
}

HandleCmdResult DPUHandler::HandleFreeBuffer(const uint64_t req_buffer_ptr, const uint64_t req_buffer_len) {
    (void)req_buffer_ptr;
    (void)req_buffer_len;
    return HandleCmdResult::Success;
}
