 ...
 * The 16 bits header spicify the number of bytes of the whole packet
 * (header + payload).
 */
struct data_packet {
    uint16_t header;
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    while (data_idx < size) {
        ...
        size_t payload_size = data_packet_ptr->header - sizeof(data_packet_ptr->header);
        data_idx += data_packet_ptr->header;
