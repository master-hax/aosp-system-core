...
#include "../../../../../system/core/trusty/confirmationui/TrustyConfirmationUI.h"
...
TrustyConfirmationUI::promptUserConfirmation_(const MsgString& promptText,
    ...
    auto app = std::make_shared<TrustyApp>();

    // start the TA
    app->start(kTrustyDeviceName, kConfirmationuiAppName);
    ...
    auto hsBegin = [&]() -> std::tuple<TeeuiRc, Nonce> {
        ...
        if (error || rc != TeeuiRc::OK) {
            LOG(ERROR) << "Failed to begin secure input handshake (" << error << "/" << uint32_t(rc)
                       << ")";
            rc = error ? TeeuiRc::SystemError : rc;
        }
        return result;
    };
    ...
    auto hsFinalize = [&](const Signature& sig, const Nonce& nCi) -> TeeuiRc {
        ...
        if (error || rc != TeeuiRc::OK) {
            LOG(ERROR) << "Failed to finalize secure input handshake (" << error << "/"
                       << uint32_t(rc) << ")";
            rc = error ? TeeuiRc::SystemError : rc;
        }
        return rc;
    ...
    TeeuiRc eventRC = TeeuiRc::OperationPending;
    std::mutex evResultLock;
    auto inputResult = [&](TeeuiRc rc) {
        {
            std::unique_lock<std::mutex> lock(evResultLock);
            if (eventRC == TeeuiRc::OperationPending) {
                eventRC = rc;
            }
        }
    ...
    if (error == -2) {
        ...
        LOG(ERROR) << "PromptUserConfirmationMsg failed: " << error;
        return result;  // TeeuiRc::SystemError;
    }
    if (rc != TeeuiRc::OK) {
    ...
    // make sure the secure touch session gets ended
    std::mutex stLock;
    ...
    LOG(INFO) << "Result yields " << error << "/" << uint32_t(rc);
    if (error) {
...
TrustyConfirmationUI::deliverSecureInputEvent(const HardwareAuthToken& secureInputToken) {
    ResponseCode rc = ResponseCode::Ignored;
    {
        ...
        if (error) return ResponseCode::SystemError;
        auto& [trc] = response;
...
IConfirmationUI* HIDL_FETCH_IConfirmationUI(const char* /* name */) {
