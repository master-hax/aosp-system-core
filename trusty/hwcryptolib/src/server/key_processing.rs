...
use android_hardware_security_see::aidl::android::hardware::security::see::hwcrypto::base_types::{
    EvictReason::EvictReason, KeyClass::KeyClass, KeyPermissions::KeyPermissions, KeyType::KeyType,
    KeyUse::KeyUse, OpaqueKeyMaterial::OpaqueKeyMaterial,
};
...
impl TryFrom<i64> for CoseLabels {
    ...
    fn try_from(value: i64) -> Result<Self, Self::Error> {
        match value {
            ...
            x if x == CoseLabels::KeyClass as i64 => Ok(CoseLabels::KeyClass),
...
fn get_key_class(value: u64) -> Result<KeyClass, HwCryptoError> {
    match value {
        x if x == KeyClass::EPHEMERAL.0 as u64 => Ok(KeyClass::EPHEMERAL),
        x if x == KeyClass::HARDWARE.0 as u64 => Ok(KeyClass::HARDWARE),
        x if x == KeyClass::PORTABLE.0 as u64 => Ok(KeyClass::PORTABLE),
        _ => Err(hwcrypto_err!(SERIALIZATION_ERROR, "unsupported KeyClass value {}", value)),
    }
}
...
fn parse_cose_header(
    ...
    match label {
        ...
        CoseLabels::KeyClass => {
            let key_class = parse_cborium_u64(&header_element.1, "KeyClass")?;
            key_header.key_class = get_key_class(key_class)?;
            Ok(())
        }
...
fn serialize_key(
    ...
    .value(CoseLabels::KeyClass as i64, Value::Integer(key.key_header.key_class.0.into()))
    ...
    let policy = KeyPolicy {
        ...
        key_class: key.key_header.key_class,
...
fn wrap_derivation_allowed(
    ...
    match *key_class {
        KeyClass::EPHEMERAL => Ok(true), //ephemeral keys can be used to derive/wrap any other key
        KeyClass::HARDWARE => {
        ...
        _ => Err(hwcrypto_err!(UNSUPPORTED, "unsupported Key class {:?}", key_class)),
    }
}
...
fn check_key_policies(key: &ClearKey, rng: &mut dyn crypto::Rng) -> Result<(), HwCryptoError> {
    ...
    if !wrap_derivation_allowed(&key.key_header.key_class, &key.key_header.key_permissions)? {
        return Err(hwcrypto_err!(
            UNSUPPORTED,
            "wrapping issue. Key class {:?}, key permissions {:?}",
            key.key_header.key_class,
...
mod tests {
    ...
    fn check_wrapping_allowed() {
        ...
        assert!(
            wrap_derivation_allowed(&KeyClass::EPHEMERAL, &all_requested).unwrap(),
            "ephemeral keys should be able to wrap any key"
        );
        assert!(
            wrap_derivation_allowed(&KeyClass::EPHEMERAL, &all_but_hardware).unwrap(),
            "ephemeral keys should be able to wrap any key"
        );
        assert!(
            wrap_derivation_allowed(&KeyClass::EPHEMERAL, &all_but_ephemeral).unwrap(),
            "ephemeral keys should be able to wrap any key"
        );
        assert!(
            wrap_derivation_allowed(&KeyClass::EPHEMERAL, &all_but_portable).unwrap(),
            "ephemeral keys should be able to wrap any key"
        );
        assert!(
            wrap_derivation_allowed(&KeyClass::EPHEMERAL, &just_hardware).unwrap(),
            "ephemeral keys should be able to wrap any key"
        );
        assert!(
            wrap_derivation_allowed(&KeyClass::EPHEMERAL, &just_ephemeral).unwrap(),
            "ephemeral keys should be able to wrap any key"
        );
        assert!(
            wrap_derivation_allowed(&KeyClass::EPHEMERAL, &just_portable).unwrap(),
            "ephemeral keys should be able to wrap any key"
        );
        //checking hardware keys
        assert!(
            !wrap_derivation_allowed(&KeyClass::HARDWARE, &all_requested).unwrap(),
            "hardware keys should be able to wrap any key but ephemeral"
        );
        assert!(
            !wrap_derivation_allowed(&KeyClass::HARDWARE, &all_but_hardware).unwrap(),
            "hardware keys should be able to wrap any key but ephemeral"
        );
        assert!(
            wrap_derivation_allowed(&KeyClass::HARDWARE, &all_but_ephemeral).unwrap(),
            "hardware keys should be able to wrap any key but ephemeral"
        );
        assert!(
            !wrap_derivation_allowed(&KeyClass::HARDWARE, &all_but_portable).unwrap(),
            "hardware keys should be able to wrap any key but ephemeral"
        );
        assert!(
            wrap_derivation_allowed(&KeyClass::HARDWARE, &just_hardware).unwrap(),
            "hardware keys should be able to wrap any key but ephemeral"
        );
        assert!(
            !wrap_derivation_allowed(&KeyClass::HARDWARE, &just_ephemeral).unwrap(),
            "hardware keys should be able to wrap any key but ephemeral"
        );
        assert!(
            wrap_derivation_allowed(&KeyClass::HARDWARE, &just_portable).unwrap(),
            "hardware keys should be able to wrap any key but ephemeral"
        );
        //checking portable keys
        assert!(
            !wrap_derivation_allowed(&KeyClass::PORTABLE, &all_requested).unwrap(),
            "portable keys should only be able to wrap portable keys"
        );
        assert!(
            !wrap_derivation_allowed(&KeyClass::PORTABLE, &all_but_hardware).unwrap(),
            "portable keys should only be able to wrap portable keys"
        );
        assert!(
            !wrap_derivation_allowed(&KeyClass::PORTABLE, &all_but_ephemeral).unwrap(),
            "portable keys should only be able to wrap portable keys"
        );
        assert!(
            !wrap_derivation_allowed(&KeyClass::PORTABLE, &all_but_portable).unwrap(),
            "portable keys should only be able to wrap portable keys"
        );
        assert!(
            !wrap_derivation_allowed(&KeyClass::PORTABLE, &just_hardware).unwrap(),
            "portable keys should only be able to wrap portable keys"
        );
        assert!(
            !wrap_derivation_allowed(&KeyClass::PORTABLE, &just_ephemeral).unwrap(),
            "portable keys should only be able to wrap portable keys"
        );
        assert!(
            wrap_derivation_allowed(&KeyClass::PORTABLE, &just_portable).unwrap(),
            "portable keys should only be able to wrap portable keys"
        );
    }
    ...
    fn serialize_deserialize_aes_key() {
        ...
        assert_eq!(
            deserialized_key.key_header.key_class,
            KeyClass::EPHEMERAL,
