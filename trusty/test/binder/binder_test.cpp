...
class BinderTest : public testing::Test {
  protected:
    void SetUp() override {
        ...
        int srv_sock_fd = ioctl(srv_ufd.get(), TIPC_IOC_GET_SOCKET);
        ASSERT_GE(srv_sock_fd, 0);

        base::unique_fd srv_sock_ufd(srv_sock_fd);
        mSess = RpcSession::make(RpcTransportCtxFactoryRaw::make());
        mSess->setMaxIncomingThreads(0);
        mSess->setMaxOutgoingThreads(1);

        auto status = mSess->setupPreconnectedClient(std::move(srv_sock_ufd),
