...
TEST_F(BinderTest, RepeatIntConstants) {
...
TEST_F(BinderTest, RepeatLongConstants) {
...
TEST_F(BinderTest, RepeatStrings) {
...
TEST_F(BinderTest, Threads) {
    ...
    for (size_t i = 0; i < kMaxTestThreads; i++) {
        auto threadFn = [&](ThreadResult& result) {
            {
            ...
            result.status = mSrv.get()->ReverseInt(ints, &repeated, &result.reversed);
        };
        threads.emplace_back(threadFn, std::ref(threadResults[i]));
    }
