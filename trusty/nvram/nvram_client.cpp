...
bool ParseControlList(const std::string& list, Vector<Control>* controls) {
  size_t end = 0;
  size_t pos = 0;
  do {
    end = list.find(',', pos);
...
bool FormatControlList(const Vector<Control>& controls, std::string* list) {
  bool result = true;
  list->clear();
...
int HandleCreateSpace(char* args[]) {
  ...
  CHECK(request.authorization_value.Assign(args[3], strlen(args[3])));
...
int HandleDeleteSpace(char* args[]) {
  ...
  request.index = std::stoul(args[0], nullptr, 0);
  CHECK(request.authorization_value.Assign(args[1], strlen(args[1])));
...
int HandleWriteSpace(char* args[]) {
  ...
  request.index = std::stoul(args[0], nullptr, 0);
  CHECK(request.buffer.Assign(args[1], strlen(args[1])));
  CHECK(request.authorization_value.Assign(args[2], strlen(args[2])));
