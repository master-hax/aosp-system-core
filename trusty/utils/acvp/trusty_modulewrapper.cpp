...
class ModuleWrapper {
  ...
  public:
    ...
    Result<void> SendMessage(const Args& args);
...
Result<void> ModuleWrapper::SendMessage(const Args& args) {
    Span<const uint8_t> algorithm = args.algorithm();

    assert(args.count() < ACVP_MAX_NUM_ARGUMENTS);
    assert(algorithm.size() < ACVP_MAX_NAME_LENGTH);

    struct acvp_req request;
    request.num_args = args.count();
    request.algo_len = algorithm.size();
    memcpy(request.algo, algorithm.data(), algorithm.size());

    shm_size_ = ACVP_MIN_SHARED_MEMORY;
    if (args.TotalArgLength() > shm_size_) {
        shm_size_ = AlignUpToPage(args.TotalArgLength());
    }
    request.buffer_size = shm_size_;

    struct iovec iovs[ACVP_MAX_NUM_ARGUMENTS + 1];
    iovs[0] = {
            .iov_base = &request,
    ...
    size_t cur_offset = 0;
    struct acvp_arg request_args[ACVP_MAX_NUM_ARGUMENTS];
    for (int i = 0; i < args.count(); ++i) {
        request_args[i].start = cur_offset;
        request_args[i].len = args[i].size();
        memcpy(shm_buffer_ + cur_offset, args[i].data(), args[i].size());
        iovs[i + 1] = {
                .iov_base = &request_args[i],
                .iov_len = sizeof(struct acvp_arg),
        };
    }
    ...
    int rc = tipc_send(tipc_fd_, iovs, args.count() + 1, &shm, 1);
    if (rc != sizeof(struct acvp_req) + sizeof(struct acvp_arg) * args.count()) {
...
int main() {
    for (;;) {
        ...
        const std::string name(reinterpret_cast<const char*>(args->algorithm().data()),
                               args->algorithm().size());
