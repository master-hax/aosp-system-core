    ...
    while(1) {
        ...
        for (int index = 0; index < record_list_.size(); index++) {
            ...
            if (complete_cnt != counters[index] && start_cnt == complete_cnt) {
                ...
                std::string fmt = "/%s.%lu.profraw";
                int sz = std::snprintf(nullptr, 0, fmt.c_str(), uuid_list_[index].c_str(), counters[index]);
                std::string filename(sz+1, '.');
                std::sprintf(filename.data(), fmt.c_str(), uuid_list_[index].c_str(), counters[index]);
