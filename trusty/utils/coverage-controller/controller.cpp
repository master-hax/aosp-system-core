...
class Controller {
    ...
    private:
        std::vector<unique_ptr<CoverageRecord> >record_list_;
        ...
        void connectCoverageServer() {
            ...
            if (coverage_srv_fd < 0) {
                fprintf(stderr, \
                        "Error: Failed to connect to Trusty coverage server.\n");
                return;
            }
        }
        ...
        void setUpShm() {
            ...
            while(1) {
                ...
                if (resp.hdr.cmd == (LINE_COVERAGE_CLIENT_CMD_OPEN |
                                        LINE_COVERAGE_CLIENT_CMD_RESP_BIT)) {
                    break;
                }

                if (resp.hdr.cmd == (req.hdr.cmd | LINE_COVERAGE_CLIENT_CMD_RESP_BIT)) {
                    uuid_list_.push_back(resp.send_list_args.uuid);
                    record_list_.push_back(make_unique<CoverageRecord>(TIPC_DEV,
                                                                        &uuid_list_.back()));
                ...
                int rc = write(coverage_srv_fd, &req, sizeof(req));
                    if (rc != (int)sizeof(req)) {
                        fprintf(stderr, \
                              "failed to send request to coverage server:: %d\n", rc);
                }
            }
        }
