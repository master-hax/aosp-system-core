...
class DPUHandler {
  private:
    ...
    HandleCmdResult HandleAllocateBuffer(const uint64_t req_buffer_len,
                                         struct secure_dpu_allocate_buffer_rsp* args);
    HandleCmdResult HandleFreeBuffer(const uint64_t req_buffer_ptr,
                                     const uint64_t req_buffer_len);
    HandleCmdResult HandleStartSecureDisplay();
    HandleCmdResult HandleStopSecureDisplay();

    std::tuple<HandleCmdResult, std::string> handleCmd(const uint8_t in_buf[],
