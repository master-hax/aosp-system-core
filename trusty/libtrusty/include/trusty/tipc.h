 ...
 * buffer, a partial page copying has to happen and pff an additional memory
 * will be allocated to safely complete data exchange without having undesired
 * data exposure. In this case the content of pages that needs to be partially
 * shared will be moved to these newly allocated pages, the leftover space will
 * be sanitized and these new pages will be actually shared instead of original
 * once. Since tipc_memref_prepare_aligned call only accepts page aligned
 * shareable region, it is impossible to get into the situation where
 * additional memory will be required and zero copy sharing will be achieved.
