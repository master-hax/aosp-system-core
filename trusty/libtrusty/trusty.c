...
int tipc_send_msg(int fd, const struct iovec* iov, unsigned int iov_cnt,
    ...
    for (i = 0; i < mrefv_cnt; i++) shmemv[i] = mrefv[i].shmem;
...
#define ROUNDUP(v, a) (((uintptr_t)(v) + ((uintptr_t)(a)-1)) & ~(((uintptr_t)a) - 1))
...
static int memref_validate(struct tipc_memref* mr, bool aligned) {
    ...
    if (aligned) {
        ...
        if (mr->shr_base & (mr->page_size - 1)) return -EINVAL;

        if (mr->shr_size & (mr->page_size - 1)) return -EINVAL;
    }
...
static uint32_t recalc_shr_region(struct tipc_memref* mr) {
    ...
    /* page align end of sharable region if possible */
    ...
    if (ROUNDDN(shr_beg, mr->page_size) == ROUNDDN(shr_end - 1, mr->page_size)) {
        /* sharable region ends are on the same page */
        if (mr->shr_size == mr->page_size)
            return 0;
        else
            return 1;
    } else {
        /* sharble region ends are on different pages */
        if (shr_beg & (mr->page_size - 1)) pgcnt++;
        if (shr_end & (mr->page_size - 1)) pgcnt++;
    }
 ...
 * Page aligning is not strictly nessesary but strongly recommended.
 *
 * Return: 0 on success negative value on error.
 */
int tipc_memref_prepare_unaligned(struct tipc_memref* mr, uint32_t flags, void* shr_base,
    ...
    if (!mr || !phsize || !phoff) return -EINVAL;
 ...
 *
 * Note 1: In general, if data flow direction specified to @tipc_memref_init call
 ...
 * Note 2: The page pool if internally allocated by @tipc_memref_prepare will be freed
 * by this call.
 *
 * Return: none
 */
void tipc_memref_finish(struct tipc_memref* mr, size_t size) {
    ...
    if (mr->aux_pages) {
        /* we are tracking extra pages pages, free them */
        free(mr->aux_pages);
        mr->aux_pages = NULL;
    }
}
