...
ssize_t tipc_send(int fd, const struct iovec* iov, int iovcnt, struct trusty_shm* shms,
    ...
    if (ATRACE_ENABLED()) {
        int iov_bytes = 0;
        if (iovcnt > 0 && iov[0].iov_len >= sizeof(uint32_t)) {
            iov_bytes = *(uint32_t*)(iov[0].iov_base);
        }
        snprintf(buffer, EVENT_BUFFER_LEN, "%s_%d_%08x", __func__, fd, iov_bytes);
    } else {
        snprintf(buffer, EVENT_BUFFER_LEN, "");
    }
