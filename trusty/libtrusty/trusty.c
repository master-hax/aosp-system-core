...
static int tipc_vsock_connect(const char* type_cid_port_str, const char* srv_name) {
    ...
    if (ret) {
        ALOGE("%s: failed to set vsock connect timeout (err=%d)\n", __func__, errno);
    ...
    do {
        ...
        if (ret && (errno == ENODEV || errno == ESOCKTNOSUPPORT) && --retry) {
             ...
             */
            ALOGE("%s: can't connect to vsock for tipc service \"%s\" (err=%d) %d retries "
                  "remaining\n",
                  __func__, srv_name, errno, retry);
            sleep(1);
        } else {
            retry = 0;
        }
    ...
    if (ret != sizeof(status)) {
        ALOGE("%s: failed to read status byte for connect to tipc service name \"%s\" (err=%d)\n",
