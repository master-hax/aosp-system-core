...
#define PORT_START_ITERATIONS 100
...
TEST_F(AppMgrTest, AppRestartStress)
{
    ...
    while(!count || (restarts != count && !timed_out)) {
...
TEST_F(AppMgrTest, AppSartPortStress)
{
    ...
    if (count == 0) {
        print_interval = DEFAULT_PRINT_INTERVAL;
        std::cout << "Runnig for infinite starts" << std::endl;
    }
    else {
        print_interval = count >= DEFAULT_PRINT_COUNT ?
                         count / DEFAULT_PRINT_COUNT : 1;
