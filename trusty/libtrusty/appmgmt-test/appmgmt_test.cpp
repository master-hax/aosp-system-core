...
static unsigned int target_count = DEFAULT_COUNT;
...
"  -c, --count           number of restarts to use in stress test. if 0 the test runs indefinitely. Default 1M.\n"
"\n"
;
...
TEST_F(AppMgrTest, AppRestartStress)
{
    ...
    if (target_count == 0) {
        ...
        forever = false;
        print_interval = target_count >= DEFAULT_PRINT_COUNT ?
	        target_count / DEFAULT_PRINT_COUNT : 1;
        timeout = target_count * TIMEOUT_PER_RESTART;
        cout << "Running for " << target_count << " restarts"  << endl;
    }

    start_time = gettime_msecs();

    while(forever || (restarts != target_count && !timed_out)) {
        fd_ = tipc_connect(TIPC_DEFAULT_DEVNAME, PERSISTENT_PORT);
        if (fd_ < 0) {
...
retry:
    ...
    ASSERT_EQ(restarts, target_count) << "Could not complete stress test";
}
