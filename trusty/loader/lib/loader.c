...
static int rsp_to_loader_err(int rsp) {
    switch (rsp) {
        ...
        case LOADER_ERR_INTERNAL:
            return ERR_SERVER;
        case LOADER_ERR_TIMED_OUT:
            return ERR_TIMEOUT;
        case LOADER_ERR_INVALID_REQ:
...
static int get_max_tipc_size(__attribute__((unused)) struct tipc_obj* obj) {
    return MAX_TIPC_PAYLOAD_SIZE;
}

static int send_handler(struct tipc_obj* obj, const void* data, int size) {
    struct tipc_ctx* ctx = containerof(obj, struct tipc_ctx, obj);

    return write(ctx->fd, data, size);
}

static int recv_handler(struct tipc_obj* obj, void* data, int size) {
    struct tipc_ctx* ctx = containerof(obj, struct tipc_ctx, obj);
...
const char* loader_error_to_str(int rsp) {
    switch (rsp) {
        case NO_ERROR:
            return "no error";
        case ERR_TIMEOUT:
            return "server timed out while receiving request";
        case ERR_INPUT:
        ...
        case ERR_INVALID_RSP:
            return "received unknown response from the server";
        default:
            return "unkown error";
    }
}
