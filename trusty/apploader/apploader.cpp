...
static ssize_t send_load_message(int tipc_fd, int package_fd, off64_t package_size) {
    struct apploader_message msg = {
            ...
            .arg1 = 0,
            .arg2 = static_cast<uint64_t>(package_size),
    };
    ...
    return tipc_send(tipc_fd, &iov, 1, &shm, 1);
}

static ssize_t read_response(int tipc_fd) {
    struct apploader_message resp;
    ...
    if (resp.cmd != (APPLOADER_CMD_LOAD_APPLICATION | APPLOADER_RESP_BIT)) {
        fprintf(stderr, "Invalid command in respose: %u\n", resp.cmd);
        return -EINVAL;
    }

    uint32_t error = resp.arg1;
    switch (error) {
