...
/**
 * enum coverage_record_type - Type of coverage information present in the
 * corresponding record section.
 */
enum coverage_record_type {
    COV_START = 0x434f5652, /* Magic header start marker */
    COV_TOTAL_LENGTH = 0,   /* Total length of the entire coverage record, must
                             * be the last header item. */
    COV_8BIT_COUNTERS = 1,  /* 8bit counter for each instrumentation point */
    COV_INSTR_PCS = 2,      /* Pointer length offset of each instrumentation
                             * point from the start of the binary */
};
 ...
 * coverage record.
 *
 * Coverage records start with a header which a list of struct
 ...
 * the the region's offset from the offset of the next header item.
 */
struct coverage_record_header {
    enum coverage_record_type type;
    uint32_t offset;
};
