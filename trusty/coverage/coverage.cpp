...
Result<void> CoverageRecord::RegisterShm(unique_fd memfd) {
    ...
    ssize_t rc = tipc_send(coverage_srv_fd_, req_iov, countof(req_iov), &shm, 1);
    if (rc < 0) {
        return ErrnoError() << "failed tipc_send(): ";
    }

    coverage_client_open_resp resp;
    iovec resp_iov[] = {
            {
                    .iov_base = &hdr,
                    .iov_len = sizeof(hdr),
    ...
    rc = readv(coverage_srv_fd_, resp_iov, countof(resp_iov));
    if (rc < 0) {
