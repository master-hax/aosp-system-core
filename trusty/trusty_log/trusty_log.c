...
static void trusty_filter_print_log(struct log_data_footer footer, char* app_name, char* log_text) {
    ...
    if (app_name) {
        fprintf(tlog_out_file, "%llu: %s: %s", footer.timestamp, app_name, log_text);
    } else {
        fprintf(tlog_out_file, "%llu: %s", footer.timestamp, log_text);
    }
}
...
static int trusty_debugfs_read_logs(char* trusty_debugfs_log_file) {
    ...
    if (!f) {
        fprintf(stderr, "read from trusty-log debugfs failed");
        return EXIT_FAILURE;
    }
