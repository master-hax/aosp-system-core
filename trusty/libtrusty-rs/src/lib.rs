...
impl TipcChannel {
    ...
    ///
    /// If there was no incoming message then the length of `buf` is set to 0.
    ///
    /// # Errors
    ///
    ...
    pub fn recv(&mut self, buf: &mut Vec<u8>) -> Result<()> {
        // If no space has been allocated in the buffer reserve some initial capacity.
        if buf.capacity() == 0 {
            buf.reserve(128);
        }
        ...
        loop {
            ...
            match self.0.read(buf.as_mut_slice()) {
                ...
                Err(err) => {
                    if let Some(90) = err.raw_os_error() {
                        buf.reserve(buf.capacity());
                    } else if err.kind() == ErrorKind::Interrupted {
                        // If we get an interrupted error the operation can be retried
                        // as-is, i.e. we don't need to allocate additional space.
                        continue;
                    } else {
                        buf.truncate(0);
                        return Err(err);
    ...
    /// occurred when reading the message. A return value of 0 indicates that there
    /// were no incoming messages to receive.
    ///
    /// # Errors
    ///
