...
static TIPC_DEFAULT_DEVNAME: &str = "/dev/trusty-ipc-dev0";
static ECHO_NAME: &str = "com.android.ipc-unittest.srv.echo";
...
fn run_echo() {
    let mut connection = TipcStream::connect(TIPC_DEFAULT_DEVNAME, ECHO_NAME)
        .expect("Failed to connect to Trusty service");

    write!(connection, "uwu hewwo?").expect("Failed to write to connection");
