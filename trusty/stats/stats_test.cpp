...
using ::trusty::stats::nw::setter::IStatsSetter;
...
class Stats : public BnStats {
  public:
    ...
    Status reportVendorAtom(const ::VendorAtom& vendorAtom) {
        ...
        mQueueVendorAtom.push_back(vendorAtom);
        mCondVar.notify_one();
        return Status::ok();
    }
    ...
    status_t getVendorAtom(VendorAtom* pVendorAtom, int64_t waitForMs) {
        ...
        *pVendorAtom = mQueueVendorAtom.back();
        mQueueVendorAtom.pop_back();
        return NO_ERROR;
    }
  ...
  private:
    ...
    std::vector<VendorAtom> mQueueVendorAtom;
};
