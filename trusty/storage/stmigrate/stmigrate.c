...
/*
 * stmigrate - For the storage server, migrate file backed storage
 *             to a block device.
 *
 *             For migration, it copies the contents of the file
 *             to the block device, deletes the file, and creates
 *             a symlink to the block device with the original name
 *             of the file.
 *
 *             If migration fails at any point, the file remains
 *             unchanged, however the block device may have been
 *             written to. During the copy, we rename "<file>" to
 *             "<file>.bak" in case of failure, in which case we
 *             rename it back to "<file>"
 *
 * Usage: stmigrate <file name> <block device name>
 */
int main(int argc, char* argv[]) {
    ...
    while (offset < size_file) {
        ...
        tocopy = MIN((size_file - offset), BLK_SIZE);

        if (read(file_fd, buf, tocopy) != tocopy) {
            printf("Error reading from %s - %s\n", argv[1], strerror(errno));
            goto end;
        }

        if (write(blkdev_fd, buf, tocopy) != tocopy) {
    ...
    backup_len = strlen(argv[1]) + strlen(".bak") + 1;
    if (backup_len > PATH_MAX) {
    ...
    snprintf(backup, backup_len, "%s.%s", argv[1], "bak");
