    ...
    switch (sense_key) {
        ...
        case UNIT_ATTENTION:
            ALOGD("UNIT ATTENTION with sense data: key=%hhu, asc=%hhu, ascq=%hhu\n", sense_key,
                  additional_sense_code, additional_sense_code_qualifier);
            if (additional_sense_code == 0 && additional_sense_code_qualifier == 0) {
                /* The kernel has cleared the ASC? */
                return SCSI_RES_RETRY;
            }
            if (additional_sense_code == 0x04 && (additional_sense_code_qualifier == 0x01 ||
                                                  additional_sense_code_qualifier == 0x02)) {
                /*
                 * LOGICAL UNIT IS IN PROCESS OF BECOMING READY or INITIALIZING
                 * COMMAND REQUIRED - the device is not ready and we should try
                 * again.
                 */
                return SCSI_RES_RETRY;
            }
            ...
            if (additional_sense_code == 0x3f && additional_sense_code_qualifier == 0x0e) {
                /*
                 * REPORTED LUNS DATA HAS CHANGED - another device on the same
                 * target might report this status and we end up seeing it on
                 * this device as well.
                 */
                return SCSI_RES_RETRY;
            }
            if (additional_sense_code == 0x2f) {
                /*
                 * COMMANDS CLEARED - our command was cleared by an external
                 * condition, presumably it did not complete.
                 */
                return SCSI_RES_RETRY;
            }
