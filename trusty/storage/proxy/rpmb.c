...
/* Returns zero if the sense data was valid and no errors were present */
static int check_scsi_sense(const uint8_t* sense_buf, size_t len) {
    ...
    switch (sense_key) {
        ...
        case RECOVERED_ERROR:
            ALOGI("Recovered SCSI error: asc=%hhu, ascq=%hhu\n", additional_sense_code,
                  additional_sense_code_qualifier);
            return 0;
    }

    ALOGE("Unexpected SCSI sense: key=%hhu, asc=%hhu, ascq=%hhu\n", sense_key,
...
static void check_sg_io_hdr(const sg_io_hdr_t* io_hdrp) {
    ...
    if (io_hdrp->masked_status != GOOD && io_hdrp->sb_len_wr > 0) {
        int sense_error = check_scsi_sense(io_hdrp->sbp, io_hdrp->sb_len_wr);
        if (sense_error) {
            ALOGE("Unexpected SCSI sense. masked_status: %hhu, host_status: %hu\n",
                  io_hdrp->masked_status, io_hdrp->host_status);
            return;
        }
    }
    ...
    switch (io_hdrp->masked_status) {
        ...
        case CHECK_CONDITION:
            ...
            break;
        default:
            ALOGE("SG_IO failed with masked_status: %hhu\n", io_hdrp->masked_status);
    }

    if (io_hdrp->host_status != 0) {
        ALOGE("SG_IO failed with host_status: %hu\n", io_hdrp->host_status);
    }
}
