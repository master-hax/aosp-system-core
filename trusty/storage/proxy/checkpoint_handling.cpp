...
int is_data_checkpoint_active(bool* active) {
    ...
    size_t checkpointValueEnd = dataEntry->fs_options.find(",", checkpointValueStart);
    if (checkpointValueEnd == std::string::npos) {
        checkpointValueEnd = dataEntry->fs_options.length();
    }

    /*
     * The data entry looks like "... foo,checkpoint=disable:0,bar ..."
     *                                               ^        ^
     *                                               |        |
     *                                               |    checkpointValueEnd
     *                                               |
     *                                        checkpointValueStart
     *
     * We want to know if the checkpoint value is prefixed by "disable", but we
     * only scan up to the first comma (or end of the string).
     */
    size_t checkpointValueSize =
            std::min(checkpointValueEnd - checkpointValueStart, strlen("disable"));

    std::string checkpointValue =
            dataEntry->fs_options.substr(checkpointValueStart, checkpointValueSize);
