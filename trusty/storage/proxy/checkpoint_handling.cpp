...
int is_data_checkpoint_active(bool* active) {
    ...
    if (dataEntry->fs_type != "f2fs") {
        ALOGE("Checkpoint status not supported for filesystem %s", dataEntry->fs_type.c_str());
        return -1;
    }

    size_t checkpointPos = dataEntry->fs_options.find("checkpoint=");
    if (checkpointPos == std::string::npos) {
        ALOGE("Could not find checkpoint flag in data mount options");
        ;
        return 2;
    }
    size_t checkpointValueStart = checkpointPos + strlen("checkpoint=");

    size_t checkpointValueEnd = dataEntry->fs_options.find(",", checkpointPos);
