...
static const char USERDATA_BACKING_FILE[] = "0";
static const char PERSIST_BACKING_FILE[] = "persist/0";
...
int determine_max_file_size(enum MaxFileSizeFrom max_file_size_source) {
    const char* source = NULL;
    switch (max_file_size_source) {
        case UserData:
            source = USERDATA_BACKING_FILE;
            break;
        case Persist:
            source = PERSIST_BACKING_FILE;
            break;
        default:
            ALOGE("Invalid file size source: %d\n", max_file_size_source);
            return -1;
    }

    /* Get full source path for ss source */
    char* full_path = NULL;
    int rc = asprintf(&full_path, "%s/%s", ssdir_name, source);
    if (rc < 0) {
        ALOGE("%s: asprintf failed\n", __func__);
        return -1;
    }
    ...
    const char* possibly_mapped_file = get_backed_storage(source);
...
int storage_init(const char* dirname, const struct file_backing_storage* mappings,
    ...
    int rc = determine_max_file_size(max_file_size_source);
    if (rc < 0) {
        return rc;
    }
