 ...
 * write. */
static int possibly_open_mapped_file(const char* short_path, const char* full_path, int open_flags,
...
int storage_file_open(struct storage_msg* msg, const void* r, size_t req_len,
    ...
    if (req->flags & STORAGE_FILE_OPEN_CREATE) {
        ...
        if (req->flags & STORAGE_FILE_OPEN_CREATE_EXCLUSIVE) {
            ...
            rc = possibly_open_mapped_file(req->name, path, open_flags, &mapping_entry);
