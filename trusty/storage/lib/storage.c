...
int storage_open_file_ext(storage_session_t session, file_handle_t *handle_p, const char *name,
                          uint32_t flags, uint32_t opflags)
{
    ...
    struct storage_msg msg = { .cmd = STORAGE_FILE_OPEN, .flags = msg_flags };
...
void storage_close_file(storage_session_t session, file_handle_t handle)
{
    ...
    struct storage_file_close_req req = { .handle = handle };
    struct iovec tx[2] = {{&msg, sizeof(msg)}, {&req, sizeof(req)}};
    struct iovec rx[1] = {{&msg, sizeof(msg)}};

    ssize_t rc = send_reqv(session, tx, 2, rx, 1);
    rc = check_response(&msg, rc);
    if (rc < 0) {
...
int storage_delete_file_ext(storage_session_t session, const char *name, uint32_t opflags)
{
    uint32_t msg_flags = 0;
    if (opflags & STORAGE_OP_COMPLETE) {
        msg_flags |= STORAGE_MSG_FLAG_TRANSACT_COMPLETE;
    }
    struct storage_msg msg = { .cmd = STORAGE_FILE_DELETE, .flags = msg_flags };
...
static int _read_chunk(storage_session_t session, file_handle_t handle,
                       storage_off_t off, void *buf, size_t size)
{
    struct storage_msg msg = { .cmd = STORAGE_FILE_READ };
    struct storage_file_read_req req = { .handle = handle, .size = size, .offset = off };
    struct iovec tx[2] = {{&msg, sizeof(msg)}, {&req, sizeof(req)}};
    struct iovec rx[2] = {{&msg, sizeof(msg)}, {buf, size}};

    ssize_t rc = send_reqv(session, tx, 2, rx, 2);
    return check_response(&msg, rc);
}
                  ...
                  storage_off_t off, void *buf, size_t size)
{
    ...
    while (size) {
        if (chunk > size)
            chunk = size;
        rc = _read_chunk(session, handle, off, ptr, chunk);
        if (rc < 0)
            return rc;
        if (rc == 0)
            break;
...
static int _write_req(storage_session_t session, file_handle_t handle,
                      storage_off_t off, const void *buf, size_t size,
                      uint32_t msg_flags)
{
    struct storage_msg msg = { .cmd = STORAGE_FILE_WRITE, .flags = msg_flags, };
    struct storage_file_write_req req = { .handle = handle, .offset = off, };
                      ...
                      uint32_t opflags)
{
    ...
    while (size) {
        ...
        rc = _write_req(session, handle, off, ptr, chunk, msg_flags);
        if (rc < 0)
            return rc;
        if ((size_t)rc != chunk) {
                              ...
                              storage_off_t file_size, uint32_t opflags)
{
    ...
    struct storage_msg msg = { .cmd = STORAGE_FILE_SET_SIZE, .flags = msg_flags };
    struct storage_file_set_size_req req = { .handle = handle, .size = file_size, };
    struct iovec tx[2] = {{&msg, sizeof(msg)}, {&req, sizeof(req)}};
    struct iovec rx[1] = {{&msg, sizeof(msg)}};
...
int storage_get_file_size(storage_session_t session, file_handle_t handle,
                           storage_off_t *size_p)
{
    struct storage_msg msg = { .cmd = STORAGE_FILE_GET_SIZE };
    struct storage_file_get_size_req  req = { .handle = handle, };
    struct iovec tx[2] = {{&msg, sizeof(msg)}, {&req, sizeof(req)}};
