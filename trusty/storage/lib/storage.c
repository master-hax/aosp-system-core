...
int storage_read(file_handle_t fh, storage_off_t off, void *buf, size_t size)
{
    int rc;
    int bytes_read = 0;
...
int storage_write_ext(file_handle_t fh, storage_off_t off,
...
int storage_set_file_size_ext(file_handle_t fh, storage_off_t file_size, uint32_t opflags)
{
    uint32_t msg_flags = 0;
    if (opflags & STORAGE_OP_COMPLETE) {
        msg_flags |= STORAGE_MSG_FLAG_TRANSACT_COMPLETE;
