                                       ...
                                       size_t chunk_len, bool complete)
{
    ...
    if (complete)
        ...
        rc = storage_write_ext(handle, off, data_buf, sizeof(data_buf), 0);
...
TEST_P(StorageServiceTest, SetFileSize) {
    ...
    rc = storage_set_file_size(handle, blk/2);
    ASSERT_EQ(0, rc);
    ...
    // set file size to 0
    rc = storage_set_file_size(handle, 0);
    ASSERT_EQ(0, rc);
...
TEST_P(StorageServiceTest, WriteSparse) {
    ...
    rc = storage_write(handle, 1, &val, sizeof(val));
    ASSERT_EQ(-EINVAL, rc);
...
TEST_P(StorageServiceTest, BadFileHnadle) {
    ...
    rc = storage_write(handle1,  0, &val, sizeof(val));
    ASSERT_EQ(-EINVAL, rc);
    ...
    // set size
    rc = storage_set_file_size(handle1,  0);
    ASSERT_EQ(-EINVAL, rc);
...
TEST_P(StorageServiceTest, TransactDiscardSetSize) {
    ...
    rc = storage_set_file_size_ext(handle,  (storage_off_t)exp_len/2, 0);
    ASSERT_EQ(0, rc);
    ...
    rc = storage_set_file_size_ext(handle,  (storage_off_t)exp_len/3, 0);
    ASSERT_EQ(0, rc);
...
TEST_P(StorageServiceTest, TransactRewriteExistingSetSize) {
    ...
    for (uint i = 1; i < 32; i++) {
        ...
        rc = storage_set_file_size(handle, i * blk);
        ASSERT_EQ(0, rc);
    ...
    for (uint i = 1; i < 32; i++) {
        ...
        // update size (with commit)
        rc = storage_set_file_size(handle, (32 - i) * blk);
        ASSERT_EQ(0, rc);
...
TEST_P(StorageServiceTest, TransactResumeAfterNonFatalError) {
    ...
    uint32_t val = 0xDEDBEEF;
    rc = storage_write_ext(handle,  exp_len/2 + 1, &val, sizeof(val), 0);
    ASSERT_EQ(-EINVAL, rc);
    ...
    rc = storage_set_file_size_ext(handle, exp_len/2 + 1, 0);
    ASSERT_EQ(-EINVAL, rc);
...
TEST_P(StorageServiceTest, Transact2_Write_Write_Commit_Commit) {
    ...
    rc = storage_write_ext(handle2, 0, &val, sizeof(val), 0);
    ASSERT_EQ(-EBUSY, rc);

    rc = storage_get_file_size(handle2, &fsize);
    ASSERT_EQ(-EBUSY, rc);

    rc = storage_set_file_size_ext(handle2,  fsize, 0);
    ASSERT_EQ(-EBUSY, rc);
