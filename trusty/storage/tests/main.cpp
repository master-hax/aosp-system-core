...
public:
    ...
    void WriteZeroChunk(file_handle_t handle, storage_off_t off, size_t chunk_len, bool complete, bool aux = false );
    void WritePatternChunk(file_handle_t handle, storage_off_t off, size_t chunk_len, bool complete, bool aux = false);
    void WritePattern(file_handle_t handle, storage_off_t off, size_t data_len, size_t chunk_len, bool complete, bool aux = false);

    void ReadChunk(file_handle_t handle, storage_off_t off, size_t chunk_len,
                   size_t head_len, size_t pattern_len, size_t tail_len, bool aux = false);
    void ReadPattern(file_handle_t handle, storage_off_t off, size_t data_len, size_t chunk_len, bool aux = false);
    void ReadPatternEOF(file_handle_t handle, storage_off_t off, size_t chunk_len, size_t exp_len, bool aux = false);
...
void StorageServiceTest::WriteZeroChunk(file_handle_t handle, storage_off_t off,
                                       size_t chunk_len, bool complete, bool aux )
{
    int rc;
    ...
    if (complete)
        rc = storage_write(aux ? aux_session_ : session_,
                           handle, off, data_buf, sizeof(data_buf));
    else
        rc = storage_write_ext(aux ? aux_session_ : session_,
                               handle, off, data_buf, sizeof(data_buf), 0);
                                      ...
                                      bool aux)
{
    ...
    while (data_len) {
        if (data_len < chunk_len)
            chunk_len = data_len;
        WritePatternChunk(handle, off, chunk_len, (chunk_len == data_len) && complete, aux);
                                   ...
                                   size_t tail_len, bool aux)
{
    ...
    rc = storage_read(aux ? aux_session_ : session_,
                      handle, off, data_buf, chunk_len);
    ASSERT_EQ((int)chunk_len, rc);
...
void StorageServiceTest::ReadPattern(file_handle_t handle, storage_off_t off,
                                     size_t data_len, size_t chunk_len, bool aux)
{
    int rc;
    uint32_t data_buf[chunk_len/sizeof(uint32_t)];
...
void StorageServiceTest::ReadPatternEOF(file_handle_t handle, storage_off_t off,
                                        size_t chunk_len, size_t exp_len, bool aux)
{
    int rc;
    size_t bytes_read = 0;
    uint32_t data_buf[chunk_len/sizeof(uint32_t)];
...
TEST_P(StorageServiceTest, OpenInvalidFileName) {
    ...
    storage_close_file(session_, handle);
    storage_delete_file(session_, max_name);
}
...
TEST_P(StorageServiceTest, BadFileHnadle) {
    ...
    rc = storage_write(session_, handle + 1,  0, &val, sizeof(val));
    ASSERT_EQ(-EINVAL, rc);

    // read from invalid handle
    rc = storage_read(session_, handle + 1,  0, &val, sizeof(val));
    ASSERT_EQ(-EINVAL, rc);

    // set size
    rc = storage_set_file_size(session_, handle + 1,  0);
    ASSERT_EQ(-EINVAL, rc);

    // get size
    storage_off_t fsize = (storage_off_t)(-1);
    rc = storage_get_file_size(session_, handle + 1,  &fsize);
    ASSERT_EQ(-EINVAL, rc);

    // close (there is no way to check errors here)
    storage_close_file(session_, handle + 1);
...
TEST_P(StorageServiceTest, ClosedFileHnadle) {
    ...
    // write to invalid file handle
    uint32_t val = 0xDEDBEEF;
    rc = storage_write(session_, handle1,  0, &val, sizeof(val));
    ASSERT_EQ(-EINVAL, rc);

    // read from invalid handle
    rc = storage_read(session_, handle1,  0, &val, sizeof(val));
    ASSERT_EQ(-EINVAL, rc);

    // set size
    rc = storage_set_file_size(session_, handle1,  0);
    ASSERT_EQ(-EINVAL, rc);
...
TEST_P(StorageServiceTest, TransactCommitWrites) {
    ...
    // write more data (without commit)
    WritePattern(handle, exp_len/2, exp_len/2, blk, false);
    ASSERT_FALSE(HasFatalFailure());
...
TEST_P(StorageServiceTest, TransactRewriteExistingSetSize) {
    ...
    for (uint i = 1; i < 32; i++) {
        // open truncate (no commit)
        rc = storage_open_file_ext(session_, &handle, fname, 0, 0);
        ASSERT_EQ(0, rc);
        ...
        rc = storage_set_file_size(session_, handle, i * blk);
        ASSERT_EQ(0, rc);
    ...
    for (uint i = 1; i < 32; i++) {
        ...
        // update size (with commit)
        rc = storage_set_file_size(session_, handle, (32 - i) * blk);
        ASSERT_EQ(0, rc);
...
TEST_P(StorageServiceTest, TransactResumeAfterNonFatalError) {
    ...
    uint32_t val = 0xDEDBEEF;
    rc = storage_write_ext(session_, handle,  exp_len/2 + 1, &val, sizeof(val), 0);
    ASSERT_EQ(-EINVAL, rc);

    // read past end of file
    rc = storage_read(session_, handle, exp_len/2 + 1, &val, sizeof(val));
    ASSERT_EQ(-EINVAL, rc);

    // try to extend file past end of file
    rc = storage_set_file_size_ext(session_, handle, exp_len/2 + 1, 0);
    ASSERT_EQ(-EINVAL, rc);
    ...
    rc = storage_delete_file(session_, "foo");
    ASSERT_EQ(-ENOENT, rc);
...
TEST_P(StorageServiceTest, Transact2_WriteNC) {
    ...
    // session 2
    WritePattern(handle2, 0, blk, blk, true, true);
    ASSERT_FALSE(HasFatalFailure());

    // read it back
    ReadPatternEOF(handle2, 0, blk, blk, true);
    ASSERT_FALSE(HasFatalFailure());

    // cleanup
    storage_close_file(session_, handle1);
    storage_close_file(aux_session_, handle2);

    storage_delete_file(session_, fname1);
    storage_delete_file(aux_session_, fname2);
}


TEST_P(StorageServiceTest, DISABLED_Transact2_DeleteNC) {
    int rc;
    file_handle_t handle1;
    file_handle_t handle2;
    size_t blk = 2048;
    ...
    // cleanup
    storage_close_file(session_, handle1);
    storage_delete_file_ext(session_, fname1, 0);

    storage_close_file(aux_session_, handle2);
    storage_delete_file_ext(aux_session_, fname2, 0);
...
TEST_P(StorageServiceTest, Transact2_Write_Read) {
    ...
    rc = storage_get_file_size(aux_session_, handle2, &fsize);
    ASSERT_EQ(0, rc);
    ASSERT_EQ((storage_off_t)0, fsize);

    // S2: read it back (should no data)
    ReadPatternEOF(handle2, 0, blk, 0, true);
    ASSERT_FALSE(HasFatalFailure());
    ...
    ReadPatternEOF(handle2, 0, blk, exp_len, true);
    ASSERT_FALSE(HasFatalFailure());
...
TEST_P(StorageServiceTest, Transact2_Write_Write_Commit_Commit) {
    ...
    // S2: write (no commit)
    WritePattern(handle2, 0, exp_len/2, blk, false, true);
    ASSERT_FALSE(HasFatalFailure());
    ...
    uint32_t val = 0;
    rc = storage_read(aux_session_, handle2, 0, &val, sizeof(val));
    ASSERT_EQ(-EBUSY, rc);

    rc = storage_write_ext(aux_session_, handle2, 0, &val, sizeof(val), 0);
    ASSERT_EQ(-EBUSY, rc);

    rc = storage_get_file_size(aux_session_, handle2, &fsize);
    ASSERT_EQ(-EBUSY, rc);

    rc = storage_set_file_size_ext(aux_session_, handle2,  fsize, 0);
    ASSERT_EQ(-EBUSY, rc);

    rc = storage_delete_file_ext(aux_session_, fname, 0);
    ASSERT_EQ(-EBUSY, rc);

    rc = storage_open_file_ext(aux_session_, &handle3, fname,
...
TEST_P(StorageServiceTest, Transact2_Write_Write_Discard_Commit) {
    ...
    // S2: read it again (should be exp_len)
    ReadPatternEOF(handle2, 0, blk, exp_len/2, true);
    ASSERT_FALSE(HasFatalFailure());
