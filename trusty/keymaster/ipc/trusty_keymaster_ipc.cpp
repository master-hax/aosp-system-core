...
std::variant<int, std::vector<uint8_t>> trusty_keymaster_call_2(uint32_t cmd, void* in,
    ...
    nsecs_t timeout_time = 0;
    {
        ...
        if (p == 0) {
            ...
            ALOGW("write for cmd %d is taking more than %ld nsecs", cmd, timeout_ns);
        } else if (p < 0) {
    ...
    if (timeout_time != 0) {
        ...
        ALOGW("write for cmd %d finished after %ld nsecs", cmd,
              write_done - timeout_time + timeout_ns);
    }
    free(msg);
    ...
    while (true) {
        ...
        timeout_time = 0;
        {
            ...
            if (p == 0) {
                timeout_time = systemTime(SYSTEM_TIME_MONOTONIC);
                ALOGW("readv for cmd %d is taking more than %ld nsecs", cmd, timeout_ns);
            } else if (p < 0) {
        ...
        if (timeout_time != 0) {
            ...
            ALOGW("readv for cmd %d finished after %ld nsecs", cmd,
                  read_done - timeout_time + timeout_ns);
        }
        if (rc < 0) {
