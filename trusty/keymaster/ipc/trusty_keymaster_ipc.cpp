...
static const int64_t iotimeout = seconds_to_nanoseconds(10);
...
std::variant<int, std::vector<uint8_t>> trusty_keymaster_call_2(uint32_t cmd, void* in,
    ...
    nsecs_t timeout_time = 0;
    {
        ...
        int p = poll(&pfd, 1, iotimeout);
        if (p == 0) {
            timeout_time = systemTime(SYSTEM_TIME_MONOTONIC);
            ALOGW("write is taking more than %ld nsecs", iotimeout);
        } else if (p < 0) {
    ...
    if (timeout_time != 0) {
        ...
        ALOGW("write finished after %ld nsecs", write_done - timeout_time + iotimeout);
    }
    free(msg);
    ...
    while (true) {
        ...
        timeout_time = 0;
        {
            ...
            if (p == 0) {
                timeout_time = systemTime(SYSTEM_TIME_MONOTONIC);
                ALOGW("readv is taking more than %ld nsecs", iotimeout);
            } else if (p < 0) {
        ...
        if (timeout_time != 0) {
            ...
            ALOGW("readv finished after %ld nsecs", read_done - timeout_time + iotimeout);
        }
        if (rc < 0) {
