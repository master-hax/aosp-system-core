...
static int clear_cert_chain(const xmlChar* algorithm_str) {
    ...
    ret = set_attestation_key_or_cert_bin(KM_CLEAR_ATTESTATION_CERT_CHAIN, algorithm, NULL, 0);
    printf("algorithm %s, clear cert chain done, ret %d\n", algorithm_str, ret);
    return 0; /* skip error, not yet implemented */
    return ret;
}
