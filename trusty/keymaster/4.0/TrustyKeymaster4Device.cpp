...
Return<void> TrustyKeymaster4Device::computeSharedHmac(
    ...
    ComputeSharedHmacRequest request(impl_->MessageVersion());
...
Return<void> TrustyKeymaster4Device::verifyAuthorization(
    ...
    VerifyAuthorizationRequest request(impl_->MessageVersion());
...
Return<ErrorCode> TrustyKeymaster4Device::addRngEntropy(const hidl_vec<uint8_t>& data) {
    ...
    AddEntropyRequest request(impl_->MessageVersion());
    request.random_data.Reinitialize(data.data(), data.size());

    AddEntropyResponse response(impl_->MessageVersion());
...
Return<void> TrustyKeymaster4Device::generateKey(const hidl_vec<KeyParameter>& keyParams,
    ...
    GenerateKeyRequest request(impl_->MessageVersion());
    request.key_description.Reinitialize(KmParamSet(keyParams));

    GenerateKeyResponse response(impl_->MessageVersion());
...
Return<void> TrustyKeymaster4Device::getKeyCharacteristics(const hidl_vec<uint8_t>& keyBlob,
    ...
    GetKeyCharacteristicsRequest request(impl_->MessageVersion());
    request.SetKeyMaterial(keyBlob.data(), keyBlob.size());
    addClientAndAppData(clientId, appData, &request.additional_params);

    GetKeyCharacteristicsResponse response(impl_->MessageVersion());
...
Return<void> TrustyKeymaster4Device::importKey(const hidl_vec<KeyParameter>& params,
    ...
    ImportKeyRequest request(impl_->MessageVersion());
    request.key_description.Reinitialize(KmParamSet(params));
    request.key_format = legacy_enum_conversion(keyFormat);
    request.SetKeyMaterial(keyData.data(), keyData.size());

    ImportKeyResponse response(impl_->MessageVersion());
...
Return<void> TrustyKeymaster4Device::importWrappedKey(
    ...
    ImportWrappedKeyRequest request(impl_->MessageVersion());
    ...
    ImportWrappedKeyResponse response(impl_->MessageVersion());
...
Return<void> TrustyKeymaster4Device::exportKey(KeyFormat exportFormat,
    ...
    ExportKeyRequest request(impl_->MessageVersion());
    request.key_format = legacy_enum_conversion(exportFormat);
    request.SetKeyMaterial(keyBlob.data(), keyBlob.size());
    addClientAndAppData(clientId, appData, &request.additional_params);

    ExportKeyResponse response(impl_->MessageVersion());
...
Return<void> TrustyKeymaster4Device::upgradeKey(const hidl_vec<uint8_t>& keyBlobToUpgrade,
    ...
    UpgradeKeyRequest request(impl_->MessageVersion());
    request.SetKeyMaterial(keyBlobToUpgrade.data(), keyBlobToUpgrade.size());
    request.upgrade_params.Reinitialize(KmParamSet(upgradeParams));

    UpgradeKeyResponse response(impl_->MessageVersion());
...
Return<ErrorCode> TrustyKeymaster4Device::deleteKey(const hidl_vec<uint8_t>& keyBlob) {
    DeleteKeyRequest request(impl_->MessageVersion());
    request.SetKeyMaterial(keyBlob.data(), keyBlob.size());

    DeleteKeyResponse response(impl_->MessageVersion());
...
Return<ErrorCode> TrustyKeymaster4Device::deleteAllKeys() {
    DeleteAllKeysRequest request(impl_->MessageVersion());
    DeleteAllKeysResponse response(impl_->MessageVersion());
...
Return<void> TrustyKeymaster4Device::begin(KeyPurpose purpose, const hidl_vec<uint8_t>& key,
    ...
    BeginOperationRequest request(impl_->MessageVersion());
    request.purpose = legacy_enum_conversion(purpose);
    request.SetKeyMaterial(key.data(), key.size());
    request.additional_params.Reinitialize(KmParamSet(extendedParams));

    BeginOperationResponse response(impl_->MessageVersion());
...
Return<void> TrustyKeymaster4Device::update(uint64_t operationHandle,
    ...
    UpdateOperationRequest request(impl_->MessageVersion());
    UpdateOperationResponse response(impl_->MessageVersion());
    hidl_vec<KeyParameter> resultParams;
...
Return<void> TrustyKeymaster4Device::finish(uint64_t operationHandle,
                                            ...
                                            finish_cb _hidl_cb) {
    (void)verificationToken;
    FinishOperationRequest request(impl_->MessageVersion());
    hidl_vec<KeyParameter> extendedParams = injectAuthToken(inParams, authToken);
    request.op_handle = operationHandle;
    ...
    FinishOperationResponse response(impl_->MessageVersion());
...
Return<ErrorCode> TrustyKeymaster4Device::abort(uint64_t operationHandle) {
    AbortOperationRequest request(impl_->MessageVersion());
    request.op_handle = operationHandle;

    AbortOperationResponse response(impl_->MessageVersion());
