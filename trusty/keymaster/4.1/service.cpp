...
int main() {
    ...
    int err = trustyKeymaster->Initialize(keymaster::KmVersion::KEYMASTER_4_1);
    if (err != 0) {
        LOG(FATAL) << "Could not initialize TrustyKeymaster (" << err << ")";
        return -1;
    }

    IKeymasterDevice* keymaster = new ::keymaster::V4_1::TrustyKeymaster41Device(trustyKeymaster);
