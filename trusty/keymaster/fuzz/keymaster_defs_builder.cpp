...
// Builds an AuthSet with a set of tags specified by the given invariant then optionally append
// AuthSet tags provided by the proto test case.
void BuildAuthorizationSet(__attribute__((unused)) const fuzz::AuthorizationSet& proto_set,
                           keymaster::AuthorizationSet& set, Invariant invariant) {
    switch (invariant) {
        case Invariant::BeginOp: {
            // TODO: Add TAG_APPLICATION_ID and TAG_APPLICATION_DATA to the AuthorizationSet
            set = keymaster::AuthorizationSetBuilder()
                          .Digest(KM_DIGEST_NONE)
                          .Padding(KM_PAD_NONE)
                          .build();
            // Optionally append extra tags provided by the proto test case
            // TODO: Ensure that at least some begin op commands succeed before appending the proto
            // set by uncommenting the next line
