...
// Creates an instance of `Message` for keymaster commands given the common prefix of the keymaster
// request and response names and the command code.
#define INST_MESSAGE(command, code)                     \
    template <>                                         \
...
INST_MESSAGE(GenerateKey, 0);
INST_MESSAGE(BeginOperation, 1);
INST_MESSAGE(UpdateOperation, 2);
INST_MESSAGE(FinishOperation, 3);
