...
// lowercase) names.
//
// unique_fd is also known as ScopedFd/ScopedFD/scoped_fd; mentioned here to
// help you find this class if you're searching for one of those names.
//
...
struct DefaultCloser {
    ...
    static void Tag(int fd, void* old_addr, void* new_addr) {
        if (android_fdsan_exchange_owner_tag) {
            uint64_t old_tag = android_fdsan_create_owner_tag(
                    ANDROID_FDSAN_OWNER_TYPE_UNIQUE_FD,
                    reinterpret_cast<uint64_t>(old_addr));
            uint64_t new_tag = android_fdsan_create_owner_tag(
                    ANDROID_FDSAN_OWNER_TYPE_UNIQUE_FD,
...
template <typename Closer>
inline bool Pipe(unique_fd_impl<Closer>* read,
...
template <typename Closer>
inline bool Socketpair(int domain,
                       int type,
                       int protocol,
...
// This helper hides the details of converting to unique_fd.
template <typename Closer>
inline bool Socketpair(int type,
                       unique_fd_impl<Closer>* left,
                       unique_fd_impl<Closer>* right) {
...
struct borrowed_fd {
    ...
    template <typename T>
    /* implicit */ borrowed_fd(const unique_fd_impl<T>& ufd)
            : fd_(ufd.get()) {}  // NOLINT
