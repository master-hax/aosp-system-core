...
/* Set to 1 to dump to the log the states that needs a new
...
static shortcut_t shortcuts[] = {
...
#if 1 /* XXX: EXPERIMENTAL - REMOVE ME */
    ...
    /* THIS IS A CHEAT, we ignore dithering during clamp+modulation */
    /* This mode is mainly used during animations, and dithering really kills performance */
    { { { 0x03515104, 0x00000177, { 0x00001001, 0x00000000 } },
        { 0xFFFFFFFF, 0xFFFFFFFF, { 0xFFFFFFFF, 0x0000003F } } },
        "565 fb, 8888 tx, SRC_OVER clamp modulate dither", scanline_t32cb16blend_clamp_mod, init_y },
    /* AND THIS ONE IS THE SAME CHEAT for xRGB textures */
    { { { 0x03515104, 0x00000177, { 0x00001002, 0x00000000 } },
        { 0xFFFFFFFF, 0xFFFFFFFF, { 0xFFFFFFFF, 0x0000003F } } },
        "565 fb, x888 tx, SRC_OVER clamp modulate dither", scanline_x32cb16blend_clamp_mod, init_y },
...
struct blender_32to16 {
    ...
    void blend(uint32_t s, uint16_t* dst) {
        if (s == 0)
            return;
...
struct blender_16to16_modulate : blender_modulate {
    ...
    void blend(uint16_t s16, uint16_t* dst) {
        uint32_t  s = s16;
...
/* This is used to perform dithering operations. Due to the way dithering
 * works, it includes normal writes and blends (SRC_OVER).
 * we don't put this into a blender object.
 */
struct ditherer_32to16 {
    ditherer_32to16(const context_t* c) {
        const int x = c->iterators.xl;
        const int y = c->iterators.y;
        m_dither_line = &c->ditherMatrix[ ((y & GGL_DITHER_MASK)<<GGL_DITHER_ORDER_SHIFT) ];
        m_dither_index = x & GGL_DITHER_MASK;
    }
    uint16_t write(uint32_t  s) {
        /* Assume s is 32-bit ABGR */
        uint32_t r = s & 0xff;
        uint32_t g = (s >> 8) & 0xff;
        uint32_t b = (s >> 16) & 0xff;
        return write(r, g, b);
    }
    uint16_t write(uint32_t& r, uint32_t& g, uint32_t &b) {
        int dither = m_dither_line[m_dither_index];
        m_dither_index = (m_dither_index+1) & GGL_DITHER_MASK;
        /* dither in on GGL_DITHER_BITS, and each of r, g, b is on 8 bits */
        r += (dither >> (GGL_DITHER_BITS-8 +5));
        g += (dither >> (GGL_DITHER_BITS-8 +6));
        b += (dither >> (GGL_DITHER_BITS-8 +5));
        if (r > 0xff)
            r = 0xff;
        if (g > 0xff)
            g = 0xff;
    ...
    void blend(uint32_t s, uint16_t* dst) {
        ...
        if (sA == 0xff) {
            *dst = write(s);
            ...
            dither <<= (8 - GGL_DITHER_BITS);
            sR = ((sR << 8) + (f*dR) + dither) >> 8;
            sG = ((sG << 8) + (f*dG) + dither) >> 8;
            sB = ((sB << 8) + (f*dB) + dither) >> 8;
...
static void scanline_t32cb16_dither(context_t* c)
{
    ...
    dst_iterator16  di(c);
    ditherer_32to16  dither(c);

    while (di.count--) {
        uint32_t s = si.get_pixel32();
        *di.dst++ = dither.write(s);
    }
}
...
{
    dst_iterator16 di(c);
    ditherer_32to16 ditherer(c);

    horz_iterator32  hi(c);
    while (di.count--) {
        uint32_t s = hi.get_pixel32();
        ditherer.blend(s, di.dst);
        di.dst++;
    }
}
