...
Charger::Charger(ChargerHalHealthLoopInterface* hal_health_loop)
    : batt_anim_(BASE_ANIMATION), hal_health_loop_(hal_health_loop) {}
...
void Charger::UpdateScreenState(int64_t now) {
    ...
    if (healthd_draw_ == nullptr) {
        std::optional<bool> out_screen_on = hal_health_loop_->ChargerShouldKeepScreenOn();
        if (out_screen_on.has_value()) {
            if (!*out_screen_on) {
                LOGV("[%" PRId64 "] leave screen off\n", now);
                batt_anim_.run = false;
                next_screen_transition_ = -1;
                if (hal_health_loop_->ChargerIsOnline()) request_suspend(true);
                return;
            }
        }
    ...
    if (batt_anim_.cur_frame == 0) {
        ...
        if (health_info_.battery_level >= 0 && batt_anim_.num_frames != 0) {
            ...
            if (hal_health_loop_->ChargerIsOnline()) {
...
void Charger::OnInit(struct healthd_config* config, void* cookie) {
    int ret;
    int i;
    int epollfd;
    ...
    if (!ret) {
        ...
        hal_health_loop_->ChargerRegisterEvent(epollfd, &charger_event_handler, EVENT_WAKEUP_FD);
    }
    ...
    hal_health_loop_->ChargerInitConfig(config, cookie);
