...
void Charger::UpdateScreenState(int64_t now) {
    ...
    if (healthd_draw_ == nullptr) {
        ...
        if (android::sysprop::ChargerProperties::disable_init_blank().value_or(false)) {
            healthd_draw_->blank_screen(true, (int)drm_);
    ...
    if (screen_switch_ == SCREEN_SWITCH_ENABLE) {
        healthd_draw_->blank_screen(true, 0);
        healthd_draw_->blank_screen(true, 1);
        screen_blanked_ = true;
        screen_switch_ = SCREEN_SWITCH_DISABLE;
    }

    if (screen_blanked_) {
        healthd_draw_->blank_screen(false, (int)drm_);
...
int Charger::SetSwCallback(int code, int value) {
    ...
    if (code == SW_LID) {
        if ((screen_switch_ == SCREEN_SWITCH_DEFAULT) || (value && (drm_ == DRM_INNER)) ||
            (!value && (drm_ == DRM_OUTER))) {
            screen_switch_ = SCREEN_SWITCH_ENABLE;
            drm_ = value ? DRM_OUTER : DRM_INNER;
            keys_[code].pending = true;
        }
    }
