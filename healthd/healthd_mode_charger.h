...
class ChargerHalHealthLoopInterface {
  public:
    virtual ~ChargerHalHealthLoopInterface() = default;
    virtual std::optional<bool> ChargerShouldKeepScreenOn() = 0;
    virtual bool ChargerIsOnline() = 0;
    virtual void ChargerInitConfig(healthd_config* config, void* cookie) = 0;
    using BoundFunction =
...
class Charger {
  public:
    explicit Charger(ChargerHalHealthLoopInterface* hal_health_loop);
    virtual ~Charger();

    // Hooks for ChargerHalHealthLoopInterface
    void OnHeartbeat();
    int OnPrepareToWait();
    // |cookie| is passed to ChargerHalHealthLoopInterface::ChargerInitConfig
    void OnInit(struct healthd_config* config, void* cookie);
  ...
  private:
    ...
    ChargerHalHealthLoopInterface* hal_health_loop_;
};
}  // namespace android
