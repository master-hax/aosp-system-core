...
int BatteryMonitor::setChargingPolicy(int value) {
    bool result;
    int ret = 0;
    ...
    // ret: 0: NAME_NOT_FOUND, 1: operation successed, -1: operation failed
    return ret;
}
...
status_t BatteryMonitor::setProperty(int id, int value) {
    ...
    switch (id) {
    case BATTERY_PROP_CHARGING_POLICY:
        result = setChargingPolicy(value);
        if (!result) ret = (result > 0) ? OK : BAD_VALUE;
        break;
