...
template <typename T>
static std::optional<T> mapSysfsString(const char* str,
                        SysfsStringEnumMap<T> map[]) {
...
void BatteryMonitor::updateValues(void) {
    *mHealthInfo = HealthInfo_2_1{};
    ...
    if (!mHealthdConfig->batteryCurrentAvgPath.isEmpty())
        mHealthInfo->legacy.batteryCurrentAverage = getIntField(mHealthdConfig->batteryCurrentAvgPath);
