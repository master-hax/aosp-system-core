...
ChargerLoop::ChargerLoop(Charger* charger) :
    HalHealthLoop(charger),
    charger_(charger) {}
...
void Charger::UpdateScreenState(int64_t now) {
    ...
    if (healthd_draw_ == nullptr) {
        if (!ScreenOn(health_info_)) {
            LOGV("[%" PRId64 "] leave screen off\n", now);
            batt_anim_.run = false;
            next_screen_transition_ = -1;
            if (charger_connected_) request_suspend(true);
            return;
        }
...
void Charger::SetNextKeyCheck(key_state* key, int64_t timeout) {
    ...
    if (next_key_check_ == -1 || then < next_key_check_)
        next_key_check_ = then;
}
...
void ChargerLoop::Heartbeat() {
    // charger* charger = &charger_state;
    int64_t now = curr_time_ms();

    charger_->HandleInputState(now);
    charger_->HandlePowerSupplyState(now);

    /* do screen update last in case any of the above want to start
     * screen transitions (animations, etc)
     */
    charger_->UpdateScreenState(now);
}

void Charger::OnHealthInfoUpdate(const HealthInfo_2_1& health_info) {
    const auto& props = health_info.legacy.legacy;

    // TODO: Use BatteryMonitor::isChargerOnline;
    charger_connected_ =
...
int ChargerLoop::PrepareToWait(void) {
    int64_t now = curr_time_ms();
    int64_t next_event = INT64_MAX;
    int64_t timeout;

    LOGV("[%" PRId64 "] next screen: %" PRId64 " next key: %" PRId64 " next pwr: %" PRId64 "\n",
         now, charger_->next_screen_transition_, charger_->next_key_check_, charger_->next_pwr_check_);

    if (charger_->next_screen_transition_ != -1) next_event = charger_->next_screen_transition_;
    if (charger_->next_key_check_ != -1 && charger_->next_key_check_ < next_event)
        next_event = charger_->next_key_check_;
    if (charger_->next_pwr_check_ != -1 && charger_->next_pwr_check_ < next_event)
        next_event = charger_->next_pwr_check_;
...
void Charger::InitAnimation() {
    ...
    LOGV("  animation: %d %d '%s' (%d)\n", batt_anim_.num_cycles,
         batt_anim_.first_frame_repeats, batt_anim_.animation_file.c_str(),
         batt_anim_.num_frames);
    LOGV("  fail_file: '%s'\n", batt_anim_.fail_file.c_str());
    LOGV("  clock: %d %d %d %d %d %d '%s'\n", batt_anim_.text_clock.pos_x,
         batt_anim_.text_clock.pos_y, batt_anim_.text_clock.color_r,
         batt_anim_.text_clock.color_g, batt_anim_.text_clock.color_b,
         batt_anim_.text_clock.color_a, batt_anim_.text_clock.font_file.c_str());
...
void Charger::Init(HealthLoop* loop, struct healthd_config* config) {
    int ret;
    int i;
    int epollfd;
    ...
    ret = ev_init(std::bind(&Charger::InputCallback, this, std::placeholders::_1, std::placeholders::_2));
    if (!ret) {
        epollfd = ev_get_epollfd();
        loop->RegisterEvent(epollfd, &charger_event_handler, EVENT_WAKEUP_FD);
    }
    ...
    ret = res_create_multi_display_surface(batt_anim_.animation_file.c_str(), &scale_count, &scale_fps,
                                           &scale_frames);
    if (ret < 0) {
    ...
    ev_sync_key_state(
        std::bind(&Charger::SetKeyCallback, this, std::placeholders::_1, std::placeholders::_2));
    ...
    // Initializes the internal BatteryMonitor.
    Health::Init(loop, config);
...
int healthd_charger_main(int argc, char** argv) {
    ...
    sp<Charger> charger = new Charger("charger");
    ChargerLoop loop(charger.get());
