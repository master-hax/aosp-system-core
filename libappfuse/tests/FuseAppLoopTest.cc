...
class Callback : public FuseAppLoop::Callback {
 public:
  ...
  int32_t OnWrite(uint64_t inode, uint64_t /*offset*/, uint32_t /*size*/,
  ...
  int32_t OnRead(
      uint64_t inode, uint64_t /*offset*/, uint32_t /*size*/,
      void* /*data*/) override {
    requests.push_back({
      .code = FUSE_READ,
      .inode = inode
...
class FuseAppLoopTest : public ::testing::Test {
 ...
 protected:
  ...
  void SetUp() override {
    ...
    ASSERT_EQ(0, socketpair(AF_UNIX, SOCK_SEQPACKET, 0, sockets_));
    thread_ = std::thread([this] {
      FuseAppLoop loop;
      loop.Start(sockets_[1], &callback_);
    });
  }
