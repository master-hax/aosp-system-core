...
const char MetricsCollectors::kComprDataSizeName[] = "compr_data_size";
const char MetricsCollectors::kOrigDataSizeName[] = "orig_data_size";
const char MetricsCollectors::kZeroPagesName[] = "zero_pages";
...
// static
double MetricsCollectors::GetActiveTime() {
  struct timespec ts;
...
uint32_t MetricsCollectors::GetOsVersionHash() {
...
void MetricsCollectors::Init(bool testing,
...
int MetricsCollectors::OnInit() {
  ...
  if (bus_->is_connected()) {
    ...
    bus_->AddFilterFunction(&MetricsCollectors::MessageFilter, this);
  ...
  device_ = weaved::Device::CreateInstance(
      bus_,
      base::Bind(&MetricsCollectors::UpdateWeaveState, base::Unretained(this)));
  device_->AddCommandHandler(
      "_metrics._enableAnalyticsReporting",
      base::Bind(&MetricsCollectors::OnEnableMetrics, base::Unretained(this)));
  device_->AddCommandHandler(
      "_metrics._disableAnalyticsReporting",
      base::Bind(&MetricsCollectors::OnDisableMetrics, base::Unretained(this)));

  latest_cpu_use_microseconds_ = cpu_usage_collector_->GetCumulativeCpuUse();
  base::MessageLoop::current()->PostDelayedTask(FROM_HERE,
      base::Bind(&MetricsCollectors::HandleUpdateStatsTimeout,
...
void MetricsCollectors::OnShutdown(int* return_code) {
  if (!testing_ && bus_->is_connected()) {
    ...
    bus_->RemoveFilterFunction(&MetricsCollectors::MessageFilter, this);
...
void MetricsCollectors::OnEnableMetrics(
...
void MetricsCollectors::OnDisableMetrics(
    const std::weak_ptr<weaved::Command>& cmd) {
  auto command = cmd.lock();
  if (!command)
    return;
...
void MetricsCollectors::UpdateWeaveState() {
  if (!device_)
    return;
...
// static
DBusHandlerResult MetricsCollectors::MessageFilter(DBusConnection* connection,
                                                   DBusMessage* message,
                                                   void* user_data) {
  ...
  MetricsCollectors* daemon = static_cast<MetricsCollectors*>(user_data);
...
void MetricsCollectors::ProcessUserCrash() {
...
void MetricsCollectors::ProcessKernelCrash() {
  // Counts the active time up to now.
  UpdateStats(TimeTicks::Now(), Time::Now());
...
void MetricsCollectors::ProcessUncleanShutdown() {
  // Counts the active time up to now.
  UpdateStats(TimeTicks::Now(), Time::Now());
...
bool MetricsCollectors::CheckSystemCrash(const string& crash_file) {
...
void MetricsCollectors::StatsReporterInit() {
...
void MetricsCollectors::ScheduleMeminfoCallback(int wait) {
  if (testing_) {
    return;
  }
  base::TimeDelta waitDelta = base::TimeDelta::FromSeconds(wait);
  base::MessageLoop::current()->PostDelayedTask(FROM_HERE,
      base::Bind(&MetricsCollectors::MeminfoCallback, base::Unretained(this),
                 waitDelta),
      waitDelta);
}

void MetricsCollectors::MeminfoCallback(base::TimeDelta wait) {
  string meminfo_raw;
...
// static
bool MetricsCollectors::ReadFileToUint64(const base::FilePath& path,
                                         uint64_t* value) {
  std::string content;
...
bool MetricsCollectors::ReportZram(const base::FilePath& zram_dir) {
...
bool MetricsCollectors::ProcessMeminfo(const string& meminfo_raw) {
...
bool MetricsCollectors::FillMeminfo(const string& meminfo_raw,
...
void MetricsCollectors::ScheduleMemuseCallback(double interval) {
  if (testing_) {
    return;
  }
  base::MessageLoop::current()->PostDelayedTask(FROM_HERE,
      base::Bind(&MetricsCollectors::MemuseCallback, base::Unretained(this)),
      base::TimeDelta::FromSeconds(interval));
}

void MetricsCollectors::MemuseCallback() {
...
bool MetricsCollectors::MemuseCallbackWork() {
  string meminfo_raw;
  const FilePath meminfo_path(kMeminfoFileName);
  if (!base::ReadFileToString(meminfo_path, &meminfo_raw)) {
    LOG(WARNING) << "cannot read " << meminfo_path.value().c_str();
...
bool MetricsCollectors::ProcessMemuse(const string& meminfo_raw) {
  static const MeminfoRecord fields_array[] = {
    { "MemTotal", "MemTotal" },  // SPECIAL CASE: total system memory
    { "ActiveAnon", "Active(anon)" },
    { "InactiveAnon", "Inactive(anon)" },
...
void MetricsCollectors::SendSample(const string& name, int sample,
                                   int min, int max, int nbuckets) {
  metrics_lib_->SendToUMA(name, sample, min, max, nbuckets);
}

void MetricsCollectors::SendKernelCrashesCumulativeCountStats() {
...
void MetricsCollectors::SendAndResetDailyUseSample(
...
void MetricsCollectors::SendAndResetCrashIntervalSample(
...
void MetricsCollectors::SendAndResetCrashFrequencySample(
...
void MetricsCollectors::SendLinearSample(const string& name, int sample,
...
void MetricsCollectors::UpdateStats(TimeTicks now_ticks,
                                    Time now_wall_time) {
...
void MetricsCollectors::HandleUpdateStatsTimeout() {
  UpdateStats(TimeTicks::Now(), Time::Now());
  base::MessageLoop::current()->PostDelayedTask(FROM_HERE,
      base::Bind(&MetricsCollectors::HandleUpdateStatsTimeout,
                 base::Unretained(this)),
