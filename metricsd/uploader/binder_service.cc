...
BinderService::BinderService(const std::shared_ptr<CrashCounters>& counters)
    : counters_(counters) {
  CHECK(counters_);
}
...
Status BinderService::recordLinearHistogram(const String16& name,
                                            int sample,
                                            int max) {
...
Status BinderService::recordCrash(const String16& type) {
