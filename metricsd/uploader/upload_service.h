...
// and upload them periodically to the server.
//
// A given metrics sample can be in one of three location.
...
// * staged log: protobug message waiting to be uploaded.
//
...
//
// Periodically (every |upload_interval| seconds):
// * we take a snapshot of the in-memory metrics and create the staged log
// * save the staged log to disk to avoid losing it if metricsd crashes between
//   two uploads.
