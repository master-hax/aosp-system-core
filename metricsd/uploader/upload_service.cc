...
void UploadService::UploadEvent() {
  // If the system shutdown or crash while uploading a report, we may not have
  // deleted an old log.
  RemoveFailedLog();
...
void UploadService::SendStagedLog() {
  ...
  // Increase the failed count in case the daemon crash while sending the log.
...
void UploadService::StageCurrentLog() {
  ...
  if (!staged_log->PopulateSystemProfile(system_profile_setter_.get())) {
    LOG(INFO) << "Error while adding metadata to the log. Discarding the "
                 << "log.";
    return;
  }
  ...
  failed_upload_count_.Set(0);
  if (int(encoded_log.size()) != base::WriteFile(staged_log_path_, encoded_log.data(),
                      encoded_log.size())) {
    LOG(INFO) << "failed to persist to " << staged_log_path_.value();
  }
}
