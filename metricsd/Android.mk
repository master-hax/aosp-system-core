...
metrics_collectors_shared_libraries := $(libmetrics_shared_libraries) \
  libbrillo-dbus \
  libbrillo-http \
  libchrome-dbus \
  libdbus \
...
# metrics_collectors daemon.
# ========================================================
include $(CLEAR_VARS)
LOCAL_MODULE := metrics_collectors
LOCAL_C_INCLUDES := $(metrics_includes)
LOCAL_CFLAGS := $(metrics_CFLAGS)
LOCAL_CLANG := true
LOCAL_CPP_EXTENSION := $(metrics_cpp_extension)
LOCAL_CPPFLAGS := $(metrics_CPPFLAGS)
LOCAL_INIT_RC := metrics_collectors.rc
LOCAL_REQUIRED_MODULES := \
  metrics.json \
  metrics.schema.json
LOCAL_RTTI_FLAG := -frtti
LOCAL_SHARED_LIBRARIES := $(metrics_collectors_shared_libraries)
LOCAL_SRC_FILES := $(metrics_collectors_common) \
  metrics_collectors_main.cc
include $(BUILD_EXECUTABLE)
...
# Unit tests for metrics_collectors.
# ========================================================
include $(CLEAR_VARS)
...
LOCAL_RTTI_FLAG := -frtti
LOCAL_SHARED_LIBRARIES := $(metrics_collectors_shared_libraries)
LOCAL_SRC_FILES := $(metrics_collectors_tests_sources) \
  $(metrics_collectors_common)
