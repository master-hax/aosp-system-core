...
void MetricsCollector::Init(bool testing, MetricsLibraryInterface* metrics_lib,
  ...
  daily_active_use_.reset(
      new PersistentInteger("Platform.UseTime.PerDay", private_metrics_directory));
  version_cumulative_active_use_.reset(
      new PersistentInteger("Platform.CumulativeUseTime", private_metrics_directory));
  version_cumulative_cpu_use_.reset(
      new PersistentInteger("Platform.CumulativeCpuTime", private_metrics_directory));
  ...
  user_crash_interval_.reset(
      new PersistentInteger("Platform.UserCrashInterval", private_metrics_directory));

  any_crashes_daily_count_.reset(
      new PersistentInteger("Platform.AnyCrashes.PerDay", private_metrics_directory));
  any_crashes_weekly_count_.reset(
      new PersistentInteger("Platform.AnyCrashes.PerWeek", private_metrics_directory));
  user_crashes_daily_count_.reset(
      new PersistentInteger("Platform.UserCrashes.PerDay", private_metrics_directory));
  ...
  daily_cycle_.reset(new PersistentInteger("daily.cycle", private_metrics_directory));
  weekly_cycle_.reset(
...
void MetricsCollector::OnEnableMetrics(
  ...
  if (base::WriteFile(
      shared_metrics_directory_.Append(metrics::kConsentFileName),
      "", 0) != 0) {
...
void MetricsCollector::OnDisableMetrics(
  ...
  if (!base::DeleteFile(shared_metrics_directory_.Append(metrics::kConsentFileName),
                        false)) {
