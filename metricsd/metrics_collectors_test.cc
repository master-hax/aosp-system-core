...
class MetricsCollectorsTest : public testing::Test {
 protected:
  virtual void SetUp() {
  ...
  // The MetricsCollectors under test.
  MetricsCollectors daemon_;
...
TEST_F(MetricsCollectorsTest, MessageFilter) {
  // Ignore calls to SendToUMA.
  EXPECT_CALL(metrics_lib_, SendToUMA(_, _, _, _, _)).Times(AnyNumber());

  DBusMessage* msg = dbus_message_new(DBUS_MESSAGE_TYPE_METHOD_CALL);
  DBusHandlerResult res =
      MetricsCollectors::MessageFilter(/* connection */ nullptr, msg, &daemon_);
  ...
  msg = NewDBusSignalString("/",
                            ...
                            "UserCrash",
                            signal_args);
  res = MetricsCollectors::MessageFilter(/* connection */ nullptr, msg, &daemon_);
...
TEST_F(MetricsCollectorsTest, SendSample) {
  ExpectSample("Dummy.Metric", 3);
  daemon_.SendSample("Dummy.Metric", /* sample */ 3,
                     /* min */ 1, /* max */ 100, /* buckets */ 50);
}

TEST_F(MetricsCollectorsTest, ProcessMeminfo) {
  string meminfo =
      "MemTotal:        2000000 kB\nMemFree:          500000 kB\n"
      "Buffers:         1000000 kB\nCached:           213652 kB\n"
      "SwapCached:            0 kB\nActive:           133400 kB\n"
...
TEST_F(MetricsCollectorsTest, ProcessMeminfo2) {
  string meminfo = "MemTotal:        2000000 kB\nMemFree:         1000000 kB\n";
  // Not enough fields.
  EXPECT_FALSE(daemon_.ProcessMeminfo(meminfo));
}

TEST_F(MetricsCollectorsTest, SendZramMetrics) {
  ...
  CreateUint64ValueFile(
      temp_dir_.path().Append(MetricsCollectors::kComprDataSizeName),
      compr_data_size);
  CreateUint64ValueFile(
      temp_dir_.path().Append(MetricsCollectors::kOrigDataSizeName),
      orig_data_size);
  CreateUint64ValueFile(
      temp_dir_.path().Append(MetricsCollectors::kZeroPagesName), zero_pages);
