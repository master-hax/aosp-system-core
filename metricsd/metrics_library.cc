...
#include "android/brillo/metrics/IMetricsService.h"
#include "constants.h"

static const char kCrosEventHistogramName[] = "Platform.CrOSEvent";
static const int kCrosEventHistogramMax = 100;
static const char kMetricsServiceName[] =
    "android.brillo.metrics.IMetricsService";
...
using android::brillo::metrics::IMetricsService;
...
bool MetricsLibrary::CheckService() {
  if (proxy_.get() && android::IInterface::asBinder(proxy_)->isBinderAlive())
    return true;

  String16 name(kMetricsServiceName);
  proxy_ = android::interface_cast<IMetricsService>(
...
bool MetricsLibrary::SendToUMA(
  ...
  return CheckService() &&
         proxy_->recordHistogram(String16(name.c_str()), sample, min, max,
                                 nbuckets)
             .isOk();
}

bool MetricsLibrary::SendEnumToUMA(const std::string& name, int sample, int max) {
  return CheckService() &&
         proxy_->recordLinearHistogram(String16(name.c_str()), sample, max)
             .isOk();
}

bool MetricsLibrary::SendBoolToUMA(const std::string& name, bool sample) {
  return CheckService() &&
         proxy_->recordLinearHistogram(String16(name.c_str()), sample ? 1 : 0, 2)
             .isOk();
}

bool MetricsLibrary::SendSparseToUMA(const std::string& name, int sample) {
  return CheckService() &&
         proxy_->recordSparseHistogram(String16(name.c_str()), sample).isOk();
}
...
bool MetricsLibrary::SendCrashToUMA(const char* crash_kind) {
  return CheckService() && proxy_->recordCrash(String16(crash_kind)).isOk();
}
