...
inFastboot() {
  fastboot devices | grep "^${ANDROID_SERIAL}[ 	]" > /dev/null
}
...
Returns true if current return value is true and the result matches" ]
EXPECT_PROPERTY() {
  save_ret=${?}
  property="${1}"
  value="${2}"
...
[ "USAGE: report_bootstat_logs <expected_property_value>
...
report_bootstat_logs() {
  ...
  adb logcat -b all -d |
  grep bootstat |
  grep -v -F "bootstat: Service started: /system/bin/bootstat --record_boot_complete
...
bootstat: Canonical boot reason: ${1}
...
init    : Service 'exec 3 (/system/bin/bootstat --record_boot_complete)
 (/system/bin/bootstat --record_boot_reason)'...
 (/system/bin/bootstat --record_boot_reason)' (pid 
 (/system/bin/bootstat --record_time_since_factory_reset)'...
 (/system/bin/bootstat --record_time_since_factory_reset)' (pid 
...
end_test() {
  ...
  if [ ${save_ret} = 0 ]; then
    echo "[38;5;40m[       OK ][0m ${TEST} ${*}"
  else
    echo "[38;5;196m[  FAILED  ][0m ${TEST} ${*}"
  fi
  return ${save_ret}
}
...
test_properties() {
  wait_for_screen
  retval=0
  for prop in ro.boot.bootreason persist.sys.boot.reason sys.boot.reason; do
...
test_ota() {
  ...
  if [ -z "${TARGET_PRODUCT}" -o -z "${ANDROID_PRODUCT_OUT}" -o -z "${ANDROID_BUILD_TOP}" ]; then
...
test_factory_reset() {
  ...
  report_bootstat_logs reboot,factory_reset
}
...
test_kernel_panic() {
  ...
  EXPECT_PROPERTY persist.sys.boot.reason kernel_panic_sysrq
...
- adb shell setprop sys.powerctl shutdown,thermal-shutdown
- (power up the device)
- (wait until screen is up, boot has completed)
- adb shell getprop sys.boot.reason
- NB: should report shutdown,thermal-shutdown" ]
...
if [ -n "${successes}" ]; then
  echo "[38;5;40m[  PASSED  ][0m ${successes}"
fi
if [ -n "${failures}" ]; then
  echo "[38;5;196m[  FAILED  ][0m ${failures}"
fi
exit ${ret}
