...
filter_kBootReasonMap() {
  while read id match; do
    case ${match} in
      __BOOTSTAT_UNKNOWN__)
        echo ${id} ${match}
        echo ${id} completely unknown value
        echo ${id} 'reboot\\.empty'
        ;;
      reboot\\.empty)
        echo ${id}
        echo ${id} reboot.empty
...
test_kBootReasonMap() {
  ...
  report_bootstat_logs -t${T} \
    ...
    "-bootstat: Unknown boot reason: completely unknown value" \
