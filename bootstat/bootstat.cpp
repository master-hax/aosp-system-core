...
static void SetProperty(const char* key, const std::string& val) {
...
static const char system_reboot_reason_property[] = "sys.boot.reason";
static const char last_reboot_reason_property[] = LAST_REBOOT_REASON_PROPERTY;
static const char bootloader_reboot_reason_property[] = "ro.boot.bootreason";
...
std::string BootReasonStrToReason(const std::string& boot_reason) {
  ...
  if (isKernelRebootReason(ret) && (GetProperty(last_reboot_reason_property) != "")) {
    // rewrite as it must be old news
