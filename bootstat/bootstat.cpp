...
class pstoreConsole {
 private:
  ...
  size_t numError(const char* l, const std::string& _r) const {
    ...
    size_t maxError = n * 8 / kBitErrorRate;
    size_t count = 0;
    do {
      count += numError(*--le, *--re);
      if (count > maxError) return std::string::npos;
 ...
 public:
  ...
  size_t rfind(const std::string& needle) const {
    ...
    for (const char* p = console.c_str();;) {
      size_t ret = numError(p, needle.c_str());
      if (ret != std::string::npos) {
        return console.c_str() - p;
      }
...
std::string BootReasonStrToReason(const std::string& boot_reason) {
  ...
  if (ret == "kernel_panic") {
    ...
    if (isBluntRebootReason(ret)) {
      ...
      // example bit error rate issues for 100%
      //   'l=100 ' no bits in error
      //   'l=1 0 ' single bit error
      //   'l=0 0 ' double bit error
      //   'l=1   ' double bit error
      //   'l=0   ' triple bit error
      //   'l=10  ' single bit error, cosmetic from our perspective
      ...
      // character after digit sequence is not a space or 0.
      if ((level <= 100) && (endptr != digits.c_str()) && (endptr[0] == ' ') &&
          (endptr[1] != ' ') && (endptr[1] != '0')) {
