...
std::string BootReasonStrToReason(const std::string& boot_reason) {
  ...
  if (isBluntRebootReason(ret)) {
    ...
    if (isBluntRebootReason(ret)) {
      // content buffer no longer will have console data. Beware if more
      // checks added below that depend on parsing console content.
      content = GetProperty("persist.sys.boot.reason");
  ...
  if (isKernelRebootReason(ret)) {
    // rewrite persistent reboot reason, as it must be old news
    property_set("persist.sys.boot.reason", ret.c_str());
  }
  return ret;
}
...
std::string CalculateBootCompletePrefix() {
  ...
  if (!boot_event_store.GetBootEvent(kBuildDateKey, &record)) {
    ...
    property_set("sys.boot.reason", "reboot,factory_reset");
  } else if (build_date != record.second) {
    boot_complete_prefix = "ota_" + boot_complete_prefix;
    boot_event_store.AddBootEventWithValue(kBuildDateKey, build_date);
    LOG(INFO) << "Canonical boot reason: " << "reboot,ota";
    property_set("sys.boot.reason", "reboot,ota");
  }
