...
// ::isprint(c) and likely_space() natation will prevent us from being
// called for fundamentally printable entries, except for '\r' and '\b'.
//
...
// \a (non-printable), #, &, %, / and 0xA7 (non-printable?) are common
...
bool addKernelPanicSubReason(const pstoreConsole& console, std::string& ret) {
  ...
  if ((console.rfind("SysRq : Trigger a crash") != std::string::npos) ||
    ...
    if (pos != std::string::npos) {
      ret += "," + getSubreason(console, pos + strlen(sysrqSubreason), true);
    }
    return true;
  }
  ...
  if (pos != std::string::npos) {
    ...
    auto reason = getSubreason(console, pos + panic.length(), false);
    if (reason.length() > 3) {
      ret += "," + reason;
    }
    return true;
...
std::string BootReasonStrToReason(const std::string& boot_reason) {
  ...
  if (ret == "kernel_panic") {
    ...
    if (readPstoreConsole(content)) {
      ...
      if (pos != std::string::npos) {
        std::string subReason(getSubreason(content, pos + strlen(cmd), true));
