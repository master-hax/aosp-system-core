...
void RecordBootloaderTimings(
    BootEventRecordStore* boot_event_store,
    const BootloaderTimingMap& bootloader_timings) {
  int32_t total_time = 0;
  for (auto const &timing : bootloader_timings) {
...
void RecordAbsoluteBootTime(
    BootEventRecordStore* boot_event_store,
    const BootloaderTimingMap& bootloader_timings,
    std::chrono::milliseconds uptime) {
  static const std::string systemWait = "SW";
  int32_t bootloader_time_ms = 0;

  for (auto const &timing : bootloader_timings) {
    if (timing.first.compare(systemWait) != 0) {
