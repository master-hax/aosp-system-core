...
TEST(ueventd_parser, ExternalFirmwareHandlers) {
    auto ueventd_file = R"(
external_firmware_handler devpath handler_path
external_firmware_handler /devices/path/firmware/something001.bin /vendor/bin/firmware_handler.sh
external_firmware_handler /devices/path/firmware/something001.bin "/vendor/bin/firmware_handler.sh --has --arguments"
)";
...
TEST(ueventd_parser, ExternalFirmwareHandlersDuplicate) {
    auto ueventd_file = R"(
external_firmware_handler devpath handler_path
external_firmware_handler devpath handler_path2
)";
...
external_firmware_handler /devices/path/firmware/firmware001.bin /vendor/bin/touch.sh
...
uevent_socket_rcvbuf_size 6M
    ...
    auto external_firmware_handlers = std::vector<ExternalFirmwareHandler>{
            {"/devices/path/firmware/firmware001.bin", "/vendor/bin/touch.sh"},
    };

    size_t uevent_socket_rcvbuf_size = 6 * 1024 * 1024;

    TestUeventdFile(ueventd_file, {subsystems, sysfs_permissions, permissions, firmware_directories,
                                   external_firmware_handlers, false, uevent_socket_rcvbuf_size});
}
...
external_firmware_handler blah
external_firmware_handler blah blah blah
