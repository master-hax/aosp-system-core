...
int main(int argc, char** argv) {
    ...
    if (!partition_map.empty()) {
        for (const auto& p : partition_search_order) {
            if (partition_map.find(p) != partition_map.end()) {
                if (!parser.ParseConfig(partition_map.at(p) + "etc/init")) {
                    LOG(ERROR) << "Failed to parse partition '" << p << "'";
                    return EXIT_FAILURE;
                }
            }
        }
    ...
    if (failures > 0) {
        ...
        if (!partition_map.empty()) {
            error += "init scripts in partitions";
        } else {
            error += "init script '" + std::string(*argv) + "'";
        }
        LOG(ERROR) << error << " with " << failures << " error(s).";
        return EXIT_FAILURE;
    }
    return EXIT_SUCCESS;
}
