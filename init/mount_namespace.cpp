...
static bool BindMount(const std::string& source, const std::string& mountPoint,
                      bool recursive = false) {
    unsigned long mountflags = MS_BIND;
    if (recursive) {
        mountflags |= MS_REC;
    }
    if (mount(source.c_str(), mountPoint.c_str(), nullptr, mountflags, nullptr) == -1) {
        PLOG(ERROR) << "Could not bind-mount " << source << " to " << mountPoint;
        return false;
    }
    return true;
}

static bool MakeShared(const std::string& mountPoint, bool recursive = false) {
    unsigned long mountflags = MS_SHARED;
    if (recursive) {
        mountflags |= MS_REC;
    }
    if (mount(nullptr, mountPoint.c_str(), nullptr, mountflags, nullptr) == -1) {
...
static bool MakePrivate(const std::string& mountPoint, bool recursive = false) {
...
static bool BindMountBionic(const std::string& linkerSource, const std::string& libDirSource,
                            const std::string& linkerMountpoint, const std::string& libMountDir) {
    if (access(linkerSource.c_str(), F_OK) != 0) {
        PLOG(INFO) << linkerSource << " does not exist. skipping mounting bionic there.";
        // This can happen for 64-bit bionic in 32-bit only device.
        // It is okay to skip mounting the 64-bit bionic.
        return true;
    }
    if (!BindMount(linkerSource, linkerMountpoint)) {
        return false;
    }
    if (!MakePrivate(linkerMountpoint)) {
        return false;
    }
    for (const auto& libname : kBionicLibFileNames) {
        std::string mountPoint = libMountDir + libname;
        std::string source = libDirSource + libname;
        if (!BindMount(source, mountPoint)) {
            return false;
        }
        if (!MakePrivate(mountPoint)) {
            return false;
        }
    }
    return true;
...
static android::base::unique_fd bootstrapNsFd;
static android::base::unique_fd defaultNsFd;

static std::string bootstrapNsId;
static std::string defaultNsId;
...
bool SetupMountNamespaces() {
    ...
    bootstrapNsFd.reset(OpenMountNamespace());
    bootstrapNsId = GetMountNamespaceId();
    ...
    if (IsBionicUpdatable() && !IsRecoveryMode()) {
        ...
        defaultNsFd.reset(OpenMountNamespace());
        defaultNsId = GetMountNamespaceId();
        ...
        if (setns(bootstrapNsFd.get(), CLONE_NEWNS) == -1) {
...
bool SwitchToDefaultMountNamespace() {
    ...
    if (defaultNsId != GetMountNamespaceId()) {
        if (setns(defaultNsFd.get(), CLONE_NEWNS) == -1) {
...
bool SwitchToBootstrapMountNamespaceIfNeeded() {
    ...
    if (bootstrapNsId != GetMountNamespaceId() && bootstrapNsFd.get() != -1 &&
        IsBionicUpdatable()) {
        if (setns(bootstrapNsFd.get(), CLONE_NEWNS) == -1) {
