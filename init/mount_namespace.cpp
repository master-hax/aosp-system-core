...
// This is for the first-stage init
bool SetupBootstrapMountNamespace() {
    ...
    if (!bind_mount("/bionic", "/bionic", true /*recursive*/)) return false;
    if (!make_private("/bionic")) return false;
    ...
    LOG(INFO) << "SetupBootstrapMountNamespace done";
    return true;
}

// This is for the second stage init
bool SetupDefaultMountNamespace() {
    // current namespace set up by the first stage init  is named as 'bootstrap'
    bootstrap_ns_fd.reset(open_mount_namespace());
    bootstrap_ns_id = get_mount_namespace_id();
    ...
    if (bionic_updatable() && !IsRecoveryMode()) {
    ...
    LOG(INFO) << "SetupDefaultMountNamespace done";
    return true;
}
...
bool SwitchToBootstrapMountNamespaceIfNeeded() {
    ...
    if (bootstrap_ns_id != get_mount_namespace_id() && bootstrap_ns_fd.get() != -1 &&
        bionic_updatable()) {
        if (setns(bootstrap_ns_fd.get(), CLONE_NEWNS) == -1) {
