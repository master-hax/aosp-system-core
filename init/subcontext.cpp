...
void RunCommand(const SubcontextCommand::ExecuteCommand& execute_command,
                ...
                const KeywordFunctionMap* function_map) {
    ...
    auto map_result = function_map->FindFunction(args);
...
int SubcontextMain(int argc, char** argv, const KeywordFunctionMap* function_map) {
    ...
    while (true) {
        ...
        auto init_message = ReadMessage(init_fd);
        if (!init_message) {
        ...
        if (auto result = SendMessage(init_fd, reply); !result) {
...
std::vector<Subcontext>* InitializeSubcontexts() {
    static const char* const paths_and_secontexts[][2] = {
        {"/vendor", "u:object_r:vendor_init:s0"},
    };
