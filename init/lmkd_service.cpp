...
static LmkdRegistrationResult register_process(uid_t uid, pid_t pid, int oom_score_adjust) {
...
static bool unregister_process(pid_t pid) {
    if (lmkd_socket < 0) {
...
static void register_services(pid_t exclude_pid) {
    for (const auto& service : ServiceList::GetInstance().services()) {
        ...
        if (svc->oom_score_adjust() != DEFAULT_OOM_SCORE_ADJUST) {
            ...
            if (register_process(svc->uid(), svc->pid(), svc->oom_score_adjust()) !=
                LMKD_REG_SUCCESS) {
...
void LmkdRegister(const std::string& name, uid_t uid, pid_t pid, int oom_score_adjust) {
    ...
    result = register_process(uid, pid, oom_score_adjust);
