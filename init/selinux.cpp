...
bool FileProtectedByFsVerity(const std::string& path) {
...
Result<void> StagedSepolicyVerifyAndExtract() {
    ...
    if (ec) {
        // LOG(FATAL) << "Failed to copy " << from << " to " << to << ": " << ec.message();
    ...
    if (ec) {
        return Error() << "Failed to remove staged directory.";
    }
    return {};
}
...
int SetupSelinux(char** argv) {
    ...
    PrepareApexSepolicy();
    LOG(INFO) << "SELinux policy prepared successfully";
