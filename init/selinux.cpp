...
// The split policy is for supporting treble devices.  It splits the the SEPolicy across files on
// /system/etc/selinux (plat) and /vendor/etc/selinux (nonplat).  This is necessary to allow the
// system image to be updated independently of the vendor image, while maintaining contributions
// from both partitions in the SEPolicy.  This is especially important for VTS testing, where the
...
//    system needs to compile the SEPolicy.  /system contains the SEPolicy compiler, secilc, and it
//    is used by the LoadSplitPolicy() function below to compile the SEPolicy to a temp directory
//    and load it.  That function contains more documentation with the specific implementation
//    details if needed.

// There is a helper function that has the SELinux logs written to kmsg, to match init's logging.

// There are a set of helper functions that wrap the selabel_lookup*() functions for file contexts
// to safely cache the sehandle related to these file contexts.
...
void SelinuxRestoreContext() {
    ...
    if
        constexpr(WORLD_WRITABLE_KMSG) { selinux_android_restorecon("/dev/kmsg_debug", 0); }
