...
// /system/etc/selinux (the 'plat' portion of the policy), /vendor/etc/selinux (the 'vendor' portion
// of the policy), /system_ext/etc/selinux, /product/etc/selinux and /odm/etc/selinux.  This is
// necessary to allow images to be updated independently of the vendor image, while maintaining
// contributions from multiple partitions in the SEPolicy.  This is especially important for VTS
// testing, where the SEPolicy on the Google System Image may not be identical to the system image
// shipped on a vendor's device.
...
//    are the sha256 hashes of the parts of the SEPolicy on /system, /system_ext and /product
...
//    and product contain sha256 hashes of their SEPolicy. Init loads this
//    precompiled_sepolicy directly if and only if the hashes along with the precompiled SEPolicy on
//    /vendor or /odm match the hashes for system, system_ext and product SEPolicy,
//    respectively.
// 2) If these hashes do not match, then either /system, /system_ext or /product (or some of
//    them) have been updated out of sync with /vendor (or /odm if it is present) and init needs
...
bool OpenSplitPolicy(PolicyFile* policy_file) {
    ...
    // * (optional) policy needed due to logic on product, system_ext or odm.
