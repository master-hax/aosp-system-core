...
bool FindPrecompiledSplitPolicy(std::string* file) {
    ...
    for (auto& [actual_id_path, precompiled_id_path] : sepolicy_hashes) {
        std::string actual_id;
        if (!ReadFirstLine(actual_id_path.c_str(), &actual_id)) {
            PLOG(INFO) << "Failed to read " << actual_id_path;
            return false;
        }

        std::string precompiled_id;
        if (!ReadFirstLine(precompiled_id_path.c_str(), &precompiled_id)) {
            PLOG(INFO) << "Failed to read " << precompiled_id_path;
            return false;
        }
    ...
    *file = std::move(precompiled_sepolicy);
