...
const std::vector<std::string> kApkSepolicyTmp{"apex_sepolicy.cil", "apex_sepolicy.sha256"};
// Files that need to persist because they are used by userspace processes.
const std::vector<std::string> kApkSepolicy{"apex_file_contexts", "apex_property_contexts",
...
Result<void> LoadSepolicyApexCerts() {
    ...
    // builds. TODO(b/199914227) don't even try to load debug cert on non-debug builds.
    if (access(kSigningCertDebug, F_OK) < 0) {
        LoadKeyFromFile(keyring_id, "fsv_sepolicy_apex_debug", kSigningCertDebug);
    }
...
Result<void> SepolicyVerify(bool supportsFsVerity) {
    ...
    if (!sepolicySignature.ok()) {
        return Error() << "Apex SEPolicy failed to signature check";
    }
    return {};
}
...
// Apex sepolicy is shipped within an APK within an APEX. Because
...
// 2. Extracts files from APK and stores them in /dev/selinux.
...
//
// If no APK is available, continue booting as normal. If an error occurs,
// log the error and continue booting. TODO(b/199914227) add metrics so that
