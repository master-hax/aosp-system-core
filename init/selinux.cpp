...
Result<void> GetPolicyFromApex(std::string dir) {
...
Result<void> SepolicySignature(std::string dir) {
...
Result<void> SepolicyVerify(std::string dir, bool supportsFsVerity) {
    if (supportsFsVerity) {
    ...
    auto sepolicySignature = SepolicySignature(dir);
...
void PrepareApexSepolicy() {
    ...
    if (!sepolicyVerify.ok()) {
        ...
        // If signature verifcation fails, fall back to version on /system.
