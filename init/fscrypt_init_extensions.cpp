...
static int set_policy_on(const std::string& ref_basename, const std::string& dir) {
    EncryptionPolicy policy_key_ref;
    ...
    if (!ParseOptions(options_string, &policy_key_ref.options)) {
        LOG(ERROR) << "Invalid encryption options string: " << options_string;
        return -1;
    }

    if (!EnsurePolicy(policy_key_ref, dir)) {
        std::string ref_hex;
        BytesToHex(policy_key_ref.key_raw_ref, &ref_hex);
