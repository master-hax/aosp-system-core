...
static int set_policy_on(const std::string& ref_basename, const std::string& dir) {
    PolicyKeyRef policy_key_ref;
    std::string ref_filename = std::string("/data") + ref_basename;
    if (!android::base::ReadFileToString(ref_filename, &policy_key_ref.key_raw_ref)) {
    ...
    if (!FscryptParsePolicy(options_string, &policy_key_ref.policy)) {
        LOG(ERROR) << "Invalid encryption options string: " << options_string;
        return -1;
    }

    if (!FscryptPolicyEnsure(policy_key_ref, dir)) {
        std::string ref_hex;
        StrToHex(policy_key_ref.key_raw_ref, &ref_hex);
