...
void dump_parser_state() {
    if (false) {
        ...
        ActionQueue::get_instance().dump_parser_state();
    }
}
...
static void parse_line_service(struct parse_state *state, int nargs, char **args)
{
    ...
    case K_onrestart:
        ...
        svc->onrestart->add_command(kw_func(kw), nargs, args);
        break;
    case K_critical:
...
{
    std::string ret_err;
    Action* ret = ActionQueue::get_instance().parse_action(nargs, args, &ret_err);
...
static void parse_line_action(struct parse_state* state, int nargs, char **args)
{
    ...
    act->add_command(kw_func(kw), nargs, args, state->filename, state->line);
}
