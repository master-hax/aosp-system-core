...
void Command::build_command_string(char* cmd_str, int length)
{
    for (int i = 0; i < nargs; i++) {
        strlcat(cmd_str, args[i], length);
        if (i < nargs - 1) {
            strlcat(cmd_str, " ", length);
        }
    }
}

void Command::build_source_string(char* source, int length)
{
    if (!filename.empty())
        snprintf(source, length, " (%s:%d)", filename.c_str(), line);
    else
        *source = '\0';
}
                                     ...
                                     const std::string& value)
{
    ...
    for (auto& t : property_triggers) {
        if (!t.first.compare(name)) {
            ...
            if (property_get(t.first.c_str(), prop_val) <= 0 ||
                (t.second.compare("*") &&
...
void Action::build_triggers_string(char* name_str, int length) {
    bool first = true;
    for (auto& t : property_triggers) {
        if (!first)
            strlcat(name_str, " ", length);
        strlcat(name_str, t.first.c_str(), length);
        strlcat(name_str, "=", length);
        strlcat(name_str, t.second.c_str(), length);
    }
    if (!event_trigger.empty()) {
        if (!first)
            strlcat(name_str, " ", length);
        strlcat(name_str, event_trigger.c_str(), length);
    }
}
...
void dump_parser_state() {
    if (false) {
        ...
        for (auto& a : action_list) {
            INFO("on ");
            char name_str[256] = "";
            a->build_triggers_string(name_str, sizeof(name_str));
            INFO("%s", name_str);
            INFO("\n");
            ...
            for (auto& c : a->commands) {
                INFO("  %p", c->func);
                char cmd_str[512];
                c->build_command_string(cmd_str, sizeof(cmd_str));
                INFO(" %s", cmd_str);
            }
            INFO("\n");
        }
