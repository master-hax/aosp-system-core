...
static void parse_line_service(struct parse_state *state, int nargs, char **args)
{
    ...
    Service* svc = (Service*) state->context;
    int kw = lookup_keyword(args[0]);
                           ...
                           const std::string& filename, int line, std::string* err)
{
    ...
    if (static_cast<int>(args.size()) < n) {
        *err = android::base::StringPrintf("%s requires %d %s\n",
                                           args[0].c_str(), n - 1,
                                           n > 2 ? "arguments" : "argument");
        return false;
    }

    act->AddCommand(kw_func(kw), args, filename, line);
    return true;
}
...
static void parse_line_action(struct parse_state* state, int nargs, char **args)
{
    ...
    std::vector<std::string> str_args(args, args + nargs);
    std::string ret_err;
    bool ret = add_command_to_action(act, str_args, state->filename, state->line, &ret_err);
    if (!ret) {
        parse_error(state, "%s\n", ret_err.c_str());
    }
}
