    ...
    if (false) {
        ...
        list_for_each(node, &service_list) {
            ...
            for (socketinfo* si = svc->sockets; si; si = si->next) {
                LOG(DEBUG) << "  socket " << si->name << " " << si->type << " 0" << std::oct << si->perm;
            }
        }
...
parser_done:
    list_for_each(node, &import_list) {
         ...
         if (!init_parse_config(import->filename)) {
             LOG(ERROR) << "could not import file '" << import->filename
                        << "' from '" << fn << "': " << strerror(errno);
         }
    }
}
...
service* make_exec_oneshot_service(int nargs, char** args) {
    ...
    if (svc == NULL) {
        LOG(ERROR) << "Couldn't allocate service for exec of '" << argv[0] << "': " << strerror(errno);
        return NULL;
    }
