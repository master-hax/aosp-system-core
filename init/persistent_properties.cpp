...
Result<void> WritePersistentPropertyFile(const PersistentProperties& persistent_properties) {
    ...
    // fsync() is required.
    auto dist_fd = unique_fd{open(persistent_property_filename.c_str(), O_RDONLY | O_CLOEXEC)};
    if (dist_fd < 0) {
        return ErrnoError() << "Unable to open persistent properties file for fsync()";
    }
    fsync(dist_fd.get());
