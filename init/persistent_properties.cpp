...
void AddPersistentProperty(const std::string& key, const std::string& value,
                           PersistentProperties* persistent_properties) {
    auto persistent_property_record = persistent_properties->add_properties();
    persistent_property_record->set_key(key);
    ...
    __system_property_foreach(
        [](const prop_info* pi, void* cookie) {
            __system_property_read_callback(
                pi,
                [](void* cookie, const char* key, const char* value, unsigned serial) {
                    if (StartsWith(key, "persist.")) {
                        auto properties =
                            reinterpret_cast<PersistentProperties*>(cookie);
                        AddPersistentProperty(key, value, properties);
                    }
                },
                cookie);
        },
...
Result<PersistentProperties> PersistentPropertyFileParser::Parse() {
    ...
    while (position_ < contents_.size()) {
        ...
        AddPersistentProperty(*key, *value, &result);
    }
...
Result<PersistentProperties> LoadPersistentPropertyFile() {
    ...
    LOG(INFO) << "Could not parse intermediate format, trying protobuf format: "
              << parsed_contents.error();
...
void WritePersistentProperty(const std::string& key, const std::string& value) {
    auto file_contents = ReadPersistentPropertyFile();
    PersistentProperties persistent_properties;
    ...
    auto it = std::find_if(persistent_properties.mutable_properties()->begin(),
                           ...
                           [&key](const auto& record) { return record.key() == key; });
    if (it != persistent_properties.mutable_properties()->end()) {
        it->set_key(key);
        it->set_value(value);
    } else {
        AddPersistentProperty(key, value, &persistent_properties);
    }
