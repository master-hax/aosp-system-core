...
public class MteUpgradeTest extends BaseHostJUnit4Test {
    ...
    public void setUp() throws Exception {
        CommandResult result =
                getDevice().executeShellV2Command("/system/bin/mte_upgrade_test_binary --checking");
    ...
    public void tearDown() throws Exception {
        ...
        getDevice().executeShellV2Command("stop mte_upgrade_test_crash");
        getDevice().executeShellV2Command("stop mte_upgrade_test_crash_overriden");
    ...
    public void testCrash() throws Exception {
        ...
        CommandResult result = getDevice().executeShellV2Command("start mte_upgrade_test_crash");
        ...
        for (String tombstone : tombstonesAfter) {
            ...
            if (!tombstoneProto.getCommandLineList().stream().anyMatch(
                        x -> x.contains("mte_upgrade_test_binary"))) {
                continue;
            }
    ...
    @Test
    public void testCrashOverriden() throws Exception {
        String uuid = java.util.UUID.randomUUID().toString();
        getDevice().reboot();
        assertThat(getDevice().setProperty("sys.mte_crash_test_uuid", uuid)).isTrue();

        CommandResult result = getDevice().executeShellV2Command("start mte_upgrade_test_crash_overriden");
        assertThat(result.getExitCode()).isEqualTo(0);
        java.lang.Thread.sleep(20000);
        String[] tombstonesAfter = getDevice().getChildren("/data/tombstones");
        ArrayList<String> segvCodeNames = new ArrayList<String>();
    ...
    public void testDowngrade() throws Exception {
        CommandResult result =
                getDevice().executeShellV2Command("MEMTAG_OPTIONS=async MEMTAG_TIMED_UPGRADE_TO_SYNC=5"
                        + " /system/bin/mte_upgrade_test_binary --check-downgrade");
        assertThat(result.getExitCode()).isEqualTo(0);
    }
}
