...
static char ro_build_date_utc[PROP_VALUE_MAX];
static int ro_build_date_utc_len;
                         ...
                         bool setrestoreconlast)
{
    ...
    if (setrestoreconlast && ro_build_date_utc_len && S_ISDIR(sb->st_mode))
        setxattr(pathname, RESTORECON_LAST, ro_build_date_utc,
                 ro_build_date_utc_len, 0);
    return 0;
}
...
int restorecon_recursive(const char* pathname)
{
    ...
    if (!ro_build_date_utc_len) {
        ro_build_date_utc_len = property_get("ro.build.date.utc",
                                             ro_build_date_utc);
        if (ro_build_date_utc_len <= 0)
            INFO("Could not get ro.build.date.utc (len %d)",
                  ro_build_date_utc_len);
    }

    if (ro_build_date_utc_len) {
        size = getxattr(pathname, RESTORECON_LAST, xattr_value,
                        ro_build_date_utc_len);
        if (size == ro_build_date_utc_len &&
            memcmp(ro_build_date_utc, xattr_value,
                   ro_build_date_utc_len) == 0) {
            INFO("Skipping restorecon_recursive(%s)\n", pathname);
            return 0;
        }
    }
