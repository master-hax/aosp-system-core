...
Result<Nothing> Epoll::Open() {
    if (epoll_fd_ >= 0) return Nothing();
...
Result<Nothing> Epoll::RegisterHandler(int fd, std::function<void()> handler, uint32_t events) {
    if (!events) {
    ...
    if (epoll_ctl(epoll_fd_, EPOLL_CTL_ADD, fd, &ev) == -1) {
        Result<Nothing> result = ErrnoError() << "epoll_ctl failed to add fd";
...
Result<Nothing> Epoll::UnregisterHandler(int fd) {
...
Result<Nothing> Epoll::Wait(std::optional<std::chrono::milliseconds> timeout) {
    int timeout_ms = -1;
