...
std::string TestFrame::Format(const std::vector<int>& chord) {
    std::string ret("[");
    auto first = true;
    for (auto& code : chord) {
        if (!first) ret += '+';
...
std::string TestFrame::Format(const std::vector<const std::vector<int>>& chords) {
    std::string ret("{");
    auto first = true;
    for (const auto& keycodes : chords) {
        if (!first) ret += ',';
        first = false;
        ret += Format(keycodes);
    }
...
TEST(keychords, key) {
    ...
    EXPECT_TRUE(test_frame.IsOnlyChord(escape_chord))
        << "expected only " << test_frame.Format(escape_chord) << " got " << test_frame.Format();
}
