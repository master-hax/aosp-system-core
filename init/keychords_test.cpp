...
int GetChordId(const std::vector<int>& chord) {
    auto it = chord_map.find(&chord);
    if (it == chord_map.end()) return 0;
    return it->second;
}
...
void instantiate() {
    ...
    epoll.Open();
    for (auto& chord_and_id : chord_map) {
        chord_and_id.second = GetKeychordId(*chord_and_id.first);
        EXPECT_NE(chord_and_id.second, 0);
    }
    KeychordInit(&epoll);
...
TEST(keychords, key) {
    ...
    instantiate();
    EXPECT_TRUE(ev.init());
    auto keychord_id = GetChordId(escape_chord);
    EXPECT_NE(keychord_id, 0);
    SetChords(escape_chord);
    for (int retry = 1000; retry && keychord_id != last_keychord_id; --retry) RelaxForMs();
...
TEST(keychords, keys_in_series) {
    ...
    instantiate();
    EXPECT_TRUE(ev.init());
    int keychord_id = GetChordId(triple1_chord);
    EXPECT_NE(keychord_id, 0);
    for (auto& key : triple1_chord) {
        SetChord(key);
        ClrChord(key);
    }
    for (int retry = 1000; retry && (last_keychord_id == -1); --retry) RelaxForMs();
    EXPECT_NE(last_keychord_id, keychord_id);
    EXPECT_EQ(last_keychord_id, -1);
}
...
TEST(keychords, keys_in_parallel) {
    ...
    instantiate();
    EXPECT_TRUE(ev.init());
    int keychord_id = GetChordId(triple2_chord);
    EXPECT_NE(keychord_id, 0);
