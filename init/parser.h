...
//  functions return Result<Nothing> indicating if they have an error. It will be reported along
...
class SectionParser {
  public:
    ...
    virtual Result<Nothing> ParseSection(std::vector<std::string>&& args,
                                         const std::string& filename, int line) = 0;
    virtual Result<Nothing> ParseLineSection(std::vector<std::string>&&, int) { return Nothing(); };
    virtual Result<Nothing> EndSection() { return Nothing(); };
...
class Parser {
  public:
    ...
    //  be written.
    using LineCallback = std::function<Result<Nothing>(std::vector<std::string>&&)>;
