...
bool IsDyntransitionAllowed(std::string const& from, std::string const& to) {
    static const access_vector_t kDyntransition =
            string_to_av_perm(string_to_security_class("process"), "dyntransition");
    struct av_decision avd;
    if (security_compute_av(from.c_str(), to.c_str(), string_to_security_class("process"),
                            kDyntransition, &avd) != 0) {
...
Result<Success> Service::Start() {
    ...
    if (!pre_apexd_ && GetBoolProperty("ro.nativezygote.enable", false)) {
        if (IsDyntransitionAllowed("u:r:nativezygote:s0", scon)) {
            nz_client = &system_native_zygote_;
        } else if (IsDyntransitionAllowed("u:r:nativezygote_vendor:s0", scon)) {
