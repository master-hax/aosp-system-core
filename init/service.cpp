...
Service::Service(const std::string& name, const std::string& classname,
    ...
    : name_(name), classname_(classname), flags_(0), pid_(0), time_started_(0),
      ...
      ioprio_class_(IoSchedClass_NONE), ioprio_pri_(0), process_pri_(0), args_(args) {
...
Service::Service(const std::string& name, const std::string& classname,
    ...
    : name_(name), classname_(classname), flags_(flags), pid_(0), time_started_(0),
      ...
      seclabel_(seclabel), ioprio_class_(IoSchedClass_NONE), ioprio_pri_(0), process_pri_(0),
      args_(args) {
    onrestart_.InitSingleTrigger("onrestart");
}
...
bool Service::HandlePriority(const std::vector<std::string>& args, std::string* err) {
    process_pri_ = std::stoi(args[1]);

    if (process_pri_ < ANDROID_PRIORITY_HIGHEST || process_pri_ > ANDROID_PRIORITY_LOWEST) {
        process_pri_ = 0;
...
bool Service::Start() {
    ...
    if (pid == 0) {
        ...
        if (process_pri_ != 0) {
            if (setpriority(PRIO_PROCESS, 0, process_pri_) != 0) {
