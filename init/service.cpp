...
Result<Success> Service::ParseUpdateable(std::vector<std::string>&& args) {
    updateable_ = true;
    return Success();
}
...
const Service::OptionParserMap::Map& Service::OptionParserMap::map() const {
    ...
    static const Map option_parsers = {
        ...
        {"updateable",  {0,     0,    &Service::ParseUpdateable}},
...
Result<Success> Service::ExecStart() {
    if (is_updateable() && !ServiceList::GetInstance().IsServicesUpdated()) {
        // Don't delay the service for ExecStart() as the semantic is that
        // the caller might depend on the side effect of the execution.
        return Error() << "Cannot start an updateable service '" << name_
...
Result<Success> ServiceParser::EndSection() {
    if (service_) {
        ...
        if (old_service) {
            ...
            if (StartsWith(filename_, "/apex"), !old_service->is_updateable()) {
                return Error() << "cannot update a non-updateable service '" << service_->name()
                               << "' with a config in APEX: " << filename_;
            }
