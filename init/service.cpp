...
ServiceEnvironmentInfo::ServiceEnvironmentInfo(const std::string& name, const std::string& value)
...
Service::Service(const std::string& name, const std::string& classname,
    ...
    : name_(name), classname_(classname), flags_(0), pid_(0), time_started_(0), time_crashed_(0),
    nr_crashed_(0), uid_(0), gid_(0), nr_supp_gids_(0), seclabel_(""),
    writepid_files_(nullptr), keycodes_(nullptr), nkeycodes_(0),
...
void Service::Wait()
{
    ...
    for (const auto& si : sockets_) {
        std::string tmp = android::base::StringPrintf(ANDROID_SOCKET_DIR "/%s", si->name.c_str());
        unlink(tmp.c_str());
    }
...
void Service::Start(const std::vector<std::string>& dynamic_args)
{
    ...
    if (!seclabel_.empty()) {
        ...
        rc = security_compute_create(mycon, fcon, string_to_security_class("process"), &ret_scon);
        if (rc == 0) {
            scon = ret_scon;
            free(ret_scon);
        }
    ...
    if (pid == 0) {
        ...
        for (const auto& si : sockets_) {
            int socket_type = ((!si->type.compare("stream") ? SOCK_STREAM :
                                (!si->type.compare("dgram") ? SOCK_DGRAM : SOCK_SEQPACKET)));
            int s = create_socket(si->name.c_str(), socket_type, si->perm, si->uid, si->gid,
                                  !si->socketcon.empty() ? si->socketcon.c_str() : scon.c_str());
            if (s >= 0) {
                publish_socket(si->name.c_str(), s);
            }
        }
