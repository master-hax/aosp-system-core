...
bool Service::ParseShutdown(const std::vector<std::string>& args, std::string* err) {
    if (!args[1].compare("never")) {
        if (args.size() != 2) {
            *err = "shutdown never accepts no other arguments";
            return false;
        }
    ...
    if (!args[1].compare("after_apps")) {
        unsigned tmp_flags = SVC_SHUTDOWN_FLAGS_AFTER_APPS;

        for (unsigned i = 2; i < args.size(); i++) {
            if (!args[i].compare("sigterm")) {
                tmp_flags |= SVC_SHUTDOWN_FLAGS_AFTER_APPS_SIGTERM;
            } else if (args[i].compare("sigkill")) {
                *err = "shutdown after_apps has received an invalid flag: " + args[i];
                return false;
            }
        }
        flags_ |= SVC_SHUTDOWN_CRITICAL;
    ...
    *err = "shutdown accepts the following arguments, \"never|after_apps\", got: " + args[1];
    return false;
}
...
Service::OptionParserMap::Map& Service::OptionParserMap::map() const {
    ...
    static const Map option_parsers = {
        ...
        {"shutdown",    {1,     2,    &Service::ParseShutdown}},
