...
void Service::KillProcessGroup(int signal) {
    ...
    if (!process_cgroup_empty_) {
        ...
        int r;
        if (signal == SIGTERM) {
            r = killProcessGroupOnce(cgroup_, uid_, pid_, signal);
        } else {
            r = killProcessGroup(cgroup_, uid_, pid_, signal);
        }
...
Result<Success> Service::Start() {
    ...
    cgroup_ = use_memcg ? kMemoryCgroup : kCpuacctCgroup;

    errno = -createProcessGroup(cgroup_, uid_, pid_);
    if (errno != 0) {
