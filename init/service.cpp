...
bool Service::HandleGroup(const std::vector<std::string>& args, std::string* err) {
    if (args.size() > NR_SVC_SUPP_GIDS + 2) {
...
Service::ServiceHandlerMap Service::service_option_map_ = {
    {"class",       {1,     &Service::HandleClass}},
    {"console",     {0,     &Service::HandleConsole}},
    {"critical",    {0,     &Service::HandleCritical}},
    {"disabled",    {0,     &Service::HandleDisabled}},
    {"group",       {1,     &Service::HandleGroup}},
    {"ioprio",      {2,     &Service::HandleIoprio}},
    {"keycodes",    {1,     &Service::HandleKeycodes}},
    {"oneshot",     {0,     &Service::HandleOneshot}},
    {"onrestart",   {1,     &Service::HandleOnrestart}},
    {"seclabel",    {1,     &Service::HandleSeclabel}},
    {"setenv",      {2,     &Service::HandleSetenv}},
    {"socket",      {3,     &Service::HandleSocket}},
    {"user",        {1,     &Service::HandleUser}},
    {"writepid",    {1,     &Service::HandleWritepid}},
};
...
bool Service::HandleLine(const std::vector<std::string>& args, std::string* err) {
    ...
    if (!service_option_map_.count(args[0])) {
        *err = StringPrintf("invalid option '%s'", args[0].c_str());
        return false;
    }

    auto option_info = service_option_map_.at(args[0]);

    auto min_args = std::get<size_t>(option_info);
    if (args.size() < min_args + 1) {
        *err = StringPrintf("%s requires %zu argument%s",
                            args[0].c_str(), min_args,
                            min_args > 2 ? "s" : "");
        return false;
    }

    auto handler = std::get<LineHandler>(option_info);
...
bool ServiceParser::ParseLineSection(const std::vector<std::string>& args,
    ...
    return svc_->HandleLine(args, err);
}
