    ...
    if (!process_cgroup_empty_) {
        ...
        if (report_oneshot && max_processes > 0) {
            LOG(WARNING)
                    ...
                    << " additionally processes from a oneshot process group for service '" << name_
...
void Service::Reap(const siginfo_t& siginfo) {
    // Legacy behavior from ~2007: we allowed 'oneshot' services to daemonize and therefore do not
    // kill their process group unless they are commanded to restart.  This changes as of Android R,
    // where we will kill 'oneshot' services' process groups in all cases.  We report a warning
    // message where we detect this difference in behavior has occurred.
