...
bool Service::ParseSwappiness(const std::vector<std::string>& args, std::string* err) {
...
bool Service::ParseLimitInBytes(const std::vector<std::string>& args, std::string* err) {
...
bool Service::ParseSoftLimitInBytes(const std::vector<std::string>& args, std::string* err) {
    if (!ParseInt(args[1], &soft_limit_in_bytes_, 0)) {
        *err = "oom_score_adjust value must be equal or greater than 0";
        return false;
    }
    return true;
}
...
const Service::OptionParserMap::Map& Service::OptionParserMap::map() const {
    ...
    static const Map option_parsers = {
        ...
        {"swappiness",  {1,     1,    &Service::ParseSwappiness}},
        {"soft_limit_in_bytes",
                        {1,     1,    &Service::ParseSoftLimitInBytes}},
        {"limit_in_bytes",
                        {1,     1,    &Service::ParseLimitInBytes}},
...
bool Service::Start() {
    ...
    if (errno != 0) {
        ...
        if (swappiness_ != -1) {
            if (setProcessGroupSwappiness(uid_, pid_, swappiness_) != 0) {
                PLOG(ERROR) << "setProcessGroupSwappiness failed";
            }
        }

        if (soft_limit_in_bytes_ != -1) {
            if (setProcessGroupSoftLimit(uid_, pid_, soft_limit_in_bytes_) != 0) {
                PLOG(ERROR) << "setProcessGroupSoftLimit failed";
            }
        }

        if (limit_in_bytes_ != -1) {
            if (setProcessGroupLimit(uid_, pid_, limit_in_bytes_) != 0) {
