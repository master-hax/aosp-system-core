...
void Service::Reap(const siginfo_t& siginfo) {
    ...
    std::string native_watchdog_reboot_time =
            "persist.init.crashrecovery.native_watchdog_reboot_time";
    ...
    if (((flags_ & SVC_CRITICAL) || is_process_updatable) && !(flags_ & SVC_RESTART) &&
        ...
        if (now < time_crashed_ + fatal_crash_window_ || !boot_completed) {
            if (++crash_count_ > 4) {
                ...
                if (epochTime - GetIntProperty(native_watchdog_reboot_time, 0) > throttle_window) {
                    SetProperty(native_watchdog_reboot_time, std::to_string(epochTime));
                    auto exit_reason =
