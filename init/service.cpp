...
bool Service::HandleLine(const std::vector<std::string>& args, std::string* err) {
    ...
    auto option_info_it = service_option_map_.find(args[0]);
    if (option_info_it == service_option_map_.end()) {
        *err = StringPrintf("invalid option '%s'", args[0].c_str());
        return false;
    }

    auto option_info = option_info_it->second;

    auto num_args = args.size() - 1;
    auto min_args = std::get<0>(option_info);
    auto max_args = std::get<1>(option_info);
    ...
    if (num_args < min_args || num_args > max_args) {
        if (max_args == std::numeric_limits<decltype(max_args)>::max()) {
            ...
            *err = StringPrintf("%s takes between %zu and %zu arguments",
                                args[0].c_str(), min_args, max_args);
        }
        return false;
    }
