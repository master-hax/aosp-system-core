...
static void HandleCapabilities(const CapSet& capabilities, const std::string& service_name) {
    if (capabilities.any()) {
        if (!set_caps(capabilities)) {
            LOG(FATAL) << "couldn't set capabilities for " << service_name;
        }
    }
}
...
void Service::SetProcessAttributes() {
    ...
    if (capabilities_.any() && prctl(PR_SET_KEEPCAPS, 1, 0, 0, 0) != 0) {
    ...
    HandleCapabilities(capabilities_, name_);
}
