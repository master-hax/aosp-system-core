...
Service::Service(const std::string& name, const std::string& classname,
    ...
    : name_(name), classname_(classname), flags_(0), pid_(0), time_started_(0),
      ...
      seclabel_(""), keycodes_(nullptr), nkeycodes_(0),
...
bool Service::HandleLine(int kw, const std::vector<std::string>& args, std::string* err) {
    ...
    case K_setenv: { /* name value */
        ...
        ServiceEnvironmentInfo ei(args[1], args[2]);
    ...
    case K_socket: {/* name type perm [ uid gid context ] */
        ...
        SocketInfo si(args[1], args[2], uid, gid, perm, socketcon);
...
bool Service::Start(const std::vector<std::string>& dynamic_args) {
    ...
    if (pid == 0) {
        ...
        strs.reserve(args_.size() + dynamic_args.size() + 1);
        for (const auto& s : args_) {
