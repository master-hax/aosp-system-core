...
Result<Success> Service::ParseCapabilities(const std::vector<std::string>& args) {
    ...
    for (size_t i = 1; i < args.size(); i++) {
        ...
        if (res < 0) {
            return Err() << StringPrintf("invalid capability '%s'", arg.c_str());
        }
        unsigned int cap = static_cast<unsigned int>(res);  // |res| is >= 0.
        if (cap > last_valid_cap) {
            return Err() << StringPrintf("capability '%s' not supported by the kernel", arg.c_str());
        }
...
Result<Success> Service::ParseGroup(const std::vector<std::string>& args) {
    ...
    if (!gid) {
        return Err() << "Unable to decode GID for '" << args[1] << "': " << gid.error();
    }
    gid_ = *gid;
    ...
    for (std::size_t n = 2; n < args.size(); n++) {
        ...
        if (!gid) {
            return Err() << "Unable to decode GID for '" << args[n] << "': " << gid.error();
        }
...
Result<Success> Service::ParsePriority(const std::vector<std::string>& args) {
    ...
    if (!ParseInt(args[1], &priority_,
        ...
        return Err() << StringPrintf("process priority value must be range %d - %d",
...
Result<Success> Service::ParseIoprio(const std::vector<std::string>& args) {
    if (!ParseInt(args[2], &ioprio_pri_, 0, 7)) {
        return Err() << "priority value must be range 0 - 7";
    }
    ...
    if (args[1] == "rt") {
        ...
        return Err() << "ioprio option usage: ioprio <rt|be|idle> <0-7>";
    }
...
Result<Success> Service::ParseOnrestart(const std::vector<std::string>& args) {
    ...
    if (auto result = onrestart_.AddCommand(str_args, line); !result) {
        return Err() << "cannot add Onrestart command: " << result.error();
    }
    return Ok();
}
...
Result<Success> Service::ParseNamespace(const std::vector<std::string>& args) {
    for (size_t i = 1; i < args.size(); i++) {
        if (args[i] == "pid") {
            ...
            return Err() << "namespace must be 'pid' or 'mnt'";
        }
    }
    return Ok();
}
...
Result<Success> Service::ParseOomScoreAdjust(const std::vector<std::string>& args) {
    if (!ParseInt(args[1], &oom_score_adjust_, -1000, 1000)) {
        return Err() << "oom_score_adjust value must be in range -1000 - +1000";
    }
    return Ok();
}
...
Result<Success> Service::ParseMemcgSwappiness(const std::vector<std::string>& args) {
    if (!ParseInt(args[1], &swappiness_, 0)) {
        return Err() << "swappiness value must be equal or greater than 0";
    }
    return Ok();
}
...
Result<Success> Service::ParseMemcgLimitInBytes(const std::vector<std::string>& args) {
    if (!ParseInt(args[1], &limit_in_bytes_, 0)) {
        return Err() << "limit_in_bytes value must be equal or greater than 0";
    }
    return Ok();
}
...
Result<Success> Service::ParseMemcgSoftLimitInBytes(const std::vector<std::string>& args) {
    if (!ParseInt(args[1], &soft_limit_in_bytes_, 0)) {
        return Err() << "soft_limit_in_bytes value must be equal or greater than 0";
    }
    return Ok();
}
...
Result<Success> Service::ParseShutdown(const std::vector<std::string>& args) {
    ...
    return Err() << "Invalid shutdown option";
}
...
Result<Success> Service::AddDescriptor(const std::vector<std::string>& args) {
    ...
    if (args.size() > 4) {
        ...
        if (!uid) {
            return Err() << "Unable to find UID for '" << args[4] << "': " << uid.error();
        }
    }
    ...
    if (args.size() > 5) {
        ...
        if (!gid) {
            return Err() << "Unable to find GID for '" << args[5] << "': " << gid.error();
        }
    }
    ...
    if (old != descriptors_.end()) {
        return Err() << "duplicate descriptor " << args[1] << " " << args[2];
    }
...
Result<Success> Service::ParseSocket(const std::vector<std::string>& args) {
    if (!StartsWith(args[2], "dgram") && !StartsWith(args[2], "stream") &&
        ...
        return Err() << "socket type must be 'dgram', 'stream' or 'seqpacket'";
    }
...
Result<Success> Service::ParseFile(const std::vector<std::string>& args) {
    if (args[2] != "r" && args[2] != "w" && args[2] != "rw") {
        return Err() << "file type must be 'r', 'w' or 'rw'";
    }
    if ((args[1][0] != '/') || (args[1].find("../") != std::string::npos)) {
        return Err() << "file name must not be relative";
    }
...
Result<Success> Service::ParseUser(const std::vector<std::string>& args) {
    ...
    if (!uid) {
        return Err() << "Unable to find UID for '" << args[1] << "': " << uid.error();
    }
    uid_ = *uid;
    return Ok();
}
...
Result<Success> Service::ParseLine(const std::vector<std::string>& args) {
    ...
    if (!parser) return Err() << parser.error();
...
Result<Success> ServiceParser::ParseSection(std::vector<std::string>&& args,
    ...
    if (args.size() < 3) {
        return Err() << "services must have a name and a program";
    }

    const std::string& name = args[1];
    if (!IsValidName(name)) {
        return Err() << "invalid service name '" << name << "'";
    }

    Service* old_service = service_list_->FindService(name);
    if (old_service) {
        return Err() << "ignored duplicate definition of service '" << name << "'";
    }
...
Result<Success> ServiceParser::ParseLineSection(std::vector<std::string>&& args, int line) {
    return service_ ? service_->ParseLine(std::move(args)) : Ok();
}
