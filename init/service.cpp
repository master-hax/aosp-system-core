...
void Service::DumpState() const {
    ...
    LOG(INFO) << "  class '" << base::Join(classnames_, " ") << "'";
    LOG(INFO) << "  exec " << base::Join(args_, " ");
    std::for_each(descriptors_.begin(), descriptors_.end(),
...
bool Service::ParseSocket(const std::vector<std::string>& args, std::string* err) {
    if (!base::StartsWith(args[2], "dgram") && !base::StartsWith(args[2], "stream") &&
        !base::StartsWith(args[2], "seqpacket")) {
...
Service* ServiceManager::MakeExecOneshotService(const std::vector<std::string>& args) {
    ...
    exec_count_++;
    std::string name =
        "exec " + std::to_string(exec_count_) + " (" + base::Join(str_args, " ") + ")";
