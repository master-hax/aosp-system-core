...
TEST(ueventd, setegid_IsPerThread) {
    // We must be root for this test to run.
    if (getuid() != 0) return;
...
TEST(ueventd, setfscreatecon_IsPerThread) {
    // We must be root for this test to run.
    if (getuid() != 0) return;
    // SELinux must be enabled and in permissive mode for this test to run.
    if (!is_selinux_enabled() || security_getenforce() == 1) return;
...
TEST(ueventd, selabel_lookup_MultiThreaded) {
    ...
    std::vector<std::thread> threads;
    std::generate_n(back_inserter(threads), 10, [&]() { return std::thread(thread_function); });
