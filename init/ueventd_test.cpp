...
static int FutexWake(volatile void* ftx, int count) {
    return syscall(__NR_futex, ftx, FUTEX_WAKE, count, nullptr, nullptr, 0);
}

static int FutexWait(volatile void* ftx, int value) {
    return syscall(__NR_futex, ftx, FUTEX_WAIT, value, nullptr, nullptr, 0);
}

template <typename T, typename F>
void WriteFromMultipleThreads(std::vector<std::pair<std::string, T>>& files_and_parameters,
                              F function) {
    std::atomic_uint threads_ready;
    std::atomic_uint can_create_files;

    auto make_thread_function = [&](const std::string& file, const auto& parameter) {
        return [&]() {
            function(parameter);

            threads_ready++;
            FutexWake(&threads_ready, 1);

            while (!can_create_files) {
                FutexWait(&can_create_files, 0);
            }
    ...
    auto threads_ready_count = threads_ready.load();
    while (threads_ready_count != threads.size()) {
        FutexWait(&threads_ready, threads_ready_count);
        threads_ready_count = threads_ready.load();
    }

    can_create_files = 1;
    FutexWake(&can_create_files, std::numeric_limits<int>::max());
