...
int setservicecon(const char *s)
{
    if (is_selinux_enabled() >= 0 && security_check_context(s) < 0)
        return -1;
    servicecon = s;
    return 0;
}
...
void service_start(struct service *svc, const char *dynamic_args)
{
    ...
    /* as requested, set our gid, supplemental gids, and uid */
        ...
        } else if (needs_setservicecon && servicecon) {
            if (is_selinux_enabled() > 0 && setexeccon(servicecon) < 0) {
