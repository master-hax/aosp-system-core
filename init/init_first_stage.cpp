...
bool FirstStageMount::InitDevices() {
    ...
    if (!InitRequiredDevices()) return false;
...
bool FirstStageMount::InitRequiredDevices() {
    ...
    if (need_dm_verity_) {
        ...
        if (!found) {
            uevent_listener_.PollWithTimeout(dm_callback, std::chrono::seconds(10));
        }
        if (!found) {
    ...
    if (!required_devices_partition_names_.empty()) {
        uevent_listener_.PollWithTimeout(uevent_callback, std::chrono::seconds(10));
    }
...
bool FirstStageMount::InitVerityDevice(const std::string& verity_device) {
    ...
    if (!found) {
        uevent_listener_.PollWithTimeout(verity_callback, std::chrono::seconds(10));
    }
    if (!found) {
        LOG(ERROR) << "device-mapper device not found";
        return false;
