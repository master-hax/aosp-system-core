...
bool FirstStageMount::MountPartitions() {
    for (auto it = mount_fstab_recs_.begin(); it != mount_fstab_recs_.end();) {
        bool mounted = false;
        std::string start_mount_point = (*it)->mount_point;
        do {
            if (!mounted) {
                if (SetUpDmVerity(*it)) {
            ...
            it++;
        } while (it != mount_fstab_recs_.end() && (*it)->mount_point == start_mount_point);
