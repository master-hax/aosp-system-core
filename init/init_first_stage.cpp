...
ListenerAction FirstStageMountVBootV2::UeventCallback(const Uevent& uevent) {
    ...
    int mmc_host_pos = uevent.path.find("/mmc_host");
    std::string uevent_partition_name = uevent.path.substr(uevent.path.rfind("/", mmc_host_pos - 1) + 1,
			mmc_host_pos - uevent.path.rfind("/", mmc_host_pos - 1) -1);
    if (!uevent.partition_name.empty() && uevent_partition_name == devices_partition_path &&
