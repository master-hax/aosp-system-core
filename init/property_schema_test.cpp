...
TEST(property_schema, CheckSchema_any) {
    EXPECT_TRUE(CheckSchema("any", ""));
    EXPECT_TRUE(CheckSchema("any", "-234"));
    EXPECT_TRUE(CheckSchema("any", "234"));
    EXPECT_TRUE(CheckSchema("any", "true"));
    EXPECT_TRUE(CheckSchema("any", "false"));
    EXPECT_TRUE(CheckSchema("any", "45645634563456345634563456"));
    EXPECT_TRUE(CheckSchema("any", "some other string"));
}
...
TEST(property_schema, CheckSchema_regex) {
    EXPECT_TRUE(CheckSchema("regex \\d+", "123"));
    EXPECT_FALSE(CheckSchema("regex \\d+", "123abc"));

    EXPECT_TRUE(CheckSchema("regex [a-z]+", "abc"));
    EXPECT_FALSE(CheckSchema("regex [a-z]+", "abc123"));

    EXPECT_TRUE(CheckSchema("regex [a-z]+\\s+[a-z]+", "abc abcd"));
    EXPECT_TRUE(CheckSchema("regex [a-z]+ [a-z]+", "abc abcd"));
    EXPECT_FALSE(CheckSchema("regex [a-z]+ [a-z]+", "abc"));
}
...
TEST(property_schema, IsSchemaValid) {
    ...
    EXPECT_TRUE(IsSchemaValid("any"));
    EXPECT_TRUE(IsSchemaValid("bool"));
    EXPECT_FALSE(IsSchemaValid("any other"));
    ...
    EXPECT_FALSE(
        IsSchemaValid("int " + std::to_string(std::numeric_limits<int64_t>::min()) + "9 0"));
    ...
    EXPECT_FALSE(
        IsSchemaValid("uint " + std::to_string(std::numeric_limits<int64_t>::max()) + " 0"));
    ...
    EXPECT_TRUE(IsSchemaValid("regex \\s+"));
    EXPECT_TRUE(IsSchemaValid("regex [a-z]+"));
    EXPECT_TRUE(IsSchemaValid("regex [a-z]+\\s+\\d+"));
    EXPECT_FALSE(IsSchemaValid("regex [a-"));
    EXPECT_FALSE(IsSchemaValid("regex (a-"));
}
