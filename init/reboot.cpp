...
void RunRebootThread(unsigned int cmd, std::chrono::milliseconds shutdown_timeout) {
    ...
    if (sem_init(&reboot_semaphore, false, 0) == -1 ||
        clock_gettime(CLOCK_MONOTONIC, &shutdown_timeout_timespec) == -1) {
    ...
    constexpr unsigned int shutdown_watchdog_timeout = 30;
...
// reboot_target Reboot target string like "bootloader". Otherwise, it should be an
//              empty string.
