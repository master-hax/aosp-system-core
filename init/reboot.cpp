...
void DoReboot(unsigned int cmd, const std::string& reason, const std::string& rebootTarget,
    ...
    auto shutdown_timeout_ms = 0ms;
    if (!SHUTDOWN_ZERO_TIMEOUT) {
        if (is_thermal_shutdown) {
            constexpr unsigned int thermal_shutdown_timeout = 1;
            shutdown_timeout_ms = std::chrono::milliseconds(thermal_shutdown_timeout);
        } else {
            constexpr unsigned int shutdown_timeout_default = 6;
            auto shutdown_timeout_property = android::base::GetUintProperty(
                "ro.build.shutdown_timeout", shutdown_timeout_default);
            shutdown_timeout_ms = std::chrono::milliseconds(shutdown_timeout_property);
        }
    }
    LOG(INFO) << "Shutdown timeout: " << shutdown_timeout_ms.count() << " ms";
    ...
    if (shutdown_timeout_ms > 0ms) {
        ...
        auto termination_wait_timeout_ms = shutdown_timeout_ms / 2;
        while (t.duration() < termination_wait_timeout_ms) {
    ...
    sync();
    UmountStat stat = TryUmountAndFsck(runFsck, shutdown_timeout_ms - t.duration());
