...
class MountEntry {
  public:
    ...
    bool Umount(bool remountFirst) {
        int r;
        if (writable_ && remountFirst) {
            r = mount(mnt_fsname_.c_str(), mnt_dir_.c_str(), mnt_type_.c_str(),
                      MS_REMOUNT | MS_RDONLY, nullptr);
            if (r == 0) {
                LOG(INFO) << "remounted " << mnt_fsname_ << ":" << mnt_dir_ << " opts " << mnt_opts_;
            } else {
                PLOG(WARNING) << "cannot remount " << mnt_fsname_ << ":" << mnt_dir_ << " opts "
                              << mnt_opts_;
    ...
    void FlushBlockDevice() {
        android::base::unique_fd fd(open(mnt_fsname_.c_str(), O_RDONLY | O_EXCL));
        if (fd < 0) {
            ...
            PLOG(ERROR) << "Cannot open block device exclusively:" << mnt_fsname_;
            fd.reset(open(mnt_fsname_.c_str(), O_RDONLY));
            if (fd < 0) {
                PLOG(ERROR) << "Cannot open block device:" << mnt_fsname_;
                return;
            }
        }
        if (ioctl(fd, BLKFLSBUF, 0) < 0) {
            PLOG(ERROR) << "BLKFLSBUF ioctl failed for block device:" << mnt_fsname_;
        }
    }
...
static bool UmountPartitions(std::vector<MountEntry>* partitions, bool remountFirst) {
    ...
    for (auto& entry : *partitions) {
        if (!entry.Umount(remountFirst)) umount_done = false;
    }
    return umount_done;
}
