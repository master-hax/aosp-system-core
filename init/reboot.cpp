...
bool HandlePowerctlMessage(const std::string& command) {
    ...
    auto shutdown_hdl = [cmd, command, reboot_target, run_fsck](const std::vector<std::string>&) {
        DoReboot(cmd, command, reboot_target, run_fsck);
        return 0;
    };
    ActionManager::GetInstance().QueueBuiltinAction(shutdown_hdl, "shutdown_done");
