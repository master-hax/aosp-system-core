...
void RebootThread(unsigned int cmd, bool is_thermal_shutdown,
                  std::chrono::milliseconds shutdown_timeout, bool runFsck,
...
void DoReboot(unsigned int cmd, const std::string& reason, const std::string& rebootTarget,
    ...
    if (sem_init(&reboot_semaphore, false, 0) == -1 ||
        ...
        // These should never fail, but if they do, skip the graceful shutdown and reboot immediately.
        RebootSystem(cmd, rebootTarget);
        abort();
    }

    std::thread reboot_thread(&RebootThread, cmd, is_thermal_shutdown, shutdown_timeout, runFsck,
                              &reboot_semaphore);
    ...
    if (runFsck) {
        // Additional 5 minute timeout for the fsck.
        shutdown_timeout_timespec.tv_sec += 5 * 60;
    }
    ...
    if (sem_return == -1) {
        LOG(ERROR) << "Reboot thread timed out, dumping tasks";
        android::base::WriteStringToFile("t", "/proc/sysrq-trigger");
    }
