...
static void DoReboot(unsigned int cmd, const std::string& reason, const std::string& rebootTarget,
    ...
    if (!SHUTDOWN_ZERO_TIMEOUT) {
        ...
        auto shutdown_timeout_property = android::base::GetUintProperty("ro.build.shutdown_timeout",
                                                                        shutdown_timeout_default);
        shutdown_timeout = std::chrono::seconds(shutdown_timeout_property);
        if (is_thermal_shutdown && shutdown_timeout_property > max_thermal_shutdown_timeout) {
            shutdown_timeout = std::chrono::seconds(max_thermal_shutdown_timeout);
        }
    }
