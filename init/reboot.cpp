...
class MountEntry {
  public:
    ...
    static bool IsEmulatedDevice(const struct mntent& mntent) {
        static constexpr char SDCARDFS_NAME[] = "sdcardfs";
        return android::base::StartsWith(mntent.mnt_fsname, "/data/") &&
               strncmp(mntent.mnt_type, SDCARDFS_NAME, sizeof(SDCARDFS_NAME) - 1) == 0;
    }
...
static void RebootSystem(unsigned int cmd, const std::string& rebootTarget) {
    switch (cmd) {
    ...
    // normal case should not return from reboot.
