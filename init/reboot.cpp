...
static void DumpUmountDebuggingInfo(bool dumpAll) {
    int status;
...
static UmountStat TryUmountAndFsck(bool runFsck, int timeoutMs) {
    ...
    if (stat != UMOUNT_STAT_SUCCESS) {
        ...
        if (DUMP_ON_UMOUNT_FAILURE) DumpUmountDebuggingInfo(false /*dumpAll*/);
        KillAllProcesses();
        // even if it succeeds, still it is timeout and do not run fsck with all processes killed
        UmountPartitions(0);
        if (DUMP_ON_UMOUNT_FAILURE) DumpUmountDebuggingInfo(true /*dumpAll*/);
    }
