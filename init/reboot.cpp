...
void RebootMonitorThread(unsigned int cmd, const std::string& rebootTarget, sem_t* reboot_semaphore,
                         std::chrono::milliseconds shutdown_timeout, bool* RebootMonRun) {
    timespec shutdown_timeout_timespec;
    unsigned int remaining_shutdown_time = 0;

    while (*RebootMonRun == true) {
        sem_wait(reboot_semaphore);
        ...
        if (remaining_shutdown_time == 0) {
            ...
            // If there are some remaining shutdown time left from previous round, we just use
            // remaining time here.
