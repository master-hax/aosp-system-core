...
static UmountStat TryUmountAndFsck(bool runFsck, std::chrono::milliseconds timeout,
    ...
    if (stat == UMOUNT_STAT_SUCCESS && runFsck) {
        // To terminate reboot monitor thread.
...
void RebootMonitorThread(sem_t* reboot_semaphore, std::chrono::milliseconds shutdown_timeout) {
    ...
    if (sem_return == -1) {
        ...
        if (android::base::GetProperty("ro.debuggable", "0") == "1") {
            ...
            std::string string_pid = std::to_string(getpid());
...
static void DoReboot(unsigned int cmd, const std::string& reason, const std::string& rebootTarget,
    ...
    std::thread reboot_monitor_thread(&RebootMonitorThread, &reboot_semaphore, shutdown_timeout);
    ...
    UmountStat stat = TryUmountAndFsck(runFsck, shutdown_timeout - t.duration(), &reboot_semaphore);
