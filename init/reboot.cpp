...
static void DumpUmountDebuggingInfo() {
    ...
    android::base::WriteStringToFile("l", PROC_SYSRQ);
    android::base::WriteStringToFile("w", PROC_SYSRQ);
}
...
static void KillAllProcesses() {
    android::base::WriteStringToFile("i", PROC_SYSRQ);
}
...
void RebootMonitorThread(unsigned int cmd, const std::string& rebootTarget, sem_t* reboot_semaphore,
    ...
    if (sem_return == -1) {
        ...
        if (cmd == ANDROID_RB_POWEROFF) {
            android::base::WriteStringToFile("s", PROC_SYSRQ);

            android::base::WriteStringToFile("u", PROC_SYSRQ);
