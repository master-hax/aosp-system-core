...
void DoReboot(unsigned int cmd, const std::string& reason, const std::string& rebootTarget,
    ...
    const std::set<std::string> firstToKills{"tombstoned"};
    const std::set<std::string> killAfterApps{"logd", "adbd"};
    // watchdogd is a vendor specific component but should be alive to complete shutdown safely.
    const std::set<std::string> toStarts{"watchdogd", "vold"};
    ServiceManager::GetInstance().ForEachService(
        [&firstToKills, &killAfterApps, &toStarts](Service* s) {
            if (firstToKills.count(s->name())) {
                s->Stop();
            } else if (killAfterApps.count(s->name())) {
                s->SetShutdownCritical();
            } else if (toStarts.count(s->name())) {
                s->Start();
                s->SetShutdownCritical();
            } else if (s->classnames().count("shutdown_critical")) {
                s->SetShutdownCritical();
            }
        });
    ...
    ServiceManager::GetInstance().ForEachService([&killAfterApps](Service* s) {
        if (!s->IsShutdownCritical() || killAfterApps.count(s->name())) s->Stop();
    });
    ServiceManager::GetInstance().ReapAnyOutstandingChildren();
