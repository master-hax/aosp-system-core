...
void FirmwareTestWithExternalHandler(const std::string& test_name, bool expect_new_firmware) {
    ...
    auto external_firmware_handler =
            ExternalFirmwareHandler("/devices/led", "test_firmware001.bin", test_path);
    ...
    // Always test the base case that the handler isn't invoked if the devpath or firmware don't
    // match.
    ...
    auto uevent_different_firmware = Uevent{
            .path = "/devices/led",
            .firmware = "test_firmware002.bin",
    };
    EXPECT_EQ("test_firmware002.bin", firmware_handler.GetFirmwarePath(uevent_different_firmware));
}
...
int HandleChange(int argc, char** argv) {
    ...
    if (getenv("DEVPATH") != "/devices/led"s) {
