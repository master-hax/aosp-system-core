...
static void write_persistent_property(const char *name, const char *value)
{
    ...
    fd = mkstemp(tempPath);
    if (fd < 0) {
        LOG(ERROR) << "Unable to write persistent property to temp file "
...
static void load_persistent_properties() {
    ...
    while ((entry = readdir(dir.get())) != NULL) {
        ...
        if (fd == -1) {
            LOG(ERROR) << "Unable to open persistent property file \""
        ...
        struct stat sb;
        if (fstat(fd, &sb) == -1) {
            LOG(ERROR) << "fstat on property file \"" << entry->d_name
        ...
        if (length >= 0) {
            ...
            LOG(ERROR) << "Unable to read persistent property file "
                       << entry->d_name << ": " << strerror(errno);
        }
        close(fd);
    }
