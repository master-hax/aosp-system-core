...
static void set_build_fingerprint(bool legacy) {
    std::string build_fingerprint = ConstructBuildFingerprint(legacy, GetProperty);
    std::string build_prop_name = legacy ? LEGACY_FINGERPRINT_PROP : FINGERPRINT_PROP;
    LOG(INFO) << "Setting property '" << build_prop_name << "' to '" << build_fingerprint << "'";

    std::string error;
    uint32_t res = PropertySet(build_prop_name, build_fingerprint, &error);
    if (res != PROP_SUCCESS) {
        LOG(ERROR) << "Error setting property '" << build_prop_name << "': err=" << res << " ("
                   << error << ")";
    }
}
