...
class Executor {
  ...
  private:
    ...
    void StartThread() {
        ...
        auto thread = std::thread{&Executor::ThreadFunction, this};
...
static bool stop_property_service = false;
...
static void HandleInitSocket() {
    ...
    switch (init_message.msg_case()) {
        ...
        case InitMessage::kStopPropertyService: {
            stop_property_service = true;
            break;
        }
        default:
...
static void PropertyServiceThread() {
    ...
    while (true) {
        ...
        async_control_message.MaybeJoin();
        async_restorecon.MaybeJoin();
        if (stop_property_service) {
            stop_property_service = false;
            return;
        }
    }
}
...
void SendStopPropertyServiceMessage() {
    auto init_message = InitMessage{};
    init_message.set_stop_property_service(true);
...
void StopPropertyService() {
    async_control_message.StopAndJoin();
    async_restorecon.StopAndJoin();
    SendStopPropertyServiceMessage();
    property_service_thread.join();
}

void RestartPropertyService() {
    auto new_thread = std::thread{PropertyServiceThread};
    property_service_thread.swap(new_thread);
    async_restorecon.Restart();
    async_control_message.Restart();
}
