...
void property_init() {
    ...
    if (pa_workspace.fd == -1) {
        PLOG(ERROR) << "Failed to open " << PROP_FILENAME;
        return;
    }
}
...
static void write_persistent_property(const char *name, const char *value)
{
    ...
    fd = mkstemp(tempPath);
    if (fd < 0) {
        PLOG(ERROR) << "Unable to write persistent property to temp file "
                    << tempPath;
        return;
    }
...
static void handle_property_set_fd()
{
    ...
    if (nr == 0) {
        LOG(ERROR) << "sys_prop: timeout waiting for uid=" << cr.uid
                   << " to send property message.";
        close(s);
        return;
    } else if (nr < 0) {
        PLOG(ERROR) << "sys_prop: error waiting for uid=" << cr.uid
                    << " to send property message: ";
        close(s);
        return;
    }
    ...
    if(r != sizeof(prop_msg)) {
        PLOG(ERROR) << "sys_prop: mis-match msg size received: " << r
...
static void load_persistent_properties() {
    ...
    while ((entry = readdir(dir.get())) != NULL) {
        ...
        if (fd == -1) {
            PLOG(ERROR) << "Unable to open persistent property file \""
                        << entry->d_name << "\"";
            continue;
        }
        ...
        struct stat sb;
        if (fstat(fd, &sb) == -1) {
            PLOG(ERROR) << "fstat on property file \"" << entry->d_name
                        << "\" failed";
            close(fd);
            continue;
        }
        ...
        if (length >= 0) {
            ...
            PLOG(ERROR) << "Unable to read persistent property file "
                        << entry->d_name;
        }
        close(fd);
    }
