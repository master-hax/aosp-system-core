...
static void handle_property_set(SocketConnection& socket,
  ...
  if (StartsWith(name, "ctl.")) {
      ...
      property_info_area->GetPropertyInfo(control_string.c_str(), &target_context, &schema);
      ...
      const char* target_context = nullptr;
      const char* schema = nullptr;
      property_info_area->GetPropertyInfo(name.c_str(), &target_context, &schema);
      ...
      if (schema == nullptr || !CheckSchema(schema, value)) {
          LOG(ERROR) << "sys_prop(" << cmd_name << "): schema check failed, schema: '"
                     << (schema ?: "(null)") << "' value: '" << value << "'";
          if (!legacy_protocol) {
...
Result<PropertyInfoEntry> ParsePropertyInfoLine(const std::string& line) {
    ...
    if (!schema.empty() && !IsSchemaValid(schema)) {
        return Error() << "Schema '" << schema << "' is not valid";
    }
