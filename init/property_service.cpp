...
static void handle_property_set_fd() {
    ...
    case PROP_MSG_SETPROP: {
        ...
        std::string error;
        auto result = HandlePropertySet(prop_name, prop_value, source_context, cr, nullptr, &error);
...
uint32_t InitPropertySet(const std::string& name, const std::string& value) {
    ...
    std::string error;
    auto result = HandlePropertySet(name, value, kInitContext, cr, nullptr, &error);
    if (*result != PROP_SUCCESS) {
...
static void load_override_properties() {
    if (ALLOW_LOCAL_PROP_OVERRIDE) {
        ...
        for (const auto& [name, value] : properties) {
            std::string error;
            if (PropertySet(name, value, nullptr, &error) != PROP_SUCCESS) {
        ...
        for (const auto& source : Split(ro_product_props_source_order, ",")) {
            ...
            if (!target_prop_val.empty()) {
                LOG(INFO) << "Setting product property " << base_prop << " to '" << target_prop_val
                          << "' (from " << target_prop << ")";
                std::string error;
                auto res = PropertySet(base_prop, target_prop_val, nullptr, &error);
                if (*res != PROP_SUCCESS) {
...
static void property_initialize_build_id() {
    ...
    std::string error;
    auto res = PropertySet(ID_PROP, build_id, nullptr, &error);
    if (res != PROP_SUCCESS) {
...
static void property_derive_legacy_build_fingerprint() {
    ...
    std::string error;
    auto res = PropertySet(LEGACY_FINGERPRINT_PROP, legacy_build_fingerprint, nullptr, &error);
    if (*res != PROP_SUCCESS) {
...
static void property_derive_build_fingerprint() {
    ...
    std::string error;
    auto res = PropertySet(FINGERPRINT_PROP, build_fingerprint, nullptr, &error);
    if (*res != PROP_SUCCESS) {
...
static void property_initialize_ro_cpu_abilist() {
    ...
    for (const auto& [prop, prop_val] : set_prop_list) {
        ...
        std::string error;
        auto res = PropertySet(prop, prop_val, nullptr, &error);
        if (*res != PROP_SUCCESS) {
...
static void property_initialize_ro_vendor_api_level() {
    ...
    std::string error;
    auto res = PropertySet(VENDOR_API_LEVEL_PROP, std::to_string(api_level), nullptr, &error);
    if (*res != PROP_SUCCESS) {
