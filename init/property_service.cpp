...
bool LoadPropertyContextsFromFile(const std::string& filename,
  ...
  for (const auto& line : lines) {
    ...
    if (!property_and_context) {
      LOG(ERROR) << "Could not read line from '" << filename << "': " << property_and_context.error();
      continue;
    }
...
void CreateSerializedPropertyContext() {
  ...
  if (access("/system/etc/selinux/plat_property_contexts", R_OK) != -1) {
    ...
    if (!LoadPropertyContextsFromFile("/plat_property_contexts",
                                      &properties_and_contexts)) {
      return;
    }
    LoadPropertyContextsFromFile("/nonplat_property_contexts",
                                 &properties_and_contexts);
  }
  auto serialized_contexts = BuildTrie(properties_and_contexts,
                                       std::vector<std::pair<std::string, std::string>>(),
                                       "u:object_r:default_prop:s0");
  if (!serialized_contexts) {
    LOG(ERROR) << "Unable to serialize property contexts: " << serialized_contexts.error();
    return;
  }

  constexpr static const char kPropertyContextsPath[] = "/dev/__properties__/property_contexts";
  if (!WriteStringToFile(*serialized_contexts, kPropertyContextsPath,
                         0444, 0, 0, false)) {
