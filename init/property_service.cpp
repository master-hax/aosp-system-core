...
void HandleControlResponse(const PropertyMessage& property_message) {
    ...
    auto& control_response = property_message.control_response();
    uint32_t response =
...
static void HandleInitSocket() {
    ...
    switch (property_message.msg_case()) {
        case PropertyMessage::kControlResponse: {
            HandleControlResponse(property_message);
            break;
        }
        default:
            LOG(ERROR) << "Unknown/Unexpected message type from property service: "
