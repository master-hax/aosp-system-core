        ...
        *eol++ = 0;
        ...
        if (!strncmp(key, "import ", 7) && flen == 0) {
            ...
            *value++ = 0;
            ...
            ucred cr = {.pid = 1, .uid = 0, .gid = 0};
            std::string error;
            /* If timestamp of ro.product.build.fingerprint value is later than ro.build.fingerprint value,
               use the former value replace the value of the latter @{ */
            if(!strcmp(key, "ro.build.fingerprint")) {
                std::string property_value = android::base::GetProperty("ro.product.build.fingerprint", "");
                if(!property_value.empty() && strcmp(property_value.c_str(), value) > 0) {
