...
static void property_initialize_ro_product_props() {
    ...
    for (size_t i = 0; i < arraysize(RO_PRODUCT_PROPS); i++) {
        std::string base_prop(RO_PRODUCT_PROPS_PREFIX);
        base_prop += RO_PRODUCT_PROPS[i];

        std::string base_prop_val = android::base::GetProperty(base_prop, EMPTY);
        if (!base_prop_val.empty()) {
            continue;
        }

        for (size_t j = 0; j < arraysize(RO_PRODUCT_PROPS_SOURCE_ORDER); j++) {
            std::string target_prop(RO_PRODUCT_PROPS_PREFIX);
            target_prop += RO_PRODUCT_PROPS_SOURCE_ORDER[j];
            target_prop += RO_PRODUCT_PROPS[i];

            std::string target_prop_val = android::base::GetProperty(target_prop, EMPTY);
...
static void property_derive_build_fingerprint() {
    ...
    build_fingerprint = android::base::GetProperty("ro.product.brand", UNKNOWN);
    build_fingerprint += '/';
    build_fingerprint += android::base::GetProperty("ro.product.name", UNKNOWN);
    build_fingerprint += '/';
    build_fingerprint += android::base::GetProperty("ro.product.device", UNKNOWN);
    build_fingerprint += ':';
    build_fingerprint += android::base::GetProperty("ro.build.version.release", UNKNOWN);
    build_fingerprint += '/';
    build_fingerprint += android::base::GetProperty("ro.build.id", UNKNOWN);
    build_fingerprint += '/';
    build_fingerprint += android::base::GetProperty("ro.build.version.incremental", UNKNOWN);
    build_fingerprint += ':';
    build_fingerprint += android::base::GetProperty("ro.build.type", UNKNOWN);
    build_fingerprint += '/';
    build_fingerprint += android::base::GetProperty("ro.build.tags", UNKNOWN);
