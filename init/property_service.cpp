...
constexpr auto MAX_API_LEVEL_IF_UNSET = 1000000;
...
static void property_initialize_ro_vendor_api_level() {
    ...
    auto board_api_level =
            android::base::GetIntProperty("ro.board.first_api_level", MAX_API_LEVEL_IF_UNSET);
    if (board_api_level != MAX_API_LEVEL_IF_UNSET) {
        // Update the board_api_level with "ro.board.api_level" only if both "ro.board.api_level"
        // and "ro.board.first_api_level" are defined.
        board_api_level = android::base::GetIntProperty("ro.board.api_level", board_api_level);
    }

    auto product_first_api_level =
            android::base::GetIntProperty("ro.product.first_api_level", MAX_API_LEVEL_IF_UNSET);
    if (product_first_api_level == MAX_API_LEVEL_IF_UNSET) {
        // Fallback to "ro.build.version.sdk" if the "ro.product.first_api_level" is not defined.
        product_first_api_level =
                android::base::GetIntProperty("ro.build.version.sdk", MAX_API_LEVEL_IF_UNSET);
    }

    auto api_level = std::min(vendor_api_level_of(product_first_api_level), board_api_level);
    std::string error;
