...
bool LoadKernelModules(BootMode boot_mode, bool want_console, bool want_parallel, bool want_parallel_loop,
                       int& modules_loaded) {
    struct utsname uts {};
    if (uname(&uts)) {
    ...
    bool retval = (want_parallel) ? m.LoadModulesParallel(std::thread::hardware_concurrency(), want_parallel_loop)
...
#undef CHECKCALL
    ...
    auto want_parallel_loop =
            bootconfig.find("androidboot.load_modules_loop = \"true\"") != std::string::npos;

    boot_clock::time_point module_start_time = boot_clock::now();
    int module_count = 0;
    BootMode boot_mode = GetBootMode(cmdline, bootconfig);
    if (!LoadKernelModules(boot_mode, want_console, want_parallel,
                           want_parallel_loop, module_count)) {
