...
bool LoadKernelModules(BootMode boot_mode, bool want_console, bool want_parallel,
    bool want_parallel_loop, int& modules_loaded) {
    struct utsname uts {};
    if (uname(&uts)) {
    ...
    bool retval = (want_parallel) ?
            m.LoadModulesParallel(std::thread::hardware_concurrency(), want_parallel_loop):
...
#undef CHECKCALL
    ...
    auto want_parallel_loop =
            bootconfig.find("androidboot.disable_parallel_loop = \"true\"") == std::string::npos;
