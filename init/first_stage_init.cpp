...
bool LoadKernelModules(bool recovery, bool want_console, bool want_parallel, int& modules_loaded) {
    ...
    bool retval = (want_parallel) ? m.LoadModulesParallel(8) : m.LoadListedModules(!want_console);
    modules_loaded = m.GetModuleCount();
    if (modules_loaded > 0) {
        return retval;
    }
...
#undef CHECKCALL
    ...
    auto want_parallel = cmdline.find("load_modules_parallel=true") != std::string::npos;
