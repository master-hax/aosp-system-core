...
class FirstStageMount {
  ...
  protected:
    ...
    bool InitRequiredDmLinearDevices(const android::fs_mgr::LpMetadata& metadata);
...
bool FirstStageMount::InitRequiredDmLinearDevices(const android::fs_mgr::LpMetadata& metadata) {
    std::string slot_suffix = fs_mgr_get_slot_suffix();
    for (const auto& block_device : metadata.block_devices) {
        std::string partition_name =
                GetBlockDevicePartitionName(metadata, block_device, slot_suffix);
...
bool FirstStageMount::CreateLogicalPartitions() {
    ...
    auto after_parse = [this](const android::fs_mgr::LpMetadata& metadata) -> bool {
        return InitRequiredDmLinearDevices(metadata);
    };
    return android::fs_mgr::CreateLogicalPartitions(lp_metadata_partition_, after_parse);
}
