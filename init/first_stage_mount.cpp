...
// are well-defined interfaces between them and /system. Otherwise, the GSI
...
bool FirstStageMount::TrySkipProductPartitions() {
    ...
    // Only allows skipping /product or /product_services.
    const std::vector<std::string> kValidSkipPartitions = {"product", "product_services"};

    for (const auto& skip_partition : Split(skip_config, "\n")) {
        if (skip_partition.empty()) {
            continue;
        }
        if (find(kValidSkipPartitions.begin(), kValidSkipPartitions.end(), skip_partition) ==
            kValidSkipPartitions.end()) {
        ...
        auto removing_fstab_entry =
                std::find_if(mount_fstab_recs_.begin(), mount_fstab_recs_.end(),
                             [&skip_partition](const auto& rec) {
                                 return rec->mount_point == "/" + skip_partition;
                             });
