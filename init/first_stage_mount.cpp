...
class FirstStageMountVBootV2 : public FirstStageMount {
  public:
    ...
    bool DoCreateDevices() override;  // Creates devices and logical partitions from storage devices
    bool DoFirstStageMount() override;  // Mounts fstab entries read from device tree.
...
static Result<Fstab> ReadFirstStageFstabMicrodroid(const std::string& cmdline) {
    Fstab fstab;
    static const constexpr char* kMicrodroidFstab = "/fstab.microdroid";
    if (!ReadFstabFromFile(kMicrodroidFstab, &fstab)) {
        return Error() << "failed to read " << kMicrodroidFstab;
    }
