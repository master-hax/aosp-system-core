...
class FirstStageMountVBootV2 : public FirstStageMount {
  ...
  protected:
    ...
    std::vector<std::string> vbmeta_parts_;
...
FirstStageMountVBootV2::FirstStageMountVBootV2(Fstab fstab)
    ...
    for (auto&& partition : Split(device_tree_vbmeta_parts, ",")) {
        if (!partition.empty()) {
            vbmeta_parts_.emplace_back(std::move(partition));
        }
    }

    for (const auto& entry : fstab_) {
        if (!entry.vbmeta_partition.empty()) {
            vbmeta_parts_.emplace_back(entry.vbmeta_partition);
        }
    }

    if (vbmeta_parts_.empty()) {
...
bool FirstStageMountVBootV2::GetDmVerityDevices() {
    ...
    // e.g., The vbmeta_parts_ will be "vbmeta,boot,system,vendor"
    // for libavb to verify metadata, even if there is only /vendor in the
    // above mount_fstab_recs_.
    if (need_dm_verity_) {
        if (vbmeta_parts_.empty()) {
            LOG(ERROR) << "Missing vbmeta parts";
            return false;
        }
        std::string ab_suffix = fs_mgr_get_slot_suffix();
        for (const auto& partition : vbmeta_parts_) {
