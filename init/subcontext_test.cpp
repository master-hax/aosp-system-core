...
TestFunctionMap BuildTestFunctionMap() {
    ...
    test_function_map.Add(
        "return_pids_as_error", 0, 0, true,
        [](const std::vector<std::string>& args, const std::string& context) -> Result<Success> {
            return Error() << getpid() << " " << getppid();
        });

    // For SetProp
    test_function_map.Add("setprop", 2, 2, true,
    ...
    test_function_map.Add("add_word", 1, 1, true,
                          [words](const std::vector<std::string>& args, const std::string& context) {
                              words->emplace_back(args[1]);
                              return Success();
                          });
    test_function_map.Add("return_words_as_error", 0, 0, true,
                          [words](const std::vector<std::string>& args, const std::string& context)
                              -> Result<Success> { return Error() << Join(*words, " "); });

    // For RecoverAfterAbort
    test_function_map.Add(
        "cause_log_fatal", 0, 0, true,
        [](const std::vector<std::string>& args, const std::string& context) -> Result<Success> {
