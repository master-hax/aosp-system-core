...
TEST(subcontext, CheckDifferentPid) {
    RunTest([](auto& subcontext, auto& context_string) {
        ...
        auto pids = Split(result.error().error_string, " ");
...
TEST(subcontext, MultipleCommands) {
    RunTest([](auto& subcontext, auto& context_string) {
        ...
        ASSERT_FALSE(result);
        EXPECT_EQ(Join(expected_words, " "), result.error().error_string);
...
TEST(subcontext, RecoverAfterAbort) {
    RunTest([](auto& subcontext, auto& context_string) {
        ...
        EXPECT_EQ("Sane error!", result2.error().error_string);
...
TEST(subcontext, ContextString) {
    RunTest([](auto& subcontext, auto& context_string) {
        ...
        ASSERT_FALSE(result);
        ASSERT_EQ(context_string, result.error().error_string);
    });
}
...
TEST(subcontext, ExpandArgsFailure) {
    RunTest([](auto& subcontext, auto& context_string) {
        ...
        auto result = subcontext.ExpandArgs(args);
        ASSERT_FALSE(result);
        EXPECT_EQ("Failed to expand '" + args[1] + "'", result.error().error_string);
    });
}
