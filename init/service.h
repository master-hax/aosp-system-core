...
private:
    ...
    using LineHandler = bool (Service::*) (const std::vector<std::string>& args,
                                           std::string* err);
    using ServiceHandlerMap =
        const std::map<std::string, std::tuple<std::size_t, std::size_t, LineHandler>>;

    static ServiceHandlerMap service_option_map_;
