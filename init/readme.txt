...
An example is the logcatd.rc and Android.mk files located in
system/core/logcat directory.  The LOCAL_INIT_RC macro in the
Android.mk files places logcatd.rc in /system/etc/init/ during the
build process.  Init loads logcatd.rc during the mount_all command and allows
the service to be run and the action to be queued when appropriate.

This break up of init .rc files according to their daemon is preferred
to the previously used monolithic init .rc files.  This approach ensures
that the only service entries that init reads correspond to services whose
binaries are in fact present on the file system, which was not the case with
the monolithic init .rc files.  This additionals will aid in merge conflict
resolution when multiple services are added to the system, as they will go
into separate files.
