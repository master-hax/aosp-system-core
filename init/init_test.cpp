...
void TestStartApexServices(const std::vector<std::string>& service_names, const std::string& apex_name) {
...
void InitApexService(const std::string_view& init_template) {
    ...
    LOG(INFO) << "apex init script: " << init_script;
...
void TestApexServicesInit(const std::vector<std::string>& apex_services,
            const std::vector<std::string>& other_apex_services, const std::vector<std::string> non_apex_services) {
    ...
    LOG(INFO) << "Done stop apex services";
...
service apex_test_service_multi_apex_c /system/bin/toybox
    user shell
    group shell
    seclabel $selabel
)init";
...
service apex_test_service_non_apex /system/bin/toybox
    user shell
    group shell
    seclabel $selabel
)init";
...
service apex_test_service_mixed_a /system/bin/yes
    ...
    std::string_view other_apex_script_template = R"init(
service apex_test_service_mixed_b /system/bin/id
    user shell
    group shell
    seclabel $selabel
)init";
    ...
    std::string_view non_apex_script_template = R"init(
service apex_test_service_mixed_c /system/bin/toybox
    user shell
    group shell
    seclabel $selabel
)init";
