...
void TestInit(const std::string& init_script_file, const BuiltinFunctionMap& test_function_map,
              ...
              ServiceList* service_list, Subcontext* subcontext = nullptr) {
...
void TestInitText(const std::string& init_script, const BuiltinFunctionMap& test_function_map,
    ...
    TestInit(tf.path, test_function_map, commands, 
        action_manager, service_list, subcontext);
}
...
void InitApexService(const std::string_view& init_template, const std::string& apex_name,
    ...
    if (apex_name.empty()) {
       TestInitText(init_script, test_function_map, commands, &ActionManager::GetInstance(),
            &ServiceList::GetInstance(), subcontext);
    }
    else {
        ...
        TestInitText(apex_dir, init_script, test_function_map, commands,
            &ActionManager::GetInstance(), &ServiceList::GetInstance(), subcontext);
    }
}
...
TEST(init, RemoveActionFromApex) {
    ...
    std::unique_ptr<Subcontext> subcontext(new Subcontext({"/vendor"}, kVendorContext, true ));
    std::vector<std::string> apex_list = {"com.android.apex.test_service",
                "com.android.other_apex.test_service"};
    subcontext->SetApexList(std::move(apex_list));
...
on boot
    ...
    InitApexService(script_template, "com.android.apex.test_service", subcontext.get());
...
on boot
    ...
    InitApexService(non_apex_script_template, "", subcontext.get());
