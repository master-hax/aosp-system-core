...
StableActionablePropertyManager::StableActionablePropertyManager() {
    ...
    if (enabled_) {
        ...
        LoadExceptionalActionablePropertyFile();
    }
}

void StableActionablePropertyManager::LoadExceptionalActionableProperties(const std::string& data) {
    for (const auto& line : android::base::Split(data, "\n")) {
        std::string property_name = android::base::Trim(line);
        if (!property_name.empty()) {
            exceptional_actionable_properties_.emplace(property_name);
        }
    }
}

void StableActionablePropertyManager::LoadExceptionalActionablePropertyFile() {
    ...
    LoadExceptionalActionableProperties(*file_contents);
}
...
bool StableActionablePropertyManager::IsActionable(const std::string& prop_name) {
    ...
    if (exceptional_actionable_properties_.find(prop_name) !=
        exceptional_actionable_properties_.end()) {
        return true;
    }
