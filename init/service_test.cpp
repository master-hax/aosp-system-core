...
static std::string CgroupPath(pid_t pid) {
    ...
    while (std::getline(is, line)) {
        if (line.substr(0, 3) == "0::") {
            return line.substr(3, std::string::npos);
        }
    }
    return {};
}
...
class ServiceStopTest : public testing::TestWithParam<bool> {
  public:
    ServiceStopTest() {
        ...
        EXPECT_GE(sigprocmask(SIG_BLOCK, &mask, nullptr), 0);
        sigchld_fd_.reset(signalfd(-1, &mask, SFD_CLOEXEC));
        EXPECT_GE(sigchld_fd_, 0);
        LOG(INFO) << "Changing sigchld_fd into " << sigchld_fd_.get();
        saved_sigchld_fd_ = Service::SetSigchldFd(sigchld_fd_.get());
    }
    ~ServiceStopTest() {
        LOG(INFO) << "Changing sigchld_fd into " << saved_sigchld_fd_;
        Service::SetSigchldFd(saved_sigchld_fd_);
        sigset_t mask;
        sigemptyset(&mask);
        sigaddset(&mask, SIGCHLD);
        EXPECT_GE(sigprocmask(SIG_UNBLOCK, &mask, nullptr), 0);
    }

  private:
    unique_fd sigchld_fd_;
    int saved_sigchld_fd_ = -1;
};
