...
node path:
  ...
  2. USB devices are created as /dev/<uevent DEVNAME> if DEVNAME was specified for the uevent,
     otherwise as /dev/bus/usb/<bus_id>/<device_id> where bus_id = uevent MINOR / 128 + 1 and
     device_id = uevent MINOR % 128 + 1.
...
When /dev/null is created, its permissions will be set to 0666, its user to root and its group to
root.
...
device in. The section takes the below format.
...
Indicates that all nodes with a SUBSYSTEM=sound will be created as /dev/snd/<basename uevent
DEVPATH>.
...
attribute, cpufreq/scaling_max_freq, will have its permissions set to 0664, its user to to system
...
entries. Ueventd will wait until after `post-fs` in init, to keep retrying before believing the
...
nodes. For boot time purposes, this is done in parallel across a set of child processes. ueventd.cpp
