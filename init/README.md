...
`class\_reset <serviceclass>`
> Stop all services of the specified class if they are
  currently running, without disabling them. They can be restarted
  later using class\_start.
...
`restorecon\_recursive <path> [ <path>\* ]`
...
`verity\_update\_state <mount_point>`
> Internal implementation detail used to update dm-verity state and
  set the partition._mount\_point_.verified properties used by adb remount
...
`ro.boottime.init.cold\_boot\_wait`
