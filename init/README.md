...
the system.img mount to / (if necessary).  Note that in Android Q, system.img always contains
TARGET_ROOT_OUT and always is mounted at / by the time first stage init finishes.
...
mount system.img, since it is by definition already mounted as the rootfs.
