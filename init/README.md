...
> Start a service running if it is not already running.
  ...
  This creates an important consequences that if the service offers
  ...
  Secondly, prerequisites from init, such as chown'ing a file that's
  needed by the service must be handled before the service has been
  started.  Otherwise, there will be a race condition and it is
  possible that init will handle the prerequisite before the service
  actually uses it _sometimes_, causing difficult to debug issues.
