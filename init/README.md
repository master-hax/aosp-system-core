...
1. The `seclabel` option must be used to provide the original SELabel for the
   service that is run within logwrapper.
2. SELinux must either be run in permissive mode (recommended) or the domain
   transitions from init to logwrapper and from logwrapper to the service it's
   running must be added to their respective SEPolicy files.
