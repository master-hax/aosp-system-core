...
Note that when `ro.property_service.async_persist_write` is `true`, there is no defined ordering
...
When `ro.property_service.async_persist_write` is `true`, triggers for these
