...
class KeywordMap {
  public:
    ...
    const Result<Function> FindFunction(const std::vector<std::string>& args) const {
        ...
        if (args.empty()) return Err() << "Keyword needed, but not provided";
        ...
        if (function_info_it == map().end()) {
            return Err() << StringPrintf("Invalid keyword '%s'", keyword.c_str());
        }
        ...
        if (min_args == max_args && num_args != min_args) {
            return Err() << StringPrintf("%s requires %zu argument%s", keyword.c_str(), min_args,
                                         (min_args > 1 || min_args == 0) ? "s" : "");
        }

        if (num_args < min_args || num_args > max_args) {
            if (max_args == std::numeric_limits<decltype(max_args)>::max()) {
                return Err() << StringPrintf("%s requires at least %zu argument%s", keyword.c_str(),
                                             min_args, min_args > 1 ? "s" : "");
            } else {
                return Err() << StringPrintf("%s requires between %zu and %zu arguments",
                                             keyword.c_str(), min_args, max_args);
            }
        }

        return Ok(std::get<Function>(function_info));
    }
