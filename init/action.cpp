...
Command::Command(BuiltinFunction f,
                        ...
                        const std::string& filename, int line, std::string* err)
{
    ...
    auto function_info = FindBuiltinFunctionInfo(args[0]);
    if (!function_info) {
        *err = StringPrintf("invalid command '%s'", args[0].c_str());
        return false;
    }
    ...
    auto function = std::get<BuiltinFunction>(*function_info);
...
void Action::AddCommands(const Action& action)
{
...
void ActionManager::AddAction(std::unique_ptr<Action> action)
{
    ...
    if (old_action_it != actions_.end()) {
        (*old_action_it)->AddCommands(*action);
    } else {
