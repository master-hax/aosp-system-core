...
Result<Success> Command::InvokeFunc() const {
    ...
    for (std::size_t i = 1; i < args_.size(); ++i) {
        if (!expand_props(args_[i], &expanded_args[i])) {
            return Err() << "cannot expand '" << args_[i] << "'";
        }
    }
...
Result<Success> Action::AddCommand(const std::vector<std::string>& args, int line) {
    if (!function_map_) {
        return Err() << "no function map available";
    }

    auto function = function_map_->FindFunction(args);
    if (!function) return Err() << function.error();
...
Result<Success> Action::ParsePropertyTrigger(const std::string& trigger) {
    ...
    if (equal_pos == std::string::npos) {
        return Err() << "property trigger found without matching '='";
    }
    ...
    if (auto [it, inserted] = property_triggers_.emplace(prop_name, prop_value); !inserted) {
        return Err() << "multiple property triggers found for same property";
    }
    return Ok();
}
...
Result<Success> Action::InitTriggers(const std::vector<std::string>& args) {
    ...
    for (std::size_t i = 0; i < args.size(); ++i) {
        if (args[i].empty()) {
            return Err() << "empty trigger is not valid";
        }

        if (i % 2) {
            if (args[i] != "&&") {
                return Err() << "&& is the only symbol allowed to concatenate actions";
            } else {
                continue;
            }
        }
        ...
        if (!args[i].compare(0, prop_str.length(), prop_str)) {
            ...
            if (!event_trigger_.empty()) {
                return Err() << "multiple event triggers are not allowed";
            }
...
Result<Success> Action::InitSingleTrigger(const std::string& trigger) {
    ...
    if (auto result = InitTriggers(name_vector); !result) {
        return Err() << "InitTriggers() failed: " << result.error();
    }
    return Ok();
}
...
Result<Success> ActionParser::ParseSection(std::vector<std::string>&& args,
    ...
    if (triggers.size() < 1) {
        return Err() << "Actions must have a trigger";
    }
...
Result<Success> ActionParser::ParseLineSection(std::vector<std::string>&& args, int line) {
    return action_ ? action_->AddCommand(std::move(args), line) : Ok();
}
