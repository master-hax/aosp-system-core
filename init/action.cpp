...
Result<Success> RunBuiltinFunction(BuiltinFunction function, const std::vector<std::string>& args) {
    std::vector<std::string> expanded_args;
    expanded_args.resize(args.size());
    expanded_args[0] = args[0];
...
Result<Success> ActionParser::ParseSection(std::vector<std::string>&& args,
    ...
    for (const auto& [prefix, subcontext] : subcontexts_) {
        if (StartsWith(filename, prefix.c_str())) {
