                  ...
                  gid_t gid, const char *socketcon)
{
    ...
    if (fd < 0) {
        LOG(ERROR) << "Failed to open socket '" << name << "': " << strerror(errno);
        return -1;
    }
    ...
    if (ret != 0 && errno != ENOENT) {
        LOG(ERROR) << "Failed to unlink old socket '" << name << "': " << strerror(errno);
        goto out_close;
    }
    ...
    if (ret) {
        LOG(ERROR) << "Failed to bind socket '" << name << "': " << strerror(errno);
        goto out_unlink;
    }
    ...
    if ( (chown(addr.sun_path, uid, gid) == 1) || 
        ...
        LOG(ERROR) << "Failed set perms on socket '" << name << "': " << strerror(errno);
        goto out_unlink;
    }
...
int write_file(const char* path, const char* content) {
    ...
    if (fd == -1) {
        LOG(INFO) << "write_file: Unable to open '" << path << "': " << strerror(errno);
        return -1;
    }
    int result = android::base::WriteStringToFd(content, fd) ? 0 : -1;
    if (result == -1) {
        LOG(INFO) << "write_file: Unable to write to '" << path << "': " << strerror(errno);
    }
    close(fd);
    return result;
}
...
void make_link(const char *oldpath, const char *newpath)
{
    ...
    if (ret) {
        LOG(ERROR) << "Failed to create directory " << buf << ": "
                   << strerror(errno) << " (" << errno << ")";
    }

    ret = symlink(oldpath, newpath);
    if (ret && errno != EEXIST) {
        LOG(ERROR) << "Failed to symlink " << oldpath << " to " << newpath << ": "
                   << strerror(errno) << " (" << errno << ")";
    }
}
