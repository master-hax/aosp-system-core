...
static bool get_boot_config_from_kernel_cmdline(const std::string& name, std::string* out_val) {
    CHECK(out_val != nullptr);

    std::string cmdline_key("androidboot." + name);
    bool found = false;
...
const std::string& get_android_dt_dir() {
    ...
    std::call_once(lazy_init_flag, [&]() {
        // A custom Android DT path can be specified in kernel cmdline
        if (!get_boot_config_from_kernel_cmdline("android_dt_dir", &android_dt_dir)) {
