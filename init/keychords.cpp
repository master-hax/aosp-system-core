...
Keychords::Entry::Entry(std::chrono::milliseconds duration)
    : notified_(false), duration_(duration), matched_(kMatchedOff) {}
    ...
    for (auto& e : entries_) {
        ...
        for (auto& code : e.first) {
            ...
            if (!current_.GetBit(code)) {
                ...
                e.second.matched_ = e.second.kMatchedOff;
                found = false;
                break;
            }
        }
        ...
        if (e.second.duration_ != e.second.kDurationOff) {
            e.second.matched_ = android::base::boot_clock::now() + e.second.duration_;
            continue;
        }
...
std::optional<std::chrono::milliseconds> Keychords::CheckAndCalculateNextIfLess(
    ...
    for (auto& e : entries_) {
        if (e.second.notified_ || (e.second.duration_ == e.second.kDurationOff) ||
            (e.second.matched_ == e.second.kMatchedOff))
            continue;
        if (now == e.second.kMatchedOff) now = android::base::boot_clock::now();
        if (e.second.matched_ > now) {
            auto duration =
                std::chrono::duration_cast<std::chrono::milliseconds>(e.second.matched_ - now);
