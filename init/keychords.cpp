...
struct KeychordEntry {
    ...
    KeychordEntry(std::chrono::milliseconds duration, const std::set<int>& keycodes)
        : duration(duration), matched(matched_off), keycodes(keycodes), notified(false) {}
};
...
bool RegisterKeychord(const std::set<int>& keycodes) {
    ...
    keychord_entries.emplace_back(KeychordEntry(
        (code < 0) ? std::chrono::milliseconds(-code) : KeychordEntry::duration_off, keycodes));
    return true;
}
...
std::optional<std::chrono::milliseconds> KeychordWait(std::optional<std::chrono::milliseconds> wait) {
    if (keychord_entries.empty()) return wait;
