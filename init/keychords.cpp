...
Keychords::~Keychords() {
    ...
    while (!registration.empty()) {
        GeteventCloseDevice(registration.begin()->first);
    }
}
...
size_t Keychords::Mask::size() const {
    return bits.size() * sizeof(mask_t) * bits_per_byte;
}
...
Keychords::Entry::Entry(const std::set<int>& keycodes) : keycodes(keycodes), notify(false) {}
