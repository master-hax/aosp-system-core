...
void add_service_keycodes(Service* svc)
{
    ...
    if (svc->get_keycodes()) {
        ...
        size = sizeof(*keychord) + svc->get_nkeycodes() * sizeof(keychord->keycodes[0]);
        ...
        keychord->count = svc->get_nkeycodes();
        svc->set_keychord_id(keychord->id);

        for (i = 0; i < svc->get_nkeycodes(); i++) {
            keychord->keycodes[i] = svc->get_keycodes()[i];
        }
        keychords_count++;
...
static void handle_keychord() {
    ...
    if (adb_enabled == "running") {
        Service* svc = ServiceManager::GetInstance().ServiceFindByKeychord(id);
        if (svc) {
...
void keychord_init() {
    ServiceManager::GetInstance().ServiceForEach(add_service_keycodes);
