    ...
    for (auto& [keycodes, entry] : entries_) {
        ...
        for (auto& code : keycodes) {
            ...
            if (!current_.GetBit(code)) {
                ...
                entry.matched = entry.kMatchedOff;
                found = false;
                break;
            }
        }
        ...
        if (entry.duration != entry.kDurationOff) {
...
std::optional<std::chrono::milliseconds> Keychords::CheckAndCalculateNextIfLess(
    ...
    for (auto& [keycodes, entry] : entries_) {
        if (entry.notified || (entry.duration == entry.kDurationOff) ||
            (entry.matched == entry.kMatchedOff))
            continue;
        if (now == entry.kMatchedOff) now = android::base::boot_clock::now();
