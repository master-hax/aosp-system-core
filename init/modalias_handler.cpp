...
ModaliasHandler::ModaliasHandler() {
    ...
    alias_parser.AddSingleLineParser("alias", aliasCallback);
    for (auto base_path : base_paths) alias_parser.ParseConfig(base_path + "modules.alias");

    Parser dep_parser;
    auto depCallback = std::bind(&ModaliasHandler::ParseDepCallback, this, _1);
    dep_parser.AddSingleLineParser("", depCallback);
    for (auto base_path : base_paths) dep_parser.ParseConfig(base_path + "modules.dep");
}
...
Result<Success> ModaliasHandler::InsmodWithDeps(const std::string& module_name,
    ...
    if (module_deps_.count(module_name) < 1) {
        return Error() << "Module '" << module_name << "' not in dependency file";
    }

    auto& dependencies = module_deps_[module_name];
