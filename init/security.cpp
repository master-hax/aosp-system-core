...
// older kernel.
//
// There is no direct/synchronous way of finding out that a syscall failed
// due to SELinux. Therefore we test for a combination of success/failures
// that are explained by the platform's SELinux policy for the "init"
// domain:
// * cpu-scoped perf_event is allowed
// * read() on the event fd is disallowed with EACCES
...
Result<void> TestPerfEventSelinuxAction(const BuiltinArguments&) {
    ...
    unique_fd fd(static_cast<int>(syscall(__NR_perf_event_open, &pe, /*pid=*/-1,
                                          ...
                                          /*group_fd=*/-1, /*flags=*/0)));
    if (fd == -1) {
        LOG(ERROR) << "Unexpected perf_event_open error: " << strerror(errno);
        return {};
    }
    ...
    if (read_ret != -1) {
        ...
        LOG(ERROR) << "Unexpected perf_event read error: " << strerror(errno);
        return {};
    }
    ...
    if (ioctl_ret != -1) {
        ...
        LOG(ERROR) << "Unexpected perf_event ioctl error: " << strerror(errno);
        return {};
    }
