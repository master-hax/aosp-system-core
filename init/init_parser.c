...
static int push_chars(char **dst, const char *chars, int cnt)
{
...
/* Method finds the closing brace and copy all characters up to the brace into a buffer.
 *
 ...
 *         will point to the next character after the brace.
 * dst   - Destination buffer to copy all the characters inside the braces.
 * limit - Max size of the destination buffer.
 *
 * Returns 0 on success,
 *        -1 on missing closing brace
 *        -2 on a too long property name
 */
static int find_closing_brace(const char *src, char **c, char *dst, size_t limit)
{
    unsigned int len;
    char *cval = *c, *cbp;
    ...
    cbp = strchr(cval, '}');
    if (!cbp) {
        /* Assuming there are multiple ${property} in "src"
         *  => the error message is relative to (*c) */
        ERROR("unable to find closing '}' in '%s'", (*c));
        return -1;
    }
    ...
    memcpy(dst, cval, len);
    dst[len] = '\0';
    *c = (cbp + 1); /* Position now right after closing brace */
    return 0;
}
...
char *expand_references(const char *src)
{
    ...
    while (*src_ptr) {
        ...
        case '{': {
            ...
            if (!dst) {
                ERROR("out of memory for re-allocation %lu", dst_sz + alloc_adjust);
                goto err;
            }
        ...
        default:
            ERROR("expected '{', or '$' after '$' in '%s'", src);
            goto err;
        }
        src_ptr = c;
    }
