...
Result<pid_t> NativeZygoteClient::SendRequest(NativeZygoteRequest const& req) {
    ...
    if (write(*socket_, serialized_string.c_str(), serialized_size) != serialized_size) {
        CloseSocket();
        EnsureSocketOpen();
        if (write(*socket_, serialized_string.c_str(), serialized_size) != serialized_size) {
    ...
    pid_t pid;
    if (read(*socket_, &pid, sizeof(pid)) != sizeof(pid)) {
        CloseSocket();
...
bool NativeZygoteClient::EnsureSocketOpen() {
    if (!socket_) {
        int sock = socket_local_client(socket_name_, ANDROID_SOCKET_NAMESPACE_RESERVED,
                                       SOCK_SEQPACKET);
        if (sock != -1) {
