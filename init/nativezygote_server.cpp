...
// From libdl_android
extern "C" void android_run_executable(const char* path, char* const argv[]);
...
void NativeZygoteServer::SetProcessAttributesAndCaps() {
    ...
    for (const auto& rlimit_param : req_.rlimits()) {
        ...
        attr.rlimits.emplace_back(rlimit_param.type(), rlim);
    }
    attr.uid = req_.uid();
    attr.gid = req_.gid();
...
void NativeZygoteServer::Specialize() {
    ...
    for (const auto& ns_to_enter : req_.namespaces_to_enter()) {
        info.namespaces_to_enter.emplace_back(ns_to_enter.nstype(), ns_to_enter.path());
    }
