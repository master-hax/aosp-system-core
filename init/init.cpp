...
void property_changed(const char *name, const char *value)
{
    if (property_triggers_enabled)
        ActionManager::get_instance().queue_property_trigger(name, value);
}
...
{
    ActionManager::get_instance().queue_all_property_triggers();
...
int main(int argc, char** argv) {
    ...
    ActionManager& aq = ActionManager::get_instance();

    aq.queue_event_trigger("early-init");

    // Queue an action that waits for coldboot done so we know ueventd has set up all of /dev...
    aq.queue_builtin_action(wait_for_coldboot_done_action, "wait_for_coldboot_done");
    // ... so that we can start queuing up actions that require stuff from /dev.
    aq.queue_builtin_action(mix_hwrng_into_linux_rng_action, "mix_hwrng_into_linux_rng");
    aq.queue_builtin_action(keychord_init_action, "keychord_init");
    aq.queue_builtin_action(console_init_action, "console_init");

    // Trigger all the boot actions to get us started.
    aq.queue_event_trigger("init");
    ...
    if (bootmode == "charger") {
        aq.queue_event_trigger("charger");
    } else {
        aq.queue_event_trigger("late-init");
    }

    // Run all property triggers based on current state of the properties.
    aq.queue_builtin_action(queue_property_triggers_action, "queue_property_triggers");

    while (true) {
        if (!waiting_for_exec) {
            aq.execute_one_command();
            restart_processes();
        }
