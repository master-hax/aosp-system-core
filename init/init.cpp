...
static void SetupCgroups(ActionManager& am) {
    ...
    for (const auto& [name, descriptor] : cgroup_descriptors) {
        ...
        if (auto result = am.QueueBuiltinAction(std::move(args), "mkdir"); !result) {
        ...
        if (controller->version() == 1) {
            ...
            if (name == "cpuset") {
                ...
                args = {"mount", name, "none", controller->path(), "nodev", "noexec", "nosuid"};
            } else {
            ...
            args = {"mount", "cgroup2", "none", controller->path(), "nodev", "noexec", "nosuid"};
        }
        if (auto result = am.QueueBuiltinAction(std::move(args), "mount"); !result) {
    ...
    if (auto result = am.QueueBuiltinAction(std::move(args), "chmod"); !result) {
