...
static void RecordStageBoottimes(const boot_clock::time_point& second_stage_start_time) {
    ...
    if (auto first_stage_start_time_str = getenv(env_first_stage_started_at);
        first_stage_start_time_str) {
        property_set("ro.boottime.init", first_stage_start_time_str);
        android::base::ParseInt(first_stage_start_time_str, &first_stage_start_time_ns);
    }
    unsetenv(env_first_stage_started_at);

    int64_t selinux_start_time_ns = -1;
    if (auto selinux_start_time_str = getenv(env_selinux_started_at); selinux_start_time_str) {
        android::base::ParseInt(selinux_start_time_str, &selinux_start_time_ns);
    }
    unsetenv(env_selinux_started_at);
