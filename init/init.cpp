...
class EarlyMountManager {
  ...
  protected:
    ...
    std::vector<fstab_rec*> fstab_recs_;
};
...
// Early mount /vendor, /system, or /odm. The fstab is read from device-tree.
...
EarlyMountManager::EarlyMountManager() : fstab_(fs_mgr_read_fstab_dt(), fs_mgr_free_fstab) {
    if (!fstab_) {
    ...
    for (auto mount_point : {"/odm", "/system", "/vendor"}) {
        fstab_rec* fstab_rec = fs_mgr_get_entry_for_mount_point(fstab_.get(), mount_point);
        if (fstab_rec != nullptr) {
            fstab_recs_.push_back(fstab_rec);
        }
    }
}
...
bool EarlyMountManager::StartEarlyMount() {
    ...
    if (fstab_recs_.empty()) return true;
...
bool EarlyMountManager::MountEarlyPartitions() {
    ...
    for (auto fstab_rec : fstab_recs_) {
...
bool EarlyMountVBootV1::SetUpDmVerity(fstab_rec* fstab_rec, bool* out_need_create_verity_dev) {
    *out_need_create_verity_dev = false;
    ...
    return true;  // return true to mount the partition.
}
...
bool EarlyMountVBootV2::GetDeviceInitPartitions(std::set<std::string>* out_partitions,
    ...
    *out_need_verity = false;
    ...
    // fstab_rec->blk_device has ab_suffix
    for (auto fstab_rec : fstab_recs_) {
    ...
    if (*out_need_verity) {
        ...
        for (const auto& partition : partitions) {
            ...
            // a partition twice. e.g., /vendor in both places:
            //   - device_tree_vbmeta_parts_ = "vbmeta,boot,system,vendor"
            //   - fstab_recs_: /vendor_a
...
bool EarlyMountVBootV2::SetUpDmVerity(fstab_rec* fstab_rec, bool* out_need_create_verity_dev) {
    ...
    // Sets INIT_AVB_VERSION here (first stage) for init to set ro.boot.avb_version
    // in the second stage.
