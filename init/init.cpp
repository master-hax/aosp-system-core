...
void service::NotifyStateChange(const char* new_state) {
    ...
    if (snprintf(prop_name, sizeof(prop_name), "init.svc.%s", name) >= PROP_NAME_MAX) {
        ...
        LOG(ERROR) << "Property name \"init.svc." << name << "\" too long; not setting to " << new_state;
        return;
    }
...
void service_start(struct service *svc, const char *dynamic_args)
{
    ...
    if ((!(svc->flags & SVC_ONESHOT)) && dynamic_args) {
        LOG(ERROR) << "service '" << svc->args[0] << "' must be one-shot to use dynamic args, disabling";
        svc->flags |= SVC_DISABLED;
        return;
    }
    ...
    if (pid == 0) {
        ...
        if (svc->writepid_files_) {
            ...
            for (auto& file : *svc->writepid_files_) {
                if (!android::base::WriteStringToFile(pid_str, file)) {
                    LOG(ERROR) << "couldn't write " << pid_str << " to " << file
                               << ":" << strerror(errno);
                }
            }
        }
        ...
        if (svc->ioprio_class != IoSchedClass_NONE) {
            if (android_set_ioprio(getpid(), svc->ioprio_class, svc->ioprio_pri)) {
                LOG(ERROR) << "Failed to set pid " << getpid()
                           << " ioprio = " << svc->ioprio_class << ","
                           << svc->ioprio_pri << ":" << strerror(errno);
            }
        }
        ...
        if (svc->gid) {
            if (setgid(svc->gid) != 0) {
                LOG(ERROR) << "setgid failed: " << strerror(errno);
                _exit(127);
            }
        }
        if (svc->nr_supp_gids) {
        ...
        if (svc->uid) {
            if (setuid(svc->uid) != 0) {
                LOG(ERROR) << "setuid failed: " << strerror(errno);
                _exit(127);
            }
        }
        if (svc->seclabel) {
            if (setexeccon(svc->seclabel) < 0) {
                LOG(ERROR) << "cannot setexeccon('" << svc->seclabel << "'): " << strerror(errno);
                _exit(127);
            }
        }
        ...
        if (!dynamic_args) {
            if (execve(svc->args[0], (char**) svc->args, (char**) ENV) < 0) {
                LOG(ERROR) << "cannot execve('" << svc->args[0] << "'): " << strerror(errno);
            }
        } else {
...
static int mix_hwrng_into_linux_rng_action(const std::vector<std::string>& args)
{
    ...
    if (hwrandom_fd == -1) {
        if (errno == ENOENT) {
          ...
          LOG(ERROR) << "Failed to open /dev/hw_random: " << strerror(errno);
        }
        goto ret;
    }
    ...
    if (urandom_fd == -1) {
        LOG(ERROR) << "Failed to open /dev/urandom: " << strerror(errno);
        goto ret;
    }
    ...
    while (total_bytes_written < sizeof(buf)) {
        ...
        if (chunk_size == -1) {
            LOG(ERROR) << "Failed to read from /dev/hw_random: " << strerror(errno);
            goto ret;
        ...
        if (chunk_size == -1) {
            LOG(ERROR) << "Failed to write to /dev/urandom: " << strerror(errno);
            goto ret;
        }
...
static void selinux_initialize(bool in_kernel_domain) {
    ...
    if (in_kernel_domain) {
        ...
        if (selinux_android_load_policy() < 0) {
            LOG(ERROR) << "failed to load policy: " << strerror(errno);
            security_failure();
        }
...
int main(int argc, char** argv) {
    ...
    if (is_first_stage) {
        if (restorecon("/init") == -1) {
            LOG(ERROR) << "restorecon failed: " << strerror(errno);
            security_failure();
        }
        char* path = argv[0];
        char* args[] = { path, const_cast<char*>("--second-stage"), nullptr };
        if (execv(path, args) == -1) {
            LOG(ERROR) << "execv(\"" << path << "\") failed: " << strerror(errno);
            security_failure();
        }
    }
