...
static bool mount_early_partitions(const std::vector<fstab_rec*>& fstab_recs,
    ...
    for (auto rec : fstab_recs) {
        ...
        if (fs_mgr_is_avb(rec)) {
            if (!init_avb_handle) {
                avb_handle.reset(android::fs_mgr::AvbOpen(device_file_by_name_prefix));
            ...
            } else if (android::fs_mgr::AvbSetup(avb_handle.get(), rec,
        ...
        if (need_create_dm_device) {
            // The exact block device name (rec->blk_device) is change to
            // "/dev/block/dm-XX" when dm-verity is enabled on it.
            // We create that device by running coldboot on /sys/block/dm-XX.
            std::string dm_device(basename(rec->blk_device));
            std::string syspath = "/sys/block/" + dm_device;
