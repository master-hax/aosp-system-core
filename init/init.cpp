...
// directly in the property thread, however we still must signal init to inform it that there is a
// change to process, so we use this FD.  It is non-blocking, since we do not care how many signals
// have accumulated, only that init will wake whenever it calls epoll().
