...
static bool shutting_down = true;
...
static void reset_wait_for_prop() {
...
void property_changed(const std::string& name, const std::string& value) {
    ...
    if (name == "sys.powerctl") {
        if (RebootHandler::GetInstance().HandlePowerctlMessage(value)) {
            if (waiting_for_prop) {
                LOG(INFO) << "Skip existing wait_for_prop " << wait_prop_name;
                reset_wait_for_prop();
            }
            if (ServiceManager::GetInstance().IsWaitingForExec()) {
                LOG(INFO) << "Skip existing wait_for_exec";
