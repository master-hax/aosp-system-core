...
static int CallPrefetch() {
    std::string path = "/metadata/ota/ureadahead";
    // std::string subcommand = GetProperty("ro.boot.kiwi.prefetch", "");
    const bool ureadahead_replay = access(path.c_str(), F_OK) == 0;
    const bool ureadahead_replay_force = access("/metadata/ota/ureadahead_force", F_OK) == 0;
    // if (access(path.c_str(), F_OK) != 0) {
    //     ureadahead_replay = false;
    // }
    if (!ureadahead_replay || ureadahead_replay_force) {
