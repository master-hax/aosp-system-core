...
int main(int argc, char** argv) {
    ...
    if (is_first_stage) {
        ...
        setenv("INIT_STARTED_AT", StringPrintf("%" PRIu64, uint64_t(std::chrono::duration_cast<std::chrono::nanoseconds>(start_time.time_since_epoch()).count())).c_str(), 1);
        LOG(INFO) << "init CLOCK_BOOTTIME is " << std::chrono::duration_cast<std::chrono::nanoseconds>(start_time.time_since_epoch()).count();
