...
static Result<Success> mix_hwrng_into_linux_rng_action(const std::vector<std::string>& args) {
    ...
    if (hwrandom_fd == -1) {
        if (errno == ENOENT) {
            ...
            return PErr() << "Failed to open /dev/hw_random";
        }
    }
    ...
    if (urandom_fd == -1) {
        return PErr() << "Failed to open /dev/urandom";
    }
    ...
    while (total_bytes_written < sizeof(buf)) {
        ...
        if (chunk_size == -1) {
            return PErr() << "Failed to read from /dev/hw_random";
        } else if (chunk_size == 0) {
            return PErr() << "Failed to read from /dev/hw_random: EOF";
        }

        chunk_size = TEMP_FAILURE_RETRY(write(urandom_fd, buf, chunk_size));
        if (chunk_size == -1) {
            return PErr() << "Failed to write to /dev/urandom";
        }
