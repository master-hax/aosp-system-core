...
static int StopServicesFromApex(const std::string& apex_name) {
...
static void RemoveServiceAndAction(const std::string& apex_name) {
    ...
    ServiceList::GetInstance().RemoveServiceIf([&](const std::unique_ptr<Service>& s) -> bool {
        if (s->is_from_apex() &&
            GetApexNameFromFileName(s->filename()).compare(apex_name)==0) {
...
static bool HandleControlMessage(std::string_view message, const std::string& name,
    ...
    if ( service->has_restart_paused() && action == "start") {
        LOG(ERROR) << "Restart of the service " << service->name()
                   << " has been paused during rebootless update of APEX";
        return false;
    }
