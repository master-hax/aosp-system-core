...
static void SetupSelinux(char** argv) {
    ...
    LOG(INFO) << "Checking SELinux context";

    char* our_context_raw = nullptr;
    if (getcon(&our_context_raw) != 0) {
        LOG(FATAL) << "getcon() unexpectedly failed";
    }

    std::string our_context = our_context_raw;
    free(our_context_raw);

    if (our_context == "u:r:init:s0") {
        // If we're running with the init context then we're already in stage 2 init and we return
        // back to main to let 2nd stage init run.
        return;
    }

    if (our_context != "kernel") {
        LOG(FATAL) << "Expected to either be running with context 'kernel' or "
