...
static int StopApexServices(const std::string& apex_name) {
    ...
    int still_running = StopServicesAndLogViolations(service_names, kServiceStopTimeout, true /* SIGTERM */);
    // Send SIGKILL to ones that didn't terminate cleanly.
    if (still_running > 0) {
        still_running = StopServicesAndLogViolations(service_names, kServiceStopTimeout, false /* SIGKILL */);
    }
    return still_running;
}

static Result<void> DoUnloadApex(const std::string& apex_name) {
    if (StopApexServices(apex_name) > 0) {
