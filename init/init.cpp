...
static std::string find_precompiled_split_policy() {
    constexpr const char precompiled_sepolicy[] = "/vendor/etc/selinux/precompiled_sepolicy";
...
static bool selinux_load_split_policy() {
    ...
    std::string precompiled_sepolicy(find_precompiled_split_policy());
    if (!precompiled_sepolicy.empty()) {
        android::base::unique_fd fd(
            open(precompiled_sepolicy.c_str(), O_RDONLY | O_CLOEXEC | O_BINARY));
        if (fd != -1) {
            if (selinux_android_load_policy_from_fd(fd, precompiled_sepolicy.c_str()) < 0) {
                LOG(ERROR) << "Failed to load SELinux policy from " << precompiled_sepolicy;
                return false;
            }
            return true;
        }
