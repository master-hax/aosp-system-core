...
static int init_signal_fd = -1;

static std::unique_ptr<Subcontext> subcontext;

// Init epolls various FDs to know wait for various inputs.  We previously waited on property
// changes with a blocking socket that contained the information related to the change, however, it
// was easy to fill that socket and deadlock the system.  Now we use locks to handle the property
// changes directly in the property thread, however there are still cases (triggering shutdown and
// waiting for properties), that require signalling init, so we use this FD.  It is non-blocking,
// since we do not care how many signals have accumulated, only that init will wake whenever it
// calls epoll().
...
static void SignalInitLoop() {
    const char value[] = "1";
    write(init_signal_fd, value, sizeof(value));
}
...
int SecondStageMain(int argc, char** argv) {
    ...
    while (true) {
        ...
        if (!(prop_waiter_state.IsWaiting() || Service::is_exec_service_running())) {
