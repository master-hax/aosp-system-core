...
static char **get_block_device_symlinks(struct uevent *uevent)
{
    ...
    if(!strcmp(type, "mtd")) {
        ...
        snprintf(mtd_name_path, sizeof(mtd_name_path), "/sys/devices/virtual/%s/%s/name", type, device);
        int mtd_fd = open(mtd_name_path, O_RDONLY);
        if(mtd_fd < 0) {
            ERROR("Unable to open %s for reading", mtd_name_path);
            return NULL;
        }
        char mtd_name[64];
        int nr = read(mtd_fd, mtd_name, sizeof(mtd_name) - 1);
        if (nr <= 0)
            return NULL;
        close(mtd_fd);
        mtd_name[nr - 1] = '\0';
