...
TEST(init_parser, make_exec_oneshot_service_invalid_syntax) {
    ...
    // No "--" in "exec /system/bin/toybox".
    argv[1] = "/system/bin/toybox";
...
TEST(init_parser, make_exec_oneshot_service_too_many_supplementary_gids) {
    int argc = 0;
    const char* argv[4 + NR_SVC_SUPP_GIDS + 3];
    argv[argc++] = "exec";
    argv[argc++] = "seclabel";
    argv[argc++] = "root"; // uid.
    argv[argc++] = "root"; // gid.
    for (int i = 0; i < NR_SVC_SUPP_GIDS; ++i) {
        argv[argc++] = "root"; // Supplementary gid.
    }
    argv[argc++] = "--";
    argv[argc++] = "/system/bin/id";
    argv[argc] = nullptr;
    ASSERT_EQ(nullptr, make_exec_oneshot_service(2, argv));
}

static void Test_make_exec_oneshot_service(bool seclabel, bool uid, bool gid, bool supplementary_gids) {
    int argc = 0;
    const char* argv[10];
    argv[argc++] = "exec";
    if (seclabel) {
        argv[argc++] = "u:r:su:s0"; // seclabel
        if (uid) {
            argv[argc++] = "log";      // uid
            if (gid) {
                argv[argc++] = "shell";     // gid
                if (supplementary_gids) {
                    argv[argc++] = "system";    // supplementary gid 0
                    argv[argc++] = "adb";       // supplementary gid 1
                }
            }
        }
    }
    argv[argc++] = "--";
    argv[argc++] = "/system/bin/toybox";
    argv[argc++] = "id";
    argv[argc] = nullptr;
...
TEST(init_parser, make_exec_oneshot_service_with_everything) {
    Test_make_exec_oneshot_service(true, true, true, true);
}

TEST(init_parser, make_exec_oneshot_service_with_seclabel_uid_gid) {
    Test_make_exec_oneshot_service(true, true, true, false);
}

TEST(init_parser, make_exec_oneshot_service_with_seclabel_uid) {
    Test_make_exec_oneshot_service(true, true, false, false);
}

TEST(init_parser, make_exec_oneshot_service_with_seclabel) {
    Test_make_exec_oneshot_service(true, false, false, false);
}

TEST(init_parser, make_exec_oneshot_service_with_just_command) {
    Test_make_exec_oneshot_service(false, false, false, false);
}
