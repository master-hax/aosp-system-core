...
static int do_installkey(const std::vector<std::string>& args) {
    ...
    std::string unencrypted_dir = args[1] + e4crypt_unencrypted_folder;
    if (do_installkeys_ensure_dir_exists(unencrypted_dir.c_str())) {
        PLOG(ERROR) << "Failed to create " << unencrypted_dir;
        return -1;
    }

    std::vector<std::string> exec_params = {"exec", "/system/bin/vdc", "--wait", "cryptfs",
                                            "enablefilecrypto"};
    return ServiceManager::GetInstance().Exec(exec_params) ? 0 : -1;
}
