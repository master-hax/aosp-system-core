...
static Result<void> MountApexForDefaultNamespace() {
    ...
    // Now, we're in the "default" mount namespace and need a fresh RW /apex for
    ...
    if (mount(nullptr, "/apex", nullptr, MS_PRIVATE, nullptr) == -1) {
        return ErrnoError() << "Failed to remount /apex as private";
    }

    // 2. Umount /apex. Now /apex is back to tmpfs mount and still "shared".
    if (umount2("/apex", MNT_DETACH) == -1) {
        return ErrnoError() << "Failed to umount /apex";
    }

    // 3. Make /apex private so that /apex in the bootstrap mount namespace
    //    won't receive the further APEX mount events in the default mount
    //    namespace. Processes in the bootstrap mount namespace should see
    //    bootstrap APEXes in /apex.
    if (mount(nullptr, "/apex", nullptr, MS_PRIVATE, nullptr) == -1) {
        return ErrnoError() << "Failed to remount /apex as private";
    }
