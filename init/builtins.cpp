...
static Result<void> do_mount_all(const BuiltinArguments& args) {
    ...
    auto mount_fstab_outcome = fs_mgr_mount_all(&fstab, mount_all->mode);
    ...
    if (mount_fstab_outcome.userdata_mounted) {
        ...
        LOG(INFO) << "Userdata mounted using "
                  ...
                  << " result : " << mount_fstab_outcome.code;
        initial_mount_fstab_return_code = mount_fstab_outcome.code;
    }

    if (queue_event) {
        /* queue_fs_event will queue event based on mount_fstab return code
         * and return processed return code*/
        auto queue_fs_result = queue_fs_event(mount_fstab_outcome.code, false);
