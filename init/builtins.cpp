...
static Result<void> do_mount(const BuiltinArguments& args) {
    ...
    if (android::base::StartsWith(source, "loop@")) {
        ...
        int offset = strlen("loop@");

        // Open source file
        if (wait) {
            wait_for_file(source + offset, kCommandRetryTimeout);
        }

        unique_fd fd(TEMP_FAILURE_RETRY(open(source + offset, mode | O_CLOEXEC)));
        if (fd < 0) {
            return ErrnoError() << "open(" << source + offset << ", " << mode << ") failed";
        }

        // Open loop control device
        std::string loop_control_dev = "/dev/loop-control";
        unique_fd control_fd(TEMP_FAILURE_RETRY(open(loop_control_dev.c_str(), O_RDWR | O_CLOEXEC)));
        if (control_fd < 0) {
        ...
        int rc = ioctl(control_fd.get(), LOOP_CTL_GET_FREE);
        if (rc < 0) {
            return ErrnoError() << "ioctl(" << control_fd << ", " <<  LOOP_CTL_GET_FREE
                                << ") failed";
        }
