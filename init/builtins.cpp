...
// Our aim is to please, if we miss the target, oh well. Turn off backlight
// while we are performing power down cleanup activities.
static void turnOffBacklight() {
    static const char off[] = "0";
    if (android::base::WriteStringToFile(off,
            "/sys/class/leds/lcd-backlight/brightness")) {
        return;
    }

    static const char backlight[] = "/sys/class/backlight";
    std::unique_ptr<DIR, int(*)(DIR*)> dir(opendir(backlight), closedir);
    if (!dir) {
        return;
    }
    ...
    while ((dp = readdir(dir.get())) != NULL) {
        ...
        std::string fileName = android::base::StringPrintf("%s/%s/brightness",
                                                           backlight,
                                                           dp->d_name);
        if (android::base::WriteStringToFile(off, fileName)) {
            return;
        }
    }
}
