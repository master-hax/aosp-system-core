...
int do_mount_all(int nargs, char **args)
{
    ...
    if (ret == FS_MGR_MNTALL_DEV_NEEDS_ENCRYPTION) {
         ...
         * that action.
         */
    ActionQueue::getInstance().queue_event_trigger("nonencrypted");
...
{
    ActionQueue::getInstance().queue_event_trigger(args[1]);
    return 0;
}
...
int do_verity_load_state(int nargs, char **args) {
    ...
    if (rc == 0 && mode == VERITY_MODE_LOGGING) {
        ActionQueue::getInstance().queue_event_trigger("verity-logging");
    }
    return rc;
}
