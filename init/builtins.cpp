...
static Result<Success> do_parse_apex_configs(const BuiltinArguments& args) {
    glob_t glob_result;
    // @ is added to filter the latet paths, which are bind mounts of the places
    // where the APEXes are really mounted at, out. Otherwise, we will parse the
    // same file twice.
    static constexpr char glob_pattern[] = "/apex/*@*/etc/*.rc";
    if (glob(glob_pattern, 0, nullptr, &glob_result) != 0) {
