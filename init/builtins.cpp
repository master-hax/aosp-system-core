...
static int do_wait_for_prop(const std::vector<std::string>& args) {
    ...
    if (!is_legal_property_name(name)) {
        LOG(ERROR) << "do_wait_for_prop(\"" << name << "\", \"" << value << "\") failed: bad name";
        return -1;
    }
    if (value_len >= PROP_VALUE_MAX) {
        LOG(ERROR) << "do_wait_for_prop(\"" << name << "\", \"" << value << "\") failed: "
                   << "value too long";
        return -1;
    }

    std::string curr_value = property_get(name);
    if (strncmp(curr_value.c_str(), value, PROP_VALUE_MAX)) {
        // Current property value is not equal to expected value
        wait_property(name, value);
    } else {
        LOG(INFO) << "do_wait_for_prop(\"" << name << "\", \"" << value << "\") already set";
    }
