...
static Result<Nothing> reboot_into_recovery(const std::vector<std::string>& options) {
...
static Result<Nothing> do_class_start(const BuiltinArguments& args) {
...
static Result<Nothing> do_class_start_post_data(const BuiltinArguments& args) {
...
static Result<Nothing> do_class_stop(const BuiltinArguments& args) {
    ForEachServiceInClass(args[1], &Service::Stop);
    return Nothing();
}

static Result<Nothing> do_class_reset(const BuiltinArguments& args) {
    ForEachServiceInClass(args[1], &Service::Reset);
    return Nothing();
}

static Result<Nothing> do_class_reset_post_data(const BuiltinArguments& args) {
    if (args.context != kInitContext) {
...
static Result<Nothing> do_class_restart(const BuiltinArguments& args) {
...
static Result<Nothing> do_domainname(const BuiltinArguments& args) {
...
static Result<Nothing> do_enable(const BuiltinArguments& args) {
...
static Result<Nothing> do_exec(const BuiltinArguments& args) {
...
static Result<Nothing> do_exec_background(const BuiltinArguments& args) {
    auto service = Service::MakeTemporaryOneshotService(args.args);
    if (!service) {
...
static Result<Nothing> do_exec_start(const BuiltinArguments& args) {
...
static Result<Nothing> do_export(const BuiltinArguments& args) {
...
static Result<Nothing> do_hostname(const BuiltinArguments& args) {
...
static Result<Nothing> do_ifup(const BuiltinArguments& args) {
    struct ifreq ifr;
...
static Result<Nothing> do_insmod(const BuiltinArguments& args) {
    int flags = 0;
...
static Result<Nothing> do_interface_restart(const BuiltinArguments& args) {
...
static Result<Nothing> do_interface_start(const BuiltinArguments& args) {
    Service* svc = ServiceList::GetInstance().FindInterface(args[1]);
    if (!svc) return Error() << "interface " << args[1] << " not found";
...
static Result<Nothing> do_interface_stop(const BuiltinArguments& args) {
    Service* svc = ServiceList::GetInstance().FindInterface(args[1]);
    if (!svc) return Error() << "interface " << args[1] << " not found";
    svc->Stop();
    return Nothing();
...
static Result<Nothing> do_mkdir(const BuiltinArguments& args) {
    mode_t mode = 0755;
    if (args.size() >= 3) {
...
/* umount <path> */
static Result<Nothing> do_umount(const BuiltinArguments& args) {
...
static Result<Nothing> do_mount(const BuiltinArguments& args) {
 ...
 */
static Result<Nothing> queue_fs_event(int code) {
 ...
 * not return.
 */
static Result<Nothing> do_mount_all(const BuiltinArguments& args) {
    std::size_t na = 0;
    bool import_rc = true;
...
/* umount_all <fstab> */
static Result<Nothing> do_umount_all(const BuiltinArguments& args) {
    Fstab fstab;
...
static Result<Nothing> do_swapon_all(const BuiltinArguments& args) {
    Fstab fstab;
    if (!ReadFstabFromFile(args[1], &fstab)) {
...
static Result<Nothing> do_setprop(const BuiltinArguments& args) {
    property_set(args[1], args[2]);
    return Nothing();
}

static Result<Nothing> do_setrlimit(const BuiltinArguments& args) {
...
static Result<Nothing> do_start(const BuiltinArguments& args) {
    Service* svc = ServiceList::GetInstance().FindService(args[1]);
...
static Result<Nothing> do_stop(const BuiltinArguments& args) {
    Service* svc = ServiceList::GetInstance().FindService(args[1]);
    if (!svc) return Error() << "service " << args[1] << " not found";
    svc->Stop();
    return Nothing();
...
static Result<Nothing> do_restart(const BuiltinArguments& args) {
    Service* svc = ServiceList::GetInstance().FindService(args[1]);
    if (!svc) return Error() << "service " << args[1] << " not found";
    svc->Restart();
    return Nothing();
...
static Result<Nothing> do_trigger(const BuiltinArguments& args) {
...
static Result<Nothing> do_symlink(const BuiltinArguments& args) {
...
static Result<Nothing> do_rm(const BuiltinArguments& args) {
...
static Result<Nothing> do_rmdir(const BuiltinArguments& args) {
...
static Result<Nothing> do_sysclktz(const BuiltinArguments& args) {
    struct timezone tz = {};
...
static Result<Nothing> do_verity_update_state(const BuiltinArguments& args) {
    int mode;
...
static Result<Nothing> do_write(const BuiltinArguments& args) {
...
static Result<Nothing> readahead_file(const std::string& filename, bool fully) {
...
static Result<Nothing> do_readahead(const BuiltinArguments& args) {
    struct stat sb;
...
static Result<Nothing> do_copy(const BuiltinArguments& args) {
...
static Result<Nothing> do_chown(const BuiltinArguments& args) {
...
static Result<Nothing> do_chmod(const BuiltinArguments& args) {
...
static Result<Nothing> do_restorecon(const BuiltinArguments& args) {
    int ret = 0;
...
static Result<Nothing> do_restorecon_recursive(const BuiltinArguments& args) {
    std::vector<std::string> non_const_args(args.args);
    non_const_args.insert(std::next(non_const_args.begin()), "--recursive");
    return do_restorecon({std::move(non_const_args), args.context});
}

static Result<Nothing> do_loglevel(const BuiltinArguments& args) {
...
static Result<Nothing> do_load_persist_props(const BuiltinArguments& args) {
    load_persist_props();
    return Nothing();
}

static Result<Nothing> do_load_system_props(const BuiltinArguments& args) {
    LOG(INFO) << "deprecated action `load_system_props` called.";
    return Nothing();
}

static Result<Nothing> do_wait(const BuiltinArguments& args) {
...
static Result<Nothing> do_wait_for_prop(const BuiltinArguments& args) {
...
static Result<Nothing> ExecWithRebootOnFailure(const std::string& reboot_reason,
...
static Result<Nothing> do_installkey(const BuiltinArguments& args) {
    if (!is_file_crypto()) return Nothing();
...
static Result<Nothing> do_init_user0(const BuiltinArguments& args) {
    return ExecWithRebootOnFailure(
        "init_user0_failed",
        {{"exec", "/system/bin/vdc", "--wait", "cryptfs", "init_user0"}, args.context});
}

static Result<Nothing> do_mark_post_data(const BuiltinArguments& args) {
    ServiceList::GetInstance().MarkPostData();

    return Nothing();
}

static Result<Nothing> do_parse_apex_configs(const BuiltinArguments& args) {
    glob_t glob_result;
...
static Result<Nothing> do_enter_default_mount_ns(const BuiltinArguments& args) {
