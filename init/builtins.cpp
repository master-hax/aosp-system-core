...
static Result<void> do_mount_all(const BuiltinArguments& args) {
    ...
    if (mount_fstab_result.second) {
        ...
        LOG(INFO) << "Userdata mounted using "
                  ...
                  << " result : " << mount_fstab_result.first;
        initial_mount_fstab_return_code = mount_fstab_result.first;
    }

    if (queue_event) {
        /* queue_fs_event will queue event based on mount_fstab return code
         * and return processed return code*/
        auto queue_fs_result = queue_fs_event(mount_fstab_result.first, false);
