...
static SuccessOrErr do_mkdir(const std::vector<std::string>& args) {
    ...
    if (args.size() >= 4) {
        ...
        if (!DecodeUid(args[3], &uid, &decode_uid_err)) {
            return Err() << "Unable to find UID for '" << args[3] << "': " << decode_uid_err;
        }
        gid_t gid = -1;

        if (args.size() == 5) {
            if (!DecodeUid(args[4], &gid, &decode_uid_err)) {
                return Err() << "Unable to find GID for '" << args[3] << "': " << decode_uid_err;
            }
        }
...
static SuccessOrErr do_chown(const std::vector<std::string>& args) {
    ...
    if (!DecodeUid(args[1], &uid, &decode_uid_err)) {
        return Err() << "Unable to find UID for '" << args[1] << "': " << decode_uid_err;
    }
    ...
    if (args.size() == 4) {
        if (!DecodeUid(args[2], &gid, &decode_uid_err)) {
            return Err() << "Unable to find GID for '" << args[2] << "': " << decode_uid_err;
        }
    }
