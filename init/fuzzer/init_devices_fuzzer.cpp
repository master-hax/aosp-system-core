...
constexpr int32_t kMaxVector = 5;
...
void InitDeviceFuzzer::createDeviceHandlerParam() {
    ...
    for (size_t idx = 0; idx < kMaxVector; ++idx) {
        ...
        SysfsPermissions sysfsPermissions(name, attribute, perm, uid, gid, mFdp.ConsumeBool());
        mSysfsPermissions.emplace_back(sysfsPermissions);

        Permissions permissions("/dev/device*name*", perm, uid, gid, mFdp.ConsumeBool());
