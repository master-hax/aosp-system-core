...
void InitUeventDFuzzer::process() {
    ...
    std::string refBaseName = mFdp.ConsumeBool() ? mFdp.PickValueInArray(kFscryptRefPath)
                                                 : mFdp.ConsumeRandomLengthString(kMaxBytes);
    FscryptSetDirectoryPolicy(refBaseName, mFdp.PickValueInArray(kAction),
                              mFdp.ConsumeRandomLengthString(kMaxBytes));

    if (mFdp.ConsumeBool()) {
        // sleep is used to explore different paths, because of mutex locks in the source code
        std::this_thread::sleep_for(std::chrono::milliseconds(mFdp.ConsumeIntegralInRange<int32_t>(
