...
void InitKeychordsFuzzer::invokeMountHandler() {
    ...
    /*
    REMOUNT is done to trigger EPOLLERR | EPOLLPRI which will trigger the mount handler callback.
    REMOUNT requires adb root permission. Otherwise the coverage for mount_handler file decreases.
    */
    mount(ksource.c_str(), ktarget.c_str(), kFileSystemType.c_str(), MS_REMOUNT, NULL);
    invokeCall();
}
