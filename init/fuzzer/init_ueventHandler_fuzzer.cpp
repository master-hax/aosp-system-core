...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    if (fdp.ConsumeBool()) {
        ...
        std::vector<ExternalFirmwareHandler> extenalHandlers;
        ...
        for (size_t idx = 0; idx < fdp.ConsumeIntegralInRange<size_t>(kMinSize, kMaxSize); ++idx) {
            ...
            extenalHandlers.push_back(externalFirmwareHandler);
            firmwareDirectories.push_back(fdp.ConsumeRandomLengthString(kMaxBytes));
        }
        FirmwareHandler firmwareHandler = FirmwareHandler(firmwareDirectories, extenalHandlers);
