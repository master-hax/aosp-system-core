...
static Result<std::vector<std::string>> CollectApexConfigs(
...
static Result<void> ParseConfigs(const std::vector<std::string>& configs) {
    ...
    auto filtered = FilterVersionedConfigs(
            configs, android::base::GetIntProperty("ro.build.version.sdk", INT_MAX));
    if (filtered.empty()) {
        return {};
    }
...
Result<void> ParseApexConfigs(const std::string& apex_name) {
    auto configs = OR_RETURN(CollectApexConfigs(apex_name, /*skip_apexes=*/{}));
    if (configs.empty()) {
        return {};
    }
...
Result<void> ParseAllApexConfigs(bool bootstrap) {
    ...
    if (!bootstrap) {
        ...
        // skip them in CollectApexConfigs.
