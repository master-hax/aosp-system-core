...
std::vector<std::string> CollectApexConfigs(const std::string& apex_name) {
    glob_t glob_result;
...
Result<void> ParseApexConfigs(const std::string& apex_name) {
    auto configs = CollectApexConfigs(apex_name);
    if (configs.empty()) {
        LOG(ERROR) << "Unable to find APEX config files";
        return {};
    }
