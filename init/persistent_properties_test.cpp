...
PersistentProperties VectorToPersistentProperties(
    ...
    for (const auto& [key, value] : input_properties) {
...
void CheckPropertiesEqual(std::vector<std::pair<std::string, std::string>> expected,
    ...
    for (const auto& persistent_property_record : persistent_properties.properties()) {
        auto it = std::find_if(expected.begin(), expected.end(),
                               [persistent_property_record](const auto& entry) {
                                   return entry.first == persistent_property_record.key() &&
                                   entry.second == persistent_property_record.value();
                               });
        ASSERT_TRUE(it != expected.end()) << "Found unexpected proprety ("
                                          << persistent_property_record.key()
                                          << ", " << persistent_property_record.value() << ")";
        expected.erase(it);
    }
    auto joiner = [](const auto& vector) {
        std::string result;
    ...
    EXPECT_TRUE(expected.empty()) << "expected still contains:" << joiner(expected);
}
...
TEST(persistent_properties, EndToEnd) {
    ...
    ASSERT_TRUE(WritePersistentPropertyFile(SetToPersistentProperties(persistent_properties)));
...
TEST(persistent_properties, UpdatePropertyBadParse) {
    ...
    auto read_back_properties = LoadPersistentProperties();
    EXPECT_GT(read_back_properties.properties().size(), 0U);

    auto it = std::find_if(
        read_back_properties.properties().begin(), read_back_properties.properties().end(), [](const auto& entry) {
