...
class PlatformDeviceList {
  ...
  private:
    struct DeviceEntry {
        DeviceEntry(const std::string& name, const std::string& path) : name(name), path(path) {}
        std::string name;
        std::string path;
    };
    std::vector<DeviceEntry> platform_devices_;
};
...
void PlatformDeviceList::Add(const std::string& path) {
    ...
    std::string name;
    if (android::base::StartsWith(path, devices_platform_prefix.c_str())) {
        name = path.substr(devices_platform_prefix.length());
    } else if (android::base::StartsWith(path, devices_prefix.c_str())) {
...
bool PlatformDeviceList::Find(const std::string& path, std::string* name,
                              std::string::size_type* path_length) const {
    for (auto it = platform_devices_.rbegin(); it != platform_devices_.rend(); ++it) {
        ...
        if (platform_device_path_length < path.length() &&
            ...
            android::base::StartsWith(path, it->path.c_str())) {
...
void PlatformDeviceList::Remove(const std::string& path) {
    auto it = std::find_if(platform_devices_.begin(), platform_devices_.end(),
                           [&path](const auto& entry) { return path == entry.path; });
