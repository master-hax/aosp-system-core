...
std::vector<std::string> DeviceHandler::GetBlockDeviceSymlinks(const Uevent& uevent) const {
    ...
    if (!uevent.partition_name.empty()) {
        ...
        // symlink of /dev/block/by-name/<device> for symmetry.
        auto device_name_sanitized = device;
        SanitizePartitionName(&device_name_sanitized);
        links.emplace_back("/dev/block/by-name/" + device_name_sanitized);
    }
