...
bool ReadFileToStringTimeout(const std::string& path, std::string* content,
                             std::chrono::nanoseconds timeout) {
    auto ret = false;
    android::base::Timer t;
    while (t.duration() < timeout) {
        ret = ReadFileToString(path, content);
        if (ret && !content->empty()) break;
...
static bool FindDmDevicePartition(const std::string& path, std::string* result) {
    ...
    if (!ReadFileToStringTimeout("/sys" + path + "/dm/name", result, 200ms)) {
