...
static void load_firmware(uevent* uevent, const std::string& root, int fw_fd,
                          int loading_fd, int data_fd) {
    struct stat sb;
    if (fstat(fw_fd, &sb) == -1) {
        PLOG(ERROR) << "couldn't stat firmware fd for " << uevent->firmware;
        return;
    }
    ...
    // Copy the firmware.
    int rc = sendfile(data_fd, fw_fd, nullptr, sb.st_size);
    if (rc == -1) {
...
try_loading_again:
    for (size_t i = 0; i < arraysize(firmware_dirs); i++) {
        ...
        if (fw_fd != -1) {
            load_firmware(uevent, root, fw_fd, loading_fd, data_fd);
            return;
        }
    }
    ...
    PLOG(ERROR) << "firmware: could not find firmware for " << uevent->firmware;
    // TODO: does this make sense? we won't have written "1" in we get here...
...
static void handle_firmware_event(uevent* uevent) {
    ...
    if (pid == 0) {
        ...
        LOG(INFO) << "process_firmware_event for " << uevent->firmware << " took " << t.duration() << "s";
        _exit(EXIT_SUCCESS);
    } else if (pid == -1) {
