...
bool FindPlatformDevice(const std::string& path, std::string* out_path) {
...
static void handle_platform_device_event(uevent* uevent) {
    if (uevent->action == "add") {
        ...
        auto it = std::find_if(platform_devices.begin(), platform_devices.end(),
                               [uevent](const auto& path) { return uevent->path == path; });
