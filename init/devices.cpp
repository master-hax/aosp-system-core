...
static bool FindPciDevicePrefix(const std::string& path, std::string* result) {
    ...
    if (!base::StartsWith(path, "/devices/pci")) return false;
...
static bool FindVbdDevicePrefix(const std::string& path, std::string* result) {
    ...
    if (!base::StartsWith(path, "/devices/vbd-")) return false;
...
bool Permissions::Match(const std::string& path) const {
    if (prefix_) return base::StartsWith(path, name_.c_str());
...
bool SysfsPermissions::MatchWithSubsystem(const std::string& path,
    ...
    std::string path_basename = base::Basename(path);
...
bool DeviceHandler::FindPlatformDevice(std::string path, std::string* platform_device_path) const {
    ...
    std::string directory = base::Dirname(path);

    while (directory != "/" && directory != ".") {
        std::string subsystem_link_path;
        if (base::Realpath(directory + "/subsystem", &subsystem_link_path) &&
...
std::vector<std::string> DeviceHandler::GetCharacterDeviceSymlinks(const Uevent& uevent) const {
    ...
    if (!base::StartsWith(path, "/usb")) return {};
...
std::vector<std::string> DeviceHandler::GetBlockDeviceSymlinks(const Uevent& uevent) const {
    ...
    if (FindPlatformDevice(uevent.path, &device)) {
        ...
        if (base::StartsWith(device, devices_platform_prefix.c_str())) {
            device = device.substr(devices_platform_prefix.length());
        } else if (base::StartsWith(device, devices_prefix.c_str())) {
...
void DeviceHandler::HandleDevice(const std::string& action, const std::string& devpath, int block,
    ...
    if (action == "add") {
        ...
        for (const auto& link : links) {
            if (mkdir_recursive(base::Dirname(link), 0755, sehandle_)) {
                PLOG(ERROR) << "Failed to create directory " << base::Dirname(link);
            }
    ...
    if (action == "remove") {
        for (const auto& link : links) {
            ...
            if (base::Readlink(link, &link_path) && link_path == devpath) {
                unlink(link.c_str());
            }
        }
...
void DeviceHandler::HandleBlockDeviceEvent(const Uevent& uevent) const {
    ...
    std::string name = base::Basename(uevent.path);
    std::string devpath = base + name;

    std::vector<std::string> links;
    if (base::StartsWith(uevent.path, "/devices")) {
...
void DeviceHandler::HandleGenericDeviceEvent(const Uevent& uevent) const {
    ...
    if (base::StartsWith(uevent.subsystem, "usb")) {
        if (uevent.subsystem == "usb") {
            if (!uevent.device_name.empty()) {
                ...
                devpath = base::StringPrintf("/dev/bus/usb/%03d/%03d", bus_id, device_id);
            }
        } else {
        ...
        devpath = "/dev/" + base::Basename(uevent.path);
    }

    mkdir_recursive(base::Dirname(devpath), 0755, sehandle_);
