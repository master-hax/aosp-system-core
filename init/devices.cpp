...
bool DeviceHandler::FindSysfsDevice(std::string path, std::string* sysfs_device_path) const {
    sysfs_device_path->clear();

    // Uevents don't contain the mount point, so we need to add it here.
    path.insert(0, sysfs_mount_point_);

    return Realpath(path + "/device", sysfs_device_path);
}
...
void DeviceHandler::HandleBindInternal(std::string driver_name, std::string action,
                                       const Uevent& uevent) {
    if (uevent.subsystem == "block") return;
    ...
    for (const TrackedUevent& tracked : tracked_uevents_) {
        if (tracked.canonical_device_path != (sysfs_mount_point_ + uevent.path)) continue;

        LOG(VERBOSE) << "Propagating " << uevent.action << " as " << action << " for "
                     << uevent.path;

        devpath = driver->ParseDevPath(tracked.uevent);
