...
void fixup_sys_perms(const char *upath)
{
    ...
    list_for_each(node, &sys_perms) {
        ...
        if ((strlen(buf) + strlen(dp->attr) + 1) > sizeof(attr_buf))
            break;

        snprintf(attr_buf, sizeof(attr_buf), "%s/%s", buf, dp->attr);
        INFO("fixup %s %d %d 0%o\n", attr_buf, dp->uid, dp->gid, dp->perm);
        chown(attr_buf, dp->uid, dp->gid);
        chmod(attr_buf, dp->perm);
    }
