...
Result<std::string> FirmwareHandler::RunExternalHandler(const std::string& handler,
...
std::string FirmwareHandler::GetFirmwarePath(const Uevent& uevent) const {
    for (const auto& external_handlers : external_firmware_handlers_) {
        if (external_handlers.devpath == uevent.path &&
            external_handlers.firmware == uevent.firmware) {
            LOG(INFO) << "Launching external firmware handler '" << external_handlers.handler_path
                      << "' for devpath: '" << uevent.path << "' firmware: '" << uevent.firmware
                      << "'";

            auto result = RunExternalHandler(external_handlers.handler_path, uevent);
            if (!result) {
