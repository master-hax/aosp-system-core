...
static void LockAllSystemPages() {
    auto callback = [](const android::procinfo::MapInfo& map) -> void {
        if (android::base::StartsWith(map.name, "/dev/")) {
            return;
        }
        if (!android::base::StartsWith(map.name, "/")) {
            return;
        }
        ...
        if (mlock(start, len) < 0) {
            LOG(FATAL) << "mlock failed, " << start << " for " << len << " bytes.";
        }
    };

    if (!android::procinfo::ReadProcessMaps(getpid(), callback)) {
        LOG(FATAL) << "Could not parse /proc/" << getpid() << "/maps file for init, "
