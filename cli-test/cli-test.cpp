...
static bool RunCommands(const char* what, const std::vector<std::string>& commands) {
  ...
  for (auto& command : commands) {
    ...
    if (exit_status != 0) {
      result = false;
      fprintf(stderr, "Command (%s) \"%s\" %s", what, command.c_str(),
...
static bool CheckOutput(const char* what, std::string actual_output,
  ...
  std::regex re(FILES);
  actual_output = std::regex_replace(actual_output, re, "$FILES");
...
static void ShowHelp(bool full) {
  fprintf(full ? stdout : stderr, "usage: shell-test-runner [-v] FILE...\n");
