...
fn readahead(id: usize, file: Arc<File>, record: &Record, buffer: &mut [u8]) -> Result<(), Error> {
    debug!("readahead {:?}", record);
    let _ = get_dbg(id, "readahead");

    let mut current_offset = record.offset.try_into().unwrap();
    let mut remaining_data = record.length.try_into().unwrap();
    const READ_SZ: usize = 1024 * 1024;
    let mut ret: i32 = 0;
    let _input_data: usize = record.length.try_into().unwrap();
    let mut _total_read: usize = 0;

    /*
        if input_data <= 131072 {
            return Ok(());
        }
    */
    ...
    loop {
        ...
        unsafe {
            ...
            if bytes_read == -1 {
                ret = -1;
                info!("AKILESH: pread failed {}", nix::errno::errno());
                break;
            }
            ...
            _total_read += bytes_read as usize;
            current_offset += bytes_read as off_t;
...
fn worker_internal(
    ...
    loop {
        ...
        if let Err(e) = readahead(id, file, &record, buffer) {
            if exit_on_error {
                return Err(e);
            } else {
                error!(
