...
static bool parse_line(const char* path, size_t line_number, const char* line, pkg_info* info) {
  ...
  int fields =
      sscanf(line, "%ms %lu %d %ms %ms %ms %d %ld %ms", &info->name, &uid, &debuggable, &info->data_dir,
             &info->seinfo, &gid_list, &profileable_from_shell, &info->version_code, &profiling_recipients);
