...
Missing features:
- protect the filesyste image with dm-verity
...
def CreateApex(args, work_dir):
  ...
  # Step 1: create an empty ext4 image that is succfficiently big
  ...
  f = open(android_manifest_file, 'w+')
  ...
  cmd = ['aapt']
  cmd.append('p') # package
  cmd.extend(['-M', android_manifest_file])
  cmd.extend(['-0', '']) # don't compress any file
  cmd.extend(['-F', unaligned_apex_file])
  cmd.append(content_dir)
  RunCommand(cmd, args.verbose)
