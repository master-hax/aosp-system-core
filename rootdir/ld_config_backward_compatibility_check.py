...
def find_latest_vndk_snapshot_version():
  """Retruns latest vndk snapshot version in current source tree.
  It will skip the test if the snapshot directories are not found.

  Retruns:
    latest_vndk_snapshot_version: string
  """
  vndk_dir_list = glob.glob(PREBUILTS_VNDK_DIR + "/v*")
  if not vndk_dir_list:
    print("warning: cannot find vndk snapshot directory.")
  ...
  return max(vndk_ver_list)

def get_vendor_configuration(ld_conig_file):
  """Reads the ld.config.txt file to parse the '[vendor]' section of the file.
  It finds the namespace configurations that include vendor directories.
  ...
  Returns:
    configs: dict{string:[string]}, dictionary of namespace configurations.
             it has property names as keys and the directory list as values.
  """
  try:
    conf_file = open(ld_conig_file)
  except IOError:
    print("error: could not read %s" % ld_conig_file)
    sys.exit(1)

  configs = dict()
  reading_vendor_config = False
  ...
  with conf_file:
    for line in conf_file:
      ...
      if reading_vendor_config:
        if line[0] == '[' and line[-1] == ']':
          # completed reading [vendor] section
          return configs
        ...
        directory = line[found + opr_len:].strip()
...
def check_backward_compatibility(ld_config, vndk_snapshot_version):
  ...
  old ld.config.txt file. If any of the old vendor namespace configurations
  are missing, the test will fail. It is allowed to have a new namespace
  configurations in current ld.config.txt file.
  ...
  for namespace in old_config:
    if namespace not in current_config:
      ...
      return False
    if not all(path in current_config[namespace]
                                        for path in old_config[namespace]):
      print("error: some vendor paths for %s in ld.config.%s.txt are missing "
            "in %s" % (namespace, vndk_snapshot_version, ld_config))
      return False
  return True
