...
vndk_sp_using_core_variant_libraries_file := $(library_lists_dir)/vndk_sp_using_core_variant.libraries.$(vndk_version).txt
endif
...
$(eval $(call write-libs-to-file,$(VNDK_SP_USING_CORE_VARIANT_LIBRARIES),$(vndk_sp_using_core_variant_libraries_file)))
endif
...
$(LOCAL_BUILT_MODULE): PRIVATE_VNDK_SP_USING_CORE_VARIANT_LIBRARIES_FILE := $(vndk_sp_using_core_variant_libraries_file)
deps += $(vndk_using_core_variant_libraries_file) $(vndk_sp_using_core_variant_libraries_file)
endif
...
ifeq ($(my_vndk_use_core_variant),true)
	...
	$(call private-filter-out-private-libs,$(PRIVATE_VNDK_SP_USING_CORE_VARIANT_LIBRARIES_FILE),$(PRIVATE_INTERMEDIATES_DIR)/vndk_sp_using_core_variant_filtered)
	$(hide) sed -i.bak -e "s?%VNDK_SP_USING_CORE_VARIANT_LIBRARIES%?$$(cat $(PRIVATE_INTERMEDIATES_DIR)/vndk_sp_using_core_variant_filtered)?g" $@
else
	# Unlike LLNDK or VNDK-SP, VNDK[_SP]_USING_CORE_VARIANT_LIBRARIES can be
	# nothing if TARGET_VNDK_USE_CORE_VARIANT is not set.  In this case, we need
	# to remove the entire line in the linker config so that we are not left with
	# a line like:
	#   namespace.sphal.link.default.shared_libs +=
	$(hide) sed -i.bak -e "s?^.*\+= %VNDK_USING_CORE_VARIANT_LIBRARIES%\$$??" $@
	$(hide) sed -i.bak -e "s?^.*\+= %VNDK_SP_USING_CORE_VARIANT_LIBRARIES%\$$??" $@
endif
...
vndk_sp_using_core_variant_libraries_file :=
vndk_sp_using_core_variant_libraries_list :=
endif
