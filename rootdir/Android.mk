...
# Update namespace configuration file with library lists and
# VNDK version
#
# $(1): Input file name
...
vndk_version_suffix := $(if $(BOARD_VNDK_VERSION),-$(PLATFORM_VNDK_VERSION),)
...
$$(LOCAL_BUILT_MODULE): PRIVATE_VNDK_VERSION := $$(vndk_version_suffix)
$$(LOCAL_BUILT_MODULE): $(LOCAL_PATH)/etc/$(1)
	@echo "Generate: $$< -> $$@"
	@mkdir -p $$(dir $$@)
...
$(eval $(call update_and_install_ld_config,ld.config.txt.in))

else ifeq ($(PRODUCT_TREBLE_LINKER_NAMESPACES)|$(SANITIZE_TARGET),true|)
# for treblized but VNDK not enforced devices
LOCAL_MODULE_STEM := $(call append_vndk_version,$(LOCAL_MODULE))
$(eval $(call update_and_install_ld_config,ld.config.txt))
...
# BOARD_VNDK_VERSION defined and uses the strict namespace configuration based
# on "ld.config.txt.in". However for the devices, that have not defined
