...
class ThreadCaptureTest : public ThreadListTest {
 public:
  ...
  void Fork(std::function<void()>&& child_init,
    ...
    ASSERT_EQ(0, pipe2(start_pipe, O_CLOEXEC));
    android::base::unique_fd ufd_start_pipe0{start_pipe[0]};
    android::base::unique_fd ufd_start_pipe1{start_pipe[1]};

    int stop_pipe[2] = {-1, -1};
    ASSERT_EQ(0, pipe2(stop_pipe, O_CLOEXEC));
    android::base::unique_fd ufd_stop_pipe0{stop_pipe[0]};
    android::base::unique_fd ufd_stop_pipe1{stop_pipe[1]};
    ...
    if (pid == 0) {
      // child
      child_init();
      EXPECT_EQ(1, TEMP_FAILURE_RETRY(write(start_pipe[1], "+", 1))) << strerror(errno);
      char buf;
      EXPECT_EQ(1, TEMP_FAILURE_RETRY(read(stop_pipe[0], &buf, 1))) << strerror(errno);
      child_cleanup();
      _exit(0);
    } else {
      // parent
      ASSERT_GT(pid, 0);
      char buf;
      ASSERT_EQ(1, TEMP_FAILURE_RETRY(read(start_pipe[0], &buf, 1))) << strerror(errno);

      parent(pid);

      ASSERT_EQ(1, TEMP_FAILURE_RETRY(write(stop_pipe[1], "+", 1))) << strerror(errno);
      siginfo_t info{};
...
TEST_F(ThreadCaptureTest, capture_signal) {
  ...
  [&](pid_t child){
...

    {
      ...
      usleep(100000);
      char buf;
      ASSERT_EQ(1, TEMP_FAILURE_RETRY(read(pipefd[0], &buf, 1)));
      ASSERT_EQ(buf, '+');
