...
void ion_alloc_test() {
    ...
    if (_ion_alloc_test(&fd, &handle)) return;
...

{
    ...
    if (fork()) {
        ...
        *(int*)CMSG_DATA(cmsg) = share_fd;
        ...
        sendmsg(sd[0], &msg, 0);
        if (recvmsg(sd[0], &msg, 0) < 0) perror("master recv msg 2");
        printf("master? [%10s] should be [child]\n", ptr);

        /* send ping */
        sendmsg(sd[0], &msg, 0);
        printf("master->master? [%10s]\n", ptr);
        if (recvmsg(sd[0], &msg, 0) < 0) perror("master recv 1");
        close(fd);
        _exit(0);
    } else {
        struct cmsghdr* cmsg;
        ...
        if (recvmsg(sd[1], &child_msg, 0) < 0) perror("child recv msg 1");
...
int main(int argc, char* argv[]) {
    ...
    printf("test %d, len %zu, align %zu, map_flags %d, prot %d, heap_mask %d,"
           " alloc_flags %d\n",
           test, len, align, map_flags, prot, heap_mask, alloc_flags);
    switch (test) {
        case ALLOC_TEST:
            ion_alloc_test();
            break;
