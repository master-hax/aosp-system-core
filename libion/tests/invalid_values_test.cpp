...
class InvalidValues : public IonAllHeapsTest {
 public:
    ...
    struct ion_handle *m_validHandle;
    int m_validShareFd;
    struct ion_handle * const m_badHandle = (struct ion_handle *)(-1);
};
...
void InvalidValues::SetUp()
{
    ...
    ASSERT_EQ(0, ion_alloc(m_ionFd, 4096, 0, m_firstHeap, 0, &m_validHandle))
      << m_ionFd << " " << m_firstHeap;
    ASSERT_TRUE(m_validHandle != NULL);
...
TEST_F(InvalidValues, ion_free)
{
    ...
    /* NULL handle */
    EXPECT_EQ(-EINVAL, ion_free(m_ionFd, NULL));
    /* bad handle */
...
TEST_F(InvalidValues, ion_map)
{
    ...
    /* NULL handle */
    EXPECT_EQ(-EINVAL, ion_map(m_ionFd, NULL, 4096, PROT_READ, 0, 0, &ptr, &map_fd));
    /* bad handle */
...
TEST_F(InvalidValues, ion_share)
{
    ...
    /* NULL handle */
    EXPECT_EQ(-EINVAL, ion_share(m_ionFd, NULL, &share_fd));
    /* bad handle */
