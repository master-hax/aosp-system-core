...
bool Elf::Read(uintptr_t offset, uint8_t* buffer, size_t bytes) {
  word_t data_word;
  ...
  for (size_t i = 0; i < hdr_.e_phnum; i++) {
    ...
    if (phdr.p_type == PT_NOTE) {
      ...
      while (hdr_size >= sizeof(NhdrType)) {
        NhdrType nhdr;
        if (!Read(offset, reinterpret_cast<uint8_t*>(&nhdr), sizeof(nhdr))) {
          return false;
        }
        offset += sizeof(nhdr);
        if (nhdr.n_type == NT_GNU_BUILD_ID) {
          ...
          if (!Read(offset, build_id_data, nhdr.n_descsz)) {
            return false;
          }
...
bool ElfGetBuildId(Backtrace* backtrace, uintptr_t start_addr,
                   uintptr_t end_addr, std::string* build_id) {
  Elf elf(backtrace, start_addr, end_addr);
