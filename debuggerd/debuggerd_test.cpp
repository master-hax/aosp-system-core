...
TEST(tombstoned, crash_detail_return_value) {
  ...
  crash_detail_t* cd = android_register_crash_detail_c_strs("hello", data);
  EXPECT_EQ(android_unregister_crash_detail(cd), data);
}
...
TEST_F(CrasherTest, crash_detail_single) {
  ...
  unique_fd output_fd;
  StartProcess([]() {
    android_register_crash_detail_c_strs("CRASH_DETAIL_NAME", g_crash_detail_value);
    abort();
  });
  StartIntercept(&output_fd);
  FinishCrasher();
...
TEST_F(CrasherTest, crash_detail_mixed) {
  ...
  StartProcess([]() {
    ...
    android_register_crash_detail_c_strs("CRASH_DETAIL_NAME", data);
    abort();
  });
  StartIntercept(&output_fd);
  FinishCrasher();
...
TEST_F(CrasherTest, crash_detail_many) {
  ...
  StartProcess([]() {
    for (int i = 0; i < 1000; ++i) {
      ...
      auto* h = android_register_crash_detail_c_strs(name.data(), value.data());
      android_unregister_crash_detail(h);
    }

    android_register_crash_detail_c_strs("FINAL_NAME", "FINAL_VALUE");
    android_register_crash_detail_c_strs("FINAL_NAME2", "FINAL_VALUE2");
    abort();
  });
  StartIntercept(&output_fd);
  FinishCrasher();
...
TEST_F(CrasherTest, crash_detail_single_changes) {
  ...
  unique_fd output_fd;
  StartProcess([]() {
    android_register_crash_detail_c_strs("CRASH_DETAIL_NAME", g_crash_detail_value_changes);
...
TEST_F(CrasherTest, crash_detail_multiple) {
  ...
  StartProcess([]() {
    android_register_crash_detail_c_strs("CRASH_DETAIL_NAME", g_crash_detail_value);
    android_register_crash_detail_c_strs("CRASH_DETAIL_NAME2", g_crash_detail_value2);
    abort();
  });
  StartIntercept(&output_fd);
  FinishCrasher();
...
TEST_F(CrasherTest, crash_detail_remove) {
  ...
  unique_fd output_fd;
  StartProcess([]() {
    auto* detail1 = android_register_crash_detail_c_strs("CRASH_DETAIL_NAME", g_crash_detail_value);
