...
TEST_F(CrasherTest, gwp_asan_overflow) {
  ...
  ASSERT_MATCH(result, R"(signal 11 \(SIGSEGV\), code 2 \(SEGV_ACCERR\))");
  ASSERT_MATCH(result, R"(Cause: \[GWP-ASan\]: Buffer Overflow, 0 bytes right of a 16-byte allocation.*
...
TEST_F(CrasherTest, gwp_asan_underflow) {
  ...
  ASSERT_MATCH(result, R"(signal 11 \(SIGSEGV\), code 2 \(SEGV_ACCERR\))");
  ASSERT_MATCH(result, R"(Cause: \[GWP-ASan\]: Buffer Underflow, 4 bytes left of a 16-byte allocation.*
