...
TEST_F(CrasherTest, crash_detail_single) {
  ...
  StartProcess([]() {
    android_register_crash_detail("CRASH_DETAIL_NAME", g_crash_detail_value,
                                  sizeof(g_crash_detail_value), CRASH_DETAIL_STRING);
    abort();
  });
  StartIntercept(&output_fd);
  FinishCrasher();
...
TEST_F(CrasherTest, crash_detail_single_bytes) {
  ...
  unique_fd output_fd;
  StartProcess([]() {
    android_register_crash_detail("CRASH_DETAIL_NAME", "\1", sizeof("\1"), CRASH_DETAIL_BYTES);
    abort();
  });
  StartIntercept(&output_fd);
  FinishCrasher();
  ...
  std::string result;
  ConsumeFd(std::move(output_fd), &result);
  ASSERT_MATCH(result, R"(CRASH_DETAIL_NAME: '0100')");
}
...
TEST_F(CrasherTest, crash_detail_many) {
  ...
  StartProcess([]() {
    ...
    android_register_crash_detail("FINAL_NAME", "FINAL_VALUE", sizeof("FINAL_VALUE"),
                                  CRASH_DETAIL_STRING);
    android_register_crash_detail("FINAL_NAME2", "FINAL_VALUE2", sizeof("FINAL_VALUE2"),
                                  CRASH_DETAIL_STRING);
    abort();
  });
  StartIntercept(&output_fd);
