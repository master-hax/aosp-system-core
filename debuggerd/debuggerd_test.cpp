...
TEST(tombstoned, java_trace_intercept_smoke) {
  ...
  unique_fd intercept_fd, output_fd;
  InterceptStatus status;
  tombstoned_intercept(this_pid, &intercept_fd, &output_fd, &status, kDebuggerdJavaBacktrace);
  ASSERT_EQ(InterceptStatus::kRegistered, status);

  // First connect to tombstoned requesting a native backtrace. This
  // shouldn result in a "regular" FD and not the installed intercept.
  const char native[] = "native";
  unique_fd tombstoned_socket, input_fd;
  ASSERT_TRUE(
      tombstoned_connect(this_pid, &tombstoned_socket, &input_fd, kDebuggerdNativeBacktrace));
  ...
  ASSERT_TRUE(tombstoned_connect(this_pid, &tombstoned_socket, &input_fd, kDebuggerdJavaBacktrace));
