...
GwpAsanTestParameters gwp_asan_tests[] = {
    ...
    {/* alloc_size */ static_cast<size_t>(getpagesize()), /* free_before_access */ false,
     ...
     getpagesize() == 4096 ? "Buffer Overflow, 2 bytes right of a 4096-byte allocation"
                           : "Buffer Overflow, 2 bytes right of a 16384-byte allocation"},
    {/* alloc_size */ static_cast<size_t>(getpagesize()), /* free_before_access */ false,
     /* access_offset */ -1,
     getpagesize() == 4096 ? "Buffer Underflow, 1 byte left of a 4096-byte allocation"
                           : "Buffer Underflow, 1 byte left of a 16384-byte allocation"},
};
...
TEST_F(CrasherTest, verify_map_format) {
  ...
  // Verify none.
  const char* page_string = getpagesize() == 0x1000 ? "1000" : "4000";
  match_str = android::base::StringPrintf(
      "    %s-%s ---         0      %s\\n",
