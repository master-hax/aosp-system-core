...
TEST_P(SizeParamCrasherTest, mte_uaf) {
  ...
  ASSERT_MATCH(result, R"(Cause: \[MTE\]: Use After Free, 0 bytes into a )" + std::to_string(GetParam()) + R"(-byte allocation.*
...
TEST_P(SizeParamCrasherTest, mte_overflow) {
  ...
  ASSERT_MATCH(result, R"(signal 11 \(SIGSEGV\))");
  ASSERT_MATCH(result, R"(Cause: \[MTE\]: Buffer Overflow, 0 bytes right of a )" + std::to_string(GetParam()) + R"(-byte allocation.*
...
TEST_P(SizeParamCrasherTest, mte_underflow) {
  ...
  ASSERT_MATCH(result, R"(Cause: \[MTE\]: Buffer Underflow, 4 bytes left of a )" + std::to_string(GetParam()) + R"(-byte allocation.*
