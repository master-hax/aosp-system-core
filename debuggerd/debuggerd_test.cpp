...
TEST_F(CrasherTest, capabilities) {
  ...
  StartProcess([]() {
    ...
    static_assert(CAP_LAST_CAP > 32, "CAP_LAST_CAP <= 31");
    ...
    // Turn on every other capability.
    for (int i = 0; i < 32; i += 2) {
      capdata[0].permitted |= 1ULL << i;
      capdata[0].effective |= 1ULL << i;
    }

    for (int i = 32; i < CAP_LAST_CAP; i += 2) {
      capdata[1].permitted |= 1ULL << i;
      capdata[1].effective |= 1ULL << i;
    }

    // Turn off CAP_SYS_PTRACE.
    capdata[0].permitted &= ~(1ULL << CAP_SYS_PTRACE);
    capdata[0].effective &= ~(1ULL << CAP_SYS_PTRACE);
