...
TEST_P(GwpAsanCrasherTest, run_gwp_asan_test) {
  ...
  StartProcess([&params]() {
    ...
    // debuggerd tests use logcat and a dedbuggerd handler to extract the
    // results of the test. stdout/stderr echoing the subprocess' stdout/stderr
    // is undesirable, as it trips up some codein atest that makes suer that the
    // same test that started is the same test that ended. For this reason, just
    // have the subprocess not output anything about the test to stdout/stderr
    // by closing the files.
    close(STDOUT_FILENO);
    close(STDERR_FILENO);
