...
SymbolTable *symbol_table_create(const char *filename)
{
    ...
    table->symbols = malloc(table->num_symbols * sizeof(Symbol));
    if(!table->symbols) {
        free(table);
        table = NULL;
        goto out_unmap;
    ...
    qsort(table->symbols, table->num_symbols, sizeof(Symbol), qcompar);
 ...
 *      symbol with the highest address which is still lower
 *      than the specified address.
 */
const Symbol *symbol_table_lookup(SymbolTable *table, unsigned int addr)
{
    return bsearch((void*)addr, table->symbols, table->num_symbols, sizeof(Symbol), bcompar);
}
