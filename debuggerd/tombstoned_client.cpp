...
bool tombstoned_connect(pid_t pid, unique_fd* tombstoned_socket, unique_fd* output_fd) {
  ...
  if (sockfd == -1) {
    __safe_format_log(ANDROID_LOG_ERROR, "libc", "failed to connect to tombstoned: %s",
                      strerror(errno));
    return false;
  }
  ...
  if (TEMP_FAILURE_RETRY(write(sockfd, &packet, sizeof(packet))) != sizeof(packet)) {
    __safe_format_log(ANDROID_LOG_ERROR, "libc", "failed to write DumpRequest packet: %s",
                      strerror(errno));
    return false;
  }
  ...
  if (rc == -1) {
    __safe_format_log(ANDROID_LOG_ERROR, "libc",
  ...
  if (fcntl(tmp_output_fd.get(), F_SETFL, flags | O_APPEND) != 0) {
    __safe_format_log(ANDROID_LOG_WARN, "libc", "failed to set output fd flags: %s",
                      strerror(errno));
  }
