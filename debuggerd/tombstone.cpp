...
static void dump_backtrace_and_stack(Backtrace* backtrace, log_t* log, bool is_at_fault) {
  if (backtrace->NumFrames()) {
    _LOG(log, (is_at_fault? logtype::FAULT_BACKTRACE : logtype::NOFAULT_BACKTRACE), "\nbacktrace:\n");
    dump_backtrace_to_log(backtrace, log, "    ", is_at_fault);
...
static void dump_thread(Backtrace* backtrace, log_t* log, int* total_sleep_time_usec, bool is_at_fault) {
  wait_for_stop(backtrace->Tid(), total_sleep_time_usec);

  dump_registers(log, backtrace->Tid(), is_at_fault);
  dump_backtrace_and_stack(backtrace, log, is_at_fault);
...
static bool dump_sibling_thread_report(
  ...
  while ((de = readdir(d)) != NULL) {
    ...
    if (backtrace->Unwind(0)) {
      dump_thread(backtrace.get(), log, total_sleep_time_usec, false);
    }
...
static bool dump_crash(log_t* log, pid_t pid, pid_t tid, int signal, int si_code,
  ...
  if (backtrace->Unwind(0)) {
    ...
    dump_thread(backtrace.get(), log, total_sleep_time_usec, true);
  }
...
char* engrave_tombstone(pid_t pid, pid_t tid, int signal, int original_si_code,
  ...
  *detach_failed = dump_crash(&log, pid, tid, signal, original_si_code, abort_msg_address,
  ...
  LOG_ERROR("Tombstone written to: %s\n", path);
