...
void dump_registers(log_t* log, pid_t tid, bool is_at_fault) {
  ...
  for (i = 0; i < NUM_VFP_REGS; i += 2) {
    _LOG(log, ltype, "    d%-2d %016llx  d%-2d %016llx\n",
         i, vfp_regs.fpregs[i], i+1, vfp_regs.fpregs[i+1]);
  }
  _LOG(log, ltype, "    scr %08lx\n", vfp_regs.fpscr);
#endif
}
