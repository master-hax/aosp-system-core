...
static bool perform_dump(const debugger_request_t& request, int log_fd, int tombstone_fd,
    ...
    switch (signal) {
      ...
      case SIGSTOP:
        if (request.action == DEBUGGER_ACTION_DUMP_TOMBSTONE) {
          ...
          dump_backtrace(log_fd, -1, backtrace_map, request.pid, request.tid, siblings);
        } else {
