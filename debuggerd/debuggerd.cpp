...
// Fork a process to send signals for the worker processes, after they've dropped privileges.
static bool fork_signal_sender() {
  int sfd[2];
...
static bool send_signal(pid_t pid, pid_t tid, int signal) {
  if (signal_fd == -1) {
