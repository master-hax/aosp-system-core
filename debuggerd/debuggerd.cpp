...
static void handle_request(int fd, bool* detach_failed) {
  ...
  int stop_signal;
  if (!attach_pid(request.tid, &total_sleep_time_usec, &detach_failed, &stop_signal)) {
    return;
  }
  ...
  while (signal == SIGSTOP && request.action != DEBUGGER_ACTION_DUMP_TOMBSTONE && request.action != DEBUGGER_ACTION_DUMP_BACKTRACE) {
    ALOGV("stopped -- continuing\n");
    if (ptrace(PTRACE_CONT, request.tid, 0, 0) == -1) {
      ALOGE("ptrace continue failed: %s\n", strerror(errno));
      // Need to detach.
      return;
    }

    signal = get_signal(tid, total_sleep_time_usec, NULL);
  }
  ...
  if (detach_tid(request.tid, detach_failed) && attach_gdb) {
...
static int do_server() {
  ...
  for (;;) {
    ...
    handle_request(fd, &detach_failed);
