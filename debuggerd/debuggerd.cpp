...
static void wait_for_user_action(pid_t pid) {
  ...
  ALOGE("debuggerd resuming process %d", pid);
}
...
static int read_request(int fd, debugger_request_t* out_request) {
  ...
  if (status == sizeof(debugger_msg_t)) {
    ALOGD("crash request of size %d abort_msg_address=0x%" PRIPTR "\n", status, msg.abort_msg_address);
  } else {
...
static void handle_request(int fd) {
  ALOGD("handle_request(%d)\n", fd);

  debugger_request_t request;
  memset(&request, 0, sizeof(request));
  int status = read_request(fd, &request);
  if (!status) {
    ALOGD("BOOM: pid=%d uid=%d gid=%d tid=%d\n",
      ...
      if (TEMP_FAILURE_RETRY(write(fd, "\0", 1)) != 1) {
        ...
        for (;;) {
          ...
          switch (signal) {
            ...
            default:
              ALOGD("stopped -- unexpected signal\n");
...
static int do_server() {
  ...
  ALOGE("debuggerd: " __DATE__ " " __TIME__ "\n");
