...
int dump_backtrace_to_file_timeout(pid_t tid, DebuggerdDumpType dump_type, int timeout_secs,
  ...
  android::status_t status = android::getBinderStateInfo(tid, binderState);
  if (status == android::OK) {
    if (!WriteStringToFd(binderState, fd)) {
      LOG(WARNING) << TAG "Failed to dump binder state info for pid: " << tid;
    }
  } else {
    LOG(WARNING) << TAG "Failed to get binder state info for pid: " << tid << " status: " << status;
  }
