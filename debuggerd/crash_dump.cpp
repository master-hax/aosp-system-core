...
static void ReadCrashInfo(unique_fd& fd, siginfo_t* siginfo,
  ...
  *siginfo = crash_info->siginfo;
  regs->reset(Regs::CreateFromUcontext(Regs::CurrentMachineType(), &crash_info->ucontext));
...
int main(int argc, char** argv) {
  ...
  if (ptrace(PTRACE_INTERRUPT, vm_pid, 0, 0) != 0) {
    PLOG(FATAL) << "failed to interrupt vm process";
  }
  ...
  if (syscall(__NR_kcmp, vm_pid, pseudothread_tid, KCMP_FILES, 0, 0) != 0) {
    if (errno == ENOSYS) {
      // XXX: DO NOT MERGE WITHOUT FIXING THIS
      LOG(ERROR) << "kcmp missing";
    } else {
      PLOG(FATAL) << "vm process file descriptor table mismatch";
    }
  }
  ...
  // Detach from all of our attached threads before performing the dump.
  for (const auto& [thread, _] : thread_info) {
    if (ptrace(PTRACE_DETACH, thread, 0, 0) != 0) {
      PLOG(WARNING) << "failed to detach from thread " << thread;
    }
  }

  std::shared_ptr<unwindstack::Memory> process_memory;
  std::unique_ptr<BacktraceMap> map(BacktraceMap::CreateNew(vm_pid, false, &process_memory));
  if (!map) {
  ...
  if (backtrace) {
    ...
    engrave_tombstone(std::move(output_fd), map.get(), process_memory.get(), thread_info,
                      target_thread, abort_address, &amfd_data);
  }
