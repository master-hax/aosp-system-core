...
static bool GetGuestRegistersFromCrashedProcess([[maybe_unused]] pid_t tid,
  ...
  errno = 0;
  uintptr_t fs_base = static_cast<uintptr_t>(
      ptrace(PTRACE_PEEKUSER, tid, offsetof(user_regs_struct, fs_base), nullptr));
  if (errno == 0) {
  ...
  header_ptr = static_cast<uintptr_t>(ptrace(PTRACE_PEEKDATA, tid, ptr_to_guest_slot, 0));
  if (errno != 0) {
  ...
  if (ptrace(PTRACE_PEEKDATA, tid, ptr, 0) != NATIVE_BRIDGE_GUEST_STATE_SIGNATURE) {
...
static void ReadGuestRegisters(std::unique_ptr<unwindstack::Regs>* regs, pid_t tid) {
  NativeBridgeGuestRegs guest_regs;
  bool is_success = GetGuestRegistersFromCrashedProcess(tid, &guest_regs);
  if (!is_success) {
    return;
  }
