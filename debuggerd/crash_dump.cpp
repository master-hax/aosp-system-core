...
int main(int argc, char** argv) {
  ...
  // Retarget ourselves to init, so that the signal handler can waitpid on the
  ...
  if (backtrace) {
    ...
    engrave_tombstone(output_fd.get(), backtrace_map.get(), target, main_tid, siblings,
  ...
  if (wait_for_gdb) {
    if (syscall(__NR_tgkill, target, main_tid, resume_signal)) {
