...
int main(int argc, char** argv) {
  ...
  std::string parent_proc_path = "/proc/" + std::to_string(parent);
  int parent_proc_fd = open(parent_proc_path.c_str(), O_DIRECTORY | O_RDONLY);
  if (parent_proc_fd == -1) {
    PLOG(FATAL) << "failed to open " << parent_proc_path;
  }
  ...
  check_process(parent_proc_fd, parent);
  ...
  if (resume_signal == 0) {
    if (!android::procinfo::GetProcessTids(parent, &siblings)) {
