...
static bool wait_for_stop(pid_t tid) {
  while (true) {
    int status;
    ...
    if (WIFSTOPPED(status)) {
      if (WSTOPSIG(status) != SIGTRAP) {
        LOG(WARNING) << "thread " << tid << " already stopped?";
      }
      return true;
    }
  }
...
static bool ptrace_interrupt(pid_t tid) {
...
int main(int argc, char** argv) {
...

  {
    ...
    for (pid_t thread : threads) {
      ...
      if (!ptrace_seize_thread(target_proc_fd, thread, &error)) {
        bool fatal = thread == target_thread;
  ...
  for (const auto& [thread, _] : thread_info) {
    LOG(INFO) << "detaching from thread " << thread;
    if (ptrace(PTRACE_DETACH, thread, 0, 0) != 0) {
      PLOG(ERROR) << "failed to detach from thread " << thread;
    }
  }
  ...
  if (backtrace) {
    ...
    engrave_tombstone(std::move(output_fd), map.get(), process_memory.get(), thread_info,
                      target_thread, abort_address, &open_files, &amfd_data);
  }
