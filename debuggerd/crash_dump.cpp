...
static void check_parent(int proc_fd, pid_t expected_ppid) {
  ...
  if (!android::base::GetProcessInfoFromProcPidFd(&proc_info, proc_fd)) {
...
int main(int argc, char** argv) {
  ...
  if (!android::base::GetProcessInfo(&target_info, main_tid)) {
  ...
  if (forkpid == -1) {
    ...
    // FIXME: FIXME: FIXME: Make this not a giant hack.
    sleep(1);
    exit(0);
  }
  ...
  if (resume_signal == 0) {
    if (!android::base::GetProcessTids(&siblings, parent)) {
