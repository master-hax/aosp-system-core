...
static void* GetGuestStateTlsPointer([[maybe_unused]] pid_t tid) {
  ...
  uintptr_t header_ptr = static_cast<uintptr_t>(ptrace(PTRACE_PEEKDATA, tid, ptr_to_guest_slot, 0));
  if (errno != 0) {
    PLOG(ERROR) << "failed to get guest state TLS slot content for thread " << tid;
    return nullptr;
  }
  return reinterpret_cast<void*>(header_ptr);
...
static void ReadGuestRegisters(std::unique_ptr<unwindstack::Regs>* regs, pid_t tid) {
  ...
  // void* guest_state_data_copy = reinterpret_cast<void*>((std::make_unique<unsigned
  // char[]>(guest_state_data_size)).get()); void* guest_state_data_copy =
  // (std::make_unique<unsigned char[]>(guest_state_data_size)).get();
  auto guest_state_data_copy = (std::make_unique<unsigned char[]>(guest_state_data_size)).get();
