...
static void* GetGuestStateTlsPointer([[maybe_unused]] pid_t tid) {
...
#if defined(__x86_64__)
  {
    ...
    uintptr_t header_ptr = ptrace(PTRACE_PEEKDATA, tid, ptr_to_guest_slot, 0);
...
static void ReadGuestRegisters(std::unique_ptr<unwindstack::Regs>* regs, pid_t tid) {
  NativeBridgeGuestRegs* guest_regs = new NativeBridgeGuestRegs();
  ...
  LoadGuestStateRegisters(reinterpret_cast<const void*>(guest_state_data_copy),
                          guest_state_data_size, guest_regs);
