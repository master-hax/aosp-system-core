...
int main(int argc, char** argv) {
  ...
  for (const auto& [tid, thread] : thread_info) {
    int resume_signal = thread.signo;
    if (resume_signal == DEBUGGER_SIGNAL) {
      resume_signal = 0;
    }

    LOG(INFO) << "detaching from thread " << tid << " with signal " << resume_signal;
  ...
  if (backtrace) {
    ...
    dump_backtrace(std::move(output_fd), map.get(), thread_info, target_thread);
  } else {
