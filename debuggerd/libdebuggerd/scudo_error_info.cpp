...
static long ReadTag(uintptr_t addr) {
  char x;
  struct iovec src_iovs[] = {{.iov_base = reinterpret_cast<void*>(addr), .iov_len = 1}};
  struct iovec dst_iov = {.iov_base = &x, .iov_len = 1};
  if (process_vm_readv(getpid(), &dst_iov, 1, src_iovs, 1, 0) != 1) {
    return -1;
  }
...
bool GetScudoErrorInfo(unwindstack::Memory* process_memory, const ProcessInfo& process_info,
  ...
  auto memory_tags_size = (memory_end - memory_begin) / kTagGranuleSize;
  ...
  if (in_process) {
    ...
    memory_ptr = reinterpret_cast<char*>(memory_begin);
