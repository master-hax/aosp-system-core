...
static bool get_build_id(unwindstack::Memory* memory, uintptr_t base_addr, uint8_t* e_ident,
  ...
  if (memory->PartialRead(base_addr + EI_NIDENT, reinterpret_cast<uint8_t*>(&hdr) + EI_NIDENT,
  ...
  for (size_t i = 0; i < hdr.e_phnum; i++) {
    PhdrType phdr;
    if (memory->PartialRead(base_addr + hdr.e_phoff + i * hdr.e_phentsize,
    ...
    if (phdr.p_type == PT_NOTE) {
      ...
      while (hdr_size >= sizeof(NhdrType)) {
        NhdrType nhdr;
        if (memory->PartialRead(addr, reinterpret_cast<uint8_t*>(&nhdr), sizeof(nhdr)) !=
            sizeof(nhdr)) {
          return false;
        }
        addr += sizeof(nhdr);
        if (nhdr.n_type == NT_GNU_BUILD_ID) {
          ...
          if (memory->PartialRead(addr, build_id_data, nhdr.n_descsz) != nhdr.n_descsz) {
            return false;
          }
...
bool elf_get_build_id(unwindstack::Memory* memory, uintptr_t addr, std::string* build_id) {
  ...
  if (memory->PartialRead(addr, e_ident, SELFMAG) != SELFMAG) {
    return false;
  }
  ...
  if (memory->PartialRead(addr + SELFMAG, e_ident + SELFMAG, EI_NIDENT - SELFMAG) !=
      EI_NIDENT - SELFMAG) {
    return false;
  }
