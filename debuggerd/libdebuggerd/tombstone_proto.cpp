...
static void dump_crash_details(Tombstone* tombstone, std::shared_ptr<unwindstack::Memory>& process_memory,
  ...
  while (address) {
    ...
    if (!process_memory->ReadFully(address, &page, sizeof(page))) {
      async_safe_format_log(ANDROID_LOG_ERROR, LOG_TAG, "failed to read crash detail page: %s",
                            strerror(errno));
    }
    ...
    for (size_t i = 0; i < page.used; ++i) {
      ...
      if (!process_memory->ReadFully(reinterpret_cast<uintptr_t>(crash_detail.name), name.data(), crash_detail.name_size)) {
        async_safe_format_log(ANDROID_LOG_ERROR, LOG_TAG, "failed to read name: %s",
                              strerror(errno));
                              continue;
      }
      std::string data(crash_detail.data_size, '\0');
      if (!process_memory->ReadFully(reinterpret_cast<uintptr_t>(crash_detail.name), data.data(), crash_detail.data_size)) {
        async_safe_format_log(ANDROID_LOG_ERROR, LOG_TAG, "failed to read name: %s",
                              strerror(errno));
                              continue;
      }
      auto* proto_detail = tombstone->add_crash_detail();
      proto_detail->set_name(name);
      proto_detail->set_string_data(data);
    }
