...
void set_human_readable_cause(Cause* cause, uint64_t fault_addr) {
  ...
  cause->set_human_readable(StringPrintf("[%s]: %s, %lu byte%s %s a %lu-byte allocation at 0x%lx",
                                         tool_str, error_type_str, static_cast<unsigned long>(diff),
                                         byte_suffix, location_str,
                                         static_cast<unsigned long>(heap_object.size()),
                                         static_cast<unsigned long>(heap_object.address())));
}
