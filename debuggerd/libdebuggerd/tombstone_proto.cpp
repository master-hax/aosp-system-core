...
static void dump_crash_details(Tombstone* tombstone,
  ...
  while (address) {
    ...
    for (size_t i = 0; i < page.used; ++i) {
      ...
      switch (crash_detail.type) {
        case CRASH_DETAIL_STRING:
          // We don't need to copy around NUL-bytes.
          proto_detail->set_string_data(data.c_str());
          break;
        case CRASH_DETAIL_BYTES:
          proto_detail->set_bytes_data(data);
          break;
      }
    }
