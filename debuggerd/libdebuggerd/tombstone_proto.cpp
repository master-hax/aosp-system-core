...
static void log_unwinder_error(unwindstack::ErrorData& error) {
  ...
  async_safe_format_log(ANDROID_LOG_ERROR, LOG_TAG, "  error address: 0x%" PRIx64, error.address);
}
...
static void dump_thread(Tombstone* tombstone, unwindstack::AndroidUnwinder* unwinder,
  ...
  std::unique_ptr<unwindstack::Regs> saved_initial_regs;
  data.saved_initial_regs = &saved_initial_regs;
  if (!unwinder->Unwind(thread_info.tid, data)) {
    async_safe_format_log(ANDROID_LOG_ERROR, LOG_TAG, "Unwind failed for tid %d.", thread_info.tid);
    log_unwinder_error(data.error);
  } else {
    dump_thread_backtrace(data.frames, thread);
  }
  dump_registers(unwinder, saved_initial_regs, thread, memory_dump);
