  ...
  while (address) {
    ...
    for (size_t i = 0; i < page.used; ++i) {
      ...
      if (!crash_detail.data) {
        async_safe_format_log(ANDROID_LOG_ERROR, LOG_TAG, "crash detail: no data");
        continue;
      }
      ...
      if (!process_memory->ReadFully(reinterpret_cast<uintptr_t>(crash_detail.name), name.data(),
        ...
        async_safe_format_log(ANDROID_LOG_ERROR, LOG_TAG, "crash detail: failed to read name: %s",
                              strerror(errno));
        continue;
      }
      ...
      if (!process_memory->ReadFully(reinterpret_cast<uintptr_t>(crash_detail.data), data.data(),
        ...
        async_safe_format_log(ANDROID_LOG_ERROR, LOG_TAG, "crash detail: failed to read data: %s",
                              strerror(errno));
        continue;
      }
