...
bool tombstone_proto_to_text(const Tombstone& tombstone, CallbackType callback) {
  ...
  if (main_guest_thread_it == threads.end()) {
    ...
    print_guest_thread(callback, tombstone, main_guest_thread, tombstone.tid(), true, true);
  }
  ...
  for (const auto& tid : thread_ids) {
    ...
    if (guest_threads.find(tid) != guest_threads.end()) {
      print_guest_thread(callback, tombstone, guest_threads.find(tid)->second, tid, false, false);
    }
  }
