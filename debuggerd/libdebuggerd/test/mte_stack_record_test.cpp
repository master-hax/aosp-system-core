...
TEST_P(MteStackHistoryTest, TestSize) {
  ...
  std::optional<android::procinfo::MapInfo> info = FindMapping(data);
...
TEST_P(MteStackHistoryTest, TestFree) {
  ...
  auto before_end = FindMapping(static_cast<char*>(data) + size - 1);
  ASSERT_TRUE(before_end.has_value());
  EXPECT_TRUE(before->start == before_end->start);
