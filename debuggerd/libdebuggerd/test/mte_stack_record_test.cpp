...
TEST_P(MteStackHistoryTest, TestSize) {
  ...
  bool found = false;
  android::procinfo::ReadMapFile("/proc/self/maps", [&found, size, data](const android::procinfo::MapInfo& info) {
...
static bool IsMapped(void* data) {
  bool found = false;
  android::procinfo::ReadMapFile("/proc/self/maps", [&found, data](const android::procinfo::MapInfo& info) {
    auto data_int = reinterpret_cast<uint64_t>(data) & ((1ULL << 56ULL) - 1ULL);
    if (info.start <= data_int && data_int < info.end) {
      found = true;
    }
