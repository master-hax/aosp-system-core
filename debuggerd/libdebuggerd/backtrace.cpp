...
static char* get_thread_name(char* buf, size_t buflen, pid_t tid) {
  FILE* fp;
  char path[PATH_MAX];
  ...
  if ((fp = fopen(path, "r"))) {
    threadname = fgets(buf, buflen, fp);
    fclose(fp);
    if (threadname) {
...
static void log_thread_name(log_t* log, pid_t tid) {
  ...
  char* threadname = get_thread_name(threadnamebuf, sizeof(threadnamebuf), tid);
