...
static void dump_stack_segment(log_t* log, unwindstack::Maps* maps,
                               unwindstack::Memory* process_memory, uint64_t* sp, size_t words,
                               int label) {
...
static void dump_stack(log_t* log, unwindstack::Unwinder* unwinder) {
  size_t first = 0, last;
  ...
  dump_stack_segment(log, unwinder->GetMaps(), unwinder->GetProcessMemory().get(), &sp, STACK_WORDS,
                     -1);
  ...
  for (size_t i = first; i <= last; i++) {
    ...
    if (i != 0) {
      stack_size = frame->sp - frames[i - 1].sp;
    }
    if (i == last) {
      dump_stack_segment(log, unwinder->GetMaps(), unwinder->GetProcessMemory().get(), &sp,
                         STACK_WORDS, i);
      if (sp < frame->sp + stack_size) {
        _LOG(log, logtype::STACK, "         ........  ........\n");
      }
    } else {
      size_t words = stack_size / sizeof(word_t);
      if (words == 0) {
        words = 1;
      } else if (words > STACK_WORDS) {
        words = STACK_WORDS;
      }
      dump_stack_segment(log, unwinder->GetMaps(), unwinder->GetProcessMemory().get(), &sp, words,
                         i);
    }
  }
}
...
void engrave_tombstone_ucontext(int tombstone_fd, uint64_t abort_msg_address, siginfo_t* siginfo,
  ...
  unwindstack::UnwinderFromPid unwinder(256, pid);
