...
static std::string check_stack_overflow(uint64_t fault_addr, uint64_t sp, unwindstack::Maps* maps) {
...
static void dump_probable_cause(log_t* log, const siginfo_t* si, unwindstack::Maps* maps,
  ...
  if (si->si_signo == SIGSEGV && si->si_code == SEGV_MAPERR) {
    if (si->si_addr < reinterpret_cast<void*>(4096)) {
      ...
      cause = check_stack_overflow(reinterpret_cast<uint64_t>(si->si_addr), regs->sp(), maps);
    }
    ...
    if (map_info != nullptr && map_info->flags == PROT_EXEC) {
      ...
      cause = check_stack_overflow(fault_addr, regs->sp(), maps);
    }
