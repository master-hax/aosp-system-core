...
static size_t MemoryRead(Memory* process_memory, uintptr_t addr, void* buf, size_t len) {
  char* p = static_cast<char*>(buf);

  // Try to read the entire thing in one go.
  if (process_memory->Read(addr, p, len)) {
    return len;
  }

  // Break the read up across page boundaries.
  ssize_t result = 0;
  while (len > 0) {
    size_t bytes_to_read = PAGE_SIZE - addr % PAGE_SIZE;
    if (!process_memory->Read(addr, p, len)) {
      return result;
    }
...
static void dump_stack_segment(log_t* log, BacktraceMap* backtrace_map, Memory* process_memory,
  ...
  // TODO: Word align sp?
  size_t bytes_read = MemoryRead(process_memory, *sp, stack_data, sizeof(word_t) * words);
...
static void dump_all_maps(log_t* log, BacktraceMap* map, Memory* process_memory, siginfo_t* si) {
  uintptr_t addr = 0;
  bool print_fault_address_marker = si ? signal_has_si_addr(si->si_signo, si->si_code) : false;
...
// Weak noop implementations, real implementations are in <arch>/machine.cpp.
__attribute__((weak)) void dump_registers(log_t* log, Regs*) {
  _LOG(log, logtype::REGISTERS, "    register dumping unimplemented on this architecture");
}

__attribute__((weak)) void dump_memory_and_code(log_t* log, Memory*, Regs*) {
  _LOG(log, logtype::REGISTERS, "    register dumping unimplemented on this architecture");
}
...
static bool dump_thread(log_t* log, BacktraceMap* map, Memory* process_memory,
  ...
  if (primary_thread) {
    ...
    if (map) {
      dump_all_maps(log, map, process_memory, thread_info.siginfo);
    }
  }
...
void engrave_tombstone_ucontext(int tombstone_fd, uintptr_t abort_msg_address, siginfo_t* siginfo,
  ...
  BacktraceMap* backtrace_map = BacktraceMap::CreateNew(getpid(), false, &process_memory);
  engrave_tombstone(unique_fd(dup(tombstone_fd)), backtrace_map, process_memory.get(), threads, tid,
                    abort_msg_address, nullptr, nullptr);
}
