...
static bool parse_int_with_error_reporting(const char* s, const char* name, int* v) {
...
static bool property_parse_int(const char* name, int* out) {
  ...
  __system_property_read_callback(
      ...
      [](void* raw_cookie, const char* name, const char* value, uint32_t) {
        ...
        if (parse_int_with_error_reporting(value, name, cookie->out)) cookie->empty = false;
      },
      &cookie);
  return !cookie.empty;
}
...
static int permissive_mte_renable_timer() {
  if (char* env = getenv("MTE_PERMISSIVE_REENABLE_TIME_CPUMS")) {
    int v;
    if (parse_int_with_error_reporting(env, "MTE_PERMISSIVE_REENABLE_TIME_CPUMS", &v)) return v;
  }
...
static void debuggerd_signal_handler(int signal_number, siginfo_t* info, void* context) {
  ...
  if (info->si_signo == SIGSEGV &&
    ...
    if (int reenable_timer = permissive_mte_renable_timer(); reenable_timer > 0) {
      async_safe_format_log(ANDROID_LOG_ERROR, "libc",
