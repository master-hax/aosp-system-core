...
static void debuggerd_signal_handler(int signal_number, siginfo_t* info, void* context) {
  ...
  if (info->si_signo == BIONIC_SIGNAL_DEBUGGER) {
    ...
    // Back channel to init (see system/core/init/service.cpp) to signal that this 
    // process crashed due to an ASYNC MTE fault and should be considered for upgrade to
    // SYNC mode. We are re-using the ART profiler signal, which is always handled
    // (ignored in native processes, handled for generating a dump in ART processes),
    // so a process will never crash from this signal except from here.
    info->si_signo = __SIGRTMIN + 6;
    resend_signal(info);
  }
#endif
  else {
