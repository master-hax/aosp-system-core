...
static void debuggerd_signal_handler(int signal_number, siginfo_t* info, void* context) {
  ...
  if (info->si_signo == BIONIC_SIGNAL_DEBUGGER) {
    ...
    pthread_mutex_unlock(&crash_mutex);
  } else if (recoverable_gwp_asan_crash) {
    gwp_asan_callbacks.debuggerd_gwp_asan_post_crash_report(info->si_addr);
    pthread_mutex_unlock(&crash_mutex);
...
bool debuggerd_handle_mte_signal(int signal_number, siginfo_t* info, void* context) {
  if (!is_permissive_mte()) return false;
  // Because permissive MTE disabled MTE for the entire thread, we're less
  ...
  // (see the comment about recoverable GWP-ASan below), but that shouldn't
  // happen if MTE is disabled for the entire thread. This might need to be
  // changed if there's some low-hanging bug that happens across multiple
  // threads in quick succession.
...
bool debuggerd_handle_signal(int signal_number, siginfo_t* info, void* context) {
  ...
  if (info->si_code == SEGV_MTEAERR || info->si_code == SEGV_MTESERR) {
    return debuggerd_handle_mte_signal(signal_number, info, context);
  }
