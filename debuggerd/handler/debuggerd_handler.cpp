...
static int ParseIntWithErrorReporting(const char* s, const char* name) {
    int v;
    if (android::base::ParseInt(s, &v)) {
      return v;
    }
    async_safe_format_log(ANDROID_LOG_ERROR, "libc", "invalid %s: %s (%s)", name, s, strerror(errno));
    return 0;
}
...
static bool property_parse_int(const char* name, int* out) {
  ...
  struct cookie_t {
    ...
    const char* name;
    bool empty;
  } cookie {out, name, true};
  __system_property_read_callback(
      pi,
      [](void* raw_cookie, const char*, const char* value, uint32_t) {
        // Property is set to empty value, ignoring.
        if (!*value)
          return;
        cookie_t* cookie = reinterpret_cast<cookie_t*>(raw_cookie);
        *cookie->out = ParseIntWithErrorReporting(value, cookie->name);
        cookie->empty = false;
      },
      &cookie);
  return !cookie.empty;
...
static int permissive_mte_renable_timer() {
  if (char* env = getenv("MTE_PERMISSIVE_REENABLE_TIMER"))
    return  ParseIntWithErrorReporting(env, "MTE_PERMISSIVE_REENABLE_TIMER");

  char process_deviceconf_sysprop_name[512];
  async_safe_format_buffer(process_deviceconf_sysprop_name, sizeof(process_deviceconf_sysprop_name),
                           "persist.device_config.memory_safety_native.permissive_reenable_timer.process.%s",
                           getprogname());
  char process_sysprop_name[512];
  async_safe_format_buffer(process_sysprop_name, sizeof(process_sysprop_name),
                           "persist.permissive_reenable_timer.process.%s",
                           getprogname());
  int v;
  if (property_parse_int(process_sysprop_name, &v))
    return v;
  if (property_parse_int("persist.sys.mte.permissive_reenable_timer.default", &v))
    return v;
  if (property_parse_int(process_deviceconf_sysprop_name, &v))
    return v;
  if (property_parse_int("persist.device_config.memory_safety_native.permissive_reenable_timer.default", &v))
    return v;
  return 0;
}
...
static void debuggerd_signal_handler(int signal_number, siginfo_t* info, void* context) {
  ...
  if (info->si_signo == SIGSEGV &&
    ...
    if (int reenable_timer = permissive_mte_renable_timer()) {
      async_safe_format_log(ANDROID_LOG_ERROR, "libc",
                             "MTE ERROR DETECTED BUT RUNNING IN PERMISSIVE MODE. CONTINUING WITH MTE DISABLED FOR %d MS.", reenable_timer);
      timer_t timerid{};
      struct sigevent sev {};
      ...
      if (timer_create(CLOCK_MONOTONIC, &sev, &timerid) == -1) {
        fatal_errno("failed to timer_create");
      }
      ...
      if (timer_settime(timerid, 0, &its, nullptr) == -1) {
        fatal_errno("failed to timer_settime");
      }
    } else {
      async_safe_format_log(ANDROID_LOG_ERROR, "libc",
