...
static bool property_parse_int(const char* name, int* out) {
  ...
  __system_property_read_callback(
      ...
      [](void* raw_cookie, const char* name, const char* value, uint32_t) {
        ...
        *cookie->out = parse_int_with_error_reporting(value, name);
        cookie->empty = false;
      },
      &cookie);
  return !cookie.empty;
...
static void __enable_mte_signal_handler(int, siginfo_t* info, void*) {
  ...
  async_safe_format_log(ANDROID_LOG_INFO, "libc", "Re-enabling MTE on thread, value: %x",
  ...
  if (prctl(PR_SET_TAGGED_ADDR_CTRL, tagged_addr_ctrl, 0, 0, 0) < 0) {
    async_safe_fatal("failed to PR_SET_TAGGED_ADDR_CTRL: %m");
  }
}
...
#define SA_EXPOSE_TAGBITS 0x00000800
  ...
  if (level >= 0 && (level & PR_TAGGED_ADDR_ENABLE) &&
