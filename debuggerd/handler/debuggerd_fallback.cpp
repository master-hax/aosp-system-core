...
// The fallback code is happening in a signal handler, so this code will
// enable a fallback allocator to be used for all allocations while in
// the signal handler. This avoids race condition corruption of the
// normal linker allocator, and if the signal handler is called while
// holding an allocation lock.
...
static uint64_t pack_thread_fd(pid_t tid, int fd) {
  packed_thread_output packed(tid, fd);
  uint64_t result;
...
static void trace_handler(siginfo_t* info, ucontext_t* ucontext) {
  ...
  if (info->si_value.sival_ptr == kDebuggerdFallbackSivalPtrRequestDump) {
    ...
    // Stop using the fallback allocator before the close so that this thread
    // has guaranteed it is not using the fallback allocator before the thread
    // backtracing all of the threads tries to re-acquire the fallback allocator.
    allocator.Disable();
