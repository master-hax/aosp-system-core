...
public class MteRecoveryTest extends BaseHostJUnit4Test {
    String mUUID;
    ...
    Tombstone parseTombstone(String tombstonePath) throws Exception {
        File tombstoneFile = getDevice().pullFile(tombstonePath, /* TODO REMOVE */ 0);
    ...
    public void testCrash() throws Exception {
        CommandResult result =
                getDevice()
                        .executeShellV2Command(
                                "MTE_UNSAFE_MODE=1 /data/local/tmp/mte_crash testCrash " + mUUID);
        ...
        for (String tombstone : tombstones) {
            ...
            if (!tombstoneProto.getCommandLineList().stream()
                    .anyMatch(x -> x.contains("testCrash"))) {
                continue;
            }
            numberTombstones++;
        }
