 ...
 protected:
  ...
  void AddEntry(const std::string& name, const std::vector<uint8_t>& content,
    ...
    if (include_data_descriptor) {
      ...
      android::base::put_unaligned(local_entry.data_descriptor.data(),
                                   DataDescriptor::kOptSignature);
      if (compressed_size_in_extended) {
        android::base::put_unaligned<uint64_t>(local_entry.data_descriptor.data() + 8,
                                               compressed_size_in_extended);
        android::base::put_unaligned<uint64_t>(local_entry.data_descriptor.data() + 16,
                                               uncompressed_size_in_extended);
      } else {
        android::base::put_unaligned<uint32_t>(local_entry.data_descriptor.data() + 8,
                                               compressed_size_in_extended);
        android::base::put_unaligned<uint32_t>(local_entry.data_descriptor.data() + 12,
                                               uncompressed_size_in_extended);
      }
    }
