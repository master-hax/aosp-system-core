 ...
 protected:
  ...
  void AddEntry(const std::string& name, const std::vector<uint8_t>& content,
    ...
    if (include_data_descriptor) {
      ...
      // Skip the 4 bytes signature and crc32.
      size_t write_offset = 2 * sizeof(uint32_t);
      if (compressed_size_in_extended) {
        android::base::put_unaligned<uint64_t>(local_entry.data_descriptor.data() + write_offset,
                                               compressed_size_in_extended);
        write_offset += sizeof(uint64_t);
        android::base::put_unaligned<uint64_t>(local_entry.data_descriptor.data() + write_offset,
                                               uncompressed_size_in_extended);
      } else {
        android::base::put_unaligned<uint32_t>(local_entry.data_descriptor.data() + write_offset,
                                               compressed_size_in_extended);
