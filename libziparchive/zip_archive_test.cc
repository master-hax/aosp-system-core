...
TEST_F(Zip64ParseTest, openFileIncorrectDataSizeInExtendedField) {
  AddEntry("a.txt", std::vector<uint8_t>(100, 'a'), true, false, false);
  ASSERT_EQ(1, file_entries_.size());
  auto& extended_field = file_entries_[0].extended_field;
  // data size exceeds the extended field size.
  android::base::put_unaligned(extended_field.data() + 2, 100);
  ConstructEocd();
  ConstructZipFile();
...
TEST_F(Zip64ParseTest, findEntry) {
  ...
  ASSERT_EQ(std::set<std::string_view>({"a.txt", "b.txt"}), result);
