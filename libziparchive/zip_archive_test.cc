...
static bool WriteFully(const int fd, const uint8_t* buf, ssize_t buf_size) {
  while (buf_size > 0) {
    ssize_t bytes_written = TEMP_FAILURE_RETRY(write(fd, buf, buf_size));
    if (bytes_written == -1) {
      return false;
    }
...
static bool ReadFully(const int fd, uint8_t* buf, size_t buf_size) {
  if (lseek64(fd, 0, SEEK_SET) != 0) {
    return false;
  }

  while (buf_size > 0) {
    ssize_t bytes_read = TEMP_FAILURE_RETRY(read(fd, buf, buf_size));
    if (bytes_read <= 0) {
      return false;
    }
    buf_size -= bytes_read;
...
TEST(ziparchive, EntryLargerThan32K) {
  ...
  int fd = make_temporary_file(temp_file_pattern);
  ASSERT_NE(-1, fd);
  ASSERT_TRUE(WriteFully(fd, reinterpret_cast<const uint8_t*>(kAbZip),
                         sizeof(kAbZip) - 1));
  ZipArchiveHandle handle;
  ...
  ASSERT_TRUE(ReadFully(output_fd, &file_contents[0], file_contents.size()));
