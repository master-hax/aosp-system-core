...
TEST(ziparchive, EmptyEntries) {
  TemporaryFile tmp_file("empty_entries_test_XXXXXX");
  ASSERT_NE(-1, tmp_file.fd);
  const ssize_t file_size = sizeof(kEmptyEntriesZip);
  ASSERT_EQ(file_size, TEMP_FAILURE_RETRY(write(tmp_file.fd, kEmptyEntriesZip, file_size)));
  ...
  TemporaryFile tmp_output_file("empty_entries_output_XXXXXX");
...
TEST(ziparchive, EntryLargerThan32K) {
  TemporaryFile tmp_file("entry_larger_than_32k_test_XXXXXX");
  ASSERT_NE(-1, tmp_file.fd);
  ...
  TemporaryFile tmp_output_file("entry_larger_than_32k_test_output_XXXXXX");
  ASSERT_NE(-1, tmp_output_file.fd);
  ASSERT_EQ(0, ExtractEntryToFile(handle, &entry, tmp_output_file.fd));
...
TEST(ziparchive, TrailerAfterEOCD) {
  TemporaryFile tmp_file("trailer_after_eocd_test_XXXXXX");
  ASSERT_NE(-1, tmp_file.fd);
  ...
  ASSERT_EQ(file_size, TEMP_FAILURE_RETRY(write(tmp_file.fd, kEmptyEntriesZip, file_size)));
  ASSERT_EQ(trailer_size, TEMP_FAILURE_RETRY(write(tmp_file.fd, trailer, trailer_size)));
...
TEST(ziparchive, ExtractToFile) {
  TemporaryFile tmp_file("zip_archive_input_XXXXXX");
  ASSERT_NE(-1, tmp_file.fd);
  const uint8_t data[8] = { '1', '2', '3', '4', '5', '6', '7', '8' };
  const ssize_t data_size = sizeof(data);

  ASSERT_EQ(data_size, TEMP_FAILURE_RETRY(write(tmp_file.fd, data, data_size)));
  ...
  ASSERT_EQ(data_size, TEMP_FAILURE_RETRY(read(tmp_file.fd, read_buffer, data_size)));
  ...
  ASSERT_EQ(static_cast<ssize_t>(entry.uncompressed_length),
            TEMP_FAILURE_RETRY(
                read(tmp_file.fd, &uncompressed_data[0], entry.uncompressed_length)));
