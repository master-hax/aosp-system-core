...
class Zip64Test(unittest.TestCase):
  ...
  def test_largeEntriesSmallerThan4G(self):
    zip_path = tempfile.NamedTemporaryFile(suffix='.zip')
    # Add a few large entries, but each of them is smaller than 4GiB.
  ...
  def test_largeNumberOfEntries(self):
    zip_path = tempfile.NamedTemporaryFile(suffix='.zip')
    entry_dict = {}
    # Add more entries than 65536 (UINT16_MAX), and final zip file size is roughly 5GiB.
    for num in range(0, 65545):
      entry_dict[str(num)] = 80
