 ...
 * Returns 0 on success, and negative values on failure.
 */
int OpenArchiveFd(const int fd, const char* debugFileName, ZipArchiveHandle *handle);
 ...
 */
int FindEntry(const ZipArchiveHandle handle, const char* entryName, ZipEntry* data);
