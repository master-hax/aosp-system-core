...
// UTF-8 sequences longer than a byte.
...
inline bool IsValidEntryNameLenGe16(const uint8_t* entry_name,
                                    const size_t length) {
  uint8_t* str = const_cast<uint8_t*>(entry_name);
  size_t len = length;

  uint64_t chunk8, chunk8_1;
  constexpr unsigned sz8 = sizeof(chunk8);
  constexpr unsigned sz16 = 2 * sizeof(chunk8);
  ...
  // First bytes to align len to 16.
  size_t incr = (len % sz16) ? (len % sz16) : sz16;

  while (len > 0) {
    // Load 128bit.
    __builtin_memcpy(&chunk8, str, sz8);
    __builtin_memcpy(&chunk8_1, str + 8, sz8);

    // Is there any element in vector with value >127 (multiple byte sequence)?
    // ((byte & 0x80 ) != 0).
    if (chunk8 & kVdup8_80 || chunk8_1 & kVdup8_80)
...
inline bool IsValidEntryNameLenGe8(const uint8_t* entry_name,
                                   const size_t length) {
  uint8_t* str = const_cast<uint8_t*>(entry_name);
  size_t len = length;
  ...
  // First bytes to align len to 8.
  size_t incr = (len % sz8) ? (len % sz8) : sz8;
...
inline bool IsValidEntryName(const uint8_t* entry_name, const size_t length) {
  ...
  return IsValidUTF8(entry_name, length);
}
