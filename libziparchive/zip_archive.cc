...
static bool isZipStringEqual(const uint8_t* start, const ZipString& zip_string,
  ...
  ZipString from_offset = zip_string_offset.GetZipString(start);
...
static inline uint32_t GetOffset(const uint8_t* name, const uint8_t* start) {
  CHECK_GT(name, start);
  CHECK_LT(reinterpret_cast<uintptr_t>(name) - reinterpret_cast<uintptr_t>(name), 0xffffff);
...
int32_t Next(void* cookie, ZipEntry* data, ZipString* name) {
  ...
  for (uint32_t i = currentOffset; i < hash_table_length; ++i) {
    ZipString from_offset = hash_table[i].GetZipString(archive->central_directory.GetBasePtr());
