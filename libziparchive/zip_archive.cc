...
static int32_t AddToHash(ZipStringOffset* hash_table, const uint32_t hash_table_size,
  ...
  CHECK_GT(name.data(), start_char);
  CHECK_LT(name.data(), start_char + 0xffffff);  // TODO: shouldn't this be 0xffff instead?
...
static int32_t FindEntry(const ZipArchive* archive, const int32_t ent, ZipEntry* data) {
  ...
  const uint8_t* ptr = reinterpret_cast<const uint8_t*>(hash_name.data());
