...
static inline bool ReadAtOffset(FileReaderWrapper file_reader, uint8_t* buf, size_t len, off64_t off) {
#if !defined(_WIN32)
...
static int32_t MapCentralDirectory0(FileReaderWrapper& file_reader, const char* debug_file_name,
   ...
   * in archive.
   */
  archive->central_directory.resize(static_cast<size_t>(eocd->cd_size));
  if (!ReadAtOffset(archive->file_reader, archive->central_directory.data(),
...
static int32_t ParseZipArchive(ZipArchive* archive) {
  const uint8_t* const cd_ptr = archive->central_directory.data();
  const size_t cd_length = archive->central_directory.size();
...
static int32_t FindEntry(const ZipArchive* archive, const int ent,
  ...
  // this mapped region.
  const uint8_t* base_ptr = archive->central_directory.data();
  if (ptr < base_ptr || ptr > base_ptr + archive->central_directory.size()) {
