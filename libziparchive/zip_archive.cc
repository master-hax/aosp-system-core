...
static int64_t EntryToIndex(const ZipStringOffset* hash_table, const uint32_t hash_table_size,
  ...
  ZipString zip_string_offset;
  while (hash_table[ent].name_offset != 0) {
    hash_table[ent].GetZipString(start, &zip_string_offset);
    if (zip_string_offset == name) {
      return ent;
    }
...
static int32_t AddToHash(ZipStringOffset* hash_table, const uint64_t hash_table_size,
  ...
  hash_table[ent].name_offset = name.GetOffset(start);
...
static int32_t ParseZipArchive(ZipArchive* archive) {
  ...
  for (uint16_t i = 0; i < num_entries; i++) {
    ...
    const int add_result =
        AddToHash(archive->hash_table, archive->hash_table_size, entry_name,
                  static_cast<const uint8_t*>(archive->central_directory.GetBasePtr()));
    if (add_result != 0) {
...
static int32_t FindEntry(const ZipArchive* archive, const int ent, ZipEntry* data) {
  ...
  ZipString zip_string_offset;
  archive->hash_table[ent].GetZipString(
      static_cast<const uint8_t*>(archive->central_directory.GetBasePtr()), &zip_string_offset);
  const uint8_t* ptr = zip_string_offset.name;
  ...
  if (lfh->file_name_length == nameLen) {
    ...
    archive->hash_table[ent].GetZipString(
        static_cast<const uint8_t*>(archive->central_directory.GetBasePtr()), &zip_string_offset);
    if (memcmp(zip_string_offset.name, name_buf.data(), nameLen)) {
      return kInconsistentInformation;
    }
...
int32_t FindEntry(const ZipArchiveHandle handle, const ZipString& entryName, ZipEntry* data) {
  ...
  const int64_t ent =
      EntryToIndex(archive->hash_table, archive->hash_table_size, entryName,
                   static_cast<const uint8_t*>(archive->central_directory.GetBasePtr()));
  if (ent < 0) {
...
int32_t Next(void* cookie, ZipEntry* data, ZipString* name) {
  ...
  for (uint32_t i = currentOffset; i < hash_table_length; ++i) {
    hash_table[i].GetZipString(static_cast<const uint8_t*>(archive->central_directory.GetBasePtr()),
                               &zip_string_offset);
    if (hash_table[i].name_offset != 0 &&
        (handle->prefix.name_length == 0 || zip_string_offset.StartsWith(handle->prefix)) &&
        (handle->suffix.name_length == 0 || zip_string_offset.EndsWith(handle->suffix))) {
      handle->position = (i + 1);
      const int error = FindEntry(archive, i, data);
      if (!error) {
        name->name = zip_string_offset.name;
