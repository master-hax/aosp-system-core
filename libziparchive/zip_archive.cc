...
static bool isZipStringEqual(const uint8_t* start, const ZipString& zip_string,
  ...
  zip_string_offset.GetZipString(start, &from_offset);
...
static inline uint32_t GetOffset(const uint8_t* name, const uint8_t* start) {
  CHECK_LT(reinterpret_cast<uintptr_t>(name), 0xffffff);
  CHECK_GT(name, start);
...
static int32_t FindEntry(const ZipArchive* archive, const int ent, ZipEntry* data) {
  ...
  ZipString from_offset;
  archive->hash_table[ent].GetZipString(archive->central_directory.GetBasePtr(), &from_offset);
...
int32_t Next(void* cookie, ZipEntry* data, ZipString* name) {
  ...
  for (uint32_t i = currentOffset; i < hash_table_length; ++i) {
    hash_table[i].GetZipString(archive->central_directory.GetBasePtr(), &from_offset);
