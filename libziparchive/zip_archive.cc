...
#define kDDUncompressed_length        8               // offset to uncompressed length
...
static int MapCentralDirectory0(int fd, const char* debug_file_name,
        ZipArchive* archive, off_t file_length, size_t read_amount,
        uint8_t* scan_buffer) {
...
int VerifyCrcAndLengths(const ZipArchive* archive,
    ...
    if (entry->has_data_descriptor) {
        ...
        entry->uncompressed_length = get4LE(ddBuf + kDDUncompressed_length);
    }
