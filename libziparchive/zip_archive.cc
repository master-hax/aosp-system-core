...
static int FindEntry(const ZipArchive* pArchive, const int ent,
    ...
    if (lfhNameLen == nameLen) {
        ...
        uint8_t* nameBuf = (uint8_t*) malloc(nameLen * sizeof(uint8_t));
        ssize_t actual = TEMP_FAILURE_RETRY(read(pArchive->mFd, nameBuf, sizeof(nameBuf)));

        if (actual != sizeof(nameBuf) || memcmp(name, nameBuf, nameLen) != 0) {
            ALOGW("Zip: failed reading lfh from offset %ld", nameOffset);
            free(nameBuf);
            return -1;
        }
...
int Next(ZipArchiveHandle handle, uint32_t* cookie, ZipEntry* data) {
    ...
    for (uint32_t i = currentOffset; i < hashTableLength; ++i) {
        if (hashTable[i].name != NULL) {
            *cookie = (i + 1);
            return FindEntry(archive, i, data);
        }
    }
