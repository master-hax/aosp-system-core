...
// The struct holds some common field in the central directory record and the local file header. In
// zip64 format, the real value of these field may not fit into uint32_t. And we need to parse the
// extra field in the cdr or lfh.
struct Zip32EntryInfo {
  uint32_t uncompressed_size;
  uint32_t compressed_size;
  std::optional<uint32_t> local_file_header_offset;
};
...
static ZipError FindCentralDirectoryInfoForZip64(const char* debugFileName, ZipArchive* archive,
  ...
  if (eocdOffset <= sizeof(Zip64EocdLocator)) {
    ALOGW("Zip: Not enough space for zip64 eocd locator");
    return kInvalidFile;
  }
  // We expect to find the zip64 eocd locator immediately before the zip eocd.
  const int64_t locator_offset = eocdOffset - sizeof(Zip64EocdLocator);
  ...
  if (!archive->mapped_zip.ReadAtOffset(reinterpret_cast<uint8_t*>((&zip64_eocd_locator)),
    ...
    ALOGW("Zip: Read %zu from offset %" PRId64 " failed %s", sizeof(Zip64EocdLocator),
          locator_offset, debugFileName);
    return kIoError;
  }

  if (zip64_eocd_locator.locator_signature != Zip64EocdLocator::kSignature) {
    ALOGW("Zip: Zip64 eocd locator signature not found at offset %" PRId64, locator_offset);
    return kInvalidFile;
  }

  const int64_t zip64_eocd_offset = zip64_eocd_locator.zip64_eocd_offset;
  if (zip64_eocd_offset > locator_offset - sizeof(Zip64EocdRecord)) {
    ALOGW("Zip: Bad zip64 eocd offset %" PRIu64, zip64_eocd_offset);
    return kInvalidFile;
  }
  ...
  if (!archive->mapped_zip.ReadAtOffset(reinterpret_cast<uint8_t*>(&zip64_eocd),
    ...
    ALOGW("Zip: read %zu from offset %" PRId64 " failed %s", sizeof(Zip64EocdLocator),
          static_cast<int64_t>(zip64_eocd_offset), debugFileName);
    return kIoError;
  }

  if (zip64_eocd.record_signature != Zip64EocdRecord::kSignature) {
    ALOGW("Zip: Zip64 eocd record signature not found at offset %" PRId64, zip64_eocd_offset);
    return kInvalidFile;
  }

  if (zip64_eocd.cd_start_offset > zip64_eocd_offset - zip64_eocd.cd_size) {
    ALOGW("Zip: Bad offset for zip64 central directory. cd offset %" PRIu64 ", cd size %" PRIu64
          ", zip64 eocd offset %" PRIu64,
          zip64_eocd.cd_start_offset, zip64_eocd.cd_size, zip64_eocd_offset);
    return kInvalidFile;
  }
...
static std::unique_ptr<Zip64ExtendedInfo> ParseZip64ExtendedInfoInExtraField(
    const uint8_t* extraFieldStart, uint16_t extraFieldLength, const Zip32EntryInfo& zip32Info) {
  ...
  while (offset < extraFieldLength - 4) {
    auto header_id = get_unaligned<uint16_t>(extraFieldStart + offset);
    auto data_size = get_unaligned<uint16_t>(extraFieldStart + offset + 2);

    offset += 4;
    if (data_size > extraFieldLength - offset) {
      ALOGW("Zip: Data size exceeds the boundary of extra field, data size %" PRIu16, data_size);
      return nullptr;
    }
    ...
    if (zip32Info.uncompressed_size == UINT32_MAX || zip32Info.compressed_size == UINT32_MAX) {
      if (zip32Info.local_file_header_offset == UINT32_MAX) {
        expectedDataSize = 24;
      } else {
        expectedDataSize = 16;
      }
    } else if (zip32Info.local_file_header_offset.value() == UINT32_MAX) {
      expectedDataSize = 8;
    }

    if (data_size != expectedDataSize) {
      ALOGW("Invalid data size in extra field, expect %" PRIu16 ", get %" PRIu16
            ", uncompressed size %" PRIu32 ", compressed size %" PRIu32
            ", local header offset %" PRIu32,
            data_size, expectedDataSize, zip32Info.uncompressed_size, zip32Info.compressed_size,
            zip32Info.local_file_header_offset.value_or(0));
      return nullptr;
    }
    ...
    return std::unique_ptr<Zip64ExtendedInfo>(new Zip64ExtendedInfo{
        header_id, data_size, uncompressedFileSize, compressedFileSize, localHeaderOffset});
  }
...
static ZipError ParseZipArchive(ZipArchive* archive) {
  ...
  for (uint64_t i = 0; i < num_entries; i++) {
    ...
    if (local_header_offset == UINT32_MAX) {
      auto zip64_info = ParseZip64ExtendedInfoInExtraField(
          extra_field, extra_length,
          Zip32EntryInfo{.uncompressed_size = cdr->uncompressed_size,
                         .compressed_size = cdr->compressed_size,
                         .local_file_header_offset = cdr->local_file_header_offset});
      if (!zip64_info || !zip64_info->local_header_offset.has_value()) {
        ALOGW("Zip: Failed to parse the local header offset in zip64 extended info.");
        return kInvalidFile;
      }
    }
...
static int32_t FindEntry(const ZipArchive* archive, std::string_view entryName,
  ...
  if (cdr->uncompressed_size == UINT32_MAX || cdr->compressed_size == UINT32_MAX ||
    ...
    const uint8_t* extraField = ptr + sizeof(CentralDirectoryRecord) + cdr->file_name_length;
    auto zip64Info = ParseZip64ExtendedInfoInExtraField(
        extraField, cdr->extra_field_length,
        Zip32EntryInfo{.uncompressed_size = cdr->uncompressed_size,
                       .compressed_size = cdr->compressed_size,
                       .local_file_header_offset = cdr->local_file_header_offset});
    if (!zip64Info) {
      ALOGW("Zip: failed to parse zip64 extended info.");
      return kInvalidFile;
    }

    if (cdr->uncompressed_size == UINT32_MAX || cdr->compressed_size == UINT32_MAX) {
      if (!zip64Info->uncompressed_file_size.has_value() ||
          !zip64Info->compressed_file_size.has_value()) {
        ALOGW("Zip: failed to find uncompressed size or compressed size in zip64 extended info.");
        return kInvalidFile;
      }

      data->uncompressed_length = static_cast<uint32_t>(zip64Info->uncompressed_file_size.value());
      data->compressed_length = static_cast<uint32_t>(zip64Info->compressed_file_size.value());
    }

    if (local_header_offset == UINT32_MAX) {
      if (!zip64Info->local_header_offset.has_value()) {
        ALOGW("Zip: failed to find local offset offset in zip64 extended info.");
        return kInvalidFile;
      }
      local_header_offset = zip64Info->local_header_offset.value();
    }
  }
  ...
  if (local_uncompressed_size == UINT32_MAX || local_compressed_size == UINT32_MAX) {
    const off64_t local_extra_field_offset = name_offset + lfh->file_name_length;
    const uint16_t local_extra_field_size = lfh->extra_field_length;
    if (local_extra_field_offset > cd_offset - local_extra_field_size) {
      ALOGW("Zip: extra field has a bad size for entry %s", std::string(entryName).c_str());
      return kInvalidFile;
    }

    std::vector<uint8_t> local_extra_field(local_extra_field_size);
    if (!archive->mapped_zip.ReadAtOffset(local_extra_field.data(), lfh->extra_field_length,
                                          local_extra_field_offset)) {
      ALOGW("Zip: failed reading lfh extra field from offset %" PRId64, local_extra_field_offset);
      return kIoError;
    }

    auto zip64_info = ParseZip64ExtendedInfoInExtraField(
        local_extra_field.data(), local_extra_field_size,
        Zip32EntryInfo{.uncompressed_size = lfh->uncompressed_size,
                       .compressed_size = lfh->compressed_size});
    if (!zip64_info) {
      ALOGW("Zip: Failed to parse the  zip64 extended info.");
      return kInvalidFile;
    }
    if (!zip64_info->uncompressed_file_size.has_value() ||
        !zip64_info->compressed_file_size.has_value()) {
      ALOGW("Zip: failed to find uncompressed size or compressed size in zip64 extended info.");
      return kInvalidFile;
    }
    local_uncompressed_size = zip64_info->uncompressed_file_size.value();
    local_compressed_size = zip64_info->compressed_file_size.value();
  }
  ...
  if ((lfh->gpb_flags & kGPBDDFlagMask) == 0) {
    ...
    if (data->compressed_length != local_compressed_size ||
        data->uncompressed_length != local_uncompressed_size || data->crc32 != lfh->crc32) {
      ALOGW("Zip: size/crc32 mismatch. expected {%" PRIu32 ", %" PRIu32 ", %" PRIx32
            "}, was {%" PRIu64 ", %" PRIu64 ", %" PRIx32 "}",
            data->compressed_length, data->uncompressed_length, data->crc32, local_compressed_size,
            local_uncompressed_size, lfh->crc32);
      return kInconsistentInformation;
    }
  } else {
