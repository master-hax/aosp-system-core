...
static ZipError FindCentralDirectoryInfoForZip64(const char* debug_file_name, ZipArchive* archive,
                                                 off64_t eocd_offset,
                                                 CentralDirectoryInfo* cdInfo) {
  if (eocd_offset <= sizeof(Zip64EocdLocator)) {
  ...
  const int64_t locator_offset = eocd_offset - sizeof(Zip64EocdLocator);
  ...
  if (!archive->mapped_zip.ReadAtOffset(reinterpret_cast<uint8_t*>((&zip64_eocd_locator)),
    ...
    ALOGW("Zip: Read %zu from offset %" PRId64 " failed %s", sizeof(Zip64EocdLocator),
          locator_offset, debug_file_name);
    return kIoError;
  }
...
static ZipError MapCentralDirectory(const char* debug_file_name, ZipArchive* archive) {
  ...
  if (cdInfo.cd_size >= std::numeric_limits<size_t>::max()) {
...
static std::unique_ptr<Zip64ExtendedInfo> ParseZip64ExtendedInfoInExtraField(
    const uint8_t* extra_field_start, uint16_t extra_length, const CentralDirectoryRecord* cdr) {
  if (extra_length <= 4) {
    ALOGW("Zip: Extra field isn't large enough to hold zip64 info, size %" PRIu16, extra_length);
    return nullptr;
  }
  ...
  while (offset < extra_length - 4) {
    ...
    if (cdr->uncompressed_size == UINT32_MAX || cdr->compressed_size == UINT32_MAX) {
      if (cdr->local_file_header_offset == UINT32_MAX) {
        expected_data_size = 24;
      } else {
        expected_data_size = 16;
      }
    } else if (cdr->local_file_header_offset == UINT32_MAX) {
    ...
    if (data_size != expected_data_size) {
      ALOGW("Invalid data size in extra field, expect %" PRIu16 ", get %" PRIu16
            ...
            data_size, expected_data_size, cdr->uncompressed_size, cdr->compressed_size,
            cdr->local_file_header_offset);
      return nullptr;
    }
    ...
    if (cdr->uncompressed_size == UINT32_MAX || cdr->compressed_size == UINT32_MAX) {
      uncompressed_file_size = get_unaligned<uint64_t>(extra_field_start + offset);
      compressed_file_size = get_unaligned<uint64_t>(extra_field_start + offset + 8);
      offset += 16;
    }

    if (cdr->local_file_header_offset == UINT32_MAX) {
      local_header_offset = get_unaligned<uint64_t>(extra_field_start + offset);
      offset += 8;
    }
...
static ZipError ParseZipArchive(ZipArchive* archive) {
  ...
  for (uint64_t i = 0; i < num_entries; i++) {
    ...
    if (local_header_offset == UINT32_MAX) {
      auto zip64_info = ParseZip64ExtendedInfoInExtraField(extra_field, extra_length, cdr);
...
static int32_t FindEntry(const ZipArchive* archive, std::string_view entryName,
  ...
  if (cdr->uncompressed_size == UINT32_MAX || cdr->compressed_size == UINT32_MAX ||
    ...
    auto zip64_info = ParseZip64ExtendedInfoInExtraField(extra_field, cdr->extra_field_length, cdr);
    if (!zip64_info) {
    ...
    if (cdr->uncompressed_size == UINT32_MAX || cdr->compressed_size == UINT32_MAX) {
      ...
      if (zip64_info->compressed_file_size >= UINT32_MAX ||
        ...
        ALOGW("Zip: File size larger than UINT32_MAX isn't supported yet. compressed size %" PRIu64
              ", uncompressed size %" PRIu64,
              zip64_info->compressed_file_size.value(), zip64_info->uncompressed_file_size.value());
        return kInvalidFile;
      }
    }
