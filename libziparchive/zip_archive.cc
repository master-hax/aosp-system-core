...
static int32_t FindEntry(const ZipArchive* archive, const std::string_view entryName,
  ...
  if (!archive->mapped_zip.ReadAtOffset(lfh_buf, sizeof(lfh_buf), local_header_offset)) {
    ALOGW("Zip: failed reading lfh name from offset %" PRIu64, local_header_offset);
    return kIoError;
  }
  ...
  if (lfh->lfh_signature != LocalFileHeader::kSignature) {
    ALOGW("Zip: didn't find signature at start of lfh, offset=%" PRIu64, local_header_offset);
    return kInvalidOffset;
  }
