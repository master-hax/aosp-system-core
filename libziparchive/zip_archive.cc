...
static ZipError ParseZip64ExtendedInfoInExtraField(
  ...
  while (offset < extraFieldLength - 4) {
    ...
    uint16_t dataStartOffset = offset;
    ...
    if (zip32CompressedSize == UINT32_MAX) {
      compressedFileSize = get_unaligned<uint64_t>(extraFieldStart + offset);
      offset += 8;
    }
    ...
    uint16_t bytesRead = offset - dataStartOffset;
    if (bytesRead == 0) {
...
template <typename T>
static T ReadUnalignedAndIncrementOffset(uint8_t** address) {
  auto ret = get_unaligned<T>(*address);
  *address += sizeof(T);
  return ret;
}
...
static int32_t ValidateDataDescriptor(MappedZipFile& mapped_zip, ZipEntry* entry) {
  ...
  descriptor.crc32 = ReadUnalignedAndIncrementOffset<uint32_t>(&ddReadPtr);
  if (entry->zip64_format_size) {
    descriptor.compressed_size = ReadUnalignedAndIncrementOffset<uint64_t>(&ddReadPtr);
    descriptor.uncompressed_size = ReadUnalignedAndIncrementOffset<uint64_t>(&ddReadPtr);
  } else {
    descriptor.compressed_size = ReadUnalignedAndIncrementOffset<uint32_t>(&ddReadPtr);
    descriptor.uncompressed_size = ReadUnalignedAndIncrementOffset<uint32_t>(&ddReadPtr);
  }
