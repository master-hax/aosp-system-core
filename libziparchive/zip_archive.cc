...
static int32_t MapCentralDirectory(const char* debug_file_name, ZipArchive* archive) {
  ...
  scan_buffer.resize(read_amount);
...
int MappedZipFile::GetFileDescriptor() {
  if (!has_fd) {
...
off64_t MappedZipFile::GetFileLength() {
  if (has_fd) {
    ...
    return static_cast<off64_t>(data_length);
  }
}
...
bool MappedZipFile::ReadData(uint8_t* buffer, size_t read_amount) {
  if (has_fd) {
    ...
    memcpy(buffer, reinterpret_cast<uint8_t*>(base_ptr) + read_pos, read_amount);
...
#if !defined(_WIN32)
  if (has_fd) {
    if (static_cast<size_t>(TEMP_FAILURE_RETRY(pread64(fd, buf, len, off))) != len) {
...
void CentralDirectory::Initialize(void* map_base_ptr, off64_t cd_start_offset, size_t cd_size) {
    base_ptr = reinterpret_cast<uint8_t*>(map_base_ptr) + cd_start_offset;
    length = cd_size;
}
