...
//#if !defined(_WIN32)
//  return TEMP_FAILURE_RETRY(pread64(des_fd.fd, buf, len, off));
//#else
...
#endif
      if (ret != 0) {
        ALOGE("failed to create dir %s, error: %s\n", dir_path.c_str(), strerror(errno));
        return false;
      }
...
#define UNZIP_FILEMODE 0644
  ...
  while (Next(cookie, &entry, &name) == 0) {
    ...
    if (fd == -1) {
      ALOGE("Can't create target file \"%s\": %s\n", path.c_str(), strerror(errno));
      return false;
    }
#ifdef _ANDROID_
    if (secontext) {
...
off64_t desp::getFileLength() {
  if (file_type) {
    ...
    if (result == -1) {
      ALOGE("Zip: lseek on fd %d failed: %s\n", fd, strerror(errno));
    }
    return result;
  } else{
