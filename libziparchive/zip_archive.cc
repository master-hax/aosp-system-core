...
static int32_t InflateToFile(int fd, const ZipEntry* entry,
  ...
  std::unique_ptr<uint8_t[]> read_buf(new uint8_t[kBufSize]);
  std::unique_ptr<uint8_t[]> write_buf(new uint8_t[kBufSize]);
  z_stream zstream;
  int zerr;
  ...
  zstream.avail_in = 0;
  zstream.next_out = reinterpret_cast<Bytef*>(write_buf.get());
  ...
  do {
    ...
    if (zstream.avail_in == 0) {
      ...
      const ZD_TYPE actual = TEMP_FAILURE_RETRY(read(fd, read_buf.get(), getSize));
      if (actual != getSize) {
    ...
    if (zstream.avail_out == 0 ||
      ...
      const size_t write_size = zstream.next_out - write_buf.get();
      // The file might have declared a bogus length.
      if (write_size + write_count > length) {
        return -1;
      }
      memcpy(begin + write_count, write_buf.get(), write_size);
      write_count += write_size;

      zstream.next_out = write_buf.get();
      zstream.avail_out = kBufSize;
    }
  } while (zerr == Z_OK);
