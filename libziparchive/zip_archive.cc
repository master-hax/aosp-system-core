...
bool MappedZipFile::ReadAtOffsetSync(uint8_t* buf, size_t len, off64_t off) {
  if (has_fd_) {
    // This doesn't guarantee that we won't have races with the fd read position if the fd is duped
    // or inherited, but it guarantees consistency when operating on the same ZipArchiveHandle.
    std::lock_guard<std::mutex> lock(mutex_);
    if (lseek64(fd_, off, SEEK_SET) != off) {
      ALOGE("Zip: lseek to %" PRId64 " failed: %s\n", off, strerror(errno));
      return false;
    }

    if (!android::base::ReadFully(fd_, buf, len)) {
...
#endif
  return ReadAtOffsetSync(buf, len, off);
}
