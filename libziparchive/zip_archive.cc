...
static ZipError ParseZip64ExtendedInfoInExtraField(
    const uint8_t* extraFieldStart, uint16_t extraFieldLength, uint32_t zip32_uncompressed_size,
    uint32_t zip32_compressed_size, std::optional<uint32_t> zip32_local_file_header_offset,
  ...
  while (offset < extraFieldLength - 4) {
    ...
    if (zip32_uncompressed_size == UINT32_MAX || zip32_compressed_size == UINT32_MAX) {
      expectedDataSize += 16;
    }
    if (zip32_local_file_header_offset == UINT32_MAX) {
      expectedDataSize += 8;
    }
    ...
    if (dataSize != expectedDataSize) {
      auto local_offset_string = zip32_local_file_header_offset.has_value()
                                     ? std::to_string(zip32_local_file_header_offset.value())
                                     : "not exist";
      ALOGW("Zip: Invalid data size in zip64 extended field, expect %" PRIu16 ", get %" PRIu16
            ...
            expectedDataSize, dataSize, zip32_uncompressed_size, zip32_compressed_size,
            local_offset_string.c_str());
      return kInvalidFile;
    }
