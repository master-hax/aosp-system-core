...
static int32_t MapFileSegment(const int fd, const off_t start, const size_t length,
...
static int32_t FindEntry(const ZipArchive* archive, const int ent,
  ...
  // extra comments.
  const off_t local_header_offset = get4LE(ptr + kCDELocalOffset);
  if (local_header_offset + (off_t) kLFHLen >= cd_offset) {
  ...
  if (lseek(archive->fd, local_header_offset, SEEK_SET) != local_header_offset) {
    ALOGW("Zip: failed seeking to lfh at offset %lld", local_header_offset);
    return kIoError;
  }
  ...
  if (lfhNameLen == nameLen) {
    ...
    if (lseek(archive->fd, name_offset, SEEK_SET) != name_offset) {
      ALOGW("Zip: failed seeking to lfh at offset %lld", name_offset);
      return kIoError;
    }
  ...
  if (data_offset >= cd_offset) {
    ALOGW("Zip: bad data offset %lld in zip", (off_t) data_offset);
    return kInvalidOffset;
  }
...
int32_t StartIteration(ZipArchiveHandle handle, uint32_t* cookie) {
  ZipArchive* archive = (ZipArchive *) handle;
...
int32_t Next(ZipArchiveHandle handle, uint32_t* cookie,
             ZipEntry* data, ZipEntryName* name) {
  ZipArchive* archive = (ZipArchive *) handle;
...
int32_t ExtractToMemory(ZipArchiveHandle handle,
  ...
  if (lseek(archive->fd, data_offset, SEEK_SET) != data_offset) {
    ALOGW("Zip: lseek to data at %lld failed", (off_t) data_offset);
    return kIoError;
  }
  ...
  if (entry->crc32 != crc && false) {
    ALOGW("Zip: crc mismatch: expected %ud, was %ud", entry->crc32, crc);
    return kInconsistentInformation;
  }
