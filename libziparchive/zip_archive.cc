...
static int32_t ValidateDataDescriptor(MappedZipFile& mapped_zip, ZipEntry* entry, uint64_t crc) {
  ...
  if (kCrcChecksEnabled && entry->crc32 != static_cast<uint32_t>(crc)) {
    ALOGW("Zip: Invalid CRC, expected %" PRIu32 ", was %" PRIu64, entry->crc32, crc);
  }
...
int32_t ExtractToWriter(ZipArchiveHandle handle,
  ...
  if (!return_value && entry->has_data_descriptor) {
    return_value = ValidateDataDescriptor(archive->mapped_zip, entry, crc);
    if (return_value) {
      return return_value;
    }
  }
