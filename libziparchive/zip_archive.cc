...
class ZipWriter {
 public:
  ...
  virtual void Close(bool is_error) = 0;
  virtual ~ZipWriter() {};
 protected:
  ZipWriter() {}
 private:
...
class FileWriter : public ZipWriter {
 public:
  static FileWriter* create(int fd, const ZipEntry* entry) {
    ...
    return new FileWriter(fd, current_offset, declared_length);
  }
  ...
  virtual void Close(bool is_error) {
    if (is_error) {
      int result = TEMP_FAILURE_RETRY(ftruncate(fd_, start_offset_));
      if (result == -1) {
        ALOGW("Zip: unable to truncate file to %" PRId64 "on error: %s", start_offset_,
              strerror(errno));
      }
    }
  }
 ...
 private:
  FileWriter(const int fd, const off64_t start_offset,
...
static int32_t InflateEntryToWriter(int fd, const ZipEntry* entry,
  ...
  do {
    ...
    if (zstream.avail_out == 0 ||
      ...
      // if (write_size + write_count > uncompressed_length) {
      //   return -1;
      // }
