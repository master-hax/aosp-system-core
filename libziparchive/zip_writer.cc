...
int32_t ZipWriter::FinishEntry() {
  ...
  if (ShouldUseDataDescriptor()) {
    ...
    // We haven't supported zip64 format yet. Write both uncompressed size and compressed size as
    // uint32_t.
    ...
    for (const auto& field : dataDescriptor) {
      if (fwrite(&field, sizeof(field), 1, file_) != 1) {
        return HandleError(kIoError);
      }
    }
