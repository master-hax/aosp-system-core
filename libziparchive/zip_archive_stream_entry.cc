...
bool ZipArchiveStreamEntry::Init(const ZipEntry& entry) {
  ...
  if (lseek64(archive->fd, data_offset, SEEK_SET) != data_offset) {
    ALOGW("lseek to data at %" PRId64 " failed", data_offset);
    return false;
  }
  crc32_ = entry.crc32;
  return true;
...
const std::vector<uint8_t>* ZipArchiveStreamEntryUncompressed::Read() {
  ...
  ZipArchive* archive = reinterpret_cast<ZipArchive*>(handle_);
  const ssize_t bytes_read = TEMP_FAILURE_RETRY(read(archive->fd, data_.data(), bytes));
  if (bytes_read <= 0 || bytes != static_cast<size_t>(bytes_read)) {
    if (bytes_read < 0) {
      ALOGE("Error reading from archive fd: %s", strerror(-bytes_read));
    } else if (bytes_read == 0) {
      ALOGE("Error reading from archive fd no data read");
    } else {
      ALOGE("Unable to read all data from zip, possibly corrupted zip?");
    }
    length_ = 0;
    return nullptr;
  }

  if (static_cast<size_t>(bytes_read) < data_.size()) {
    data_.resize(bytes_read);
  }
...
bool ZipArchiveStreamEntryCompressed::Verify() {
  return z_stream_init_ && uncompressed_length_ == 0 && compressed_length_ == 0 && crc32_ == computed_crc32_;
}
...
const std::vector<uint8_t>* ZipArchiveStreamEntryCompressed::Read() {
  ...
  while (true) {
    if (z_stream_.avail_in == 0) {
      ...
      ZipArchive* archive = reinterpret_cast<ZipArchive*>(handle_);
      ssize_t bytes_read = TEMP_FAILURE_RETRY(read(archive->fd, in_.data(), bytes));
      if (bytes_read <= 0 || bytes != static_cast<size_t>(bytes_read)) {
        if (bytes_read < 0) {
          ALOGE("Error reading from archive fd: %s", strerror(-bytes_read));
        } else if (bytes_read == 0) {
