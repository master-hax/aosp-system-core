...
int ifc_add_ipv4_route(const char *name, in_addr_t dst, int prefix_length,
      in_addr_t gw)
{
    struct rtentry rt;
    int result;
...
{
    return ifc_add_ipv4_route(name, dst, 32, 0);
}
...
int ifc_add_ipv6_route(const char *name, struct in6_addr dst, int prefix_length,
      struct in6_addr gw)
{
...
int ifc_add_route(const char *name, const char *dst, int prefix_length,
      const char *gw)
{
    int ret = 0;
    ...
    if (addr_ai->ai_family == AF_INET6) {
        memcpy(&ipv6_addr, addr_ai->ai_addr, sizeof(struct sockaddr_in6));
        memcpy(&ipv6_gw, gw_ai->ai_addr, sizeof(struct sockaddr_in6));
        ret = ifc_add_ipv6_route(name, ipv6_addr.sin6_addr, prefix_length,
              ipv6_gw.sin6_addr);
    } else if (addr_ai->ai_family == AF_INET) {
        memcpy(&ipv4_addr, addr_ai->ai_addr, sizeof(struct sockaddr_in));
