...
static void usage() {
    std::cerr << "Usage: tzdatacheck SYSTEM_TZ_DIR DATA_TZ_DIR\n"
            "\n"
            "Compares the headers of two tzdata files. If the one in SYSTEM_TZ_DIR "
 ...
 */
static bool readHeader(const std::string tzDataFileName, char* headerBytes, int byteCount) {
    ...
    int bytesRead = fread(headerBytes, 1, byteCount, tzDataFile);
