...
static const int PATH_ERR = -1;
static const int PATH_NONE = 0;
static const int PATH_IS_DIR = 1;
static const int PATH_NOT_DIR = 2;
...
static int checkPath(const std::string& path) {
    ...
    return S_ISDIR(buf.st_mode) ? PATH_IS_DIR : PATH_NOT_DIR;
}
...
static bool deleteDir(const std::string& dirToDelete) {
    ...
    if (pathStatus == PATH_NONE) {
        LOG(INFO) << "Path " << dirToDelete << " does not exist";
        return true;
    }
    if (pathStatus != PATH_IS_DIR) {
...
int main(int argc, char* argv[]) {
    ...
    if (dataCurrentDirStatus == PATH_NONE) {
    ...
    // nothing to do.
    if (dataCurrentDirStatus != PATH_IS_DIR) {
