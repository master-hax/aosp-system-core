...
#if 0
// This implementation is untested but believed to be correct, and is left
// here for reference in case Mutex::timedlock() is enabled for host builds.
status_t Mutex::timedLock(nsecs_t timeoutMilliseconds)
{
    DWORD dwWaitResult;

    dwWaitResult = WaitForSingleObject((HANDLE) mState, timeoutMilliseconds);
