...
struct OkOrFail<status_t> {
    ...
    // Turns status_t into a success value. This is trivial for status_t because the special value
    // OK of type status_t is the success value.
    static status_t unwrap(status_t&& s) {
        assert(is_ok(s));
        return s;
    }
    ...
    OkOrFail() = delete;
    OkOrFail(const status_t&) = delete;
    status_t val_;
