...
// and hence lives as long as the last weak reference. (It can also
// theoretically be deallocated in the RefBase destructor iff the strong
// reference count was never incremented and the weak count is zero, e.g.  if
// the RefBase object is explicitly destroyed without decrementing the strong
// count.  We believe this does not happen, and log that case.) In this case,
// the RefBase destructor should be invoked from decStrong ference.
// In the OBJECT_LIFETIME_WEAK case, the weakref_impl is always deallocated
// in the RefBase destructor, which is always invoked by decWeak. DecStrong
...
RefBase::~RefBase()
{
    ...
    if ((flags & OBJECT_LIFETIME_MASK) == OBJECT_LIFETIME_WEAK) {
        ...
        LOG_ALWAYS_FATAL_IF(mRefs->mWeak.load() != 0,
                ...
                "reference count");
        // It may be worthwhile to always report if we get here.
        // But currently AaptAssets::filter stack allocates a
        // WeakResourceFilter which derives from RefBase, causing log spam if
        // we do this. There are other less important, but similar, cases.
        delete mRefs;
    }
