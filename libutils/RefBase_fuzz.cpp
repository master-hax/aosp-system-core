...
struct RefBaseSubclass : public RefBase {
  public:
    RefBaseSubclass(std::atomic<bool>* deleted_check) : mDeleted(deleted_check) {
        *mDeleted = false;
    }

    virtual ~RefBaseSubclass() { *mDeleted = true; }
...
std::atomic<bool> refDeleted(false);
RefBaseSubclass* ref;
...
void loop(const std::vector<uint8_t>& fuzzOps) {
    ...
    // strong references.
    while (!refDeleted && ref != nullptr && canDecrementStrong(&state)) {
        ref->decStrong(nullptr);
        state.strong_count--;
    }
    // Clean up any remaining weak references
    while (!refDeleted && ref != nullptr && canDecrementWeak(&state)) {
        ref->getWeakRefs()->decWeak(nullptr);
        state.weak_count--;
    }
}
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ref = new RefBaseSubclass(&refDeleted);
