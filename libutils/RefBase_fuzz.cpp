...
struct RefBaseSubclass : public RefBase {
  public:
    RefBaseSubclass(bool* deleted_check, RWLock* deleted_mtx)
        : mDeleted(deleted_check), mRwLock(*deleted_mtx) {
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ref = new RefBaseSubclass(&refDeleted, &rwLock);
