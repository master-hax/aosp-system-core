...
std::vector<std::function<void(RefThreadState*)>> unlockedOperations = {
...
std::vector<std::function<void(RefThreadState*)>> lockedOperations = {
...
void loop(const std::vector<uint8_t>& fuzzOps) {
    ...
    uint8_t lockedOpSize = lockedOperations.size();
    uint8_t totalOperationTypes = lockedOpSize + unlockedOperations.size();
    for (auto op : fuzzOps) {
        auto opVal = op % totalOperationTypes;
        if (opVal >= lockedOpSize) {
            unlockedOperations[opVal % lockedOpSize](&state);
        } else {
