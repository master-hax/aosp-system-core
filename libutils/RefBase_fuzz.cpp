...
std::vector<std::function<void(RefThreadState*)>> operations = {
        ...
        [](RefThreadState*) -> void {
            RWLock::AutoRLock lock(gRefDeletedLock);
            ref->getStrongCount();
        },
        [](RefThreadState*) -> void {
            RWLock::AutoRLock lock(gRefDeletedLock);
        ...
        [](RefThreadState* refState) -> void {
            // Lock not needed here, refState is per-thread.
        ...
        [](RefThreadState* refState) -> void {
            ...
            if (!gRefDeleted && weakRefs->attemptIncStrong(nullptr)) {
                refState->strongCount++;
            }
        },
        [](RefThreadState* refState) -> void {
            RWLock::AutoRLock lock(gRefDeletedLock);
            if (!gRefDeleted && weakRefs->attemptIncWeak(nullptr)) {
                refState->weakCount++;
            }
        },
        [](RefThreadState* refState) -> void {
