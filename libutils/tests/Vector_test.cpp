...
TEST_F(VectorTest, CopyOnWrite_CopyAndAddElements) {
    ...
    EXPECT_EQ(vector.size(), static_cast<size_t>(3));

    // copy the vector
    other = vector;

    EXPECT_EQ(other.size(), static_cast<size_t>(3));

    // add an element to the first vector
    vector.add(4);

    // make sure the sizes are correct
    EXPECT_EQ(vector.size(), static_cast<size_t>(4));
    EXPECT_EQ(other.size(), static_cast<size_t>(3));
    ...
    // make sure the sizes are correct
    EXPECT_EQ(vector.size(), static_cast<size_t>(4));
    EXPECT_EQ(other.size(), static_cast<size_t>(4));
