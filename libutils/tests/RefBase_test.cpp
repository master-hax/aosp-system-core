...
TEST(RefBase, StrongCopies) {
    ...
    ASSERT_EQ(0, foo->getWeakRefs()->getWeakCount());
    {
    ...
    ASSERT_FALSE(isDeleted || wp1 == nullptr) << "deleted too early! still has a reference!";
    {
        // Now let's double check it deletes on time
        sp<Foo> sp2 = std::move(sp1);
    }
    ASSERT_TRUE(isDeleted && wp1 == nullptr) << "foo was leaked!";
}
...
public:
    ...
    ~Bar() {
        EXPECT_TRUE(mVisited1 && mVisited2);
        (*mDeleteCount)++;
    }
    bool mVisited1;
    bool mVisited2;
...
TEST(RefBase, RacingDestructors) {
    ...
    for (int i = 0; i < NITERS; ++i) {
        ...
        ASSERT_TRUE(bar->getWeakRefs()->getWeakCount() == 0);
        sp3 = nullptr;
    }
    t.join();
    ASSERT_TRUE(deleteCount == NITERS) << "Deletions missed!";
}
