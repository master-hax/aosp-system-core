...
static constexpr int NITERS = 1000000;  // We use 1/20 that for a slow test.
...
TEST(RefBase, RacingPromotions) {
    ...
    if (setExclusiveCpus(&origCpus, &myCpus, &otherCpus)) {
        ...
        for (int i = 0; i < NITERS / 20; ++i) {
            ...
            wp<Bar> wp1(bar);
            bar->mVisited1 = true;
            wp1 = nullptr;  // Should no longer destroy the object.
            wp1 = bar;
            wpBuffer = wp1;
            wpBufferFull = true;
            ASSERT_TRUE(bar->getWeakRefs()->getWeakCount() >= 1);
        ...
        ASSERT_EQ(NITERS / 20, deleteCount) << "Deletions missed!";
    }  // Otherwise this is slow and probably pointless on a uniprocessor.
}
