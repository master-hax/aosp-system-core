...
TEST_F(LooperTest, PollOnce_WhenNonZeroTimeoutAndNotAwoken_WaitsForTimeout) {
    ...
    EXPECT_EQ(LOOPER_POLL_TIMEOUT, result)
...
TEST_F(LooperTest, PollOnce_WhenNonZeroTimeoutAndAwokenBeforeWaiting_ImmediatelyReturns) {
    ...
    EXPECT_EQ(LOOPER_POLL_WAKE, result)
            << "pollOnce result should be LOOPER_POLL_CALLBACK because loop was awoken";
}
...
TEST_F(LooperTest, PollOnce_WhenZeroTimeoutAndNoSignalledFDs_ImmediatelyReturns) {
    ...
    handler.setCallback(mLooper, pipe.receiveFd, LOOPER_EVENT_INPUT);
...
TEST_F(LooperTest, PollOnce_WhenZeroTimeoutAndSignalledFD_ImmediatelyInvokesCallbackAndReturns) {
    ...
    EXPECT_NEAR(0, elapsedMillis, TIMING_TOLERANCE_MS)
            << "elapsed time should be approx. zero";
    EXPECT_EQ(LOOPER_POLL_CALLBACK, result)
            << "pollOnce result should be LOOPER_POLL_CALLBACK because FD was signalled";
    EXPECT_EQ(1, handler.callbackCount)
            << "callback should be invoked exactly once";
    EXPECT_EQ(pipe.receiveFd, handler.fd)
            << "callback should have received pipe fd as parameter";
    EXPECT_EQ(LOOPER_EVENT_INPUT, handler.events)
            << "callback should have received LOOPER_EVENT_INPUT as events";
}
...
TEST_F(LooperTest, PollOnce_WhenNonCallbackFdIsSignalled_ReturnsIdent) {
    ...
    pipe.writeSignal();
    mLooper->addFd(pipe.receiveFd, expectedIdent, LOOPER_EVENT_INPUT, NULL, expectedData);
    ...
    EXPECT_EQ(LOOPER_EVENT_INPUT, events)
            << "pollOnce should have returned LOOPER_EVENT_INPUT as events";
...
TEST_F(LooperTest, AddFd_WhenCallbackAdded_ReturnsOne) {
    Pipe pipe;
    int result = mLooper->addFd(pipe.receiveFd, 0, LOOPER_EVENT_INPUT, NULL, NULL);
...
TEST_F(LooperTest, AddFd_WhenIdentIsNegativeAndCallbackIsNull_ReturnsError) {
    Pipe pipe;
    int result = mLooper->addFd(pipe.receiveFd, -1, LOOPER_EVENT_INPUT, NULL, NULL);
...
TEST_F(LooperTest, PollOnce_WhenCallbackAddedTwice_OnlySecondCallbackShouldBeInvoked) {
    ...
    handler1.setCallback(mLooper, pipe.receiveFd, LOOPER_EVENT_INPUT);
    handler2.setCallback(mLooper, pipe.receiveFd, LOOPER_EVENT_INPUT); // replace it
    pipe.writeSignal(); // would cause FD to be considered signalled
...
TEST_F(LooperTest, SendMessage_WhenOneMessageIsEnqueue_ShouldInvokeHandlerDuringNextPoll) {
    ...
    EXPECT_EQ(LOOPER_POLL_CALLBACK, result)
            << "pollOnce result should be LOOPER_POLL_CALLBACK because message was sent";
...
TEST_F(LooperTest, SendMessageDelayed_WhenSentToTheFuture_ShouldInvokeHandlerAfterDelayTime) {
    ...
    EXPECT_EQ(LOOPER_POLL_WAKE, result)
            << "pollOnce result should be LOOPER_POLL_WAKE due to wakeup";
    ...
    EXPECT_EQ(LOOPER_POLL_TIMEOUT, result)
            << "pollOnce result should be LOOPER_POLL_TIMEOUT because there were no messages left";
}
...
TEST_F(LooperTest, RemoveMessage_WhenRemovingAllMessagesForHandler_ShouldRemoveThoseMessage) {
    ...
    EXPECT_EQ(LOOPER_POLL_WAKE, result)
            << "pollOnce result should be LOOPER_POLL_WAKE because looper was awoken";
    EXPECT_EQ(size_t(0), handler->messages.size())
    ...
    EXPECT_EQ(LOOPER_POLL_TIMEOUT, result)
            << "pollOnce result should be LOOPER_POLL_TIMEOUT because there was nothing to do";
    EXPECT_EQ(size_t(0), handler->messages.size())
            << "no messages to handle";
}
...
TEST_F(LooperTest, RemoveMessage_WhenRemovingSomeMessagesForHandler_ShouldRemoveThoseMessage) {
    ...
    EXPECT_NEAR(0, elapsedMillis, TIMING_TOLERANCE_MS)
            << "elapsed time should approx. zero because message was sent so looper was awoken";
    EXPECT_EQ(LOOPER_POLL_CALLBACK, result)
            << "pollOnce result should be LOOPER_POLL_CALLBACK because two messages were sent";
