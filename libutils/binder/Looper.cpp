...
int Looper::pollOnce(std::chrono::milliseconds timeout, int* outFd, int* outEvents, void** outData) {
    int result = 0;
    for (;;) {
...
#if DEBUG_POLL_AND_WAKE
    ALOGD("%p ~ pollOnce - waiting: timeout=%d", this, timeout.count());
#endif
...
#if DEBUG_POLL_AND_WAKE
        ALOGD("%p ~ pollOnce - next message in %" PRId64 "ns, adjusted timeout: timeout=%d",
                this, mNextMessageUptime - now, timeout.count());
#endif
    }
...
void Looper::sendMessageAtTime(const std::chrono::steady_clock::time_point uptime,
    ...
    ALOGD("%p ~ sendMessageAtTime - uptime=%" PRId64 ", handler=%p, what=%d",
            this, uptimetime_since_epoch(), handler.get(), message.what);
#endif
