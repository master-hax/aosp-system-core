...
static constexpr auto emptyStringData = String16::makeStaticData(u"");

static inline char16_t* getEmptyString() {
    return const_cast<char16_t*>(emptyStringData.data);
}
...
static char16_t* allocFromUTF8(const char* u8str, size_t u8len)
{
    ...
    SharedBuffer* buf = alloc(sizeof(char16_t) * (u16len + 1));
    if (buf) {
...
static char16_t* allocFromUTF16(const char16_t* u16str, size_t u16len) {
    ...
    SharedBuffer* buf = alloc((u16len + 1) * sizeof(char16_t));
    ...
    : mString(nullptr)
{
    ...
    char16_t* data = static_cast<char16_t*>(alloc(sizeof(char16_t))->data());
    data[0] = 0;
    mString = data;
}
...
void* String16::edit() {
    SharedBuffer* buf;
    if (isStaticString()) {
        buf = alloc((size() + 1) * sizeof(char16_t));
        if (buf) {
            buf->acquire();
