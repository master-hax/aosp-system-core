...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    std::string format = dataProvider.ConsumeRandomLengthString(MAX_STR_SIZE);
    ...
    printer.printFormatLine(format.c_str(), line.c_str());
    logPrinter.printLine(line.c_str());
    logPrinter.printFormatLine(format.c_str(), line.c_str());
    filePrinter.printLine(line.c_str());
    filePrinter.printFormatLine(format.c_str(), line.c_str());
    return 0;
}
