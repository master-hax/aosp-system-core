...
void utf16_to_utf8(const char16_t* src, size_t src_len, char* dst, size_t dst_len)
{
    ...
    while (in < in_end) {
        ...
        /* surrogate pair */
        if (in < in_end && ((w2 = *in) & 0xfc00) == 0xdc00) {
            if (out + 4 > out_end)
                goto err_out;
...
err_out:
    LOG_ALWAYS_FATAL_IF(out >= out_end, "target utf8 string size %zu too short", dst_len);
}
...
ssize_t utf8_to_utf16_length(const uint8_t* u8str, size_t u8len, bool overreadIsFatal)
{
    ...
    while (in < in_end) {
        ...
        if (LIKELY((c & 0xe0) == 0xc0)) {
            in += 1;
            continue;
        }
        if (LIKELY((c & 0xf0) == 0xe0)) {
            in += 2;
            continue;
        }
        if (LIKELY((c & 0xf8) == 0xf0)) {
            utf16_len++; in += 3;
            continue;
        }
        /* invalid leading byte, we produce U+FFFF and keep going */
        utf16_len++;
        ALOGW("Invalid UTF-8 leading byte: 0x%02x", c);
    }
    if (in > in_end && overreadIsFatal)
...
char16_t* utf8_to_utf16_no_null_terminator(
    ...
    while (in < in_end && out < out_end) {
        ...
        if (LIKELY((c & 0xf0) == 0xe0)) {
            ...
            c2 = *in++; c3 = *in++;
            *out++ = (char16_t)(((c & 0x0f) << 12) | ((c2 & 0x3f) << 6) |
                                (c3 & 0x3f));
            continue;
        }
        if (LIKELY((c & 0xf8) == 0xf0)) {
            ...
            c2 = *in++; c3 = *in++; c4 = *in++;
            w = (char32_t)(((c & 0x7) << 18) | ((c2 & 0x3f) << 12) |\
                           ((c3 & 0x3f) << 6) | (c4 & 0x3f));
