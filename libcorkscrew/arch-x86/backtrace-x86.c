...
uintptr_t rewind_pc_arch(const memory_t* memory __attribute__((unused)), uintptr_t pc) {
    /* TODO: x86 instructions are 1-16 bytes, to define exact size of previous instruction
       ...
       Returning pc is probably enough for now, it will point to the next instruction starting address. */
    return pc;
}
...
/* Getting unsigned LEB128 encoded data. From 1 to 4 bytes. */
...
/* Getting signed LEB128 encoded data. From 1 to 4 bytes. */
