...
int keychord_initialize(keychord_register_epoll_handler_fn register_epoll_handler,
...
int keychord_initialize() {
    return keychord_initialize(keychord_default_register_epoll_handler,
...
int keychord_enable(int d, int type, const int* keycodes, size_t num_keycodes, int duration_ms) {
    if (d != 0) {
        errno = EBADF;
        return -1;
    }
...
int keychord_enable(int d, int type, const std::vector<int>& keycodes,
                    std::chrono::milliseconds duration) {
    if (d != 0) {
        errno = EBADF;
        return -1;
