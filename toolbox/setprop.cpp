...
extern "C" int setprop_main(int argc, char** argv) {
    ...
    if (name.front() == '.' || name.back() == '.') {
        std::cerr << "Property names must not start or end with a '.'";
        return EXIT_FAILURE;
    }

    if (name.find("..")) {
        std::cerr << "'..' is not allowed in a property name";
        return EXIT_FAILURE;
    }

    for (const auto& c : name) {
        if (!isalnum(c) && !strchr(":@_.-", c)) {
            std::cerr << "Invalid character '" << c << "' in name '" << name << "'";
            return EXIT_FAILURE;
        }
    }
    ...
    if (value.size() >= PROP_VALUE_MAX && !StartsWith(value, "ro.")) {
        std::cerr << "Value '" << value << "' is too long, " << value.size()
                  << " bytes vs a max of " << PROP_VALUE_MAX;
        return EXIT_FAILURE;
    }

    if (mbstowcs(nullptr, value.data(), 0) == static_cast<std::size_t>(-1)) {
        std::cerr << "Value '" << value << "' is not a UTF8 encoded string";
        return EXIT_FAILURE;
    }
