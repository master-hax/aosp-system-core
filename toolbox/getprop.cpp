...
void PrintAllProperties(ResultType result_type) {
    ...
    if (result_type != ResultType::Value) {
        for (auto& [name, value] : properties) {
            ...
            property_info_file->GetPropertyInfo(name.c_str(), &context, &schema);
...
void PrintProperty(const char* name, const char* default_value, ResultType result_type) {
    switch (result_type) {
        ...
        case ResultType::Schema: {
            const char* schema = nullptr;
            property_info_file->GetPropertyInfo(name, nullptr, &schema);
...
extern "C" int getprop_main(int argc, char** argv) {
    ...
    while (true) {
        ...
        int arg = getopt_long(argc, argv, "SZ", long_options, nullptr);
        ...
        switch (arg) {
            case 'h':
                std::cout
                    << "usage: getprop [-Z] [-S] [NAME [DEFAULT]]\n\n"
                       "Gets an Android system property, or lists them all.\n"
                       "Use -Z to return the property context instead of the property value.\n"
                       "Use -S to return the property schema instead of the property value.\n"
                       "If -S and -Z are both specified only the last specified one is used.\n"
                    << std::endl;
                return 0;
            case 'S':
