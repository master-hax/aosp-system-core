...
void print_usage(void) {
    ...
    LOG(INFO) << "  -s, --syslog: print to syslog, not stderr";
    LOG(INFO) << "  -q, --quiet: disable messages";
    LOG(INFO) << "  -v, --verbose: enable more messages";
    LOG(INFO);
}
...
extern "C" int modprobe_main(int argc, char** argv) {
    ...
    int opt;
    int option_index = 0;
    // NB: toybox defines -l and -D options that are not in upstream.
    // clang-format off
