...
extern "C" int modprobe_main(int argc, char** argv) {
    ...
    if (mod_dirs.empty()) {
        ...
        const std::vector<std::filesystem::path> module_paths = {
                std::filesystem::path("/lib/modules/"), std::filesystem::path("/odm/lib/modules/"),
                std::filesystem::path("/vendor/lib/modules/"),
                std::filesystem::path("/system/lib/modules/")};
        int n;
        ...
        for (const auto& path_entry : module_paths) {
            kernel_dirs = NULL;
            ...
            if (n >= 0) {
                ...
                // All directories that matched KernelVersionNameFilter were added but modules can
                // still be directly placed inside the parent directory in a flat structure. Add it
                // to the search path implicitly.
