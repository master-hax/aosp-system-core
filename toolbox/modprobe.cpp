...
#define check_mode_parallel()                                   \
    if (mode == AddModulesModeParallel) {                      \
        LOG(ERROR) << "multiple mode and parallel flags specified"; \
        print_usage();                                 \
        return EXIT_FAILURE;                           \
    }
...
extern "C" int modprobe_main(int argc, char** argv) {
    ...
    // clang-format on
    while ((opt = getopt_long(argc, argv, "ap::bd:Dhlqrsv", long_options, &option_index)) != -1) {
        switch (opt) {
            case 'a':
    ...
    if (mode == AddModulesModeParallel) {
        ...
        for (const auto& base_path : mod_dirs) {
            std::string file = base_path + "/" + module_filename;
        ...
        if (!found_file) {
            PLOG(ERROR) << "Cannot find file: " << module_filename.c_str();
            rv = EXIT_FAILURE;
        } else if (!m.LoadModulesParallel(std::thread::hardware_concurrency())) {
            PLOG(ERROR) << "Failed to load modules in dirs: " <<
