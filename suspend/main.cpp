...
int main() {
    int wakeupCountFd = TEMP_FAILURE_RETRY(open(kSysPowerWakeupCount, O_CLOEXEC | O_RDWR));
    if (wakeupCountFd < 0) {
        PLOG(ERROR) << "error opening " << kSysPowerWakeupCount;
        return 1;
    }
    int stateFd = TEMP_FAILURE_RETRY(open(kSysPowerState, O_CLOEXEC | O_RDWR));
    if (stateFd < 0) {
    ...
    sp<ISystemSuspend> suspend = new SystemSuspend(wakeupCountFd, stateFd);
