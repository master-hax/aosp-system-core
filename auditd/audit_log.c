...
static int open_log(FILE **file, const char *path, int flags, const char *fmode, mode_t mode)
{
    ...
    *file = fopen(path, fmode);
    ...
    if (rc < 0) {
        rc = -errno;
        SLOGE("Could not fchmod log file");
        goto err;
    }
    ...
    if (rc < 0) {
        rc = -errno;
        SLOGE("Could not fcntl log file");
        goto err;
    }
...
int audit_log_rotate(audit_log *l)
{
    ...
    //open_log(FILE **file, const char *path, int flags, const char *fmode, mode_t mode)
...
int audit_log_put_kmsg(audit_log *l)
{
    ...
    /*
     * This could be rewritten to make use of write_formatted_log if we strip the type number
     * convert to an int, and then pass it on to that function with a pointer to the start of
     * audit, thus stripping type=<type>. However, that seemed like a waste of clock cycles to
