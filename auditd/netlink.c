 ...
 * @return
 *  This function returns 0 on success, else -1 with errno set.
 */
 ...
 * @return
 *  This function returns a positive sequence number on success, else -1 with errno set.
 */
static int audit_send(int fd, int type, const void *data, unsigned int size) {
    ...
    do {
        ...
        rc = sendto(fd, &req, req.nlh.nlmsg_len, 0, (struct sockaddr*) &addr,
                sizeof(addr));
    ...
    if ((uint32_t) rc != req.nlh.nlmsg_len) {
        ...
        rc = -errno;
        SLOGE("Error sending data over the netlink socket: %s\n", strerror(-errno));
        goto out;
    }
...
int audit_set_pid(int fd, uint32_t pid, rep_wait_t wmode) {
    ...
    if (rc < 0) {
        SLOGE("Could net set pid for audit events, error: %s\n", strerror(-rc));
        return rc;
    }
...
int audit_get_reply(int fd, struct audit_reply *rep, reply_t block, int peek) {
    ...
    do {
        ...
        if (len < 0 && errno != EINTR) {
            ...
            SLOGE("Error receiving from netlink socket, error: %s\n", strerror(errno));
            return -errno;
        }
    ...
    if (nladdrlen != sizeof(nladdr)) {
        SLOGE("Protocol fault, error: %s\n", strerror(EPROTO));
        return -EPROTO;
    }

    /* Make sure the netlink message was not spoof'd */
    if (nladdr.nl_pid) {
        SLOGE("Invalid netlink pid received, expected 0 got: %d\n", nladdr.nl_pid);
        return -EINVAL;
    }
...
void audit_close(int fd) {
    ...
    int rc = close(fd);
    if(rc < 0) {
        SLOGE("Attempting to close invalid fd %d, error: %s\n", fd, strerror(errno));
    }
    return;
}
