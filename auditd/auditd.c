...
static void drop_privileges_or_die(void) {
	...
	if (prctl(PR_SET_KEEPCAPS, 1) < 0) {
		SLOGE("Failed on prctl KEEPCAPS: %s\n", strerror(errno));
		exit(1);
	}

	if (setgid(AID_AUDIT) < 0) {
	    SLOGE("Failed on setgid: %s\n", strerror(errno));
		exit(1);
	}

	if (setuid(AID_AUDIT) < 0) {
	    SLOGE("Failed on setuid: %s\n", strerror(errno));
		exit(1);
	}
...
int main(int argc, char *argv[]) {
	...
	if (rc < 0) {
		rc = errno;
		SLOGE("Failed on set signal handler: %s\n", strerror(errno));
		goto err;
	}
	...
	if (audit_fd < 0) {
		rc = errno;
		SLOGE("Failed on audit_set_pid with error: %s\n", strerror(errno));
		goto err;
	}
	...
	while (!quit) {
		...
		if (rc == 0) {
			...
			if (errno != EINTR) {
			    SLOGE("Failed to poll audit log socket: %d : %s\n", errno, strerror(errno));
			}
			continue;
		}

		if (audit_get_reply(audit_fd, &rep, GET_REPLY_BLOCKING, 0) < 0) {
		    SLOGE("Failed on audit_get_reply with error: %s\n", strerror(errno));
			continue;
		}
