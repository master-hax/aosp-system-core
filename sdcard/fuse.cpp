...
static void fuse_status(struct fuse *fuse, __u64 unique, int err)
{
    ...
    if (ret == -1) {
        PLOG(ERROR) << "*** STATUS FAILED: " << strerror(errno) << " ***";
    } else if (static_cast<size_t>(ret) != sizeof(hdr)) {
        PLOG(ERROR) << "*** STATUS FAILED: written " << ret << " expected "
                    << sizeof(hdr) << " ***";
    }
}
...
static void fuse_reply(struct fuse *fuse, __u64 unique, void *data, int len)
{
    ...
    if (ret == -1) {
        PLOG(ERROR) << "*** REPLAY FAILED: " << strerror(errno) << " ***";
    } else if (static_cast<size_t>(ret) != sizeof(hdr) + len) {
        PLOG(ERROR) << "*** REPLAY FAILED: written " << ret << " expected "
                    << sizeof(hdr) + len << " ***";
    }
}
...
static void fuse_notify_delete(struct fuse* fuse, const __u64 parent,
    ...
    if (ret == -1) {
        if (errno != ENOENT) {
            PLOG(ERROR) << "*** NOTIFY FAILED: " << strerror(errno) << " ***";
        }
    } else if (static_cast<size_t>(ret) != sizeof(hdr) + sizeof(data) + namelen + 1) {
        PLOG(ERROR) << "*** NOTIFY FAILED: written " << ret << " expected "
