    ...
    if (multi_user) {
        ...
        if (!sdcardfs_setup(source_path, dest_path_default, uid, gid, multi_user, userid,
            ...
            !sdcardfs_setup(source_path, dest_path_write, uid, gid, multi_user, userid,
                            AID_EVERYBODY, full_write ? 0007 : 0027, derive_gid, default_normal,
                            use_esdfs)) {
        ...
        if (!sdcardfs_setup(source_path, dest_path_default, uid, gid, multi_user, userid,
                            AID_SDCARD_RW, 0006, derive_gid, default_normal, use_esdfs) ||
            !sdcardfs_setup(source_path, dest_path_read, uid, gid, multi_user, userid,
                            AID_EVERYBODY, full_write ? 0027 : 0022, derive_gid, default_normal,
                            use_esdfs) ||
            !sdcardfs_setup(source_path, dest_path_write, uid, gid, multi_user, userid,
                            AID_EVERYBODY, full_write ? 0007 : 0022, derive_gid, default_normal,
                            use_esdfs)) {
            LOG(FATAL) << "failed to sdcardfs_setup";
        }
    }
