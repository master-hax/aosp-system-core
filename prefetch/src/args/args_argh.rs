...
use std::{ffi::OsString, option::Option, path::PathBuf, result::Result::Ok, str::FromStr};
...
{
    let args_vec = args_vec.unwrap().into_iter();
    let mut owned_args: Vec<String> = vec![];
    for arg in args_vec {
        owned_args.push(arg.into().into_string().unwrap());
    }
    let mut args: Vec<&str> = vec![];
    for arg in &owned_args {
        args.push(arg);
    }
    MainArgs::from_args(&[(args[0])], &args[1..]).map_err(|e| Error::InvalidArgs {
        arg_name: "".to_owned(),
        arg_value: "".to_owned(),
        error: e.output,
    })
