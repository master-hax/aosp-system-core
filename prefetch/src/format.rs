...
trait HashMapKey<'a>: Eq + PartialEq + Ord + Hash + Serialize + Deserialize<'a> {}
trait HashMapValue<'a>: Eq + PartialEq + Ord + Serialize + Deserialize<'a> {}
...
impl RecordsFile {
    ...
    pub fn open_file(&self, id: FileId, exclude_files_regex: Vec<Regex>) -> Result<File, Error> {
    ...
    pub fn insert_or_update_inode_info(&mut self, id: FileId, info: InodeInfo) {
        if let Some(inode) = self.inner.inode_map.get_mut(&id) {
            inode.paths.push(info.paths.get(0).unwrap().clone());
        } else {
    ...
    pub fn serialize_records_to_csv(&self, writer: &mut dyn Write) -> Result<(), Error> {
        ...
        for record in &self.inner.records {
            let mut inode_info = self.inner.inode_map.get(&record.file_id).unwrap().clone();
            inode_info.paths.sort();
            let record = TempRecord {
                timestamp: record.timestamp,
                file: inode_info.paths.get(0).unwrap(),
...
pub mod tests {
    ...
    use std::{assert_eq, os::unix::fs::symlink};
    ...
    fn bad_magic() {
        ...
        rf.header.magic[0] = MAGIC_UUID[0] + 1;

        let serialized = serde_cbor::to_vec(&rf).unwrap();
        let deser_err: Result<RecordsFile, _> = serde_cbor::from_slice(&serialized);
        let error = deser_err.unwrap_err().to_string();
        assert_eq!(
            error,
            "Failed to parse magic number. \
            Expected: [16, 84, 60, 184, 96, 219, 73, 69, 161, 213, 222, 167, 210, 59, 5, 73] \
            Found: [17, 84, 60, 184, 96, 219, 73, 69, 161, 213, 222, 167, 210, 59, 5, 73]"
        );
    }
    ...
    fn digest_fails() {
        ...
        let deser_err: Result<RecordsFile, _> = serde_cbor::from_slice(&serialized);
        let error = deser_err.unwrap_err().to_string();
        assert!(error.starts_with("file consistency check failed."));
        assert!(error.ends_with("Found: 0"));
    }
}
