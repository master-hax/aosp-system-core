...
fn is_highly_privileged_path(path: &Path) -> bool {
    // Following directories contain a mix of files with and without access to stat/read from
    // init process. We do not completely exclude these directories as there is still a lot of
...
impl MountInfo {
    ...
    fn parse_line(
        ...
        if excluded_filesystem_types.contains(fs_type) {
            debug!("excluding fs type: {} for {}", fs_type, line);
...
impl TraceSubsystem for MemTraceSubsystem {
    ...
    fn build_records_file(&mut self) -> Result<RecordsFile, Error> {
        ...
        for (device, root_path) in self.mount_info.get_included() {
            ...
            {
                let path = match entry.map_err(|e| Error::Custom {
                    error: format!(
                        "Failed to walk directory: {} with: {}",
                ...
                let stat = path.metadata().map_err(|e| Error::Stat {
                    source: e,
...
mod tests {
    ...
    use crate::tracer::tests::{copy_files, generate_test_data_in, setup_test_dir};
 ...
 .gms.peent-843  [001] ....   485.545820: mm_filemap_add_to_page_cache: dev 254:6 ino cf3 page=6233ce pfn=58108 ofs=0
    ...
    static MOUNTINFO_BUFFER: &str = r#"
            15 26 0:16 / /dev rw,nosuid,relatime shared:2 - tmpfs tmpfs rw,seclabel,mode=755
16 15 0:17 / /dev/pts rw,relatime shared:3 - devpts devpts rw,seclabel,mode=600,ptmxmode=000
17 26 0:18 / /proc rw,relatime shared:4 - proc proc rw,gid=3009,hidepid=invisible
18 26 0:19 / /sys rw,relatime shared:5 - sysfs sysfs rw,seclabel
19 18 0:14 / /sys/fs/selinux rw,relatime shared:6 - selinuxfs selinuxfs rw
20 26 0:20 / /mnt rw,nosuid,nodev,noexec,relatime shared:7 - tmpfs tmpfs rw,seclabel,mode=75,gid=100
25 26 253:14 / /metadata rw,noatime shared:8 - ext4 /dev/block/by-name/metadata rw,seclabel
26 24 254:3 / / ro,nodev,noatime shared:1 - ext4 /dev/block/dm-3 ro,seclabel,errors=panic
27 26 254:4 / /vendor ro,noatime shared:9 - ext4 /dev/block/dm-4 ro,seclabel,errors=panic
28 26 254:5 / /product ro,noatime shared:10 - ext4 /dev/block/dm-5 ro,seclabel,errors=panic
21 26 0:21 / /apex rw,nosuid,nodev,noexec,relatime - tmpfs tmpfs rw,seclabel,mode=755
31 21 254:3 /system/apex/com.adbd /apex/com.adbd ro,noatime shared:1 - ext4 /dev/block/dm-3 errors
32 21 254:3 /system/apex/appsearch /apex/appsearch ro,noatime shared:1 - ext4 /dev/block/dm-3 ro
33 21 254:3 /system/apex/com.art /apex/com.android.art ro shared:1 - ext4 /dev/block/dm-3 seclabel
67 26 254:6 / /data rw,nosuid,nodev,atime shared:24 - f2fs /dev/block/dm-6 rw,lazytime,memory=normal
68 67 254:6 /data /data/user/0 rw,noatime shared:24 - f2fs /dev/block/dm-6 rw,lazytime,memory=normal
69 26 0:35 / /data_mirror rw,nosuid,nodev,noexec,relatime shared:25 - tmpfs tmpfs rw,mode=70,gid=100
"#;
    ...
    fn sample_mountinfo_buf() -> (String, HashMap<DeviceNumber, PathBuf>) {
        let map: HashMap<DeviceNumber, PathBuf> = [
            // ignored because file is in /dev (makedev(0, 16), PathBuf::from("/dev")),
            // ignored because file is in /dev (makedev(0, 17), PathBuf::from("/dev/pts")),
            // ignored because file is in /proc (makedev(0, 18), PathBuf::from("/proc")),
            // ignored because file is in /sys (makedev(0, 19), PathBuf::from("/sys")),
            // ignored because file is in /sys (makedev(0, 14), PathBuf::from("/sys/fs/selinux")),
            // ignored because /mnt is tmpfs (makedev(0, 20), PathBuf::from("/mnt")),
            (makedev(253, 14), PathBuf::from("/metadata")),
            (makedev(254, 3), PathBuf::from("/")),
            (makedev(254, 4), PathBuf::from("/vendor")),
            (makedev(254, 5), PathBuf::from("/product")),
            // ignored because the file is in /apex (makedev(0, 21), PathBuf::from("/apex")),
            (makedev(254, 6), PathBuf::from("/data")),
            // ignored because fs type tmpfs (makedev(0, 35), PathBuf::from("/data_mirror")),
        ]
        .iter()
        .cloned()
        .collect();

        (MOUNTINFO_BUFFER.to_owned(), map)
    }

    #[test]
    fn test_parse_mountinfo_buf() {
        let (buf, expected) = sample_mountinfo_buf();
        let mount_info = MountInfo::with_buf(&buf).unwrap();
        assert_eq!(expected.len(), mount_info.get_included().len());
        for (k, v) in &expected {
            assert_eq!(v, mount_info.included_devices.get(k).unwrap());
        }
    }
    ...
    fn test_add_line() {
        ...
        let (rf, mut files) = generate_test_data_in(None, true, Some(page_size().unwrap() as u64));
        let (_uncached_rf, uncached_files) = copy_files(Path::new(&test_base_dir), &mut files, &rf);
