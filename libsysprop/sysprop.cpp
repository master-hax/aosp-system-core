...
void SystemPropertyBase<T>::CheckNamespace() {
    ...
    const char* name_ptr = data_->full_name_.c_str();

    if (strncmp(name_ptr, kPrefixWriteonce, strlen(kPrefixWriteonce)) == 0) {
        name_ptr += strlen(kPrefixWriteonce);
    }

    bool is_vendor_or_odm_namespace =
            strncmp(name_ptr, kNamespaceVendor, strlen(kNamespaceVendor)) == 0 ||
            strncmp(name_ptr, kNamespaceVendor, strlen(kNamespaceOdm)) == 0;
