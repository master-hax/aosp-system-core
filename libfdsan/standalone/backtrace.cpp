...
static auto& backtrace_mutex = *new std::mutex();
static auto& backtrace_cv = *new std::condition_variable();
static auto& backtrace_maps = *new std::vector<Map>();
static std::atomic<size_t> generation_number;

static std::unique_ptr<BacktraceMap> CreateBacktraceMap() {
  std::unique_ptr<BacktraceMap> result(BacktraceMap::CreateLocalUnsafe());
  return result;
}
