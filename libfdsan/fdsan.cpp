...
class FdsanLock {
 public:
  FdsanLock(FdsanMutex& mutex) : mutex_(&mutex.mutex) { lock(); }
  ~FdsanLock() {
    if (locked_) {
      unlock();
    }
  ...
  void lock() {
    if (locked_) {
      async_safe_fatal("attempted to lock an already locked FdsanLock");
    }
  ...
  void unlock() {
    if (!locked_) {
      async_safe_fatal("attempted to unlock an unlocked FdsanLock");
    }
...
pthread_rwlock_t FdsanLock::fork_mutex = PTHREAD_RWLOCK_INITIALIZER;

struct Fd {
  std::array<FdEvent, kEventHistoryLength> events;
  size_t available_event = 0;

  void* close_tag = nullptr;
...
static void __attribute__((constructor)) fdsan_initialize() {
  ...
  pthread_atfork(FdsanLock::prefork, FdsanLock::postfork_parent, FdsanLock::postfork_child);
...
void* fdsan_set_close_tag(int fd, void* tag) {
  if (!fdsan_should_record()) {
    // We were called reentrantly, probably from libunwindstack.
    // Do nothing.
    return nullptr;
  }

  if (!fd_is_valid(fd)) {
    async_safe_fatal("attempted to set close tag on invalid fd %d", fd);
  }

  auto& fd_info = fd_table[fd];
  FdsanLock lock(fd_info.mutex);
  std::swap(tag, fd_info.close_tag);
  return tag;
}

int fdsan_close_with_tag(int fd, void* tag) {
  if (!fdsan_should_record()) {
  ...
  auto& fd_info = fd_table[fd];
  FdsanLock lock(fd_info.mutex);
  if (fd_info.close_tag != tag) {
    fdsan_report_unowned_close(fd, fd_info.close_tag, tag);
  }
...
static void fdsan_clear_fd_history(int fd) {
  ...
  FdsanLock lock(fd_table[fd].mutex);
