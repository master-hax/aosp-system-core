...
static void ctrl_data_handler(int data, uint32_t events __unused,
 ...
 */
static int find_and_kill_process(int min_score_adj, const char *reason, struct timespec *tm) {
    int i;
    int killed_size = 0;
    ...
    for (i = OOM_SCORE_ADJ_MAX; i >= min_score_adj; i--) {
        ...
        while (true) {
            ...
            killed_size = kill_one_process(procp, min_score_adj, reason, tm);
            if (killed_size >= 0) {
#ifdef LMKD_LOG_STATS
...
static void mp_event_psi(int data, uint32_t events, struct polling_params *poll_params) {
    ...
    static int64_t last_kill_memfree = 0;
    ...
    if (level == VMPRESS_LEVEL_CRITICAL && events != 0) {
    ...
    if (just_killed) {
        /* Don't kill if device is responding and successfully reclaiming memory */
        if (vs.field.nr_free_pages > last_kill_memfree) {
            last_kill_memfree = vs.field.nr_free_pages;
            poll_params->polling_interval_ms = PSI_POLL_PERIOD_SHORT_MS;
            goto no_kill;
        }
        just_killed = false;
        cycle_after_kill = true;
        kill_done_tm = curr_tm;
    ...
    if (get_time_diff_ms(&kill_done_tm, &curr_tm) < PSI_WINDOW_SIZE_MS) {
        /* Fast polling after kill has finished */
        poll_params->polling_interval_ms = PSI_POLL_PERIOD_SHORT_MS;
    } else {
...
no_kill:
     ...
     */
    if (events || just_killed || reclaim == DIRECT_RECLAIM) {
...
do_kill:
    if (low_ram_device) {
        ...
        if (find_and_kill_process(level_oomadj[level], NULL, &curr_tm) == 0) {
            if (debug_process_killing) {
        ...
        pages_freed = find_and_kill_process(min_score_adj, NULL, &curr_tm);
    ...
    while (1) {
        ...
        if (poll_params.poll_handler) {
            ...
            if (get_time_diff_ms(&poll_params.last_poll_tm, &curr_tm) >=
                ...
                if (poll_params.update != POLLING_NO_CHANGE) {
