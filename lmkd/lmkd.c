...
static void stop_wait_for_proc_kill(bool finished) {
    ...
    if (debug_process_killing) {
        ...
        if (clock_gettime(CLOCK_MONOTONIC_COARSE, &curr_tm) != 0) {
            ALOGE("Failed to get current time");
            /* Continue anyway because it's not fatal */
        }
...
static void mp_event_psi(int data, uint32_t events, struct polling_params *poll_params) {
    ...
    if (kill_reason != NONE) {
        if (find_and_kill_process(min_score_adj, kill_reason, kill_desc, &mi, &curr_tm) > 0) {
            killing = true;
        } else {
...
static void poll_kernel() {
    ...
    while (1) {
        ...
        int fields_read = sscanf(rd_buf,
                                 ...
                                 &pid, &uid, &group_leader_pid, &mem_st.pgfault, &mem_st.pgmajfault, &rss_in_pages,
        ...
        if (fields_read == 10 && group_leader_pid == pid) {
            ...
            stats_write_lmk_kill_occurred(LMK_KILL_OCCURRED, uid,
                                          taskname, oom_score_adj, min_score_adj, 0, &mem_st);
        }
...
static inline void init_poll_kernel() {}
