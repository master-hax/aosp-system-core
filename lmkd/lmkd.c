...
static char *proc_get_name(int pid, char *buf, ssize_t buf_size) {
    char path[PATH_MAX];
    int fd;
    char *cp;
    ssize_t ret;
...
static void cmd_procprio(LMKD_CTRL_PACKET packet) {
    ...
    if (use_inkernel_interface) {
        stats_store_taskname(params.pid, proc_get_name(params.pid, path, LINE_MAX),
                             kpoll_info.poll_fd);
        return;
    }
...
static int kill_one_process(struct proc* procp, int min_oom_score, int kill_reason,
    ...
    taskname = proc_get_name(pid, buf, LINE_MAX);
    if (!taskname) {
        goto out;
    }
