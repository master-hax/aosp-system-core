...
static char *reread_file(struct reread_data *data) {
    ...
    while (true) {
        ...
        /*
         * Since we are reading /proc files we can't use fstat to find
         * out the real size of the file. Instead increase the buffer by
         * 50% and keep retrying.
         */
        buf_size += buf_size / 2;
...
static int kill_one_process(struct proc* procp, int min_oom_score, const char *reason) {
    ...
    if (reason) {
        ALOGE("Kill '%s' (%d), uid %d, oom_adj %d to free %ldkB; reason: %s", taskname, pid,
              uid, procp->oomadj, tasksize * page_k, reason);
    } else {
        ALOGE("Kill '%s' (%d), uid %d, oom_adj %d to free %ldkB", taskname, pid,
...
static enum zone_watermark get_lowest_watermark(struct zoneinfo *zi)
{
    ...
    for (int node_idx = 0; node_idx < zi->node_count; node_idx++) {
        ...
        enum zone_watermark zone_wmark[MAX_NR_ZONES];

        memset(&zone_wmark, 0, sizeof(zone_wmark));
...
static void mp_event_psi(struct event_handler_params *params) {
    enum kill_reasons {
    ...
    if (is_kill_pending()) {
        ...
        params->polling_interval_ms = PSI_POLL_PERIOD_SHORT_MS;
        goto no_kill;
    }
    ...
    if (swap_is_low && wmark > WMARK_NONE) {
        ...
        params->polling_interval_ms = PSI_POLL_PERIOD_LONG_MS;
    }
    ...
    if (!low_ram_device && cycle_after_kill && wmark > WMARK_HIGH) {
        ...
        sprintf(kill_desc, "%s watermark is breached even after kill",
            wmark > WMARK_LOW ? "min" : "low");
    } else if (level == VMPRESS_LEVEL_CRITICAL && params->events != 0) {
        /* device is too busy during lowmem event (kill to prevent ANR) */
        kill_reason = NOT_RESPONDING;
        sprintf(kill_desc, "device is not responding");
    } else if (swap_is_low && thrashing > 50) {
        /* page cache is thrashing */
        kill_reason = LOW_SWAP_AND_THRASHING;
        sprintf(kill_desc, "device is low on swap (%" PRId64
            "<%" PRId64 ") and thrashing (%ld%%)",
            mi.field.free_swap, swap_low_threshold, thrashing);
    } else if (swap_is_low && wmark > WMARK_HIGH) {
        /* both free memory and swap are low */
        kill_reason = LOW_MEM_AND_SWAP;
        sprintf(kill_desc, "%s watermark is breached and swap is low (%" PRId64
            "<%" PRId64 ")", wmark > WMARK_LOW ? "min" : "low",
            mi.field.free_swap, swap_low_threshold);
    }
...
no_kill:
     ...
     */
    params->restart_polling = (params->events || wmark > WMARK_NONE);
}
...
static int init(void) {
    ...
    if (reread_file(&file_data) == NULL) {
        ALOGW("Failed to read %s", file_data.filename);
    }
