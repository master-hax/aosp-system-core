...
static void set_process_group(int pid, SchedPolicy sp) {
    DIR* d;
    ...
    while ((de = readdir(d))) {
        ...
        if (set_sched_policy(t_pid, sp)) {
            ALOGW("Failed to set_sched_policy on pid(%d) t_pid(%d) to %d", pid, t_pid, (int)sp);
            continue;
        }
    }
    closedir(d);
...
static int kill_one_process(struct proc* procp) {
    ...
    if (setpriority(PRIO_PROCESS, pid, ANDROID_PRIORITY_HIGHEST)) {
        ALOGW("Unable to raise priority of the killing process '%s' (%d): errno=%d", taskname, pid,
              errno);
    }
    set_process_group(pid, SP_FOREGROUND);
