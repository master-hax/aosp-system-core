...
static bool init_mp_psi(enum vmpressure_level level) {
    ...
    if (level < VMPRESS_LEVEL_LOW || level >= VMPRESS_LEVEL_COUNT)
        return false;

    fd = init_psi_monitor(psi_thresholds[level_idx].stall_type,
        psi_thresholds[level_idx].threshold_ms * US_PER_MS,
    ...
    if (register_psi_monitor(epollfd, fd, &vmpressure_hinfo[level_idx]) < 0) {
...
static int init(void) {
    ...
    if (use_inkernel_interface) {
        ...
        use_psi_monitors =
            property_get_bool("ro.lmk.use_psi", true) &&
            init_mp_psi(VMPRESS_LEVEL_LOW) &&
            init_mp_psi(VMPRESS_LEVEL_MEDIUM) &&
            init_mp_psi(VMPRESS_LEVEL_CRITICAL);
