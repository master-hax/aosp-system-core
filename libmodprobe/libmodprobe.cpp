...
bool Modprobe::LoadModulesParallel(int num_threads) {
    ...
    while (!mod_with_deps.empty()) {
        ...
        for (auto [it_mod, it_dep] : mod_with_deps) {
        ...
        for (auto module_loaded : module_loaded_) {
            ...
            for (auto it = mod_with_softdeps.begin(); it != mod_with_softdeps.end(); it++) {
                it->second.erase(module_loaded);
            }
        }

        // Remove loaded module form dependencies of other modules which are not loaded yet
        for (auto module_loaded_path : module_loaded_paths_) {
            for (auto it = mod_with_deps.begin(); it != mod_with_deps.end(); it++) {
