...
bool Modprobe::LoadModulesParallel(int num_threads) {
    ...
    for (const auto& module : module_load_) {
        ...
        if (dependencies.empty()) {
            LOG(ERROR) << "LMP: Hard-dep: Module " << module << " not in .dep file";
            return false;
        }
    ...
    while (!mod_with_deps.empty()) {
		...
		std::unordered_set<std::string> blocklist_mod;
        ...
        for (const auto& [it_mod, it_dep] : mod_with_deps) {
            ...
            if (IsBlocklisted(cnd_last)) {
                LOG(ERROR) << "LMP: Blocklist: Module-dep " << cnd_last << " : failed to load module " << it_mod;
                return false;
            }
            ...
            if (module_options_[cnd_last].find("load_sequential=1") != std::string::npos) {
                ...
                if (std::find(mods_path_to_load.begin(), mods_path_to_load.end(), cnd_last) == mods_path_to_load.end())
        ...
        auto thread_function = [&] {
            ...
            while (!mods_path_to_load.empty()) {
                ...
                lk.unlock();
                ret &= LoadWithAliases(mod_to_load, true);
                lk.lock();
            }
        };
        ...
        // Remove blocklist module from mod_with_deps
        for (const auto& blk_mod : blocklist_mod) {
            if (mod_with_deps.find(blk_mod) != mod_with_deps.end()) {
                mod_with_deps.erase(blk_mod);
            }
        }
