...
bool Modprobe::ParseDynOptionsCallback(const std::vector<std::string>& args) {
    ...
    std::string handler_path = android::base::Join(std::vector<std::string>(it, args.end()), ' ');
    handler_path.erase(std::remove(handler_path.begin(), handler_path.end(), '\"'),
                       handler_path.end());

    LOG(DEBUG) << "Launching external module options handler: '" << handler_path
               << " for module: " << module;

    // There is no need to set envs for external module options handler - pass
    // empty map.
    std::unordered_map<std::string, std::string> envs_map;
    auto result = RunExternalHandler(handler_path, pwd->pw_uid, 0, envs_map);
    if (!result.ok()) {
