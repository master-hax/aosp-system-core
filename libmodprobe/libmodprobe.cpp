...
bool Modprobe::ParseBlacklistCallback(const std::vector<std::string>& args) {
    ...
    this->module_blacklist_.emplace_back(canonical_name);
...
bool Modprobe::InsmodWithDeps(const std::string& module_name, const std::string& parms) {
    ...
    if (!Insmod(dependencies[0], parms)) {
        return false;
    }
...
bool Modprobe::LoadWithAliases(const std::string& module_name, bool strict,
    ...
    for (const auto& module : modules_to_load) {
        ...
        if (InsmodWithDeps(module, parms)) module_loaded = true;
    }
