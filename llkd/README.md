...
If one of the above conditions is met, the `llkd` blocklist can be adjusted to
cover kernel components. Stack symbol checking involves an additional process
blocklist to prevent sepolicy violations on services that block `ptrace`
operations.
...
### ro.llk.blocklist.process <!-- {:#ro-llk-blocklist-process} -->
...
Note: `false` is an extremely unlikely process to want to blocklist.

### ro.llk.blocklist.parent <!-- {:#ro-llk-blocklist-parent} -->
...
### ro.llk.blocklist.uid <!-- {:#ro-llk-blocklist-uid} -->
...
### ro.llk.blocklist.process.stack <!-- {:#ro-llk-blocklist-process-stack} -->
...
* Properties are limited to 92 characters (however, this is ignored for defaults
  defined in the `include/llkd.h` file in the sources).
