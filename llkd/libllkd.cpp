...
bool llkSkipName(proc* pprocp, proc* procp,
                 const std::unordered_map<std::string, std::unordered_set<std::string>>& blacklist =
                         llkBlacklistParentAndChild) {
    if (!pprocp || !procp || blacklist.empty()) return false;
    if (llkSkipName(procp, llkSkipName(std::to_string(pprocp->pid), blacklist))) return true;
    if (llkSkipName(procp, llkSkipName(pprocp->getComm(), blacklist))) return true;
    if (llkSkipName(procp, llkSkipName(pprocp->getCmdline(), blacklist))) return true;
    return llkSkipName(procp,
...
bool llkCheckStack(proc* procp, const std::string& piddir) {
    ...
    if (llkSkipName(procp, llkBlacklistStack)) return false;
...
void llkLogConfig(void) {
    LOG(INFO) << "ro.config.low_ram=" << llkFormat(llkLowRam) << "\n"
              ...
              << llkFormat(llkBlacklistParentAndChild) << "\n"
...
milliseconds llkCheck(bool checkRunning) {
    ...
    for (auto dp = llkTopDirectory.read(); dp != nullptr; dp = llkTopDirectory.read()) {
        ...
        for (auto tp = taskDirectory.read(dir::task, dp); tp != nullptr;
            ...
            if (pprocp) {
                if (llkSkipName(pprocp, procp)) break;
                if (llkSkipName(pprocp, llkBlacklistParent)) break;
            } else {
