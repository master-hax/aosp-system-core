...
bool khtEnable = LLK_ENABLE_DEFAULT;                 // [khungtask] panic enable
...
void llkValidate() {
    ...
    if (llkCheckMs > llkTimeoutMs) {
        llkCheckMs = llkTimeoutMs;
    } else if (llkCheckMs == 0ms) {
        llkCheckMs = llkTimeoutMs / LLK_CHECKS_PER_TIMEOUT_DEFAULT;
    }

    for (size_t state = 0; state < ARRAY_SIZE(llkStateTimeoutMs); ++state) {
        if ((llkStateTimeoutMs[state] == 0ms) || (llkStateTimeoutMs[state] > llkTimeoutMs)) {
            llkStateTimeoutMs[state] = llkTimeoutMs;
        } else if (llkStateTimeoutMs[state] < LLK_TIMEOUT_MS_MINIMUM) {
            llkStateTimeoutMs[state] = LLK_TIMEOUT_MS_MINIMUM;
        }
        if (llkCheckMs > llkStateTimeoutMs[state]) {
            llkCheckMs = llkStateTimeoutMs[state];
        }
    }

    if (llkCheckMs < LLK_CHECK_MS_MINIMUM) {
        llkCheckMs = LLK_CHECK_MS_MINIMUM;
    }
    if ((llkCycle == 0ms) || (llkCycle > llkCheckMs)) {
        llkCycle = llkCheckMs;
    }
}
...
std::string llkProcGetName(pid_t tid, const char* comm, const char* cmdline) {
    ...
    if (__predict_true(comm != nullptr) && (*comm != '\0')) {
        return comm;
    }
...
int llkKillOneProcess(pid_t pid, char state, pid_t tid, const char* tcomm = nullptr,
    ...
    if (r) {
        LOG(ERROR) << "kill(" << pid << ")=" << r << ' ' << strerror(errno);
    }
...
bool llkWriteStringToFile(const std::string& string, const std::string& file) {
    ...
    return WriteStringToFd(string, fd);
}

bool llkWriteStringToFileConfirm(const std::string& string, const std::string& file) {
    auto fd = llkFileToWriteFd(file);
    auto ret = (fd < 0) ? false : WriteStringToFd(string, fd);
    std::string content;
    if (!ReadFileToString(file, &content)) return ret;
    return Trim(content) == string;
}
...
long long getSchedValue(const std::string& schedString, const char* key) {
    ...
    if ((pos >= schedString.size()) || !::isdigit(schedString[pos])) {
        return -1;
    }
    return ::atoll(schedString.substr(pos).c_str());
}
...
bool llkInit(const char* threadname) {
    ...
    khtEnable = GetBoolProperty(KHT_ENABLE_PROPERTY, khtEnable);
    llkMlockall = GetBoolProperty(LLK_MLOCKALL_PROPERTY, llkMlockall);
    llkTestWithKill = GetBoolProperty(LLK_KILLTEST_PROPERTY, llkTestWithKill);
    ...
    defaultBlacklistProcess = GetProperty(LLK_BLACKLIST_PROCESS_PROPERTY, defaultBlacklistProcess);
    llkBlacklistProcess = Split(defaultBlacklistProcess);
    if (!llkSkipName("[khungtaskd]")) {  // ALWAYS ignore as special
        llkBlacklistProcess.emplace("[khungtaskd]");
    }
    llkBlacklistParent =
        Split(GetProperty(LLK_BLACKLIST_PARENT_PROPERTY, std::to_string(kernelPid) + "," +
                                                             std::to_string(kthreaddPid) +
                                                             "," LLK_BLACKLIST_PARENT_DEFAULT));
    llkBlacklistUid = Split(GetProperty(LLK_BLACKLIST_UID_PROPERTY, LLK_BLACKLIST_UID_DEFAULT));
    ...
    if (llkEnable) {
        if (llkMlockall &&
            ...
            LOG(WARNING) << "mlockall failed " << strerror(errno);
        }
