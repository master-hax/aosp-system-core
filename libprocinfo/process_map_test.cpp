...
TEST(process_map, ReadMapFile) {
  ...
  ASSERT_TRUE(android::procinfo::ReadMapFile(
      map_file,
      [&](uint64_t start, uint64_t end, uint16_t flags, uint64_t pgoff, uint64_t inode,
  ...
  ASSERT_EQ(maps[0].inode, 10267643ULL);
  ...
  ASSERT_EQ(maps[876].inode, 2407ULL);
  ...
  ASSERT_EQ(maps[1260].inode, 10266154ULL);
