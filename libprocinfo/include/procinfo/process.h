...
bool GetProcessInfo(pid_t tid, ProcessInfo* process_info, bool report_error = true);

// Parse the contents of <fd>/status into |process_info|.
// |fd| should be an fd pointing at a /proc/<pid> directory.
bool GetProcessInfoFromProcPidFd(int fd, ProcessInfo* process_info, bool report_error = true);

// Fetch the list of threads from a given process's /proc/<pid> directory.
// |fd| should be an fd pointing at a /proc/<pid> directory.
template <typename Collection>
auto GetProcessTidsFromProcPidFd(int fd, Collection* out, bool report_error = true) ->
...
template <typename Collection>
auto GetProcessTids(pid_t pid, Collection* out, bool report_error = true) ->
    typename std::enable_if<sizeof(typename Collection::value_type) >= sizeof(pid_t), bool>::type {
  ...
  return GetProcessTidsFromProcPidFd(fd.get(), out, report_error);
}
