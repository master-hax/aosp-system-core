...
bool RealBinderWrapper::Init() {
  return binder_watcher_.Init();
}
...
sp<IBinder> RealBinderWrapper::WaitForService(const std::string& service_name) {
  sp<IServiceManager> service_manager = defaultServiceManager();
  if (!service_manager.get()) {
    LOG(ERROR) << "Unable to get service manager";
    return sp<IBinder>();
  ...
  sp<IBinder> binder =
      service_manager->getService(String16(service_name.c_str()));
  if (!binder.get())
    LOG(ERROR) << "Unable to get \"" << service_name << "\" service";
  return binder;
}
