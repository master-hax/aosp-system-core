...
TEST(TestProcMemInfo, SmapsOrRollupSmapsTest) {
    ...
    ProcMemInfo proc_mem(pid);
    MemUsage stats;
    ASSERT_TRUE(proc_mem.SmapsOrRollup(path, &stats));
    EXPECT_EQ(stats.rss, 250332);
    EXPECT_EQ(stats.pss, 108384);
    EXPECT_EQ(stats.uss, 90520);
    EXPECT_EQ(stats.private_clean, 32664);
    EXPECT_EQ(stats.private_dirty, 57856);
    EXPECT_EQ(stats.swap_pss, 0);
}
...
TEST(TestProcMemInfo, SmapsTotalsTest) {
    std::string exec_dir = ::android::base::GetExecutableDirectory();
    std::string path = ::android::base::StringPrintf("%s/testdata1/smaps", exec_dir.c_str());
    ProcMemInfo proc_mem(pid);
    ...
    MemUsage stats;
    for (auto& vma : vmas) {
        stats.rss += vma.usage.rss;
        stats.pss += vma.usage.pss;
        stats.uss += vma.usage.uss;
        stats.private_clean += vma.usage.private_clean;
        stats.private_dirty += vma.usage.private_dirty;
        stats.swap_pss += vma.usage.swap_pss;
    }
...
TEST(SysMemInfoParser, TestVmallocInfo) {
    ...
    SysMemInfo smi;
    EXPECT_EQ(smi.ReadVmallocInfo(vmallocinfo), 29884416);
}
