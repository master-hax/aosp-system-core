...
TEST_F(ValidateProcMemInfo, TestMapsSize) {
    const std::vector<std::unique_ptr<Vma>>& maps = proc_mem->Maps();
...
TEST_F(ValidateProcMemInfo, TestWorkingTestReset) {
    // Expect reset to succeed
    EXPECT_TRUE(proc_mem->WssReset());
}

TEST_F(ValidateProcMemInfo, TestWssEquality) {
...
class ValidatePageInfo : public ::testing::Test {
  protected:
    void SetUp() override {
        ASSERT_EQ(0, pm_kernel_create(&ker));
        ASSERT_EQ(0, pm_process_create(ker, pid, &proc));
...
TEST_F(ValidatePageInfo, TestPageSize) {
    PageInfo& pi = PageInfo::Instance();
    EXPECT_EQ(pm_kernel_pagesize(ker), pi.PageSize());
}

TEST_F(ValidatePageInfo, TestPageFlags) {
    PageInfo& pi = PageInfo::Instance();
...
TEST_F(ValidatePageInfo, TestPageCounts) {
    PageInfo& pi = PageInfo::Instance();
    pi.InitPageAcct(false);
...
TEST_F(ValidatePageInfo, TestPageIdle) {
...
Hugepagesize:       2048 kB)meminfo";
    ...
    EXPECT_EQ(mi.mem_total(), 3019740);
    EXPECT_EQ(mi.page_tables(), 2900);
}
