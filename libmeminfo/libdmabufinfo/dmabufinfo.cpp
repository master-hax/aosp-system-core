        ...
        switch (line[0]) {
            ...
            case 'e':
                if (strncmp(line, "exp_name:", 9) == 0) {
                    char* c = line + 9;
                    *exporter = ::android::base::Trim(std::string(c));
                }
                break;
            case 'n':
...
static bool ReadDmaBufMapRefs(pid_t pid, std::vector<DmaBuffer>* dmabufs) {
    ...
    auto account_dmabuf = [&](uint64_t start, uint64_t end, uint16_t /* flags */,
        ...
        // Here, we know 'name' points to the name at the end of 'line'.
        ...
        // TODO: Add inode number to the callback of ReadMapFileContent instead
        const char* p = name;
        p--;
        // skip spaces
        while (p != line && *p == ' ') {
