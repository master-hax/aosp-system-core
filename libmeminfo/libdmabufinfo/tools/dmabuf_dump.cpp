...
static void PrintDmaBufInfo(const std::vector<DmaBuffer>& bufs) {
    ...
    for (it = pid_set.begin(); it != pid_set.end(); ++it) {
        std::cout << ::android::base::StringPrintf("%16s", GetProcessBaseName(*it).c_str());
    }
    ...
    memset(&pss[0], 0, sizeof(long) * pid_count);

    for (auto buf = bufs.begin(); buf != bufs.end(); ++buf) {

        std::cout << ::android::base::StringPrintf("%16lu\t%10" PRIu64 "\t%lu\t", buf->inode(),buf->size(),
                buf->count());

        memset(&fds[0], 0, sizeof(int) * pid_count);
        memset(&maps[0], 0, sizeof(int) * pid_count);
