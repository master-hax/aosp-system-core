...
bool MemInfo::InitPageAcct(bool pageidle_enable) {
    if (pageidle_enable && !MemInfo::KernelHasPageIdle()) {
...
bool MemInfo::PageFlags(uint64_t pfn, uint64_t* flags) {
...
bool MemInfo::PageMapCount(uint64_t pfn, uint64_t* mapcount) {
...
int MemInfo::IsPageIdle(uint64_t pfn) {
    if (pageidle_fd_ < 0) {
...
int MemInfo::MarkPageIdle(uint64_t pfn) const {
...
int MemInfo::GetPageIdle(uint64_t pfn) const {
    off64_t offset = pfn_to_idle_bitmap_offset(pfn);
    uint64_t idle_bits;
