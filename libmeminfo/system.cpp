...
const std::vector<std::string> MemInfo::kDefaultMemInfoTags = {
...
bool MemInfo::ReadMemInfo(const std::string& path) {
    return ReadMemInfo(MemInfo::kDefaultMemInfoTags, path);
}

// TODO: Delete this function if it can't match up with the c-like implementation below.
// Currently, this added about 50 % extra overhead on hi - key.
#if 0
bool MemInfo::ReadMemInfo(const std::vector<std::string>& tags, const std::string& path) {
    std::string buffer;
