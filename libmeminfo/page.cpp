...
bool PageInfo::InitPageAcct(bool pageidle_enable) {
    if (pageidle_enable && !PageInfo::KernelHasPageIdle()) {
...
bool PageInfo::PageFlags(uint64_t pfn, uint64_t* flags) {
...
bool PageInfo::PageMapCount(uint64_t pfn, uint64_t* mapcount) {
...
int PageInfo::IsPageIdle(uint64_t pfn) {
    if (pageidle_fd_ < 0) {
...
int PageInfo::MarkPageIdle(uint64_t pfn) const {
...
int PageInfo::GetPageIdle(uint64_t pfn) const {
    off64_t offset = pfn_to_idle_bitmap_offset(pfn);
    uint64_t idle_bits;
