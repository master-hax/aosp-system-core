...
struct LibRecord {
  public:
    ...
    const std::unordered_map<ProcessRecord*, MemUsage>& Mappings() const { return mappings_; };
    void AddUsage(ProcessRecord* p, const MemUsage& usage) {
        add_mem_usage(&mappings_[p], usage);
  ...
  private:
    ...
    std::unordered_map<ProcessRecord*, MemUsage> mappings_;
};
...
bool scan_libs_per_process(pid_t pid) {
    ...
    // TODO: We are leaking these. FIXME
    ProcessRecord* prec = new ProcessRecord(pid);
    if (!prec->valid()) {
...
int main(int argc, char* argv[]) {
    ...
    for (auto& l : libs) {
        ...
        for (auto& mapping : lib.Mappings()) {
            ProcessRecord* p = mapping.first;
            const MemUsage& usage = mapping.second;
