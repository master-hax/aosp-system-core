...
static bool scan_libs_per_process(pid_t pid) {
    ...
    for (auto& map : maps) {
        ...
        if (!prefix_filter.empty() && !::android::base::StartsWith(map.name, prefix_filter)) {
            continue;
        }
...
int main(int argc, char* argv[]) {
    ...
    auto rss_sort = [](const ProcessRecord& a, const ProcessRecord& b) {
        return reverse_sort ? a.usage().rss < b.usage().rss : a.usage().rss > b.usage().rss;
    };
