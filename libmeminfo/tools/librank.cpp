...
static bool scan_libs_per_process(pid_t pid) {
    ...
    for (auto& map : maps) {
        ...
        auto [it, inserted] = g_libs.insert(std::pair<std::string, LibRecord>(map.name, LibRecord(map.name)));
...
int main(int argc, char* argv[]) {
    ...
    for (auto& lib : v_libs) {
        ...
        for (auto it = lib.processes().begin(); it != lib.processes().end(); ++it) {
