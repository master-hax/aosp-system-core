...
struct ProcessRecord {
  public:
    ProcessRecord(pid_t pid, uint64_t pgflags = 0, uint64_t pgflags_mask = 0)
        : pid_(-1),
          procmem_(nullptr),
          oomadj_(1001),
          cmdline_(""),
        ...
        std::unique_ptr<::android::meminfo::ProcMemInfo> procmem =
                std::make_unique<::android::meminfo::ProcMemInfo>(pid, false, pgflags_,
                                                                  pgflags_mask_);
        if (procmem == nullptr) {
...
static void scan_stats(std::stringstream& ss, ProcessRecord& proc) {
...
int main(int argc, char* argv[]) {
    auto pss_sort = [](const ProcessRecord& a, const ProcessRecord& b) {
        return reverse_sort ? a.Usage().pss < b.Usage().pss : a.Usage().pss > b.Usage().pss;
    };

    auto uss_sort = [](const ProcessRecord& a, const ProcessRecord& b) {
        return reverse_sort ? a.Usage().uss < b.Usage().uss : a.Usage().uss > b.Usage().uss;
    };

    auto rss_sort = [](const ProcessRecord& a, const ProcessRecord& b) {
        return reverse_sort ? a.Usage().rss < b.Usage().pss : a.Usage().pss > b.Usage().pss;
    };

    auto vss_sort = [](const ProcessRecord& a, const ProcessRecord& b) {
        return reverse_sort ? a.Usage().vss < b.Usage().vss : a.Usage().vss > b.Usage().vss;
    };

    auto swap_sort = [](const ProcessRecord& a, const ProcessRecord& b) {
        return reverse_sort ? a.Usage().swap < b.Usage().swap : a.Usage().swap > b.Usage().swap;
    };
    ...
    if (!read_all_pids(&pids, [&](pid_t pid) -> bool {
            ProcessRecord proc(pid, pgflags, pgflags_mask);
            if (!proc.valid()) {
            ...
            if (proc.Usage().vss == 0) return true;
    ...
    // start dumping output in stringstream here ..
    std::stringstream ss;
    scan_header(ss);
    ss << std::endl;
