...
static void scan_usage(std::stringstream& ss, const MemUsage& usage, std::string padding,
                       bool show_wss) {
    // clear string stream first.
    ss.str("");
    if (!show_wss)
        ss << std::setw(6) << bytes_to_kb(usage.vss) << padding;
    ss << std::setw(6) << bytes_to_kb(usage.rss) << padding << std::setw(6)
       << bytes_to_kb(usage.pss) << padding << std::setw(6) << bytes_to_kb(usage.uss) << padding
       << std::setw(6) << bytes_to_kb(usage.shared_clean) << padding << std::setw(6)
       << bytes_to_kb(usage.shared_dirty) << padding << std::setw(6)
       << bytes_to_kb(usage.private_clean) << padding << std::setw(6)
       << bytes_to_kb(usage.private_dirty) << padding;
}
...
static int show(ProcMemInfo& proc, bool hide_zeroes, bool show_wss) {
    ...
    const std::vector<std::unique_ptr<::android::meminfo::Vma>>& maps = proc.Maps();
    ...
    std::stringstream ss;
    for (auto& vma : maps) {
        MemUsage& vma_stats = show_wss ? vma->wss : vma->usage;
