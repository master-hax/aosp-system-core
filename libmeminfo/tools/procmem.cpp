...
static void scan_stats(std::stringstream& ss, const MemUsage& stats) {
    if (!show_wss) {
        ss << ::android::base::StringPrintf("%6ldK  ", stats.vss / 1024);
    }

    ss << ::android::base::StringPrintf("%6ldK  %6ldK  %6ldK  %6ldK  %6ldK  %6ldK  %6ldK  ",
...
static int show(const MemUsage& proc_stats, const std::vector<Vma>& maps) {
    ...
    for (auto& vma : maps) {
        ...
        scan_stats(ss, vma_stats);
        ss << vma.name << std::endl;
    }
    scan_separator(ss);
    scan_stats(ss, proc_stats);
    ss << "TOTAL" << std::endl;
    std::cout << ss.str();
...
int main(int argc, char* argv[]) {
    ...
    ProcMemInfo proc(pid, show_wss, 0, 0, reset_wss);
    if (reset_wss) {
        if (!proc.WssReset()) {
