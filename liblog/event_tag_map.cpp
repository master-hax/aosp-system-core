...
public:
    ...
    bool operator== (const MapString& rval) const {
        ...
        return fast<strncmp>(str(), rval.str(), len()) == 0;
    }
    bool operator!= (const MapString& rval) const {
        return !(*this == rval);
    }
...
template <> struct _LIBCPP_TYPE_VIS_ONLY std::hash<TagFmt>
    ...
    size_t operator()(const TagFmt __t) const _NOEXCEPT {
        // Tag is typically unique.  Would cost us an extra 100ns for the
        ...
        if (verbose) {
            fprintf(stderr,
                    OUT_TAG ": duplicate tag entries %" PRIu32
                        ":%.*s:%.*s and %" PRIu32 ":%.*s:%.*s)\n",
                    it->first,
                    (int)it->second.first.len(), it->second.first.str(),
                    (int)it->second.second.len(), it->second.second.str(),
                    tag,
                    (int)tagfmt.first.len(), tagfmt.first.str(),
                    (int)tagfmt.second.len(), tagfmt.second.str());
        }
        return false;
    }
...
// strings with '\0' in the deprecated android_lookupEventTag function.
LIBLOG_ABI_PUBLIC EventTagMap* android_openEventTagMap(const char* fileName) {
    ...
    newTagMap->mapAddr = mmap(NULL, end, PROT_READ | PROT_WRITE,
                              MAP_PRIVATE, // needed for android_lookupEventTag
                              fd, 0);
    save_errno = errno;
    close(fd);
...
LIBLOG_ABI_PUBLIC const char* android_lookupEventTag_len(const EventTagMap* map,
    ...
    if (!str) return NULL;
    if (len) *len = str->first.len();
    return str->first.str();
}
