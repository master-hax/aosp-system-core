...
__ABI_PUBLIC__ const char log_time::default_format[] = "%m-%d %H:%M:%S.%q";
__ABI_PUBLIC__ const timespec log_time::EPOCH = { 0, 0 };

// Add %#q for fractional seconds to standard strptime function

__ABI_PUBLIC__ char *log_time::strptime(const char *s, const char *format) {
    time_t now;
#ifdef __linux__
...
__ABI_PUBLIC__ log_time log_time::operator-= (const timespec &T) {
...
__ABI_PUBLIC__ log_time log_time::operator-= (const log_time &T) {
    // No concept of negative time, clamp to EPOCH
    if (*this <= T) {
        return *this = EPOCH;
    }
