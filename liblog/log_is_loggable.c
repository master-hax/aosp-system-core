...
__ABI_PUBLIC__ int __android_log_is_loggable(int prio, const char *tag,
                                             int default_prio)
{
...
__HIDDEN__ int __android_log_is_debuggable()
{
    ...
    if (tag_cache.c || lock()) { /* ro property does not change after set */
        ret = tag_cache.c == '1';
    } else {
        ...
        if (change_detected) {
            refresh_cache(&tag_cache, "ro.debuggable");
 ...
 */
__ABI_PUBLIC__ clockid_t android_log_clockid()
{
...
__ABI_PUBLIC__ int __android_log_security()
{
