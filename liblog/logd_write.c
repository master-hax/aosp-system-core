...
static void * __android_fifo_thread_start(void *obj)
{
    ...
    while (true) {
        ...
        retry = max((FIFO_MAX_SLEEP + (FIFO_SLEEP / 2)) / FIFO_SLEEP, 1);
        while (b->started && !sem_getvalue(&b->full, &i) && (i <= 0) && retry--) {
            usleep(FIFO_SLEEP);
        }
...
static void __write_to_log_fifo_free()
{
    ...
    if (b) {
        ...
        __android_fifo_write_barrier(b);
        blocked = !sem_getvalue(&b->full, &blocked) && (blocked <= 0);
        ...
        for (retry = max((FIFO_MAX_SLEEP + (FIFO_SLEEP / 2)) / FIFO_SLEEP, 1);
                __android_fifo_buffer && retry; --retry) {
            usleep(FIFO_SLEEP);
        }
    }
}
