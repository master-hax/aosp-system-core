...
#if FAKE_LOG_DEVICE == 0
#if (defined(HAVE_PTHREADS) && __BIONIC__) /* pthreads in libc (for free) */
static pthread_key_t tid_specific_key;

static void cached_gettid_init()
{
    pthread_key_create(&tid_specific_key, NULL);
}
#endif

static pid_t cached_gettid()
{
#if (defined(HAVE_PTHREADS) && __BIONIC__)
    static pthread_once_t tid_once_control = PTHREAD_ONCE_INIT;
    void *ret;

    pthread_once(&tid_once_control, cached_gettid_init);

    ret = pthread_getspecific(tid_specific_key);
    if (ret == NULL) {
        ret = (void *)(intptr_t)gettid();
        pthread_setspecific(tid_specific_key, ret);
    }
    return (pid_t)(intptr_t)ret;
#else
    return gettid();
#endif
}
...
static int __write_to_log_kernel(log_id_t log_id, struct iovec *vec, size_t nr)
{
    ...
    static char aid_logd = 0;

    if (aid_logd || (getuid() == AID_LOGD)) {
        aid_logd = 1;
        /*
