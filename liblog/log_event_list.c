...
#define MAX_LIST_NEST_DEPTH 8

typedef struct {
    uint32_t tag;
    unsigned pos; /* Read/write position into buffer */
    unsigned count[MAX_LIST_NEST_DEPTH]; /* Number of elements */
    unsigned list[MAX_LIST_NEST_DEPTH]; /* position for list counter */
...
int android_log_write_list_begin(android_log_context ctx) {
    ...
    if (context->list_nest_depth >= MAX_LIST_NEST_DEPTH) {
        context->overflow = true;
        return -EOVERFLOW;
    }
    needed = sizeof(uint8_t) + sizeof(uint8_t);
...
static android_log_list_element android_log_read_next_internal(
    ...
    if (!context || (kAndroidLoggerRead != context->read_write_flag) ||
            (context->list_nest_depth >= MAX_LIST_NEST_DEPTH) ||
            (context->count[context->list_nest_depth] >=
    ...
    switch ((int)elem.type) {
        case EVENT_TYPE_FLOAT:
            /* Rely on union translate elem.data.int32 to elem.data.float32 */
            /* FALLTHRU */
        case EVENT_TYPE_INT:
        {
            ...
            pos += elem.len;
            elem.complete = !context->list_nest_depth && (context->count[0] == 1);
            if (!peek) {
                if (!--(context->count[context->list_nest_depth])) {
        ...
        case EVENT_TYPE_LIST:
        {
            ...
            elem.complete = context->list_nest_depth >= (MAX_LIST_NEST_DEPTH - 1);
            if (((pos + sizeof(uint8_t)) > context->len) ||
                    ((list_count = context->storage[pos++]) <= 0)) {
                elem.type = EVENT_TYPE_UNKNOWN;
                elem.complete = true;
            }
            if (!peek) {
                if (context->count[context->list_nest_depth] > 0) {
                    --(context->count[context->list_nest_depth]);
                }
                context->list_nest_depth++;
                if (!elem.complete) {
                    context->count[context->list_nest_depth] = list_count;
                }
                context->pos = pos;
            }
            return elem;
