    ...
    if (logger_list->mode & O_PSTORE) {
        ...
        while(1) {
            if (preread_count < sizeof(buf)) {
                ret = read(logger_list->sock, &buf.magic + preread_count,
            ...
            if ((buf.magic != LOGGER_MAGIC)
             || (buf.len <= sizeof(buf))
             || (buf.len > (sizeof(buf) + LOGGER_ENTRY_MAX_PAYLOAD))
             || (buf.id > LOG_ID_MAX)
             || (buf.realtime.tv_nsec >= NS_PER_SEC)) {
                do {
                    memmove(&buf.magic, &buf.magic + 1, --preread_count);
                } while (preread_count && (buf.magic != LOGGER_MAGIC));
                continue;
            }
            ...
            uid = android_getuid();
            if ((uid != AID_ROOT) && (uid != buf.uid)) {
                memset(log_msg, 0, sizeof(*log_msg)); /* security wipe */
                continue;
            }
            if (ret != (ssize_t)(buf.len - sizeof(buf))) {
                continue;
            }
            if ((logger_list->start.tv_sec || logger_list->start.tv_nsec)
             && ((logger_list->start.tv_sec > buf.realtime.tv_sec)
              || ((logger_list->start.tv_sec == buf.realtime.tv_sec)
               && (logger_list->start.tv_nsec > buf.realtime.tv_nsec)))) {
                continue;
            }
            if (logger_list->pid && (logger_list->pid != buf.pid)) {
                continue;
            }
            log_msg->entry_v3.len = buf.len - sizeof(buf);
            log_msg->entry_v3.hdr_size = sizeof(log_msg->entry_v3);
            log_msg->entry_v3.pid = buf.pid;
            log_msg->entry_v3.tid = buf.tid;
            log_msg->entry_v3.sec = buf.realtime.tv_sec;
            log_msg->entry_v3.nsec = buf.realtime.tv_nsec;
            log_msg->entry_v3.lid = buf.id;
            logger_for_each(logger, logger_list) {
