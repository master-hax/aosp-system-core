...
__WEAK__ int socket_local_client(const char *name, int namespaceId, int type)
{
    errno = ENOSYS;
    return -ENOSYS;
}
...
__WEAK__ int socket_make_sockaddr_un(const char *name, int namespaceId,
 ...
 */
__WEAK__ int socket_local_client_connect(int fd, const char *name,
                                         int namespaceId,
                                         int type __unused)
{
...
__ABI_PUBLIC__ const char *android_log_id_to_name(log_id_t log_id)
{
...
__ABI_PUBLIC__ log_id_t android_name_to_log_id(const char *logName)
{
    const char *b;
    int ret;
...
__ABI_PUBLIC__ log_id_t android_logger_get_id(struct logger *logger)
{
    return logger->id;
}
...
__ABI_PUBLIC__ int android_logger_clear(struct logger *logger)
{
    char buf[512];
...
__ABI_PUBLIC__ long android_logger_get_log_size(struct logger *logger)
{
    char buf[512];
...
__ABI_PUBLIC__ int android_logger_set_log_size(struct logger *logger,
                                               unsigned long size)
{
    char buf[512];
 ...
 * log consumed)
 */
__ABI_PUBLIC__ long android_logger_get_log_readable_size(struct logger *logger)
{
    char buf[512];
 ...
 */
__ABI_PUBLIC__ int android_logger_get_log_version(
 ...
 * returns statistics
 */
__ABI_PUBLIC__ ssize_t android_logger_get_statistics(
...
__ABI_PUBLIC__ ssize_t android_logger_get_prune_list(
...
__ABI_PUBLIC__ int android_logger_set_prune_list(
        struct logger_list *logger_list __unused,
        char *buf, size_t len)
{
...
__ABI_PUBLIC__ struct logger_list *android_logger_list_alloc(
        int mode,
        unsigned int tail,
        pid_t pid)
{
...
__ABI_PUBLIC__ struct logger_list *android_logger_list_alloc_time(
        int mode,
        log_time start,
        pid_t pid)
{
...
__ABI_PUBLIC__ struct logger *android_logger_open(
        struct logger_list *logger_list,
        log_id_t id)
{
    struct logger *logger;
...
__ABI_PUBLIC__ struct logger_list *android_logger_list_open(
        log_id_t id,
        int mode,
        unsigned int tail,
        pid_t pid)
...
__ABI_PUBLIC__ int android_logger_list_read(struct logger_list *logger_list,
                                            struct log_msg *log_msg)
{
    int ret, e;
    struct logger *logger;
...
/* Close all the logs */
__ABI_PUBLIC__ void android_logger_list_free(struct logger_list *logger_list)
{
    if (logger_list == NULL) {
        return;
    }
