...
struct android_log_transport_write {
  ...
  unsigned logMask; /* cache of available success (available) */
...
struct android_log_transport_read {
   ...
   * so we do not have an expicit open call.
   */
  int (*read)(struct android_log_logger_list *logger_list,
              struct android_log_transport_context *transp,
              struct log_msg *log_msg);
  /* Assumption this one only called if not ANDROID_LOG_NONBLOCK (blocking) */
...
struct android_log_transport_context {
  ...
  unsigned logMask;      /* list of requested log buffers */
