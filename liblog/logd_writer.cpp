...
// logd_socket is only opened once.  We cannot use a static within a function to remain async-safe,
// thus we open a socket if we see that logd_socket is uninitialized and attempt to exchange it into
// the atomic logd_socket.  If the compare/exchange was successful, then that will be the socket
// used for the duration of the program, otherwise a different thread has already opened and written
// the socket to the atomic, so close the new socket and return.
