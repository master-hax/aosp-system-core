...
TEST(liblog, time_drift) {
  ...
  for (size_t i = 0; i < arraysize(spec); ++i) {
    ...
    int64_t diff = end_span - start_span;
    diff *= i * 2;
  ...
  uint64_t max = 0, min = ULLONG_MAX;
  for (size_t i = 0; i < arraysize(spec); ++i) {
    uint64_t diff = spec[i].end - spec[i].start;
    spec[i].diff = diff;
  ...
  int64_t sum = 0;
  uint64_t abs = 0;
  std::cerr << info << "Kernel drift results in seconds/year (0.032ppm):\n";
  for (size_t i = 0; i < arraysize(spec); ++i) {
    int64_t drift = spec[i].diff - spec[0].diff;
  ...
  if ((max - min) > (sums * duration * NS_PER_SEC / 100)) {
    std::cerr << warning << "large drift: " << std::dec
              << (max - min) * 100 / sums / duration / NS_PER_SEC << "%\n";
    return;
  }

  // if values are small, our calculations are likely to be meaningless.
  if (abs < (2 * sums * duration * (arraysize(spec) - 1))) {
    std::cerr << warning << "negligable drift: " << std::dec
              << abs / (arraysize(spec) - 1) / sums << "ns/" << duration
              << "s\n";
    return;
  }
