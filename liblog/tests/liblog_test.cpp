...
static int count_matching_ts(log_time ts) {
    ...
    for (;;) {
        ...
        if (log_msg.entry.len != (4 + 1 + 8)) continue;
        if (log_msg.id() != LOG_ID_EVENTS) continue;

        char *eventData = log_msg.msg();
        if (!eventData) continue;
        if (eventData[4] != EVENT_TYPE_LONG) continue;
...
TEST(liblog, enoent) {
    ...
    // will fail if we are setuid(AID_SYSTEM), beware test interference.
    // Stopping the logger can affect some test expectations as they count
    // on the log buffers filled with existing content, and this effectively
    // does a logcat -c emptying it. So we want this test to be as near to
    // the bottom of the file. For example liblog.android_logger_get_ is
    // one of those tests.
    system("stop logd");
    usleep(1000000);
...
TEST(liblog, __security) {
    ...
    // that it can be determining that the property is not set.
