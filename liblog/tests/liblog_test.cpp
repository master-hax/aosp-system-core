...
static void RunLogTests(log_id_t log_buffer, FWrite write_messages, FCheck check_message) {
  ...
  auto logger_list_guard =
      make_scope_guard([&logger_list] { android_logger_list_close(logger_list); });
  ...
  auto logger_list_non_block_guard = make_scope_guard(
      [&logger_list_non_block] { android_logger_list_close(logger_list_non_block); });
