...
TEST(liblog_default_tag, no_default_tag) {
  ...
  // liblog does not automatically set the default tag if none was provided.
  __android_log_buf_write(LOG_ID_MAIN, ANDROID_LOG_WARN, nullptr, "message");
  EXPECT_TRUE(message_seen);
  message_seen = false;

  // libbase does automatically set the default tag to Basename(getprogname()) if none was provided.
  expected_tag = Basename(getprogname());
  LOG(WARNING) << "message";
  EXPECT_TRUE(message_seen);
  message_seen = false;

  // Subsequent liblog writes will use the default tag set by libbase.
  __android_log_buf_write(LOG_ID_MAIN, ANDROID_LOG_WARN, nullptr, "message");
  EXPECT_TRUE(message_seen);
}

TEST(liblog_default_tag, libbase_default_tag) {
  using namespace android::base;
  bool message_seen = false;
...
TEST(liblog_default_tag, liblog_default_tag) {
  using namespace android::base;
  bool message_seen = false;
