...
TEST(liblog_global_state, SetAborter_with_liblog) {
  ...
  static const char kMessage[] = "libbase test message";
  ...
  ASSERT_EXIT({ LOG(FATAL) << kMessage; }, testing::ExitedWithCode(kExpectedExitCode), "");

  auto liblog_aborter_function = [](const char* message) __attribute__((noreturn)) {
    EXPECT_EQ(kMessage, message);
    _exit(kExpectedExitCode);
  };
