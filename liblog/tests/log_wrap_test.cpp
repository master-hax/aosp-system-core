...
static void read_with_wrap() {
  ...
  const int mode = ANDROID_LOG_RDONLY | ANDROID_LOG_NONBLOCK | ANDROID_LOG_WRAP;
  ...
  logger_list = android_logger_list_alloc_time(mode, start, 0);
...
TEST(liblog, wrap_mode_blocks) {
  ...
  time_t elapsed = finish.tv_sec - start.tv_sec;
  EXPECT_GT(elapsed, 40);
#else
