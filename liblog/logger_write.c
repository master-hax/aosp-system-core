...
static int check_uid_permissions(uid_t allowed_uid, gid_t allowed_gid) {
  uid_t uid = __android_log_uid();

  /* Matches clientHasCredentials() in logd */
  if ((uid != AID_SYSTEM) && (uid != AID_ROOT) && (uid != allowed_uid)) {
    uid = geteuid();
    if ((uid != AID_SYSTEM) && (uid != AID_ROOT) && (uid != allowed_uid)) {
      gid_t gid = getgid();
      if ((gid != AID_SYSTEM) && (gid != AID_ROOT) && (gid != allowed_gid)) {
        gid = getegid();
        if ((gid != AID_SYSTEM) && (gid != AID_ROOT) && (gid != allowed_gid)) {
          int num_groups;
          gid_t* groups;
          ...
          while (num_groups > 0) {
            if (groups[num_groups - 1] == allowed_gid) {
              break;
            }
            --num_groups;
          }
...
#if defined(__ANDROID__)
  return check_uid_permissions(AID_LOG_STATS, AID_LOG_STATS);
#endif
  return 0;
}
