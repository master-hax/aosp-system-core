...
static std::string& GetDefaultTag() {
  static std::string default_tag = []() -> std::string {
    std::string progname = getprogname();
    return basename(progname.data());
  }();
  return default_tag;
}
...
size_t __android_log_get_default_tag(char* tag, size_t tag_size) {
  ...
  snprintf(tag, tag_size, "%s", GetDefaultTag().c_str());  // no strlcpy on host.
  return GetDefaultTag().length();
}
