        ...
        size_t *p_outLength)
{
    ...
    if (p_format->uid_output) {
        if (entry->uid >= 0) {
            ...
            /*
             * liblog is Android specific, bionic guarentees that
             * calls to non-rentrant getpwuid() are thread safe.
             */
#ifndef __BIONIC__
#error "This code assumes that getpwuid is thread safe, only true with Bionic!"
#endif
