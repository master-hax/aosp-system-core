 ...
 *
 * one-utf8-character-at-a-time algorithm borrowed from utf8_length()
 */
static ssize_t utf8_len(const char *src, size_t len)
{
    const char *cur = src;
...
/*
 * Convert to printable from message to p buffer, return end pointer. If p is
 * NULL, do not copy, but return the dereferenced NULL pointer count
 * non-the-less.
 */
static char *convertPrintable(char *p, const char *message, size_t messageLen)
{
    bool print = p != NULL;
