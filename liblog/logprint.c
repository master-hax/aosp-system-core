...
static void convertMonotonic(const AndroidLogEntry *entry)
{
    AndroidLogEntry *e = (AndroidLogEntry *)entry;
    ...
    list_for_each(node, &convertHead) {
        ...
        if (e->tv_sec < next->time.tv_sec) {
            break;
        } else if (e->tv_sec == next->time.tv_sec) {
            if (e->tv_nsec < next->time.tv_nsec) {
                break;
            }
        }
        list = next;
    ...
    if (next) {
        ...
        time.tv_sec = e->tv_sec;
        time.tv_nsec = e->tv_nsec;
    ...
    /* Apply the correction factor to the record */
    e->tv_nsec = e->tv_nsec - convert.tv_nsec;
    e->tv_sec = e->tv_sec - convert.tv_sec;
    if (e->tv_nsec < 0) {
        e->tv_nsec += NS_PER_SEC;
        e->tv_sec -= 1;
    }
}
