    ...
    const char** fmtStr, size_t* fmtLen)
{
    ...
    if (!cp || !findChar(&cp, &len, '(')) {
        ...
        if (findChar(&cp, &len, '|') && findChar(&cp, &len, INT_MAX)) {
            switch(*cp) {
            case '1':
                if (type != EVENT_TYPE_INT) len = 0;
                break;
            case '2':
                if (type != EVENT_TYPE_LONG) len = 0;
                break;
            case '3':
                if (type != EVENT_TYPE_STRING) len = 0;
                break;
            case '4':
                if (type != EVENT_TYPE_LIST) len = 0;
                break;
            case '5':
                if (type != EVENT_TYPE_FLOAT) len = 0;
                break;
            default: /* let's forgive their transgression */
                break;
            }
            if (len) {
                ++cp;
    ...
    if (cp && len) {
        if (findChar(&cp, &len, '|') && findChar(&cp, &len, INT_MAX)) {
            ...
            case '2': /* Number of bytes */
                ...
                break;
            case '3': /* Number of milliseconds */
                if ((lval <= -1000) || (1000 <= lval)) {
                    /* repaint as (fractional) seconds */
                    outBuf[0] = outBuf[-1];
                    outBuf[-1] = outBuf[-2];
                    outBuf[-2] = outBuf[-3];
                    outBuf[-3] = '.';
        ...
        char *messageBuf, int messageBufLen)
{
    ...
    if ((result == 1) && fmtStr) {
        /* We overflowed :-(, lets repaint the line w/o the format dressings */
        eventData = (const unsigned char*)buf->msg;
        if (buf2->hdr_size) {
            eventData = ((unsigned char *)buf2) + buf2->hdr_size;
        }
