...
LIBLOG_ABI_PRIVATE ssize_t __android_log_pmsg_file_read(
    ...
    while (pmsgRead(&logger_list, &transp, &transp.logMsg) > 0) {
        ...
        if (node == &name_list) {
            ...
            /*
             * Insert in reverse numeric or alpha sorted order as
             * represented by log rotation:
             *
             *   log.10
             *   . . .
             *   log.2
            ...
            list_for_each_reverse(node, &name_list) {
                ...
                const char *l = names->name, *r = a_name->name;
                int compare = 0;
                for (; *l || *r; ++l, ++r) {
                    if (isdigit(*l) && isdigit(*r)) {
                        char *endl, *endr;
                        unsigned long long nl = strtoull(l, &endl, 10);
                        unsigned long long nr = strtoull(r, &endr, 10);
                        l = endl - 1;
                        r = endr - 1;
                        if (nl == nr) {
                            continue;
