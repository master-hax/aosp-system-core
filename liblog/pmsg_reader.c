...
LIBLOG_ABI_PRIVATE ssize_t __android_log_pmsg_file_read(
        ...
        char prio,
        const char *prefix,
        typeof(&__android_log_pmsg_file_write) write_fn) {
    ssize_t ret;
    ...
    while (pmsgRead(&logger_list, &transp, &transp.logMsg) > 0) {
        ...
        /* We do not have an existing entry, create one */
        if (node == &name_list) {
            ...
            list_add_head(&name_list, &names->node);
        }

        /* Remove any that match our sequence number */
        ...
        /* Add content */
        content = calloc(1, sizeof(content->node) + hdr_size + transp.logMsg.entry.len);
        if (!content) {
            ret = -ENOMEM;
            break;
        }
    ...
    list_for_each_safe(node, n, &name_list) {
        ...
        if (buf) {
            if (len) {
                /* Buffer contains an enforced trailing nul just beyond length */
                ssize_t r;
                *strchr(names->name, ':') = '/'; /* Convert back to filename */
                r = (*write_fn)(names->id, names->prio, names->name, buf, len);
                if ((ret >= 0) && (r > 0)) {
                    if (ret == SSIZE_MAX) {
                        ret = r;
                    } else {
