...
#define INVOKE_VOID_METHOD(name, args...)                \
    do {                                                 \
        EnsureInitialized();                             \
        void* method = g_Methods[k_##name];              \
        reinterpret_cast<decltype(&name)>(method)(args); \
    } while (0)
...
void AStatsEvent_build(AStatsEvent* event) {
    INVOKE_VOID_METHOD(AStatsEvent_build, event);
}
...
void AStatsEvent_release(AStatsEvent* event) {
    INVOKE_VOID_METHOD(AStatsEvent_release, event);
}

void AStatsEvent_setAtomId(AStatsEvent* event, uint32_t atomId) {
    INVOKE_VOID_METHOD(AStatsEvent_setAtomId, event, atomId);
}

void AStatsEvent_writeInt32(AStatsEvent* event, int32_t value) {
    INVOKE_VOID_METHOD(AStatsEvent_writeInt32, event, value);
}

void AStatsEvent_writeInt64(AStatsEvent* event, int64_t value) {
    INVOKE_VOID_METHOD(AStatsEvent_writeInt64, event, value);
}

void AStatsEvent_writeFloat(AStatsEvent* event, float value) {
    INVOKE_VOID_METHOD(AStatsEvent_writeFloat, event, value);
}

void AStatsEvent_writeBool(AStatsEvent* event, bool value) {
    INVOKE_VOID_METHOD(AStatsEvent_writeBool, event, value);
}

void AStatsEvent_writeByteArray(AStatsEvent* event, const uint8_t* buf, size_t numBytes) {
    INVOKE_VOID_METHOD(AStatsEvent_writeByteArray, event, buf, numBytes);
}

void AStatsEvent_writeString(AStatsEvent* event, const char* value) {
    INVOKE_VOID_METHOD(AStatsEvent_writeString, event, value);
}

void AStatsEvent_writeAttributionChain(AStatsEvent* event, const uint32_t* uids,
                                       const char* const* tags, uint8_t numNodes) {
    INVOKE_VOID_METHOD(AStatsEvent_writeAttributionChain, event, uids, tags, numNodes);
}

void AStatsEvent_addBoolAnnotation(AStatsEvent* event, uint8_t annotationId, bool value) {
    INVOKE_VOID_METHOD(AStatsEvent_addBoolAnnotation, event, annotationId, value);
}

void AStatsEvent_addInt32Annotation(AStatsEvent* event, uint8_t annotationId, int32_t value) {
    INVOKE_VOID_METHOD(AStatsEvent_addInt32Annotation, event, annotationId, value);
}
...
void AStatsSocket_close() {
    INVOKE_VOID_METHOD(AStatsSocket_close);
}