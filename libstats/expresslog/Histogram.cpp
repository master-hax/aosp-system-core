...
UniformOptions::UniformOptions(int binCount, float minValue, float exclusiveMaxValue) {
    if (binCount < 1) {
        ALOGE("Bin count should be positive number");
        mBinCount = kInvalidBinIndex;
        return;
    }
...
int UniformOptions::getBinsCount() const {
    return mBinCount;
}

int UniformOptions::getBinForSample(float sample) {
    if (mBinCount != -1) {
        if (sample < mMinValue) {
            // goes to underflow
            return 0;
    ...
    ALOGE("Histogram UniformOptions has invalid configuration");
...
void Histogram::logSample(float sample) {
    ...
    FrameworkStatsLog.write(FrameworkStatsLog.EXPRESS_HISTOGRAM_SAMPLE_REPORTED, mMetricIdHash,
                            /*count*/ 1, binIndex);
    stats_write(EXPRESS_HISTOGRAM_SAMPLE_REPORTED, mMetricIdHash, amount);
}
