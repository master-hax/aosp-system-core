...
TEST(UniformOptions, getBinsCount) {
    Histogram::UniformOptions options1(1, 100, 1000);
    ASSERT_EQ(3, options1.getBinsCount());

    Histogram::UniformOptions options10(10, 100, 1000);
    ASSERT_EQ(12, options10.getBinsCount());
}

TEST(UniformOptions, constructZeroBinsCount) {
    Histogram::UniformOptions options1(0, 100, 1000);
    ASSERT_EQ(kInvalidBinIndex, options1.getBinsCount());
    ASSERT_EQ(kInvalidBinIndex, options1.getBinForSample(0));
}
...
TEST(UniformOptions, constructMaxValueLessThanMinValue) {
    Histogram::UniformOptions options1(10, 1000, 100);
    ASSERT_EQ(kInvalidBinIndex, options1.getBinsCount());
    ASSERT_EQ(kInvalidBinIndex, options1.getBinForSample(0));
}

TEST(UniformOptions, testBinIndexForRangeEqual1) {
    Histogram::UniformOptions options(10, 1, 11);
...
TEST(UniformOptions, testBinIndexForRangeEqual2) {
    Histogram::UniformOptions options(10, 1, 21);
    for (int i = 0, bins = options.getBinsCount(); i < bins; i++) {
...
TEST(UniformOptions, testBinIndexForRangeEqual5) {
    Histogram::UniformOptions options(2, 0, 10);
...
TEST(UniformOptions, testBinIndexForRangeEqual10) {
    Histogram::UniformOptions options(10, 1, 101);
...
TEST(UniformOptions, testBinIndexForRangeEqual90) {
    ...
    Histogram::UniformOptions options(binCount, minValue, maxValue);
