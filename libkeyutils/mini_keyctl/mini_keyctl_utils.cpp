...
// Find the keyring id. request_key(2) is more ideal but it cannot find some keyrings like
// .fs-verity. To support all cases, this function looks up a keyring's ID by parsing /proc/keys.
// The keyring description may contain other information in the descritption section depending on
// the key type, only the first word in the keyring description is used for searching.
