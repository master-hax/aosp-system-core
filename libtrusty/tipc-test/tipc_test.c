...
static uint msgsize = 32;
static uint msgburst = 32;
...
static void parse_options(int argc, char **argv)
{
	...
	while (1)
	{
		...
		case 'h':
		      print_usage_and_exit(argv[0],  0, true);
		break;
...
static int connect_foo(uint repeat)
{
	...
	for (i = 0; i < repeat; i++) {
		...
		if (fd >= 0) {
			fprintf(stderr, "secceded to connect to '%s' service\n",
				"foo");
			tipc_close(fd);
		}
	}
...
int main(int argc, char **argv)
{
	...
	if (!test_name) {
		...
		print_usage_and_exit(argv[0], -1, true);
	}

	if (strcmp(test_name, "connect") == 0) {
		return connect_test(repeat);
	}

	if (strcmp(test_name, "connect_foo") == 0) {
		return connect_foo(repeat);
	}

	if (strcmp(test_name, "burst_write") == 0) {
		return burst_write_test(repeat, msgburst, msgsize, variable);
	}

	if (strcmp(test_name, "select") == 0) {
		return select_test(repeat, msgburst,  msgsize);
	}

	if (strcmp(test_name, "blocked_read") == 0) {
		return blocked_read_test(repeat);
	}

	if (strcmp(test_name, "closer1") == 0) {
		return closer1_test(repeat);
	}

	if (strcmp(test_name, "closer2") == 0) {
		return closer2_test(repeat);
	}

	if (strcmp(test_name, "closer3") == 0) {
		return closer3_test(repeat);
	}
	
	if (strcmp(test_name, "echo") == 0) {
		return echo_test(repeat, msgsize, variable);
	}

	if (strcmp(test_name, "ta2ta-ipc") == 0) {
		return ta2ta_ipc_test();
	}

	if (strcmp(test_name, "dev-uuid") == 0) {
		return dev_uuid_test();
	}

	if (strcmp(test_name, "ta-access") == 0) {
