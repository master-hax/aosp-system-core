...
SystemProfileAndroid::SystemProfileAndroid()
    : initialized_(false),
    testing_(false),
    config_root_("/"),
...
SystemProfileAndroid::SystemProfileAndroid(bool testing,
...
bool SystemProfileAndroid::Initialize() {
  CHECK(!initialized_)
  ...
  base::SysInfo::GetLsbReleaseValue("BRILLO_CHANNEL", &channel);
...
bool SystemProfileAndroid::InitializeOrCheck() {
  return initialized_ || Initialize();
}

void SystemProfileAndroid::Populate(
...
std::string SystemProfileAndroid::GetPersistentGUID(const std::string& filename) {
  std::string guid;
...
bool SystemProfileAndroid::GetHardwareId(std::string* hwid) {
  CHECK(hwid);
  *hwid = "test hardwareid";
  return true;
}

metrics::SystemProfileProto_Channel SystemProfileAndroid::ProtoChannelFromString(
