...
TEST(property_schema, CheckSchema_double) {
  EXPECT_FALSE(CheckSchema("double", ""));
  EXPECT_FALSE(CheckSchema("double", "abc"));
  EXPECT_FALSE(CheckSchema("double", "-abc"));
  EXPECT_TRUE(CheckSchema("double", "0.0"));
  EXPECT_TRUE(CheckSchema("double", std::to_string(std::numeric_limits<double>::min())));
  EXPECT_TRUE(CheckSchema("double", std::to_string(std::numeric_limits<double>::max())));
  EXPECT_TRUE(CheckSchema("double", "123.1"));
  EXPECT_TRUE(CheckSchema("double", "-123.1"));

  EXPECT_FALSE(CheckSchema("double -10.1 10.1", ""));
  EXPECT_FALSE(CheckSchema("double -10.1 10.1", "abc"));
  EXPECT_FALSE(CheckSchema("double -10.1 10.1", "-abc"));
  EXPECT_FALSE(CheckSchema("double -10.1 10.1", "-11.1"));
  EXPECT_FALSE(CheckSchema("double -10.1 10.1", "11.1"));
  EXPECT_TRUE(CheckSchema("double -10.1 10.1", "-1.1"));
  EXPECT_TRUE(CheckSchema("double -10.1 10.1", "0"));
  EXPECT_TRUE(CheckSchema("double -10.1 10.1", "1.1"));
}

TEST(property_schema, CheckSchema_size) {
  EXPECT_FALSE(CheckSchema("size", ""));
  EXPECT_FALSE(CheckSchema("size", "ab"));
  EXPECT_FALSE(CheckSchema("size", "abcd"));
  EXPECT_FALSE(CheckSchema("size", "0"));

  EXPECT_TRUE(CheckSchema("size", "512g"));
  EXPECT_TRUE(CheckSchema("size", "512k"));
  EXPECT_TRUE(CheckSchema("size", "512m"));

  EXPECT_FALSE(CheckSchema("size", "512gggg"));
  EXPECT_FALSE(CheckSchema("size", "512mgk"));
  EXPECT_FALSE(CheckSchema("size", "g"));
  EXPECT_FALSE(CheckSchema("size", "m"));
}
...
TEST(property_schema, IsSchemaValid) {
  ...
  EXPECT_TRUE(IsSchemaValid("double"));
  EXPECT_TRUE(IsSchemaValid("double -10.1 10.1"));
  EXPECT_TRUE(IsSchemaValid("double 0.0 10.2"));
  EXPECT_FALSE(IsSchemaValid("double 10.1"));
  EXPECT_FALSE(IsSchemaValid("double 10.1 9.1"));
  EXPECT_FALSE(IsSchemaValid("double 10.1 -11.1"));
