...
// Legacy code for mapping property prefix -> SElinux label
// Copied from bionic/libc/system_properties/contexts_split.cpp.
class ContextListNode {
 public:
  ContextListNode(ContextListNode* next, const char* context)
      : context(strdup(context)), next(next) {}

  ~ContextListNode() { free(context); }
...
class LegacyPropertyMapping {
 public:
  LegacyPropertyMapping(const char* input_mappings);
  ContextListNode* GetContextForName(const char* name);

 private:
  bool InitializePropertiesFromFile(const char* input_file);

  ContextListNode* contexts_;
  PrefixNode* prefixes_;
};

LegacyPropertyMapping::LegacyPropertyMapping(const char* input_file) {
  auto input_lines = Split(input_file, "\n");

  for (auto& line : input_lines) {
    char* prop_prefix = nullptr;
    char* context = nullptr;

    int items = read_spec_entries(line.data(), 2, &prop_prefix, &context);
    if (items <= 0) {
      continue;
    }
    if (items == 1) {
    ...
    auto old_context =
        ListFind(contexts_, [context](ContextListNode* l) { return !strcmp(l->context, context); });
    if (old_context) {
...
ContextListNode* LegacyPropertyMapping::GetContextForName(const char* name) {
...
static void LegacyLookupOreo(benchmark::State& state) {
  ...
  for (auto _ : state) {
    for (const auto& property : properties) {
      benchmark::DoNotOptimize(mapping.GetContextForName(property.c_str()));
    }
  }
}
