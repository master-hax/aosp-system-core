...
Result<Success> AddToTrie(TrieBuilderNode* builder_root, std::set<std::string>* contexts,
                          const std::string& name, const std::string& context, bool exact) {
  ...
  if (context_it == contexts->end()) {
    return Error() << "Unable to store context string";
  }
  ...
  while (name_pieces.size() > 1) {
    ...
    if (child == nullptr) {
      return Error() << "Unable to allocate Trie node";
    }
    current_node = child;
  ...
  if (exact) {
    if (!current_node->AddExactMatchContext(name_pieces.front(), context_pointer)) {
      return Error() << "Duplicate exact match detected for '" << name << "'";
    }
  } else if (!ends_with_dot) {
    if (!current_node->AddPrefixContext(name_pieces.front(), context_pointer)) {
      return Error() << "Duplicate prefix match detected for '" << name << "'";
    }
  } else {
    auto child = current_node->FindChild(name_pieces.front());
    if (child == nullptr) {
