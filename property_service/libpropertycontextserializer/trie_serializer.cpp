...
template <auto size_array, auto match_offset_array_offset_pointer,
          auto match_size_array_offset_pointer,
          auto context_offset_array_offset_pointer>
void TrieSerializer::WriteMatchArray(
  ...
  uint32_t context_offset_array_offset = arena_->AllocateUint32Array(sorted_matches.size());
  arena_->trie(trie_offset)->*context_offset_array_offset_pointer = context_offset_array_offset;
  ...
  for (unsigned int i = 0; i < sorted_matches.size(); ++i) {
    ...
    uint32_t context_string_offset = contexts().FindContextOffset(context_pointer->c_str());
    arena_->uint32_array(context_offset_array_offset)[i] = context_string_offset;
...
void TrieSerializer::WriteTriePrefixMatches(const TrieBuilderNode& builder_node,
  ...
  WriteMatchArray<&TrieNode::num_prefixes_, &TrieNode::prefixes_, &TrieNode::prefix_lens_,
                  &TrieNode::prefix_contexts_>
      (sorted_matches, trie_offset);
}

void TrieSerializer::WriteTrieExactMatches(const TrieBuilderNode& builder_node, uint32_t trie_offset) {
  auto sorted_matches = builder_node.exact_matches_with_context();
  // Exact matches are sorted alphabetically
  std::sort(sorted_matches.begin(), sorted_matches.end(), [](const auto& lhs, const auto& rhs) {
    return lhs.first < rhs.first;
  });
  WriteMatchArray<&TrieNode::num_exact_matches_, &TrieNode::exact_matches_, nullptr,
                  &TrieNode::exact_match_contexts_>(sorted_matches, trie_offset);
}
...
uint32_t TrieSerializer::CreateTrieNode(const TrieBuilderNode& builder_node) {
  ...
  if (builder_node.context() != nullptr && !builder_node.context()->empty()) {
    arena_->trie(trie_offset)->context_ =
        contexts().FindContextOffset(builder_node.context()->c_str());
  } else {
    arena_->trie(trie_offset)->context_ = 0;
  }
  ...
  for (unsigned int i = 0; i < sorted_children.size(); ++i) {
    arena_->uint32_array(children_offset_array_offset)[i] =
        CreateTrieNode(builder_node.children()[i]);
  }
  return trie_offset;
}
...
std::vector<char> TrieSerializer::SerializeTrie(const TrieBuilderNode& builder_root,
