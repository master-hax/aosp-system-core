...
TEST(propertycontextserializer, TrieNodeCheck) {
  ...
  ASSERT_TRUE(BuildTrie(property_infos, "default", "default", &serialized_trie, &build_trie_error))
      << build_trie_error;
  ...
  EXPECT_STREQ("default", property_context_area->schema(root_node.schema_index()));
  ...
  EXPECT_STREQ("1st", property_context_area->schema(test_node.schema_index()));
  ...
  EXPECT_EQ(1U, test_node.num_prefixes());
  {
    ...
    EXPECT_STREQ("test", prefix->name);
    EXPECT_EQ(4U, prefix->namelen);
    EXPECT_STREQ("2nd", property_context_area->context(prefix->context_index));
    EXPECT_STREQ("2nd", property_context_area->schema(prefix->schema_index));
  }
  ...
  EXPECT_EQ(3U, test_node.num_exact_matches());
  {
    ...
    EXPECT_STREQ("test1", match1->name);
    EXPECT_STREQ("test2", match2->name);
    EXPECT_STREQ("test3", match3->name);

    EXPECT_STREQ("3rd", property_context_area->context(match1->context_index));
    EXPECT_STREQ("3rd", property_context_area->context(match2->context_index));
    EXPECT_STREQ("3rd", property_context_area->context(match3->context_index));

    EXPECT_STREQ("3rd", property_context_area->schema(match1->schema_index));
    EXPECT_STREQ("3rd", property_context_area->schema(match2->schema_index));
    EXPECT_STREQ("3rd", property_context_area->schema(match3->schema_index));
  }
  ...
  EXPECT_STREQ("string", final_match->name);
  EXPECT_STREQ("4th", property_context_area->context(final_match->context_index));
  EXPECT_STREQ("4th", property_context_area->schema(final_match->schema_index));
}
...
TEST(propertycontextserializer, FindContextForProperty) {
  ...
  ASSERT_STREQ("default", property_context_area->schema(root_node.schema_index()));

  const char* context;
  const char* schema;
  property_context_area->GetPropertyInfo("abc", &context, &schema);
  EXPECT_STREQ("default", context);
  EXPECT_STREQ("default", schema);
  property_context_area->GetPropertyInfo("abc.abc", &context, &schema);
  EXPECT_STREQ("default", context);
  EXPECT_STREQ("default", schema);
  property_context_area->GetPropertyInfo("123.abc", &context, &schema);
  EXPECT_STREQ("default", context);
  EXPECT_STREQ("default", schema);

  property_context_area->GetPropertyInfo("test.a", &context, &schema);
  EXPECT_STREQ("1st", context);
  EXPECT_STREQ("1st", schema);
  property_context_area->GetPropertyInfo("test.b", &context, &schema);
  EXPECT_STREQ("1st", context);
  EXPECT_STREQ("1st", schema);
  property_context_area->GetPropertyInfo("test.c", &context, &schema);
  EXPECT_STREQ("1st", context);
  EXPECT_STREQ("1st", schema);

  property_context_area->GetPropertyInfo("test.test", &context, &schema);
  EXPECT_STREQ("5th", context);
  EXPECT_STREQ("5th", schema);
  property_context_area->GetPropertyInfo("test.testa", &context, &schema);
  EXPECT_STREQ("2nd", context);
  EXPECT_STREQ("2nd", schema);
  property_context_area->GetPropertyInfo("test.testb", &context, &schema);
  EXPECT_STREQ("2nd", context);
  EXPECT_STREQ("2nd", schema);
  property_context_area->GetPropertyInfo("test.testc", &context, &schema);
  EXPECT_STREQ("2nd", context);
  EXPECT_STREQ("2nd", schema);

  property_context_area->GetPropertyInfo("test.test.a", &context, &schema);
  EXPECT_STREQ("2nd", context);
  EXPECT_STREQ("2nd", schema);
  property_context_area->GetPropertyInfo("test.test.b", &context, &schema);
  EXPECT_STREQ("2nd", context);
  EXPECT_STREQ("2nd", schema);
  property_context_area->GetPropertyInfo("test.test.c", &context, &schema);
  EXPECT_STREQ("2nd", context);
  EXPECT_STREQ("2nd", schema);

  property_context_area->GetPropertyInfo("test.test1", &context, &schema);
  EXPECT_STREQ("3rd", context);
  EXPECT_STREQ("3rd", schema);
  property_context_area->GetPropertyInfo("test.test2", &context, &schema);
  EXPECT_STREQ("7th", context);
  EXPECT_STREQ("7th", schema);
  property_context_area->GetPropertyInfo("test.test3", &context, &schema);
  EXPECT_STREQ("3rd", context);
  EXPECT_STREQ("3rd", schema);

  property_context_area->GetPropertyInfo("test.test11", &context, &schema);
  EXPECT_STREQ("2nd", context);
  EXPECT_STREQ("2nd", schema);
  property_context_area->GetPropertyInfo("test.test22", &context, &schema);
  EXPECT_STREQ("2nd", context);
  EXPECT_STREQ("2nd", schema);
  property_context_area->GetPropertyInfo("test.test33", &context, &schema);
  EXPECT_STREQ("2nd", context);
  EXPECT_STREQ("2nd", schema);

  property_context_area->GetPropertyInfo("this.is.a.long.string", &context, &schema);
  EXPECT_STREQ("4th", context);
  EXPECT_STREQ("4th", schema);

  property_context_area->GetPropertyInfo("this.is.a.long", &context, &schema);
  EXPECT_STREQ("default", context);
  EXPECT_STREQ("default", schema);
  property_context_area->GetPropertyInfo("this.is.a", &context, &schema);
  EXPECT_STREQ("default", context);
  EXPECT_STREQ("default", schema);
  property_context_area->GetPropertyInfo("this.is", &context, &schema);
  EXPECT_STREQ("default", context);
  EXPECT_STREQ("default", schema);
  property_context_area->GetPropertyInfo("this", &context, &schema);
  EXPECT_STREQ("default", context);
  EXPECT_STREQ("default", schema);

  property_context_area->GetPropertyInfo("test.test2.a", &context, &schema);
...
TEST(propertycontextserializer, RealProperties) {
  ...
  auto serialized_trie = std::string();
  auto build_trie_error = std::string();
  ASSERT_TRUE(BuildTrie(property_infos, "u:object_r:default_prop:s0", "string", &serialized_trie,
                        &build_trie_error))
      << build_trie_error;
  ...
  for (const auto& [property, context] : properties_and_contexts) {
    ...
    property_context_area->GetPropertyInfo(property.c_str(), &returned_context, nullptr);
