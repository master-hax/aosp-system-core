...
TEST(propertycontextserializer, RealProperties) {
  ...
  auto serialized_trie = BuildTrie(prefixes_with_context, exact_matches_with_context,
  ...
  for (const auto& [property, context] : properties_and_contexts) {
    EXPECT_EQ(context, property_context_area->FindContextForProperty(property.c_str())) <<
        property;
  }
