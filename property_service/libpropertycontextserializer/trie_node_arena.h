...
class TrieNodeArena {
 public:
  TrieNodeArena() : data_(5000, '\0'), current_data_pointer_(0) {}
  ...
  TrieNode* trie(uint32_t offset) {
    return reinterpret_cast<TrieNode*>(data_.data() + offset);
  }
  ...
  std::vector<char> truncated_data() const {
    auto result = data_;
