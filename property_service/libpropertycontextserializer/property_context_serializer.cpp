...
bool BuildTrie(const std::vector<std::pair<std::string, std::string>>& prefixes_with_context,
               const std::vector<std::pair<std::string, std::string>>& exact_matches_with_context,
               const std::string& default_context, std::string* serialized_trie,
               std::string* error) {
  ...
  for (const auto& [prefix, context] : prefixes_with_context) {
    if (!AddToTrie(&builder_root, &contexts, prefix, context, false, error)) {
      return false;
    }
  }
  for (const auto& [exact_match, context] : exact_matches_with_context) {
    if (!AddToTrie(&builder_root, &contexts, exact_match, context, true, error)) {
      return false;
    }
  }

  auto trie_serializer = TrieSerializer();
  *serialized_trie = trie_serializer.SerializeTrie(builder_root, contexts);
  return true;
}
