...
TEST(propertycontextserializer, BuildTrie_Simple) {
  ...
  EXPECT_TRUE(AddToTrie(&builder_root, &contexts, "test.", "1st", false, &error));
  EXPECT_TRUE(AddToTrie(&builder_root, &contexts, "test.test", "2nd", false, &error));
  EXPECT_TRUE(AddToTrie(&builder_root, &contexts, "test.test1", "3rd", true, &error));
  EXPECT_TRUE(AddToTrie(&builder_root, &contexts, "test.test2", "3rd", true, &error));
  EXPECT_TRUE(AddToTrie(&builder_root, &contexts, "test.test3", "3rd", true, &error));
  EXPECT_TRUE(AddToTrie(&builder_root, &contexts, "this.is.a.long.string", "4th", true, &error));
