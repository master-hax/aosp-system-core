...
class TrieSerializer {
 public:
  ...
  std::vector<char> SerializeTrie(const TrieBuilderNode& builder_root, const std::set<std::string>& contexts);
 ...
 private:
  ...
  const PropertyContextArea contexts() const {
    mmap_base = arena_->data().data();
    return PropertyContextArea();
  }
