...
bool TrieNode::FindChildForString(const char* name, uint32_t namelen, TrieNode* child) const {
  auto node_index = Find(trie_node_base_->num_child_nodes_, [this, name, namelen](auto array_offset) {
    int cmp = strncmp(child_node(array_offset).name(), name, namelen);
    if (cmp == 0 && child_node(array_offset).name()[namelen] != '\0') {
