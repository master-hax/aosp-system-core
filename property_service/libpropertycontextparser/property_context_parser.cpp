...
void PropertyContextArea::GetPropertyInfoIndexes(const char* name, uint32_t* context_index,
  ...
  for (uint32_t i = 0; i < trie_node.num_exact_matches(); ++i) {
    if (!strcmp(trie_node.exact_match(i)->name, remaining_name)) {
  ...
  for (uint32_t i = 0; i < trie_node.num_prefixes(); ++i) {
    ...
    if (!strncmp(trie_node.prefix(i)->name, remaining_name, prefix_len)) {
...
void PropertyContextArea::GetPropertyInfo(const char* property, const char** context,
...
bool PropertyContextAreaFile::LoadDefaultPath() {
  return LoadPath("/dev/__properties__/property_contexts");
}

bool PropertyContextAreaFile::LoadPath(const char* filename) {
  ...
  if ((fd_stat.st_uid != 0) || (fd_stat.st_gid != 0) ||
      ...
      (fd_stat.st_size < static_cast<off_t>(sizeof(PropertyContextArea)))) {
    close(fd);
    return false;
  }
  ...
  auto property_context_area = reinterpret_cast<PropertyContextArea*>(map_result);
  if (property_context_area->version() != 1 || property_context_area->size() != mmap_size) {
...
void PropertyContextAreaFile::Reset() {
  if (mmap_size_ > 0) {
