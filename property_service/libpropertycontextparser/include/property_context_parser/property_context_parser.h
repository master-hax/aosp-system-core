...
class TrieNode {
 public:
  ...
  TrieNode child_node(int n) const {
    uint32_t child_node_offset = serialized_data_->uint32_array(trie_node_base_->child_nodes_)[n];
  ...
  const char* prefix(int n) const {
    uint32_t prefix_offset = serialized_data_->uint32_array(trie_node_base_->prefixes_)[n];
    return serialized_data_->string(prefix_offset);
  }
  uint32_t prefix_len(int n) const {
    return serialized_data_->uint32_array(trie_node_base_->prefix_lens_)[n];
  }
  uint32_t prefix_context_index(int n) const {
    return serialized_data_->uint32_array(trie_node_base_->prefix_contexts_)[n];
  }

  uint32_t num_exact_matches() const { return trie_node_base_->num_exact_matches_; }
  const char* exact_match(int n) const {
    uint32_t exact_match_offset = serialized_data_->uint32_array(trie_node_base_->exact_matches_)[n];
    return serialized_data_->string(exact_match_offset);
  }
  uint32_t exact_match_context_index(int n) const {
    return serialized_data_->uint32_array(trie_node_base_->exact_match_contexts_)[n];
  }
...
class PropertyContextAreaFile {
 public:
  PropertyContextAreaFile() : mmap_base_(nullptr), mmap_size_(0) {}
  ~PropertyContextAreaFile() { Reset(); }

  PropertyContextAreaFile(const PropertyContextAreaFile&) = delete;
  void operator=(const PropertyContextAreaFile&) = delete;
  PropertyContextAreaFile(PropertyContextAreaFile&&) = default;
  PropertyContextAreaFile& operator=(PropertyContextAreaFile&&) = default;

  bool LoadDefaultPath();
  bool LoadPath(const char* filename);

  const PropertyContextArea* operator->() const {
    return reinterpret_cast<const PropertyContextArea*>(mmap_base_);
  }
