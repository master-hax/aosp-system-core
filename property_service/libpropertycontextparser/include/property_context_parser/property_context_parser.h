...
class TrieNode {
 ...
 public:
  ...
  const TrieNode* child_node(int n) const {
    uint32_t child_node_offset = reinterpret_cast<const uint32_t*>(mmap_base + child_nodes_)[n];
    return reinterpret_cast<const TrieNode*>(mmap_base + child_node_offset);
  }
  ...
  const char* prefix(int n) const {
    uint32_t prefix_offset = reinterpret_cast<const uint32_t*>(mmap_base + prefixes_)[n];
    return reinterpret_cast<const char*>(mmap_base + prefix_offset);
  }
  uint32_t prefix_len(int n) const {
    return reinterpret_cast<const uint32_t*>(mmap_base + prefix_lens_)[n];
  }
  const char* prefix_context(int n) const {
    uint32_t prefix_context_offset =
        reinterpret_cast<const uint32_t*>(mmap_base + prefix_contexts_)[n];
    return reinterpret_cast<const char*>(mmap_base + prefix_context_offset);
  }

  uint32_t num_exact_matches() const { return num_exact_matches_; }
  const char* exact_match(int n) const {
    uint32_t exact_match_offset =
        reinterpret_cast<const uint32_t*>(mmap_base + exact_matches_)[n];
    return reinterpret_cast<const char*>(mmap_base + exact_match_offset);
  }
  const char* exact_match_context(int n) const {
    uint32_t exact_match_context_offset =
        reinterpret_cast<const uint32_t*>(mmap_base + exact_match_contexts_)[n];
    return reinterpret_cast<const char*>(mmap_base + exact_match_context_offset);
  }

  const char* context() const {
    if (context_ == 0) return nullptr;
    return mmap_base + context_;
  }
...
class PropertyContextArea {
 public:
  ...
  const TrieNode* trie(uint32_t offset) const {
    ...
    return reinterpret_cast<const TrieNode*>(mmap_base + offset);
  }

  const char* string(uint32_t offset) const {
    if (offset != 0 && offset > size()) return nullptr;
    return static_cast<const char*>(mmap_base + offset);
  }

  const uint32_t* uint32_array(uint32_t offset) const {
    if (offset != 0 && offset > size()) return nullptr;
    return reinterpret_cast<const uint32_t*>(mmap_base + offset);
  }
};
