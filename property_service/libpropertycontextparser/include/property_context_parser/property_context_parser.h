...
class TrieNode {
 public:
  ...
  const char* name() const { return node_property_entry()->name; }
  ...
  const PropertyEntry* prefix(int n) const {
    uint32_t prefix_entry_offset =
        serialized_data_->uint32_array(trie_node_base_->prefix_entries_)[n];
  ...
  const PropertyEntry* exact_match(int n) const {
    uint32_t exact_match_entry_offset =
        serialized_data_->uint32_array(trie_node_base_->exact_match_entries_)[n];
    return reinterpret_cast<const PropertyEntry*>(serialized_data_->data_base() +
 ...
 private:
  const PropertyEntry* node_property_entry() const {
    return reinterpret_cast<const PropertyEntry*>(serialized_data_->data_base() +
                                                  trie_node_base_->property_entry_);
  }
...
class PropertyContextArea : private SerializedData {
 public:
  ...
  int FindSchemaIndex(const char* context) const;
  ...
  TrieNode root_node() const { return trie(uint32_array(0)[4]); }
 ...
 private:
  ...
  uint32_t schemas_offset() const { return uint32_array(0)[3]; }
