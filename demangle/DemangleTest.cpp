...
TEST(DemangleTest, TemplateArguments) {
  ...
  ASSERT_EQ("art(one<char>)", demangler.Parse("_ZN3artE3oneIcE"));
  ASSERT_EQ("art(one<char>)", demangler.Parse("_ZN3artE3oneIcE"));
}
...
TEST(DemangleTest, demangle_c) {
  char* demangled;

  demangled = demangle_c("_ZN1a1b1cES0_");
  ASSERT_STREQ("a::b::c(a::b)", demangled);
  free(demangled);

  ASSERT_EQ(nullptr, demangle_c("_"));

  ASSERT_EQ(nullptr, demangle_c("_Z"));

  ASSERT_EQ(nullptr, demangle_c("_Za"));

  demangled = demangle_c("_Zaa");
  ASSERT_STREQ("operator&&", demangled);
  free(demangled);

  ASSERT_EQ(nullptr, demangle_c("Xa"));
}
