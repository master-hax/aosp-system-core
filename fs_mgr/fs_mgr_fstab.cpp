...
struct fs_mgr_flag_values {
    ...
    unsigned int file_contents_mode = 0;
    unsigned int file_names_mode = 0;
    unsigned int erase_blk_size = 0;
    unsigned int logical_blk_size = 0;
};
...
#define FS_OPTIONS_LEN 1024
    ...
    while ((len = getline(&line, &alloc_len, fstab_file)) != -1) {
        ...
        fstab.entries.emplace_back(std::move(entry));
    }

    if (fstab.entries.empty()) {
...
bool Fstab::ReadFromFile(const std::string& path, Fstab* fstab) {
...
struct fstab* fs_mgr_read_fstab(const char* fstab_path) {
    Fstab fstab;
    if (!Fstab::ReadFromFile(fstab_path, &fstab)) {
        return nullptr;
    }
...
bool Fstab::ReadFromDt(Fstab* fstab) {
...
bool Fstab::ReadDefault(Fstab* fstab) {
    Fstab dt_fstab;
    ReadFromDt(&dt_fstab);
    ...
    *fstab = std::move(dt_fstab);
    ...
    if (!default_fstab_path.empty()) {
        ReadFromFile(default_fstab_path, &default_fstab);
    } else {
    ...
    return !fstab->entries.empty();
}

struct fstab* fs_mgr_read_fstab_default() {
    Fstab fstab;
    if (!Fstab::ReadDefault(&fstab)) {
        return nullptr;
    }
...
Fstab LegacyFstabToFstab(const struct fstab* legacy_fstab) {
    ...
    for (int i = 0; i < legacy_fstab->num_entries; i++) {
        FstabEntry entry;
        entry.blk_device = legacy_fstab->recs[i].blk_device;
        entry.logical_partition_name = legacy_fstab->recs[i].logical_partition_name;
        entry.mount_point = legacy_fstab->recs[i].mount_point;
        entry.fs_type = legacy_fstab->recs[i].fs_type;
        entry.flags = legacy_fstab->recs[i].flags;
        entry.fs_options = legacy_fstab->recs[i].fs_options;
        entry.fs_mgr_flags.val = legacy_fstab->recs[i].fs_mgr_flags;
        entry.key_loc = legacy_fstab->recs[i].key_loc;
        entry.key_dir = legacy_fstab->recs[i].key_dir;
        entry.verity_loc = legacy_fstab->recs[i].verity_loc;
        entry.length = legacy_fstab->recs[i].length;
        entry.label = legacy_fstab->recs[i].label;
        entry.partnum = legacy_fstab->recs[i].partnum;
        entry.swap_prio = legacy_fstab->recs[i].swap_prio;
        entry.max_comp_streams = legacy_fstab->recs[i].max_comp_streams;
        entry.zram_size = legacy_fstab->recs[i].zram_size;
        entry.reserved_size = legacy_fstab->recs[i].reserved_size;
        entry.file_contents_mode = legacy_fstab->recs[i].file_contents_mode;
        entry.file_names_mode = legacy_fstab->recs[i].file_names_mode;
        entry.erase_blk_size = legacy_fstab->recs[i].erase_blk_size;
        entry.logical_blk_size = legacy_fstab->recs[i].logical_blk_size;
        entry.sysfs_path = legacy_fstab->recs[i].sysfs_path;
...
fstab* FstabToLegacyFstab(const Fstab& fstab) {
    ...
    legacy_fstab->num_entries = fstab.entries.size();
    legacy_fstab->recs =
            static_cast<fstab_rec*>(calloc(legacy_fstab->num_entries, sizeof(fstab_rec)));

    for (int i = 0; i < legacy_fstab->num_entries; i++) {
        legacy_fstab->recs[i].blk_device = strdup(fstab.entries[i].blk_device.c_str());
        legacy_fstab->recs[i].logical_partition_name =
                strdup(fstab.entries[i].logical_partition_name.c_str());
        legacy_fstab->recs[i].mount_point = strdup(fstab.entries[i].mount_point.c_str());
        legacy_fstab->recs[i].fs_type = strdup(fstab.entries[i].fs_type.c_str());
        legacy_fstab->recs[i].flags = fstab.entries[i].flags;
        legacy_fstab->recs[i].fs_options = strdup(fstab.entries[i].fs_options.c_str());
        legacy_fstab->recs[i].fs_mgr_flags = fstab.entries[i].fs_mgr_flags.val;
        legacy_fstab->recs[i].key_loc = strdup(fstab.entries[i].key_loc.c_str());
        legacy_fstab->recs[i].key_dir = strdup(fstab.entries[i].key_dir.c_str());
        legacy_fstab->recs[i].verity_loc = strdup(fstab.entries[i].verity_loc.c_str());
        legacy_fstab->recs[i].length = fstab.entries[i].length;
        legacy_fstab->recs[i].label = strdup(fstab.entries[i].label.c_str());
        legacy_fstab->recs[i].partnum = fstab.entries[i].partnum;
        legacy_fstab->recs[i].swap_prio = fstab.entries[i].swap_prio;
        legacy_fstab->recs[i].max_comp_streams = fstab.entries[i].max_comp_streams;
        legacy_fstab->recs[i].zram_size = fstab.entries[i].zram_size;
        legacy_fstab->recs[i].reserved_size = fstab.entries[i].reserved_size;
        legacy_fstab->recs[i].file_contents_mode = fstab.entries[i].file_contents_mode;
        legacy_fstab->recs[i].file_names_mode = fstab.entries[i].file_names_mode;
        legacy_fstab->recs[i].erase_blk_size = fstab.entries[i].erase_blk_size;
        legacy_fstab->recs[i].logical_blk_size = fstab.entries[i].logical_blk_size;
        legacy_fstab->recs[i].sysfs_path = strdup(fstab.entries[i].sysfs_path.c_str());
    }
    return legacy_fstab;
}
