...
static bool is_dt_compatible() {
    ...
    std::string dt_value;
    LINFO << "Reading " << file_name;
    if (android::base::ReadFileToString(file_name, &dt_value)) {
        // trim the trailing '\0' out, otherwise the comparison
        // will produce false-negatives.
        dt_value.resize(dt_value.size() - 1);
        LINFO << "Read value " << dt_value;
        if (dt_value == "android,firmware") {
            return true;
        }
    }

    LERROR << "firmware/android is not compatible with 'android,firmware'";
    return false;
}
...
static bool is_dt_fstab_compatible() {
    ...
    LERROR << "firmware/android/fstab is not compatible with 'android,fstab'";
    return false;
}
