...
using FstabUniquePtr = std::unique_ptr<fstab, decltype(&fs_mgr_free_fstab)>;
 ...
 */
static std::set<std::string> extract_boot_devices(FstabUniquePtr fstab) {
    if (!fstab) return {};
...
std::set<std::string> fs_mgr_get_boot_devices() {
    ...
    FstabUniquePtr fstab(fs_mgr_read_fstab_default(), fs_mgr_free_fstab);
    return extract_boot_devices(std::move(fstab));
}
