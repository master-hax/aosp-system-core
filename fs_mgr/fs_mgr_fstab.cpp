 ...
 */
static std::set<std::string> extract_boot_devices(const struct fstab* fstab) {
...
std::set<std::string> fs_mgr_get_boot_devices(FsManagerMountStage mount_stage) {
    ...
    if (mount_stage == FsManagerMountStage::kFirstStageMount) {
        return extract_boot_devices(fs_mgr_read_fstab_dt());
    } else if (mount_stage == FsManagerMountStage::kSecondStageMount) {
        return extract_boot_devices(fs_mgr_read_fstab_default());
    }
