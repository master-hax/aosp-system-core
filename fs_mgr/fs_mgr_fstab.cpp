...
static std::string ResolveBlockDevice(const std::string& block_device) {
    ...
    while (true) {
        ...
        sys_dir = "/sys/dir/" + sub_device_name;
    }
}

FstabEntry* GetMountedEntryForMountPoint(Fstab* fstab, const std::string& path) {
    Fstab mounts;
    if (!ReadFstabFromFile("/proc/mounts", &mounts)) {
        LERROR << "Failed to read /proc/mounts";
        return nullptr;
    }
    auto mounted_entry = GetEntryForMountPoint(&mounts, path);
    if (mounted_entry == nullptr) {
        LWARNING << path << " is not mounted";
        return nullptr;
    }
    std::string resolved_block_device = ResolveBlockDevice(mounted_entry->blk_device);
    LINFO << path << " is mounted on " << resolved_block_device;
    for (auto& entry : *fstab) {
        if (entry.mount_point != path) {
            continue;
        }
    ...
    LERROR << "Didn't find entry that was used to mount " << path;
    return nullptr;
}
