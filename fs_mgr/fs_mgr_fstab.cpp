...
#undef CheckFlag
        ...
        if (StartsWith(flag, "encryptable=")) {
            ...
            for (char i = 'a'; i <= 'z'; i++) {
                std::string zoned, partition = "sd";
                partition.push_back(i);
                if (!ReadFileToString("/sys/class/block/" + partition + "/queue/zoned", &zoned)) {
                    break;
                }
                if (!android::base::StartsWith(zoned, "host-managed")) {
                    continue;
                }
                entry->zoned_device = "/dev/block/" + partition;
                ...
                LINFO << "Removed ATGC in fs_options as " << entry->fs_options
                      << " for zoned partition=" << entry->zoned_device;
                break;
            }
        } else {
