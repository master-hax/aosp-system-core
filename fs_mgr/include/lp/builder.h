...
class Partition final {
  public:
    Partition(const std::string& name, const uint8_t guid[16], uint32_t attributes);
...
class MetadataBuilder {
  public:
    ...
    std::shared_ptr<Partition> AddPartition(const std::string& name, const uint8_t guid[16],
                                            uint32_t attributes);
    ...
    std::shared_ptr<Partition> FindPartition(const std::string& name);
  ...
  private:
    MetadataBuilder();
    bool Init(uint64_t blockdevice_size, uint32_t metadata_reserved);
    bool Init(const LpMetadata& metadata);

  private:
    LpMetadataHeader header_;
    std::vector<std::shared_ptr<Partition>> partitions_;
    std::map<std::string, std::shared_ptr<Partition>> partition_names_;
