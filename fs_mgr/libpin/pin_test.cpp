...
void PrintErrorResult(const char* cmd, const char* msg, android::pin::Result result) {
    std::cerr << cmd << ": " << msg << ": " << result.GetFile() << ":" << result.GetLine() << ": "
              << result.GetScope() << "::" << result.GetFunction()
              << "(): " << result.GetDescription();
    if (int error = result.GetErrno()) std::cerr << ": " << strerror(error);
    std::cerr << "\n";
}
...
bool VerifyPattern(const char* cmd, const char* bdev, int bdev_fd, const char* file, int file_fd,
    ...
    android::pin::ExtentMap em;
    memset(&em, 0, sizeof(em));
    android::pin::FiemapExtent* fe = em.em_fiemap.fm_extents;
    fe->fe_physical = 0;
    fe->fe_logical = 0;
    fe->fe_length = 0;
    ...
    for (off_t offset = 0; offset < file_size;) {
        ...
        auto result = FileGetExtentMap(file_fd, offset, leftover, &em);
        if (result.IsError()) {
            PrintErrorResult(cmd, "could not get extent map", result);
            return false;
        }
...
bool CommandEnsure(const char* cmd, const char* bdev, const char* file, bool verify_file) {
    UniqueFd bdev_fd(open(bdev, O_RDONLY | O_DIRECT | O_CLOEXEC));
    if (bdev_fd < 0) {
        PrintErrno(cmd, "could not open block device", bdev);
        return false;
    }
    UniqueFd file_fd(open(file, O_RDONLY | O_CLOEXEC));
    if (file_fd < 0) {
    ...
    auto result = android::pin::FileEnsureReliablyPinned(file_fd, bdev_fd, bdev_string);
    if (result.IsError()) {
...
bool CommandCreate(const char* cmd, const char* bdev, const char* file, off_t file_size,
    ...
    UniqueFd file_fd(open(file, O_RDWR | O_CREAT | O_EXCL | O_CLOEXEC, 0600));
    if (file_fd < 0) {
    ...
    std::string bdev_string(bdev);
    auto result =
            android::pin::FileAllocateSpaceAndReliablyPin(file_fd, bdev_fd, bdev_string, file_size);
    if (result.IsError()) {
    ...
    result = android::pin::FileEnsureReliablyPinned(file_fd, bdev_fd, bdev_string);
    if (result.IsError()) {
...
bool WriteOneByte(const char* cmd, const char* file, off_t offset) {
    UniqueFd file_fd(open(file, O_RDWR | O_CLOEXEC));
    if (file_fd < 0) {
...
bool BoundaryTests(const char* cmd, const char* bdev, const char* file, off_t file_size,
    ...
    constexpr off_t block_size = android::pin::kF2fsBlockSize;
    constexpr off_t segment_size = android::pin::kF2fsSegmentSize;
...
bool CommandSupport(const char* cmd, const char* bdev) {
    std::string bdev_string(bdev);
    auto result = android::pin::BdevFileSystemSupportsReliablePinning(bdev_string);
    if (result.IsError()) {
...
bool CommandTest(const char* cmd, const char* bdev, const char* dir) {
    ...
    if (sfs.f_bsize != android::pin::kF2fsBlockSize) {
        PrintError(cmd, "stafs() invalid f_bsize", dir);
        return false;
    }

    off_t free_space = GetCwdFreeSpace(cmd, dir);
    if (free_space < 20 * android::pin::kF2fsSegmentSize) {
    ...
    if (all_succeeded) {
        ...
        file_size &= ~(android::pin::kF2fsSegmentSize - 1);
