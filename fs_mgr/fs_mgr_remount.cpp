...
bool IsNormalReadonlyPartition(const android::fs_mgr::FstabEntry& entry) {
...
static bool IsRemountable(Fstab& candidates, const FstabEntry& entry) {
    if (!IsNormalReadonlyPartition(entry)) {
        return false;
    }
...
static Fstab::iterator FindPartition(Fstab& fstab, const std::string& partition) {
...
static RemountStatus GetRemountList(Fstab& fstab, const std::vector<std::string>& argv,
                                    Fstab* partitions) {
    auto candidates = fs_mgr_overlayfs_candidate_list(fstab);
...
static int do_remount(int argc, char* argv[]) {
    ...
    if (partitions.empty() || just_disabled_verity) {
        if (auto_reboot) {
            reboot(setup_overlayfs);
        }
        if (user_please_reboot_later) {
            return MUST_REBOOT;
        }
        LOG(WARNING) << "No partitions to remount";
        return check_result.second;
    }
