...
// Constants.
using android::fs_mgr::kVBMetaVerifyResultError;
using android::fs_mgr::kVBMetaVerifyResultErrorVerification;
using android::fs_mgr::kVBMetaVerifyResultOK;
...
TEST_F(AvbUtilTest, GetAvbFooter) {
    ...
    auto fd = OpenReadFd(system_path);
...
TEST_F(AvbUtilTest, VerifyVBMetaSignature) {
    ...
    EXPECT_EQ(kVBMetaVerifyResultOK, VerifyVBMetaSignature(vbmeta, expected_public_key));

    // Converts the expected key into an 'unexpected' key.
    expected_public_key[10] ^= 0x80;
    EXPECT_EQ(kVBMetaVerifyResultErrorVerification,
...
TEST_F(AvbUtilTest, VerifyVBMetaSignatureWithModification) {
    ...
    EXPECT_TRUE(TestVBMetaModification(kVBMetaVerifyResultErrorVerification, vbmeta,
...
TEST_F(AvbUtilTest, VerifyVBMetaSignatureNotSigned) {
    ...
    EXPECT_EQ(kVBMetaVerifyResultErrorVerification, VerifyVBMetaSignature(vbmeta, ""));
}
...
TEST_F(AvbUtilTest, VerifyVBMetaSignatureInvalidVBMeta) {
    ...
    VBMetaData invalid_vbmeta((const uint8_t*)vbmeta_buffer.data(), vbmeta_buffer.size(),
                              "invalid_vbmeta");
    EXPECT_EQ(kVBMetaVerifyResultError, VerifyVBMetaSignature(invalid_vbmeta, ""));
}
...
TEST_F(AvbUtilTest, VerifyVBMetaDataWithoutFooter) {
    ...
    EXPECT_EQ(kVBMetaVerifyResultOK, verify_result);
...
TEST_F(AvbUtilTest, VerifyVBMetaDataError) {
    ...
    EXPECT_NE(nullptr, vbmeta);
    EXPECT_TRUE(CompareVBMeta(system_path, *vbmeta));
    EXPECT_EQ(kVBMetaVerifyResultErrorVerification, verify_result);
...
TEST_F(AvbUtilTest, GetChainPartitionInfo) {
    ...
    // Loads the key blobs for comparision.
...
TEST_F(AvbUtilTest, LoadAndVerifyVbmetaImpl) {
    ...
    EXPECT_EQ(kVBMetaVerifyResultOK,
...
TEST_F(AvbUtilTest, LoadAndVerifyVbmetaImplWithSuffixes) {
    ...
    EXPECT_EQ(kVBMetaVerifyResultError,
              LoadAndVerifyVbmetaImpl(
