...
std::unique_ptr<FsAvbHashtreeDescriptor> GetHashtreeDescriptor(
    ...
    for (const auto& vbmeta : vbmeta_images) {
        if (!vbmeta_partition.empty() && vbmeta.partition() != vbmeta_partition) {
            LWARNING << vbmeta_partition << " / " << vbmeta.partition() << " skipped";
            continue;
        }
        LWARNING << vbmeta_partition << " / " << vbmeta.partition() << " proceed";
