...
std::unique_ptr<AvbFooter> GetAvbFooter(const unique_fd& fd) {
    ...
    switch (vbmeta_ret) {
        case AVB_VBMETA_VERIFY_RESULT_OK:
            if (pk_data == nullptr || pk_len <= 0) {
                ...
                return kVBMetaVerifyResultError;
            }
            if (!VerifyPublicKeyBlob(pk_data, pk_len, expected_public_key_blob)) {
                ...
                return kVBMetaVerifyResultErrorVerification;
            }
            return kVBMetaVerifyResultOK;
...
std::unique_ptr<VBMetaData> VerifyVBMetaData(const unique_fd& fd, const std::string& partition_name,
                                             const std::string& expected_public_key_blob,
    ...
    if (verify_result == kVBMetaVerifyResultOK ||
        verify_result == kVBMetaVerifyResultErrorVerification) {
        return vbmeta;
    }
...
VBMetaVerifyResult LoadAndVerifyVbmetaImpl(
    ...
    if (!allow_verification_error && verify_result == kVBMetaVerifyResultErrorVerification) {
    ...
    if (load_chained_vbmeta) {
        ...
        for (auto& chain : chain_partitions) {
            auto sub_ret = LoadAndVerifyVbmetaImpl(
                    ...
                    out_vbmeta_images);
            if (sub_ret != kVBMetaVerifyResultOK) {
                verify_result = sub_ret;  // might be 'ERROR' or 'ERROR VERIFICATION'.
                if (verify_result == kVBMetaVerifyResultError) {
