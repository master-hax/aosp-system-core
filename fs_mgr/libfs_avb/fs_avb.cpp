...
bool CheckPublicKeyIsMatching(const FstabEntry& fstab_entry, const std::string& public_key_data,
...
bool CheckHashtreeDescriptorRootDigestIsMatching(const FstabEntry& fstab_entry,
...
AvbUniquePtr AvbHandle::LoadAndVerifyVbmeta(const FstabEntry& fstab_entry,
    ...
    // Verify vbmeta image checking by either root digest of hashtree descriptor, or public key.
    if (!fstab_entry.avb_hashtree_descriptor_digest.empty()) {
        if (!CheckHashtreeDescriptorRootDigestIsMatching(fstab_entry, avb_handle->vbmeta_images_,
        ...
        if (!CheckPublicKeyIsMatching(fstab_entry, public_key_data, preload_avb_key_blobs)) {
            avb_handle->status_ = AvbHandleStatus::kVerificationError;
