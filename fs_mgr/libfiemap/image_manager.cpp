...
ImageManagerStatus ImageManager::CreateBackingImage(
    ...
    status_t status = SplitFiemap::Create(data_path, size, 0, &fw, on_progress);
    if (status != OK) {
        return ImageManagerStatus(status);
    }

    bool reliable_pinning;
    if (!FilesystemHasReliablePinning(data_path, &reliable_pinning)) {
        return ImageManagerStatus(ErrorStatusFromErrno());
    }
...
ImageManagerStatus ImageManager::ZeroFillNewImage(const std::string& name, uint64_t bytes) {
    auto data_path = GetImageHeaderPath(name);
