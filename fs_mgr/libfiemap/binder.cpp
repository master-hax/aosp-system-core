...
class ImageManagerBinder final : public IImageManager {
  public:
    ...
    ImageManagerStatus ZeroFillNewImage(const std::string& name, uint64_t bytes) override;
...
static ImageManagerStatus ToImageManagerStatus(const char* func, const binder::Status& status) {
    if (!status.isOk()) {
        LOG(ERROR) << func << " binder returned: " << status.toString8().string();
        if (status.serviceSpecificErrorCode() != 0) {
            return ImageManagerStatus(status.serviceSpecificErrorCode());
        } else if (status.transactionError() != OK) {
            return ImageManagerStatus(status.transactionError());
        } else {
            return ImageManagerStatus(UNKNOWN_ERROR);
        }
    }
...
ImageManagerStatus ImageManagerBinder::CreateBackingImage(
        const std::string& name, uint64_t size, int flags,
    ...
    return ToImageManagerStatus(__PRETTY_FUNCTION__, status);
}
...
ImageManagerStatus ImageManagerBinder::ZeroFillNewImage(const std::string& name, uint64_t bytes) {
