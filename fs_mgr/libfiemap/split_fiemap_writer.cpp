...
std::unique_ptr<SplitFiemap> SplitFiemap::Create(const std::string& file_path, uint64_t file_size,
    ...
    if (Create(file_path, file_size, max_piece_size, &ret, progress) != OK) {
        return nullptr;
    }
    return ret;
}

status_t SplitFiemap::Create(const std::string& file_path, uint64_t file_size,
    ...
    while (remaining_bytes) {
        ...
        status_t status = FiemapWriter::Open(chunk_path, chunk_size, &writer, true, on_progress);
        if (status != OK) {
            out.reset();
            return status;
        }
