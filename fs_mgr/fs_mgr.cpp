...
static void check_fs(const std::string& blk_device, const std::string& fs_type,
    ...
    if (is_extfs(fs_type)) {
        ...
        const char* f2fs_fsck_forced_argv[] = {F2FS_FSCK_BIN, "-f", blk_device};

        if (should_force_check(*fs_stat)) {
            LINFO << "Running " << F2FS_FSCK_BIN << " -f " << blk_device;
            ret = android_fork_execvp_ext(
                ARRAY_SIZE(f2fs_fsck_forced_argv), const_cast<char **>(f2fs_fsck_forced_argv),
                &status, true, LOG_KLOG | LOG_FILE,
               true, const_cast<char *>(FSCK_LOG_FILE), NULL, 0);
        } else {
