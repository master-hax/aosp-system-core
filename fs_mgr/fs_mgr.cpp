...
static std::string realpath(const std::string& blk_device) {
    ...
    if (!Realpath(blk_device.c_str(), &real_path)) {
...
static int __mount(const std::string& source, const std::string& target,
    ...
    errno = 0;
    ret = mount(source.c_str(), target.c_str(), rec->fs_type, mountflags, rec->fs_options);
    save_errno = errno;
...
static int fs_match(const std::string& in1, const std::string& in2) {
