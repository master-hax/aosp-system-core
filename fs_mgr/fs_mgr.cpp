...
static bool is_extfs(const char *fs_type)
{
    return !strcmp(fs_type, "ext4") ||
           !strcmp(fs_type, "ext3") ||
           !strcmp(fs_type, "ext2");
}

static bool should_force_check(int fs_stat) {
    return fs_stat & (FS_STAT_E2FSCK_F_ALWAYS |
...
static bool read_ext4_superblock(const char *blk_device,
...
static bool run_tune2fs(const char *argv[], int argc)
{
    ...
    ret = android_fork_execvp_ext(argc, const_cast<char **>(argv),
                                  NULL, true, LOG_KLOG | LOG_FILE,
                                  true, NULL, NULL, 0);
    return ret == 0;
}
...
{
    bool has_quota = (sb->s_feature_ro_compat &
    ...
    if (!tune2fs_available()) {
        LERROR << "Unable to " << (want_quota ? "enable " : "disable ")
               << "quotas on " << blk_device << " because "
               << TUNE2FS_BIN " is missing";
        return;
    }
    ...
    if (!run_tune2fs(argv, ARRAY_SIZE(argv))) {
        LERROR << "Failed to run " << TUNE2FS_BIN " to "
               << (want_quota ? "enable " : "disable ")
               << "quotas on " << blk_device;
        *fs_stat |= FS_STAT_TOGGLE_QUOTAS_FAILED;
    }
}

static void do_late_tune2fs(const char *blk_device, const struct fstab_rec *rec,
                            const struct ext4_super_block *sb, int *fs_stat)
{
    // Set the number of reserved filesystem blocks if needed.
    if (rec->fs_mgr_flags & MF_RESERVEDSIZE) {
        uint64_t reserved_blocks = rec->reserved_size / EXT4_BLOCK_SIZE(sb);
        uint64_t reserved_threshold = ext4_blocks_count(sb) * 0.02;

        if (reserved_blocks > reserved_threshold) {
        ...
        if (ext4_r_blocks_count(sb) != reserved_blocks) {
            if (tune2fs_available()) {
                ...
                LINFO << "Setting reserved block count on "
                      << blk_device << " to " << reserved_blocks;
                if (!run_tune2fs(argv, ARRAY_SIZE(argv))) {
                    LERROR << "Failed to run " TUNE2FS_BIN " to set the "
                           << "number of reserved blocks on " << blk_device;
                    *fs_stat |= FS_STAT_SET_RESERVED_BLOCKS_FAILED;
                }
            } else {
                LERROR << "Unable to set the number of reserved blocks on "
...
// the fstab record.
//
static int prepare_fs_for_mount(const char *blk_device,
                                const struct fstab_rec *rec)
{
    ...
    if (is_extfs(rec->fs_type) && (rec->fs_mgr_flags & MF_RESERVEDSIZE)) {
        ...
        if (read_ext4_superblock(blk_device, &sb, &fs_stat)) {
            do_late_tune2fs(blk_device, rec, &sb, &fs_stat);
        }
    }
