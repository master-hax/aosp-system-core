...
static int mount_with_alternatives(Fstab* fstab, int start_idx, int* end_idx, int* attempted_idx) {
    ...
    if (!end_idx || !attempted_idx || start_idx >= fstab->size()) {
        errno = EINVAL;
    ...
    for (i = start_idx;
        ...
        if (mounted) {
            LERROR << __FUNCTION__
                   << "(): skipping fstab dup mountpoint=" << (*fstab)[i].mount_point << " rec["
                   << i << "].fs_type=" << (*fstab)[i].fs_type << " already mounted as "
        ...
        if (fs_stat & FS_STAT_INVALID_MAGIC) {
            LERROR << __FUNCTION__ << "(): skipping mount due to invalid magic, mountpoint="
                   << (*fstab)[i].mount_point << " blk_dev=" << realpath((*fstab)[i].blk_device)
        ...
        while (retry_count-- > 0) {
            if (!__mount((*fstab)[i].blk_device, (*fstab)[i].mount_point, (*fstab)[i])) {
                *attempted_idx = i;
                ...
                // retry after fsck
                check_fs((*fstab)[i].blk_device, (*fstab)[i].fs_type, (*fstab)[i].mount_point,
                         &fs_stat);
            }
        }
...
static bool should_use_metadata_encryption(const FstabEntry& entry) {
    return entry.fs_mgr_flags.file_encryption || entry.fs_mgr_flags.force_fde_or_fbe ||
           entry.fs_mgr_flags.key_directory;
}
...
int fs_mgr_mount_all(Fstab* fstab, int mount_mode) {
    ...
    for (size_t i = 0; i < fstab->size(); i++) {
        ...
        if ((*fstab)[i].fs_mgr_flags.vold_managed || (*fstab)[i].fs_mgr_flags.recovery_only ||
            (*fstab)[i].fs_mgr_flags.first_stage_mount ||
            ((mount_mode == MOUNT_MODE_LATE) && !(*fstab)[i].fs_mgr_flags.late_mount) ||
            ((mount_mode == MOUNT_MODE_EARLY) && (*fstab)[i].fs_mgr_flags.late_mount)) {
            continue;
        }

        // Skip swap and raw partition entries such as boot, recovery, etc.
        if ((*fstab)[i].fs_type == "swap" || (*fstab)[i].fs_type == "emmc" ||
            (*fstab)[i].fs_type == "mtd") {
            continue;
        }

        // Skip mounting the root partition, as it will already have been mounted.
        if ((*fstab)[i].mount_point == "/" || (*fstab)[i].mount_point == "/system") {
            if (((*fstab)[i].flags & MS_RDONLY) != 0) {
                fs_mgr_set_blk_ro((*fstab)[i].blk_device);
            }
            continue;
        }

        // Translate LABEL= file system labels into block devices.
        if (is_extfs((*fstab)[i].fs_type)) {
            if (!TranslateExtLabels(&(*fstab)[i])) {
                LERROR << "Could not translate label to block device";
                continue;
            }
        }

        if ((*fstab)[i].fs_mgr_flags.logical) {
            if (!fs_mgr_update_logical_partition(&(*fstab)[i])) {
                LERROR << "Could not set up logical partition, skipping!";
                continue;
            }
        }

        if (!checkpoint_manager.Update(&(*fstab)[i])) {
            continue;
        }

        if ((*fstab)[i].fs_mgr_flags.wait && !fs_mgr_wait_for_file((*fstab)[i].blk_device, 20s)) {
            LERROR << "Skipping '" << (*fstab)[i].blk_device << "' during mount_all";
            continue;
        }

        if ((*fstab)[i].fs_mgr_flags.avb) {
            if (!avb_handle) {
            ...
            if (avb_handle->SetUpAvbHashtree(&(*fstab)[i], true /* wait_for_verity_dev */) ==
                AvbHashtreeResult::kFail) {
                LERROR << "Failed to set up AVB on partition: " << (*fstab)[i].mount_point
                       << ", skipping!";
                /* Skips mounting the device. */
                continue;
            }
        } else if (((*fstab)[i].fs_mgr_flags.verify)) {
            int rc = fs_mgr_setup_verity(&(*fstab)[i], true);
        ...
        if (!mret) {
            int status = handle_encryptable((*fstab)[attempted_idx]);
            ...
            if (status != FS_MGR_MNTALL_DEV_NOT_ENCRYPTABLE) {
                ...
                if (status == FS_MGR_MNTALL_DEV_NEEDS_METADATA_ENCRYPTION) {
                    if (!call_vdc(
                                {"cryptfs", "encryptFstab", (*fstab)[attempted_idx].mount_point})) {
        ...
        bool wiped = partition_wiped((*fstab)[top_idx].blk_device.data());
        bool crypt_footer = false;
        if (mret && mount_errno != EBUSY && mount_errno != EACCES &&
            (*fstab)[top_idx].fs_mgr_flags.formattable && wiped) {
            /* top_idx and attempted_idx point at the same partition, but sometimes
             * at two different lines in the fstab.  Use the top one for formatting
             * as that is the preferred one.
             */
            LERROR << __FUNCTION__ << "(): " << realpath((*fstab)[top_idx].blk_device)
                   << " is wiped and " << (*fstab)[top_idx].mount_point << " "
                   << (*fstab)[top_idx].fs_type << " is formattable. Format it.";

            checkpoint_manager.Revert(&(*fstab)[top_idx]);

            if ((*fstab)[top_idx].is_encryptable() && (*fstab)[top_idx].key_loc != KEY_IN_FOOTER) {
                unique_fd fd(TEMP_FAILURE_RETRY(
                        open((*fstab)[top_idx].key_loc.c_str(), O_WRONLY | O_CLOEXEC)));
                if (fd >= 0) {
                    LINFO << __FUNCTION__ << "(): also wipe " << (*fstab)[top_idx].key_loc;
                    wipe_block_device(fd, get_file_size(fd));
                } else {
                    PERROR << __FUNCTION__ << "(): " << (*fstab)[top_idx].key_loc
                           << " wouldn't open";
                }
            } else if ((*fstab)[top_idx].is_encryptable() &&
                       (*fstab)[top_idx].key_loc == KEY_IN_FOOTER) {
                crypt_footer = true;
            }
            if (fs_mgr_do_format((*fstab)[top_idx], crypt_footer) == 0) {
        ...
        if (mret && mount_errno != EBUSY && mount_errno != EACCES &&
            (*fstab)[attempted_idx].is_encryptable()) {
            if (wiped) {
                LERROR << __FUNCTION__ << "(): " << (*fstab)[attempted_idx].blk_device
                       << " is wiped and " << (*fstab)[attempted_idx].mount_point << " "
                       << (*fstab)[attempted_idx].fs_type << " is encryptable. Suggest recovery...";
                encryptable = FS_MGR_MNTALL_DEV_NEEDS_RECOVERY;
                continue;
            } else {
                ...
                LERROR << __FUNCTION__ << "(): possibly an encryptable blkdev "
                       << (*fstab)[attempted_idx].blk_device << " for mount "
                       << (*fstab)[attempted_idx].mount_point << " type "
                       << (*fstab)[attempted_idx].fs_type;
                if (fs_mgr_do_tmpfs_mount((*fstab)[attempted_idx].mount_point.c_str()) < 0) {
                    ++error_count;
                    continue;
                }
            }
            encryptable = FS_MGR_MNTALL_DEV_MIGHT_BE_ENCRYPTED;
                   ...
                   should_use_metadata_encryption((*fstab)[attempted_idx])) {
            if (!call_vdc({"cryptfs", "mountFstab", (*fstab)[attempted_idx].mount_point})) {
                ++error_count;
            }
            ...
            if ((*fstab)[attempted_idx].fs_mgr_flags.no_fail) {
                PERROR << android::base::StringPrintf(
                        "Ignoring failure to mount an un-encryptable or wiped "
                        "partition on %s at %s options: %s",
                        (*fstab)[attempted_idx].blk_device.c_str(),
                        (*fstab)[attempted_idx].mount_point.c_str(),
                        (*fstab)[attempted_idx].fs_options.c_str());
            } else {
                PERROR << android::base::StringPrintf(
