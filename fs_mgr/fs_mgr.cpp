...
std::string fs_mgr_get_encryption_in_progress_file_name(const std::string& mnt) {
    std::string progress_file = "in_progess_" + mnt;
    std::replace(progress_file.begin(), progress_file.end(), '/', '_');
    return "/metadata/vold/metadata_encryption/" + progress_file;
}

bool WasMetadataEncryptionInterrupted(const FstabEntry& entry) {
    if (!should_use_metadata_encryption(entry)) return false;
    struct stat buf;
    return (stat(fs_mgr_get_encryption_in_progress_file_name(entry.mount_point).c_str(), &buf) ==
            0);
}
