...
static void tune_metadata_csum(const std::string& blk_device, const FstabEntry& entry,
    ...
    // Force e2fsck is required for tune2fs to enable metadata_csum.
    if (!run_command(e2fsck_forced_argv, ARRAY_SIZE(e2fsck_forced_argv))) {
        LERROR << "Failed trying to run " << E2FSCK_BIN;
        *fs_stat |= FS_STAT_E2FSCK_FAILED;
        return;
    }
    ...
    if (!run_command(argv_t, ARRAY_SIZE(argv_t))) {
        ...
        *fs_stat |= FS_STAT_ENABLE_METADATA_CSUM_FAILED;
        ...
        if (!run_command(argv_r, ARRAY_SIZE(argv_r))) {
