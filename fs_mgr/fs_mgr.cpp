...
static void tune_quota(const std::string& blk_device, const FstabEntry& entry,
    ...
    bool want_projid =
            true;  // android::base::GetBoolProperty("external_storage.projid.enabled", false);
    LINFO << "has_quota" << has_quota;
    LINFO << "want_quota" << want_quota;
    LINFO << "want_projid" << want_projid;
