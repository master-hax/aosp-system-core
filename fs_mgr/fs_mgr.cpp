...
static void check_fs(const char *blk_device, char *fs_type, char *target, int *fs_stat)
{
    ...
    if (!strcmp(fs_type, "ext2") || !strcmp(fs_type, "ext3") || !strcmp(fs_type, "ext4")) {
        ...
        if (!(*fs_stat & FS_STAT_FULL_MOUNT_FAILED)) {  // already tried if full mount failed
            ...
            if (!ret) {
                ...
                while (true) {
                    ...
                    PERROR << __FUNCTION__ << "(): umount(" << target << ")=" << result;
                    retry++;
                    if (retry > 5) break;
                    sleep(1);
                }
        ...
        if (access(E2FSCK_BIN, X_OK)) {
            ...
            bool force_check = *fs_stat & (FS_STAT_E2FSCK_F_ALWAYS | FS_STAT_UNCLEAN_SHUTDOWN |
                                           FS_STAT_QUOTA_ENABLED | FS_STAT_TUNE2FS_FAILED |
            ...
            while (true) {
                ...
                if (!__mount(fstab->recs[i].blk_device, fstab->recs[i].mount_point,
                    ...
                    *attempted_idx = i;
                    ...
                    if (retry >= 2) break;  // run check_fs only once
                    fs_stat |= FS_STAT_FULL_MOUNT_FAILED;
