...
std::pair<int, bool> fs_mgr_mount_all(Fstab* fstab, int mount_mode) {
    ...
    for (int i = 0; i < static_cast<int>(fstab->size()); i++) {
        ...
        if (mret) {
            ...
            userdata_mounted = current_entry.mount_point == "/data";
...
int fs_mgr_remount_userdata_into_checkpointing(Fstab* fstab) {
    ...
    if (!force_umount && fstab_entry->fs_mgr_flags.checkpoint_fs) {
        ...
        return result.first == FS_MGR_MNTALL_FAIL ? -1 : 0;
    }
    return 0;
}
