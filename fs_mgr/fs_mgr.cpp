...
static void check_fs(const char *blk_device, char *fs_type, char *target, int *fs_stat)
{
    ...
    if (is_extfs(fs_type)) {
        ...
        if (access(E2FSCK_BIN, X_OK)) {
            std::string path;
            Realpath(blk_device, &path);
            LINFO << "Not running " << E2FSCK_BIN << " on " << path
                  << " (executable not in system image)";
        } else {
            std::string path;
            Realpath(blk_device, &path);
            LINFO << "Running " << E2FSCK_BIN << " on " << path;
        ...
        std::string path;
        Realpath(blk_device, &path);
        LINFO << "Running " << F2FS_FSCK_BIN << " -a " << path;
    ...
    for (i = start_idx;
         ...
         i++) {
            ...
            if (fs_stat & FS_STAT_INVALID_MAGIC) {
                std::string path;
                Realpath(fstab->recs[i].blk_device, &path);
                LERROR << __FUNCTION__ << "(): skipping mount due to invalid magic, mountpoint="
                       << fstab->recs[i].mount_point << " blk_dev=" << path << " rec[" << i
                       << "].fs_type=" << fstab->recs[i].fs_type;
...
int fs_mgr_mount_all(fstab* fstab, int mount_mode) {
    ...
    for (i = 0; i < fstab->num_entries; i++) {
        ...
        if (mret && mount_errno != EBUSY && mount_errno != EACCES &&
             ...
             */
            std::string path;
            Realpath(fstab->recs[top_idx].blk_device, &path);
            LERROR << __FUNCTION__ << "(): " << path << " is wiped and "
                   << fstab->recs[top_idx].mount_point << " " << fstab->recs[top_idx].fs_type
