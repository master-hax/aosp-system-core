...
static std::string ChildBlockDev(const std::string& blockdev) {
    ...
    const auto it = std::filesystem::directory_iterator(children_path, ec);
    if (begin(it) != end(it)) {
        return Basename(begin(it)->path().string());
    }
    return {};
}
...
static std::string PartitionParent(const std::string& blockdev) {
    ...
    std::error_code ec;
    for (const auto& entry : std::filesystem::directory_iterator("/sys/class/block", ec)) {
