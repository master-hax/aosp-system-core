...
std::string fs_mgr_get_super_partition_name() {
    ...
    if (fs_mgr_get_boot_config_from_kernel_cmdline("super_partition", &super_partition)) {
        ...
        if (slot_suffix.empty()) {
            LFATAL << "Only A/B devices can specify a retrofit super partition";
            return "";
        }
        android::fs_mgr::PartitionOpener opener;
        bool super_a = ProbeForMetadata(opener, super_partition + "_a");
        bool super_b = ProbeForMetadata(opener, super_partition + "_b");
        if (super_a == super_b) {
            LERROR << "Could not identify a super partition";
            return super_partition + fs_mgr_get_slot_suffix();
        }
        return super_a ? (super_partition + "_a") : (super_partition + "_b");
    }
