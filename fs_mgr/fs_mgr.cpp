...
int fs_mgr_mount_all(Fstab* fstab, int mount_mode) {
    ...
    for (size_t i = 0; i < fstab->size(); i++) {
        ...
        if (current_entry.fs_mgr_flags.wait &&
            !android::base::WaitForFile(current_entry.blk_device, 20s)) {
...
static int fs_mgr_do_mount_helper(Fstab* fstab, const std::string& n_name,
    ...
    for (auto& fstab_entry : *fstab) {
        ...
        if (fstab_entry.fs_mgr_flags.wait && !android::base::WaitForFile(n_blk_device, 20s)) {
...
bool fs_mgr_swapon_all(const Fstab& fstab) {
    ...
    for (const auto& entry : fstab) {
        ...
        if (entry.fs_mgr_flags.wait && !android::base::WaitForFile(entry.blk_device, 20s)) {
