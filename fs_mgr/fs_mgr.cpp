...
static void tune_reserved_size(const std::string& blk_device, const FstabEntry& entry,
    ...
    if (reserved_blocks > max_reserved_blocks) {
        LWARNING << "Reserved blocks " << reserved_blocks << " is too large; " << "capping to "
                 << max_reserved_blocks;
...
FstabEntry* LocateFormattableEntry(FstabEntry* begin, FstabEntry* const end) {
    ...
    for (; begin->blk_device == blk_device && begin < end; begin++) {
    ...
    if (f2fs_entry) {
        LOG(INFO) << "Using F2FS for " << begin->blk_device << " @ " << begin->mount_point
    ...
    for (int i = 0; i < static_cast<int>(fstab->size()); i++) {
        ...
        if (mount_errno != EBUSY && mount_errno != EACCES &&
            ...
            if (fs_mgr_do_format(*formattable_entry) == 0) {
                ...
                LERROR << __FUNCTION__ << "(): Format failed. " << "Suggest recovery...";
