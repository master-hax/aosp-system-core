...
bool fs_mgr_update_verity_state(std::function<fs_mgr_verity_state_callback> callback,
    ...
    if (fsrec == nullptr) {
        ...
        fsrec = fstab->recs;
        num_entries = fstab->num_entries;
    }
    for (; num_entries > 0; --num_entries, ++fsrec) {
        if (!fs_mgr_is_verified(fsrec) && !fs_mgr_is_avb(fsrec)) continue;
        ...
        if (!dm.GetTableStatus(mount_point, &table) || table.empty() || table[0].data.empty()) {
            if (fsrec->fs_mgr_flags & MF_VERIFYATBOOT) {
                status = "V";
            } else {
