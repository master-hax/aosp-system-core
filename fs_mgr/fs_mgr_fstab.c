        ...
        for (i = 0; fl[i].name; i++) {
            if (!strncmp(p, fl[i].name, strlen(fl[i].name))) {
                ...
                if ((fl[i].flag == MF_CRYPT) && flag_vals) {
                     ...
                     * erase block size. Get and return it.
                     */
                    flag_vals->erase_blk_size = strtoul(strchr(p, '=') + 1, NULL, 0);
                } else if ((fl[i].flag == MF_LOGICALBLKSIZE) && flag_vals) {
                    /* The logical block size flag is followed by an = and the flash
                     * logical block size. Get and return it.
                     */
                    flag_vals->logical_blk_size = strtoul(strchr(p, '=') + 1, NULL, 0);
                }
                break;
            }
        }
