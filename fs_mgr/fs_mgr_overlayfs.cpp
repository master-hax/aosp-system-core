...
bool fs_mgr_overlayfs_mount(const std::string& mount_point) {
    ...
    for (const auto& entry : fstab) {
        ...
        // Clean up original mount point, failure is possible, but not serious.
        umount2(entry.mount_point.c_str(), MNT_DETACH);
    ...
    for (const auto& [mount_point, dir] : move) {
        // bind temporary back to the mount point
        if (!fs_mgr_overlayfs_bind_mount(dir, mount_point)) {
            retval = false;
            save_errno = errno;
        }
        // Clean up temporary mount and directory
