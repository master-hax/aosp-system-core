...
bool fs_mgr_overlayfs_setup_scratch(const fstab* fstab, bool* change) {
    ...
    if (!partition_exists) {
        ...
        auto builder = MetadataBuilder::New(kSuperDevice, slot_number);
        if (!builder) {
            PERROR << "open " << kSuperDevice << " metadata";
            return false;
        }
        partition_exists = builder->FindPartition(android::base::Basename(kScratchMountPoint));
        if (!partition_exists) {
            uuid_t uuid;
            ...
            if (!CreateLogicalPartition(kSuperDevice, slot_number, partition_name, true, 0s,
                                        &scratch_device))
                return false;
        }
    }
    ...
    if (system((mnt_type == "f2fs") ? ((kMkF2fs + " -d1 " + scratch_device).c_str())
                                    : ((kMkExt4 + " -b 4096 -t ext4 -m 0 -M "s +
                                        kScratchMountPoint + " -O has_journal " + scratch_device)
                                               .c_str()))) {
        PERROR << "make " << mnt_type << " filesystem on " << scratch_device;
    } else if (change) {
        *change = true;
    }
