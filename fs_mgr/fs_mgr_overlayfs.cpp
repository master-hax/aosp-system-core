...
bool fs_mgr_overlayfs_setup_scratch(const struct fstab* fstab, bool* change) {
    if (!fs_mgr_overlayfs_scratch_can_be_mounted(fs_mgr_overlayfs_scratch_device())) return true;
    auto slot_number = fs_mgr_overlayfs_slot_number();
    auto builder = MetadataBuilder::New(kSuperDevice, slot_number);
    if (!builder) {
    ...
    if (!CreateLogicalPartition(kSuperDevice, slot_number, partition_name, true, 0s, &device_path))
        return false;
