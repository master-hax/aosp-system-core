...
void fs_mgr_overlayfs_teardown_all(const std::string&) {}
...
static void UnmapScratchDevice(const std::string& partition_name) {
    DestroyLogicalPartition(partition_name);
}
...
void fs_mgr_overlayfs_teardown_all(const std::string& mount_point) {
    ...
    if (EnsureScratchMapped(&scratch_device, &mapped)) {
        ...
        if (mapped) {
            UnmapScratchDevice(android::base::Basename(kScratchMountPoint));
        }
    }
    ...
    if (MapDsuScratchDevice(&scratch_device)) {
        ...
        UnmapScratchDevice(android::gsi::kDsuScratch);
    }
}
