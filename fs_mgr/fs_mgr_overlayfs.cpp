...
bool fs_mgr_overlayfs_already_mounted(const std::string& mount_point, bool overlay_only = true) {
    ...
    for (const auto& entry : fstab) {
        if (overlay_only && "overlay"s != entry.fs_type && "overlayfs"s != entry.fs_type) continue;
...
std::vector<std::string> fs_mgr_candidate_list(Fstab* fstab, const char* mount_point = nullptr) {
    ...
    if (std::find_if(fstab->begin(), fstab->end(),
                     [](const auto& entry) { return entry.mount_point == "/"; }) != fstab->end()) {
        return mounts;
    }
