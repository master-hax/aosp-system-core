...
bool fs_mgr_overlayfs_rw_access(const std::string& path) {
...
bool fs_mgr_overlayfs_teardown_one(const std::string& overlay, const std::string& mount_point,
    ...
    if (!mount_point.empty()) {
        ...
        if (!fs_mgr_overlayfs_rw_access(super_device)) return ret;
...
bool fs_mgr_overlayfs_mount_scratch(const fstab* fstab, const std::string& device_path,
    ...
    std::unique_ptr<struct fstab, decltype(&fs_mgr_free_fstab)> local_fstab(
            static_cast<struct fstab*>(calloc(1, sizeof(struct fstab))), fs_mgr_free_fstab);
    ...
    if (fs_mgr_do_mount_one(fsrec)) return true;
    auto save_errno = errno;
...
bool fs_mgr_overlayfs_setup_scratch(const fstab* fstab, bool* change) {
    ...
    auto partition_exists = !scratch_device.empty() && fs_mgr_overlayfs_rw_access(scratch_device);
    if (!partition_exists) {
        auto slot_number = fs_mgr_overlayfs_slot_number();
        auto super_device = fs_mgr_overlayfs_super_device(slot_number);
        if (!fs_mgr_overlayfs_rw_access(super_device)) return false;
...
bool fs_mgr_overlayfs_scratch_can_be_mounted(const std::string& scratch_device) {
    ...
    if (fs_mgr_overlayfs_rw_access(scratch_device)) return true;
    auto slot_number = fs_mgr_overlayfs_slot_number();
    auto super_device = fs_mgr_overlayfs_super_device(slot_number);
    if (!fs_mgr_overlayfs_rw_access(super_device)) return false;
    auto builder = MetadataBuilder::New(super_device, slot_number);
...
bool fs_mgr_overlayfs_setup(const char* backing, const char* mount_point, bool* change) {
    ...
    for (const auto& overlay_mount_point : kOverlayMountPoints) {
        ...
        if (overlay_mount_point == kScratchMountPoint) {
            if (!fs_mgr_overlayfs_rw_access(
                        fs_mgr_overlayfs_super_device(fs_mgr_overlayfs_slot_number())) ||
