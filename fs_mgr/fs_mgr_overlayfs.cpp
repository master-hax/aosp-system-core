...
bool fs_mgr_overlayfs_mount_all(std::vector<fstab_rec*>&) {
    return false;
}
...
std::vector<std::string> fs_mgr_overlayfs_required_devices(std::vector<fstab_rec*>&) {
    return {};
}
...
std::vector<std::string> fs_mgr_overlayfs_required_devices(std::vector<fstab_rec*>& fsrecs) {
    std::vector<fstab_rec> recs;
    for (const auto& rec : fsrecs) recs.push_back(*rec);
    fstab fstab = {static_cast<int>(fsrecs.size()), &recs[0]};
