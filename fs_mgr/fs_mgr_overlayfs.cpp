        ...
        if ((partition_size > kMinimumSize) || !partition->size()) {
            ...
            if (partition_size > partition->size()) {
                if (!builder->ResizePartition(partition, partition_size)) {
                    const auto other_slot_suffix = fs_mgr_get_other_slot_suffix();
                    // Remove <other> partitions
                    for (const auto& group : builder->ListGroups()) {
                        for (const auto& part : builder->ListPartitionsInGroup(group)) {
                            const auto& name = part->name();
                            if (!android::base::EndsWith(name, other_slot_suffix)) continue;
                            // Only resize to zero if not currently
                            // instantiated, otherwise likely in use.
                            auto& dm = DeviceMapper::Instance();
                            std::string path;
                            if (dm.GetDmDevicePathByName(name, &path)) continue;
                            builder->ResizePartition(part, 0);
                        }
                    }
                    // recalculate partition_size and try again
                    partition_size =
                            builder->AllocatableSpace() - builder->UsedSpace() + partition->size();
                    partition_size = std::max(std::min(kMinimumSize, partition_size - margin_size),
                                              partition_size / 2);
