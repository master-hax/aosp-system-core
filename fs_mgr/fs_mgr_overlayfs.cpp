...
bool fs_mgr_overlayfs_scratch_present() {
    return false;
}
    ...
    if (system((mnt_type == "f2fs") ? ((kMkF2fs + " -d1 " + scratch_device).c_str())
                                    : ((kMkExt4 + " -b 4096 -t ext4 -m 0 -M "s +
                                        kScratchMountPoint + " -O has_journal " + scratch_device)
                                               .c_str()))) {
        PERROR << "failed to make " << mnt_type << " filesystem on " << scratch_device;
    } else if (change) {
        *change = true;
    }
...
bool fs_mgr_overlayfs_mount_all(const fstab* fstab) {
    ...
    for (const auto& mount_point : fs_mgr_candidate_list(fstab)) {
        ...
        if (scratch_can_be_mounted) {
            ...
            if (fs_mgr_wait_for_file(scratch_device, 20s)) {
...
bool fs_mgr_overlayfs_scratch_present() {
    return fs_mgr_overlayfs_scratch_can_be_mounted(fs_mgr_overlayfs_scratch_device());
}
