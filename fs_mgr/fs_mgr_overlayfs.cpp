...
bool fs_mgr_overlayfs_setup_dir(const std::string& dir, std::string* overlay, bool* change) {
    auto ret = true;
    auto top_dir = dir + kOverlayTopDir;
    if (setfscreatecon(kOverlayfsFileContext)) {
        ret = false;
        PERROR << "setfscreatecon " << kOverlayfsFileContext;
    }
    auto save_errno = errno;
    if (!mkdir(top_dir.c_str(), 0755)) {
        if (change) *change = true;
    } else if (errno != EEXIST) {
        ret = false;
        PERROR << "mkdir " << top_dir;
    } else {
        errno = save_errno;
    }
    setfscreatecon(nullptr);

    if (overlay) *overlay = std::move(top_dir);
    return ret;
}
...
bool fs_mgr_overlayfs_mount_scratch(const fstab* fstab, const std::string& device_path,
    ...
    if (save_errno == ENOENT) {
        if (access(kScratchMountPoint.c_str(), F_OK)) {
            target_missing = "(missing)";
        } else if (access(device_path.c_str(), F_OK)) {
            source_missing = "(missing)";
        }
        errno = save_errno;
    }
    // hijack __mount() report format to help triage
    auto report = "__mount(source="s + device_path + source_missing +
                  ",target=" + kScratchMountPoint + target_missing + ",type=" + mnt_type + ")=";
    if (ret) {
        PERROR << report << ret;
...
bool fs_mgr_overlayfs_setup_scratch(const fstab* fstab, bool* change) {
    ...
    auto partition_exists =
            !scratch_device.empty() && access(scratch_device.c_str(), R_OK | W_OK) == 0;
    if (!partition_exists) {
...
bool fs_mgr_overlayfs_setup(const char* backing, const char* mount_point, bool* change) {
    ...
    for (const auto& overlay_mount_point : kOverlayMountPoints) {
        ...
        if (overlay_mount_point == kScratchMountPoint) {
            if (!fs_mgr_overlayfs_has_super() || !fs_mgr_overlayfs_has_logical(fstab.get())) {
                continue;
            }
            if (!fs_mgr_overlayfs_setup_scratch(fstab.get(), change)) return false;
        } else {
