...
void PrintError(const char* msg1, const char* msg2) {
    std::cerr << msg1 << ": " << msg2 << "\n";
}

void PrintErrno(const char* msg1, const char* msg2) {
...
void PrintErrorResult(const char* msg, const android::f2fs_pin::Result& result) {
    std::cerr << msg << ": " << result.error();
    std::cerr << "\n";
}
...
bool WritePattern(const char* file, int file_fd, off_t file_size) {
    ...
    for (off_t remaining = file_size; remaining > 0;) {
        ...
        if (written != n) {
            if (written < 0)
                PrintErrno("write() failed", file);
            else
                PrintError("partial write()", file);
            return false;
        }
        remaining -= written;
    }
    if (fsync(file_fd) < 0) {
        PrintError("fsync() failed", file);
        return false;
    }
    return true;
}
...
bool VerifyPattern(const char* bdev, int bdev_fd, const char* file, int file_fd, off_t file_size) {
    ...
    for (off_t offset = 0; offset < file_size;) {
        ...
        if (!result_extent_map) {
            PrintErrorResult("could not get extent map",
        ...
        if (fe->fe_logical != offset) {
            PrintError("file should not have holes", file);
            return false;
        }
        ...
        while (length > 0) {
            ...
            if (nread != n) {
                if (nread < 0)
                    PrintErrno("pread() failed", bdev);
                else
                    PrintError("partial read()", bdev);
                return false;
            }
            uint64_t* end = block + n / sizeof(uint64_t);
            for (uint64_t* p = block; p < end; ++p, ++value)
                if (*p != value) {
                    PrintError("wrong data when reading file directly from device", file);
                    return false;
                }
            physical += n;
            length -= n;
...
bool EnsurePinned(const char* bdev, const char* file, bool verify_file) {
    ...
    if (!res) {
        PrintErrorResult("FileEnsureReliablyPinned() failed", res);
        return false;
    }
...
bool CreatePinned(const char* bdev, const char* file, off_t file_size, bool init_file) {
    ...
    if (file_fd < 0) {
        PrintErrno("could not create file", file);
        return false;
    }
    std::string bdev_string(bdev);
    auto res = android::f2fs_pin::FileAllocateSpaceAndReliablyPin(file_fd, bdev_fd, bdev_string,
                                                                  file_size);
    if (!res) {
        PrintErrorResult("FileAllocateSpaceAndReliablyPin() failed", res);
        return false;
    }
    ...
    if (!res) {
        PrintErrorResult("file not reliably pinned after writing into it", res);
        return false;
    }
    return VerifyPattern(bdev, bdev_fd, file, file_fd, file_size);
}
