...
static ResultFileSize FileGetSize(int file_fd) {
    struct stat st;
    if (fstat(file_fd, &st) < 0) return ErrnoErrorf("fstat() failed");
...
static ResultDeviceNumber FileGetFileSystemDeviceNumber(int file_fd) {
    ...
    if (statx(file_fd, "", AT_EMPTY_PATH, STATX_BASIC_STATS, &stx) < 0)
        return ErrnoErrorf("statx() failed");

    if ((stx.stx_mode & S_IFMT) != S_IFREG) return Errorf("file_fd not a regular device");
...
static ResultDeviceNumber BdevGetDeviceNumber(int bdev_fd) {
    ...
    if ((stx.stx_mode & S_IFMT) != S_IFBLK) return Errorf("bdev_fd not a block device");
...
static ResultDeviceNumber BdevNameGetDeviceNumber(const std::string& bdev_name) {
    ...
    if (statx(-1, bdev_name.c_str(), 0, STATX_BASIC_STATS, &stx) < 0)
        return ErrnoErrorf("statx() failed: {}", bdev_name);

    if ((stx.stx_mode & S_IFMT) != S_IFBLK)
        return Errorf("bdev_name not a block device: {}", bdev_name);
...
static ResultDeviceNumber FileGetFileSystemDeviceNumber(int file_fd) {
    struct stat st;
    if (fstat(file_fd, &st) < 0) return ErrnoErrorf("stat() failed");

    if ((st.st_mode & S_IFMT) != S_IFREG) return Errorf("file_fd not a regular device");
...
static ResultDeviceNumber BdevGetDeviceNumber(int bdev_fd) {
    struct stat st;
    if (fstat(bdev_fd, &st) < 0) return ErrnoErrorf("stat() failed");

    if ((st.st_mode & S_IFMT) != S_IFBLK) return Errorf("bdev_fd not a block device");
...
static ResultDeviceNumber BdevNameGetDeviceNumber(const std::string& bdev_name) {
    struct stat st;
    if (stat(bdev_name.c_str(), &st) < 0) return ErrnoErrorf("fstat() failed: {}", bdev_name);
...
static Result FileOnFileSystemOnBdev(int file_fd, int bdev_fd) {
    ...
    if (!bdev_device_number.Equals(file_system_device_number))
        return Errorf("file_fd not on file system on bdev_fd");
...
static Result BdevNameDeviceNumberSameAsBdev(const std::string& bdev_name, int bdev_fd) {
    ...
    if (!bdev_device_number.Equals(bdev_name_device_number))
        return Errorf("device numbers different between bdev_name and bdev_fd: {}", bdev_name);
...
static ResultMainBlkaddrOffset BdevGetMainBlkaddrOffset(const std::string& bdev_name) {
    ...
    if (length == 0 || bdev_name[0] != '/' || index == std::string::npos || index + 1 == length)
        return Errorf("bdev_name is not absolute: {}", bdev_name);
    ...
    if (fd < 0) return ErrnoErrorf("open() failed: {}", main_blkaddr_file);
    ssize_t nread = read(fd, buf, sizeof(buf) - 1);

    if (nread < 0) return ErrnoErrorf("read() failed");

    buf[nread] = 0;
    errno = 0;  // to use strtoull(3) errno has to be cleared first
    unsigned long long llsz = strtoull(buf, nullptr, 0);
    if (errno) return ErrnoErrorf("invalid main_blkaddr: {}", main_blkaddr_file);
...
static Result BdevHasPinFileFeature(const std::string& bdev_name) {
    ...
    if (fd < 0) return ErrnoErrorf("open() failed: {}", features_file);
    ...
    if (file_size > 1024 * 1024)
        return Errorf("size of features file is too large: {}", features_file);

    char* bp = new char[file_size + 1];
    if (bp == NULL) Errorf("could not allocate buffer to read f2fs bdev features");
    ...
    if (nread < file_size) {
        ...
        switch (read(fd, bp + nread, (size_t)(file_size - nread))) {
            case 0:
                break;
            case -1:
                return ErrnoErrorf("second read reading f2fs bdev features failed");
            default:
                return Errorf("partial read reading f2fs bdev features");
        }
        file_size = nread;
    }
    bp[file_size] = 0;
    ...
    return Errorf("pin_file feature not supported by bdev");
}

static Result FileOnF2fsFileSystem(int file_fd) {
    struct statfs sfs;
    if (fstatfs(file_fd, &sfs) < 0) return ErrnoErrorf("fstafs() failed");

    if (sfs.f_type != F2FS_SUPER_MAGIC) return Errorf("fstafs() not a F2FS fs");

    if (sfs.f_bsize != kF2fsBlockSize) return Errorf("fstafs() invalid f_bsize");
...
static Result FilePin(int file_fd) {
    ...
    if (ioctl(file_fd, F2FS_IOC_SET_PIN_FILE, &set) < 0)
        return ErrnoErrorf("ioctl() to pin file failed");
...
static Result FileEnsureItsPinned(int file_fd) {
    ...
    if (ioctl(file_fd, FS_IOC_GETFLAGS, &flags) < 0)
        return ErrnoErrorf("ioctl() F2FS_IOC_GETFLAGS");

    if (!(flags & FS_NOCOW_FL)) return Errorf("expected file to be pinned, it is not pinned");

    return {};
}

static Result FileAllocateAndFsync(int file_fd, off_t size) {
    if (fallocate(file_fd, 0, (off_t)0, size) < 0) return ErrnoErrorf("fallocate() failed");

    if (fsync(file_fd) < 0) return ErrnoErrorf("fsync() failed");

    return {};
}

static Result FileTruncateAndFsync(int file_fd) {
    if (ftruncate(file_fd, (off_t)0) < 0) return ErrnoErrorf("ftruncate() failed");
...
ResultExtentMap FileGetExtentMap(int file_fd, off_t offset, off_t length) {
    ...
    if (ioctl(file_fd, FS_IOC_FIEMAP, &em.em_fiemap) < 0)
        return ErrnoErrorf("ioctl() FS_IOC_FIEMAP failed");
...
Result FiemapExtentValidate(const FiemapExtent& fe) {
    if (fe.fe_logical > INT64_MAX || fe.fe_physical > INT64_MAX || fe.fe_length > INT64_MAX ||
        fe.fe_length == 0)
        return Errorf("invalid extent");

    if (fe.fe_flags & kEntangledDataExtentFlags)
        return Errorf("extent flags improper for direct I/O from device");

    if (fe.fe_flags & ~kAllFiemapExtentFlags) return Errorf("unknown extent flags");
...
static Result FileVerifyItsReliablyPinned(int file_fd, off_t main_blkaddr_offset) {
    ...
    if (ioctl(file_fd, F2FS_IOC_GET_PIN_FILE, &attempts) < 0)
        return ErrnoErrorf("ioctl() get internal F2FS file unpinning choice failed");

    if (attempts != 0) return Errorf("F2FS will eventually unpin this file");

    auto result_file_size = FileGetSize(file_fd);
    if (!result_file_size) return result_file_size.error();

    off_t file_size = result_file_size.value();
    if (file_size % kF2fsSegmentSize) return Errorf("file size is not a multiple of 2MB");
    ...
    for (off_t offset = 0; offset < file_size;) {
        ...
        if (fe->fe_logical != offset) return Errorf("file should not have holes");

        if (fe->fe_length > leftover)
            return Errorf("file should not have storage past end of file");

        if (fe->fe_physical < main_blkaddr_offset)
            return Errorf("file storage should not be prior to main_blkaddr");

        if ((fe->fe_physical - main_blkaddr_offset) % kF2fsSegmentSize)
            return Errorf("extent is not 2MB aligned with respect to main_blkaddr");

        if (fe->fe_length % kF2fsSegmentSize) return Errorf("extent space is not multiple of 2MB");
...
Result FileAllocateSpaceAndReliablyPin(int file_fd, int bdev_fd, const std::string& bdev_name,
    ...
    if (size % kF2fsSegmentSize) return Errorf("size not a multiple of 2MB");

    auto result_file_size = FileGetSize(file_fd);
    if (!result_file_size) return result_file_size.error();

    off_t file_size = result_file_size.value();
    if (file_size != 0) return Errorf("file size is not zero");
...
Result FileEnsureReliablyPinned(int file_fd, int bdev_fd, const std::string& bdev_name) {
    ...
    if (fsync(file_fd) < 0) return ErrnoErrorf("fsync() failed: {}", bdev_name);
