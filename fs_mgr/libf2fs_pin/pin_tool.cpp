...
bool CommandEnsure(const char* cmd, const char* bdev, const char* file, bool verify_file) {
    ...
    if (!result) LOG(ERROR) << cmd << ": EnsurePinned() failed\n";
    return result;
}
...
bool CommandCreate(const char* cmd, const char* bdev, const char* file, off_t file_size,
    ...
    if (!result) LOG(ERROR) << cmd << ": CreatePinned() failed\n";
    return result;
}
...
bool CommandSupport(const char* cmd, const char* bdev) {
    ...
    if (!result) {
        LOG(ERROR) << cmd << ": reliable pinning not supported: " << result.error();
        return false;
    }
    return true;
}
...
int main(int argc, char* argv[]) {
    ...
    if (argc == 5) {
        ...
        if (!StrToSize(argv[4], &file_size)) {
            LOG(ERROR) << cmd << ": invalid file size value: " << argv[4];
            UsageAndExit(cmd);
        }
