...
bool EnsureExpectingErrors(const char* bdev, const char* file) {
    ...
    if (result) {
        LOG(ERROR) << "expected FileEnsureReliablyPinned() to fail, it did not: " << file;
        (void)unlink(file);
        return false;
    }
    return true;
...
bool WriteOneByte(const char* file, off_t offset) {
    ...
    if (pwrite(file_fd, "x", 1, offset) != 1) {
        PLOG(ERROR) << "could not write one byte: " << file;
        (void)unlink(file);
        return false;
    }
    return true;
...
bool PunchHoleAndFsync(const char* file, off_t offset, off_t length) {
    ...
    if (fallocate(file_fd, FALLOC_FL_PUNCH_HOLE | FALLOC_FL_KEEP_SIZE, offset, length) < 0) {
        PLOG(ERROR) << "could not punch hole in file: " << file;
        (void)unlink(file);
        return false;
    }
    if (fsync(file_fd) == -1) {
        PLOG(ERROR) << "could not fsync file: " << file;
        (void)unlink(file);
        return false;
    }
    return true;
...
class Args {
  public:
    Args() {
        ...
        if (chdir(cdir) < 0) {
            PLOG(ERROR) << "chdir() failed: " << cdir;
        }
        struct statfs sfs;
        if (statfs(".", &sfs) < 0) {
            PLOG(ERROR) << "statfs() failed: " << cdir;
            return;
        }
        if (sfs.f_type != F2FS_SUPER_MAGIC) {
            LOG(ERROR) << "stafs() not a F2FS fs: " << cdir;
            return;
        }
        if (sfs.f_bsize != kF2fsBlockSize) {
            LOG(ERROR) << "stafs() invalid f_bsize: " << cdir;
            return;
        }
    ...
    const char* bdev() const { return bdev_.c_str(); }
    const char* file() const { return "pin_test_file.data"; }
