...
static std::string construct_verity_table(const AvbHashtreeDescriptor& hashtree_desc,
    ...
    std::string kernel_verity_mode;
    if (verity_mode == "enforcing") {
        kernel_verity_mode = "restart_on_corruption";
    } else if (verity_mode == "logging") {
        kernel_verity_mode = "ignore_corruption";
    } else if (verity_mode != "eio") {  // Default kernel_verity_mode is eio.
    ...
    if (!kernel_verity_mode.empty()) {
        optional_argc += 1;
        optional_args << kernel_verity_mode << " ";
    }
...
static bool load_verity_table(struct dm_ioctl* io, const std::string& dm_device_name, int fd,
    ...
    int res = snprintf(verity_params, bufsize, "%s", verity_table.c_str());
