...
fs_mgr_avb_handle* fs_mgr_avb_open(struct fstab* fstab, int* out_result) {
    FS_MGR_CHECK(fstab != nullptr);
    FS_MGR_CHECK(out_result != nullptr);

    *out_result = FS_MGR_SETUP_AVB_FAIL;
    ...
    if (!verify_vbmeta_images(*h->avb_slot_verify_data, fs_mgr_vbmeta_prop)) {
        ...
        if (hashtree_disabled) {
            *out_result = FS_MGR_SETUP_AVB_HASHTREE_DISABLED;
            return nullptr;
        }
    }

    if (verify_result == AVB_SLOT_VERIFY_RESULT_OK) {
        *out_result = FS_MGR_SETUP_AVB_SUCCESS;
...
int fs_mgr_setup_avb(struct fs_mgr_avb_handle* handle, struct fstab_rec* fstab_entry) {
    if (!handle || !fstab_entry || !handle->avb_slot_verify_data ||
