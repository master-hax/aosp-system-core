...
static bool load_verity_table(struct dm_ioctl* io, const std::string& dm_device_name, int fd,
    ...
    // snprintf allows writing at most bufsize bytes (including the terminating null byte).
    ...
    // ret is the number of characters printed (excluding the terminating null byte).
    int ret = snprintf(verity_params, bufsize, "%s", verity_table.c_str());
    if (ret < 0 || static_cast<size_t>(ret) != verity_table.size()) {
        PERROR << "Error writing verity_params, ret: " << ret
               << " (expected: " << verity_table.size() << ")";
        return false;
    }
