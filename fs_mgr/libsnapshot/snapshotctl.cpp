...
static bool GetBlockHashFromMerkelTree(int image_fd, uint64_t image_size, uint32_t data_block_size,
                                       uint32_t hash_block_size, uint64_t tree_offset,
                                       std::vector<std::vector<uint8_t>>& out_block_hash) {
    ...
    std::vector<uint8_t> hash_block(hash_block_size);
    ...
    while ((t_read_blocks < num_hash_blocks) && (blockidx < data_block_count)) {
        uint64_t ret = TEMP_FAILURE_RETRY(
                pread64(image_fd, hash_block.data(), hash_block.size(), block_offset));
        if (ret != hash_block.size()) {
        ...
        for (uint32_t offset = 0; offset < hash_block.size(); offset += padded_digest_size) {
            std::vector<uint8_t> single_hash(padded_digest_size, 0);
            std::memcpy(single_hash.data(), hash_block.data() + offset, padded_digest_size);
...
bool verify_data_blocks(int fd, const std::vector<std::vector<uint8_t>>& block_hash,
    ...
    std::vector<char> hash_block(descriptor->hash_block_size);

    while (blk < data_block_count) {
        int ret = pread64(fd, hash_block.data(), descriptor->hash_block_size, foffset);
        if (ret < 0) {
        ...
        CalculateDigest(hash_block.data(), descriptor->hash_block_size, salt.data(), salt.size(),
                        digest.data());
        if (memcmp(digest.data(), block_hash[blk].data(), 32) != 0) {
            LOG(ERROR) << "Hash mismatch for block: " << blk;
            return false;
        }
...
bool DumpVerityHash(int argc, char** argv) {
    ...
    std::string hash_file_path = std::string(argv[2]);
    bool metadata_on_super = false;
    if (argc == 4) {
        if (std::string(argv[3]) == "-verify") {
    ...
    for (const auto& pair : dm_block_devices) {
        ...
        std::vector<std::vector<uint8_t>> block_hash;
        ...
        bool ok = android::snapshot::HexToBytes(salt_str, &salt);
        if (!ok) {
        ...
        while (file_offset < dev_sz) {
            int ret = pread64(fd, hash_block.data(), descriptor->hash_block_size, file_offset);
            if (ret < 0) {
                LOG(ERROR) << "Failed to read from offset: " << file_offset;
                return false;
            }
            std::vector<uint8_t> digest(32, 0);
            CalculateDigest(hash_block.data(), descriptor->hash_block_size, salt.data(),
                            salt.size(), digest.data());
        ...
        LOG(INFO) << "GetBlockHashFromMerkelTree success: " << block_hash.size();
        LOG(INFO) << "=================================";
    }
    return true;
}
