...
bool MapSnapshots::ApplyUpdate() {
    ...
    dap->set_vabc_compression_param("zstd");
    dap->set_cow_version(3);
    dap->set_compression_factor(65536);

    auto group = dap->add_groups();
    group->set_name("google_dynamic_partitions");
    ...
    for (auto& patchfile : patchfiles) {
        ...
        pu->mutable_new_partition_info()->set_size(header.op_count_max * header.block_size);
    }
    if (!sm_->BeginUpdate()) {
