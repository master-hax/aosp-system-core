...
bool MergeCmdHandler(int /*argc*/, char** /*argv*/) {
    ...
    if (!sm->InitiateMerge()) {
        std::cerr << "Warning: Failed to initiate merge. It might have been initiated."
                  << std::endl;
    }

    // If merge has already been initiated, |initiated| == false. In any case,
    // Call ProcessUpdateState to wait for merge to complete.
    ...
    if (state == UpdateState::None || state == UpdateState::MergeCompleted) {
        if (state == UpdateState::None) {
            message << "No snapshot to merge.";
        } else {
            message << "Snapshot merged in " << passed << " ms.";
        }
        std::cout << message.rdbuf() << std::endl;
        LOG(INFO) << message.rdbuf();
        return true;
    }

    message << "Snapshot failed to merge with state " << state << ".";
    std::cerr << "Error: " << message.rdbuf() << std::endl;
    LOG(ERROR) << message.rdbuf();
    return false;
}
