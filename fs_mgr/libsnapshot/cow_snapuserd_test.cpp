...
TEST_F(SnapuserdTest, ReadWrite) {
    ...
    memset(random_buffer_2.get(), 0, size);

    system_buffer = std::make_unique<uint8_t[]>(size);
    memset(system_buffer.get(), 0, size);

    // Fill random data
    for (size_t j = 0; j < (size / MB(1)); j++) {
        ret = pread(rnd_fd, (char*)random_buffer_1.get() + offset, MB(1), 0);
        ASSERT_EQ(ret, MB(1));

        ret = pread(rnd_fd, (char*)random_buffer_2.get() + offset, MB(1), 0);
        ASSERT_EQ(ret, MB(1));

        offset += MB(1);
    }

    int sys_fd = open("/dev/block/mapper/system_a", O_RDONLY);
    ASSERT_TRUE(sys_fd > 0);

    // Read from system partition from offset 0 of size 100MB
    ret = pread(sys_fd, system_buffer.get(), size, 0);
    ASSERT_EQ(ret, size);
    close(sys_fd);
    close(rnd_fd);
    ...
    system("dmctl create system_cow user 0 1446832");
    ...
    system("dmctl create system-snapshot -ro snapshot 0 1446832 /dev/block/mapper/system_a /dev/block/mapper/system_cow P 8");
    std::unique_ptr<uint8_t[]> snapuserd_buffer;

    snapuserd_buffer = std::make_unique<uint8_t[]>(size);
    memset(snapuserd_buffer.get(), 0, size);

    offset = 0;

    int snapshot_fd = open("/dev/block/mapper/system-snapshot", O_RDONLY);
    ...
    ret = pread(snapshot_fd, snapuserd_buffer.get(), size, offset);
    ASSERT_EQ(ret, size);
    ...
    ret = memcmp(snapuserd_buffer.get(), random_buffer_1.get(), size);
    ASSERT_EQ(ret, 0);
    ...
    ret = memcmp(snapuserd_buffer.get(), system_buffer.get(), size);
    ASSERT_EQ(ret, 0);
    ...
    ret = memcmp(snapuserd_buffer.get(), random_buffer_2.get(), size);
    ASSERT_EQ(ret, 0);
