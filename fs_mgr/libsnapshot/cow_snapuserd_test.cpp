...
class SnapuserdTest : public ::testing::Test {
  protected:
    void SetUp() override {
        // TODO: We create loopback device to simulate system_a/product_a
        // partitions. Reading from loopback device results in selinux denial.
        // Selinux policy should be added for this test. For now, set
        // permissive to 0 and re-enable it back once the test is done.
        system("setenforce 0");
        ...
        system_a_ = std::make_unique<TemporaryFile>();
        ASSERT_GE(system_a_->fd, 0) << strerror(errno);

        product_a_ = std::make_unique<TemporaryFile>();
...
TEST_F(SnapuserdTest, ReadWrite) {
    ...
    std::vector<std::vector<std::string>> vec{
            {cow_system_1_->path, system_a_loop_->device(), system_device_ctrl_name_},
            {cow_product_1_->path, product_a_loop_->device(), product_device_ctrl_name_}};
