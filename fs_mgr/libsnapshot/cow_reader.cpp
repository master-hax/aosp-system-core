...
std::unique_ptr<ICowOpIter> CowReader::GetRevMergeOpIter(bool debug) {
    return std::make_unique<CowRevMergeOpIter>(ops_, merge_op_blocks_, block_map_,
                                               debug ? 0 : merge_op_start_);
}

std::unique_ptr<ICowOpIter> CowReader::GetMergeOpIter(bool debug) {
