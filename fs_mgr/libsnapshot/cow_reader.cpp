...
bool CowReader::PrepMergeOps() {
    ...
    for (int i = 0; i < ops_->size(); i++) {
        ...
        if (!has_seq_ops_ && IsOrderedOp(current_op)) {
            merge_op_blocks.get()->emplace_back(current_op.new_block);
        } else if (seq_ops_set.count(current_op.new_block) == 0) {
            other_ops.insert(current_op.new_block);
        }
        block_map.get()->insert({current_op.new_block, i});
    }
    if (merge_op_blocks.get()->size() > header_.num_merge_ops)
        num_ordered_ops_to_merge_ = merge_op_blocks.get()->size() - header_.num_merge_ops;
    else
        num_ordered_ops_to_merge_ = 0;
    merge_op_blocks.get()->reserve(merge_op_blocks.get()->size() + other_ops.size());
    for (auto block : other_ops) {
        merge_op_blocks.get()->emplace_back(block);
    }

    num_total_data_ops_ = merge_op_blocks->size();
    if (header_.num_merge_ops > 0) {
        merge_op_blocks->erase(merge_op_blocks.get()->begin(),
                               merge_op_blocks.get()->begin() + header_.num_merge_ops);
    }
...
CowRevMergeOpIter::CowRevMergeOpIter(std::shared_ptr<std::vector<CowOperation>> ops,
    ...
    block_riter_ = merge_op_blocks.get()->rbegin();
}

bool CowRevMergeOpIter::Done() {
    return block_riter_ == merge_op_blocks_.get()->rend();
}
...
const CowOperation& CowRevMergeOpIter::Get() {
    CHECK(!Done());
    return ops_->data()[map_.get()->at(*block_riter_)];
}
