...
std::unique_ptr<CowReader> CowReader::New(android::base::unique_fd&& owned_fd) {
    android::base::borrowed_fd fd = owned_fd;
    auto reader = std::unique_ptr<CowReader>(new CowReader(std::move(owned_fd), fd));
    if (!reader->Parse()) {
        return nullptr;
    }
    return reader;
...
std::unique_ptr<CowReader> CowReader::New(android::base::borrowed_fd fd) {
    auto reader = std::unique_ptr<CowReader>(new CowReader({}, fd));
    if (!reader->Parse()) {
        return nullptr;
    }
    return reader;
...
CowReader::CowReader(android::base::unique_fd&& owned_fd, android::base::borrowed_fd fd)
    : owned_fd_(std::move(owned_fd)), fd_(fd) {}
...
bool CowReader::Parse() {
    auto pos = lseek(fd_.get(), SEEK_END, 0);
    if (pos < 0) {
...
bool CowReader::GetRawBytes(uint64_t offset, void* buffer, size_t len) {
    ...
    if (lseek(fd_.get(), SEEK_SET, offset) < 0) {
