...
std::unique_ptr<AutoMountDevice> AutoMountDevice::New(const std::string& path) {
    Fstab fstab;
    ...
    std::unique_ptr<AutoMountDevice> device{new AutoMountDevice(path, std::move(fstab))};
    return device;
}

AutoMountDevice::~AutoMountDevice() {
    if (name_.empty()) return;
