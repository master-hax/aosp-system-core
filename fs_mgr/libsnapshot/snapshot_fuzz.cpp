...
#define SNAPSHOT_FUZZ_FUNCTION(FunctionName, ArgMessageType, args) \
    FUZZ_FUNCTION(SnapshotManagerAction, FunctionName, snapshot, ArgMessageType, args)

SNAPSHOT_FUZZ_FUNCTION(FinishedSnapshotWrites, bool, wipe) {
    (void)snapshot->FinishedSnapshotWrites(wipe);
}

SNAPSHOT_FUZZ_FUNCTION(ProcessUpdateState, ProcessUpdateStateArgs, args) {
...
SNAPSHOT_FUZZ_FUNCTION(Dump, void, ignored) {
    std::stringstream ss;
    (void)snapshot->Dump(ss);
}

SNAPSHOT_FUZZ_FUNCTION(CreateUpdateSnapshots, DeltaArchiveManifest, manifest) {
    (void)snapshot->CreateUpdateSnapshots(manifest);
}

SNAPSHOT_FUZZ_FUNCTION(UnmapUpdateSnapshot, std::string, name) {
    (void)snapshot->UnmapUpdateSnapshot(name);
}

SNAPSHOT_FUZZ_FUNCTION(CreateLogicalAndSnapshotPartitions, CreateLogicalAndSnapshotPartitionsArgs,
                       args) {
...
SNAPSHOT_FUZZ_FUNCTION(RecoveryCreateSnapshotDevicesWithMetadata, RecoveryCreateSnapshotDevicesArgs,
                       args) {
...
DEFINE_PROTO_FUZZER(const SnapshotFuzzData& snapshot_fuzz_data) {
    ...
    auto snapshot_manager = env->CreateSnapshotManager(snapshot_fuzz_data);
