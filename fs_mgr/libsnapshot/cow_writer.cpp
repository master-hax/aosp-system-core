...
void CowWriter::SetupHeaders() {
    ...
    header_.cluster_size = options_.cluster_size;
    footer_ = {};
...
bool CowWriter::ParseOptions() {
    ...
    if (options_.cluster_size > 0 && options_.cluster_size < 2 * sizeof(CowOperation)) {
        LOG(ERROR) << "Cluster size must be large enough for two operations to function.";
        return false;
    }
    if (options_.cluster_size % sizeof(CowOperation) != 0) {
...
void CowWriter::InitPos() {
    ...
    if (header_.cluster_size)
        next_data_pos_ = next_op_pos_ + header_.cluster_size;
    else
...
bool CowWriter::OpenForAppend(std::optional<uint64_t> label) {
    ...
    options_.cluster_size = header_.cluster_size;
    ...
    while (!iter->Done() && !found_label) {
        ...
        if (label.has_value()) {
            ...
            if (incomplete) {
                ...
                // We don't care about kCowClusterOp if it goes straight to footer
...
bool CowWriter::EmitCluster() {
    ...
    op.source = current_data_size_ + options_.cluster_size - current_cluster_size_ -
                sizeof(CowOperation);
    return WriteOperation(op);
}
...
bool CowWriter::Finalize() {
    ...
    if (options_.cluster_size && current_data_size_ > 0) {
        EmitCluster();
        extra_cluster = true;
    }
...
bool CowWriter::WriteOperation(const CowOperation& op, const void* data, size_t size) {
    ...
    if (options_.cluster_size && op.type != kCowClusterOp &&
        options_.cluster_size < current_cluster_size_ + 2 * sizeof(op)) {
...
void CowWriter::AddOperation(const CowOperation& op) {
    ...
    if (op.type == kCowClusterOp) {
        current_cluster_size_ = 0;
        current_data_size_ = 0;
    } else if (header_.cluster_size) {
        current_cluster_size_ += sizeof(op);
        current_data_size_ += op.data_length;
    }

    next_data_pos_ += op.data_length + GetNextDataOffset(op, header_.cluster_size);
    next_op_pos_ += sizeof(CowOperation) + GetNextOpOffset(op, header_.cluster_size);
