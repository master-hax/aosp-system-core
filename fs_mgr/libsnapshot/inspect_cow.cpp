...
static void ShowBad(CowReader& reader, const struct CowOperation& op) {
    size_t count;
    void* buffer = malloc(op.data_length);

    if (!buffer) {
        std::cerr << "Failed to alloc buffer!\n";
        return;
    }

    if (!reader.GetRawBytes(op.source, buffer, op.data_length, &count)) {
        ...
        if (op.data_length >= sizeof(CowOperation))
            std::cout << "The start, as an op, would be " << *(CowOperation*)buffer << "\n";
    }
