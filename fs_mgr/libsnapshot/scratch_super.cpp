...
static bool UmountScratch() {
    ...
    if (!std::filesystem::remove(kOtaMetadataMount, ec)) {
        LOG(ERROR) << "Failed to remove kOtaMetadataMount directory: " << ec.message();
        return false;
    }
...
bool SnapshotManager::CleanupMetadataScratch(const IDeviceInfo* info) {
    if (!UmountScratch()) {
        return false;
    }
    ...
    for (auto slot : slots) {
        ...
        if (builder->FindPartition(partition_name) != nullptr) {
            ...
            if (DestroyLogicalPartition(partition_name)) {
                LOG(INFO) << "CleanupMetadataScratch success for slot: " << slot;
            }
        }
    }
...
bool SnapshotManager::CreateDynamicScratch(const IDeviceInfo* info, std::string* scratch_device) {
    const auto partition_name = android::base::Basename(kOtaMetadataMount);
...
std::string GetOtaMetadataPartition() {
    std::string device;
    auto& dm = DeviceMapper::Instance();
    auto partition_name = android::base::Basename(kOtaMetadataMount);
...
std::string MapOtaMetadataPartition(const std::string& scratch_device) {
...
// will be created.
bool SnapshotManager::CreateOtaMetadataOnSuper(const IDeviceInfo* info) {
