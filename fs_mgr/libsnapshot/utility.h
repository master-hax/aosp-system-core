...
struct AutoMountDevice : AutoDevice {
    static std::unique_ptr<AutoMountDevice> New(const std::string& path);
    ~AutoMountDevice();

  private:
    AutoMountDevice(const std::string& path, android::fs_mgr::Fstab&& fstab)
