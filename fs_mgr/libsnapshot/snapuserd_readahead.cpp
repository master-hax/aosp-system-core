...
bool ReadAheadThread::ReconstructDataFromCow() {
    ...
    loff_t start_data_offset = GetBufferDataOffset();
    int num_ops = 0;
    int total_blocks_merged = 0;

    while (true) {
        struct Buffer_Metadata* bm = reinterpret_cast<struct Buffer_Metadata*>(
        ...
        metadata_offset += sizeof(struct Buffer_Metadata);
    }
...
bool ReadAheadThread::ReadAheadIOStart() {
    ...
    int num_ops = ((GetBufferDataSize()) / BLOCK_SZ);
    uint64_t source_block;
    loff_t metadata_offset = 0;

    struct Buffer_Metadata* bm =
            reinterpret_cast<struct Buffer_Metadata*>((char*)metadata_buffer_ + metadata_offset);
    ...
    loff_t offset = 0;
    loff_t file_offset = GetBufferDataOffset();
    int total_blocks_merged = 0;
    ...
    while (true) {
        ...
        while (linear_blocks) {
            ...
            bm = reinterpret_cast<struct Buffer_Metadata*>((char*)metadata_buffer_ +
                                                           metadata_offset);
        ...
        bm = reinterpret_cast<struct Buffer_Metadata*>((char*)metadata_buffer_ + metadata_offset);
        bm->new_block = 0;
        bm->file_offset = 0;

        buffer_offset += io_size;
        CHECK(offset == buffer_offset);
        CHECK((file_offset - GetBufferDataOffset()) == offset);
    }
...
bool ReadAheadThread::RunThread() {
    ...
    while (!IterDone()) {
        ...
        if (status && !snapuserd_->CommitMerge(snapuserd_->GetTotal_Ra_Blocks_Merged())) {
            return false;
        }
...
void ReadAheadThread::InitializeBuffer() {
    ...
    metadata_buffer_ = static_cast<void*>((char*)mapped_addr + GetMetadataOffset());
    read_ahead_buffer_ = static_cast<void*>((char*)mapped_addr + GetBufferDataOffset());
}
