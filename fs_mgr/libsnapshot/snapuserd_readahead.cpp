...
void ReadAheadThread::CheckOverlap(const CowOperation* cow_op) {
    ...
    if (dest_blocks_.count(cow_op->new_block) || source_blocks_.count(source_offset) ||
        (source_offset > 0 && source_blocks_.count(source_block + 1))) {
        overlap_ = true;
    }

    dest_blocks_.insert(source_offset);
    if (source_offset > 0) {
        dest_blocks_.insert(source_offset + 1);
    }
...
bool ReadAheadThread::ReadAheadIOStart() {
    ...
    while (true) {
        ...
        source_offset = source_offset + (1 - linear_blocks) * BLOCK_SZ;
