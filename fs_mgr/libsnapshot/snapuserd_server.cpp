...
int SnapuserdServer::Sendmsg(int fd, char* msg, size_t size) {
    ...
    if (ret < 0) {
        LOG(ERROR) << "Snapuserd:server: send() failed";
        return -1;
    }
    ...
    *ret = select(fd + 1, &set, NULL, NULL, &tv);
    if (*ret == -1) {  // select failed
        ...
        *ret = TEMP_FAILURE_RETRY(recv(fd, msg, MAX_PACKET_SIZE, 0));
        if (*ret < 0) {
            LOG(ERROR) << "Snapuserd:server: recv failed";
            return {};
        } else if (*ret == 0) {
...
int SnapuserdServer::Start(std::string socketname) {
    ...
    sockfd_ =
            socket_local_server(socketname.c_str(), ANDROID_SOCKET_NAMESPACE_RESERVED, SOCK_STREAM);
    if (sockfd_ < 0) {
        LOG(ERROR) << "Failed to create server socket " << socketname;
        return -1;
    }
