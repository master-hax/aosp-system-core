...
bool CowReader::Parse(android::base::borrowed_fd fd, std::optional<uint64_t> label) {
    ...
    if (!parser->Parse(fd, header_v2, label)) {
        return false;
    }
    ...
    if (auto parserv2 = parser->AsParserV2()) {
        ops_ = std::make_shared<std::vector<CowOperation>>(parserv2->ops()->size());

        // Translate the operation buffer from on disk to in memory
        for (size_t i = 0; i < parserv2->ops()->size(); i++) {
            const auto& v2_op = parserv2->ops()->at(i);
