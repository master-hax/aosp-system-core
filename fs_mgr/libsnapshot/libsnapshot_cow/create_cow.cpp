...
class CreateSnapshot {
  ...
  private:
    ...
    bool createSnapshotPatch_ = false;
...
bool CreateSnapshot::PrepareParse(std::string& parsing_file, const bool createSnapshot) {
    ...
    createSnapshotPatch_ = createSnapshot;
    ...
    if (createSnapshot) {
        ...
        writer_ = CreateCowWriter(android::snapshot::kCowVersionManifest, options, std::move(fd_));
    }
    return true;
}
...
bool CreateSnapshot::ReadBlocks(off_t offset, const int skip_blocks, const uint64_t dev_sz) {
    ...
    while (true) {
        ...
        while (num_blocks) {
            ...
            if (createSnapshotPatch_ && !WriteSnapshot(bufptr, blkindex, hash)) {
...
bool CreateSnapshot::ParsePartition() {
    ...
    if (ret && createSnapshotPatch_ && !writer_->Finalize()) {
