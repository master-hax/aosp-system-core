...
uint8_t CreateSnapshot::HexDigitToByte(char c) {
    if (c >= '0' && c <= '9') {
        return c - '0';
    }
    if (c >= 'a' && c <= 'f') {
...
bool CreateSnapshot::HexToBytes(const std::string& hex, std::vector<uint8_t>* bytes) {
    if (hex.size() % 2 != 0) {
