...
void CowWriterV2::InitWorkers() {
    ...
    for (int i = 0; i < num_compress_threads_; i++) {
        std::unique_ptr<ICompressor> compressor =
                ICompressor::FromString(compression_.algorithm, compression_.compression_level);
        auto wt = std::make_unique<CompressWorker>(compressor, header_.block_size);
...
bool CowWriterV2::CompressBlocks(size_t num_blocks, const void* data) {
    ...
    if (num_threads <= 1) {
        std::unique_ptr<ICompressor> compressor =
                ICompressor::FromString(compression_.algorithm, compression_.compression_level);
        return CompressWorker::CompressBlocks(compressor, options_.block_size, data, num_blocks,
                                              &compressed_buf_);
    }
            ...
            if (compression_.algorithm) {
                auto data = [&, this]() {
                    if (num_compress_threads_ > 1) {
                        ...
                        std::unique_ptr<ICompressor> compressor = ICompressor::FromString(
                                compression_.algorithm, compression_.compression_level);
                        auto data = CompressWorker::Compress(compressor, iter, header_.block_size);
                        return data;
                    }
                }();
