...
bool CowParserV3::Parse(borrowed_fd fd, const CowHeaderV3& header, std::optional<uint64_t> label) {
    ...
    if (label) {
        op_index = GetOpIndex(fd, label.value());
        if (op_index == std::nullopt) {
            PLOG(ERROR) << "failed to get op index from given label: " << label.value();
            return false;
        }
    }
...
std::optional<uint32_t> CowParserV3::GetOpIndex(borrowed_fd fd, const uint32_t label) {
    std::vector<ResumePoint> resume_points;
    ReadResumeBuffer(fd, &resume_points, header_);
    for (auto& resume_point : resume_points) {
        if (resume_point.label == label) {
            return resume_point.op_index;
        }
    }
    PLOG(ERROR) << "failed to find label: " << label << "from following labels";
    for (auto& resume_point : resume_points) {
        PLOG(ERROR) << resume_point.label;
    }
