...
bool CowParserV3::ParseOps(borrowed_fd fd, std::optional<uint64_t> label) {
    ...
    off_t init_offset = header_.prefix.header_size + header_.buffer_size;
    auto data_pos = lseek(fd.get(), init_offset, SEEK_SET);
    if (data_pos != init_offset) {
    ...
    if (!android::base::ReadFully(fd, ops_->data(), ops_->size() * sizeof(CowOperationV3))) {
        PLOG(ERROR) << "read ops failed";
        return false;
    }

    data_pos += header_.op_count_max;
    off_t rv = lseek(fd.get(), data_pos, SEEK_SET);
    if (rv != data_pos) {
        PLOG(ERROR) << "lseek past ops failed";
        return false;
    }
