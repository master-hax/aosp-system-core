...
bool CowWriterV3::EmitBlocks(uint64_t new_block_start, const void* data, size_t size,
    ...
    std::vector<std::basic_string<uint8_t>> compressed_blocks(num_blocks);
    std::vector<CowOperationV3> ops(num_blocks);
    std::vector<struct iovec> vec(num_blocks);
    ...
    for (size_t i = 0; i < num_blocks; i++) {
        ...
        if (compressed_data.size() >= header_.block_size) {
            // this is basically a reinterpret cast + const cast, had to
            // do this because pwritev() interface is not const-correct. Input
            // data will not be modified, as pwrite*() calls do not modify input
            std::memcpy(&vec[i].iov_base, &iter, sizeof(iter));
            vec[i].iov_len = header_.block_size;
        } else {
            compressed_blocks[i] = std::move(compressed_data);
            vec[i] = {.iov_base = compressed_blocks[i].data(),
                      .iov_len = compressed_blocks[i].size()};
        }
        op.data_length = vec[i].iov_len;
        compressed_bytes += op.data_length;
    }
    if (!WriteOperation({ops.data(), ops.size()}, {vec.data(), vec.size()})) {
        PLOG(ERROR) << "AddRawBlocks with compression: write failed. new block: " << new_block_start
                    << " compression: " << compression_.algorithm;
        return false;
    }
