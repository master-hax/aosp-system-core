...
std::vector<CowWriterV3::CompressedBuffer> CowWriterV3::ProcessBlocksWithThreadedCompression(
    ...
    std::vector<std::vector<std::basic_string<uint8_t>>> worker_buffers(num_threads);
    ...
    // compressed_vec | CB 1 | CB 2 | CB 3 |
    //                   t1     t2     t1
    // We need to retrieve the compressed data in the same order we processed it. In this case, each
    // we alternate between threads, and we need to do the same
