...
bool CowWriterV3::EmitLabel(uint64_t label) {
    // remove all labels greater than this current one. we want to avoid the situation of adding  in
    ...
    if (resume_points_->size() > header_.resume_buffer_size) {
        resume_points_->erase(resume_points_->begin());
    }

    CHECK_EQ(resume_points_->size(), header_.resume_buffer_size);
