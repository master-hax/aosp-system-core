...
CowInfo CowWriterV3::GetCowInfo() const {
    CowInfo info;
    info.cow_size = next_data_pos_;
    info.seq_buffer_size = header_.sequence_data_count;
    info.op_buffer_size = header_.op_count_max;
...
bool CowWriterV3::SyncBufferMetadata() {
    ...
    if (!android::base::WriteFullyAtOffset(
                fd_, &header_.op_count, sizeof(header_.op_count),
                sizeof(header_.sequence_data_count) + sizeof(header_.resume_point_count) +
                        sizeof(header_.resume_point_max) + sizeof(CowHeader))) {
        return false;
    }
    return Sync();
}
