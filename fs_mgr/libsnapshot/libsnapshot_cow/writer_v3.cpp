...
bool CowWriterV3::ConstructCowOpCompressedBuffers(uint64_t new_block_start, const void* data,
                                                  ...
                                                  size_t first_block_in_batch,
                                                  size_t blocks_to_write) {
    const auto bytes = reinterpret_cast<const uint8_t*>(data);
    size_t compressed_bytes = 0;
    auto&& blocks = CompressBlocks(blocks_to_write,
                                   bytes + header_.block_size * first_block_in_batch, type);
    if (blocks.empty()) {
        LOG(ERROR) << "Failed to compress blocks " << new_block_start + first_block_in_batch << ", "
    ...
    for (size_t blk_index = 0; blk_index < blocks.size(); blk_index++) {
        ...
        op.new_block = new_block_start + first_block_in_batch + j;

        op.set_type(type);
        op.set_compression_bits(std::log2(buffer.compression_factor / header_.block_size));

        if (type == kCowXorOp) {
            op.set_source((old_block + first_block_in_batch + j) * header_.block_size + offset);
        } else {
        ...
        j += (buffer.compression_factor / header_.block_size);
    }
...
bool CowWriterV3::EmitBlocks(uint64_t new_block_start, const void* data, size_t size,
    ...
    for (size_t i = 0; i < num_blocks;) {
        ...
        if (!ConstructCowOpCompressedBuffers(new_block_start, data, old_block, offset, type, i,
                                             blocks_to_write)) {
            return false;
        }
