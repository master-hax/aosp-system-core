...
bool CowWriterV3::OpenForWrite() {
    // This limitation is tied to the data field size in CowOperationV3.
    ...
    while (num_blocks) {
        ...
        while (i < size / header_.block_size && pending_blocks) {
            ...
            if (compression_.algorithm) {
                ...
                // set compression algorithm bit;
                uint64_t compression_alg = 0;
                compression_alg |= compression_.algorithm;
                compression_alg <<= 56;
                op.source |= compression_alg;
...
bool CowWriterV3::Finalize() {
    footer_.op.ops_size = footer_.op.num_ops * sizeof(CowOperationV3);
    if (lseek(fd_.get(), next_op_pos_, SEEK_SET) < 0) {
...
bool CowWriterV3::WriteOperation(const CowOperationV3& op, const void* data, size_t size) {
    ...
    if (batch_write_) {
        CowOperationV3* cow_op =
                reinterpret_cast<CowOperationV3*>(cowop_vec_[op_vec_index_].iov_base);
        std::memcpy(cow_op, &op, sizeof(CowOperationV3));
        op_vec_index_ += 1;
...
void CowWriterV3::AddOperation(const CowOperationV3& op) {
    ...
    next_op_pos_ += sizeof(CowOperationV3) + GetNextOpOffset(op, header_.cluster_ops);
}
