...
bool CowWriterV3::ParseOptions() {
    ...
    if (compression_.algorithm != kCowCompressNone) {
        ...
        if (options_.batch_write && options_.cluster_ops) {
            batch_size = options_.cluster_ops;
        }
    }
    return true;
}
...
bool CowWriterV3::EmitBlocks(uint64_t new_block_start, const void* data, size_t size,
    ...
    for (size_t i = 0; i < num_blocks; i++) {
        ...
        auto& compressed_data =
                cached_data_.emplace_back(compressor_->Compress(iter, header_.block_size));
        ...
        vec[i] = {.iov_base = compressed_data.data(), .iov_len = compressed_data.size()};
...
bool CowWriterV3::WriteOperation(std::basic_string_view<CowOperationV3> ops,
    ...
    if (ops.size() >= batch_size || ops.size() + cached_ops_.size() > batch_size) {
        FlushCacheOps();
        const off_t offset = GetOpOffset(header_.op_count, header_);
