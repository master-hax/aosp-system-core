...
bool CowWriterV3::EmitBlocks(uint64_t new_block_start, const void* data, size_t size,
    ...
    if (num_blocks * header_.block_size >= CachedDataSize()) {
        if (!FlushCacheOps()) {
            LOG(ERROR) << "failed to flush cache ops in preparation for AddRawBlocks";
            return false;
        }
    }
    while (num_blocks) {
        size_t chunk = std::min(num_blocks, batch_size_);
        if (!ConstructCowOpCompressedBuffers(new_block_start, bytes, old_block, offset, type,
                                             chunk)) {
            return false;
        }

        if (chunk == batch_size_ && !FlushCacheOps()) {
...
bool CowWriterV3::WriteOperation(std::span<const CowOperationV3> ops,
    ...
    if (!data.empty()) {
        ...
        while (to_write) {
            auto chunk = std::min(static_cast<int>(to_write), IOV_MAX);

            const auto ret = pwritev(fd_, data.data() + data.size() - to_write, chunk,
                                     next_data_pos_ + data.size() - to_write);
            if (ret != chunk) {
                PLOG(ERROR) << "write failed for data of size: " << chunk
                            << " at offset: " << next_data_pos_ + data.size() - to_write << " "
                            << errno;
                return false;
            }
            to_write -= chunk;
