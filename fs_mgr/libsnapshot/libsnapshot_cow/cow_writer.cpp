...
void CowWriter::SetupWriteOptions() {
    numCompressThreads_ = options_.num_compress_threads;
    ...
    if (header_.cluster_ops &&
        android::base::GetBoolProperty("ro.virtual_ab.compression.threaded", false)) {
        numCompressThreads_ = 2;
    }
    if (header_.cluster_ops &&
...
void CowWriter::InitWorkers() {
    for (int i = 0; i < numCompressThreads_; i++) {
    ...
    LOG(INFO) << numCompressThreads_ << " thread used for compression";
}
...
bool CowWriter::CompressBlocks(size_t num_blocks, const void* data) {
    size_t num_threads = (num_blocks == 1) ? 1 : numCompressThreads_;
...
bool CowWriter::EmitBlocks(uint64_t new_block_start, const void* data, size_t size,
    ...
    while (num_blocks) {
        size_t pendingBlocks = (std::min(kProcessingBlocks, num_blocks));

        if (compression_) {
            if (!CompressBlocks(pendingBlocks, iter)) {
                return false;
            }
            buf_iter_ = compressed_buf_.begin();
            CHECK(pendingBlocks == compressed_buf_.size());
            iter += (pendingBlocks * header_.block_size);
        }

        num_blocks -= pendingBlocks;

        while (i < size / header_.block_size && pendingBlocks) {
            CowOperation op = {};
