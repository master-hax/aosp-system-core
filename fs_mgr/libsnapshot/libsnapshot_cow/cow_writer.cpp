...
bool CowWriter::CompressBlocks(size_t num_blocks, const void* data, uint64_t new_block) {
    ...
    for (size_t i = 0; i < num_threads; i++) {
        ...
        worker->EnqueueCompressBlocks(iter, num_blocks_per_thread, new_block);
        iter += (num_blocks_per_thread * header_.block_size);
        num_blocks -= num_blocks_per_thread;
        new_block += num_blocks_per_thread;
    }
