...
TEST_F(CowTest, GetSize) {
    ...
    ASSERT_TRUE(writer.AddRawBlocks(50, data.data(), data.size()));
    ASSERT_TRUE(writer.AddZeroBlocks(51, 2));
    auto size_before = writer.GetCowInfo().cow_size;
    ASSERT_TRUE(writer.Finalize());
    auto size_after = writer.GetCowInfo().cow_size;
    ASSERT_EQ(size_before, size_after);
    struct stat buf;

    ASSERT_GE(fstat(cow_->fd, &buf), 0) << strerror(errno);
    ASSERT_EQ(buf.st_size, writer.GetCowInfo().cow_size);
}
...
TEST_F(CowTest, AppendLabelMissing) {
    ...
    ftruncate(cow_->fd, writer->GetCowInfo().cow_size - sizeof(CowFooter) - 3);
