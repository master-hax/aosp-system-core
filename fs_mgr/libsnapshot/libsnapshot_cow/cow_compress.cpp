...
std::unique_ptr<ICompressor> ICompressor::InitializeCompressor(CowCompression compression) {
    std::unique_ptr<ICompressor> compressor;
    if (compression.algorithm == kCowCompressLz4) {
        compressor = ICompressor::Lz4();
    } else if (compression.algorithm == kCowCompressBrotli) {
        compressor = ICompressor::Brotli();
    } else if (compression.algorithm == kCowCompressGz) {
        compressor = ICompressor::Gz();
    } else if (compression.algorithm == kCowCompressZstd) {
        compressor = ICompressor::Zstd();
    } else {
        return nullptr;
    }
    compressor->SetCompressionLevel(compression.compression_level);
    return compressor;
}
...
std::unique_ptr<ICompressor> ICompressor::Brotli() {
    return std::make_unique<BrotliCompressor>();
}

std::unique_ptr<ICompressor> ICompressor::Gz() {
    return std::make_unique<GzCompressor>();
}

std::unique_ptr<ICompressor> ICompressor::Lz4() {
    return std::make_unique<Lz4Compressor>();
}

std::unique_ptr<ICompressor> ICompressor::Zstd() {
