...
std::unique_ptr<ICompressor> ICompressor::Create(CowCompression compression,
    ...
    switch (compression.algorithm) {
        case kCowCompressLz4:
            return ICompressor::Lz4(compression.compression_level, BLOCK_SZ);
        case kCowCompressBrotli:
            return ICompressor::Brotli(compression.compression_level, BLOCK_SZ);
        case kCowCompressGz:
            return ICompressor::Gz(compression.compression_level, BLOCK_SZ);
...
class GzCompressor final : public ICompressor {
  public:
    GzCompressor(uint32_t compression_level, const uint32_t MAX_BLOCK_SIZE)
        : ICompressor(compression_level, MAX_BLOCK_SIZE){};
...
class Lz4Compressor final : public ICompressor {
  public:
    Lz4Compressor(uint32_t compression_level, const uint32_t MAX_BLOCK_SIZE)
        : ICompressor(compression_level, MAX_BLOCK_SIZE){};
...
class BrotliCompressor final : public ICompressor {
  public:
    BrotliCompressor(uint32_t compression_level, const uint32_t MAX_BLOCK_SIZE)
        : ICompressor(compression_level, MAX_BLOCK_SIZE){};
...
class ZstdCompressor final : public ICompressor {
  public:
    ZstdCompressor(uint32_t compression_level, const uint32_t MAX_BLOCK_SIZE)
        ...
        ZSTD_CCtx_setParameter(zstd_context_.get(), ZSTD_c_windowLog,
                               log2(GetMaxCompressionSize()));
    };
...
std::unique_ptr<ICompressor> ICompressor::Brotli(uint32_t compression_level,
                                                 const int32_t BLOCK_SZ) {
    return std::make_unique<BrotliCompressor>(compression_level, BLOCK_SZ);
}

std::unique_ptr<ICompressor> ICompressor::Gz(uint32_t compression_level, const int32_t BLOCK_SZ) {
    return std::make_unique<GzCompressor>(compression_level, BLOCK_SZ);
}

std::unique_ptr<ICompressor> ICompressor::Lz4(uint32_t compression_level, const int32_t BLOCK_SZ) {
    return std::make_unique<Lz4Compressor>(compression_level, BLOCK_SZ);
}
