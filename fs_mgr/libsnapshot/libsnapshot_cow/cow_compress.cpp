...
std::unique_ptr<ICompressor> ICompressor::FromString(std::string_view compression) {
    auto parts = android::base::Split(std::string(compression), ",");
    std::string algorithm = parts[0];
    uint32_t level;
    android::base::ParseUint(parts[1], &level);
