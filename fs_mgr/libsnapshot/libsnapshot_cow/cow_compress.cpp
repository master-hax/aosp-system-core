...
std::unique_ptr<ICompressor> ICompressor::FromString(CowCompressionAlgorithm algorithm,
                                                     uint32_t compression_level) {
    std::unique_ptr<ICompressor> compressor;
    if (algorithm == kCowCompressLz4) {
        compressor = ICompressor::Lz4();
    } else if (algorithm == kCowCompressBrotli) {
        compressor = ICompressor::Brotli();
    } else if (algorithm == kCowCompressGz) {
        compressor = ICompressor::Gz();
    } else if (algorithm == kCowCompressZstd) {
