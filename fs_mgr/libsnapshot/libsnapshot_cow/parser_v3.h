...
class CowParserV3 : public CowParserV2 {
  public:
    bool Parse(android::base::borrowed_fd fd, const CowHeadeV3& header,
               std::optional<uint64_t> label = {}) override;
    const CowHeaderV3& header() const { return header_; }
    const std::optional<CowFooter>& footer() const { return footer_; }
    std::shared_ptr<std::vector<CowOperationV3>> ops() override { return ops_; }
