...
bool SnapshotManager::MapCowDevices(LockedFile* lock, const CreateLogicalPartitionParams& params,
    ...
    *cow_name = GetCowName(partition_name);
    ...
    if (snapshot_status.cow_file_size > 0) {
        ...
        if (snapshot_status.cow_partition_size == 0) {
            *cow_name = std::move(cow_image_name);
            LOG(INFO) << "Mapped COW device for " << partition_name << " at " << *cow_name;
            return true;
        }
    ...
    CreateLogicalPartitionParams::OwnedData cow_partition_params_owned_data;
    if (!cow_partition_params.InitDefaults(&cow_partition_params_owned_data)) {
        return false;
    }
    DmTable table;
    if (!CreateDmTable(*cow_partition_params.partition_opener, *cow_partition_params.metadata,
                       *cow_partition_params.partition, cow_partition_params.block_device,
                       &table)) {
        return false;
    }
    ...
    LOG(INFO) << "Mapped COW for " << params.GetPartitionName() << " at " << cow_path;
    return true;
}
