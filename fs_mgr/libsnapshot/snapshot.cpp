...
std::unique_ptr<SnapshotManager::LockedFile> SnapshotManager::OpenLock(int open_flags,
                                                                       int lock_flags) {
    auto lock_file = GetLockPath();
    return OpenFile(lock_file, open_flags, lock_flags);
}

std::unique_ptr<SnapshotManager::LockedFile> SnapshotManager::LockShared() {
    return OpenLock(O_RDONLY, LOCK_SH);
}

std::unique_ptr<SnapshotManager::LockedFile> SnapshotManager::LockExclusive() {
    return OpenLock(O_RDONLY, LOCK_EX);
}
