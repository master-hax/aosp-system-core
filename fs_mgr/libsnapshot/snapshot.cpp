...
static inline uint64_t RoundUp(uint64_t value, uint64_t block) {
    value += block - 1;
    value &= ~(block - 1);
    return value;
}
...
bool SnapshotManager::CreateSnapshot(LockedFile* lock, const std::string& name,
    ...
    status.cow_file_size = RoundUp(status.cow_file_size, kSectorSize);
...
bool SnapshotManager::CreateCowImage(LockedFile* lock, const std::string& name, uint64_t cow_size) {
    ...
    return images_->CreateBackingImage(cow_image_name, cow_size, cow_flags);
}
...
static bool UnmapImageIfExists(android::fiemap::IImageManager* manager, const std::string& name) {
...
bool SnapshotManager::CreateLogicalAndSnapshotPartitions(const std::string& super_device) {
    ...
    for (const auto& partition : metadata->partitions) {
        ...
        if (!MapSnapshotTree(lock.get(), std::move(params), &ignore_path)) {
            return false;
        }
    }
    return true;
...
bool SnapshotManager::CreateUpdateSnapshots(android::fs_mgr::MetadataBuilder* target_metadata,
    ...
    bool res = ForEachPartition(target_metadata, target_suffix, [&](auto* target_partition) {
        ...
        if (snapshot_params->cow_file_size > 0) {
            snapshot_params->cow_file_size = RoundUp(snapshot_params->cow_file_size, kSectorSize);
            if (!CreateCowImage(lock.get(), target_partition->name(), snapshot_params->cow_file_size)) {
                return LoopDirective::BREAK;
            }
        }
...
bool SnapshotManager::MapUpdateSnapshot(const android::fs_mgr::CreateLogicalPartitionParams& params,
    ...
    CHECK(params.GetPartitionName() == params.GetDeviceName())
        << "Mapping snapshot with a different name is unsupported";
    auto lock = LockShared();
    if (!lock) return false;
    return MapSnapshotTree(lock.get(), params, snapshot_path);
}
