...
std::unique_ptr<SnapshotManager> SnapshotManager::New(IDeviceInfo* info) {
    ...
    std::string scratch_device = android::snapshot::GetOtaMetadataPartition();
    std::string scratch_metadata;
    if (!scratch_device.empty()) {
        scratch_metadata = android::snapshot::MapOtaMetadataPartition(scratch_device);
...
bool SnapshotManager::MapScratchPartitionIfNeeded(
    ...
    auto device = android::snapshot::GetOtaMetadataPartition();
    if (!device.empty()) {
        init(device);
        if (android::snapshot::MapOtaMetadataPartition(device).empty()) {
            return false;
        }
    }
    return true;
