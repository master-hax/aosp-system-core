...
Return SnapshotManager::InitializeUpdateSnapshots(
    ...
    for (auto* target_partition : ListPartitionsWithSuffix(target_metadata, target_suffix)) {
        ...
        if (it->second.using_snapuserd()) {
            ...
            if (cow_version >= 3) {
                options.op_buffer_size = it->second.estimated_ops_buffer_size();
                options.seq_buffer_size = it->second.estimated_seq_buffer_size();
            }
...
std::unique_ptr<ICowWriter> SnapshotManager::OpenCompressedSnapshotWriter(
    ...
    cow_options.op_buffer_size = status.estimated_ops_buffer_size();
    cow_options.seq_buffer_size = status.estimated_seq_buffer_size();
