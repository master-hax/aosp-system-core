...
UpdateState SnapshotManager::ProcessUpdateState(const std::function<bool()>& callback,
    ...
    while (true) {
        ...
        LOG(INFO) << "ProcessUpdateState handling state: " << result.first;

        if (result.first == UpdateState::MergeFailed) {
            AcknowledgeMergeFailure(result.second);
        }
        if (result.first != UpdateState::Merging) {
...
auto SnapshotManager::CheckMergeState(const std::function<bool()>& before_cancel) -> MergeResult {
    ...
    LOG(INFO) << "CheckMergeState for snapshots returned: " << result.first;

    if (result.first == UpdateState::MergeCompleted) {
        // Do this inside the same lock. Failures get acknowledged without the
        // lock, because flock() might have failed.
        AcknowledgeMergeSuccess(lock.get());
    } else if (result.first == UpdateState::Cancelled) {
...
auto SnapshotManager::CheckMergeState(LockedFile* lock, const std::function<bool()>& before_cancel)
    ...
    switch (update_status.state()) {
        ...
        case UpdateState::MergeCompleted:
            ...
            return MergeResult(update_status.state(), MergeFailureCode::Ok);
        ...
        case UpdateState::Unverified:
            ...
            if (HandleCancelledUpdate(lock, before_cancel)) {
                return MergeResult(UpdateState::Cancelled, MergeFailureCode::Ok);
            }
            return MergeResult(update_status.state(), MergeFailureCode::Ok);
    ...
    for (const auto& snapshot : snapshots) {
        ...
        LOG(INFO) << "CheckTargetMergeState for " << snapshot << " returned: " << result.first;

        switch (result.first) {
            case UpdateState::MergeFailed:
                failure_code = result.second;
                break;
            case UpdateState::Merging:
                merging = true;
                break;
    ...
    if (merging) {
        ...
        return MergeResult(UpdateState::Merging, MergeFailureCode::Ok);
    }
    ...
    if (needs_reboot) {
        ...
        return MergeResult(UpdateState::MergeNeedsReboot, MergeFailureCode::Ok);
    }
    if (cancelled) {
...
auto SnapshotManager::CheckTargetMergeState(LockedFile* lock, const std::string& name,
    ...
    if (target_type == "snapshot" &&
        ...
        return MergeResult(UpdateState::None, MergeFailureCode::Ok);
    }
...
MergeFailureCode SnapshotManager::ReadMergeFailureCode() {
    auto lock = LockExclusive();
    if (!lock) return MergeFailureCode::ReadStatus;
