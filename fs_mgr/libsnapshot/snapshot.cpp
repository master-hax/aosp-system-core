...
UpdateState SnapshotManager::ProcessUpdateState(const std::function<bool()>& callback,
    ...
    while (true) {
        ...
        if (callback && !callback()) {
            LOG(INFO) << "ProcessUpdateState paused.";
            return state;
        }
...
bool SnapshotManager::HandleCancelledUpdate(LockedFile* lock,
    ...
    LOG(INFO) << "Update state is being processed before reboot, taking no action.";
    return false;
}
...
bool SnapshotManager::Dump(std::ostream& os) {
    ...
    ss << "Rollback indicator: "
       << (access(GetRollbackIndicatorPath().c_str(), F_OK) == 0 ? "exists" : "does not exist");
