...
bool SnapshotManager::TryCancelUpdate(bool* needs_merge) {
    *needs_merge = false;
    ...
    const bool snapshots_on_current_slot =
            (access(GetBootSnapshotsWithoutSlotSwitch().c_str(), F_OK) == 0);
    if (snapshots_on_current_slot) {
...
bool SnapshotManager::RemoveAllUpdateState(LockedFile* lock, const std::function<bool()>& prolog) {
    ...
    std::vector<std::string> files = {
            ...
            GetBootSnapshotsWithoutSlotSwitch(),
    };
...
bool SnapshotManager::MapDmUserCow(LockedFile* lock, const std::string& name,
    ...
    } else if (snapshots_on_current_slot) {
...
std::string SnapshotManager::GetBootSnapshotsWithoutSlotSwitch() {
...
bool SnapshotManager::NeedSnapshotsInFirstStageMount() {
    if (access(GetBootSnapshotsWithoutSlotSwitch().c_str(), F_OK) == 0) {
        return true;
    }
...
bool SnapshotManager::BootFromSnapshotsWithoutSlotSwitch() {
    ...
    auto boot_file = GetBootSnapshotsWithoutSlotSwitch();
    if (!WriteStringToFileAtomic(contents, boot_file)) {
        PLOG(ERROR) << "write failed: " << boot_file;
        return false;
    }
...
bool SnapshotManager::PrepareDeviceToBootWithoutSnapshot() {
    ...
    android::base::RemoveFileIfExists(GetBootSnapshotsWithoutSlotSwitch());
