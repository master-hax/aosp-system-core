...
bool SnapshotManager::TryCancelUpdate(bool* needs_merge) {
    *needs_merge = false;
    ...
    if (BootSnapshotWithoutSlotSwitch()) {
...
bool SnapshotManager::MapDmUserCow(LockedFile* lock, const std::string& name,
    ...
    } else if (BootSnapshotWithoutSlotSwitch()) {
...
bool SnapshotManager::BootSnapshotWithoutSlotSwitch() {
    return (access(GetBootSnapshotsWithoutSlotSwitch().c_str(), F_OK) == 0);
}
