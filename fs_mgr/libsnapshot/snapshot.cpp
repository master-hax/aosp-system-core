...
bool SnapshotManager::UnmapCowImage(const std::string& name) {
    ...
    if (!DeviceMapper::Instance().DeleteDeviceIfExists(cow_name)) {
        LOG(ERROR) << "Cannot unmap outer COW device: " << cow_name;
        return false;
    }
...
bool SnapshotManager::DeleteSnapshot(LockedFile* lock, const std::string& name) {
    ...
    if (!DeviceMapper::Instance().DeleteDeviceIfExists(GetCowName(name))) {
        return false;
    }

    // This is not an error because the COW partition can just be ignored.
    if (!DeviceMapper::Instance().DeleteDeviceIfExists(GetCowPartitionDeviceName(name))) {
        LOG(WARNING) << "Cannot unmap " << GetCowPartitionDeviceName(name)
...
bool SnapshotManager::MapPartitionWithSnapshot(LockedFile* lock,
    ...
    if (!MapCowDevices(lock, cow_params, *live_snapshot_status, &created_devices, &cow_device)) {
        return false;
    }
...
bool SnapshotManager::MapCowDevices(LockedFile* lock, const CreateLogicalPartitionParams& params,
    ...
    if (snapshot_status.cow_file_size > 0) {
        ...
        // manager. In this case, using |path| in MapCowDevices is acceptable.
...
bool SnapshotManager::CreateUpdateSnapshots(android::fs_mgr::MetadataBuilder* target_metadata,
    ...
    res = ForEachPartition(target_metadata, target_suffix, [&](auto* target_partition) {
        ...
        CHECK(!cow_device.path.empty())
                << "MapCowDevices on " << target_partition->name()
