...
std::unique_ptr<SnapshotManager> SnapshotManager::New() {
    return std::unique_ptr<SnapshotManager>(new SnapshotManager("ota", "/metadata/ota"));
}

std::unique_ptr<SnapshotManager> SnapshotManager::NewForTesting() {
    return std::unique_ptr<SnapshotManager>(new SnapshotManager("ota/test", "/metadata/ota/test"));
}

SnapshotManager::SnapshotManager(const std::string& gsid_dir, const std::string& metadata_dir)
    : gsid_dir_(gsid_dir), metadata_dir_(metadata_dir) {}
...
bool SnapshotManager::CreateSnapshot(LockedFile* lock, const std::string& name,
    ...
    int cow_flags = ImageManager::CREATE_IMAGE_ZERO_FILL;
...
bool SnapshotManager::MapSnapshot(LockedFile* lock, const std::string& name,
    ...
    DmTable table;
    table.Emplace<DmTargetSnapshot>(0, snapshot_sectors, base_device, cow_dev, mode, kChunkSize);
    if (linear_sectors != 0) {
...
bool SnapshotManager::UnmapSnapshot(LockedFile* lock, const std::string& name) {
    ...
    auto cow_name = GetCowName(name);
    return images_->UnmapImageDevice(cow_name);
}
...
bool SnapshotManager::EnsureImageManager() {
    ...
    images_ = ImageManagerBridge::Open(gsid_dir_);
    if (!images_) {
