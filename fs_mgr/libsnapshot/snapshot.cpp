 ...
 *    dm-snapshots. Thus, this function checks:
...
bool SnapshotManager::UpdateUsesUserSnapshots(LockedFile* lock) {
    // The ordering here is important. We need to first check
    // for the legacy dm-snapshot before checking for userspace snapshots.
    if (IsLegacySnapuserdPostReboot()) {
        return false;
    }
