...
static std::string GetDmUserCowName(const std::string& snapshot_name, bool snapshot_user_space) {
...
bool SnapshotManager::IsSnapshotDevice(const std::string& dm_name, TargetInfo* target) {
    ...
    if (!snapshot_user_space_) {
        ...
        // dm-user device will be the snapshot device
        if (type != "user") {
            return false;
        }
    }
...
bool SnapshotManager::CollapseSnapshotDevice(const std::string& name,
    ...
    // the reference
    if (snapshot_user_space_ && EnsureSnapuserdConnected()) {
...
bool SnapshotManager::PerformInitTransition(InitTransition transition,
    ...
    size_t ok_cows = 0;
    for (const auto& snapshot : snapshots) {
        std::string user_cow_name = GetDmUserCowName(snapshot, snapshot_user_space_);
        ...
        uint64_t base_sectors = snapuserd_client_->InitDmUserCow(misc_name, cow_image_device,
...
bool SnapshotManager::MapPartitionWithSnapshot(LockedFile* lock,
    ...
    if (live_snapshot_status->compression_enabled()) {
        ...
        auto name = GetDmUserCowName(params.GetPartitionName(), snapshot_user_space_);
...
bool SnapshotManager::UnmapDmUserDevice(const std::string& snapshot_name) {
    auto dm_user_name = GetDmUserCowName(snapshot_name, snapshot_user_space_);
