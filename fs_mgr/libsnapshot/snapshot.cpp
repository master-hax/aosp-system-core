...
bool SnapshotManager::MapSnapshot(LockedFile* lock, const std::string& name,
    ...
    SnapshotStatus status;
    if (!ReadSnapshotStatus(name, &status)) {
        return false;
    }
...
bool SnapshotManager::InitiateMerge() {
    ...
    if (state != UpdateState::Unverified) {
        LOG(ERROR) << "Cannot begin a merge if an update has not been applied";
        return false;
    }
...
UpdateState SnapshotManager::CheckMergeState() {
    auto lock = LockExclusive();
    if (!lock) return UpdateState::MergeFailed;
    ...
    for (const auto& snapshot : snapshots) {
        UpdateState snapshot_state = CheckTargetMergeState(lock.get(), snapshot);
    ...
    if (needs_reboot) {
        WriteUpdateState(lock.get(), UpdateState::MergeNeedsReboot);
        return UpdateState::MergeNeedsReboot;
    }

    // Everything reported complete, so acknowledge the completed merge.
    AcknowledgeMergeSuccess(lock.get());
    return UpdateState::MergeCompleted;
}
...
UpdateState SnapshotManager::CheckTargetMergeState(LockedFile* lock, const std::string& name) {
    ...
    if (!ReadSnapshotStatus(name, &snapshot_status)) {
        return UpdateState::MergeFailed;
    }
...
bool SnapshotManager::ReadSnapshotStatus(const std::string& name, SnapshotStatus* status) {
