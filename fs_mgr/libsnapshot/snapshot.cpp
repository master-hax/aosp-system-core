...
/*
 * TODO Deprecated, remove when there are no more devices using the old StateFile format.
 * b/XXX
 */
...
SnapshotUpdateStatus SnapshotManager::ReadSnapshotUpdateStatus(LockedFile* lock) {
    ...
    if (!status.ParseFromFileDescriptor(fd.get())) {
        LOG(ERROR) << "Unable to parse " << path << " as SnapshotUpdateStatus";
        status.set_state(UpdateState::None);

        // TODO Remove the following when there are no more dogfooding devices
        ...
        // b/XXX
        {
            ...
            if (!android::base::ReadFdToString(fd, &contents)) {
                PLOG(ERROR) << "Read legacy " << path << " failed";
                return status;
            }
            LOG(INFO) << "Parsed " << path << " with old file SnapshotUpdateStatus format";
