...
bool SnapshotManager::CancelUpdate() {
    ...
    if (state == UpdateState::Unverified) {
        // We completed an update, but it can still be canceled if we haven't boot into it.
...
bool SnapshotManager::CreateSnapshot(LockedFile* lock, const std::string& name,
    ...
    if (status.cow_file_size % kSectorSize != 0) {
        LOG(ERROR) << "Snapshot " << name
                   << " cow file size is not a multiple of the sector size: " << status.cow_partition_size;
        return false;
    }
...
bool SnapshotManager::CreateLogicalAndSnapshotPartitions(const std::string& super_device) {
    ...
    for (const auto& partition : metadata->partitions) {
        if (GetPartitionGroupName(metadata->groups[partition.group_index]) == kCowGroupName) {
            LOG(INFO) << "Skip mapping partition " << GetPartitionName(partition)
                      << " in group " << kCowGroupName;
            continue;
        }
...
bool SnapshotManager::MapCowTree(LockedFile* lock,
    ...
    if (snapshot_status.cow_partition_size > 0) {
        ...
        if (!dm.GetDeviceString(cow_partition_params.GetDeviceName(), &cow_partition_device.number)) {
    ...
    if (snapshot_status.cow_file_size > 0) {
        ...
        created_devices->EmplaceBack<AutoUnmapImage>(images_.get(), GetCowImageDeviceName(params.GetPartitionName()));
    ...
    LOG(INFO) << "Mapped COW for " << params.GetPartitionName() << " at path = "
              << cow_device->path << ", number = " << cow_device->number;
    return true;
}
...
bool SnapshotManager::CreateUpdateSnapshots(android::fs_mgr::MetadataBuilder* target_metadata,
    ...
    bool res = ForEachPartition(target_metadata, target_suffix, [&](auto* target_partition) {
        ...
        if (cow_creator_ret->snapshot_status.cow_partition_size > 0) {
            CHECK(cow_creator_ret->snapshot_status.cow_partition_size % kSectorSize == 0)
                << "cow_partition_size == " << cow_creator_ret->snapshot_status.cow_partition_size
    ...
    res = ForEachPartition(target_metadata, target_suffix, [&](auto* target_partition) {
        ...
        CHECK(!cow_device.path.empty()) << "MapCowTree on " << target_partition->name()
                << " returns true but no path is returned. Returned number: "
                << cow_device.number;
