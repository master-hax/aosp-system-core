...
std::unique_ptr<ISnapshotWriter> SnapshotManager::OpenSnapshotWriter(
    ...
    if (IsCompressionEnabled()) {
        return OpenCompressedSnapshotWriter(lock.get(), params.GetPartitionName(), paths);
    } else {
        return OpenKernelSnapshotWriter(lock.get(), params.GetPartitionName(), paths);
    }
}

std::unique_ptr<ISnapshotWriter> SnapshotManager::OpenCompressedSnapshotWriter(
        LockedFile*, const std::string&, const SnapshotPaths&) {
...
std::unique_ptr<ISnapshotWriter> SnapshotManager::OpenKernelSnapshotWriter(
        LockedFile* lock, const std::string& partition_name, const SnapshotPaths& paths) {
    SnapshotStatus status;
    if (!ReadSnapshotStatus(lock, partition_name, &status)) {
        return nullptr;
    }
