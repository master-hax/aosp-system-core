...
bool SnapshotManager::InitiateMerge() {
    ...
    for (auto it = snapshots.begin(); it != snapshots.end();) {
        if (IsCancelledSnapshot(*metadata, *it)) {
            LOG(WARNING) << "Detected re-flashing for partition " << *it << ". Skip merging it.";
            if (!DeleteSnapshot(lock.get(), *it)) {
                LOG(WARNING) << "Cannot delete snapshot for re-flashed partition "  << *it
...
bool SnapshotManager::IsCancelledSnapshot(const LpMetadata& metadata, const std::string& snapshot_name) {
    auto partition = android::fs_mgr::FindPartition(metadata, snapshot_name);
...
bool SnapshotManager::IsCancelledSnapshot(const std::string& snapshot_name) {
    ...
    return IsCancelledSnapshot(*metadata, snapshot_name);
}
...
bool SnapshotManager::HandleCancelledUpdateOnNewSlot(LockedFile* lock) {
    ...
    for (const auto& snapshot_name : snapshots) {
        if (!IsCancelledSnapshot(*metadata, snapshot_name)) {
...
UpdateState SnapshotManager::InitiateMergeAndWait() {
    ...
    LOG(INFO) << "Merge finishes with state \"" << state << "\".";
    return state;
}
