...
class SnapshotMetadataUpdater {
  ...
  private:
    // Wraps a DynamicPartitionGroup with a suffixed name. Always use
    // .name instead of ->name() because .name has the suffix.
    struct Group {
        std::string name;
        const DynamicPartitionGroup* group;
        const DynamicPartitionGroup* operator->() const { return group; }
    };
    // Wraps a PartitionUpdate with a suffixed name / group name. Always use
    // .name instead of ->partition_name() because .name has the suffix.
    struct Partition {
        std::string name;
