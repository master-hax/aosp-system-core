...
time_t GetSystemBootTimestamp() {
    ...
    time_t boot_time{};
    sscanf(proc_stat.data() + pos + btime.size(), "%ld", &boot_time);
...
bool WriteToLogFile(void* data, size_t size) {
    ...
    static int fd = -2;
    static std::array<char, 1024 * 1024 * 2> buf;
...
void CachedFileLogger(android::base::LogId /*id*/, android::base::LogSeverity severity,
    ...
    for (auto line_msg : SplitString(message, "\n")) {
        const auto bytes_written = snprintf(
                buf.data(), buf.size(), "[%02d%02d/%02d%02d%02d.%ld] [%s:%s(%d)] ",
...
void DeleteOldLogs() {
    ...
    for (int i = 10; i < log_files.size(); ++i) {
