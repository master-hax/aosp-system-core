...
void SnapuserdServer::TerminateMergeThreads(std::lock_guard<std::mutex>* proof_of_lock)
{
    CHECK(proof_of_lock);
...
bool SnapuserdServer::RemoveAndJoinHandler(const std::string& misc_name) {
    ...
    LOG(INFO) << "RemoveAndJoinHandler invoked.. waiting for join..";
    auto& th = handler->thread();
    if (th.joinable()) {
        th.join();
    }
    LOG(INFO) << "RemoveAndJoinHandler done..";
    return true;
}
