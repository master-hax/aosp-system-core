...
LocalBlockServer::LocalBlockServer(std::shared_ptr<LocalBlockServerQueue> queue, Delegate* delegate,
                                   size_t buffer_size)
    : queue_(queue), delegate_(delegate), buffer_size_(buffer_size) {}
...
bool LocalBlockServer::ProcessRequests() {
    while (true) {
        ...
        CHECK_NE(ok, current_request_->errored);
        if (ok) {
...
void* LocalBlockServer::GetResponseBuffer(size_t size, size_t to_write) {
    ...
    for (const auto& buffer : buffers_) {
        used += buffer.first.size();
    }
    if (buffer_size_ - used < size) {
        return nullptr;
    }
...
std::unique_ptr<IBlockServer> LocalBlockServerQueue::Open(IBlockServer::Delegate* delegate,
                                                          size_t buffer_size) {
    return std::make_unique<LocalBlockServer>(shared_from_this(), delegate, buffer_size);
}
...
bool LocalBlockServerQueue::Read(uint64_t sector, void* buffer, uint64_t len) {
    auto available_size = num_sectors_ * SECTOR_SIZE;
    auto start_offset = sector * SECTOR_SIZE;
    if (len > available_size || start_offset > available_size - len) {
    ...
    std::unique_lock<std::mutex> lock(lock_);
    return CompleteRequest(&lock, &request);
}

bool LocalBlockServerQueue::CompleteRequest(std::unique_lock<std::mutex>* lock,
                                            BlockRequest* request) {
    if (shutdown_) {
    ...
    std::unique_lock<std::mutex> request_lock(request->m);
    request->cv.wait(request_lock, [&]() -> bool { return request->completed; });
