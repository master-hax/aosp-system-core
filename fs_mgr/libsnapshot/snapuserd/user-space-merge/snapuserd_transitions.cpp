...
bool Snapuser::WaitForMergeBegin() {
    {
        std::unique_lock<std::mutex> lock(lock_);
        while (!MergeInitiated() || !(io_state_ == MERGE_IO_TRANSITION::MERGE_BEGIN ||
...
bool Snapuser::ReadAheadIOCompleted(bool sync) {
    if (sync) {
...
bool Snapuser::WaitForMergeReady() {
    {
        std::unique_lock<std::mutex> lock(lock_);
...
void Snapuser::NotifyRAForMergeReady() {
    {
        std::lock_guard<std::mutex> lock(lock_);
        if (io_state_ != MERGE_IO_TRANSITION::IO_TERMINATED &&
            io_state_ != MERGE_IO_TRANSITION::READ_AHEAD_FAILURE) {
...
void Snapuser::NotifyIOTerminated() {
    {
        std::lock_guard<std::mutex> lock(lock_);
        io_state_ = MERGE_IO_TRANSITION::IO_TERMINATED;
    }
...
void Snapuser::ReadAheadIOFailed() {
    {
        std::lock_guard<std::mutex> lock(lock_);
        io_state_ = MERGE_IO_TRANSITION::READ_AHEAD_FAILURE;
    }
...
void Snapuser::WaitForMergeComplete() {
    {
        std::unique_lock<std::mutex> lock(lock_);
 ...
 * as there is no depedency between blocks. Hence all the I/O request
...
void Snapuser::SetMergeCompleted(size_t block_index) {
    {
...
void Snapuser::SetMergeInProgress(size_t block_index) {
    {
...
void Snapuser::SetMergeFailed(size_t block_index) {
    {
        std::unique_lock<std::mutex> lock(m_lock_);
        MergeBlockState* blk_state = merge_blk_state_[block_index].get();
...
// to resume merging.
void Snapuser::NotifyIOCompletion(uint64_t new_block) {
...
MERGE_BLOCK_STATE Snapuser::GetMergeBlockState(uint64_t new_block) {
    auto it = block_to_ra_index_.find(new_block);
