...
void UpdateVerify::VerifyUpdatePartition() {
    ...
    std::future<bool> status = std::async(std::launch::async, &UpdateVerify::VerifyPartition, this,
                                          partition_name, dm_block_devices.at(partition_name));

    if (status.get()) {
        succeeded = true;
        SNAP_LOG(INFO) << "Partition: " << partition_name
...
bool UpdateVerify::VerifyBlocks(const std::string& partition_name,
    ...
    size_t read_sz = kBlockSizeVerify;
    ...
    LOG(INFO) << "Verification success with bytes-read: " << bytes_read << " dev_sz: " << dev_sz
...
bool UpdateVerify::VerifyPartition(const std::string& partition_name,
    ...
    LOG(INFO) << "VerifyPartition: " << partition_name << " Block-device: " << dm_block_device;
