...
int WorkerThread::PrepareMerge(uint64_t* source_offset, int* pending_ops,
...
bool WorkerThread::MergeReplaceZeroOps(std::unique_ptr<ICowOpIter>& cowop_iter) {
    ...
    while (!cowop_iter->Done()) {
        ...
        int ret = pwrite(base_path_merge_fd_.get(), bufsink_.GetPayloadBufPtr(),
                         io_size, source_offset);
        if (ret < 0 || ret != io_size) {
          SNAP_LOG(ERROR) << "Merge: ReplaceZeroOps: Failed to write to backing device while merging "
                          << " at offset: " << source_offset
                          << " io_size: " << io_size;
          return false;
        }
...
bool WorkerThread::MergeOrderedOps(std::unique_ptr<ICowOpIter>& cowop_iter) {
    void* mapped_addr = snapuserd_->GetMappedAddr();
    void* read_ahead_buffer = static_cast<void*>((char*)mapped_addr + snapuserd_->GetBufferDataOffset());
    ...
    while(!cowop_iter->Done()) {
        ...
        SNAP_LOG(INFO) << "Waiting for merge begin...";
        ...
        snapuserd_->SetMergeBlockPending(block_index);

        loff_t offset = 0;
        int num_ops = snapuserd_->GetTotalRaBlocksMerged();
        SNAP_LOG(INFO) << "Merging copy-ops of size: " << num_ops;
        while(num_ops) {
            uint64_t source_offset;
            ...
            // the contents out.
            int ret = pwrite(base_path_merge_fd_.get(), (char*)read_ahead_buffer + offset,
                             io_size, source_offset);
            if (ret < 0 || ret != io_size) {
        ...
        snapuserd_->SetMergeBlockCompleted(block_index);
...
bool WorkerThread::RunMergeThread() {

    SNAP_LOG(INFO) << "Waiting for merge begin...";
    if (!snapuserd_->WaitForMergeBegin()) {
        SNAP_LOG(INFO) << "Merge terminated early...";
        return true;
    }
    ...
    if (!InitializeFds()) {
        SNAP_LOG(INFO) << "Failed to init fd...";
        return false;
    }
