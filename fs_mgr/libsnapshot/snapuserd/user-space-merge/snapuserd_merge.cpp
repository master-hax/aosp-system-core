...
bool Worker::MergeReplaceZeroOps(std::unique_ptr<ICowOpIter>& cowop_iter) {
...
bool Worker::MergeOrderedOpsSlowPath(std::unique_ptr<ICowOpIter>& cowop_iter) {
    while (!cowop_iter->Done()) {
        const CowOperation* cow_op = &cowop_iter->Get();
        ...
        // Merge one 4k block at a time
        if (cow_op->type == kCowCopyOp) {
            if (!ProcessCopyOp(cow_op)) {
                SNAP_LOG(ERROR) << "Merge: MergeOrderedOpsSlowPath: Failed to process copy op";
                return false;
            }
        } else {
            if (!ProcessXorOp(cow_op)) {
                SNAP_LOG(ERROR) << "Merge: MergeOrderedOpsSlowPath: Failed to process xor op";
                return false;
            }
        }

        size_t io_size = BLOCK_SZ;
        uint64_t source_offset = cow_op->new_block * BLOCK_SZ;

        // Merge - Write the contents back to base device
        int ret = pwrite(base_path_merge_fd_.get(), bufsink_.GetPayloadBufPtr(), io_size,
                         source_offset);
        if (ret < 0 || ret != io_size) {
            SNAP_LOG(ERROR) << "Merge: MergeOrderedOpsSlowPath: Failed to write to backing device "
                               "while merging "
                            << " at offset: " << source_offset << " io_size: " << io_size;
            return false;
        }
        ...
        if (fsync(base_path_merge_fd_.get()) < 0) {
            SNAP_LOG(ERROR) << "Merge: MergeOrderedOpsSlowPath: Failed to fsync merged data";
            return false;
        }

        // Track the merge completion
        if (!snapuserd_->CommitMerge(1)) {
            SNAP_LOG(ERROR) << "Merge: MergeOrderedOpsSlowPath: Failed to commit the merged block "
                               "in the header";
            return false;
        }
        ...
        if (snapuserd_->IsIOTerminated()) {
            SNAP_LOG(ERROR)
                    << "MergeOrderedOpsSlowPath: Worker threads terminated - shutting down merge";
            return false;
        }
    }
...
bool Worker::MergeOrderedOps(std::unique_ptr<ICowOpIter>& cowop_iter) {
    if (!snapuserd_->IsReadAheadFeaturePresent()) {
        return MergeOrderedOpsSlowPath(cowop_iter);
    }
