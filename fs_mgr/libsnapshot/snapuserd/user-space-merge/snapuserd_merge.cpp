        ...
        while (num_ops) {
            ...
            if (pending_sqe == 0 || num_ops == 0 || (linear_blocks == 0 && pending_ios_to_submit)) {
                ...
                while (pending_ios_to_complete) {
                    ...
                    // Reap all the I/O's submitted. We
                    // shouldn't bail out even if there is
                    // an error.
...
bool Worker::AsyncMerge() {
    if (!MergeOrderedOpsAsync()) {
        SNAP_LOG(ERROR) << "MergeOrderedOpsAsync failed - Falling back to synchrounus I/O";
...
bool Worker::Merge() {
    ...
    bool sync_merge_required = (retry || !merge_async_);
