...
std::shared_ptr<HandlerThread> UserSnapshotServer::AddHandler(
    ...
    if (!num_worker_threads.has_value()) {
        LOG(ERROR) << "num worker threads has no value";
        num_worker_threads = kNumWorkerThreads;
    } else {
        LOG(ERROR) << "num worker threads: " << num_worker_threads.value();
    }
    LOG(ERROR) << "----> after set num worker threads: " << num_worker_threads.value();
