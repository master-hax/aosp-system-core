        ...
        if (read_ahead_buffer_map.find(op->new_block) != read_ahead_buffer_map.end()) {
            ...
            if (!(num_ops == 0)) {
                SNAP_LOG(ERROR) << "ReconstructDataFromCow failed. Not all ops recoverd "
                                << " Pending ops: " << num_ops;
                SNAP_LOG(ERROR) << "Scratch space blocks....";

                int i = 0;
                for (auto const& x : read_ahead_buffer_map) {
                    SNAP_LOG(ERROR) << i << ": " << x.first;
                    i++;
                }

                SNAP_LOG(ERROR) << "Blocks for merging....";
                i = 0;
                while (num_ops) {
                    const CowOperation* cop = GetRAOpIter();
                    SNAP_LOG(ERROR) << i << ": " << cop->new_block;
                    num_ops -= 1;
                    RAIterNext();
                }
...
bool ReadAheadThread::ReadAheadIOStart() {
    ...
    std::unique_ptr<uint8_t[]> ra_temp_meta_buffer =
            std::make_unique<uint8_t[]>(snapuserd_->GetBufferMetadataSize());

    struct ScratchMetadata* bm =
            reinterpret_cast<struct ScratchMetadata*>((char*)ra_temp_meta_buffer.get() + metadata_offset);
    ...
    for (int i = 0; i < blocks.size(); i++) {
        ...
        if (overlap_) {
            SNAP_LOG(DEBUG) << "New-block: " << new_block;
        }
    ...
    bm = reinterpret_cast<struct ScratchMetadata*>((char*)ra_temp_meta_buffer.get() + metadata_offset);
    bm->new_block = 0;
    bm->file_offset = 0;
