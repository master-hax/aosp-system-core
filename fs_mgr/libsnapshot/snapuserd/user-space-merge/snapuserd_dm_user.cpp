...
bool Worker::ProcessReplaceOp(const CowOperation* cow_op, void* buffer) {
...
bool Worker::ReadFromSourceDevice(const CowOperation* cow_op, void* buffer) {
    uint64_t offset;
...
bool Worker::ProcessCopyOp(const CowOperation* cow_op, void* buffer) {
...
bool Worker::ProcessXorOp(const CowOperation* cow_op, void* buffer) {
    if (!ReadFromSourceDevice(cow_op, buffer)) {
        return false;
    }
...
bool Worker::ProcessZeroOp(void* buffer) {
    memset(buffer, 0, BLOCK_SZ);
    return true;
}

bool Worker::ProcessOrderedOp(const CowOperation* cow_op, void* buffer) {
...
bool Worker::ProcessCowOp(const CowOperation* cow_op, void* buffer) {
    if (cow_op == nullptr) {
...
bool Worker::ReadDataFromBaseDevice(sector_t sector, void* buffer, size_t read_size) {
...
int Worker::ReadUnalignedSector(
    ...
    void* buffer = bufsink_.AcquireBuffer(BLOCK_SZ, write_size);
    if (!buffer) {
    ...
    if (skip_size) {
        ...
        memmove(msg->payload.buf, (char*)msg->payload.buf + skip_size, write_size);
    }
    return write_size;
}
