...
void Snapuserd::SetMergeCompleted(size_t block_index) {
    {
...
void Snapuserd::SetMergeInProgress(size_t block_index) {
    {
...
void Snapuserd::SetMergeFailed(size_t block_index) {
    {
        std::unique_lock<std::mutex> lock(m_lock_);
        MergeBlockState* blk_state = merge_blk_state_[block_index].get();
...
void Snapuserd::NotifyIOCompletion(uint64_t new_block) {
...
MERGE_BLOCK_STATE Snapuserd::GetMergeBlockState(uint64_t new_block) {
    auto it = block_to_ra_index_.find(new_block);
        ...
        switch(state) {
            ...
            case MERGE_BLOCK_STATE::MERGE_IN_PROGRESS: {
                ...
                while (!(blk_state->merge_state_ == MERGE_BLOCK_STATE::MERGE_COMPLETED ||
                        blk_state->merge_state_ == MERGE_BLOCK_STATE::MERGE_FAILED)) {
                    SNAP_LOG(INFO) << "Waiting for merge to complete....";
                    m_cv_.wait(lock);
                }
