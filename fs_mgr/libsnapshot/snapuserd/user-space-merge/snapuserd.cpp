...
void Snapuserd::UpdateMergeCompletionPercentage() {
    ...
    SNAP_LOG(INFO) << "Merge-complete %: " << merge_completion_percentage_
...
bool Snapuserd::ReadMetadata() {
    ...
    while (!cowop_iter->Done()) {
        ...
        // Store operation pointer. These are the ops which will be merged
        // and I/O will be served during boot up.
        chunk_vec_.push_back(std::make_pair(ChunkToSector(cow_op->new_block), cow_op));
        cowop_iter->Next();

        if (read_ahead_feature_ && IsOrderedOp(*cow_op)) {
