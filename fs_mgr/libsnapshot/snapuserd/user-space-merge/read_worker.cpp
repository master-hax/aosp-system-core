...
bool ReadWorker::ReadAlignedSector(sector_t sector, size_t sz) {
    ...
    int ret = 0;
    const size_t max_compression_size = reader_->GetMaxCompressionSize();
    if (!max_compression_size) {
        LOG(ERROR) << "Max compression size set to 0";
        return false;
    }
    ...
    do {
        ...
        // Create a temporary buffer which can hold up to maximum supported block
        // compression.
        std::unique_ptr<uint8_t[]> temp_buffer = std::make_unique<uint8_t[]>(max_compression_size);
        while (read_size) {
            ...
            if (sector_not_found) {
                ...
                if (is_mapping_present) {
                    ...
                    if (cow_op->type() == kCowReplaceOp) {
                        is_mapping_present =
                                GetBlockOffset(cow_op, io_block, BLOCK_SZ, &block_offset);
                    } else {
                        is_mapping_present = false;
                    }
                }
                ...
                if (is_mapping_present) {
                    ...
                    if (prev_op && prev_op->new_block == cow_op->new_block) {
                        std::memcpy(buffer, (char*)temp_buffer.get() + block_offset, size);
                    } else {
                        // Get the data from the disk based on the compression
                        // size
                        if (!ProcessReplaceOp(cow_op, temp_buffer.get(), compression_size)) {
                            return false;
                        }
