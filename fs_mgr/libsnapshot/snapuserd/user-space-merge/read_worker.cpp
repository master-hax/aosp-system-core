...
bool ReadWorker::ProcessCowOp(const CowOperation* cow_op, void* buffer) {
    ...
    switch (cow_op->type()) {
        case kCowReplaceOp: {
            size_t buffer_size = CowOpCompressionSize(cow_op);
...
bool ReadWorker::ReadAlignedSector(sector_t sector, size_t sz) {
    ...
    do {
        ...
        std::unique_ptr<uint8_t[]> temp_buffer =
                std::make_unique<uint8_t[]>(reader_->GetMaxCompressionSize());
        while (read_size) {
            ...
            if (sector_not_found) {
                ...
                if (is_mapping_present) {
                    ...
                    size_t compression_size = CowOpCompressionSize(cow_op);
...
bool ReadWorker::IsMappingPresent(const CowOperation* cow_op, loff_t requested_offset,
    ...
    if (replace_op) {
        size_t max_compressed_size = CowOpCompressionSize(cow_op);
