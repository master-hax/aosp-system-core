...
Snapuser::Snapuser(const std::string& misc_name, const std::string& cow_device,
...
bool Snapuser::InitializeWorkers() {
...
std::unique_ptr<CowReader> Snapuser::CloneReaderForWorker() {
...
void Snapuser::PrepareReadAhead() {
...
void Snapuser::CheckMergeCompletionStatus() {
    if (!merge_initiated_) {
...
void Snapuser::UnmapBufferRegion() {
...
void Snapuser::ReadBlocksToCache(const std::string& dm_block_device,
...
void Snapuser::ReadBlocks(const std::string partition_name, const std::string& dm_block_device) {
    ...
    for (int i = 0; i < num_threads; i++) {
        std::async(std::launch::async, &Snapuser::ReadBlocksToCache, this, dm_block_device,
...
uint64_t Snapuser::GetBufferMetadataOffset() {
    CowHeader header;
    reader_->GetHeader(&header);
 ...
 *
 */
size_t Snapuser::GetBufferMetadataSize() {
    CowHeader header;
    reader_->GetHeader(&header);
...
size_t Snapuser::GetBufferDataOffset() {
    CowHeader header;
    reader_->GetHeader(&header);
 ...
 */
size_t Snapuser::GetBufferDataSize() {
    CowHeader header;
    reader_->GetHeader(&header);
...
struct BufferState* Snapuser::GetBufferState() {
    CowHeader header;
    reader_->GetHeader(&header);
