...
class ReadAhead {
  public:
    ReadAhead(const std::string& cow_device, const std::string& backing_device,
              const std::string& misc_name, std::shared_ptr<Snapuser> snapuserd);
    bool RunThread();
  ...
  private:
    ...
    int PrepareReadAhead(uint64_t* source_offset, int* pending_ops, std::vector<uint64_t>& blocks,
    ...
    std::shared_ptr<Snapuser> snapuserd_;
...
class Worker {
  public:
    Worker(const std::string& cow_device, const std::string& backing_device,
           ...
           const std::string& base_path_merge, std::shared_ptr<Snapuser> snapuserd);
    bool RunThread();
    bool RunMergeThread();
...
class Snapuser : public std::enable_shared_from_this<Snapuser> {
  public:
    Snapuser(const std::string& misc_name, const std::string& cow_device,
    ...
    std::shared_ptr<Snapuser> GetSharedPtr() { return shared_from_this(); }
