  ...
  public:
    void SetUp() override {
        ...
        if (avail > kMaxFree) {
            ...
            LOG(INFO) << "Allocating " << (avail - kMaxFree) << " bytes of file " << big_file->path;
    ...
    static constexpr auto kDelta = 4_MiB;
...
TEST_F(ImageManagerTest, CreateImageEnoughSpace) {
    ...
    ASSERT_TRUE(
            image_manager_->CreateBackingImage(kImageName, avail - kDelta, IImageManager::CREATE_IMAGE_DEFAULT))
...
TEST_F(ImageManagerTest, CreateImageNoSpace) {
    ...
    auto res = image_manager_->CreateBackingImage(kImageName, avail + kDelta, IImageManager::CREATE_IMAGE_DEFAULT,
                                                  callback);
