...
TEST_F(SnapshotTest, CreateSnapshot) {
    ...
    ASSERT_TRUE(sm->DeleteCowImage(lock_.get(), "test-snapshot"));
}
...
TEST_F(SnapshotTest, MergeCannotRemoveCow) {
    ...
    // gsid relies on sysprops to determine whether the image is mapped.
    // Unset this to fake a reboot.
    ASSERT_TRUE(android::base::SetProperty("gsid.mapped_image.test-snapshot-cow-img", ""));
...
TEST_F(SnapshotTest, FlashSuperDuringMerge) {
    ...
    ASSERT_TRUE(UnmapImageIfExists(init->image_manager(), "test_partition_b-cow-img"));
    FormatFakeSuper();
    ASSERT_TRUE(CreatePartition("test_partition_b", kDeviceSize));
    ASSERT_TRUE(init->NeedSnapshotsInFirstStageMount());
    ASSERT_TRUE(init->CreateLogicalAndSnapshotPartitions("super"));
