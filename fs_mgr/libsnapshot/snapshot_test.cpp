...
class SnapshotTest : public ::testing::Test {
  protected:
    ...
    android::fiemap::ImageManagerBridge* image_manager_ = nullptr;
};
...
TEST_F(SnapshotTest, CreatePartialSnapshot) {
    ASSERT_TRUE(AcquireLock());
    ...
    // Scope so delete can re-acquire the status file lock.
    {
        ...
        ASSERT_EQ(status.state, "created");
        ASSERT_EQ(status.device_size, kDeviceSize);
        ASSERT_EQ(status.snapshot_size, kSnapshotSize);
    }
...
int main(int argc, char** argv) {
    ...
    sm = SnapshotManager::NewForTesting();
    if (!sm) {
