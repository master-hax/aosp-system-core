...
TEST_F(SnapshotUpdateTest, SuperOtaMetadataTest) {
    ...
    ASSERT_TRUE(SnapshotManager::CreateOtaMetadataOnSuper(info));
    std::string scratch_device = GetOtaMetadataPartition();
    ASSERT_NE(scratch_device, "");
    ASSERT_NE(MapOtaMetadataPartition(scratch_device), "");
    ASSERT_TRUE(SnapshotManager::CleanupMetadataScratch(info));
}
