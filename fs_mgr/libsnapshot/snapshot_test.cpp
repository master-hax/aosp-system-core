...
class ImageManagerTest : public SnapshotTest {
  public:
    void SetUp() override {
        ...
        auto avail = buf.f_bsize * buf.f_bavail;
        if (avail > kMaxFree) {
            ...
            LOG(INFO) << "Allocating " << (avail - kMaxFree) << " bytes on /data";
            ASSERT_EQ(0, fallocate(big_file->fd, 0, 0, avail - kMaxFree));

            // recompute available space on /data
            ASSERT_EQ(0, statvfs(kUserDataDevice, &buf)) << strerror(errno);
            avail = buf.f_bsize * buf.f_bavail;
            ASSERT_LE(avail, kMaxFree);
        }
        LOG(INFO) << "Remaining free space on /data is " << avail << " bytes";
    }
    void TearDown() override {
        EXPECT_TRUE(!image_manager_->BackingImageExists(kImageName) ||
                    image_manager_->DeleteBackingImage(kImageName));
    }
    constexpr auto kMaxFree = 512_MiB;
    constexpr auto kDelta = 4_MiB;
    constexpr const char* kImageName = "my_image";
...
TEST_F(ImageManagerTest, CreateImageEnoughSpace) {
    if (avail <= kDelta) {
        GTEST_SKIP() << "/data is (almost) full (" << avail << " bytes available), skipping";
    }
    ASSERT_TRUE(
            image_manager_->CreateBackingImage(kImageName, avail - kDelta, CREATE_IMAGE_DEFAULT))
            << "Should be able to create image with size = " << (avail - kDelta) << " bytes";
...
TEST_F(ImageManagerTest, CreateImageNoSpace) {
    ...
    auto callback = [&](auto current, auto) {
        allocated = current;
        return true;
    };

    auto res = image_manager_->CreateBackingImage(kImageName, avail + kDelta, CREATE_IMAGE_DEFAULT,
                                                  callback);
    ASSERT_FALSE(res) << "Should not be able to create image with size = " << (avail + kDelta)
                      << " bytes because only " << avail << " bytes are free";
    ASSERT_EQ(-ENOSPC, res.status());
    ASSERT_LE(allocated, avail) << "Should not be able to allocate more than the available space";
}
