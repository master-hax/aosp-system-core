...
TEST_F(SnapshotTest, FirstStageMountAndMerge) {
    ...
    ASSERT_TRUE(sm->CreateLogicalAndSnapshotPartitions("super"));

    ASSERT_TRUE(AcquireLock());

    // Validate that we have a snapshot device.
    SnapshotManager::SnapshotStatus status;
    ASSERT_TRUE(sm->ReadSnapshotStatus(lock_.get(), "test_partition_b", &status));
    ASSERT_EQ(status.state, SnapshotManager::SnapshotState::Created);

    DeviceMapper::TargetInfo target;
    auto dm_name = sm->GetSnapshotDeviceName("test_partition_b", status);
    ASSERT_TRUE(sm->IsSnapshotDevice(dm_name, &target));
    ASSERT_EQ(DeviceMapper::GetTargetType(target.spec), "snapshot");
}
