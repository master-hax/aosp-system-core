...
class Snapuserd : public std::enable_shared_from_this<Snapuserd> {
  public:
    ...
    void SetTotal_Ra_Blocks_Merged(int x) { total_ra_blocks_merged_ = x; }
    int GetTotal_Ra_Blocks_Merged() { return total_ra_blocks_merged_; }

  private:
    bool IsChunkIdMetadata(chunk_t chunk);
    chunk_t GetNextAllocatableChunkId(chunk_t chunk_id);

    bool GetRABuffer(uint64_t block, void* buffer);
    bool ReadMetadata();
    sector_t ChunkToSector(chunk_t chunk) { return chunk << CHUNK_SHIFT; }
    chunk_t SectorToChunk(sector_t sector) { return sector >> CHUNK_SHIFT; }
