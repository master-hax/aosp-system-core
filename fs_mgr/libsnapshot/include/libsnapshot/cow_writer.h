...
class CompressWorker {
  public:
    ...
    bool RunThread();
    void EnqueueCompressBlocks(const void* buffer, size_t num_blocks, uint64_t new_block);
    bool GetCompressedBuffers(std::vector<std::basic_string<uint8_t>>* compressed_buf);
    void Finalize();

  private:
    using CompressedVec = std::vector<std::pair<uint64_t, std::basic_string<uint8_t>>>;
    ...
    struct CompressWork {
        ...
        std::vector<std::pair<uint64_t, std::basic_string<uint8_t>>> compressed_data;
    };
    ...
    bool CompressBlocks(const void* buffer, size_t num_blocks, CompressedVec* compressed_data,
                        uint64_t new_block);
};
...
class CowWriter : public ICowWriter {
  ...
  private:
    ...
    bool CompressBlocks(size_t num_blocks, const void* data, uint64_t new_block);
