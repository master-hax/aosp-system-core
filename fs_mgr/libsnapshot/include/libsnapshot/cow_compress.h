...
class ICompressor {
  public:
    explicit ICompressor(uint32_t compression_level, uint32_t compression_block_sz)
        : compression_level_(compression_level), compression_block_sz_(compression_block_sz) {}
    ...
    uint8_t GetMaxCompressionSize() const { return compression_block_sz_; }
    [[nodiscard]] virtual std::basic_string<uint8_t> Compress(const void* data,
                                                              size_t length) const = 0;

  private:
    uint32_t compression_level_;
    uint32_t compression_block_sz_;
};
}  // namespace snapshot
}  // namespace android