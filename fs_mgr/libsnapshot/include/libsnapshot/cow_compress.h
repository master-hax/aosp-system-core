...
class ICompressor {
  public:
    ICompressor(uint32_t compression_level) : compression_level_(compression_level){};
    ...
    static std::unique_ptr<ICompressor> InitializeCompressor(CowCompression compression);

    void SetCompressionLevel(uint32_t compression_level) {
        compression_level_ = compression_level;
    };

    uint32_t GetCompressionLevel() { return compression_level_; }
