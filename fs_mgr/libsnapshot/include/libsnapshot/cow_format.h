...
static constexpr uint64_t kCowOpSourceInfoTypeBit = (15ULL << 60);

static inline void SetCowOpSourceInfoType(CowOperation* op, const uint8_t type) {
    op->source_info |= (uint64_t(type) << 60);
}
static inline uint64_t GetCowOpSourceInfoType(const CowOperation* op) {
    return op->source_info & kCowOpSourceInfoTypeBit;
}
