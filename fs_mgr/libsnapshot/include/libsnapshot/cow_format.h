...
// of Cow, eventually also replacing our on disk representation once compatibility requirements are
// met.
...
// Placeholder: we will store the bit for comrpession algorithm somewhere in op->source
static inline CowCompressionAlgorithm GetCompressionAlgorithm(const CowOperationV3* op) {
    if (op) {
...
int64_t GetNextOpOffset(const CowOperationV3& op, uint32_t cluster_size);
int64_t GetNextDataOffset(const CowOperationV3& op, uint32_t cluster_size);
