...
struct CowHeaderV3 : public CowHeader {
    // Size of the block sequence buffer, in bytes
...
static constexpr off_t GetResumeOffset(const CowHeaderV3& header) {
    return GetSequenceOffset(header) + header.sequence_buffer_size;
}

static constexpr off_t GetOpOffset(uint32_t op_index, const CowHeaderV3& header) {
    return GetResumeOffset(header) + (op_index * sizeof(CowOperationV3));
}

static constexpr off_t GetDataOffset(const CowHeaderV3& header) {
    return GetOpOffset(header.op_count_max, header) + header.op_count_max * sizeof(CowOperation);
}
