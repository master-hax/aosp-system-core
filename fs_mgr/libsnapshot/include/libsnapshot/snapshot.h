...
class SnapshotManager final {
  ...
  private:
    ...
    // A descriptor of a device-mapper device.
    struct DeviceDescriptor {
        // path like /dev/block/dm-1. The path may be filled in, but it may not exist if wait
        // time is zero.
        std::string path;
        // major:minor number. This should be preferred when creating DmTable targets because
        // path may not exist.
        std::string number;

        // Return a string that can be used in DmTable.
        const std::string& GetDeviceString() const;
    };

    // Map a COW image that was previous created with CreateCowImage. Note that not all fields
    // in descriptor are filled in even if true is returned.
    ...
    // |cow_device_string|
    bool MapCowTree(LockedFile* lock, const CreateLogicalPartitionParams& params,
