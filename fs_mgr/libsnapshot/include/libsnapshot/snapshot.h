  ...
  public:
    ...
    class IDeviceInfo {
      public:
        ...
        virtual void SetMetadataDir(const std::string& value) = 0;
...
class SnapshotManager final : public ISnapshotManager {
  ...
  public:
    ...
    static bool CreateOtaMetadataOnSuper(const IDeviceInfo* info = nullptr);

    // Clean up OTA metadata scratch device partition on super
    static bool CleanupMetadataScratch(const IDeviceInfo* info = nullptr);

    // Helper function to create scratch device on super for OTA metadata
    static bool CreateDynamicScratch(const IDeviceInfo* info, std::string* scratch_device);
