...
#include <libfiemap/binder_wrapper.h>
...
class SnapshotManager final {
  public:
    ...
    // Return a new SnapshotManager for testing.
    static std::unique_ptr<SnapshotManager> NewForTesting();
  ...
  private:
    ...
    using ImageManagerBridge = android::fiemap::ImageManagerBridge;

    SnapshotManager(const std::string& gsid_dir, const std::string& metadata_dir);

    // This is created lazily since it connects via binder.
    bool EnsureImageManager();

    // Helper function for tests.
    ImageManagerBridge* image_manager() const { return images_.get(); }
    ...
    // via MapDevice(). |snapshot_size| should be the number of sectors in the
    ...
    std::unique_ptr<ImageManagerBridge> images_;
};
