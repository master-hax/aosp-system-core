...
bool SnapshotMetadataUpdater::DeletePartitions() const {
    std::set<std::string_view> partitions_to_delete;
    ...
    for (auto* existing_partition : ListPartitionsWithSuffix(builder_, target_suffix_)) {
        ...
        if (iter == partitions_.end()) {
            partitions_to_delete.insert(existing_partition->name());
        }
    }
...
bool SnapshotMetadataUpdater::DeleteGroups() const {
    // std::set<std::string> metadata_groups;
    // for (const auto& metadata_group : partition_metadata_.groups) {
    //   metadata_groups.insert(metadata_group.name);
    // }
