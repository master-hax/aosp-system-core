...
int main(int argc, char** argv) {
    ...
    if (!android::base::SetProperty("virtual_ab.userspace.snapshots.enabled", "1")) {
        return testing::AssertionFailure()
               << "Failed to enable property: virtual_ab.userspace.snapshots.enabled";
    }

    pid_t pid = android::snapshot::GetPidByName("snapuserd");
    if (pid != 0) {
        if (kill(pid, SIGKILL) < 0) {
            return testing::AssertionFailure() << "Failed to kill snapuserd process";
        }
    }
