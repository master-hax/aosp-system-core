...
class AutoMemBasedDir : public AutoDevice {
  public:
    ...
    bool SoftReset() { return RmdirRecursive(tmp_path()) && Mkdir(tmp_path()); }
...
SnapshotFuzzEnv::SnapshotFuzzEnv() {
    ...
    if (fake_root_ == nullptr || !fake_root_->HasDevice()) return;
    loop_control_ = std::make_unique<LoopControl>();
    mapped_super_ = MapSuper(fake_root_->persist_path(), loop_control_.get(), &fake_super_);
}
...
bool SnapshotFuzzEnv::InitOk() const {
    ...
    if (loop_control_ == nullptr) return false;
    if (mapped_super_ == nullptr) return false;
    return true;
}

bool ZeroFill(const std::string& file, size_t size) {
    std::string zeros(size, '\0');
    if (!WriteStringToFile(zeros, file)) {
        PLOG(ERROR) << "Cannot write zeros to " << file;
        return false;
    }
    return true;
}
...
bool SnapshotFuzzEnv::SoftReset() {
    if (!fake_root_->SoftReset()) return false;
    if (!ZeroFill(super(), SUPER_IMAGE_SIZE)) return false;
    return true;
}

std::unique_ptr<IImageManager> SnapshotFuzzEnv::CreateFakeImageManager(const std::string& path) {
...
std::unique_ptr<AutoDevice> SnapshotFuzzEnv::MapSuper(const std::string& fake_persist_path,
                                                      LoopControl* control,
                                                      std::string* fake_super) {
    auto super_img = fake_persist_path + "/super.img";
    if (!ZeroFill(super_img, SUPER_IMAGE_SIZE)) {
        return nullptr;
    }

    if (!CreateLoopDevice(control, super_img, 1s, fake_super)) {
        return nullptr;
    }
