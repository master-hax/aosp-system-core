...
bool Snapuserd::CommitMerge(int num_merge_ops) {
    ...
    if (read_ahead_feature_ && read_ahead_ops_.size() > 0) {
        BufferState* ra_state =
                reinterpret_cast<BufferState*>((char*)mapped_addr_ + sizeof(CowHeader));
...
bool Snapuserd::ReadAheadIOCompleted() {
    ...
    BufferState* ra_state = reinterpret_cast<BufferState*>((char*)mapped_addr_ + sizeof(CowHeader));
...
bool Snapuserd::MmapMetadata() {
    ...
    if (header.major_version >= 2 && header.buffer_size > 0) {
        total_mapped_addr_length_ = sizeof(CowHeader) + BUFFER_REGION_DEFAULT_SIZE;
...
bool Snapuserd::Start() {
    ...
    bool ra_thread = (read_ahead_feature_ && (read_ahead_ops_.size() > 0));
