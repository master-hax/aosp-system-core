...
uint64_t ToLittleEndian(uint64_t value) {
    ...
    // Check if system is already Little Endian
    if (*(char*)&num == 1) return value;
...
class BufferSink : public IByteSink {
  public:
    ...
    void* GetPayloadBuffer(size_t size) {
        if ((buffer_size_ - buffer_offset_) < size) return NULL;
...
class Snapuserd final {
  public:
    ...
    int Run(void);
    int ReadDmUserHeader(void);
    int WriteDmUserPayload(size_t size);
    int ConstructKernelCowHeader(void);
  ...
  private:
    ...
    std::vector<std::unique_ptr<uint8_t[]>> vec;

    // Key - Chunk ID
    // Value - cow operation
    std::map<chunk_t, const CowOperation*> chunk_map;
...
int Snapuserd::ConstructKernelCowHeader(void) {
    ...
    dh->magic = ToLittleEndian(SNAP_MAGIC);
    dh->valid = ToLittleEndian(SNAPSHOT_VALID);
    dh->version = ToLittleEndian(SNAPSHOT_DISK_VERSION);
    dh->chunk_size = ToLittleEndian(CHUNK_SIZE);
...
int Snapuserd::ProcessZeroOp(void) {
...
int Snapuserd::ReadData(chunk_t chunk, size_t size) {
    ...
    assert((read_size & (BLOCK_SIZE - 1)) == 0);

    while (read_size > 0) {
        const CowOperation* cow_op = chunk_map[chunk_key];
        assert(cow_op != NULL);
        int result;
...
int Snapuserd::ReadDiskExceptions(chunk_t chunk, size_t read_size) {
    ...
    assert(divresult.quot < vec.size());
    ...
    auto ptr = std::move(vec[divresult.quot]);
    memcpy(buffer, ptr.get(), size);
    vec[divresult.quot] = std::move(ptr);
 ...
 * 8: Chunk-id representing a data page is stored in map. Key is the
 ...
 */
int Snapuserd::ReadMetadata(void) {
    reader_ = std::make_unique<CowReader>();
    CowHeader header;

    reader_->Parse(cow_fd_);

    reader_->GetHeader(&header);

    assert(header.magic == kCowMagicNumber);
    assert(header.major_version == kCowVersionMajor);
    assert(header.minor_version == kCowVersionMinor);
    ...
    while (!cowop_iter_->Done()) {
        ...
        de->old_chunk = ToLittleEndian(cow_op->new_block);
        de->new_chunk = ToLittleEndian(next_free);

        LOG(DEBUG) << "Old-chunk: " << de->old_chunk << "New-chunk: " << de->new_chunk;

        // Store operation pointer in the map. Note, new-chunk ID is the key
        chunk_map[next_free] = cow_op;
        ...
        if (divresult.rem == NUM_SNAPSHOT_HDR_CHUNKS) {
            ...
            next_free += 1;
            if (cowop_iter_->Done()) vec.push_back(std::move(de_ptr));
        }
    }
...
int Snapuserd::ReadDmUserHeader(void) {
...
int Snapuserd::Run() {
    ...
    while (true) {
        ...
        LOG(DEBUG) << android::base::StringPrintf("read() from dm-user returned %d bytes:", ret);
        ...
        switch (header->type) {
            case DM_USER_MAP_READ: {
                ssize_t remaining_size = header->len;
                loff_t offset = 0;
                header->io_in_progress = 0;
                ret = 0;
                do {
                    size_t read_size = MIN(PAYLOAD_SIZE, remaining_size);
                    ...
                    if (header->sector == 0) {
                        ...
                        // map, then it points to a metadata page.
