...
int Snapuserd::ProcessReplaceOp(const CowOperation* cow_op) {
    LOG(DEBUG) << "ProcessReplaceOp invoked...";
...
int Snapuserd::ProcessCopyOp(const CowOperation* cow_op) {
    ...
    LOG(DEBUG) << "ProcessCopyOp invoked...";
...
int Snapuserd::ProcessZeroOp() {
    ...
    memset(buffer, 0, BLOCK_SIZE);
    LOG(DEBUG) << "ProcessZeroOp invoked...";
    return BLOCK_SIZE;
}
...
loff_t Snapuserd::GetSizeOfMergedOps(int merged_ops_cur_iter) {
    loff_t offset = 0;

    // Calculate the total offset in the COW device for which
    // merge operation is completed. Advance the merge operation pointer in the
    // vector.
    //
    // cowop_iter_ is already initialized in ReadMetadata(). Just resume the
    // merge process
    while (!cowop_iter_->MergeDone() && merged_ops_cur_iter) {
        const CowOperation* cow_op = &cowop_iter_->GetMergeIter();
        CHECK(cow_op != nullptr);

        if (cow_op->type == kCowFooterOp || cow_op->type == kCowLabelOp) {
            cowop_iter_->MergeNext();
            offset += sizeof(CowOperation);
            continue;
        }
        ...
        if (cow_op->type == kCowReplaceOp) offset += cow_op->data_length;
        offset += sizeof(CowOperation);
        merged_ops_cur_iter -= 1;
...
bool Snapuserd::ProcessMergeComplete(chunk_t chunk, void* buffer) {
    ...
    offset = GetMergeStartOffset(buffer, vec_[divresult.quot].get(), &unmerged_exceptions);

    merged_ops_cur_iter =
            GetNumberOfMergedOps(buffer, vec_[divresult.quot].get(), offset, unmerged_exceptions);

    // There should be atleast one operation merged in this cycle
    CHECK(merged_ops_cur_iter > 0);
    offset = GetSizeOfMergedOps(merged_ops_cur_iter);

    if (offset < 0) return false;

    header.offset += offset;
    reader_->UpdateMergeProgress(offset);
    if (!writer_->CommitMerge(&header)) {
...
int Snapuserd::ReadMetadata() {
    ...
    if (!reader_->ParseForMerge(cow_fd_)) {
    ...
    LOG(DEBUG) << "merge-offset: " << std::hex << header.offset;

    writer_ = std::make_unique<CowWriter>(options);
    writer_->InitializeForMerge(cow_fd_.get(), &header);
    ...
    while (!cowop_iter_->RDone()) {
        const CowOperation* cow_op = &cowop_iter_->RGet();
        ...
        if ((cow_op->type == kCowCopyOp || prev_copy_op)) {
            next_free += 1;
            if (IsChunkIdMetadata(next_free)) next_free += 1;
        }

        prev_copy_op = (cow_op->type == kCowCopyOp) ? true : false;
    ...
    // Reset the iters as we start afresh for merging process
    cowop_iter_->ResetIters();
