...
bool Snapuserd::ReadMetadata() {
    ...
    chunk_t prev_id = UINT64_MAX;
    std::map<uint64_t, const CowOperation*> map;
    std::set<uint64_t> dest_blocks;
    int pending_copy_ops = exceptions_per_area_ - num_ops;
    SNAP_LOG(INFO) << "Processing Copy-ops. num_ops done: " << num_ops << " Area: " << vec_.size()
                   << " Pending_copy_ops in this area: " << pending_copy_ops;
    while (!cowop_riter_->Done()) {
        do {
            const CowOperation* cow_op = &cowop_riter_->Get();
            if (IsMetadataOp(*cow_op)) {
            ...
            if (prev_id != UINT64_MAX) {
                int diff = (cow_op->new_block > prev_id) ? (cow_op->new_block - prev_id)
