...
ssize_t CompressedSnapshotReader::Read(void* buf, size_t count) {
    ...
    // Chop off the ending N bytes if the position is not block-aligned.
    size_t max_end_bytes = (offset_ + count) - (end_chunk * block_size_);

    MemoryByteSink sink(buf, count);

    ssize_t rv = ReadBlock(start_chunk, &sink, start_offset, {block_size_ - count});
    if (rv < 0) {
        return -1;
    }
    offset_ += rv;
    ...
    if (max_end_bytes) {
        ssize_t rv = ReadBlock(end_chunk, &sink, max_end_bytes);
        if (rv < 0) {
            return -1;
        }
        offset_ += rv;
