        ...
        int mode)
{
    ...
    if (mode == VERITY_MODE_NORMAL) {
        // allow operation with older dm-verity drivers that are unaware
        // of the mode parameter by omitting it
        if (sprintf(verity_params, "%s", table) < 0) {
            return -1;
        }
    } else if (sprintf(verity_params, "%s %d", table, mode) < 0) {
        return -1;
    }
...
int fs_mgr_load_verity_state(int *mode)
{
    ...
    if (TEMP_FAILURE_RETRY(read(fd, &s, sizeof(s))) != sizeof(s)) {
        ERROR("Failed to read %u bytes from %s (%s)\n", sizeof(s), fname, strerror(errno));
        goto out;
    }

    if (s.header != VERITY_STATE_HEADER) {
        ERROR("Unexpected verity state header (%u)\n", s.header);
        goto out;
    }
    ...
    if (s.mode != VERITY_MODE_NORMAL &&
        s.mode != VERITY_MODE_LOGGING) {
