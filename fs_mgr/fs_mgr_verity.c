...
#define VERITY_TABLE_FEC_ARGS_FORMAT \
    "use_fec_from_device %s fec_start %" PRIu64 " fec_blocks %" PRIu64 \
    " fec_roots %u ignore_zero_blocks"
        ...
        const struct verity_table_params *params)
{
    ...
    if (params->mode == VERITY_MODE_RESTART) {
        mode_flag = "restart_on_corruption";
    } else if (params->mode == VERITY_MODE_LOGGING) {
        mode_flag = "ignore_corruption";
    }

    if (params->ecc.valid) {
        if (mode_flag) {
            res = snprintf(buf, bufsize,
                    "%s 10 %s " VERITY_TABLE_FEC_ARGS_FORMAT,
                    params->table, mode_flag, params->ecc_dev, params->ecc.start / FEC_BLOCKSIZE,
                    params->ecc.blocks, params->ecc.roots);
        } else {
            res = snprintf(buf, bufsize,
                    "%s 9 " VERITY_TABLE_FEC_ARGS_FORMAT,
                    params->table, params->ecc_dev, params->ecc.start / FEC_BLOCKSIZE,
                    params->ecc.blocks, params->ecc.roots);
        }
    } else if (mode_flag) {
        res = snprintf(buf, bufsize, "%s 2 ignore_zero_blocks %s", params->table, mode_flag);
    } else {
