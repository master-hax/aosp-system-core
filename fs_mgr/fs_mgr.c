    ...
    if (!strcmp(fs_type, "ext4")) {
        ...
        if (access(TUNE2FS_BIN, X_OK)) {
            ...
            if (fd >= 0) {
                ...
                int has_quota = (sb.s_feature_ro_compat & cpu_to_le32(EXT4_FEATURE_RO_COMPAT_QUOTA)) != 0;
...
static int mount_with_alternatives(struct fstab *fstab, int start_idx, int *end_idx, int *attempted_idx)
{
    ...
    for (i = start_idx;
         ...
         i++) {
            ...
            int force_check = do_quota(fstab->recs[i].blk_device, fstab->recs[i].fs_type, &fstab->recs[i]);
