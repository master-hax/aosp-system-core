...
static int read_supber_block(int fd, struct ext4_super_block *sb)
{
    off64_t ret;
        ...
        if (access(TUNE2FS_BIN, X_OK)) {
            ...
            if (fd >= 0) {
                ...
                if (ioctl(fd, BLKSSZGET, &block_size) < 0) {
                    ERROR("Not get the block size '%s': %s\n", blk_device, strerror(errno));
                    close(fd);
                    return;
                }
                reserved_blocks = rec->reserved_size / block_size;
                ...
                if (ret == 0 && ext4_r_blocks_count(&sb) == reserved_blocks) {
                    INFO("Not needs to set reserved blocks again.\n");
                    return;
                }
                close(fd);
            } else {
