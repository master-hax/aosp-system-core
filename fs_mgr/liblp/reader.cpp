...
static bool ReadMetadataHeader(Reader* reader, LpMetadata* metadata) {
    ...
    static constexpr uint32_t kKnownIncompatFlags = 0;
    uint32_t invalid_flags = header.incompat_flags & ~kKnownIncompatFlags;
    if (invalid_flags) {
        LERROR << "Logical partition metadata header has incompatible flags: " << std::hex
               << invalid_flags;
        return false;
    }
