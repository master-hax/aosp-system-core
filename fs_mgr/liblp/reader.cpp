...
static bool ReadLogicalPartitionGeometry(int fd, LpMetadataGeometry* geometry) {
    ...
    if (!SeekFile(fd, 0, SEEK_SET) || !ReadFully(fd, buffer.get(), LP_METADATA_GEOMETRY_SIZE)) {
        return false;
    }
    ...
    if (!SeekFile(fd, -LP_METADATA_GEOMETRY_SIZE, SEEK_END) ||
        !ReadFully(fd, buffer.get(), LP_METADATA_GEOMETRY_SIZE)) {
        return false;
    }
...
static bool ValidateMetadataHeader(LpMetadataHeader& header) {
    // To compute the header's checksum, we have to temporarily set its checksum
    // field to 0.
    uint32_t checksum = header.header_checksum;
    header.header_checksum = 0;
    header.header_checksum = CRC32(reinterpret_cast<const uint8_t*>(&header), sizeof(header));
...
static std::unique_ptr<LpMetadata> ParseMetadata(int fd) {
    ...
    if (!ReadFully(fd, &metadata->header, sizeof(metadata->header))) {
        return nullptr;
    }
    ...
    if (!ReadFully(fd, buffer.get(), header.tables_size)) {
        return nullptr;
    }
...
std::unique_ptr<LpMetadata> ReadMetadata(const char* block_device, uint32_t slot_number) {
    ...
    if (!SeekFile(fd, offset, SEEK_SET)) {
        return nullptr;
    }
    ...
    if (!SeekFile(fd, offset, SEEK_END)) {
        return nullptr;
    }
...
std::unique_ptr<LpMetadata> ReadFromImageFile(const char* file) {
    ...
    if (!SeekFile(fd, LP_METADATA_GEOMETRY_SIZE, SEEK_SET)) {
        return nullptr;
    }
