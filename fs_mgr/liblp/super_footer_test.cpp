...
TEST(superfooter, SuperAVBFooter) {
  ...
  sprintf(cmd, "lpmake \
    ...
    --metadata-slots %d \
    --device %s:%d:%d:%d \
    --partition %s:readonly:%d:default \
    --image %s=%s \
    --partition %s:readonly:%d:default \
    --image %s=%s \
    --partition %s:readonly:%d:default \
    ...
    --output=%s \
    --footer ",
          512, 512, "super", 2, "super", 6 * FAKE_PARTITION_SIZE, 0, 0,
          "system", FAKE_PARTITION_SIZE, "system", system_path.c_str(),
          "vendor", FAKE_PARTITION_SIZE, "vendor", vendor_path.c_str(),
          "product", FAKE_PARTITION_SIZE, "product", product_path.c_str(),
          super_path.c_str());
