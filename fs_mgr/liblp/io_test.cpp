...
static int CreateFakeDisk(off_t size) {
...
TEST(liblp, UpdateAnyMetadataSlot) {
    ...
    ASSERT_EQ(ReadMetadata(fd, kMetadataSlots), nullptr);
...
// reserved space.
TEST(liblp, ExportTooManyPartitions) {
    unique_ptr<MetadataBuilder> builder = CreateDefaultBuilder();
    ASSERT_NE(builder, nullptr);
    ...
    for (size_t i = 0; i < max_partitions; i++) {
        ...
        Partition* partition =
                builder->AddPartition(to_string(i), TEST_GUID, LP_PARTITION_ATTR_NONE);
    ...
    // Check that adding one more overflows the metadata allotment.
    Partition* partition = builder->AddPartition("final", TEST_GUID, LP_PARTITION_ATTR_NONE);
    EXPECT_NE(partition, nullptr);
