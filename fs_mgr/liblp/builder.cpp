...
std::unique_ptr<LinearExtent> MetadataBuilder::ExtendFinalExtent(
    ...
    auto new_extent = std::make_unique<LinearExtent>(num_sectors, extent->device_index(),
                                                     extent->end_sector());
    if (IsRegionAllocated(*new_extent.get()) || IsRegionCovered(free_list, *new_extent.get())) {
...
bool MetadataBuilder::IsRegionCovered(const std::vector<Interval>& regions,
...
bool MetadataBuilder::IsRegionAllocated(const LinearExtent& candidate) const {
    for (const auto& partition : partitions_) {
        for (const auto& extent : partition->extents()) {
            LinearExtent* linear = extent->AsLinearExtent();
