...
std::unique_ptr<MetadataBuilder> MetadataBuilder::NewForUpdate(const IPartitionOpener& opener,
    ...
    auto source_groups = std::move(metadata->groups);
    for (const auto& source_group : source_groups) {
        std::string group_name = GetPartitionGroupName(source_group);

        // The default/catch-all group is not suffixed.
        if (group_name == "default") {
            metadata->groups.emplace_back(source_group);
            continue;
        }

        std::string slot_suffix = GetPartitionSlotSuffix(group_name);
        if (slot_suffix.empty() || slot_suffix != source_slot_suffix) {
            LERROR << "Invalid group name for slot " << source_slot_suffix << ": " << group_name;
            return nullptr;
        }
        std::string new_name =
                group_name.substr(0, group_name.size() - slot_suffix.size()) + target_slot_suffix;

        auto new_group = source_group;
        if (!UpdatePartitionGroupName(&new_group, new_name)) {
            LERROR << "Group name too long: " << new_name;
            return nullptr;
        }
        metadata->groups.emplace_back(new_group);
    }
