...
std::unique_ptr<MetadataBuilder> MetadataBuilder::New(const LpMetadata& metadata,
    ...
    if (opener) {
        for (size_t i = 0; i < builder->block_devices_.size(); i++) {
            std::string partition_name = fs_mgr::GetBlockDevicePartitionName(builder->block_devices_[i]);
...
std::unique_ptr<MetadataBuilder> MetadataBuilder::NewForUpdate(const IPartitionOpener& opener,
    ...
    // See comments for ModifyMetadataForRetrofitDap.
    auto super_device = GetMetadataSuperBlockDevice(*metadata.get());
    if (fs_mgr::GetBlockDevicePartitionName(*super_device) != "super" &&
        IsRetrofitDynamicPartitionsDevice()) {
        if (!ModifyMetadataForRetrofitDap(metadata.get(), source_slot_number, target_slot_number)) {
            return nullptr;
        }
    }

    if (IPropertyFetcher::GetInstance()->GetBoolProperty("ro.virtualab.enabled", false)) {
        if (!ModifyMetadataForVirtualAb(metadata.get(), source_slot_number, target_slot_number)) {
            return nullptr;
        }
    }
...
// On non-retrofit DAP devices there is only one location for metadata: the
// super partition. update_engine will remove and resize partitions as
// needed. On the other hand, for retrofit DAP devices, we'll need to
// translate block device and group names to update their slot suffixes.
bool MetadataBuilder::ModifyMetadataForRetrofitDap(
        LpMetadata* metadata,
        uint32_t source_slot_number, uint32_t target_slot_number) {
    ...
    for (const auto& source_block_device : source_block_devices) {
        std::string partition_name = fs_mgr::GetBlockDevicePartitionName(source_block_device);
...
bool MetadataBuilder::ModifyMetadataForVirtualAb(
        LpMetadata* metadata,
        uint32_t source_slot_number, uint32_t target_slot_number) {
    ...
    std::map<size_t /* old index */, size_t /* new index */> group_index_map;
    ...
    for (auto& group : metadata->groups) {
        ...
        if (slot_suffix == target_slot_suffix)
            continue;
    ...
    for (auto& partition : metadata->partitions) {
        ...
        // Replace source slot suffix with target slot suffix.
        if (slot_suffix == source_slot_suffix) {
            std::string new_name = partition_name.substr(0, partition_name.size() - slot_suffix.size())
                    + target_slot_suffix;
    ...
    for (auto* g : new_group_ptrs)
        new_groups.emplace_back(std::move(*g));
...
static bool CompareBlockDevices(const LpMetadataBlockDevice& first,
    ...
    return first.first_logical_sector == second.first_logical_sector && first.size == second.size &&
           fs_mgr::GetBlockDevicePartitionName(first) == fs_mgr::GetBlockDevicePartitionName(second);
}
...
bool MetadataBuilder::IsRetrofitDynamicPartitionsDevice() {
    return IPropertyFetcher::GetInstance()->GetBoolProperty("ro.boot.dynamic_partitions_retrofit", false);
}
...
std::string MetadataBuilder::GetBlockDevicePartitionName(uint32_t index) const {
    return index <= block_devices_.size() ?
        fs_mgr::GetBlockDevicePartitionName(block_devices_[index]) : "";
}
