...
bool MetadataBuilder::GrowPartition(Partition* partition, uint64_t requested_size) {
    ...
    for (size_t i = 1; i < extents.size(); i++) {
        ...
        intervals.emplace_back(aligned, current.start);
    }
    ...
    if (last_free_extent_start <= geometry_.last_logical_sector) {
        intervals.emplace_back(last_free_extent_start, geometry_.last_logical_sector + 1);
    }
    ...
    for (const auto& interval : intervals) {
        ...
        if (interval.end - interval.start >= sectors_needed) {
            auto extent = std::make_unique<LinearExtent>(sectors_needed, interval.start);
        ...
        auto extent = std::make_unique<LinearExtent>(interval.end - interval.start, interval.start);
        sectors_needed -= extent->num_sectors();
        new_extents.push_back(std::move(extent));
    }
    if (sectors_needed) {
