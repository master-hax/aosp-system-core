...
bool MetadataBuilder::Init(uint64_t blockdevice_size, uint32_t metadata_max_size,
    ...
    assert(geometry_.last_logical_sector >= geometry_.first_logical_sector);
    return true;
}

Partition* MetadataBuilder::AddPartition(const std::string& name, const uint8_t guid[16],
                                         uint32_t attributes) {
    if (name.empty()) {
...
bool MetadataBuilder::GrowPartition(Partition* partition, uint64_t requested_size) {
    ...
    if (sectors_needed) {
        ...
        assert(first_sector <= geometry_.last_logical_sector);
...
std::unique_ptr<LpMetadata> MetadataBuilder::Export() {
    ...
    for (const auto& partition : partitions_) {
        ...
        part.first_extent_index = (uint32_t)metadata->extents.size();
    ...
    metadata->header.partitions.num_entries = (uint32_t)metadata->partitions.size();
    metadata->header.extents.num_entries = (uint32_t)metadata->extents.size();
    return metadata;
}
