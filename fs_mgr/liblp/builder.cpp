...
bool MetadataBuilder::VerifyExtentsAgainstSourceMetadata(
        const std::vector<std::string>& partitions, const MetadataBuilder& source_metadata_builder,
        uint32_t source_slot_number, uint32_t target_slot_number) const {
    for (const auto& base_name : partitions) {
        auto target_partition_name = base_name + SlotSuffixForSlotNumber(target_slot_number);
        const auto target_partition = FindPartition(target_partition_name);
        if (!target_partition) {
        ...
        const auto source_partition = source_metadata_builder.FindPartition(source_partition_name);
        if (!source_partition) {
