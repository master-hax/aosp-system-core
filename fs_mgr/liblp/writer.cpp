...
static bool ValidateGeometryAndMetadata(int fd, const LpMetadata& metadata, std::string* blob) {
...
bool FlashPartitionTable(int fd, const LpMetadata& metadata, uint32_t slot_number) {
    ...
    if (!ValidateGeometryAndMetadata(fd, metadata, &metadata_blob)) {
        return false;
    }
...
bool UpdatePartitionTable(int fd, const LpMetadata& metadata, uint32_t slot_number) {
    ...
    // on the given block device.
    std::string blob;
    if (!ValidateGeometryAndMetadata(fd, metadata, &blob)) {
        return false;
    }
