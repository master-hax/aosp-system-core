...
class SparseBuilder {
  ...
  private:
    ...
    std::vector<std::unique_ptr<FILE, decltype(&fclose)>> temp_files_;
...
int SparseBuilder::OpenImageFile(const std::string& file) {
    ...
    std::unique_ptr<FILE, decltype(&fclose)> temp_file(tmpfile(), fclose);
    if (!temp_file) {
        PERROR << "tmpfile failed";
        return -1;
    }
    int temp_fd = fileno(temp_file.get());
    if (temp_fd < 0) {
