...
bool SparseBuilder::Export(const char* file) {
    ...
    int ret = sparse_file_write(device_images_.back().get(), fd, false, true, false);
    if (ret != 0) {
...
bool SparseBuilder::ExportFiles(const std::string& output_dir) {
    struct stat s;
    if (stat(output_dir.c_str(), &s) < 0) {
        PERROR << "stat failed: " << output_dir;
        return false;
    }
    if (!S_ISDIR(s.st_mode)) {
        LERROR << "Folder required to write split sparse images.";
        return false;
    }
    ...
    for (size_t i = 0; i < device_images_.size(); i++) {
        ...
        android::base::unique_fd fd(open(path.c_str(), O_CREAT | O_RDWR | O_TRUNC, 0644));
        if (fd < 0) {
...
uint64_t SparseBuilder::BlockToSector(uint32_t block) const {
    return (block * block_size_) / LP_SECTOR_SIZE;
}

bool SparseBuilder::Build() {
    if (sparse_file_add_fill(device_images_.back().get(), 0, LP_PARTITION_RESERVED_BYTES, 0) < 0) {
    ...
    if (!AddData(device_images_.back().get(), all_metadata_, first_sector)) {
        return false;
    }
