...
TEST(liblp, UpdateBlockDeviceInfo) {
    ...
    EXPECT_EQ(builder->GetBlockDeviceInfo().size, device_info.size);
    EXPECT_EQ(builder->GetBlockDeviceInfo().alignment, device_info.alignment);
    EXPECT_EQ(builder->GetBlockDeviceInfo().alignment_offset, device_info.alignment_offset);

    device_info.alignment = 0;
    device_info.alignment_offset = 2048;
    builder->UpdateBlockDeviceInfo(device_info);
    EXPECT_EQ(builder->GetBlockDeviceInfo().alignment, 4096);
    EXPECT_EQ(builder->GetBlockDeviceInfo().alignment_offset, device_info.alignment_offset);

    device_info.alignment = 8192;
    device_info.alignment_offset = 0;
    builder->UpdateBlockDeviceInfo(device_info);
    EXPECT_EQ(builder->GetBlockDeviceInfo().alignment, 8192);
    EXPECT_EQ(builder->GetBlockDeviceInfo().alignment_offset, 2048);
}
