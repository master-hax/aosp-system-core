...
TEST(liblp, BuildComplex) {
    ...
    EXPECT_EQ(system->extents().size(), 2);
    EXPECT_EQ(vendor->extents().size(), 1);
...
TEST(liblp, BuilderExport) {
    ...
    EXPECT_EQ(exported->partitions.size(), 2);
    EXPECT_EQ(exported->extents.size(), 3);
