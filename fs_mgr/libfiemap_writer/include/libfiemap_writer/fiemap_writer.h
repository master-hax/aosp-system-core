...
class FiemapWriter final {
  public:
    ...
    // invoked the file is being initialized (if create is true). It receives the bytes written,
    // and if false is returned, the operation will fail.
    static FiemapUniquePtr Open(const std::string& file_path, uint64_t file_size,
                                bool create = true, std::function<bool(uint64_t)> progress = {});
