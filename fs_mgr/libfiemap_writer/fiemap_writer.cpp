...
static bool block_device_to_name(uint32_t major, uint32_t minor, std::string* bdev_name) {
    ...
    std::string sysfs_path =
            ::android::base::StringPrintf("%s/%u:%u", FiemapWriter::kSysDevBlock, major, minor);
...
static bool device_mapper_stack_pop(const std::string& bdev, std::string* bdev_raw) {
    ...
    std::string dm_leaf_dir =
            ::android::base::StringPrintf("%s/%s/slaves", FiemapWriter::kSysBlock, bdev.c_str());
    ...
    return device_mapper_stack_pop(bdev_next, bdev_raw);
}

static bool file_to_block_device_path(const std::string& file_path, std::string* bdev_path) {
    struct stat sb;
    ...
    std::string bdev;
    if (!block_device_to_name(major(sb.st_dev), minor(sb.st_dev), &bdev)) {
    ...
    std::string bdev_raw;
    if (!device_mapper_stack_pop(bdev, &bdev_raw)) {
    ...
    *bdev_path =
            ::android::base::StringPrintf("%s/%s", FiemapWriter::kBlockDevDir, bdev_raw.c_str());
...
static bool get_block_device_params(int bdev_fd, const std::string& bdev_path, uint64_t* blocksz,
                                    uint64_t* bdev_size) {
...
static bool perform_file_checks(const std::string& file_path, uint64_t file_size, uint64_t blocksz,
                                uint32_t* fs_type) {
...
static bool allocate_file(int file_fd, const std::string& file_path, uint64_t blocksz,
                          uint64_t file_size) {
...
static bool pin_file(int file_fd, const std::string& file_path, uint32_t fs_type) {
...
#if 0
static bool pin_file_status(int file_fd, const std::string& file_path, uint32_t fs_type) {
    if (fs_type == EXT4_SUPER_MAGIC) {
        // No pinning necessary for ext4. The blocks, once allocated, are expected
        // to be fixed.
        return true;
...
static void log_extent(uint32_t num, const struct fiemap_extent& ext) {
    LOG(INFO) << "Extent #" << num;
...
static bool read_fiemap(int file_fd, const std::string& file_path,
                        std::vector<struct fiemap_extent>* extents) {
    uint64_t fiemap_size =
            sizeof(struct fiemap_extent) + FiemapWriter::kMaxExtents * sizeof(struct fiemap_extent);
    ...
    fiemap->fm_extent_count = FiemapWriter::kMaxExtents;
    ...
    for (uint32_t i = 0; i < fiemap->fm_mapped_extents; i++, extent++) {
        // log_extent(i + 1, *extent);
        if (extent->fe_flags & FiemapWriter::kUnsupportedExtentFlags) {
    ...
    if (!last_extent_seen) {
        ...
        if (fiemap->fm_mapped_extents == FiemapWriter::kMaxExtents) {
            LOG(ERROR) << "File is too fragmented, needs more than " << FiemapWriter::kMaxExtents
                       << " extents.";
        }
        extents->clear();
    }
...
FiemapUniquePtr FiemapWriter::Open(const std::string& file_path, uint64_t file_size, bool create) {
    ...
    if (!file_to_block_device_path(abs_path, &bdev_path)) {
    ...
    if (!get_block_device_params(bdev_fd, bdev_path, &blocksz, &bdevsz)) {
    ...
    uint32_t fs_type;
    if (!perform_file_checks(abs_path, file_size, blocksz, &fs_type)) {
    ...
    if (create) {
        if (!allocate_file(file_fd, abs_path, blocksz, file_size)) {
    ...
    if (!pin_file(file_fd, file_path, fs_type)) {
        cleanup(file_path, create);
    ...
    if (!fmap) {
        LOG(ERROR) << "Failed to allocate FiemapWriter";
        cleanup(file_path, create);
        return nullptr;
    }

    if (!read_fiemap(file_fd, abs_path, &fmap->extents_)) {
...
bool FiemapWriter::Write(off64_t off, uint8_t* buffer, uint64_t size) {
    ...
    if (!pin_file_status(file_fd_, file_path_, fs_type_)) {
