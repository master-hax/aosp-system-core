...
bool FiemapWriter::Flush() const {
    if (file_path_.empty()) {
        PLOG(ERROR) << "Failed to flush, file path is empty";
        return false;
    }
    if (fsync(bdev_fd_)) {
...
void FiemapWriter::Close() {
    if (!file_path_.empty()) {
        // implict flush before we close
        Flush();
    }
    fiemap_.clear();
    file_fd_.reset();
    bdev_fd_.reset();
    file_size_ = file_offset_ = bdev_offset_ = 0;
    file_path_.clear();
    bdev_path_.clear();
}
...
bool FiemapWriter::Write(off64_t off, uint8_t* buffer, uint64_t size) {
    if (file_path_.empty()) {
        LOG(ERROR) << "Failed write: File wasn't created";
        return false;
    }
...
uint64_t FiemapWriter::WriteExtent(const struct fiemap_extent& ext, uint8_t* buffer,
    ...
    if (!android::base::WriteFully(bdev_fd_, buffer, write_size)) {
        LOG(ERROR) << "Failed write extent, write " << bdev_path_ << " at " << bdev_offset
                   << " size " << write_size;
        return 0;
    }
...
bool FiemapWriter::FileToBlockDevPath(const std::string& file_path, std::string* bdev_path) {
    ...
    std::string bdev;
    if (!BlockDevToName(major(sb.st_dev), minor(sb.st_dev), &bdev)) {
    ...
    LOG(DEBUG) << "Popped device (" << bdev_raw << ") from device mapper stack starting with ("
               << bdev << ")";
    *bdev_path = ::android::base::StringPrintf("%s/%s", kBlockDevDir, bdev_raw.c_str());
    return true;
}
...
bool FiemapWriter::DeviceMapperStackPop(const std::string& bdev, std::string* bdev_raw) {
    ...
    std::string dm_leaf_dir =
            ::android::base::StringPrintf("%s/%s/slaves", kSysBlock, bdev.c_str());
    ...
    // recursively call into the block device we found in order to pop the device mapper stack.
...
void FiemapWriter::LogExtent(int extent_num, const struct fiemap_extent* ext) {
    LOG(INFO) << "Extent #" << extent_num;
