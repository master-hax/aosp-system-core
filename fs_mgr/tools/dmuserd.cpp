...

{
    ...
    while (1) {
        ...
        if (!android::base::ReadFully(control_fd, &msg, sizeof(msg))) {
        ...
        if (msg.type == DM_USER_MAP_WRITE) {
            if (!android::base::ReadFully(control_fd, base, msg.len)) {
                if (errno == ENOTBLK) return 0;
        ...
        if (!android::base::WriteFully(control_fd, &msg, sizeof(msg))) {
            if (errno == ENOTBLK) return 0;
        ...
        if (msg.type == DM_USER_MAP_READ) {
            if (!android::base::WriteFully(control_fd, base, msg.len)) {
                if (errno == ENOTBLK) return 0;
