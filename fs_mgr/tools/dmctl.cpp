...
using DmDevice = ::android::dm::DmDevice;
...
static int DmCreateCmdHandler(int argc, char** argv) {
    if (argc <= 1) {
        std::cerr << "DmCreateCmdHanler: Invalid arguments";
...
static int DmListCmdHandler(int argc, char** argv) {
    ...
    // all arguments here are ignored
    ...
    for (auto it = targets.begin(); it != targets.end(); it++) {
        std::cout << std::left << std::setw(20) << it->getName() << " : " << it->getVersion()
                  << std::endl;
    }
...
static CommandMap CmdMap[] = {{
                                  .cmd = "create",
                                  .handler = DmCreateCmdHandler,
                              },
                              {
                                  .cmd = "delete",
                                  .handler = DmDeleteCmdHandler,
                              },
                              {
                                  .cmd = "list",
                                  .handler = DmListCmdHandler,
                              },
                              {
                                  .cmd = "help",
...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < arraysize(CmdMap); i++) {
        if (CmdMap[i].cmd == argv[1]) {
            int err = CmdMap[i].handler(argc - 2, argv + 2);
            return err;
        }
    }
