...
class TargetParser final {
  public:
    ...
    std::unique_ptr<DmTarget> Next() {
        ...
        if (!NextUint64(&start_sector)) {
            std::cerr << "Expected start sector, got: " << LastArg() << std::endl;
            return nullptr;
        }
        if (!NextUint64(&num_sectors) || !num_sectors) {
            std::cerr << "Expected non-zero sector count, got: " << LastArg() << std::endl;
            return nullptr;
        }
        ...
        if (target_type == "zero") {
            ...
            if (!NextUint64(&physical_sector)) {
                std::cerr << "Expected sector, got: \"" << LastArg() << "\"" << std::endl;
                return nullptr;
            }
