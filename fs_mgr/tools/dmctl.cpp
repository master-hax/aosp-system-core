    ...
    std::unique_ptr<DmTarget> Next() {
        ...
        if (target_type == "zero") {
            ...
            return std::make_unique<DmTargetAndroidVerity>(start_sector, num_sectors, keyid,
                                                           block_device);
        } else if (target_type == "stripe") {
            if (!HasArgs(3)) {
                std::cerr << "Expected \"stripe\" <block_device0> <block_device1> <chunksize>" << std::endl;
                return nullptr;
            }
            ...
            return std::make_unique<DmTargetStripe>(start_sector, num_sectors, chunk_size, block_device0, block_device1);
