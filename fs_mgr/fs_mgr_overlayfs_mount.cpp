...
static bool fs_mgr_overlayfs_mount(const FstabEntry& entry) {
    ...
    std::vector<mount_info> mountinfo = ReadMountinfoFromFile("/proc/self/mountinfo");
    std::vector<mount_info> submounts;
    ...
    for (const auto& entry : mountinfo) {
        ...
        if (std::find_if(submounts.begin(), submounts.end(), [&entry](const auto& it) {
                return android::base::StartsWith(entry.mount_point, it.mount_point + "/") ||
                       it.mount_point == entry.mount_point;
            }) != submounts.end()) {
            continue;
        }
        auto erase_from = std::remove_if(submounts.begin(), submounts.end(), [&](const auto& it) {
            return android::base::StartsWith(it.mount_point, entry.mount_point + "/");
        });
        submounts.erase(erase_from, submounts.end());
        // Pick the last mount entry in case of over mounts.
        submounts.push_back(
                *std::find_if(mountinfo.rbegin(), mountinfo.rend(),
                              [&](const auto& it) { return it.mount_point == entry.mount_point; }));
    }
    ...
    std::vector<move_entry> moved_mounts;
    for (const auto& entry : submounts) {
