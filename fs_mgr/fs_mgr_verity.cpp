...
static int update_verity_table_blk_device(char *blk_device, char **table)
{
    std::string result, word;
    auto tokens = android::base::Split(*table, " ");

    for (const auto token : tokens) {
        if (strstr(token.c_str(), "/dev/block/") == token.c_str() &&
            strstr(blk_device, token.c_str()) == blk_device) {
            word = blk_device;
        } else {
            word = token;
        }
...
int fs_mgr_setup_verity(struct fstab_rec *fstab)
{
    ...
    if (fstab->fs_mgr_flags & MF_SLOTSELECT) {
        ...
        if (update_verity_table_blk_device(fstab->blk_device, &params.table))
            goto loaded;
    }
