...
static int format_ext4(const std::string& fs_blkdev, const std::string& fs_mnt_point,
    ...
    rc = logwrap_fork_execvp(arraysize(mke2fs_args), mke2fs_args, nullptr, false, LOG_KLOG, false,
                             nullptr);
    if (rc) {
...
static int format_f2fs(const std::string& fs_blkdev, uint64_t dev_sz, bool crypt_footer) {
    ...
    return logwrap_fork_execvp(arraysize(args), args, nullptr, false, LOG_KLOG, false, nullptr);
}
