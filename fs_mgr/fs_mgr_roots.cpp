...
int fs_mgr_ensure_path_mounted(struct fstab* fstab, const char* path, const char* mount_point) {
...
int fs_mgr_ensure_path_unmounted(struct fstab* fstab, const char* path) {
    const struct fstab_rec* v = fs_mgr_fstab_rec_for_path(fstab, path);
    if (v == nullptr) {
        LOG(ERROR) << "unknown volume for path [" << path << "]";
        return -1;
    }
    ...
    int result = umount(v->mount_point);
    if (result == -1) {
        PLOG(WARNING) << "Failed to umount " << v->mount_point;
    }
    return result;
}
