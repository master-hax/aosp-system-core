...
int EnsurePathMounted(Fstab* fstab, const std::string& path, const std::string& mount_pt) {
...
int EnsurePathUnmounted(Fstab* fstab, const std::string& path) {
    auto rec = GetEntryForPath(fstab, path);
    if (rec == nullptr) {
        LERROR << "unknown volume for path [" << path << "]";
        return -1;
