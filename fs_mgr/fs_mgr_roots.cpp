...
static MountState get_mount_state(const std::string& mount_point) {
    Fstab mounted_fstab;
...
int fs_mgr_ensure_path_mounted(Fstab* fstab, const std::string& path, const std::string& mount_pt) {
    auto rec = fs_mgr_get_entry_for_path(fstab, path);
    if (rec == nullptr) {
    ...
    auto mounted = get_mount_state(rec->mount_point);
    ...
    mkdir(mount_point.c_str(), 0755);  // in case it doesn't already exist
...
int fs_mgr_ensure_path_unmounted(Fstab* fstab, const std::string& path) {
    ...
    if (result == -1) {
        PLOG(WARNING) << "Failed to umount " << rec->mount_point;
    }
    return result;
}
