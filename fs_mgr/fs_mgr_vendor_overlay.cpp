...
const std::vector<const std::string> kVendorOverlaySourceDirs = {
        "/system/vendor_overlay/"s,
        "/product/vendor_overlay/"s,
};
...
struct MountPoint {
    std::string overlay_top;
    std::string mount_dir;

    MountPoint(std::string top, std::string dir)
        : overlay_top(std::move(top)), mount_dir(std::move(dir)) {}
};

std::vector<struct MountPoint> fs_mgr_get_vendor_overlay_dirs(const std::string& vndk_version) {
    std::vector<struct MountPoint> vendor_overlay_dirs;
...
bool fs_mgr_vendor_overlay_mount(const struct MountPoint& mount_point) {
    const auto vendor_mount_point = kVendorTopDir + mount_point.mount_dir;
    ...
    const auto source_directory = mount_point.overlay_top + "/" + mount_point.mount_dir;
