...
uint64_t GetPrimaryVBMetaOffset() { return 0; }

uint64_t GetBackupVBMetaOffset(uint64_t super_vbmeta_size) {
  return super_vbmeta_size / 2;
}

uint64_t ComputePartitionSize(const LpMetadata &metadata,
...
uint64_t LogicalToPhysicalOffset(const LpMetadata &metadata,
                                 const LpMetadataPartition &partition,
  ...
  for (uint32_t index = first_extent; index < last_extent; index++) {
    ...
    if (sectors_sum * LP_SECTOR_SIZE <= offset &&
      ...
      uint64_t physical_offset = extent.target_data * LP_SECTOR_SIZE +
...
GetPartitionVBMetaData(const LpMetadata &metadata,
  ...
  uint64_t physical_offset =
      LogicalToPhysicalOffset(metadata, partition, footer->vbmeta_offset);
...
bool ValidateVBMeta(int fd, uint64_t offset) {
  ...
  std::unique_ptr<uint8_t[]> buffer =
      std::make_unique<uint8_t[]>(AVB_MAGIC_LEN);
  if (!android::base::ReadFully(fd, buffer.get(), AVB_MAGIC_LEN)) {
    PERROR << __PRETTY_FUNCTION__ << " super read vbmeta " << AVB_MAGIC_LEN
           << " bytes failed";
    return false;
  }
