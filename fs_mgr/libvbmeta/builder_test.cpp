...
TEST(BuilderTest, VBMetaTableBasic) {
    ...
    Result<uint8_t> vbmeta_slot = builder->AddVBMetaImage("vbmeta" /* vbmeta_name */
    );
    EXPECT_TRUE(vbmeta_slot);
    ...
    EXPECT_EQ(table->descriptors[0].vbmeta_slot, 0);
    ...
    EXPECT_EQ(table->descriptors[1].vbmeta_slot, 2);
    ...
    EXPECT_EQ(table->descriptors[2].vbmeta_slot, 1);
