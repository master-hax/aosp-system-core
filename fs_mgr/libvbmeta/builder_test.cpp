...
TEST(BuilderTest, SuperVBMetaBasic) {
    ...
    EXPECT_EQ(vbmeta->header.descriptors_size, SUPER_VBMETA_DESCRIPTOR_SIZE * 2 + 13);
    ...
    for (int i = 0; i < 48; i++) EXPECT_EQ(vbmeta->descriptors[0].reserved[i], 0);
    ...
    for (int i = 0; i < 48; i++) EXPECT_EQ(vbmeta->descriptors[1].reserved[i], 0);
