...
bool ParseSuperVBMetaDescriptor(const void* buffer, uint32_t size,
    ...
    for (int p = 0; p < size;) {
        ...
        memcpy(descriptor.partition_name, (char*)buffer + p, descriptor.partition_name_length);
...
bool ReadSuperVBMeta(int fd, uint64_t offset, SuperVBMeta* vbmeta) {
    ...
    if (!android::base::ReadFully(fd, header_buffer.get(), SUPER_VBMETA_HEADER_SIZE)) {
        PERROR << __PRETTY_FUNCTION__ << " super vbmeta read " << SUPER_VBMETA_HEADER_SIZE
               << " bytes failed";
        return false;
    }
    ...
    if (!android::base::ReadFully(fd, descriptors_buffer.get(), vbmeta->header.descriptors_size)) {
        PERROR << __PRETTY_FUNCTION__ << " super vbmeta read " << vbmeta->header.descriptors_size
               << " bytes failed";
        return false;
    }

    return rv & ParseSuperVBMetaDescriptor(descriptors_buffer.get(),
                                           vbmeta->header.descriptors_size, &vbmeta->descriptors);
}
