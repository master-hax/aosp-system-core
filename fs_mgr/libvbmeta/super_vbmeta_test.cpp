...
bool WriteFakeAVBFooter(int fd, uint64_t vbmeta_offset, uint64_t vbmeta_size, uint8_t start_symbol,
                        uint8_t end_symbol) {
    std::unique_ptr<uint8_t[]> buffer = std::make_unique<uint8_t[]>(FAKE_PARTITION_SIZE);
    ...
    memcpy(&buffer[FAKE_PARTITION_SIZE - AVB_FOOTER_SIZE], AVB_FOOTER_MAGIC, AVB_FOOTER_MAGIC_LEN);
    memcpy(&buffer[FAKE_PARTITION_SIZE - AVB_FOOTER_SIZE + 20], &offset, sizeof(uint64_t));
    memcpy(&buffer[FAKE_PARTITION_SIZE - AVB_FOOTER_SIZE + 28], &size, sizeof(uint64_t));
    ...
    SparsePtr file(sparse_file_new(512, FAKE_PARTITION_SIZE), sparse_file_destroy);
...
TEST(SuperVBMetaTest, SuperVBMetaBasic) {
    ...
    cmd << "lpmake"
        ...
        << "super"
        << ":" << 6 * FAKE_PARTITION_SIZE << ":" << 0 << ":" << 0 << " --partition "
        << "system"
    ...
    EXPECT_TRUE(android::fs_mgr::ReadSuperVBMeta(vbmeta_fd.get(), 0, &super_vbmeta));
    ...
    EXPECT_TRUE(android::fs_mgr::ReadSuperPrimaryVBMeta(vbmeta_fd.get(), &super_vbmeta_primary));
    SuperVBMeta super_vbmeta_backup;
    EXPECT_TRUE(android::fs_mgr::ReadSuperBackupVBMeta(vbmeta_fd.get(), &super_vbmeta_backup,
                                                       SUPER_VBMETA_TOTAL_SIZE));
    ...
    std::string serial_super_vbmeta = android::fs_mgr::SerializeSuperVBMeta(super_vbmeta);
    ...
    for (const auto& partition : super_vbmeta.descriptors) {
        EXPECT_EQ(android::fs_mgr::ReadDataFromSuper(super_fd.get(), partition.vbmeta_offset),
                  vbmetas[partition.partition_name].first);
        EXPECT_EQ(android::fs_mgr::ReadDataFromSuper(
                          super_fd.get(), partition.vbmeta_offset + partition.vbmeta_size - 1),
