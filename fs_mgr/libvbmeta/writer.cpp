...
std::string SerializeSuperVBMeta(const SuperVBMeta &input) {
  ...
  vbmeta.append(reinterpret_cast<const char *>(&input.header),
                input.header.header_size);

  for (const auto &descriptor : input.descriptors) {
    vbmeta.append(reinterpret_cast<const char *>(&descriptor),
                  SUPER_VBMETA_DESCRIPTOR_SIZE);
    vbmeta.append(descriptor.partition_name, descriptor.partition_name_length);
  }

  // Ensure the size of super_vbmeta.img is a multiple of
  // SUPER_VBMETA_TOTAL_SIZE
  uint32_t resize_to = std::ceil(static_cast<double>(vbmeta.size()) /
