...
bool UpdateSuperVBMetaPartition(const std::string& super_vbmeta_name,
    ...
    // Update super vbmeta
    if (vbmeta_image) {
        Result<uint8_t> slot_number = builder.AddVBMetaImage(vbmeta_image_name);
        if (!slot_number) {
            LERROR << __PRETTY_FUNCTION__ << " " << slot_number.error();
            return false;
        }
        builder.ExportVBMetaImageToFile(slot_number.value(), *vbmeta_image);
    } else {
