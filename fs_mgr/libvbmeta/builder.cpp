...
Result<void> SuperVBMetaBuilder::Build() {
    for (const auto& [vbmeta_name, file_path] : images_path_) {
        ...
        Result<uint8_t> vbmeta_slot = AddVBMetaImage(vbmeta_name);
        if (!vbmeta_slot) {
            return vbmeta_slot.error();
        }

        Result<void> rv_export_vbmeta_image =
                ExportVBMetaImageToFile(vbmeta_slot.value(), content.value());
        if (!rv_export_vbmeta_image) {
            return rv_export_vbmeta_image.error();
        }
    }
    return {};
}
...
void SuperVBMetaBuilder::DeleteVBMetaImage(const std::string& vbmeta_name) {
    ...
    if (desc != table_.descriptors.end()) {
        ...
        table_.header.in_use &= ~(1 << desc->vbmeta_slot);
...
std::unique_ptr<VBMetaTable> SuperVBMetaBuilder::ExportVBMetaTable() {
    ...
    std::string serial = SerializeVBMetaTable(table_);
    ::SHA256(reinterpret_cast<const uint8_t*>(serial.c_str()), table_.header.total_size,
             table_.header.checksum);
...
Result<void> SuperVBMetaBuilder::ExportVBMetaTableToFile() {
    ...
    if (!rv_write_primary_vbmeta_table) {
        return rv_write_primary_vbmeta_table.error();
    }
...
Result<void> SuperVBMetaBuilder::ExportVBMetaImageToFile(const uint8_t vbmeta_slot,
                                                         const std::string& vbmeta_image) {
    Result<void> rv_write_vbmeta_image =
            WriteVBMetaImage(super_vbmeta_fd_, vbmeta_slot, vbmeta_image);
    if (!rv_write_vbmeta_image) {
        return rv_write_vbmeta_image.error();
    }

    Result<void> rv_validate_vbmeta_image =
            ValidateVBMetaImage(super_vbmeta_fd_, vbmeta_slot, vbmeta_image);
...
bool WriteToSuperVBMetaFile(const std::string& super_vbmeta_file,
    ...
    if (!rv_build) {
        LERROR << rv_build.error().message();
        return false;
    }

    Result<void> rv_export = builder.ExportVBMetaTableToFile();
    if (!rv_export) {
        LERROR << rv_export.error().message();
        return false;
    }
