...
SuperVBMetaBuilder::SuperVBMetaBuilder(const SuperVBMeta &vbmeta) {
  for (const SuperVBMetaDescriptor &descriptor : vbmeta.descriptors) {
    Add(std::string(descriptor.partition_name,
...
bool SuperVBMetaBuilder::AddPartitionImage(const LpMetadataPartition &partition,
  ...
  SparsePtr source(sparse_file_import(source_fd, true, true),
                   sparse_file_destroy);
  TemporaryFile tf;
  if (tf.fd < 0) {
...
bool SuperVBMetaBuilder::Add(const std::string &partition_name,
...
std::unique_ptr<SuperVBMeta> SuperVBMetaBuilder::Export() {
  ...
  for (const auto &vbmeta : vbmetas_) {
    ...
    descriptors_size += SUPER_VBMETA_DESCRIPTOR_SIZE +
  ...
  memset(super_vbmeta->header.checksum, 0,
         sizeof(super_vbmeta->header.checksum));
  ...
  ::SHA256(reinterpret_cast<const uint8_t *>(serial.c_str()),
           super_vbmeta->header.total_size, super_vbmeta->header.checksum);
  return super_vbmeta;
}

bool SuperVBMetaBuilder::Export(const std::string &file) {
  android::base::unique_fd fd(TEMP_FAILURE_RETRY(
      open(file.c_str(), O_CREAT | O_RDWR | O_TRUNC | O_CLOEXEC, 0644)));
  if (fd.get() < 0) {
