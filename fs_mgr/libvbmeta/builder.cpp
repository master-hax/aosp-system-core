...
Result<bool> VBMetaTableBuilder::AddVBMetaInfoForPartition(const LpMetadataPartition& partition,
    ...
    if (source_fd < 0) {
        return ErrnoError() << "Couldn't open partition image file " << file;
    }
    ...
    if (unsparsed_file.fd < 0) {
        return ErrnoError() << "Couldn't make temporary file";
    }
    ...
    if (source) {
        ...
        if (rv) {
            return ErrnoError() << "sparse_file_write failed with code " << rv;
        }
    ...
    if (super_fd < 0) {
        return ErrnoError() << "Couldn't open super image file " << super_file_;
    }
...
Result<bool> VBMetaTableBuilder::Export(const std::string& file) {
    ...
    if (fd.get() < 0) {
        return ErrnoError() << "Couldn't export vbmeta table in " << file;
    }
...
bool WriteToVBMetaTableFile(const std::string& super_file, const std::string& vbmeta_table_file,
    ...
    Result<bool> rv_build = builder.Build();
    if (!rv_build) {
        LERROR << rv_build.error().message();
        return false;
    }

    Result<bool> rv_export = builder.Export(vbmeta_table_file);
    if (!rv_export) {
