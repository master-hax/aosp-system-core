...
constexpr const static size_t kSectorSize = 512;
...
constexpr const static char* kSpecialRows[] = {
...
struct SnapshotInfo {
    ...
    static std::unique_ptr<SnapshotInfo> PrepareTargetPartition(
            const std::string& name, size_t device_size, size_t cow_size, size_t chunk_size) {
    ...
    void InitiateMerge() const {
        ...
        ASSERT_TRUE(merge_table.AddTarget(make_unique<DmTargetSnapshot>(
                target.spec.sector_start,
                target.spec.length,
                data[0],
                data[1],
                SnapshotStorageMode::Merge, chunk_size)));
...
TEST_P(LibSnapshotBenchmark, CopyToDmSnapshot) {
    TestCopyAndVerify(GetParam(), FLAGS_times,
        std::bind(&SnapshotInfo::PrepareTargetPartition, _1, _2, kAutoCowSize, kDefaultChunkSize),
        CopySourceToTarget(),
...
DEFINE_bool(merge, false,
            "Initiate merge existing 'fio' dm-snapshot device");
DEFINE_bool(wait_merge, false,
