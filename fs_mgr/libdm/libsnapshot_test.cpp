...
constexpr const static char* kSuper = "/dev/block/by-name/super";
constexpr const static char* kRandom = "/dev/urandom";
constexpr const static char* kMemInfo = "/proc/meminfo";
constexpr const static char* kDropCaches = "/proc/sys/vm/drop_caches";
constexpr const static char* kCpuPath = "/sys/devices/system/cpu/";
constexpr const static auto kMapTimeout = 3000ms;
constexpr const static size_t kRandReadTimes = 1000;
constexpr const static auto kMemPollTime = 50ms;
constexpr const static size_t kDefaultChunkSize = 8;

// Row tags
// Sample mean
constexpr const static char* kAverageRow = "SampMean";
// Uncorrected sample standard deviation, or standard deviation of the sample
// https://en.wikipedia.org/wiki/Standard_deviation#Uncorrected_sample_standard_deviation
constexpr const static char* kSdRow = "PopSd";
// (Corrected) Sample standard deviation
// https://en.wikipedia.org/wiki/Standard_deviation#Corrected_sample_standard_deviation
constexpr const static char* kSampleSdRow = "SampSd";
constexpr const static char* kMinMemFreeRow = "MinFree";
constexpr const static char* kMaxMemFreeRow = "MaxFree";
constexpr const static char* kMinMemAvailRow = "MinAvail";
constexpr const static char* kMaxMemAvailRow = "MaxAvail";
constexpr const static char* kSpecialRows[] = {kAverageRow,    kSdRow,         kSampleSdRow,
                                               kMinMemFreeRow, kMaxMemFreeRow, kMinMemAvailRow,
                                               kMaxMemAvailRow};
constexpr const static size_t kMinTagWidth = 6;
constexpr const static size_t kColumnGap = 1;
...
constexpr const static size_t kCowSlackSize = 200_MiB;
constexpr const static size_t kNoCowSize = 0;
constexpr const static size_t kAutoCowSize = ((size_t)-1);
...
struct SnapshotInfo {
    ...
    static std::unique_ptr<SnapshotInfo> PrepareTargetPartition(const std::string& name,
                                                                size_t device_size, size_t cow_size,
                                                                size_t chunk_size) {
        ...
        // Create base.
        auto builder = MetadataBuilder::New(
                kSuper, android::fs_mgr::SlotNumberForSlotSuffix(fs_mgr_get_slot_suffix()));
        if (!builder) {
        ...
        if (!partition) partition = builder->AddPartition(base_name, 0);
        if (!partition) {
        ...
        if (cow_size != kNoCowSize) {
            ...
            if (!partition) partition = builder->AddPartition(cow_name, 0);
            if (!partition) {
        ...
        params.partition_name = cow_name;
        if (!CreateLogicalPartition(params, &path)) {
        ...
        {
            unique_fd cow_fd(
                    TEMP_FAILURE_RETRY(open(info->cow->path().c_str(), O_WRONLY | O_BINARY)));
            char zeros[4096] = {0};
        ...
        EXPECT_TRUE(snap_table.AddTarget(make_unique<DmTargetSnapshot>(
                ...
                SnapshotStorageMode::Persistent, chunk_size)));
    ...
    static std::unique_ptr<SnapshotInfo> ForPartition(const std::string& name) {
        ...
        info->snapshot =
                std::make_unique<TempDevice>(name + "_snapshot" + fs_mgr_get_other_slot_suffix());
    ...
    static void DeleteDevicesForPartition(const std::string& name) {
        ...
        info->ForEach(
                [](auto* device) { FAIL() << device->name() << " should have been destroyed"; });
    }

    // Device to write to. Can be either base or snapshot.
    TempDevice* target() const { return snapshot ? snapshot.get() : base.get(); };
    ...
    void InitiateMerge() const {
        ...
        ASSERT_TRUE(DeviceMapper::Instance().GetTableInfo(snapshot->name(), &table));
        ASSERT_EQ(1, table.size());
        const DeviceMapper::TargetInfo& target = table[0];
        auto data = android::base::Split(target.data, " ");
        ASSERT_EQ(4, data.size());
        size_t chunk_size;
        ASSERT_TRUE(android::base::ParseUint(data[3], &chunk_size));

        DmTable merge_table;
        ASSERT_TRUE(merge_table.AddTarget(
                make_unique<DmTargetSnapshot>(target.spec.sector_start, target.spec.length, data[0],
                                              data[1], SnapshotStorageMode::Merge, chunk_size)));
...
bool ForFd(android::base::borrowed_fd fd, std::function<bool(char* buf, size_t len)> func,
    ...
    while ((bytes_read = TEMP_FAILURE_RETRY(
                    read(fd.get(), &buf, std::min(sizeof(buf), range.second - end)))) > 0) {
...
std::string Sha256Fd(android::base::borrowed_fd fd,
    ...
    if (!ForFd(fd, update, range)) return {};
...
class PrintingTimer : public android::base::Timer {
  public:
    ...
    void Checkpoint(const std::string& tag) {
        ...
        if (!silent_) {
            std::cout << "[TIME] " << tag << ": " << (duration_since_start - last_print_).count()
                      << "ms (" << duration_since_start.count() << "ms since start)\n";
            std::cout.flush();
        }
        durations_.emplace_back(tag, duration_since_start - last_print_);
        last_print_ = duration_since_start;
    }
    void StoreTotal() { durations_.emplace_back("total", last_print_); }

    static void PrintRow(const size_t first_cell_width, const std::string& first_cell,
    ...
    static size_t GetNumberWidth(size_t timers_count) {
        return std::max<size_t>(
                ...
                strlen(*std::max_element(&kSpecialRows[0], &kSpecialRows[arraysize(kSpecialRows)],
                                         [](auto a, auto b) { return strlen(a) < strlen(b); })));
    }
    ...
    void PrintTitle(size_t timers_count) const {
        PrintRow(GetNumberWidth(timers_count), "#", GetColumnWidths(),
                 [&](auto& os, auto column) { os << durations_[column].first; });
    }

    void Print(size_t row, size_t timers_count) const {
        if (row == 0) PrintTitle(timers_count);
        PrintRow(GetNumberWidth(timers_count), std::to_string(row), GetColumnWidths(),
                 [&](auto& os, auto column) { os << durations_[column].second.count(); });
    }
    ...
    static void PrintSummary(const std::vector<PrintingTimer>& timers) {
        ...
        PrintRow(number_width, kAverageRow, column_widths,
                 [&](auto& os, auto column) { os << ToString(averages[column], kMinTagWidth); });
        PrintRow(number_width, kSdRow, column_widths,
                 [&](auto& os, auto column) { os << ToString(pop_sds[column], kMinTagWidth); });
        PrintRow(number_width, kSampleSdRow, column_widths,
                 [&](auto& os, auto column) { os << ToString(sample_sds[column], kMinTagWidth); });
    }
...
struct MemRecord {
    ...
    static void PrintSummary(const std::vector<PrintingTimer>& timers,
        ...
        PrintingTimer::PrintRow(
                PrintingTimer::GetNumberWidth(timers.size()), kMinMemFreeRow,
                timers[0].GetColumnWidths(), [&](auto& os, auto column) {
                    os << GetSizeString((double)(merged_mem_records[column].min_mem_free * 1024));
                });
        PrintingTimer::PrintRow(
                PrintingTimer::GetNumberWidth(timers.size()), kMaxMemFreeRow,
                timers[0].GetColumnWidths(), [&](auto& os, auto column) {
                    os << GetSizeString((double)(merged_mem_records[column].max_mem_free * 1024));
                });
        PrintingTimer::PrintRow(
                PrintingTimer::GetNumberWidth(timers.size()), kMinMemAvailRow,
                timers[0].GetColumnWidths(), [&](auto& os, auto column) {
                    os << GetSizeString((double)(merged_mem_records[column].min_mem_avail * 1024));
                });
        PrintingTimer::PrintRow(
                PrintingTimer::GetNumberWidth(timers.size()), kMaxMemAvailRow,
                timers[0].GetColumnWidths(), [&](auto& os, auto column) {
...
struct MemRecorder {
  ...
  private:
    ...
    void Record() {
        char buf[120];
        ASSERT_TRUE(android::base::ReadFully(TEMP_FAILURE_RETRY(open(kMemInfo, O_RDONLY)), buf,
                                             sizeof(buf)));
...
// Magic scripts from
// https://docs.google.com/document/d/1Dw_mClBSXr9HUKYV0594ykT74_ldWY-Op652rMfRCC8/edit#heading=h.30fo5k52h8s8
void DropCaches() {
...
struct TargetReaderWriter {
    ...
    virtual void Write(android::base::borrowed_fd source_fd,
                       const std::string& target_path) const = 0;
    ...
    void RandRead(const std::string& target_path, size_t max_size,
                  size_t bytes_each_read = kSectorSize, size_t times = 100000,
...
struct CopySourceToTarget : public TargetReaderWriter {
    ...
    void Write(android::base::borrowed_fd source_fd,
               const std::string& target_path) const override {
...
class LibSnapshotBenchmark : public ::testing::TestWithParam<const char* /* name */> {
  public:
    ...
    static std::string GetSourcePartitionDevice(const std::string& name) {
        std::string path;
        EXPECT_TRUE(DeviceMapper::Instance().GetDmDevicePathByName(name + fs_mgr_get_slot_suffix(),
                                                                   &path));
        return path;
    }

    static void TestCopyAndVerify(
            const std::string& name, size_t repeat_times,
            const std::function<std::unique_ptr<SnapshotInfo>(const std::string&, size_t)>&
                    target_opener,
        ...
        std::cout << name << " source (" << source << "): " << source_size << " Bytes ("
                  << GetSizeString((double)source_size) << ")\n";
        ...
        for (size_t invocation = 0; invocation < repeat_times; ++invocation) {
            ...
            for (size_t bytes_each_read = 512; bytes_each_read <= 16_MiB;
                 bytes_each_read = bytes_each_read << 3) {
                target_reader_writer.RandRead(target, source_size, bytes_each_read, kRandReadTimes,
            ...
            if (target_merger) {
                ...
                // snapshot-merge with the table of base, but just replace it at the application
                // level.
                ...
                for (size_t bytes_each_read = 512; bytes_each_read <= 16_MiB;
                    ...
                    target_reader_writer.RandRead(target, source_size, bytes_each_read,
                                                  kRandReadTimes, bytes_each_read /* seed */);
                    checkpoint("rr" + GetSizeString(bytes_each_read));
                }
...
TEST_P(LibSnapshotBenchmark, CopyToDmSnapshot) {
    TestCopyAndVerify(GetParam(), FLAGS_times,
                      std::bind(&SnapshotInfo::PrepareTargetPartition, _1, _2, kAutoCowSize,
                                kDefaultChunkSize),
                      CopySourceToTarget(), std::bind(&SnapshotInfo::MergeAndWait, _1));
}

TEST_P(LibSnapshotBenchmark, CopyToDmLinear) {
    TestCopyAndVerify(
            GetParam(), FLAGS_times,
            std::bind(&SnapshotInfo::PrepareTargetPartition, _1, _2, kNoCowSize, kDefaultChunkSize),
            CopySourceToTarget(), nullptr);
}
...
TEST_P(LibSnapshotBenchmark, WriteRandomToDmSnapshot) {
    TestCopyAndVerify(GetParam(), FLAGS_times,
                      std::bind(&SnapshotInfo::PrepareTargetPartition, _1, _2, kAutoCowSize,
                                kDefaultChunkSize),
                      WriteRandomToTarget(), std::bind(&SnapshotInfo::MergeAndWait, _1));
}
...
TEST_P(LibSnapshotBenchmark, WriteRandomToDmLinear) {
    TestCopyAndVerify(
            GetParam(), FLAGS_times,
            std::bind(&SnapshotInfo::PrepareTargetPartition, _1, _2, kNoCowSize, kDefaultChunkSize),
            WriteRandomToTarget(), nullptr);
}

INSTANTIATE_TEST_SUITE_P(Vendor, LibSnapshotBenchmark, ::testing::Values("vendor"));
...
DEFINE_bool(setup, false,
            ...
            " testing");
DEFINE_bool(merge, false, "Initiate merge existing 'fio' dm-snapshot device");
DEFINE_bool(wait_merge, false, "Wait for merge to complete");
DEFINE_bool(teardown, false, "Destroy existing 'fio' device");
...
DEFINE_string(cow_size, "0" /* GetSizeString(kNoCowSize) */,
              ...
              "aren't created.");
DEFINE_uint64(chunk_size, kDefaultChunkSize,
              "Size of the chunk. Only valid for --setup and --cow_size is defined.");
...
int Setup() {
    ...
    auto info = SnapshotInfo::PrepareTargetPartition(kFioDeviceName, device_size, cow_size,
                                                     FLAGS_chunk_size);
    if (!info) {
    ...
    if (info->snapshot) std::cout << "snapshot=" << info->snapshot->path() << "\n";
...
int InitiateMerge() {
    ...
    info->InitiateMerge();
    std::cout << "snapshot_merge=" << info->target()->path() << "\n";
    info->Release();
    return 0;
}
...
int MergeComplete() {
    auto info = SnapshotInfo::ForPartition(kFioDeviceName);
    info->WaitMergeComplete();
    info->ProcessMergeCompleted();
    std::cout << "merged=" << info->target()->path() << "\n";
    info->Release();
    return 0;
}
...
int main(int argc, char** argv) {
    ...
    if (fio::FLAGS_merge) {
        return fio::InitiateMerge();
    }
    if (fio::FLAGS_wait_merge) {
        return fio::MergeComplete();
    }
