...
bool DeviceMapper::GetAvailableTargets(std::vector<DmTarget>* targets) {
    if (targets == nullptr) {
        LOG(ERROR) << "targets vector cannot be nullptr";
        return false;
    }

    if (!targets->empty()) {
        LOG(ERROR) << "targets vector MUST be empty";
        return false;
    }
...
bool DeviceMapper::GetAvailableDevices(std::vector<DmBlockDevice>* devices) {
    if (devices == nullptr) {
        LOG(ERROR) << "devices list cannot be nullptr";
        return false;
    }

    if (!devices->empty()) {
        LOG(ERROR) << "devices list MUST be empty";
        return false;
    }
