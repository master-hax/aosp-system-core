...
bool DeviceMapper::GetTableStatus(const std::string& name, std::vector<TargetInfo>* table) {
    ...
    for (uint32_t i = 0; i < io->target_count; i++) {
        if (cursor + sizeof(struct dm_target_spec) < sizeof(buffer)) {
            break;
        }
        ...
        uint32_t next_cursor = std::min(io->data_start + spec->next, (uint32_t)sizeof(buffer));
