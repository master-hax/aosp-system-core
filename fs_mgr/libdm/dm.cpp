...
std::unique_ptr<DmDevice> DeviceMapper::CreateDevice(const std::string& /* name */) {
    // Creates a new device mapper device with the name passed in
    return nullptr;
}

bool DeviceMapper::DeleteDevice(DmDevice* dev) {
...
bool DeviceMapper::GetAvailableTargets(std::vector<DmTarget>* targets) {
    ...
    int data_size = sizeof(struct dm_ioctl) + sizeof(struct dm_target_versions);
    // 16-bytes for the name
    data_size += DM_MAX_TYPE_NAME + 1;
    // dm wants every target spec to be aligned at 8-byte boundary
    data_size = DM_ALIGN(data_size);
    data_size *= 256;
    ...
    if (ioctl(fd_, DM_LIST_VERSIONS, io)) {
        LOG(ERROR) << "Failed to get DM_LIST_VERSIONS from kernel";
        return false;
    }
...
// find the device node for device mapper device with the given name
// and read its status from the kernel to construct a DmDevice object
// and return the same
std::unique_ptr<DmDevice> DeviceMapper::Find(const std::string& /* name */) {
    return nullptr;
}
