...
TEST(libdm, GetUnderlyingBlockDeviceByPath) {
    unique_fd tmp(CreateTempFile("file_1", 4096));
    ASSERT_GE(tmp, 0);
    LoopDevice loop(tmp, 10s);
    ASSERT_TRUE(loop.valid());
    ...
    DeviceMapper& dm = DeviceMapper::Instance();
    // Not a dm device, GetUnderlyingBlockDeviceByPath will return std::nullopt.
    ASSERT_FALSE(dm.GetUnderlyingBlockDeviceByPath(loop.device()));
    auto sub_block_device = dm.GetUnderlyingBlockDeviceByPath(dev.path());
