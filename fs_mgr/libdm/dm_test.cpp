...
static map<string, DmTargetTypeInfo> GetDmTargetInfo() {
...
bool VersionIsAtLeast(const DmTargetTypeInfo& info, uint32_t major, uint32_t minor,
                      uint32_t patch) {
    if (info.major_version() < major) {
        return false;
    }
    if (info.minor_version() < minor) {
        return false;
    }
    return info.patch_level() >= patch;
}

bool CheckSnapshotAvailability() {
    auto by_name = GetDmTargetInfo();
    auto iter = by_name.find("snapshot");
    if (iter == by_name.end()) {
        cout << "snapshot module not enabled; skipping test" << std::endl;
        return false;
    }
    if (!VersionIsAtLeast(iter->second, 1, 15, 0)) {
        cout << "snapshot module is too old; skipping test" << std::endl;
        return false;
    }

    iter = by_name.find("snapshot-merge");
    if (iter == by_name.end()) {
        cout << "snapshot-merge module not enabled; skipping test" << std::endl;
        return false;
    }
    if (!VersionIsAtLeast(iter->second, 1, 4, 0)) {
        cout << "snapshot-merge module is too old; skipping test" << std::endl;
        return false;
    }
    return true;
}
...
class SnapshotTestHarness final {
  ...
  private:
    ...
    static const uint64_t kCowDeviceSize = 1024 * 512;
