...
using ReadFully = android::base::ReadFully;
using WriteFully = android::base::WriteFully;
...
TEST(libdm, DmLinear) {
    ...
    ASSERT_TRUE(WriteFully(tmp1, message1, sizeof(message1)));
    ASSERT_TRUE(WriteFully(tmp2, message2, sizeof(message2)));
    ...
    unique_fd dev_fd(open(dev.path(), O_RDWR));
    ASSERT_GE(dev_fd, 0);

    // Test that each sector of our device is correctly mapped to each loop
    // device.
    char sector[512];
    ASSERT_TRUE(ReadFully(dev_fd, sector, sizeof(sector)));
