...
class DmTarget {
  public:
    DmTarget(const std::string& name, uint64_t start = 0, uint64_t end = 0)
    ...
    DmTarget(const struct dm_target_versions* vers) {
    ...
    uint64_t getSize(void) const {
        uint64_t size = end_ - start_;

        // returns [start_, end_] always
        return start_ ? size + 1 : size;
    }

    // Return string representation of the device mapper target version.
    std::string getVersion(void) const {
        return std::string(std::to_string(v0_) + "." + std::to_string(v1_) + "." +
                           std::to_string(v2_));
    }
    ...
    virtual std::string Serialize(void) const { return ""; }
  ...
  private:
    ...
    // Start and end sector number (logical) represented
