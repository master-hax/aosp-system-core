...
class IDeviceMapper {
  public:
    ...
    virtual bool GetTableStatus_IMA(const std::string& name, std::vector<TargetInfo>* table) = 0;
...
class DeviceMapper final : public IDeviceMapper {
  public:
    ...
    // not exist, or there were too many targets, the call will fail and return
    // false.
    bool GetTableStatus_IMA(const std::string& name, std::vector<TargetInfo>* table) override;
