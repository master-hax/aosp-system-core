...
class DeviceMapper final {
  public:
    ...
    // Same as CreateDevice(), except after creation, |path| contains the
    // result of calling DmGetDevicePathByName, and the path is guaranteed to
    // exist. If after |timeout_ms| the path is not available, the device will
    // be deleted and this function will return false.
    //
    // This variant should be preferred over CreateDevice(). It avoids a race
    // condition where, if another process deletes a device, CreateDevice may
    // acquire the same path. If this happens, the caller must make sure the
    // WaitForFile() has not early-returned because it detected the now-deleted
    // device path.
    //
