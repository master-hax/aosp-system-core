...
LIBSNAPSHOT_TEST=${DEVICE_TMP_DIR}/libsnapshot_benchmark
FIO=${DEVICE_TMP_DIR}/fio

function host_libsnapshot_benchmark() {
    local TARGET_ARCH=$($ANDROID_BUILD_TOP/build/soong/soong_ui.bash --dumpvar-mode TARGET_ARCH)
    echo ${ANDROID_TARGET_OUT_TESTCASES}/libsnapshot_benchmark/${TARGET_ARCH}/libsnapshot_benchmark
}

# source libsnapshot_benchmark.sh && prepare
function prepare() {
    [[ -d $ANDROID_BUILD_TOP ]] || {
    ...
    if [[ ! -f $ANDROID_PRODUCT_OUT/system/bin/fio ]] || [[ ! -f $(host_libsnapshot_benchmark) ]]; then
        $ANDROID_BUILD_TOP/build/soong/soong_ui.bash --make-mode fio libsnapshot_benchmark -j8
    fi
}
...
# run_benchmark_linear <device_size> <job_file>
function run_benchmark_linear() {
...
# run_benchmark_linear <device_size> <cow_size> <chunk_size> <job_file>
function run_benchmark_snapshot() {
    local device_size=$1; shift
...
function run_benchmark() {
    ...
    prepare_benchmark || return 1

    run_benchmark_linear ${device_size} $J/linear.fio \
        > ${R}linear.txt || return 1

    for override_chunk in 8 16 32 64 128; do
        print_status "Testing chunk size ${override_chunk}..."

        run_benchmark_snapshot ${device_size} ${cow_size} ${override_chunk} $J/snapshot-full.fio \
        ...
        for i in $percentages; do
            ...
            SIZE=${i}% run_benchmark_snapshot ${device_size} ${cow_size} ${override_chunk} $J/snapshot-incremental-w.fio \
...
# memory_benchmark <device_size> <cow_size> <chunk_size>
function memory_benchmark() {
    ...
    sleep 2
    run_benchmark_linear ${device_size} \
    ...
    sleep 2
    run_benchmark_snapshot ${device_size} ${cow_size} ${chunk_size} \
    ...
    for i in 1 25 50 75; do
        ...
        sleep 2
        SIZE=${i}% run_benchmark_snapshot ${device_size} ${cow_size} ${chunk_size} \
