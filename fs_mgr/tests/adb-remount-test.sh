...
D=`adb_sh df -k </dev/null` &&
  H=`echo "${D}" | head -1` &&
  D=`echo "${D}" | grep -v " /vendor/.\{1,\}$" | grep "^overlay "` &&
  echo "${H}" &&
  echo "${D}" &&
...
if [ X"${D}" != X"${H}" ]; then
  ...
  D=`adb_sh df -k </dev/null` &&
    H=`echo "${D}" | head -1` &&
    D=`echo "${D}" | grep -v " /vendor/.\{1,\}$" | grep "^overlay " || true` &&
    [ -z "${D}" ] ||
    ( echo "${H}" && echo "${D}" && false ) ||
...
if ${overlayfs_needed}; then
  ...
  !(adb_sh grep -v "^overlay\ /vendor/.*\ overlay\ ro," /proc/mounts </dev/null | grep "^overlay " | grep " overlay ro,") &&
