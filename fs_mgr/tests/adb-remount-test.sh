...
adb_su ls /sys/module/overlay/parameters/override_creds ||
...
if adb_sh ls -d /data/overlay >/dev/null 2>&1; then
...
adb_reboot &&
  ...
  echo "[ WARNING ] overlay takeover before remount" >&2
adb_root &&
  adb_wait &&
  adb remount &&
  adb_sh df -k | head -1 &&
...
B="`adb_su cat /vendor/hello 2>&1 | tr -d '^M'`" ||
