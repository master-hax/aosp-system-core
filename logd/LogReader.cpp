...
bool LogReader::onDataAvailable(SocketClient* cli) {
    ...
    if (cp) {
        ...
        timeout = std::chrono::steady_clock::now() + std::chrono::seconds(timeout_seconds);
    }
    ...
    android::prdebug(
            ...
            (int)pid, start.nsec(), timeout.time_since_epoch().count());
    ...
    auto entry = std::make_unique<LogReaderThread>(*this, *reader_list_, cli, nonBlock, tail,
                                                   logMask, pid, start, sequence, timeout,
