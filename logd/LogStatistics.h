...
class PidStatistics {
    ...
    char *name;
    bool mGone;
    // setonly, once a multiple, always a multiple.
    bool mMultiple;

    PidStatisticsGoneCollection gonePids;

    PidStatisticsGoneCollection::iterator begin() { return gonePids.begin(); }
    PidStatisticsGoneCollection::iterator end() { return gonePids.end(); }
    PidStatisticsGoneCollection::iterator
        erase(PidStatisticsGoneCollection::iterator i)
            { return gonePids.erase(i); }
    void push_back(PidStatisticsGone &p)
        { mMultiple = true; gonePids.push_back(p); }
    PidStatisticsGone *findMatch(pid_t pid);
...
public:
    ...
    PidStatisticsGone &operator= (const PidStatisticsGone &copy) {
        pid = copy.pid;
