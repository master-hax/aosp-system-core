...
class LogHashtable : public std::unordered_map<TKey, TEntry> {
...
public:
    ...
    inline iterator add(TKey key, LogBufferElement *e) {
        ...
        if (it == this->end()) {
            it = this->insert(std::make_pair(key, TEntry(e))).first;
        } else {
            it->second.add(e);
        }
        return it;
    ...
    inline iterator add(TKey key) {
        iterator it = this->find(key);
        if (it == this->end()) {
            it = this->insert(std::make_pair(key, TEntry(key))).first;
        } else {
            it->second.add(key);
        }
        return it;
