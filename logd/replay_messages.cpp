...
class PrintInteresting : public Operation {
  ...
  private:
    ...
    LogStatistics stats_simple_{false, true};
    SimpleLogBuffer simple_log_buffer_{&reader_list_, &tags_, &stats_simple_};

    LogStatistics stats_chatty_{false, true};
  ...
  public:
    SingleBufferOperation(const char* buffer) {
        if (!strcmp(buffer, "simple")) {
            ...
            log_buffer_.reset(new ChattyLogBuffer(&reader_list_, &tags_, &prune_list_, &stats_));
