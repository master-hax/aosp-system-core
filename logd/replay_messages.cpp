...
class SingleBufferOperation : public Operation {
  public:
    SingleBufferOperation(const char* buffer) {
        if (!strcmp(buffer, "simple")) {
            stats_.reset(new LogStatistics{false, false});
            ...
            stats_.reset(new LogStatistics{false, true});
...
class PrintMemory : public SingleBufferOperation {
  public:
    PrintMemory(const char* buffer) : SingleBufferOperation(buffer) {}
...
class PrintLogs : public SingleBufferOperation {
  public:
    PrintLogs(const char* buffer, const char* buffers, const char* print_point)
        : SingleBufferOperation(buffer) {
        if (buffers != nullptr) {
...
class PrintLatency : public SingleBufferOperation {
  public:
    PrintLatency(const char* buffer) : SingleBufferOperation(buffer) {}
...
int main(int argc, char** argv) {
    ...
    if (!strcmp(argv[2], "interesting")) {
        operation.reset(new PrintInteresting());
    } else if (!strcmp(argv[2], "memory_usage")) {
        operation.reset(new PrintMemory(argv[3]));
    } else if (!strcmp(argv[2], "latency")) {
        operation.reset(new PrintLatency(argv[3]));
    } else if (!strcmp(argv[2], "print_logs")) {
        operation.reset(
                new PrintLogs(argv[3], argc > 4 ? argv[4] : nullptr, argc > 5 ? argv[5] : nullptr));
    } else if (!strcmp(argv[2], "nothing")) {
        operation.reset(new SingleBufferOperation(argv[3]));
    } else {
