...
public:
    ...
    bool merge(LogBufferElement *element, unsigned short dropped) {
...
void LogBuffer::prune(log_id_t id, unsigned long pruneRows, uid_t caller_uid) {
    ...
    while (pruneRows > 0) {
        ...
        while (it != mLogElements.end()) {
            ...
            // merge any drops
            if (dropped && last.merge(element, dropped)) {
                it = erase(it, false);
                continue;
            }
