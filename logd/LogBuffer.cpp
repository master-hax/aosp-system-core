...
int LogBuffer::log(log_id_t log_id, log_time realtime, log_time monotonic,
...
void LogBuffer::convertIfMonotonicToReal(log_time &start) {
    ...
    //
    // Realtime tasks have a processing time in the range of 0-20ms, maybe more
    // if under a lot of load. We are either in 1970ish (MONOTONIC) or 2015+ish
    // (REALTIME) so to differentiate without prejudice, we add an excessively
    // large value (10 seconds) and compare to see if the time is less or
    // greater to ask the question if the time is Monotonic or Real.
    //
    monotonic.tv_sec += 10;
    if (start > monotonic) {
        return;
        ...
        static bool callback(const LogBufferElement *element, void *obj) {
            ...
            if (!me->startTimeSet) {
                if (me->start == element->getMonotonicTime()) {
                    ...
                    //
                    // - Require one element before, and one after.
                    // - The closest one before (assumption, within ns)
                    //   is our best bet.
                    //
    ...
    if (!logFindStart.found()) {
        //
        // None found, use the current difference (assumption, processing
        // within a few ms, less than ns of drift).
        //
