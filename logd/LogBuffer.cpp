...
// the result being to set an internal busy flag indicating the prune operation
// could not be completed because a reader is blocking the request.
...
bool LogBuffer::prune(log_id_t id, unsigned long pruneRows, uid_t caller_uid) {
    ...
    while (!clearAll && (pruneRows > 0)) {
        ...
        while (it != mLogElements.end()) {
            ...
            if (oldest && (watermark <= element->getRealTime())) {
                // Do not let chatty eliding to trigger any reader mitigation
                break;
            }
