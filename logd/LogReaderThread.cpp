...
void LogReaderThread::ThreadFunction() {
    ...
    while (!release_) {
        if (deadline_.time_since_epoch().count() != 0) {
            ...
            ts.tv_nsec = ns.count();
            if (pthread_cond_timedwait(&thread_triggered_condition_, LogBufferLock.native_handle(),
                                       &ts) == ETIMEDOUT) {
                deadline_ = {};
            }
            if (release_) {
        ...
        if (tail_) {
            ...
            log_buffer_->FlushTo(writer_.get(), *first_pass_state,
                                 [this](log_id_t log_id, pid_t pid, uint64_t sequence,
                                        log_time realtime) REQUIRES(LogBufferLock) {
        ...
        if (deadline_.time_since_epoch().count() == 0) {
            pthread_cond_wait(&thread_triggered_condition_, LogBufferLock.native_handle());
        }
    }
