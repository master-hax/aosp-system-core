...
LogReaderThread::LogReaderThread(LogReader& reader, LogReaderList& reader_list,
                                 ...
                                 uint64_t start, std::chrono::steady_clock::time_point timeout,
...
void LogReaderThread::ThreadFunction() {
    ...
    LogBufferInterface& logbuf = *reader_.log_buffer();
    ...
    while (!release_) {
        if (timeout_.time_since_epoch().count() != 0) {
            if (thread_triggered_condition_.wait_until(lock, timeout_) == std::cv_status::timeout) {
                timeout_ = {};
            }
            if (release_) {
                break;
