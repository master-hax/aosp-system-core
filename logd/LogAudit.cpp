...
int LogAudit::logPrint(const char *fmt, ...) {
    ...
    char *pidptr = strstr(str, " pid=");
    if (pidptr && isdigit(pidptr[5])) {
        tid = pid = atoi(pidptr + 5);
        uid = logbuf->pid_to_uid(pid);
    }

    char *comm = strstr(str, " comm=\"");
    if (comm) {
        comm += 7;
        ...
        pid = tid;
        comm = strdup("auditd");
    } else if (!(comm = const_cast<char *>(logbuf->pid_to_name(pid)))) {
        comm = strdup("unknown");
    }
