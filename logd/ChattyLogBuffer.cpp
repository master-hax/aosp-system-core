...
int ChattyLogBuffer::Log(log_id_t log_id, log_time realtime, uid_t uid, pid_t pid, pid_t tid,
    ...
    if (match == DIFFERENT) {
        if (duplicate_elements_[log_id]) {
            ...
            // If we previously had more than 1 duplicate message, log the chatty message.
            if (dropped_element && dropped_element->getDropped() > 0) {
                Log(std::move(dropped_element));
            }
            // Log the saved copy of the last duplicate message seen.
    ...
    // First duplicate message: set duplicate_elements_ appropriately.
    if (!duplicate_elements_[log_id]) {
    ...
    stats_->AddTotal(log_id, len);
    // Subsequent duplicate LIBLOG event messages: coalesce them into last_logged_elements_.
    ...
    if (dropped_count == std::numeric_limits<uint16_t>::max()) {
        ...
        current_last->setDropped(1);
    } else {
        // Use current_last here to always get the latest timestamp.
