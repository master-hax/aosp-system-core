...
class SimpleLogBuffer : public LogBuffer {
  ...
  protected:
    virtual bool Prune(log_id_t id, unsigned long prune_rows, uid_t uid) REQUIRES(LogBufferLock);
    virtual void LogInternal(LogBufferElement&& elem) REQUIRES(LogBufferLock);

    // Returns an iterator to the oldest element for a given log type, or logs_.end() if
    // there are no logs for the given log type. Requires logs_LogBufferLock to be held.
    std::list<LogBufferElement>::iterator GetOldest(log_id_t log_id) REQUIRES(LogBufferLock);
    ...
    void KickReader(LogReaderThread* reader, log_id_t id, unsigned long prune_rows)
            REQUIRES_SHARED(LogBufferLock);

    LogStatistics* stats() { return stats_; }
    LogReaderList* reader_list() { return reader_list_; }
    size_t max_size(log_id_t id) REQUIRES_SHARED(LogBufferLock) { return max_size_[id]; }
    std::list<LogBufferElement>& logs() { return logs_; }

  private:
    bool ShouldLog(log_id_t log_id, const char* msg, uint16_t len);
    void MaybePrune(log_id_t id) REQUIRES(LogBufferLock);
    ...
    std::list<LogBufferElement> logs_ GUARDED_BY(LogBufferLock);
    // Keeps track of the iterator to the oldest log message of a given log type, as an
    // optimization when pruning logs.  Use GetOldest() to retrieve.
    std::optional<std::list<LogBufferElement>::iterator> oldest_[LOG_ID_MAX] GUARDED_BY(
            LogBufferLock);
};
