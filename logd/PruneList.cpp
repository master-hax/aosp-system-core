...
std::string PruneList::Format() const {
    ...
    for (const auto& rule : prune_last_) {
        prune_rules.emplace_back(rule.Format());
    }
    for (const auto& rule : prune_first_) {
        prune_rules.emplace_back("~" + rule.Format());
    }
    return android::base::Join(prune_rules, " ");
}

bool PruneList::PruneFirst(LogBufferElement* element) const {
    for (const auto& rule : prune_first_) {
...
bool PruneList::PruneLast(LogBufferElement* element) const {
    for (const auto& rule : prune_last_) {
        if (rule.Matches(element)) {
            return true;
        }
