...
template <typename F>
int LogIdCommand(SocketClient* cli, int argc, char** argv, F function) {
    setname();
    if (argc < 2) {
    ...
    log_id_t log_id;
    if (!android::base::ParseUint(argv[1], &log_id, LOG_ID_KERNEL)) {
...
template <typename F>
int LogSizeCommand(SocketClient* cli, int argc, char** argv, F size_function) {
    return LogIdCommand(cli, argc, argv, [&](log_id_t log_id) {
        char buf[512];
        snprintf(buf, sizeof(buf), "%zu", size_function(log_id));
        cli->sendMsg(buf);
    });
}
