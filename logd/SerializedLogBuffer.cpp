...
// the list.  This is done in a thread to prevent unnecessarily slowing down the Prune() function.
...
bool SerializedLogBuffer::Prune(log_id_t log_id, size_t bytes_to_free, uid_t uid) {
    ...
    auto prune_chunks = android::base::make_scope_guard([&chunks_to_prune, log_id, this] {
        ...
        // We must inform reader threads that we've deleted logs, since they reference them.
        for (const auto& reader_thread : reader_list_->reader_threads()) {
            auto& state =
                    reinterpret_cast<SerializedFlushToState&>(reader_thread->flush_to_state());
            state.AfterClear(log_id);
        }
    });
    ...
    while (it != log_buffer.end()) {
        ...
        if (uid != 0) {
            ...
            if (it_to_prune->ClearUidLogs(uid)) {
