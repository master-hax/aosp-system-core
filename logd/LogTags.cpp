...
// Checks for file corruption, and report true false if there was no need
// to rebuild the referenced file.  Failure to rebuild is only logged, no
// return value.
...
// special pmsg events for log tags, and build up our internal
...
// Converts an events tag into a name
...
// converts an events tag into a format
...
// Caller must perform locks, can be under reader (for pre-check) or writer
// lock We use this call to invent a new deterministically random tag, unique
// is cleared if no conflicts. if format is NULL, we are in readonly mode.
...
std::string LogTags::formatEntry_locked(uint32_t tag, uid_t uid,
    ...
    // Show all, one for each registered uid (we are groot)
    std::string ret;
    for (auto &it : ut->second) {
