...
class LogBufferElement {
    ...
    union u {
        u(unsigned short len):msgLen(len) { }
        const unsigned short msgLen; // mMSg != NULL
        unsigned short dropped;      // mMsg == NULL
    } u;
...
public:
    ...
    unsigned short getDropped(void) const { return mMsg ? 0 : u.dropped; }
    unsigned short setDropped(unsigned short value) {
        if (mMsg) {
            free(mMsg);
            mMsg = NULL;
        }
        return u.dropped = value;
    }
    unsigned short getMsgLen() const { return mMsg ? u.msgLen : 0; }
