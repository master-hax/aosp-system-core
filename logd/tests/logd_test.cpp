...
TEST(logd, statistics) {
    ...
    *cp = '\0';
    ...
    EXPECT_TRUE(NULL != log_events_per_span);
...
TEST(logd, benchmark) {
    ...
    if (benchmark_already_run) {
        fprintf(stderr, "WARNING: spam already present and too much history\n"
                        "         false OK prune by worst UID check\n");
    }
...
#ifdef TARGET_USES_LOGD
    EXPECT_GE(100000, ns[log_maximum_retry]); // 42777 user
#else
    EXPECT_GE(10000, ns[log_maximum_retry]); // 5636 kernel
#endif

#ifdef TARGET_USES_LOGD
    EXPECT_GE(25000, ns[log_maximum]); // 14055 user
#else
    EXPECT_GE(10000, ns[log_maximum]); // 5637 kernel
#endif

    EXPECT_GE(4000, ns[clock_overhead]); // 2008

    EXPECT_NE(0, ns[3]);
#ifdef TARGET_USES_LOGD
    EXPECT_GE(250000, ns[log_overhead]); // 113219 user
#else
    EXPECT_GE(100000, ns[log_overhead]); // 50945 kernel
#endif

#ifdef TARGET_USES_LOGD
    EXPECT_GE(7500, ns[log_latency]); // 3718 user space
#else
    EXPECT_GE(500000, ns[log_latency]); // 254200 kernel
#endif

#ifdef TARGET_USES_LOGD
    EXPECT_GE(20000000, ns[log_delay]); // 9542541 user
#else
    EXPECT_GE(55000, ns[log_delay]); // 27341 kernel
#endif
