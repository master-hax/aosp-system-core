...
class PruneList {
  public:
    ...
    bool PruneFirst(LogBufferElement* element) const;
    bool PruneLast(LogBufferElement* element) const;

    bool HasPruneFirst() const { return !prune_first_.empty(); }
    bool HasPruneLast() const { return !prune_last_.empty(); }
  ...
  private:
    std::list<Prune> prune_first_;
    std::list<Prune> prune_last_;
