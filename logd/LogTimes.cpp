...
ok:
    if (!me->skipAhead[id]) {
        ...
        if (id == LOG_ID_EVENTS) {
            ...
            hdrLen = element->getMsgLen();
            hdrLen = strnlen(msg + 1,
                             min(LOGGER_ENTRY_MAX_PAYLOAD, hdrLen) - 2 - len)
                   + 2;
        }
        ...
        if (id == LOG_ID_EVENTS) {
            ...
            strncpy(entry->msg.message + 1, msg + 1, hdrLen - 1);
            entry->msg.message[hdrLen - 1] = '\0';
        }
