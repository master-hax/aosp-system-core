...
void LogStatistics::add(LogBufferElement *e) {
    ...
    if (e->getUid() == AID_SYSTEM) {
        pidSystemTable[log_id].add(e->getPid(), e);
    }
...
void LogStatistics::subtract(LogBufferElement *e) {
    ...
    if (e->getUid() == AID_SYSTEM) {
        pidSystemTable[log_id].subtract(e->getPid(), e);
    }
...
void LogStatistics::drop(LogBufferElement *e) {
    ...
    if (e->getUid() == AID_SYSTEM) {
        pidSystemTable[log_id].drop(e->getPid(), e);
    }
...
std::string UidEntry::format(const LogStatistics &stat, log_id_t id) const {
    ...
    if (worstUidEnabledForLogid(id)) {
        ...
        size_t totalVirtualSize = sizes + totalDropped * totalSize
                                / totalElements;
        size_t entrySize = getSizes();
        int realPermille = entrySize * 1000 / sizes;
        size_t virtualEntrySize = entrySize;
        size_t dropped = getDropped();
        if (dropped) {
            pruned = android::base::StringPrintf("%zu", dropped);
            virtualEntrySize += dropped * totalSize / totalElements;
        }
    ...
    std::unique_ptr<const PidEntry *[]> sorted
        = stat.pidSystemTable[id].sort(maximum_sorted_entries);
...
std::string PidEntry::format(const LogStatistics &stat, log_id_t /* id */) const {
    ...
    if (n) {
        ...
        const char *un = stat.uidToName(u);
        if (un) {
            name += android::base::StringPrintf(
                "%*s%s", (int)std::max(12 - name.length(), (size_t)1),
                "", un);
            free(const_cast<char *>(un));
        }
    }
...
const char *LogStatistics::pidToName(pid_t pid) const {
    ...
    pidTable_t &p = const_cast<pidTable_t &>(pidTable);
    const char *name = p.add(pid)->second.getName();
    if (!name) {
        return NULL;
    }
    return strdup(name);
