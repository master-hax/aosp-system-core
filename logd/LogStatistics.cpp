...
void LogStatistics::format(char **buf, uid_t uid, unsigned int logMask) {
    ...
    log_id_for_each(id) {
        ...
        const UidEntry **sorted = NULL;
        ssize_t index = -1;
        bool print = false;
        const UidEntry *entry;

        while ((entry = uidTable[id].next(maximum_sorted_entries, index, sorted))) {
    ...
    if (enable) {
        const PidEntry **sorted = NULL;
        ssize_t index = -1;
        bool print = false;
        const PidEntry *entry;

        while ((entry = pidTable.next(maximum_sorted_entries, index, sorted))) {
