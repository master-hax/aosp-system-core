...
static void format_line(std::string &output,
        std::string &name, std::string &size, std::string &pruned) {
    ...
    if (pruned.length()) {
        output += android::base::StringPrintf("%s%*s%*s\n", name.c_str(),
                                              (int)size_len, size.c_str(),
                                              (int)drop_len, pruned.c_str());
    } else {
        output += android::base::StringPrintf("%s%*s\n", name.c_str(),
                                              (int)size_len, size.c_str());
    }
}
    ...
    if (enable) {
        ...
        while ((index = pidTable.next(index, sorted, maximum_sorted_entries)) >= 0) {
            ...
            if (!headerPrinted) {
                ...
                name.assign("  PID/UID   COMMAND LINE");
                size.assign("BYTES");
                pruned.assign("LINES");
                format_line(output, name, size, pruned);
