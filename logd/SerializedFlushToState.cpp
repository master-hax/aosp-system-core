...
void SerializedFlushToState::AfterClear(log_id_t log_id) {
    // If we're not watching this log_id, ignore.
    if (((1 << log_id) & log_mask()) == 0) {
        return;
    }

    // If we don't have a position for this log, ignore.
    if (!log_positions_[log_id].has_value()) {
        return;
    }
