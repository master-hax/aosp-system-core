...
void SerializedFlushToState::CreateLogPosition(log_id_t log_id) {
    ...
    int read_offset = 0;
    while (read_offset < it->write_position()) {
        const auto* log_message =
                reinterpret_cast<const SerializedLogEntry*>(it->data() + read_offset);
        if (log_message->sequence() >= start()) {
            break;
        }
        read_offset += log_message->total_len();
    }
