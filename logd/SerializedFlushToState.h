...
struct MinHeapElement {
    MinHeapElement(log_id_t log_id, const SerializedLogEntry* element)
        : log_id(log_id), element(element) {}
    log_id_t log_id;
    const SerializedLogEntry* element;
    ...
    bool operator<(const MinHeapElement& rhs) const {
        return element->sequence() > rhs.element->sequence();
    }
};

class SerializedFlushToState : public FlushToState {
  public:
    // Initializes this state object by creating a log_positions_ object then calling
    // AddMinHeapEntry() for each log buffer in log_mask.
  ...
  private:
    ...
    std::list<SerializedLogChunk>* logs_;
    ...
    // A bit for each log that is set if we've read all logs for a given log_id, in other words
    // buffer_it == std::prev(logs_.end) && next_log_position == logs_write_position_.  This
    // includes the situation where there are no logs in the buffer and we are waiting for the first
    // one. These will be re-checked in each loop in case new logs came in.
