  ...
  protected:
    ...
    void TestReading(uint64_t start, LogMask log_mask, SerializedFlushToState& state) {
        ...
        while (state.HasUnreadLogs()) {
            ...
            read_sequence.emplace_back(top.element->sequence());
        }
    ...
    // numbers for future validation.  Optionally decrement the reference count.
    void AddChunkWithMessages(int buffer, bool dec_reference_count,
...
// 0: multiple chunks, with variable number of elements, with/without reference counts.
// 1: 1 chunk with 1 log and 0 ref count
// 2: 1 chunk with 1 log and >0 ref count
// 3: 1 chunk with 0 logs and >0 ref count
// 4: 1 chunk with 0 logs and 0 ref count (impossible, but SerializedFlushToState can handle it)
// 5-7: 0 chunks
