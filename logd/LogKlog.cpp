...
void LogKlog::synchronize(const char *buf) {
    char *cp = strstr(buf, "] PM: suspend e");
    if (!cp) {
        return;
    }
...
void LogKlog::calculate_correction(const log_time &monotonic,
...
int LogKlog::log(const char *buf) {
    if (auditd && !synchronizing && strstr(buf, " audit(")) {
        return 0;
    }
    ...
    if ((cp = now.strptime(buf, "[ %s.%q]"))) {
        ...
        if (!strncmp(cp, suspend, sizeof(suspend) - 1)) {
            calculate_correction(now, cp + sizeof(suspend) - 1);
        } else if (!strncmp(cp, resume, sizeof(resume) - 1)) {
            calculate_correction(now, cp + sizeof(resume) - 1);
    ...
    // beautify the kernel logs into an Android Logging format.
    while (isspace(*buf)) {
        ++buf;
    }
    if (!*buf) {
