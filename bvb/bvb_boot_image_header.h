...
#ifndef _BVB_BOOT_IMAGE_HEADER_H
#define _BVB_BOOT_IMAGE_HEADER_H
...
/* The current MAJOR and MINOR versions used - keep in sync with the bvb tool */
 ...
 * for a RSA-2048 key. By definition, a RSA public key is the key pair
 * (n, e) where |n| is the modulus and has approximately
 * |key_num_bits| bits and |e| is the public exponent. The exponent is
 * not stored since it's assumed to always be 65537.
 *
 * To optimize verification, the key block includes two precomputed
 * values, |n0inv| (fits in 32 bits) and |rr| (approximately same size
 * as |n|).
 ...
 * follow by a NUL byte and then enough padding to make the combined
 ...
 * the boot partition. See bvb_property_lookup() and
...
typedef struct BvbBootImageHeader {
  ...
  /* 144: Offset into the "Payload data" block of device tree. */
  uint64_t device_tree_offset;
  /* 152: Length of the device tree. */
  uint64_t device_tree_size;

  /* 160: Physical kernel load address. */
  uint64_t kernel_addr;

  /* 168: Physical initial ramdisk load address. */
  uint64_t initrd_addr;

  /* 176: The NUL-terminated kernel command-line string, passed to the
   * Linux kernel.
   *
  ...
  /* 4272: Padding to ensure struct is size BVB_BOOT_IMAGE_HEADER_SIZE
   * bytes. This must be set to zeroes.
   */
  uint8_t reserved[3920];
...
#endif  /* _BVB_BOOT_IMAGE_HEADER_H */
