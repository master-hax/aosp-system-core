...
Key* parse_key_data(const uint8_t* data, size_t length)
{
  ...
  bvb_rsa_public_key_header_to_host_byte_order((BvbRSAPublicKeyHeader *) data, &h);
  ...
  // allocation.
  key = (Key *) (bvb_malloc(sizeof(Key) + h.key_num_bits/32*4*2));
  if (key == NULL)
    goto fail;

  key->len = h.key_num_bits/32;
  key->n0inv = h.n0inv;
  key->n = (uint32_t *) (((uint8_t *) key) + sizeof(Key));
  key->rr = (uint32_t *) (((uint8_t *) key) + sizeof(Key) + key->len*4);
                  ...
                  const uint8_t* padding, size_t padding_num_bytes)
{
  ...
  /* Check hash. */
  if (bvb_safe_memcmp(buf + padding_num_bytes, hash, sig_num_bytes - padding_num_bytes)) {
