                                      ...
                                      size_t* out_public_key_length)
{
  ...
  // Ensure kernel, initramfs, and device_tree are entirely in the
  // Payload data block.
  ...
  if (h->device_tree_size > 0 &&
      h->device_tree_offset + h->device_tree_size > h->payload_data_block_size) {
    BVB_DEBUG(("Device tree is not entirely in its block.\n"));
    goto out;
  }
