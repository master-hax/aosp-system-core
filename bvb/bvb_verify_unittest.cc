...
TEST_F(VerifyTest, DeviceTreeOutOfBounds)
{
  GenerateBootImage("SHA256_RSA2048", "", 0,
                    base::FilePath("test/testkey_rsa2048.pem"));
  ...
  // Check we catch when device tree data goes out of bounds.
  h->device_tree_offset = htobe64(4);
  h->device_tree_size = htobe64(be64toh(h->payload_data_block_size));
  EXPECT_EQ(BVB_VERIFY_RESULT_INVALID_BOOT_IMAGE_HEADER,
            bvb_verify_boot_image(boot_image_.data(), boot_image_.size(),
                                  NULL, NULL));
}
