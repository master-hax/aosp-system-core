...
TEST(UtilTest, BootImageHeaderByteswap)
{
  ...
  h.device_tree_offset = htobe64(n); n++;
  h.device_tree_size = htobe64(n); n++;
  ...
  EXPECT_EQ(n, s.device_tree_offset); n++;
  EXPECT_EQ(n, s.device_tree_size); n++;
  ...
  static_assert(offsetof(BvbBootImageHeader, reserved) == 4272,
