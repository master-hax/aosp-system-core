...
{
  const BvbBootImageHeader *h = NULL;
  const char* ret = NULL;
  ...
  // Careful, not byteswapped.
  h = (const BvbBootImageHeader *) image_data;
  image_end = image_data + image_size;

  prop_start = image_data + sizeof(BvbBootImageHeader) +
      bvb_be64toh(h->authentication_data_block_size) +
      bvb_be64toh(h->properties_offset);

  prop_end = prop_start + bvb_be64toh(h->properties_size);

  if (prop_start > image_end || prop_end > image_end) {
  ...
  for (p = prop_start; p < prop_end; ) {
    ...
    uint64_t total = sizeof(BvbPropertyHeader) + 2 + key_nb + value_nb;
    uint64_t remainder = total&7;
