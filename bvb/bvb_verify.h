...
#ifndef _BVB_VERIFY_H
#define _BVB_VERIFY_H
...
/*
 * Checks that raw boot image at |data| of and size |length| is a
 * valid Brillo boot image. The complete contents of the boot image
 * must be passed in.
 *
 * See the |BvbBootImageHeader| struct for information about the the
 ...
 * |out_public_key_data|, if non-NULL, will be set to the length of
 * the public key data.
 *
 ...
 *      is bigger than |rollback_index|. You must also update the the
